...
;; Function-Specs test_crate::rc_clone
(declare-fun ens%test_crate!rc_clone. (alloc!vec.Vec<u8./allocator_global%.>. alloc!vec.Vec<u8./allocator_global%.>.)
 Bool
)
(assert
 (forall ((rc! alloc!vec.Vec<u8./allocator_global%.>.) (res! alloc!vec.Vec<u8./allocator_global%.>.))
  (!
   (= (ens%test_crate!rc_clone. rc! res!) (= (vstd!view.View.view.? $ (TYPE%alloc!vec.Vec.
       $ (UINT 8) $ ALLOCATOR_GLOBAL
   ...
   :pattern ((ens%test_crate!rc_clone. rc! res!))
   :qid internal_ens__test_crate!rc_clone._definition
   :skolemid skolem_internal_ens__test_crate!rc_clone._definition
)))
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/regression-491396d30b1dad7a-zulip_external_body_clone_regression_800_1/test.rs:24:1: 24:34 (#0)
(get-info :all-statistics)
(push)
 (declare-const rc! alloc!vec.Vec<u8./allocator_global%.>.)
 (declare-const tmp%1 Bool)
 (declare-const tmp@ alloc!vec.Vec<u8./allocator_global%.>.)
 (assert
  fuel_defaults
 )
 ;; assertion failed
 ...
 (assert
  (not (=>
    (ens%test_crate!rc_clone. rc! tmp@)
    (=>
