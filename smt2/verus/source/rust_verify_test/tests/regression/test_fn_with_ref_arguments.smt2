...
;; Datatypes
(declare-datatypes ((test_crate!X. 0) (test_crate!Y. 0) (tuple%2. 0)) (((test_crate!X./X
    (test_crate!X./X/?v Int)
   )
  ) ((test_crate!Y./Y (test_crate!Y./Y/?w Int))) ((tuple%2./tuple%2 (tuple%2./tuple%2/?0
     Poly
    ) (tuple%2./tuple%2/?1 Poly)
))))
(declare-fun test_crate!X./X/v (test_crate!X.) Int)
(declare-fun test_crate!Y./Y/w (test_crate!Y.) Int)
...
(assert
 (forall ((_w! Int)) (!
   (=>
    (uInv 64 _w!)
    (has_type (Poly%test_crate!Y. (test_crate!Y./Y _w!)) TYPE%test_crate!Y.)
   )
   :pattern ((has_type (Poly%test_crate!Y. (test_crate!Y./Y _w!)) TYPE%test_crate!Y.))
   :qid internal_test_crate!Y./Y_constructor_definition
   :skolemid skolem_internal_test_crate!Y./Y_constructor_definition
)))
(assert
 (forall ((x test_crate!Y.)) (!
   (= (test_crate!Y./Y/w x) (test_crate!Y./Y/?w x))
   :pattern ((test_crate!Y./Y/w x))
   :qid internal_test_crate!Y./Y/w_accessor_definition
   :skolemid skolem_internal_test_crate!Y./Y/w_accessor_definition
)))
(assert
 (forall ((x Poly)) (!
   (=>
    (has_type x TYPE%test_crate!Y.)
    (uInv 64 (test_crate!Y./Y/w (%Poly%test_crate!Y. x)))
   )
   :pattern ((test_crate!Y./Y/w (%Poly%test_crate!Y. x)) (has_type x TYPE%test_crate!Y.))
   :qid internal_test_crate!Y./Y/w_invariant_definition
   :skolemid skolem_internal_test_crate!Y./Y/w_invariant_definition
)))
(assert
 (forall ((x tuple%2.)) (!
...
(declare-fun req%test_crate!test. (Dcr Type Poly test_crate!X. test_crate!Y.) Bool)
(declare-const %%global_location_label%%0 Bool)
(assert
 (forall ((F&. Dcr) (F& Type) (f! Poly) (x! test_crate!X.) (y! test_crate!Y.)) (!
   (= (req%test_crate!test. F&. F& f! x! y!) (=>
     %%global_location_label%%0
     (closure_req F& $ (TYPE%tuple%2. (REF $) TYPE%test_crate!X. (REF $) TYPE%test_crate!Y.)
      f! (Poly%tuple%2. (tuple%2./tuple%2 (Poly%test_crate!X. x!) (Poly%test_crate!Y. y!)))
   )))
   :pattern ((req%test_crate!test. F&. F& f! x! y!))
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/regression-491396d30b1dad7a-test_fn_with_ref_arguments/test.rs:20:1: 20:57 (#0)
...
(push)
 ...
 (assert
  (has_type (Poly%test_crate!Y. y!) TYPE%test_crate!Y.)
 )
 (assert
  (closure_req F& $ (TYPE%tuple%2. (REF $) TYPE%test_crate!X. (REF $) TYPE%test_crate!Y.)
   f! (Poly%tuple%2. (tuple%2./tuple%2 (Poly%test_crate!X. x!) (Poly%test_crate!Y. y!)))
 ...
 (assert
  (not (=>
    (= tmp%2 (tuple%2./tuple%2 (Poly%test_crate!X. x!) (Poly%test_crate!Y. y!)))
    (=>
     %%location_label%%0
     (req%vstd!pervasive.exec_nonstatic_call. $ (TYPE%tuple%2. (REF $) TYPE%test_crate!X.
       (REF $) TYPE%test_crate!Y.
      ) $ BOOL (REF F&.) F& f! (Poly%tuple%2. tmp%2)
 )))))
 (get-info :version)
