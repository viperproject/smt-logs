...
(assert
 (distinct fuel%vstd!array.array_view. fuel%vstd!array.impl&%0.view. fuel%vstd!array.impl&%2.spec_index.
  ...
  fuel%vstd!seq.axiom_seq_add_index1. fuel%vstd!seq.axiom_seq_add_index2. fuel%vstd!view.impl&%0.view.
...
;; Function-Specs test_crate::seq_bad
(declare-fun ens%test_crate!seq_bad. (Int) Bool)
(assert
 (forall ((no%param Int)) (!
   (= (ens%test_crate!seq_bad. no%param) false)
   :pattern ((ens%test_crate!seq_bad. no%param))
   :qid internal_ens__test_crate!seq_bad._definition
   :skolemid skolem_internal_ens__test_crate!seq_bad._definition
)))

;; Function-Def test_crate::seq_bad
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/regression-491396d30b1dad7a-seq_add_axiom_issue990/test.rs:16:7: 16:19 (#0)
...
(push)
 ...
 (declare-const s1_2@ vstd!seq.Seq<int.>.)
 ...
 (assert
  (not (=>
    (= s1@ (%Poly%vstd!seq.Seq<int.>. (vstd!seq.Seq.push.? $ INT (vstd!seq.Seq.empty.? $
        INT
       ) (I 1)
    )))
    (=>
     (= s1_2@ (%Poly%vstd!seq.Seq<int.>. (vstd!view.View.view.? $ (ARRAY $ INT $ (CONST_INT
          2
         )
        ) (array_new $ INT 2 (%%array%%0 (I 1) (I 2)))
     )))
     (=>
      (= s2@ (%Poly%vstd!seq.Seq<int.>. (vstd!seq.Seq.empty.? $ INT)))
      (=>
       (= tmp%1 (= (vstd!seq.Seq.index.? $ INT (vstd!seq.Seq.add.? $ INT (Poly%vstd!seq.Seq<int.>.
            s1@
           ) (Poly%vstd!seq.Seq<int.>. s2@)
          ) (I 0)
         ) (vstd!seq.Seq.index.? $ INT (Poly%vstd!seq.Seq<int.>. s2@) (I (Sub 0 1)))
       ))
       (and
        (=>
         %%location_label%%0
        ...
        (=>
         tmp%1
         (=>
          (= tmp%2 (= (vstd!seq.Seq.index.? $ INT (vstd!seq.Seq.add.? $ INT (Poly%vstd!seq.Seq<int.>.
               s1_2@
              ) (Poly%vstd!seq.Seq<int.>. s2@)
             ) (I 1)
            ) (vstd!seq.Seq.index.? $ INT (Poly%vstd!seq.Seq<int.>. s2@) (I (Sub 0 1)))
...
;; Function-Recommends test_crate::seq_bad
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/regression-491396d30b1dad7a-seq_add_axiom_issue990/test.rs:16:7: 16:19 (#0)
(get-info :all-statistics)
(push)
 (declare-const no%param Int)
  ...
  (not (=>
    ...
    (=>
     (= s1_2@ (%Poly%vstd!seq.Seq<int.>. (vstd!view.View.view.? $ (ARRAY $ INT $ (CONST_INT
        ...
        ) (array_new $ INT 2 (%%array%%1 (I 1) (I 2)))
     )))
     (=>
      (= s2@ (%Poly%vstd!seq.Seq<int.>. (vstd!seq.Seq.empty.? $ INT)))
      (=>
       (= tmp%2 (vstd!seq.Seq.add.? $ INT (Poly%vstd!seq.Seq<int.>. s1@) (Poly%vstd!seq.Seq<int.>.
          s2@
       )))
       (and
        (=>
         %%location_label%%0
         (req%vstd!seq.impl&%0.spec_index. $ INT tmp%2 (I 0))
        )
        (=>
         (= tmp%1 (vstd!seq.Seq.index.? $ INT (vstd!seq.Seq.add.? $ INT (Poly%vstd!seq.Seq<int.>.
             s1@
         ...
         (=>
          ...
          (and
           (=>
            %%location_label%%1
            (req%vstd!seq.impl&%0.spec_index. $ INT (Poly%vstd!seq.Seq<int.>. s2@) tmp%4)
           )
           (=>
            (= tmp%3 (vstd!seq.Seq.index.? $ INT (Poly%vstd!seq.Seq<int.>. s2@) (I (Sub 0 1))))
            (=>
             (= tmp%1 tmp%3)
             (=>
              (= tmp%6 (vstd!seq.Seq.add.? $ INT (Poly%vstd!seq.Seq<int.>. s1_2@) (Poly%vstd!seq.Seq<int.>.
                 s2@
              )))
              (and
               (=>
                %%location_label%%2
                (req%vstd!seq.impl&%0.spec_index. $ INT tmp%6 (I 1))
               )
               (=>
                (= tmp%5 (vstd!seq.Seq.index.? $ INT (vstd!seq.Seq.add.? $ INT (Poly%vstd!seq.Seq<int.>.
                    s1_2@
                   ) (Poly%vstd!seq.Seq<int.>. s2@)
                  ) (I 1)
                ))
                (=>
                 ...
                 (=>
                  %%location_label%%3
                  (req%vstd!seq.impl&%0.spec_index. $ INT (Poly%vstd!seq.Seq<int.>. s2@) tmp%8)
 )))))))))))))))))
 (get-info :version)
 (set-option :rlimit 30000000)
 (check-sat)
