...
;; Datatypes
(declare-datatypes ((test_crate!X. 0) (tuple%1. 0)) (((test_crate!X./X (test_crate!X./X/?v
     Int
   ))
  ) ((tuple%1./tuple%1 (tuple%1./tuple%1/?0 Poly)))
))
(declare-fun test_crate!X./X/v (test_crate!X.) Int)
...
(assert
 (forall ((_v! Int)) (!
   (=>
    (uInv 64 _v!)
    (has_type (Poly%test_crate!X. (test_crate!X./X _v!)) TYPE%test_crate!X.)
   )
   :pattern ((has_type (Poly%test_crate!X. (test_crate!X./X _v!)) TYPE%test_crate!X.))
   :qid internal_test_crate!X./X_constructor_definition
   :skolemid skolem_internal_test_crate!X./X_constructor_definition
)))
(assert
 (forall ((x test_crate!X.)) (!
   (= (test_crate!X./X/v x) (test_crate!X./X/?v x))
   :pattern ((test_crate!X./X/v x))
   :qid internal_test_crate!X./X/v_accessor_definition
   :skolemid skolem_internal_test_crate!X./X/v_accessor_definition
)))
(assert
 (forall ((x Poly)) (!
   (=>
    (has_type x TYPE%test_crate!X.)
    (uInv 64 (test_crate!X./X/v (%Poly%test_crate!X. x)))
   )
   :pattern ((test_crate!X./X/v (%Poly%test_crate!X. x)) (has_type x TYPE%test_crate!X.))
   :qid internal_test_crate!X./X/v_invariant_definition
   :skolemid skolem_internal_test_crate!X./X/v_invariant_definition
)))
(assert
 (forall ((x tuple%1.)) (!
...
(declare-fun req%test_crate!test. (Dcr Type Poly test_crate!X.) Bool)
(declare-const %%global_location_label%%0 Bool)
(assert
 (forall ((F&. Dcr) (F& Type) (f! Poly) (x! test_crate!X.)) (!
   (= (req%test_crate!test. F&. F& f! x!) (=>
     %%global_location_label%%0
     (closure_req F& $ (TYPE%tuple%1. (REF $) TYPE%test_crate!X.) f! (Poly%tuple%1. (tuple%1./tuple%1
        (Poly%test_crate!X. x!)
   )))))
   :pattern ((req%test_crate!test. F&. F& f! x!))
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/regression-491396d30b1dad7a-test_fn_with_ref_arguments_1/test.rs:19:1: 19:47 (#0)
...
(push)
 ...
 (assert
  (not (=>
    (= tmp%2 (tuple%1./tuple%1 (Poly%test_crate!X. x!)))
    (=>
     %%location_label%%0
     (req%vstd!pervasive.exec_nonstatic_call. $ (TYPE%tuple%1. (REF $) TYPE%test_crate!X.)
      $ BOOL (REF F&.) F& f! (Poly%tuple%1. tmp%2)
 )))))
 (get-info :version)
