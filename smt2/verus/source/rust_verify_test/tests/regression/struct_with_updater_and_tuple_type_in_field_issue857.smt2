...
(declare-datatypes ((test_crate!S. 0) (tuple%0. 0) (tuple%2. 0)) (((test_crate!S./S (
     test_crate!S./S/?n Int
    ) (test_crate!S./S/?s vstd!set.Set<tuple%2<int./int.>.>.)
   )
  ) ((tuple%0./tuple%0)) ((tuple%2./tuple%2 (tuple%2./tuple%2/?0 Poly) (tuple%2./tuple%2/?1
     Poly
)))))
(declare-fun test_crate!S./S/n (test_crate!S.) Int)
(declare-fun test_crate!S./S/s (test_crate!S.) vstd!set.Set<tuple%2<int./int.>.>.)
...
(declare-fun Poly%vstd!set.Set<tuple%2<int./int.>.>. (vstd!set.Set<tuple%2<int./int.>.>.)
 Poly
)
...
(assert
 (forall ((x vstd!set.Set<tuple%2<int./int.>.>.)) (!
   (= x (%Poly%vstd!set.Set<tuple%2<int./int.>.>. (Poly%vstd!set.Set<tuple%2<int./int.>.>.
      x
   )))
   :pattern ((Poly%vstd!set.Set<tuple%2<int./int.>.>. x))
   :qid internal_vstd__set__Set<tuple__2<int./int.>.>_box_axiom_definition
   :skolemid skolem_internal_vstd__set__Set<tuple__2<int./int.>.>_box_axiom_definition
)))
(assert
 (forall ((x Poly)) (!
   (=>
    (has_type x (TYPE%vstd!set.Set. $ (TYPE%tuple%2. $ INT $ INT)))
    (= x (Poly%vstd!set.Set<tuple%2<int./int.>.>. (%Poly%vstd!set.Set<tuple%2<int./int.>.>.
       x
   ))))
   :pattern ((has_type x (TYPE%vstd!set.Set. $ (TYPE%tuple%2. $ INT $ INT))))
   :qid internal_vstd__set__Set<tuple__2<int./int.>.>_unbox_axiom_definition
   :skolemid skolem_internal_vstd__set__Set<tuple__2<int./int.>.>_unbox_axiom_definition
)))
(assert
 (forall ((x vstd!set.Set<tuple%2<int./int.>.>.)) (!
   (has_type (Poly%vstd!set.Set<tuple%2<int./int.>.>. x) (TYPE%vstd!set.Set. $ (TYPE%tuple%2.
      $ INT $ INT
   )))
   :pattern ((has_type (Poly%vstd!set.Set<tuple%2<int./int.>.>. x) (TYPE%vstd!set.Set.
      $ (TYPE%tuple%2. $ INT $ INT)
   )))
   :qid internal_vstd__set__Set<tuple__2<int./int.>.>_has_type_always_definition
   :skolemid skolem_internal_vstd__set__Set<tuple__2<int./int.>.>_has_type_always_definition
)))
(assert
...
(assert
 (forall ((x test_crate!S.)) (!
   (= (test_crate!S./S/n x) (test_crate!S./S/?n x))
   :pattern ((test_crate!S./S/n x))
   :qid internal_test_crate!S./S/n_accessor_definition
   :skolemid skolem_internal_test_crate!S./S/n_accessor_definition
)))
(assert
 (forall ((x test_crate!S.)) (!
...
(declare-fun test_crate!f.? (Poly Poly) Bool)
...
(assert
 (=>
  ...
  (forall ((s1! Poly) (s2! Poly)) (!
    (= (test_crate!f.? s1! s2!) (= (%Poly%test_crate!S. s2!) (test_crate!S./S (%I (I (Add (
           test_crate!S./S/n (%Poly%test_crate!S. s1!)
          ) 1
        ))
       ) (%Poly%vstd!set.Set<tuple%2<int./int.>.>. (Poly%vstd!set.Set<tuple%2<int./int.>.>.
         (test_crate!S./S/s (%Poly%test_crate!S. s1!))
    )))))
    :pattern ((test_crate!f.? s1! s2!))
    :qid internal_test_crate!f.?_definition
    :skolemid skolem_internal_test_crate!f.?_definition
))))

;; Function-Def test_crate::test
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/regression-491396d30b1dad7a-struct_with_updater_and_tuple_type_in_field_issue857/test.rs:27:1: 27:39 (#0)
(get-info :all-statistics)
(push)
 (declare-const se! vstd!set.Set<tuple%2<int./int.>.>.)
 ...
 (assert
  (not (=>
    (= s1@ (test_crate!S./S (%I (I 20)) (%Poly%vstd!set.Set<tuple%2<int./int.>.>. (Poly%vstd!set.Set<tuple%2<int./int.>.>.
        se!
    ))))
    (=>
     (= s2@ (test_crate!S./S (%I (I 21)) (%Poly%vstd!set.Set<tuple%2<int./int.>.>. (Poly%vstd!set.Set<tuple%2<int./int.>.>.
         se!
     ))))
     (=>
      (= tmp%1 (test_crate!f.? (Poly%test_crate!S. s1@) (Poly%test_crate!S. s2@)))
      (=>
       %%location_label%%0
       tmp%1
 ))))))
