...
(declare-const fuel%test_crate!pred. FuelId)
...
(assert
 (distinct fuel%vstd!seq.impl&%0.spec_index. fuel%vstd!seq.axiom_seq_index_decreases.
  fuel%vstd!seq.axiom_seq_ext_equal. fuel%vstd!seq.axiom_seq_ext_equal_deep. fuel%test_crate!pred.
...
;; Function-Decl test_crate::pred
(declare-fun test_crate!pred.? (Poly Poly) Bool)
...
;; Function-Axioms test_crate::pred
(assert
 (fuel_bool_default fuel%test_crate!pred.)
)
(assert
 (=>
  (fuel_bool fuel%test_crate!pred.)
  (forall ((a! Poly) (s! Poly)) (!
    (= (test_crate!pred.? a! s!) (< (%I a!) (vstd!seq.Seq.len.? $ INT s!)))
    :pattern ((test_crate!pred.? a! s!))
    :qid internal_test_crate!pred.?_definition
    :skolemid skolem_internal_test_crate!pred.?_definition
))))
