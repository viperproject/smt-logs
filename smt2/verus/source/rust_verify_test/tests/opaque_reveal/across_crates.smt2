...
(assert
 (distinct fuel%vstd!array.array_view. fuel%vstd!array.impl&%0.view. fuel%vstd!array.impl&%2.spec_index.
  ...
  fuel%vstd!seq.axiom_seq_subrange_index. fuel%vstd!seq_lib.impl&%0.filter. fuel%vstd!seq_lib.impl&%0.filter_lemma.
  fuel%vstd!seq_lib.impl&%0.contains. fuel%vstd!seq_lib.impl&%0.drop_last. fuel%vstd!view.impl&%0.view.
 ...
 (=>
  ...
  (forall ((A&. Dcr) (A& Type) (self! Poly) (needle! Poly)) (!
    (= (vstd!seq_lib.impl&%0.contains.? A&. A& self! needle!) (exists ((i$ Poly)) (!
       ...
       :pattern ((vstd!seq.Seq.index.? A&. A& self! i$))
       :qid user_vstd__seq_lib__impl&%0__contains_17
       :skolemid skolem_user_vstd__seq_lib__impl&%0__contains_17
    )))
...
(assert
 (=>
  ...
  (forall ((A&. Dcr) (A& Type) (self! Poly) (pred! Poly)) (!
    ...
    :qid user_vstd__seq_lib__impl&%0__filter_lemma_20
    :skolemid skolem_user_vstd__seq_lib__impl&%0__filter_lemma_20
))))
...
;; Function-Specs test_crate::aproof
(declare-fun req%test_crate!aproof. (vstd!seq.Seq<nat.>.) Bool)
...
(assert
 (forall ((s! vstd!seq.Seq<nat.>.)) (!
   (= (req%test_crate!aproof. s!) (=>
     %%global_location_label%%6
     (= s! (%Poly%vstd!seq.Seq<nat.>. (vstd!view.View.view.? $ (ARRAY $ NAT $ (CONST_INT 2))
        (array_new $ NAT 2 (%%array%%0 (I 1) (I 2)))
   )))))
   :pattern ((req%test_crate!aproof. s!))
   :qid internal_req__test_crate!aproof._definition
   :skolemid skolem_internal_req__test_crate!aproof._definition
)))

;; Function-Def test_crate::aproof
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/opaque_reveal-ffd865035fee09a0-across_crates/test.rs:16:7: 16:29 (#0)
...
(push)
 ...
 (assert
  (forall ((%%hole%%0 Int) (x$ Poly)) (!
    (= (%%apply%%0 (%%lambda%%1 %%hole%%0) x$) (B (= (%I x$) %%hole%%0)))
 ...
 (assert
  (not (=>
    (fuel_bool fuel%vstd!seq_lib.impl&%0.filter.)
    (=>
     (exists ((fuel% Fuel)) (= fuel_nat%vstd!seq_lib.impl&%0.filter. (succ (succ fuel%))))
     (=>
      (= tmp%1 (ext_eq false (TYPE%vstd!seq.Seq. $ NAT) (vstd!seq_lib.impl&%0.filter.? $ NAT
         (Poly%vstd!seq.Seq<nat.>. s!) (Poly%fun%1. (mk_fun (%%lambda%%1 1)))
        ) (vstd!seq.Seq.push.? $ NAT (vstd!seq.Seq.empty.? $ NAT) (I 1))
      ))
      (=>
       %%location_label%%0
       tmp%1
