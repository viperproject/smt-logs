...
(declare-const fuel%test_crate!all_contains. FuelId)
...
(assert
 (distinct fuel%vstd!set.axiom_set_ext_equal. fuel%vstd!set.axiom_set_ext_equal_deep.
  fuel%test_crate!all_contains. fuel%vstd!array.group_array_axioms. fuel%vstd!map.group_map_axioms.
...
;; Function-Decl test_crate::all_contains
(declare-fun test_crate!all_contains.? (Dcr Type Poly) Bool)
...
;; Function-Axioms test_crate::all_contains
(assert
 (fuel_bool_default fuel%test_crate!all_contains.)
)
(assert
 (=>
  (fuel_bool fuel%test_crate!all_contains.)
  (forall ((A&. Dcr) (A& Type) (s1! Poly)) (!
    (= (test_crate!all_contains.? A&. A& s1!) (forall ((a$ Poly)) (!
       (=>
        (has_type a$ A&)
        (vstd!set.impl&%0.contains.? A&. A& s1! a$)
       )
       :pattern ((vstd!set.impl&%0.contains.? A&. A& s1! a$))
       :qid user_test_crate__all_contains_3
       :skolemid skolem_user_test_crate__all_contains_3
    )))
    :pattern ((test_crate!all_contains.? A&. A& s1!))
    :qid internal_test_crate!all_contains.?_definition
    :skolemid skolem_internal_test_crate!all_contains.?_definition
))))

;; Function-Def test_crate::failing_proof
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/inline-3b53de2a6f5139e9-inline_poly/test.rs:22:7: 22:36 (#0)
...
(push)
 ...
 (assert
  (not (=>
    (= tmp%1 (forall ((a$$0 Poly)) (!
       (=>
        (has_type a$$0 INT)
        (vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. s!) a$$0)
       )
       :pattern ((vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. s!) a$$0))
       :qid user_test_crate__failing_proof_4
       :skolemid skolem_user_test_crate__failing_proof_4
    )))
    (=>
     %%location_label%%0
     tmp%1
...
;; Function-Recommends test_crate::failing_proof
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/inline-3b53de2a6f5139e9-inline_poly/test.rs:22:7: 22:36 (#0)
(get-info :all-statistics)
(push)
 (declare-const s! vstd!set.Set<int.>.)
