...
(declare-const fuel%test_crate!is_modest. FuelId)
...
(assert
 (distinct fuel%vstd!set.impl&%0.subset_of. fuel%vstd!set.axiom_set_new. fuel%vstd!set.axiom_set_ext_equal.
  fuel%vstd!set.axiom_set_ext_equal_deep. fuel%vstd!set.axiom_set_contains_len. fuel%vstd!set_lib.set_int_range.
  fuel%test_crate!is_modest. fuel%test_crate!is_even. fuel%vstd!array.group_array_axioms.
...
;; Function-Decl test_crate::is_modest
(declare-fun test_crate!is_modest.? (Poly) Bool)
...
(assert
 (=>
  ...
  (forall ((A&. Dcr) (A& Type) (s! Poly) (a! Poly)) (!
    ...
    :qid user_vstd__set__axiom_set_contains_len_4
    :skolemid skolem_user_vstd__set__axiom_set_contains_len_4
))))
 ...
 (=>
  ...
  (forall ((A&. Dcr) (A& Type) (self! Poly) (s2! Poly)) (!
    (= (vstd!set.impl&%0.subset_of.? A&. A& self! s2!) (forall ((a$ Poly)) (!
       ...
       :pattern ((vstd!set.impl&%0.contains.? A&. A& s2! a$))
       :qid user_vstd__set__impl&%0__subset_of_5
       :skolemid skolem_user_vstd__set__impl&%0__subset_of_5
    )))
...
;; Function-Specs vstd::set_lib::lemma_int_range
...
;; Function-Axioms test_crate::is_modest
(assert
 (fuel_bool_default fuel%test_crate!is_modest.)
)
(assert
 (=>
  (fuel_bool fuel%test_crate!is_modest.)
  (forall ((x! Poly)) (!
    (= (test_crate!is_modest.? x!) (and
      (<= 0 (%I x!))
      (< (%I x!) 10)
    ))
    :pattern ((test_crate!is_modest.? x!))
    :qid internal_test_crate!is_modest.?_definition
    :skolemid skolem_internal_test_crate!is_modest.?_definition
))))
...
;; Function-Def test_crate::is_this_set_finite
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/summer_school-9d3e788a530b70ff-e15_pass/test.rs:26:7: 26:30 (#0)
(get-info :all-statistics)
(push)
 (declare-const no%param Int)
 (declare-const tmp%1 Bool)
 (declare-const modest_evens@ vstd!set.Set<int.>.)
 (declare-const modest_numbers@ vstd!set.Set<int.>.)
 (assert
  fuel_defaults
 )
 ...
 (assert
  (forall ((x$ Poly)) (!
    (= (%%apply%%0 %%lambda%%1 x$) (B (and
       (test_crate!is_modest.? x$)
       (test_crate!is_even.? x$)
    )))
    :pattern ((%%apply%%0 %%lambda%%1 x$))
 )))
 ...
 (assert
  (not (=>
    (= modest_evens@ (%Poly%vstd!set.Set<int.>. (vstd!set.impl&%0.new.? $ INT $ (TYPE%fun%1.
        $ INT $ BOOL
       ) (Poly%fun%1. (mk_fun %%lambda%%1))
    )))
    (=>
     (= modest_numbers@ (vstd!set_lib.set_int_range.? (I 0) (I 10)))
     (and
      (=>
       %%location_label%%0
       (req%vstd!set_lib.lemma_int_range. 0 10)
      )
      (=>
       (ens%vstd!set_lib.lemma_int_range. 0 10)
       (and
        (=>
         %%location_label%%1
         (req%vstd!set_lib.lemma_len_subset. $ INT (Poly%vstd!set.Set<int.>. modest_evens@)
          (Poly%vstd!set.Set<int.>. modest_numbers@)
        ))
        (=>
         (ens%vstd!set_lib.lemma_len_subset. $ INT (Poly%vstd!set.Set<int.>. modest_evens@)
          (Poly%vstd!set.Set<int.>. modest_numbers@)
         )
         (=>
          (= tmp%1 (vstd!set.impl&%0.finite.? $ INT (Poly%vstd!set.Set<int.>. modest_evens@)))
          (=>
           %%location_label%%2
           tmp%1
 ))))))))))
