...
(declare-const fuel%test_crate!impl&%0.view. FuelId)
...
(assert
 (distinct fuel%vstd!raw_ptr.impl&%4.view. fuel%vstd!raw_ptr.ptrs_mut_eq. fuel%vstd!seq.impl&%0.spec_index.
  ...
  fuel%vstd!view.impl&%42.view. fuel%test_crate!impl&%0.view. fuel%vstd!array.group_array_axioms.
...
(declare-datatypes ((vstd!raw_ptr.Metadata. 0) (vstd!raw_ptr.PtrData. 0) (test_crate!S.
  ...
  ) ((test_crate!S./S)) ((tuple%0./tuple%0)) ((tuple%2./tuple%2 (tuple%2./tuple%2/?0 Poly)
...
(assert
 (= (proj%%vstd!view.View./V $ TYPE%test_crate!S.) $)
)
(assert
 (= (proj%vstd!view.View./V $ TYPE%test_crate!S.) INT)
)
...
;; Function-Axioms test_crate::S::view
(assert
 (fuel_bool_default fuel%test_crate!impl&%0.view.)
)
(assert
 (=>
  (fuel_bool fuel%test_crate!impl&%0.view.)
  (forall ((self! Poly)) (!
    (= (vstd!view.View.view.? $ TYPE%test_crate!S. self!) (I 7))
    :pattern ((vstd!view.View.view.? $ TYPE%test_crate!S. self!))
    :qid internal_vstd!view.View.view.?_definition
    :skolemid skolem_internal_vstd!view.View.view.?_definition
))))
...
;; Trait-Impl-Axiom
(assert
 (tr_bound%vstd!view.View. $ TYPE%test_crate!S.)
)

;; Function-Specs test_crate::p
(declare-fun ens%test_crate!p. (Dcr Type Poly tuple%2.) Bool)
(assert
 (forall ((A&. Dcr) (A& Type) (x! Poly) (r! tuple%2.)) (!
   (= (ens%test_crate!p. A&. A& x! r!) (and
     (has_type (Poly%tuple%2. r!) (TYPE%tuple%2. (proj%%vstd!view.View./V A&. A&) (proj%vstd!view.View./V
        A&. A&
       ) (proj%%vstd!view.View./V A&. A&) (proj%vstd!view.View./V A&. A&)
     ))
     (= (tuple%2./tuple%2/1 (%Poly%tuple%2. (Poly%tuple%2. r!))) (vstd!view.View.view.?
       A&. A& x!
   ))))
   :pattern ((ens%test_crate!p. A&. A& x! r!))
   :qid internal_ens__test_crate!p._definition
   :skolemid skolem_internal_ens__test_crate!p._definition
)))

;; Function-Def test_crate::p
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/assoc_type_impls-ff4a593043060323-trait_poly_fail/test.rs:16:7: 16:46 (#0)
...
(push)
 ...
 (assert
  (not (=>
    (= r! (tuple%2./tuple%2 (vstd!view.View.view.? A&. A& x!) (vstd!view.View.view.? A&.
       A& x!
    )))
    (=>
     %%location_label%%0
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/assoc_type_impls-ff4a593043060323-trait_poly_fail/test.rs:26:1: 26:10 (#0)
(get-info :all-statistics)
(push)
 ...
 (declare-const y@ tuple%2.)
 (declare-const v@0 alloc!vec.Vec<bool./allocator_global%.>.)
 (assert
  fuel_defaults
 )
 (declare-const v@1 alloc!vec.Vec<bool./allocator_global%.>.)
 ;; assertion failed
 (declare-const %%location_label%%0 Bool)
 ;; assertion failed
 ...
 (assert
  (not (=>
    (ens%alloc!vec.impl&%0.new. $ BOOL tmp%1)
    (=>
     (= v@0 (%Poly%alloc!vec.Vec<bool./allocator_global%.>. tmp%1))
     (=>
      (ens%alloc!vec.impl&%1.push. $ BOOL $ ALLOCATOR_GLOBAL (Poly%alloc!vec.Vec<bool./allocator_global%.>.
        v@0
       ) (Poly%alloc!vec.Vec<bool./allocator_global%.>. v@1) (B true)
      )
      (=>
       (= tmp%2 test_crate!S./S)
       (=>
        (ens%test_crate!p. $ TYPE%test_crate!S. (Poly%test_crate!S. tmp%2) x@)
        (=>
         (= tmp%3 (= (%I (tuple%2./tuple%2/0 (%Poly%tuple%2. (Poly%tuple%2. x@)))) 7))
         (and
          (=>
           %%location_label%%0
           tmp%3
          ...
          (=>
           tmp%3
           (=>
            (= tmp%4 (= (%I (tuple%2./tuple%2/1 (%Poly%tuple%2. (Poly%tuple%2. x@)))) 7))
            (and
             (=>
              %%location_label%%1
              tmp%4
             ...
             (=>
              tmp%4
              (=>
               (ens%test_crate!p. $ (TYPE%alloc!vec.Vec. $ BOOL $ ALLOCATOR_GLOBAL) (Poly%alloc!vec.Vec<bool./allocator_global%.>.
                 v@1
                ) y@
               )
               (=>
                (= tmp%5 (%B (vstd!seq.Seq.index.? $ BOOL (tuple%2./tuple%2/1 (%Poly%tuple%2. (Poly%tuple%2.
                      y@
                    ))
                   ) (I 0)
                )))
     ...
     (=>
      ...
      (=>
       ...
       (=>
        (ens%test_crate!p. $ TYPE%test_crate!S. (Poly%test_crate!S. tmp%2) x@)
        (=>
         (= (%I (tuple%2./tuple%2/0 (%Poly%tuple%2. (Poly%tuple%2. x@)))) 7)
         (=>
          (= (%I (tuple%2./tuple%2/1 (%Poly%tuple%2. (Poly%tuple%2. x@)))) 7)
          (=>
           (ens%test_crate!p. $ (TYPE%alloc!vec.Vec. $ BOOL $ ALLOCATOR_GLOBAL) (Poly%alloc!vec.Vec<bool./allocator_global%.>.
             v@1
            ) y@
           ...
           (=>
            (= tmp%4 (tuple%2./tuple%2/1 (%Poly%tuple%2. (Poly%tuple%2. y@))))
            (=>
             %%location_label%%0
             (req%vstd!seq.impl&%0.spec_index. $ BOOL tmp%4 (I 0))
 ))))))))))))
 (get-info :version)
 (set-option :rlimit 30000000)
 (check-sat)
