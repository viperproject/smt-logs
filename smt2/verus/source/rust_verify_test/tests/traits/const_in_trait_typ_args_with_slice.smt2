...
(assert
 (distinct fuel%vstd!array.array_view. fuel%vstd!array.impl&%0.view. fuel%vstd!array.impl&%2.spec_index.
  ...
  fuel%vstd!seq.axiom_seq_ext_equal_deep. fuel%vstd!slice.axiom_spec_len. fuel%vstd!view.impl&%0.view.
...
(declare-datatypes ((vstd!raw_ptr.Metadata. 0) (vstd!raw_ptr.PtrData. 0) (test_crate!Bar.
  ...
  ) ((test_crate!Bar./Bar (test_crate!Bar./Bar/?slice %%Function%%))) ((tuple%0./tuple%0))
))
...
(declare-fun test_crate!Bar./Bar/slice (test_crate!Bar.) %%Function%%)
...
(declare-fun TYPE%test_crate!Bar. (Dcr Type) Type)
...
(assert
 (forall ((X&. Dcr) (X& Type) (x Poly)) (!
   (=>
    (has_type x (TYPE%test_crate!Bar. X&. X&))
    (= x (Poly%test_crate!Bar. (%Poly%test_crate!Bar. x)))
   )
   :pattern ((has_type x (TYPE%test_crate!Bar. X&. X&)))
   :qid internal_test_crate__Bar_unbox_axiom_definition
   :skolemid skolem_internal_test_crate__Bar_unbox_axiom_definition
)))
(assert
 (forall ((X&. Dcr) (X& Type) (_slice! %%Function%%)) (!
   (=>
    (has_type (Poly%array%. _slice!) (ARRAY $ BOOL X&. X&))
    (has_type (Poly%test_crate!Bar. (test_crate!Bar./Bar _slice!)) (TYPE%test_crate!Bar.
      X&. X&
   )))
   :pattern ((has_type (Poly%test_crate!Bar. (test_crate!Bar./Bar _slice!)) (TYPE%test_crate!Bar.
      X&. X&
   )))
...
(assert
 (forall ((x test_crate!Bar.)) (!
   (= (test_crate!Bar./Bar/slice x) (test_crate!Bar./Bar/?slice x))
   :pattern ((test_crate!Bar./Bar/slice x))
   :qid internal_test_crate!Bar./Bar/slice_accessor_definition
   :skolemid skolem_internal_test_crate!Bar./Bar/slice_accessor_definition
)))
(assert
 (forall ((X&. Dcr) (X& Type) (x Poly)) (!
   (=>
    (has_type x (TYPE%test_crate!Bar. X&. X&))
    (has_type (Poly%array%. (test_crate!Bar./Bar/slice (%Poly%test_crate!Bar. x))) (ARRAY
      $ BOOL X&. X&
   )))
   :pattern ((test_crate!Bar./Bar/slice (%Poly%test_crate!Bar. x)) (has_type x (TYPE%test_crate!Bar.
      X&. X&
   )))
   :qid internal_test_crate!Bar./Bar/slice_invariant_definition
   :skolemid skolem_internal_test_crate!Bar./Bar/slice_invariant_definition
)))
(assert
 (forall ((x tuple%0.)) (!
...
(declare-fun tr_bound%test_crate!Trait. (Dcr Type Dcr Type) Bool)
(assert
...
(assert
 (forall ((Self%&. Dcr) (Self%& Type) (X&. Dcr) (X& Type)) (!
   (=>
    (tr_bound%test_crate!Trait. Self%&. Self%& X&. X&)
    (uInv SZ (const_int X&))
   )
   :pattern ((tr_bound%test_crate!Trait. Self%&. Self%& X&. X&))
   :qid internal_test_crate__Trait_trait_type_bounds_definition
   :skolemid skolem_internal_test_crate__Trait_trait_type_bounds_definition
)))
...
;; Function-Specs test_crate::Trait::exec_get_x
(declare-fun ens%test_crate!Trait.exec_get_x. (Dcr Type Dcr Type Poly Poly) Bool)
(assert
 (forall ((Self%&. Dcr) (Self%& Type) (X&. Dcr) (X& Type) (self! Poly) (r! Poly)) (
   !
   (= (ens%test_crate!Trait.exec_get_x. Self%&. Self%& X&. X& self! r!) (and
     (has_type r! (UINT SZ))
     (= (%I r!) (const_int X&))
   ))
   :pattern ((ens%test_crate!Trait.exec_get_x. Self%&. Self%& X&. X& self! r!))
   :qid internal_ens__test_crate!Trait.exec_get_x._definition
   :skolemid skolem_internal_ens__test_crate!Trait.exec_get_x._definition
)))
...
;; Function-Def test_crate::Bar::exec_get_x
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/traits-e45926d1282e7df0-const_in_trait_typ_args_with_slice/test.rs:27:5: 27:38 (#0)
...
(push)
 ...
 (assert
  (has_type self! (TYPE%test_crate!Bar. X&. X&))
 )
 (assert
  (uInv SZ (const_int X&))
 )
 ...
 (assert
  (not (=>
    (= tmp%2 (test_crate!Bar./Bar/slice (%Poly%test_crate!Bar. self!)))
    (=>
     (ens%vstd!array.array_as_slice. $ BOOL X&. X& tmp%2 tmp%1)
     (=>
      (ens%core!slice.impl&%0.len. $ BOOL tmp%1 tmp%3)
      (=>
       (= %return! (I tmp%3))
       (=>
        %%location_label%%0
...
(assert
 (forall ((X&. Dcr) (X& Type)) (!
   (=>
    (uInv SZ (const_int X&))
    (tr_bound%test_crate!Trait. $ (TYPE%test_crate!Bar. X&. X&) X&. X&)
   )
   :pattern ((tr_bound%test_crate!Trait. $ (TYPE%test_crate!Bar. X&. X&) X&. X&))
