...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/ext_equal-1201012c76859bc3-test_fn/test.rs:15:7: 15:70 (#0)
(get-info :all-statistics)
(push)
 (declare-const x! %%Function%%)
 (declare-const y! %%Function%%)
 (declare-const tmp%1 Bool)
 (assert
  fuel_defaults
 )
 (assert
  (has_type (Poly%fun%2. x!) (TYPE%fun%2. $ INT $ (UINT 8) $ INT))
 )
 (assert
  (has_type (Poly%fun%2. y!) (TYPE%fun%2. $ INT $ (UINT 8) $ INT))
 )
 ;; assertion failed
  ...
  (not (=>
    (forall ((i$ Poly) (j$ Poly)) (!
      (=>
       ...
       (= (%%apply%%0 x! i$ j$) (%%apply%%0 y! i$ j$))
      )
      :pattern ((%%apply%%0 x! i$ j$))
      :qid user_crate__test_0
      :skolemid skolem_user_crate__test_0
    ))
    (=>
     (= tmp%1 (ext_eq false (TYPE%fun%2. $ INT $ (UINT 8) $ INT) (Poly%fun%2. x!) (Poly%fun%2.
        y!
     )))
     (=>
      %%location_label%%0
