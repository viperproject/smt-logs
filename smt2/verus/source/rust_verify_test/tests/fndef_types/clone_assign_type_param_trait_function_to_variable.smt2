...
;; Datatypes
(declare-datatypes ((test_crate!X. 0) (tuple%1. 0)) (((test_crate!X./X (test_crate!X./X/?t
     Poly
   ))
  ) ((tuple%1./tuple%1 (tuple%1./tuple%1/?0 Poly)))
))
(declare-fun test_crate!X./X/t (test_crate!X.) Poly)
(declare-fun tuple%1./tuple%1/0 (tuple%1.) Poly)
(declare-fun TYPE%test_crate!X. (Dcr Type) Type)
...
(assert
 (forall ((T&. Dcr) (T& Type) (x Poly)) (!
   (=>
    (has_type x (TYPE%test_crate!X. T&. T&))
    (= x (Poly%test_crate!X. (%Poly%test_crate!X. x)))
   )
   :pattern ((has_type x (TYPE%test_crate!X. T&. T&)))
...
(assert
 (forall ((T&. Dcr) (T& Type) (_t! Poly)) (!
   (=>
    (has_type _t! T&)
    (has_type (Poly%test_crate!X. (test_crate!X./X _t!)) (TYPE%test_crate!X. T&. T&))
   )
   :pattern ((has_type (Poly%test_crate!X. (test_crate!X./X _t!)) (TYPE%test_crate!X. T&.
      T&
   )))
...
(assert
 (forall ((T&. Dcr) (T& Type) (x Poly)) (!
   (=>
    (has_type x (TYPE%test_crate!X. T&. T&))
    (has_type (test_crate!X./X/t (%Poly%test_crate!X. x)) T&)
   )
...
(assert
 (forall ((x test_crate!X.)) (!
   (=>
    (is-test_crate!X./X x)
    (height_lt (height (test_crate!X./X/t x)) (height (Poly%test_crate!X. x)))
   )
   :pattern ((height (test_crate!X./X/t x)))
   :qid prelude_datatype_height_test_crate!X./X/t
   :skolemid skolem_prelude_datatype_height_test_crate!X./X/t
)))
(assert
 (forall ((x tuple%1.)) (!
...
(declare-fun ens%test_crate!impl&%0.clone. (Dcr Type Poly Poly) Bool)
(assert
 (forall ((T&. Dcr) (T& Type) (self! Poly) (s! Poly)) (!
   (= (ens%test_crate!impl&%0.clone. T&. T& self! s!) (and
     (ens%core!clone.Clone.clone. $ (TYPE%test_crate!X. T&. T&) self! s!)
     (closure_ens (FNDEF%core!clone.Clone.clone. T&. T&) $ (TYPE%tuple%1. (REF T&.) T&)
      (F fndef_singleton) (Poly%tuple%1. (tuple%1./tuple%1 (test_crate!X./X/t (%Poly%test_crate!X.
          self!
       )))
      ) (test_crate!X./X/t (%Poly%test_crate!X. s!))
   )))
   :pattern ((ens%test_crate!impl&%0.clone. T&. T& self! s!))
...
(assert
 (forall ((tmp%%$ Poly) (s$ Poly) (T&. Dcr) (T& Type)) (!
   (=>
    (and
     (has_type tmp%%$ (TYPE%tuple%1. (REF $) (TYPE%test_crate!X. T&. T&)))
     (has_type s$ (TYPE%test_crate!X. T&. T&))
    )
    (=>
     (closure_ens (FNDEF%core!clone.Clone.clone. $ (TYPE%test_crate!X. T&. T&)) $ (TYPE%tuple%1.
       (REF $) (TYPE%test_crate!X. T&. T&)
   ...
   :pattern ((closure_ens (FNDEF%core!clone.Clone.clone. $ (TYPE%test_crate!X. T&. T&))
     $ (TYPE%tuple%1. (REF $) (TYPE%test_crate!X. T&. T&)) (F fndef_singleton) tmp%%$
     s$
   ))
   :qid user_test_crate__X__clone_6
   :skolemid skolem_user_test_crate__X__clone_6
)))

;; Function-Def test_crate::X::clone
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/fndef_types-2b591f3dc5716dd9-clone_assign_type_param_trait_function_to_variable/test.rs:22:5: 22:32 (#0)
...
(push)
 ...
 (declare-const t_clone@ fndef)
 (declare-const new_t@ Poly)
 (assert
  fuel_defaults
 )
 (assert
  (has_type self! (TYPE%test_crate!X. T&. T&))
 )
 (assert
  (tr_bound%core!clone.Clone. T&. T&)
 )
 ...
 (assert
  (not (=>
    (= t_clone@ fndef_singleton)
    (=>
     (= tmp%1 (tuple%1./tuple%1 (test_crate!X./X/t (%Poly%test_crate!X. self!))))
     (and
      (=>
       %%location_label%%0
       (req%vstd!pervasive.exec_nonstatic_call. $ (TYPE%tuple%1. (REF T&.) T&) T&. T& $ (
         FNDEF%core!clone.Clone.clone. T&. T&
        ) (F t_clone@) (Poly%tuple%1. tmp%1)
      ))
      (=>
       (ens%vstd!pervasive.exec_nonstatic_call. $ (TYPE%tuple%1. (REF T&.) T&) T&. T& $ (
         FNDEF%core!clone.Clone.clone. T&. T&
        ) (F t_clone@) (Poly%tuple%1. tmp%1) new_t@
       )
       (=>
        (= s! (Poly%test_crate!X. (test_crate!X./X new_t@)))
        (=>
         %%location_label%%1
         (closure_ens (FNDEF%core!clone.Clone.clone. T&. T&) $ (TYPE%tuple%1. (REF T&.) T&)
          (F fndef_singleton) (Poly%tuple%1. (tuple%1./tuple%1 (test_crate!X./X/t (%Poly%test_crate!X.
...
(assert
 (forall ((T&. Dcr) (T& Type)) (!
   (=>
    (tr_bound%core!clone.Clone. T&. T&)
    (tr_bound%core!clone.Clone. $ (TYPE%test_crate!X. T&. T&))
   )
   :pattern ((tr_bound%core!clone.Clone. $ (TYPE%test_crate!X. T&. T&)))
