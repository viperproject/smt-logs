...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/maps-df7fca472355527b-test1_fails_eq/test.rs:18:7: 18:22 (#0)
...
(push)
 ...
 (assert
  (forall ((%%hole%%0 Int) (y$ Poly)) (!
    (= (%%apply%%0 (%%lambda%%1 %%hole%%0) y$) (I (Add %%hole%%0 (%I y$))))
    :pattern ((%%apply%%0 (%%lambda%%1 %%hole%%0) y$))
 )))
 ;; assertion failed
  ...
  (not (=>
    ...
    (=>
     (= m1@ (%Poly%vstd!map.Map<int./int.>. (vstd!set.impl&%0.mk_map.? $ INT $ INT $ (TYPE%fun%1.
         $ INT $ INT
        ) (Poly%vstd!set.Set<int.>. s@) (Poly%fun%1. (mk_fun (%%lambda%%0 4)))
     )))
     (=>
      (= m2@ (%Poly%vstd!map.Map<int./int.>. (vstd!set.impl&%0.mk_map.? $ INT $ INT $ (TYPE%fun%1.
          $ INT $ INT
         ) (Poly%vstd!set.Set<int.>. s@) (Poly%fun%1. (mk_fun (%%lambda%%1 (nClip (Add 2 2)))))
      )))
      (=>
       %%location_label%%0
       (= m1@ m2@)
...
;; Function-Recommends test_crate::testfun_eq
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/maps-df7fca472355527b-test1_fails_eq/test.rs:18:7: 18:22 (#0)
(get-info :all-statistics)
(push)
 (declare-const no%param Int)
 ...
 (assert
  (forall ((%%hole%%0 Int) (y$ Poly)) (!
    (= (%%apply%%0 (%%lambda%%3 %%hole%%0) y$) (I (Add %%hole%%0 (%I y$))))
    :pattern ((%%apply%%0 (%%lambda%%3 %%hole%%0) y$))
 )))
 (assert
  (not true)
 )
