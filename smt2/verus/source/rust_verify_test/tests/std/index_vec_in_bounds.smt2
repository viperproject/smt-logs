...
;; Function-Specs test_crate::stuff
(declare-fun req%test_crate!stuff. (alloc!vec.Vec<u8./allocator_global%.>.) Bool)
(declare-const %%global_location_label%%3 Bool)
(assert
 (forall ((v! alloc!vec.Vec<u8./allocator_global%.>.)) (!
   (= (req%test_crate!stuff. v!) (=>
     %%global_location_label%%3
     (> (vstd!std_specs.vec.spec_vec_len.? $ (UINT 8) $ ALLOCATOR_GLOBAL (Poly%alloc!vec.Vec<u8./allocator_global%.>.
        v!
       )
      ) 0
   )))
   :pattern ((req%test_crate!stuff. v!))
   :qid internal_req__test_crate!stuff._definition
   :skolemid skolem_internal_req__test_crate!stuff._definition
)))

;; Function-Def test_crate::stuff
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/std-a9a93a7619503a6c-index_vec_in_bounds/test.rs:17:1: 17:22 (#0)
...
(push)
 ...
 (assert
  (not (and
    (=>
     %%location_label%%0
     (req%vstd!std_specs.vec.vec_index. $ (UINT 8) $ ALLOCATOR_GLOBAL (Poly%alloc!vec.Vec<u8./allocator_global%.>.
       v!
      ) 0
    ))
    (=>
     (ens%vstd!std_specs.vec.vec_index. $ (UINT 8) $ ALLOCATOR_GLOBAL (Poly%alloc!vec.Vec<u8./allocator_global%.>.
       v!
      ) 0 tmp%1
     )
     (and
