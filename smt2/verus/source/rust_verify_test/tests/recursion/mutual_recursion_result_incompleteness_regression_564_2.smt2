...
(declare-const fuel%test_crate!fn_two. FuelId)
(declare-const fuel%test_crate!fn_three. FuelId)
...
(assert
 (distinct fuel%vstd!seq.impl&%0.spec_index. fuel%vstd!seq.axiom_seq_index_decreases.
  ...
  fuel%vstd!seq.axiom_seq_ext_equal_deep. fuel%test_crate!NUM_LAYERS. fuel%test_crate!NUM_ENTRIES.
  fuel%test_crate!fn_two. fuel%test_crate!fn_three. fuel%vstd!array.group_array_axioms.
...
;; Function-Decl test_crate::fn_three
(declare-fun test_crate!fn_three.? (Poly Poly) vstd!seq.Seq<nat.>.)
(declare-fun test_crate!rec%fn_three.? (Poly Poly Fuel) vstd!seq.Seq<nat.>.)

;; Function-Decl test_crate::fn_two
(declare-fun test_crate!fn_two.? (Poly Poly) Int)
(declare-fun test_crate!rec%fn_two.? (Poly Poly Fuel) Int)
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/recursion-323c2b72cefe3aab-mutual_recursion_result_incompleteness_regression_564_2/test.rs:17:1: 17:36 (#0)
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/recursion-323c2b72cefe3aab-mutual_recursion_result_incompleteness_regression_564_2/test.rs:18:1: 18:38 (#0)
(get-info :all-statistics)
(push)
 (declare-const %return! Int)
 (assert
...
;; Spec-Termination test_crate::fn_three
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/recursion-323c2b72cefe3aab-mutual_recursion_result_incompleteness_regression_564_2/test.rs:30:1: 30:66 (#0)
(get-info :all-statistics)
(push)
 (declare-const layer! Poly)
 (declare-const init! Poly)
 (declare-const tmp%1 Poly)
 (declare-const tmp%2 vstd!seq.Seq<nat.>.)
 ...
 (assert
  (has_type init! (TYPE%vstd!seq.Seq. $ NAT))
 )
 ...
 (assert
  (not (=>
    ...
    (=>
     (= decrease%init1 (Sub test_crate!NUM_ENTRIES.? (vstd!seq.Seq.len.? $ NAT init!)))
     (=>
      (= decrease%init2 1)
      (or
       (and
        (=>
         (>= (vstd!seq.Seq.len.? $ NAT init!) test_crate!NUM_ENTRIES.?)
         (=>
          (= tmp%4 init!)
          %%switch_label%%0
        ))
        (=>
         (not (>= (vstd!seq.Seq.len.? $ NAT init!) test_crate!NUM_ENTRIES.?))
         (=>
          (= tmp%1 (I (vstd!seq.Seq.len.? $ NAT init!)))
          (and
           (=>
            %%location_label%%0
            (check_decrease_int (let
              ((layer!$0 (%I layer!)) (idx!$1 (%I tmp%1)))
              (Sub test_crate!NUM_LAYERS.? layer!$0)
             ) decrease%init0 (check_decrease_int (let
               ((layer!$0 (%I layer!)) (idx!$1 (%I tmp%1)))
               (Sub test_crate!NUM_ENTRIES.? idx!$1)
           ...
           (=>
            (<= 0 entry@)
            (=>
             (= tmp%3 (vstd!seq.Seq.push.? $ NAT init! (I entry@)))
             (and
              (=>
               %%location_label%%1
               (check_decrease_int (let
                 ((layer!$0 (%I layer!)) (init!$1 (%Poly%vstd!seq.Seq<nat.>. tmp%3)))
                 (Sub test_crate!NUM_LAYERS.? layer!$0)
                ) decrease%init0 (check_decrease_int (let
                  ((layer!$0 (%I layer!)) (init!$1 (%Poly%vstd!seq.Seq<nat.>. tmp%3)))
                  (Sub test_crate!NUM_ENTRIES.? (vstd!seq.Seq.len.? $ NAT (Poly%vstd!seq.Seq<nat.>. init!$1)))
                 ) decrease%init1 (check_decrease_int (let
                   ((layer!$0 (%I layer!)) (init!$1 (%Poly%vstd!seq.Seq<nat.>. tmp%3)))
                   1
                  ) decrease%init2 false
              ...
              (=>
               (= tmp%2 (test_crate!fn_three.? layer! (vstd!seq.Seq.push.? $ NAT init! (I entry@))))
               (=>
                (= tmp%4 (Poly%vstd!seq.Seq<nat.>. tmp%2))
                %%switch_label%%0
       )))))))))
       (not %%switch_label%%0)
 ))))))
...
;; Spec-Termination test_crate::fn_two
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/recursion-323c2b72cefe3aab-mutual_recursion_result_incompleteness_regression_564_2/test.rs:20:1: 20:53 (#0)
(get-info :all-statistics)
(push)
 (declare-const layer! Poly)
       ...
       (and
        (=>
         ...
         (=>
          ...
          (=>
           (= tmp%3 (vstd!seq.Seq.empty.? $ NAT))
           (and
            (=>
             %%location_label%%0
             (check_decrease_int (let
               ((layer!$0 (%I tmp%2)) (init!$1 (%Poly%vstd!seq.Seq<nat.>. tmp%3)))
               (Sub test_crate!NUM_LAYERS.? layer!$0)
              ) decrease%init0 (check_decrease_int (let
                ((layer!$0 (%I tmp%2)) (init!$1 (%Poly%vstd!seq.Seq<nat.>. tmp%3)))
                (Sub test_crate!NUM_ENTRIES.? (vstd!seq.Seq.len.? $ NAT (Poly%vstd!seq.Seq<nat.>. init!$1)))
            ...
            (=>
             (= tmp%1 (test_crate!fn_three.? (I (nClip (Add (%I layer!) 1))) (vstd!seq.Seq.empty.?
                $ NAT
             )))
             (=>
              (= tmp%4 (vstd!seq.Seq.len.? $ NAT (Poly%vstd!seq.Seq<nat.>. tmp%1)))
              %%switch_label%%0
        ))))))
        (=>
         (not (<= (nClip (Add (%I layer!) 1)) test_crate!NUM_LAYERS.?))
         (=>
          (= tmp%4 (%I (vstd!pervasive.arbitrary.? $ NAT)))
          %%switch_label%%0
       )))
       (not %%switch_label%%0)
 ))))))
...
;; Function-Axioms test_crate::fn_three
(assert
 (fuel_bool_default fuel%test_crate!fn_three.)
)
(declare-const fuel_nat%test_crate!fn_three. Fuel)
(assert
 (forall ((layer! Poly) (init! Poly) (fuel% Fuel)) (!
   (= (test_crate!rec%fn_three.? layer! init! fuel%) (test_crate!rec%fn_three.? layer!
     init! zero
   ))
   :pattern ((test_crate!rec%fn_three.? layer! init! fuel%))
   :qid internal_test_crate!fn_three._fuel_to_zero_definition
   :skolemid skolem_internal_test_crate!fn_three._fuel_to_zero_definition
)))
(assert
 (forall ((layer! Poly) (init! Poly) (fuel% Fuel)) (!
   (=>
    ...
    (= (test_crate!rec%fn_three.? layer! init! (succ fuel%)) (ite
      (>= (vstd!seq.Seq.len.? $ NAT init!) test_crate!NUM_ENTRIES.?)
      (%Poly%vstd!seq.Seq<nat.>. init!)
      (let
       ((entry$ (test_crate!rec%fn_two.? layer! (I (vstd!seq.Seq.len.? $ NAT init!)) fuel%)))
       (test_crate!rec%fn_three.? layer! (vstd!seq.Seq.push.? $ NAT init! (I entry$)) fuel%)
   ))))
   :pattern ((test_crate!rec%fn_three.? layer! init! (succ fuel%)))
   :qid internal_test_crate!fn_three._fuel_to_body_definition
   :skolemid skolem_internal_test_crate!fn_three._fuel_to_body_definition
)))
(assert
 (=>
  (fuel_bool fuel%test_crate!fn_three.)
  (forall ((layer! Poly) (init! Poly)) (!
    (=>
     (and
      (has_type layer! NAT)
      (has_type init! (TYPE%vstd!seq.Seq. $ NAT))
     )
     (= (test_crate!fn_three.? layer! init!) (test_crate!rec%fn_three.? layer! init! (succ
        fuel_nat%test_crate!fn_three.
    ))))
    :pattern ((test_crate!fn_three.? layer! init!))
    :qid internal_test_crate!fn_three.?_definition
    :skolemid skolem_internal_test_crate!fn_three.?_definition
))))

;; Function-Axioms test_crate::fn_two
(assert
 (fuel_bool_default fuel%test_crate!fn_two.)
)
(declare-const fuel_nat%test_crate!fn_two. Fuel)
(assert
 (forall ((layer! Poly) (idx! Poly) (fuel% Fuel)) (!
   (= (test_crate!rec%fn_two.? layer! idx! fuel%) (test_crate!rec%fn_two.? layer! idx!
     zero
   ))
   :pattern ((test_crate!rec%fn_two.? layer! idx! fuel%))
   :qid internal_test_crate!fn_two._fuel_to_zero_definition
   :skolemid skolem_internal_test_crate!fn_two._fuel_to_zero_definition
)))
(assert
 (forall ((layer! Poly) (idx! Poly) (fuel% Fuel)) (!
   (=>
    ...
    (= (test_crate!rec%fn_two.? layer! idx! (succ fuel%)) (ite
      (<= (nClip (Add (%I layer!) 1)) test_crate!NUM_LAYERS.?)
      (vstd!seq.Seq.len.? $ NAT (Poly%vstd!seq.Seq<nat.>. (test_crate!rec%fn_three.? (I (nClip
           (Add (%I layer!) 1)
          )
         ) (vstd!seq.Seq.empty.? $ NAT) fuel%
      )))
      (%I (vstd!pervasive.arbitrary.? $ NAT))
   )))
   :pattern ((test_crate!rec%fn_two.? layer! idx! (succ fuel%)))
   :qid internal_test_crate!fn_two._fuel_to_body_definition
   :skolemid skolem_internal_test_crate!fn_two._fuel_to_body_definition
)))
(assert
 (=>
  (fuel_bool fuel%test_crate!fn_two.)
  (forall ((layer! Poly) (idx! Poly)) (!
    (=>
     (and
      (has_type layer! NAT)
      (has_type idx! NAT)
     )
     (= (test_crate!fn_two.? layer! idx!) (test_crate!rec%fn_two.? layer! idx! (succ (succ
         fuel_nat%test_crate!fn_two.
    )))))
    :pattern ((test_crate!fn_two.? layer! idx!))
    :qid internal_test_crate!fn_two.?_definition
    :skolemid skolem_internal_test_crate!fn_two.?_definition
))))
(assert
 (forall ((layer! Poly) (idx! Poly)) (!
   (=>
    ...
    (<= 0 (test_crate!fn_two.? layer! idx!))
   )
   :pattern ((test_crate!fn_two.? layer! idx!))
   :qid internal_test_crate!fn_two.?_pre_post_definition
   :skolemid skolem_internal_test_crate!fn_two.?_pre_post_definition
)))
