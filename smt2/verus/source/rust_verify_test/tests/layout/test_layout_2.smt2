...
(assert
 (forall ((_v! Int)) (!
   (=>
    (uInv 64 _v!)
    (has_type (Poly%test_crate!S. (test_crate!S./S _v!)) TYPE%test_crate!S.)
   )
   :pattern ((has_type (Poly%test_crate!S. (test_crate!S./S _v!)) TYPE%test_crate!S.))
...
(assert
 (= (vstd!layout.size_of.? $ TYPE%test_crate!S.) 8)
)
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/layout-754b166a24df736f-test_layout_2/test.rs:20:1: 20:10 (#0)
...
(push)
 ...
 (assert
  (not (=>
    (= tmp%1 (= (uClip SZ (vstd!layout.size_of.? $ TYPE%test_crate!S.)) 8))
    (and
     (=>
      %%location_label%%0
      tmp%1
     ...
     (=>
      tmp%1
      (=>
       (= tmp%2 (= (uClip SZ (vstd!layout.align_of.? $ TYPE%test_crate!S.)) 8))
       (=>
        %%location_label%%1
        tmp%2
 )))))))
...
(push)
 ...
 (assert
  (not (and
    (=>
     %%location_label%%0
     (req%vstd!layout.size_of_as_usize. $ TYPE%test_crate!S.)
    )
    (=>
     (= tmp%1 (uClip SZ (vstd!layout.size_of.? $ TYPE%test_crate!S.)))
     (=>
      (= tmp%1 8)
      (=>
       %%location_label%%1
       (req%vstd!layout.align_of_as_usize. $ TYPE%test_crate!S.)
 ))))))
 (get-info :version)
 (set-option :rlimit 30000000)
 (check-sat)
