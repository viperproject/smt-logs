...
(declare-const fuel%test_crate!MyArray. FuelId)
...
(assert
 (distinct fuel%vstd!array.array_view. fuel%vstd!array.impl&%0.view. fuel%vstd!array.impl&%2.spec_index.
  ...
  fuel%vstd!view.impl&%12.view. fuel%vstd!view.impl&%18.view. fuel%vstd!view.impl&%24.view.
  fuel%test_crate!MyArray. fuel%vstd!array.group_array_axioms. fuel%vstd!map.group_map_axioms.
...
;; Function-Decl test_crate::MyArray
(declare-fun test_crate!MyArray.? () %%Function%%)
...
;; Function-Def test_crate::MyArray
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/consts-b7cab2e74de87614-array_incorrect_value/test.rs:17:1: 17:37 (#0)
(get-info :all-statistics)
(push)
 (declare-const %return! %%Function%%)
 (assert
  fuel_defaults
 )
 (assert
...
;; Function-Axioms test_crate::MyArray
(assert
 (fuel_bool_default fuel%test_crate!MyArray.)
)
(declare-fun %%array%%0 (Poly Poly Poly) %%Function%%)
(assert
 (forall ((%%hole%%0 Poly) (%%hole%%1 Poly) (%%hole%%2 Poly)) (!
   (let
    ((%%x%% (%%array%%0 %%hole%%0 %%hole%%1 %%hole%%2)))
    (and
     (= (%%apply%%1 %%x%% 0) %%hole%%0)
     (= (%%apply%%1 %%x%% 1) %%hole%%1)
     (= (%%apply%%1 %%x%% 2) %%hole%%2)
   ))
   :pattern ((%%array%%0 %%hole%%0 %%hole%%1 %%hole%%2))
   :qid __AIR_ARRAY_QID__
   :skolemid skolem___AIR_ARRAY_QID__
)))
(assert
 (=>
  (fuel_bool fuel%test_crate!MyArray.)
  (= test_crate!MyArray.? (%Poly%array%. (array_new $ (UINT 32) 3 (%%array%%0 (I 1) (I 2)
      (I 3)
))))))
(assert
 (has_type (Poly%array%. test_crate!MyArray.?) (ARRAY $ (UINT 32) $ (CONST_INT 3)))
)
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/consts-b7cab2e74de87614-array_incorrect_value/test.rs:19:7: 19:16 (#0)
(get-info :all-statistics)
(push)
 ...
 (assert
  (not (=>
    (= tmp%1 (= (%I (vstd!seq.Seq.index.? $ (UINT 32) (vstd!view.View.view.? $ (ARRAY $ (UINT
           32
          ) $ (CONST_INT 3)
         ) (Poly%array%. test_crate!MyArray.?)
        ) (I 1)
       )
      ) 42
    ))
...
(push)
 ...
 (assert
  (not (=>
    (= tmp%2 (Poly%array%. test_crate!MyArray.?))
    (=>
     %%location_label%%0
     (req%vstd!array.ArrayAdditionalSpecFns.spec_index. $ (ARRAY $ (UINT 32) $ (CONST_INT
        3
       )
      ) $ (UINT 32) tmp%2 (I 1)
 )))))
