...
;; Fuel
(declare-const fuel%test_crate!int_if_2. FuelId)
...
(assert
 (distinct fuel%test_crate!int_if_2. fuel%vstd!array.group_array_axioms. fuel%vstd!map.group_map_axioms.
...
;; Function-Decl test_crate::int_if_2
(declare-fun test_crate!int_if_2.? (Poly) Int)

;; Function-Specs vstd::pervasive::assert
...
;; Function-Axioms test_crate::int_if_2
(assert
 (fuel_bool_default fuel%test_crate!int_if_2.)
)
(assert
 (=>
  (fuel_bool fuel%test_crate!int_if_2.)
  (forall ((a! Poly)) (!
    (= (test_crate!int_if_2.? a!) (ite
      (= (%I a!) 2)
      3
      (ite
       (= (%I a!) 3)
       4
       (%I (vstd!pervasive.arbitrary.? $ (UINT 128)))
    )))
    :pattern ((test_crate!int_if_2.? a!))
    :qid internal_test_crate!int_if_2.?_definition
    :skolemid skolem_internal_test_crate!int_if_2.?_definition
))))
(assert
 (forall ((a! Poly)) (!
   (=>
    (has_type a! (UINT 128))
    (uInv 128 (test_crate!int_if_2.? a!))
   )
   :pattern ((test_crate!int_if_2.? a!))
   :qid internal_test_crate!int_if_2.?_pre_post_definition
   :skolemid skolem_internal_test_crate!int_if_2.?_pre_post_definition
)))

;; Function-Def test_crate::int_if
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/modes-fd368d59d05662dd-regression_int_if/test.rs:15:1: 15:12 (#0)
     ...
     (and
      (=>
       ...
       (and
        (=>
         %%location_label%%0
         (req%vstd!pervasive.assert. false)
        )
        (=>
         (ens%vstd!pervasive.assert. false)
         %%switch_label%%0
      )))
      (=>
       (not (= a@ 4))
