...
(assert
 (distinct fuel%vstd!set.impl&%0.choose. fuel%vstd!set.axiom_set_new. fuel%vstd!set.axiom_set_ext_equal.
  fuel%vstd!set.axiom_set_ext_equal_deep. fuel%vstd!set.axiom_set_choose_finite. fuel%test_crate!f.
 ...
 (forall ((%%hole%%0 Type) (%%hole%%1 Dcr) (%%hole%%2 Type) (%%hole%%3 Poly) (%%hole%%4
   ...
   (=>
    (exists ((a$ Poly)) (!
      ...
      :qid user_vstd__set__impl&%0__choose_4
      :skolemid skolem_user_vstd__set__impl&%0__choose_4
    ))
    (exists ((a$ Poly)) (!
      (and
       (and
...
(assert
 (=>
  ...
  (forall ((A&. Dcr) (A& Type) (s! Poly)) (!
    ...
    :qid user_vstd__set__axiom_set_choose_finite_5
    :skolemid skolem_user_vstd__set__axiom_set_choose_finite_5
))))
...
(assert
 (=>
  ...
  (forall ((x! Poly)) (!
    (= (test_crate!f.? x!) true)
    :pattern ((test_crate!f.? x!))
    :qid internal_test_crate!f.?_definition
    :skolemid skolem_internal_test_crate!f.?_definition
))))

;; Function-Def test_crate::test_witness
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/sets-a55b6df8c4f72a7b-test_choose_assert_witness/test.rs:22:7: 22:24 (#0)
...
(push)
 ...
 (assert
  (forall ((x$ Poly)) (!
    (= (%%apply%%0 %%lambda%%0 x$) (B (test_crate!f.? x$)))
    :pattern ((%%apply%%0 %%lambda%%0 x$))
 )))
 ;; assertion failed
 ...
 (assert
  (not (=>
    (exists ((x$ Poly)) (!
      ...
      :pattern ((test_crate!f.? x$))
      :qid user_test_crate__test_witness_6
      :skolemid skolem_user_test_crate__test_witness_6
    ))
    (=>
     (= s@ (%Poly%vstd!set.Set<int.>. (vstd!set.impl&%0.new.? $ INT $ (TYPE%fun%1. $ INT $
         BOOL
     ...
     (=>
      (= tmp%1 (exists ((x$ Poly)) (!
         (and
          ...
          (and
           (test_crate!f.? x$)
           (vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. s@) x$)
         ))
         :pattern ((test_crate!f.? x$))
         :qid user_test_crate__test_witness_7
         :skolemid skolem_user_test_crate__test_witness_7
      )))
      (and
       (=>
        %%location_label%%0
       ...
       (=>
        tmp%1
        (=>
         (= tmp%2 (vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. s@) (vstd!set.impl&%0.choose.?
            $ INT (Poly%vstd!set.Set<int.>. s@)
         )))
         (=>
          %%location_label%%1
          tmp%2
