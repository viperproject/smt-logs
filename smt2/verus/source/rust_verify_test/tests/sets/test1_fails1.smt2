...
(declare-const fuel%test_crate!set_map. FuelId)
...
(assert
 (distinct fuel%vstd!set.impl&%0.filter. fuel%vstd!set.axiom_set_new. fuel%vstd!set.axiom_set_intersect.
  fuel%vstd!set.axiom_set_ext_equal. fuel%vstd!set.axiom_set_ext_equal_deep. fuel%test_crate!set_map.
...
;; Function-Decl test_crate::set_map
(declare-fun test_crate!set_map.? (Dcr Type Poly Poly) Poly)
...
(assert
 (=>
  ...
  (forall ((A&. Dcr) (A& Type) (s1! Poly) (s2! Poly) (a! Poly)) (!
    ...
    :qid user_vstd__set__axiom_set_intersect_1
    :skolemid skolem_user_vstd__set__axiom_set_intersect_1
))))
...
;; Function-Axioms test_crate::set_map
(assert
 (fuel_bool_default fuel%test_crate!set_map.)
)
 ...
 (forall ((%%hole%%0 Dcr) (%%hole%%1 Type) (%%hole%%2 Poly) (%%hole%%3 Type) (%%hole%%4
   ...
   (= (%%apply%%0 (%%lambda%%0 %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3 %%hole%%4 %%hole%%5
      %%hole%%6 %%hole%%7
       ...
       :qid user_test_crate__set_map_5
       :skolemid skolem_user_test_crate__set_map_5
   ))))
...
(assert
 (=>
  (fuel_bool fuel%test_crate!set_map.)
  (forall ((A&. Dcr) (A& Type) (s! Poly) (f! Poly)) (!
    (= (test_crate!set_map.? A&. A& s! f!) (vstd!set.impl&%0.new.? A&. A& $ (TYPE%fun%1.
       A&. A& $ BOOL
      ) (Poly%fun%1. (mk_fun (%%lambda%%0 A&. A& s! A& A&. A& s! (%Poly%fun%1. f!))))
    ))
    :pattern ((test_crate!set_map.? A&. A& s! f!))
    :qid internal_test_crate!set_map.?_definition
    :skolemid skolem_internal_test_crate!set_map.?_definition
))))
(assert
 (forall ((A&. Dcr) (A& Type) (s! Poly) (f! Poly)) (!
   (=>
    (and
     ...
     (has_type f! (TYPE%fun%1. A&. A& A&. A&))
    )
    (has_type (test_crate!set_map.? A&. A& s! f!) (TYPE%vstd!set.Set. A&. A&))
   )
   :pattern ((test_crate!set_map.? A&. A& s! f!))
   :qid internal_test_crate!set_map.?_pre_post_definition
   :skolemid skolem_internal_test_crate!set_map.?_pre_post_definition
)))

;; Function-Def test_crate::test_set
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/sets-a55b6df8c4f72a7b-test1_fails1/test.rs:21:7: 21:20 (#0)
...
(push)
 ...
 (declare-const nonneg@ vstd!set.Set<int.>.)
 (declare-const pos1@ vstd!set.Set<int.>.)
 (declare-const pos2@ vstd!set.Set<int.>.)
 (assert
  fuel_defaults
 )
 ...
 (assert
  (not (=>
    (= nonneg@ (%Poly%vstd!set.Set<int.>. (vstd!set.impl&%0.new.? $ INT $ (TYPE%fun%1. $
        INT $ BOOL
       ) (Poly%fun%1. (mk_fun (%%lambda%%1 0)))
    )))
    (=>
     (= tmp%1 (forall ((i$ Poly)) (!
        (=>
         (has_type i$ INT)
         (= (vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. nonneg@) i$) (>= (%I
            i$
           ) 0
        )))
        :pattern ((vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. nonneg@) i$))
        :qid user_test_crate__test_set_6
        :skolemid skolem_user_test_crate__test_set_6
     )))
     (and
      (=>
       %%location_label%%0
      ...
      (=>
       tmp%1
       (=>
        (= pos1@ (%Poly%vstd!set.Set<int.>. (vstd!set.impl&%0.filter.? $ INT $ (TYPE%fun%1. $
            INT $ BOOL
           ) (Poly%vstd!set.Set<int.>. nonneg@) (Poly%fun%1. (mk_fun (%%lambda%%2 0)))
        )))
        (=>
         (= tmp%2 (forall ((i$ Poly)) (!
            (=>
             (has_type i$ INT)
             (= (vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. pos1@) i$) (> (%I i$)
               0
            )))
            :pattern ((vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. pos1@) i$))
            :qid user_test_crate__test_set_7
            :skolemid skolem_user_test_crate__test_set_7
         )))
         (and
          (=>
           %%location_label%%1
          ...
          (=>
           tmp%2
           (=>
            (= pos2@ (%Poly%vstd!set.Set<int.>. (test_crate!set_map.? $ INT (Poly%vstd!set.Set<int.>.
                nonneg@
               ) (Poly%fun%1. (mk_fun (%%lambda%%3 1)))
            )))
            (and
             (=>
              (has_type i@ INT)
              (=>
               %%location_label%%2
               (= (vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. pos2@) i@) (> (%I i@)
                 0
             ))))
             (=>
              (forall ((i$ Poly)) (!
                ...
                :pattern ((vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. pos2@) i$))
                :qid user_test_crate__test_set_8
                :skolemid skolem_user_test_crate__test_set_8
              ))
              (=>
               (= tmp%3 (forall ((i$ Poly)) (!
                  (=>
                  ...
                  :pattern ((vstd!set.impl&%0.contains.? $ INT (Poly%vstd!set.Set<int.>. pos2@) i$))
                  :qid user_test_crate__test_set_9
                  :skolemid skolem_user_test_crate__test_set_9
               )))
               (and
                (=>
                 %%location_label%%3
                ...
                (=>
                 tmp%3
                 (=>
                  (= tmp%4 (ext_eq false (TYPE%vstd!set.Set. $ INT) (Poly%vstd!set.Set<int.>. pos1@)
                    (Poly%vstd!set.Set<int.>. pos2@)
                  ))
                  (and
                   (=>
                    %%location_label%%4
                   ...
                   (=>
                    tmp%4
                    (=>
                     (= tmp%5 (ext_eq false (TYPE%vstd!set.Set. $ INT) (Poly%vstd!set.Set<int.>. pos1@)
                       (Poly%vstd!set.Set<int.>. pos2@)
                     ))
                     (=>
                      %%location_label%%5
...
;; Function-Recommends test_crate::test_set
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/sets-a55b6df8c4f72a7b-test1_fails1/test.rs:21:7: 21:20 (#0)
(get-info :all-statistics)
(push)
 (declare-const no%param Int)
 ...
 (assert
  (forall ((%%hole%%0 Int) (i$ Poly)) (!
    (= (%%apply%%0 (%%lambda%%4 %%hole%%0) i$) (B (>= (%I i$) %%hole%%0)))
    :pattern ((%%apply%%0 (%%lambda%%4 %%hole%%0) i$))
 )))
 (declare-fun %%lambda%%5 (Int) %%Function%%)
 (assert
  (forall ((%%hole%%0 Int) (i$ Poly)) (!
    (= (%%apply%%0 (%%lambda%%5 %%hole%%0) i$) (B (> (%I i$) %%hole%%0)))
    :pattern ((%%apply%%0 (%%lambda%%5 %%hole%%0) i$))
 )))
 (declare-fun %%lambda%%6 (Int) %%Function%%)
 (assert
  (forall ((%%hole%%0 Int) (i$ Poly)) (!
    (= (%%apply%%0 (%%lambda%%6 %%hole%%0) i$) (I (Add (%I i$) %%hole%%0)))
