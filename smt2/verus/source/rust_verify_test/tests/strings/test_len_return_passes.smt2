...
;; Function-Specs vstd::string::impl&%1::unicode_len
...
;; Function-Specs test_crate::test_len_return_passes
(declare-fun ens%test_crate!test_len_return_passes. (Int Int) Bool)
(assert
 (forall ((no%param Int) (ret! Int)) (!
   (= (ens%test_crate!test_len_return_passes. no%param ret!) (and
     (uInv SZ ret!)
     (= ret! 4)
   ))
   :pattern ((ens%test_crate!test_len_return_passes. no%param ret!))
   :qid internal_ens__test_crate!test_len_return_passes._definition
   :skolemid skolem_internal_ens__test_crate!test_len_return_passes._definition
)))

;; Function-Def test_crate::test_len_return_passes
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/strings-36081f4fa46d4460-test_len_return_passes/test.rs:16:1: 16:47 (#0)
...
(push)
 ...
 (assert
  (not (=>
    (and
     (= (str%strslice_is_ascii (str%new_strlit 5855249015249778634244652663434774556905419015680469773767024516615812226980129750125732631002653384719831667902420569035408758932707447157254901526889176))
      true
     )
     (= (str%strslice_len (str%new_strlit 5855249015249778634244652663434774556905419015680469773767024516615812226980129750125732631002653384719831667902420569035408758932707447157254901526889176))
      4
     )
     (and
      (= (str%strslice_get_char (str%new_strlit 5855249015249778634244652663434774556905419015680469773767024516615812226980129750125732631002653384719831667902420569035408758932707447157254901526889176)
        0
       ) 97
      )
      (= (str%strslice_get_char (str%new_strlit 5855249015249778634244652663434774556905419015680469773767024516615812226980129750125732631002653384719831667902420569035408758932707447157254901526889176)
    ...
    (=>
     (ens%vstd!string.new_strlit. (str%new_strlit 5855249015249778634244652663434774556905419015680469773767024516615812226980129750125732631002653384719831667902420569035408758932707447157254901526889176)
      tmp%1
     )
     (=>
      (ens%vstd!string.impl&%1.unicode_len. tmp%1 tmp%2)
      (=>
       (= ret! tmp%2)
       (=>
        %%location_label%%0
