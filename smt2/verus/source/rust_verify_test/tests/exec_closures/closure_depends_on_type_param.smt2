...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/exec_closures-b11772a2e0912ad7-closure_depends_on_type_param/test.rs:17:1: 17:23 (#0)
(get-info :all-statistics)
(push)
 (declare-const T&. Dcr)
 (declare-const T& Type)
 (declare-const some_t! Poly)
 (declare-const t~6 Poly)
 (declare-const s~15 Poly)
 (declare-const tmp%%1 anonymous_closure%8.)
 (declare-const tmp%1 tuple%1.)
  ...
  (not (and
    ...
    (=>
     (and
      (forall ((tmp%%$ Poly)) (!
        (=>
         (has_type tmp%%$ (TYPE%tuple%1. T&. T&))
         (=>
          (let
           ((t$ (tuple%1./tuple%1/0 (%Poly%tuple%1. tmp%%$))))
           true
          )
          (closure_req TYPE%anonymous_closure%8. $ (TYPE%tuple%1. T&. T&) (Poly%anonymous_closure%8.
            tmp%%1
           ) tmp%%$
        )))
        :pattern ((closure_req TYPE%anonymous_closure%8. $ (TYPE%tuple%1. T&. T&) (Poly%anonymous_closure%8.
           tmp%%1
          ) tmp%%$
        ))
      ...
      (forall ((tmp%%$ Poly) (s$ Poly)) (!
        (=>
         ...
         (=>
          (closure_ens TYPE%anonymous_closure%8. $ (TYPE%tuple%1. T&. T&) (Poly%anonymous_closure%8.
            tmp%%1
           ) tmp%%$ s$
          )
          (let
        ...
        :pattern ((closure_ens TYPE%anonymous_closure%8. $ (TYPE%tuple%1. T&. T&) (Poly%anonymous_closure%8.
           tmp%%1
          ) tmp%%$ s$
        ))
        :qid user_test_crate__test1_1
        :skolemid skolem_user_test_crate__test1_1
     )))
     (=>
      (= f@ tmp%%1)
      (=>
       (= tmp%1 (tuple%1./tuple%1 some_t!))
       (and
        (=>
         %%location_label%%1
         (req%vstd!pervasive.exec_nonstatic_call. $ (TYPE%tuple%1. T&. T&) T&. T& (REF $) TYPE%anonymous_closure%8.
          (Poly%anonymous_closure%8. f@) (Poly%tuple%1. tmp%1)
        ))
        (=>
         (ens%vstd!pervasive.exec_nonstatic_call. $ (TYPE%tuple%1. T&. T&) T&. T& (REF $) TYPE%anonymous_closure%8.
          (Poly%anonymous_closure%8. f@) (Poly%tuple%1. tmp%1) r@
         )
         (=>
          (= tmp%2 (= r@ some_t!))
          (=>
