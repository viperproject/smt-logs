...
;; MODULE 'module vstd::arithmetic::internals::mul_internals, function crate::vstd::arithmetic::internals::mul_internals::lemma_mul_distributes_plus'
...
(assert
 (distinct fuel%vstd.arithmetic.internals.mul_internals.lemma_mul_distributes_plus.
...
;; Function-Def crate::vstd::arithmetic::internals::mul_internals::lemma_mul_distributes_plus
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/basic-cf562145df495703-test_is_core/../../../../vstd/arithmetic/internals/mul_internals.rs:107:1: 107:74 (#0)
...
(push)
 ...
 (assert
  (not (=>
    ...
    (and
     (=>
      (= f1@ (mk_fun (%%lambda%%0 x@ z@ z@ (Mul x@ z@))))
      (=>
       (= tmp%1 (%B (%%apply%%0 f1@ (I 0))))
       (and
        (=>
         %%location_label%%0
         tmp%1
         ...
         (and
          (=>
           ...
           (=>
            ...
            (=>
             (= tmp%2 (let
               ((tmp%%$ (Mul (Add (Add x@ (%I i@)) 1) z@)))
               (and
                (= (Mul (Add x@ (Add (%I i@) 1)) z@) tmp%%$)
                (= tmp%%$ (Add (Mul (Add x@ (%I i@)) z@) z@))
             )))
             (and
              (=>
               %%location_label%%1
              ...
              (=>
               ...
               (=>
                %%location_label%%2
                (%B (%%apply%%0 f1@ (I (vstd.math.add.? i@ (I 1)))))
          ))))))
          (=>
           (forall ((i$ Poly)) (!
             (=>
             ...
             :pattern ((%%apply%%0 f1@ i$) (%%apply%%0 f1@ (I (vstd.math.add.? i$ (I 1)))))
           ...
           (and
            (=>
             ...
             (=>
              ...
              (=>
               (= tmp%3 (let
                 ((tmp%%$ (Mul (Sub (Add x@ (%I i$1@)) 1) z@)))
                 (and
                  (= (Mul (Add x@ (Sub (%I i$1@) 1)) z@) tmp%%$)
                  (= tmp%%$ (Sub (Mul (Add x@ (%I i$1@)) z@) z@))
               )))
               (and
                (=>
                 %%location_label%%3
                ...
                (=>
                 ...
                 (=>
                  %%location_label%%4
                  (%B (%%apply%%0 f1@ (I (vstd.math.sub.? i$1@ (I 1)))))
            ))))))
            (=>
             (forall ((i$ Poly)) (!
               (=>
                ...
                (=>
                 ...
                 (%B (%%apply%%0 f1@ (I (vstd.math.sub.? i$ (I 1)))))
               ))
               :pattern ((%%apply%%0 f1@ i$) (%%apply%%0 f1@ (I (vstd.math.sub.? i$ (I 1)))))
               :qid user_crate__vstd__arithmetic__internals__mul_internals__lemma_mul_distributes_plus_7
               :skolemid skolem_user_crate__vstd__arithmetic__internals__mul_internals__lemma_mul_distributes_plus_7
             ))
             (and
              (=>
               %%location_label%%5
               (req%vstd.arithmetic.internals.mul_internals.lemma_mul_induction. f1@)
              )
              (=>
               (ens%vstd.arithmetic.internals.mul_internals.lemma_mul_induction. f1@)
               (=>
                (= tmp%4 (%B (%%apply%%0 f1@ (I y@))))
                (and
                 (=>
                  %%location_label%%6
                  tmp%4
...
(assert
 (=>
  ...
  (forall ((x! Int) (y! Int) (z! Int)) (!
    (= (Mul (Add x! y!) z!) (Add (Mul x! z!) (Mul y! z!)))
    :pattern ((Mul (Add x! y!) z!))
    :qid user_crate__vstd__arithmetic__internals__mul_internals__lemma_mul_distributes_plus_5
    :skolemid skolem_user_crate__vstd__arithmetic__internals__mul_internals__lemma_mul_distributes_plus_5
))))
