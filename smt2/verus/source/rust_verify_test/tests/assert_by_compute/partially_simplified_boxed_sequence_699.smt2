...
(assert
 (distinct fuel%vstd!array.array_view. fuel%vstd!array.impl&%0.view. fuel%vstd!array.impl&%2.spec_index.
  ...
  fuel%vstd!seq.axiom_seq_ext_equal. fuel%vstd!seq.axiom_seq_ext_equal_deep. fuel%vstd!seq_lib.impl&%0.filter_lemma.
 ...
 (=>
  ...
  (forall ((A&. Dcr) (A& Type) (self! Poly) (needle! Poly)) (!
    (= (vstd!seq_lib.impl&%0.contains.? A&. A& self! needle!) (exists ((i$ Poly)) (!
       ...
       :pattern ((vstd!seq.Seq.index.? A&. A& self! i$))
       :qid user_vstd__seq_lib__impl&%0__contains_14
       :skolemid skolem_user_vstd__seq_lib__impl&%0__contains_14
    )))
     ...
     (and
      (and
       (forall ((i$ Poly)) (!
         ...
         :qid user_vstd__seq_lib__impl&%0__filter_lemma_15
         :skolemid skolem_user_vstd__seq_lib__impl&%0__filter_lemma_15
       ))
       (forall ((i$ Poly)) (!
         (=>
          (has_type i$ INT)
         ...
         :qid user_vstd__seq_lib__impl&%0__filter_lemma_16
         :skolemid skolem_user_vstd__seq_lib__impl&%0__filter_lemma_16
      )))
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/assert_by_compute-161a97ac6c96a80b-partially_simplified_boxed_sequence_699/test.rs:20:7: 20:16 (#0)
(get-info :all-statistics)
(push)
 (declare-const no%param Int)
 (declare-const s@ vstd!seq.Seq<int.>.)
 (declare-const even@ vstd!seq.Seq<int.>.)
 (assert
  fuel_defaults
 )
 ...
 (assert
  (forall ((%%hole%%0 Int) (%%hole%%1 Int) (x$ Poly)) (!
    (= (%%apply%%0 (%%lambda%%1 %%hole%%0 %%hole%%1) x$) (B (= (EucMod (%I x$) %%hole%%0)
       %%hole%%1
    )))
    :pattern ((%%apply%%0 (%%lambda%%1 %%hole%%0 %%hole%%1) x$))
 )))
 ...
 (assert
  (not (=>
    (= s@ (%Poly%vstd!seq.Seq<int.>. (vstd!view.View.view.? $ (ARRAY $ INT $ (CONST_INT 5))
       (array_new $ INT 5 (%%array%%0 (I 1) (I 2) (I 3) (I 4) (I 5)))
    )))
    (=>
     (= even@ (%Poly%vstd!seq.Seq<int.>. (vstd!seq_lib.impl&%0.filter.? $ INT (Poly%vstd!seq.Seq<int.>.
         s@
        ) (Poly%fun%1. (mk_fun (%%lambda%%1 2 0)))
     )))
     (=>
      %%location_label%%0
      (ext_eq false (TYPE%vstd!seq.Seq. $ INT) (Poly%vstd!seq.Seq<int.>. even@) (vstd!array.array_view.?
        $ INT $ (CONST_INT 2) (array_new $ INT 2 (%%array%%1 (I 2) (I 4)))
 )))))))
 (get-info :version)
...
(push)
 ...
 (declare-fun %%array%%2 (Poly Poly Poly Poly Poly) %%Function%%)
 (assert
  (forall ((%%hole%%0 Poly) (%%hole%%1 Poly) (%%hole%%2 Poly) (%%hole%%3 Poly) (%%hole%%4
     Poly
    )
    ...
    (let
     ((%%x%% (%%array%%2 %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3 %%hole%%4)))
     (and
      (= (%%apply%%1 %%x%% 0) %%hole%%0)
      (= (%%apply%%1 %%x%% 1) %%hole%%1)
      (= (%%apply%%1 %%x%% 2) %%hole%%2)
    ...
    :pattern ((%%array%%2 %%hole%%0 %%hole%%1 %%hole%%2 %%hole%%3 %%hole%%4))
    :qid __AIR_ARRAY_QID__
    :skolemid skolem___AIR_ARRAY_QID__
 )))
 ...
 (assert
  (forall ((%%hole%%0 Int) (%%hole%%1 Int) (x$ Poly)) (!
    (= (%%apply%%0 (%%lambda%%2 %%hole%%0 %%hole%%1) x$) (B (= (EucMod (%I x$) %%hole%%0)
       %%hole%%1
    )))
    :pattern ((%%apply%%0 (%%lambda%%2 %%hole%%0 %%hole%%1) x$))
 )))
 (declare-fun %%array%%3 (Poly Poly) %%Function%%)
 (assert
  (forall ((%%hole%%0 Poly) (%%hole%%1 Poly)) (!
    (let
     ((%%x%% (%%array%%3 %%hole%%0 %%hole%%1)))
     (and
      (= (%%apply%%1 %%x%% 0) %%hole%%0)
      (= (%%apply%%1 %%x%% 1) %%hole%%1)
    ))
    :pattern ((%%array%%3 %%hole%%0 %%hole%%1))
    :qid __AIR_ARRAY_QID__
    :skolemid skolem___AIR_ARRAY_QID__
 )))
 (assert
