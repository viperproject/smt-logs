...
(assert
 (distinct fuel%test_crate!X.impl&%1.is_add. fuel%test_crate!impl&%0.arrow_Bar_0. fuel%test_crate!impl&%0.arrow_Qax_0.
...
;; Datatypes
(declare-datatypes ((test_crate!X.State. 0) (test_crate!X.Step. 0) (test_crate!Foo.
   0
  ) (tuple%0. 0)
...
(declare-fun req%test_crate!test. (test_crate!X.Step.) Bool)
(declare-const %%global_location_label%%0 Bool)
(assert
 (forall ((s! test_crate!X.Step.)) (!
   (= (req%test_crate!test. s!) (=>
     %%global_location_label%%0
     (is-test_crate!X.Step./add s!)
   ))
   :pattern ((req%test_crate!test. s!))
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/state_machines-c25f1c939512e7ab-step_is_variant/test.rs:61:7: 61:26 (#0)
(get-info :all-statistics)
(push)
 (declare-const s! test_crate!X.Step.)
 (assert
  fuel_defaults
 )
 (assert
 ...
 (assert
  (not (or
    (and
     ...
     (=>
      (not (is-test_crate!X.Step./add s!))
      (or
       (and
        (=>
         (is-test_crate!X.Step./sub s!)
         (=>
          %%location_label%%1
          false
        ))
        (=>
         (not (is-test_crate!X.Step./sub s!))
         %%switch_label%%1
       ))
       (and
        (not %%switch_label%%1)
