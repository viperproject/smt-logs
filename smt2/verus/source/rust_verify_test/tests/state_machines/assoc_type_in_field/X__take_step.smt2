...
(declare-datatypes ((test_crate!X.State. 0) (tuple%0. 0)) (((test_crate!X.State./State
    (test_crate!X.State./State/?m Poly)
   )
  ) ((tuple%0./tuple%0))
))
...
(declare-fun req%test_crate!X.take_step.initialize. (Dcr Type Poly) Bool)
(declare-const %%global_location_label%%2 Bool)
(assert
 (forall ((T&. Dcr) (T& Type) (m! Poly)) (!
   (= (req%test_crate!X.take_step.initialize. T&. T& m!) (=>
     %%global_location_label%%2
     (test_crate!X.impl&%10.initialize_enabled.? T&. T& m!)
   ))
   :pattern ((req%test_crate!X.take_step.initialize. T&. T& m!))
...
(declare-fun ens%test_crate!X.take_step.initialize. (Dcr Type Poly test_crate!X.State.)
 Bool
)
(assert
 (forall ((T&. Dcr) (T& Type) (m! Poly) (post! test_crate!X.State.)) (!
   (= (ens%test_crate!X.take_step.initialize. T&. T& m! post!) (and
     (has_type (Poly%test_crate!X.State. post!) (TYPE%test_crate!X.State. T&. T&))
     (and
      (test_crate!X.impl&%10.initialize.? T&. T& (Poly%test_crate!X.State. post!) m!)
      (test_crate!X.impl&%10.invariant.? T&. T& (Poly%test_crate!X.State. post!))
   )))
   :pattern ((ens%test_crate!X.take_step.initialize. T&. T& m! post!))
