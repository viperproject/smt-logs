...
(assert
 (distinct fuel%vstd!map.impl&%0.spec_index. fuel%vstd!map.axiom_map_index_decreases_finite.
  ...
  fuel%test_crate!X.impl&%10.initialize. fuel%vstd!array.group_array_axioms. fuel%vstd!map.group_map_axioms.
...
(declare-fun req%test_crate!X.show.initialize. (Dcr Type test_crate!X.State. Poly)
 Bool
)
(declare-const %%global_location_label%%2 Bool)
(assert
 (forall ((T&. Dcr) (T& Type) (post! test_crate!X.State.) (m! Poly)) (!
   (= (req%test_crate!X.show.initialize. T&. T& post! m!) (=>
     %%global_location_label%%2
     (test_crate!X.impl&%10.initialize.? T&. T& (Poly%test_crate!X.State. post!) m!)
   ))
   :pattern ((req%test_crate!X.show.initialize. T&. T& post! m!))
...
(declare-fun ens%test_crate!X.show.initialize. (Dcr Type test_crate!X.State. Poly)
 Bool
)
(assert
 (forall ((T&. Dcr) (T& Type) (post! test_crate!X.State.) (m! Poly)) (!
   (= (ens%test_crate!X.show.initialize. T&. T& post! m!) (test_crate!X.impl&%10.init.?
     T&. T& (Poly%test_crate!X.State. post!)
   ))
   :pattern ((ens%test_crate!X.show.initialize. T&. T& post! m!))
