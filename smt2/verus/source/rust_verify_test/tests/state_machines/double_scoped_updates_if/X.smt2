...
(assert
 (distinct fuel%vstd!map.impl&%0.spec_index. fuel%vstd!map.axiom_map_index_decreases_finite.
  ...
  fuel%test_crate!X.impl&%0.arrow_3. fuel%test_crate!X.impl&%0.arrow_4. fuel%test_crate!X.impl&%0.arrow_2.
...
(declare-fun test_crate!X.Step./tr/4 (test_crate!X.Step.) Bool)
...
(declare-fun test_crate!X.impl&%0.arrow_tr_4.? (Poly) Bool)
...
(declare-fun test_crate!X.impl&%1.get_tr_4.? (Poly) Bool)
        ...
        (let
         ...
         (let
          ...
          (let
           ...
           (test_crate!X.impl&%4.tr.? pre! post! (B x$) (B y$) (B z$) (B w$) (B v$))
    )))))))
       ...
       (let
        ...
        (let
         ...
         (let
          ...
          (let
           ((v$ (test_crate!X.Step./tr/4 (%Poly%test_crate!X.Step. step!))))
           (test_crate!X.impl&%4.tr_strong.? pre! post! (B x$) (B y$) (B z$) (B w$) (B v$))
    )))))))
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/state_machines-c25f1c939512e7ab-double_scoped_updates_if/test.rs:35:1: 63:5 (#42)
          ...
          (=>
           ...
           (=>
            (= update_tmp_m$1@ (%Poly%vstd!map.Map<int./int.>. (vstd!map.impl&%0.insert.? $ INT
               $ INT (Poly%vstd!map.Map<int./int.>. update_tmp_m@) (I 0) (I 10)
            )))
            %%switch_label%%1
         )))
         (=>
