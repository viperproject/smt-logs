...
(assert
 (distinct fuel%test_crate!Y.impl&%13.add1. fuel%test_crate!impl&%0.arrow_Bar_0. fuel%test_crate!impl&%0.arrow_Qax_0.
...
(declare-sort test_crate!Y.recursing. 0)
...
(assert
 (forall ((x test_crate!Y.recursing.)) (!
   (has_type (Poly%test_crate!Y.recursing. x) TYPE%test_crate!Y.recursing.)
   :pattern ((has_type (Poly%test_crate!Y.recursing. x) TYPE%test_crate!Y.recursing.))
   :qid internal_test_crate__Y__recursing_has_type_always_definition
   :skolemid skolem_internal_test_crate__Y__recursing_has_type_always_definition
)))
(assert
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/state_machines-c25f1c939512e7ab-use_self_type/test.rs:78:1: 78:19 (#0)
(get-info :all-statistics)
(push)
 (declare-const no%param Int)
 (declare-const tmp%1 test_crate!Y.State.)
 ...
 (declare-const verus_tmp_x_tok@ test_crate!Y.x.)
 (declare-const verus_tmp_r_tok@ test_crate!Y.recursing.)
 (declare-const inst@ test_crate!Y.Instance.)
 (declare-const x_tok@0 test_crate!Y.x.)
 (declare-const r_tok@0 test_crate!Y.recursing.)
 (assert
  fuel_defaults
 )
 (declare-const x_tok@1 test_crate!Y.x.)
 (declare-const r_tok@1 test_crate!Y.recursing.)
 (declare-const r_tok@2 test_crate!Y.recursing.)
 ...
 (assert
  (not (=>
    (= tmp%1 (test_crate!Y.State./State (%I (I 5)) (%Poly%core!option.Option. (Poly%core!option.Option.
        core!option.Option./None
    ))))
    (=>
     (ens%test_crate!Y.impl&%10.ini. tmp%1 tmp%%@)
     (=>
      (= verus_tmp_inst@ (%Poly%test_crate!Y.Instance. (tuple%3./tuple%3/0 (%Poly%tuple%3.
          (Poly%tuple%3. tmp%%@)
      ))))
      (=>
       (= verus_tmp_x_tok@ (%Poly%test_crate!Y.x. (tuple%3./tuple%3/1 (%Poly%tuple%3. (Poly%tuple%3.
            tmp%%@
       )))))
       (=>
        (= verus_tmp_r_tok@ (%Poly%test_crate!Y.recursing. (tuple%3./tuple%3/2 (%Poly%tuple%3.
            (Poly%tuple%3. tmp%%@)
        ))))
        (=>
         (= inst@ verus_tmp_inst@)
         (=>
          (= x_tok@0 verus_tmp_x_tok@)
          (=>
           (= r_tok@0 verus_tmp_r_tok@)
           (and
            (=>
             %%location_label%%0
             (req%test_crate!Y.impl&%10.tr. inst@ 19 x_tok@0)
            )
            (=>
             (ens%test_crate!Y.impl&%10.tr. inst@ 19 x_tok@0 x_tok@1)
             (=>
              (= tmp%2 (= (%I (vstd!tokens.ValueToken.value.? $ TYPE%test_crate!Y.x. $ INT (Poly%test_crate!Y.x.
                   x_tok@1
                 ))
                ) 20
              ))
              (and
               ...
               (=>
                tmp%2
                (=>
                 (= tmp%3 core!option.Option./None)
                 (and
                  (=>
                   %%location_label%%2
                   (req%test_crate!Y.impl&%10.tr2. inst@ tmp%3 r_tok@0)
                  )
                  (=>
                   (ens%test_crate!Y.impl&%10.tr2. inst@ tmp%3 r_tok@0 r_tok@1)
                   (=>
                    (= tmp%4 (ext_eq false (TYPE%core!option.Option. (BOX $ ALLOCATOR_GLOBAL $) TYPE%test_crate!Y.State.)
                      (Poly%core!option.Option. core!option.Option./None) (vstd!tokens.ValueToken.value.?
                       $ TYPE%test_crate!Y.recursing. $ (TYPE%core!option.Option. (BOX $ ALLOCATOR_GLOBAL
                         $
                        ) TYPE%test_crate!Y.State.
                       ) (Poly%test_crate!Y.recursing. r_tok@1)
                    )))
                    (and
                     (=>
                      %%location_label%%3
                     ...
                     (=>
                      ...
                      (and
                       (=>
                        %%location_label%%4
                        (req%test_crate!Y.impl&%10.tr3. inst@ r_tok@1)
                       )
                       (=>
                        (ens%test_crate!Y.impl&%10.tr3. inst@ r_tok@1 r_tok@2)
                        (=>
                         (= tmp%5 (ext_eq false (TYPE%core!option.Option. (BOX $ ALLOCATOR_GLOBAL $) TYPE%test_crate!Y.State.)
                           (Poly%core!option.Option. core!option.Option./None) (vstd!tokens.ValueToken.value.?
                            $ TYPE%test_crate!Y.recursing. $ (TYPE%core!option.Option. (BOX $ ALLOCATOR_GLOBAL
                              $
                             ) TYPE%test_crate!Y.State.
                            ) (Poly%test_crate!Y.recursing. r_tok@2)
                         )))
                         (=>
                          %%location_label%%5
                          tmp%5
