...
(assert
 (distinct fuel%vstd!map.impl&%0.spec_index. fuel%vstd!map.axiom_map_index_decreases_finite.
  ...
  fuel%test_crate!A.impl&%10.initialize. fuel%test_crate!A.impl&%10.initialize_enabled.
  fuel%test_crate!A.impl&%10.invariant. fuel%vstd!array.group_array_axioms. fuel%vstd!map.group_map_axioms.
...
(declare-datatypes ((test_crate!A.State. 0) (test_crate!Y. 0) (tuple%0. 0)) (((test_crate!A.State./State
    (test_crate!A.State./State/?x vstd!map.Map<int./test_crate!Y.>.)
   )
  ) ((test_crate!Y./Y)) ((tuple%0./tuple%0))
))
...
;; Function-Specs test_crate::A::take_step::initialize
(declare-fun req%test_crate!A.take_step.initialize. (Int) Bool)
(declare-const %%global_location_label%%2 Bool)
(assert
 (forall ((no%param Int)) (!
   (= (req%test_crate!A.take_step.initialize. no%param) (=>
     %%global_location_label%%2
     (test_crate!A.impl&%10.initialize_enabled.? (I 0))
   ))
   :pattern ((req%test_crate!A.take_step.initialize. no%param))
   :qid internal_req__test_crate!A.take_step.initialize._definition
   :skolemid skolem_internal_req__test_crate!A.take_step.initialize._definition
)))
(declare-fun ens%test_crate!A.take_step.initialize. (Int test_crate!A.State.) Bool)
(assert
 (forall ((no%param Int) (post! test_crate!A.State.)) (!
   (= (ens%test_crate!A.take_step.initialize. no%param post!) (and
     (test_crate!A.impl&%10.initialize.? (Poly%test_crate!A.State. post!))
     (test_crate!A.impl&%10.invariant.? (Poly%test_crate!A.State. post!))
   ))
   :pattern ((ens%test_crate!A.take_step.initialize. no%param post!))
   :qid internal_ens__test_crate!A.take_step.initialize._definition
   :skolemid skolem_internal_ens__test_crate!A.take_step.initialize._definition
)))
