...
(assert
 (distinct fuel%vstd!std_specs.option.impl&%0.is_Some. fuel%vstd!std_specs.option.impl&%0.get_Some_0.
  ...
  fuel%test_crate!Y.impl&%1.is_tr_have. fuel%test_crate!Y.impl&%1.is_tr_add_gen. fuel%test_crate!Y.impl&%1.get_tr_add_gen_0.
  ...
  fuel%test_crate!Y.impl&%11.init_false. fuel%test_crate!Y.impl&%11.init_false_enabled.
...
(declare-datatypes ((core!option.Option. 0) (vstd!tokens.InstanceId. 0) (test_crate!Y.State.
  ...
  ((test_crate!Y.State./State (test_crate!Y.State./State/?b Bool))) ((test_crate!Y.Step./tr_add)
   (test_crate!Y.Step./tr_have) (test_crate!Y.Step./tr_add_gen (test_crate!Y.Step./tr_add_gen/?0
     Bool
    )
   ) (test_crate!Y.Step./tr_have_gen (test_crate!Y.Step./tr_have_gen/?0 Bool)) (test_crate!Y.Step./dummy_to_use_type_params
      ...
      (ite
       ...
       (ite
        ...
        (let
         ...
         (test_crate!Y.impl&%11.tr_add_gen.? pre! post! (B x$))
        )
        (and
         ...
         (let
          ...
          (test_crate!Y.impl&%11.tr_have_gen.? pre! post! (B x$))
    ))))))
    ...
    (= (test_crate!Y.impl&%11.next_strong_by.? pre! post! step!) (ite
      ...
      (ite
       ...
       (ite
        ...
        (let
         ((x$ (test_crate!Y.Step./tr_add_gen/0 (%Poly%test_crate!Y.Step. step!))))
         (test_crate!Y.impl&%11.tr_add_gen_strong.? pre! post! (B x$))
        )
        (and
         (is-test_crate!Y.Step./tr_have_gen (%Poly%test_crate!Y.Step. step!))
         (let
          ((x$ (test_crate!Y.Step./tr_have_gen/0 (%Poly%test_crate!Y.Step. step!))))
          (test_crate!Y.impl&%11.tr_have_gen_strong.? pre! post! (B x$))
    ))))))
