...
(assert
 (forall ((x test_crate!A.x.)) (!
   (has_type (Poly%test_crate!A.x. x) TYPE%test_crate!A.x.)
   :pattern ((has_type (Poly%test_crate!A.x. x) TYPE%test_crate!A.x.))
   :qid internal_test_crate__A__x_has_type_always_definition
   :skolemid skolem_internal_test_crate__A__x_has_type_always_definition
)))
(assert
...
(declare-fun ens%test_crate!test. (test_crate!A.x. tuple%2.) Bool)
(assert
 (forall ((t! test_crate!A.x.) (%return! tuple%2.)) (!
   (= (ens%test_crate!test. t! %return!) (has_type (Poly%tuple%2. %return!) (TYPE%tuple%2.
      $ TYPE%test_crate!A.x. $ TYPE%test_crate!A.x.
   )))
...
;; /home/runner/work/smt-logs/smt-logs/verus/verus/source/target/release/test_inputs/state_machines-c25f1c939512e7ab-pers_token_copy/test.rs:50:7: 50:59 (#0)
(get-info :all-statistics)
(push)
 (declare-const %return! tuple%2.)
 (declare-const t! test_crate!A.x.)
 (assert
  fuel_defaults
 )
 (assert
