...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.BitSet__Property)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap $Heap@0)) (and ($IsAlloc intType (int_2_U |n#0@@5|) Tclass._System.nat $Heap@0) (|_module.__default.BitSet#canCall| |n#0@@5|))) (and (=> (= (ControlFlow 0 2) (- 0 4)) ($Is intType (int_2_U (LitInt 0)) Tclass._System.nat)) (=> ($Is intType (int_2_U (LitInt 0)) Tclass._System.nat) (and (=> (= (ControlFlow 0 2) (- 0 3)) (or (not (= (LitInt 2) 0)) (not true))) (=> (or (not (= (LitInt 2) 0)) (not true)) (=> (= (ControlFlow 0 2) (- 0 1)) ($Is intType (int_2_U (Div |n#0@@5| (LitInt 2))) Tclass._System.nat))))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (<= (LitInt 0) |n#0@@5|)) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 5) 2))) anon0_correct)))
