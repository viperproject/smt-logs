...
(declare-fun $LZ () T@U)
(set-info :boogie-vc-id Impl$$_module.__default.plus__O__n)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (|$IsA#_module.Nat| |n#0@@3|)) (=> (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap $Heap@0)) (and (forall ((|$ih#n0#0| T@U) ) (!  (=> (and (and ($Is DatatypeTypeType |$ih#n0#0| Tclass._module.Nat) (U_2_bool (Lit boolType (bool_2_U true)))) (< (DtRank |$ih#n0#0|) (DtRank |n#0@@3|))) (|_module.Nat#Equal| (_module.__default.plus ($LS $LZ) (Lit DatatypeTypeType |#_module.Nat.O|) |$ih#n0#0|) |$ih#n0#0|))
 :qid |SoftwareFoundationsBasicsdfy.347:17|
 :skolemid |741|
 :pattern ( (_module.__default.plus ($LS $LZ) (Lit DatatypeTypeType |#_module.Nat.O|) |$ih#n0#0|))
)) (= (ControlFlow 0 2) (- 0 1)))) (|_module.Nat#Equal| (_module.__default.plus ($LS ($LS $LZ)) (Lit DatatypeTypeType |#_module.Nat.O|) |n#0@@3|) |n#0@@3|)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is DatatypeTypeType |n#0@@3| Tclass._module.Nat) ($IsAlloc DatatypeTypeType |n#0@@3| Tclass._module.Nat $Heap)) (|$IsA#_module.Nat| |n#0@@3|)) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
