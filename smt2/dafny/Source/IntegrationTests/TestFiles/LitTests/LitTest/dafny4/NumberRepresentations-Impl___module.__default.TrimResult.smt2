...
(set-info :boogie-vc-id Impl$$_module.__default.TrimResult)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap $Heap@0)) (and (forall ((|$ih#digits0#0| T@U) ) (!  (=> (and (and ($Is SeqType |$ih#digits0#0| (TSeq TInt)) (U_2_bool (Lit boolType (bool_2_U true)))) (< (|Seq#Rank| |$ih#digits0#0|) (|Seq#Rank| |digits#0@@3|))) (let ((|last#1| (- (|Seq#Length| (_module.__default.trim ($LS $LZ) |$ih#digits0#0|)) 1)))
 (=> (<= (LitInt 0) |last#1|) (or (not (= (U_2_int ($Unbox intType (|Seq#Index| (_module.__default.trim ($LS $LZ) |$ih#digits0#0|) |last#1|))) 0)) (not true)))))
 :qid |NumberRepresentationsdfy.187:18|
 :skolemid |652|
 :pattern ( (_module.__default.trim ($LS $LZ) |$ih#digits0#0|))
)) (= (ControlFlow 0 2) (- 0 1)))) (let ((|last#0| (- (|Seq#Length| (_module.__default.trim ($LS ($LS $LZ)) |digits#0@@3|)) 1)))
 (=> (<= (LitInt 0) |last#0|) (or (not (= (U_2_int ($Unbox intType (|Seq#Index| (_module.__default.trim ($LS ($LS $LZ)) |digits#0@@3|) |last#0|))) 0)) (not true))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is SeqType |digits#0@@3| (TSeq TInt)) ($IsAlloc SeqType |digits#0@@3| (TSeq TInt) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
