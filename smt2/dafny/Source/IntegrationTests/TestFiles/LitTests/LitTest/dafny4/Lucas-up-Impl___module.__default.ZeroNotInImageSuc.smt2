...
(assert (forall ((|l#0| Int) (|l#1| T@U) (|l#2| Int) (|l#3| T@U) (|$y#0| T@U) ) (! (= (U_2_bool (MapType0Select BoxType boolType (|lambda#25| |l#0| |l#1| |l#2| |l#3|) |$y#0|)) (exists ((|x#0@@1| Int) ) (!  (and (and (<= |l#0| |x#0@@1|) (|Set#IsMember| |l#1| ($Box intType (int_2_U |x#0@@1|)))) (= |$y#0| ($Box intType (int_2_U (+ |x#0@@1| |l#2|)))))
 :qid |Lucasupdfy.235:7|
 :skolemid |593|
 :pattern ( (|Set#IsMember| |l#3| ($Box intType (int_2_U |x#0@@1|))))
)))
 :qid |unknown.0:0|
 :skolemid |647|
 :pattern ( (MapType0Select BoxType boolType (|lambda#25| |l#0| |l#1| |l#2| |l#3|) |$y#0|))
)))
...
(set-info :boogie-vc-id Impl$$_module.__default.ZeroNotInImageSuc)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (not (|Set#IsMember| (_module.__default.Suc |S#0@@2|) ($Box intType (int_2_U (LitInt 0))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is SetType |S#0@@2| (TSet Tclass._System.nat)) ($IsAlloc SetType |S#0@@2| (TSet Tclass._System.nat) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
