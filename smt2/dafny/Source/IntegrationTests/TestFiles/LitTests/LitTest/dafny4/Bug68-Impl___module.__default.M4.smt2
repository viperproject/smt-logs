...
(assert  (and (and (and (= (Ctor MapType) 6) (forall ((t0@@2 T@T) (t1 T@T) (val T@U) (m T@U) (x0 T@U) ) (! (= (MapType0Select t0@@2 t1 (MapType0Store t0@@2 t1 m x0 val) x0) val)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (= |m#0@0| (Lit MapType (|Map#Build| |Map#Empty| ($Box SeqType (Lit SeqType (|Seq#Build| (|Seq#Build| |Seq#Empty| ($Box intType (int_2_U (LitInt 10)))) ($Box intType (int_2_U (LitInt 20)))))) ($Box intType (int_2_U (LitInt 33)))))) (= (ControlFlow 0 2) (- 0 1))) (|Set#IsMember| (|Map#Domain| |m#0@0|) ($Box SeqType (|Seq#Build| (|Seq#Build| |Seq#Empty| ($Box intType (int_2_U (LitInt 10)))) ($Box intType (int_2_U (LitInt 20))))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is MapType |m#0| (TMap (TSeq TInt) TInt)) ($IsAlloc MapType |m#0| (TMap (TSeq TInt) TInt) $Heap)) true) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
