...
(set-info :boogie-vc-id CheckWellformed$$_module.__default.mult)
...
(let ((anon10_Then_correct  (=> (= |n#0@@7| (|#_module.Nat.S| |_mcc#0#0|)) (=> (and (and (and ($Is DatatypeTypeType |_mcc#0#0| Tclass._module.Nat) (= |let#0#0#0| |_mcc#0#0|)) (and ($Is DatatypeTypeType |let#0#0#0| Tclass._module.Nat) (= |n'#Z#0@0| |let#0#0#0|))) (and (and ($IsAlloc DatatypeTypeType |m#0@@7| Tclass._module.Nat $Heap) ($IsAlloc DatatypeTypeType |n'#Z#0@0| Tclass._module.Nat $Heap)) (and ($IsAlloc DatatypeTypeType |m#0@@7| Tclass._module.Nat $Heap) (= (ControlFlow 0 4) (- 0 3))))) (or (< (DtRank |n'#Z#0@0|) (DtRank |n#0@@7|)) (and (= (DtRank |n'#Z#0@0|) (DtRank |n#0@@7|)) (< (DtRank |m#0@@7|) (DtRank |m#0@@7|))))))))
(let ((anon9_Else_correct  (=> (or (not (= |n#0@@7| |#_module.Nat.O|)) (not true)) (and (=> (= (ControlFlow 0 6) 4) anon10_Then_correct) (=> (= (ControlFlow 0 6) 5) anon10_Else_correct)))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is DatatypeTypeType |n#0@@7| Tclass._module.Nat) ($Is DatatypeTypeType |m#0@@7| Tclass._module.Nat)) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 8) 7))) anon0_correct))))
