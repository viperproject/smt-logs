(set-option :print-success false)
(set-info :smt-lib-version 2.6)
(set-option :auto_config false)
(set-option :type_check true)
(set-option :smt.qi.eager_threshold 100)
(set-option :smt.delay_units true)
(set-option :model_evaluator.completion true)
(set-option :model.completion true)
(set-option :model.compact false)
(set-option :smt.case_split 3)
(set-option :smt.mbqi false)
(set-option :model.v2 true)
(set-option :pp.bv_literals false)
(set-option :smt.arith.solver 2)
; done setting options


(set-info :category "industrial")
(declare-sort |T@U| 0)
(declare-sort |T@T| 0)
(declare-fun real_pow (Real Real) Real)
(declare-fun UOrdering2 (|T@U| |T@U|) Bool)
(declare-fun UOrdering3 (|T@T| |T@U| |T@U|) Bool)
(declare-fun tickleBool (Bool) Bool)
(assert (and (tickleBool true) (tickleBool false)))
(declare-fun TBool () T@U)
(declare-fun TInt () T@U)
(declare-fun TagBool () T@U)
(declare-fun TagInt () T@U)
(declare-fun TagSet () T@U)
(declare-fun TagMultiSet () T@U)
(declare-fun TagMap () T@U)
(declare-fun alloc () T@U)
(declare-fun |##_System._tuple#2._#Make2| () T@U)
(declare-fun Tagclass._System.Tuple2 () T@U)
(declare-fun Tagclass._module.State () T@U)
(declare-fun Tagclass._module.MyDt () T@U)
(declare-fun |##_module.State.State| () T@U)
(declare-fun |##_module.MyDt.MakeA| () T@U)
(declare-fun |##_module.MyDt.MakeB| () T@U)
(declare-fun |tytagFamily$_tuple#2| () T@U)
(declare-fun tytagFamily$State () T@U)
(declare-fun tytagFamily$MyDt () T@U)
(declare-fun Ctor (T@T) Int)
(declare-fun boolType () T@T)
(declare-fun intType () T@T)
(declare-fun realType () T@T)
(declare-fun bool_2_U (Bool) T@U)
(declare-fun U_2_bool (T@U) Bool)
(declare-fun int_2_U (Int) T@U)
(declare-fun U_2_int (T@U) Int)
(declare-fun real_2_U (Real) T@U)
(declare-fun U_2_real (T@U) Real)
(declare-fun Tag (T@U) T@U)
(declare-fun _System.Tuple2.___hMake2_q (T@U) Bool)
(declare-fun $Is (T@T T@U T@U) Bool)
(declare-fun DatatypeTypeType () T@T)
(declare-fun Tclass._System.Tuple2 (T@U T@U) T@U)
(declare-fun |Set#IsMember| (T@U T@U) Bool)
(declare-fun |Set#Empty| () T@U)
(declare-fun MultiSetType () T@T)
(declare-fun TMultiSet (T@U) T@U)
(declare-fun $IsGoodMultiSet (T@U) Bool)
(declare-fun _module.State.State_q (T@U) Bool)
(declare-fun Tclass._module.State () T@U)
(declare-fun |#_System._tuple#2._#Make2| (T@U T@U) T@U)
(declare-fun $IsBox (T@U T@U) Bool)
(declare-fun |Set#Card| (T@U) Int)
(declare-fun |MultiSet#Multiplicity| (T@U T@U) Int)
(declare-fun |MultiSet#FromSet| (T@U) T@U)
(declare-fun |#_module.MyDt.MakeA| (Int Bool) T@U)
(declare-fun Tclass._module.MyDt () T@U)
(declare-fun LitInt (Int) Int)
(declare-fun Lit (T@T T@U) T@U)
(declare-fun $IsAlloc (T@T T@U T@U T@U) Bool)
(declare-fun $IsGoodHeap (T@U) Bool)
(declare-fun $IsAllocBox (T@U T@U T@U) Bool)
(declare-fun DatatypeCtorId (T@U) T@U)
(declare-fun _module.MyDt.MakeA_q (T@U) Bool)
(declare-fun _module.MyDt.MakeB_q (T@U) Bool)
(declare-fun $Unbox (T@T T@U) T@U)
(declare-fun $Box (T@T T@U) T@U)
(declare-fun |#_module.MyDt.MakeB| (T@U T@U) T@U)
(declare-fun SetType () T@T)
(declare-fun TSet (T@U) T@U)
(declare-fun |#_module.State.State| (T@U) T@U)
(declare-fun DtRank (T@U) Int)
(declare-fun |Map#Domain| (T@U) T@U)
(declare-fun MapType0Select (T@T T@T T@U T@U) T@U)
(declare-fun BoxType () T@T)
(declare-fun |Map#Elements| (T@U) T@U)
(declare-fun MapType0Store (T@T T@T T@U T@U T@U) T@U)
(declare-fun |Map#Equal| (T@U T@U) Bool)
(declare-fun |Map#Card| (T@U) Int)
(declare-fun |Map#Empty| () T@U)
(declare-fun |$IsA#_module.MyDt| (T@U) Bool)
(declare-fun _module.MyDt._h0 (T@U) Bool)
(declare-fun |MultiSet#Equal| (T@U T@U) Bool)
(declare-fun |MultiSet#Card| (T@U) Int)
(declare-fun |MultiSet#Empty| () T@U)
(declare-fun TMap (T@U T@U) T@U)
(declare-fun Inv0_TMap (T@U) T@U)
(declare-fun Inv1_TMap (T@U) T@U)
(declare-fun Tclass._System.Tuple2_0 (T@U) T@U)
(declare-fun Tclass._System.Tuple2_1 (T@U) T@U)
(declare-fun _System.Tuple2._0 (T@U) T@U)
(declare-fun _System.Tuple2._1 (T@U) T@U)
(declare-fun _module.MyDt.x (T@U) Int)
(declare-fun _module.MyDt.s (T@U) T@U)
(declare-fun _module.MyDt.t (T@U) T@U)
(declare-fun Inv0_TSet (T@U) T@U)
(declare-fun Inv0_TMultiSet (T@U) T@U)
(declare-fun _module.State.m (T@U) T@U)
(declare-fun MapType () T@T)
(declare-fun BoxRank (T@U) Int)
(declare-fun MapType1Select (T@T T@T T@T T@U T@U T@U) T@U)
(declare-fun refType () T@T)
(declare-fun FieldType () T@T)
(declare-fun |lambda#0| (T@U T@U T@U Bool) T@U)
(declare-fun MapType0Type (T@T T@T) T@T)
(declare-fun MapType1Store (T@T T@T T@T T@U T@U T@U T@U) T@U)
(declare-fun MapType0TypeInv0 (T@T) T@T)
(declare-fun MapType0TypeInv1 (T@T) T@T)
(declare-fun TagFamily (T@U) T@U)
(declare-fun |Map#Values| (T@U) T@U)
(declare-fun |Map#Items| (T@U) T@U)
(assert  (and (and (and (and (and (and (and (and (= (Ctor boolType) 0) (= (Ctor intType) 1)) (= (Ctor realType) 2)) (forall ((arg0 Bool) ) (! (= (U_2_bool (bool_2_U arg0)) arg0)
 :qid |typeInv:U_2_bool|
 :pattern ( (bool_2_U arg0))
))) (forall ((x T@U) ) (! (= (bool_2_U (U_2_bool x)) x)
 :qid |cast:U_2_bool|
 :pattern ( (U_2_bool x))
))) (forall ((arg0@@0 Int) ) (! (= (U_2_int (int_2_U arg0@@0)) arg0@@0)
 :qid |typeInv:U_2_int|
 :pattern ( (int_2_U arg0@@0))
))) (forall ((x@@0 T@U) ) (! (= (int_2_U (U_2_int x@@0)) x@@0)
 :qid |cast:U_2_int|
 :pattern ( (U_2_int x@@0))
))) (forall ((arg0@@1 Real) ) (! (= (U_2_real (real_2_U arg0@@1)) arg0@@1)
 :qid |typeInv:U_2_real|
 :pattern ( (real_2_U arg0@@1))
))) (forall ((x@@1 T@U) ) (! (= (real_2_U (U_2_real x@@1)) x@@1)
 :qid |cast:U_2_real|
 :pattern ( (U_2_real x@@1))
))))
(assert (distinct TBool TInt TagBool TagInt TagSet TagMultiSet TagMap alloc |##_System._tuple#2._#Make2| Tagclass._System.Tuple2 Tagclass._module.State Tagclass._module.MyDt |##_module.State.State| |##_module.MyDt.MakeA| |##_module.MyDt.MakeB| |tytagFamily$_tuple#2| tytagFamily$State tytagFamily$MyDt)
)
(assert (= (Tag TBool) TagBool))
(assert (= (Tag TInt) TagInt))
(assert (= (Ctor DatatypeTypeType) 3))
(assert (forall ((|_System._tuple#2$T0| T@U) (|_System._tuple#2$T1| T@U) (d T@U) ) (!  (=> ($Is DatatypeTypeType d (Tclass._System.Tuple2 |_System._tuple#2$T0| |_System._tuple#2$T1|)) (_System.Tuple2.___hMake2_q d))
 :qid |unknown.0:0|
 :skolemid |468|
 :pattern ( (_System.Tuple2.___hMake2_q d) ($Is DatatypeTypeType d (Tclass._System.Tuple2 |_System._tuple#2$T0| |_System._tuple#2$T1|)))
)))
(assert (forall ((o T@U) ) (!  (not (|Set#IsMember| |Set#Empty| o))
 :qid |DafnyPreludebpl.670:15|
 :skolemid |125|
 :pattern ( (|Set#IsMember| |Set#Empty| o))
)))
(assert (= (Ctor MultiSetType) 4))
(assert (forall ((v T@U) (t0 T@U) ) (!  (=> ($Is MultiSetType v (TMultiSet t0)) ($IsGoodMultiSet v))
 :qid |DafnyPreludebpl.250:15|
 :skolemid |52|
 :pattern ( ($Is MultiSetType v (TMultiSet t0)))
)))
(assert (forall ((d@@0 T@U) ) (!  (=> ($Is DatatypeTypeType d@@0 Tclass._module.State) (_module.State.State_q d@@0))
 :qid |unknown.0:0|
 :skolemid |504|
 :pattern ( (_module.State.State_q d@@0) ($Is DatatypeTypeType d@@0 Tclass._module.State))
)))
(assert (forall ((|_System._tuple#2$T0@@0| T@U) (|_System._tuple#2$T1@@0| T@U) (|a#2#0#0| T@U) (|a#2#1#0| T@U) ) (! (= ($Is DatatypeTypeType (|#_System._tuple#2._#Make2| |a#2#0#0| |a#2#1#0|) (Tclass._System.Tuple2 |_System._tuple#2$T0@@0| |_System._tuple#2$T1@@0|))  (and ($IsBox |a#2#0#0| |_System._tuple#2$T0@@0|) ($IsBox |a#2#1#0| |_System._tuple#2$T1@@0|)))
 :qid |unknown.0:0|
 :skolemid |456|
 :pattern ( ($Is DatatypeTypeType (|#_System._tuple#2._#Make2| |a#2#0#0| |a#2#1#0|) (Tclass._System.Tuple2 |_System._tuple#2$T0@@0| |_System._tuple#2$T1@@0|)))
)))
(assert (forall ((s T@U) ) (!  (and (= (= (|Set#Card| s) 0) (= s |Set#Empty|)) (=> (or (not (= (|Set#Card| s) 0)) (not true)) (exists ((x@@2 T@U) ) (! (|Set#IsMember| s x@@2)
 :qid |DafnyPreludebpl.678:20|
 :skolemid |126|
 :pattern ( (|Set#IsMember| s x@@2))
))))
 :qid |DafnyPreludebpl.674:15|
 :skolemid |127|
 :pattern ( (|Set#Card| s))
)))
(assert (forall ((s@@0 T@U) (a T@U) ) (!  (and (= (= (|MultiSet#Multiplicity| (|MultiSet#FromSet| s@@0) a) 0)  (not (|Set#IsMember| s@@0 a))) (= (= (|MultiSet#Multiplicity| (|MultiSet#FromSet| s@@0) a) 1) (|Set#IsMember| s@@0 a)))
 :qid |DafnyPreludebpl.1069:15|
 :skolemid |212|
 :pattern ( (|MultiSet#Multiplicity| (|MultiSet#FromSet| s@@0) a))
)))
(assert (forall ((|a#9#0#0| Int) (|a#9#1#0| Bool) ) (! (= ($Is DatatypeTypeType (|#_module.MyDt.MakeA| |a#9#0#0| |a#9#1#0|) Tclass._module.MyDt)  (and ($Is intType (int_2_U |a#9#0#0|) TInt) ($Is boolType (bool_2_U |a#9#1#0|) TBool)))
 :qid |gitissue133dfy.14:23|
 :skolemid |511|
 :pattern ( ($Is DatatypeTypeType (|#_module.MyDt.MakeA| |a#9#0#0| |a#9#1#0|) Tclass._module.MyDt))
)))
(assert (forall ((x@@3 Int) ) (! (= (LitInt x@@3) x@@3)
 :qid |DafnyPreludebpl.108:29|
 :skolemid |17|
 :pattern ( (LitInt x@@3))
)))
(assert (forall ((x@@4 T@U) (T T@T) ) (! (= (Lit T x@@4) x@@4)
 :qid |DafnyPreludebpl.102:29|
 :skolemid |15|
 :pattern ( (Lit T x@@4))
)))
(assert (forall ((|_System._tuple#2$T0@@1| T@U) (|_System._tuple#2$T1@@1| T@U) (|a#2#0#0@@0| T@U) (|a#2#1#0@@0| T@U) ($h T@U) ) (!  (=> ($IsGoodHeap $h) (= ($IsAlloc DatatypeTypeType (|#_System._tuple#2._#Make2| |a#2#0#0@@0| |a#2#1#0@@0|) (Tclass._System.Tuple2 |_System._tuple#2$T0@@1| |_System._tuple#2$T1@@1|) $h)  (and ($IsAllocBox |a#2#0#0@@0| |_System._tuple#2$T0@@1| $h) ($IsAllocBox |a#2#1#0@@0| |_System._tuple#2$T1@@1| $h))))
 :qid |unknown.0:0|
 :skolemid |457|
 :pattern ( ($IsAlloc DatatypeTypeType (|#_System._tuple#2._#Make2| |a#2#0#0@@0| |a#2#1#0@@0|) (Tclass._System.Tuple2 |_System._tuple#2$T0@@1| |_System._tuple#2$T1@@1|) $h))
)))
(assert (forall ((d@@1 T@U) ) (! (= (_System.Tuple2.___hMake2_q d@@1) (= (DatatypeCtorId d@@1) |##_System._tuple#2._#Make2|))
 :qid |unknown.0:0|
 :skolemid |449|
 :pattern ( (_System.Tuple2.___hMake2_q d@@1))
)))
(assert (forall ((d@@2 T@U) ) (! (= (_module.State.State_q d@@2) (= (DatatypeCtorId d@@2) |##_module.State.State|))
 :qid |unknown.0:0|
 :skolemid |493|
 :pattern ( (_module.State.State_q d@@2))
)))
(assert (forall ((d@@3 T@U) ) (! (= (_module.MyDt.MakeA_q d@@3) (= (DatatypeCtorId d@@3) |##_module.MyDt.MakeA|))
 :qid |unknown.0:0|
 :skolemid |508|
 :pattern ( (_module.MyDt.MakeA_q d@@3))
)))
(assert (forall ((d@@4 T@U) ) (! (= (_module.MyDt.MakeB_q d@@4) (= (DatatypeCtorId d@@4) |##_module.MyDt.MakeB|))
 :qid |unknown.0:0|
 :skolemid |518|
 :pattern ( (_module.MyDt.MakeB_q d@@4))
)))
(assert (forall ((x@@5 T@U) (T@@0 T@T) ) (! (= ($Box T@@0 ($Unbox T@@0 x@@5)) x@@5)
 :qid |DafnyPreludebpl.168:18|
 :skolemid |26|
 :pattern ( ($Unbox T@@0 x@@5))
)))
(assert (forall ((d@@5 T@U) ) (!  (=> (_System.Tuple2.___hMake2_q d@@5) (exists ((|a#1#0#0| T@U) (|a#1#1#0| T@U) ) (! (= d@@5 (|#_System._tuple#2._#Make2| |a#1#0#0| |a#1#1#0|))
 :qid |unknown.0:0|
 :skolemid |450|
)))
 :qid |unknown.0:0|
 :skolemid |451|
 :pattern ( (_System.Tuple2.___hMake2_q d@@5))
)))
(assert (forall ((d@@6 T@U) ) (!  (=> (_module.MyDt.MakeA_q d@@6) (exists ((|a#8#0#0| Int) (|a#8#1#0| Bool) ) (! (= d@@6 (|#_module.MyDt.MakeA| |a#8#0#0| |a#8#1#0|))
 :qid |gitissue133dfy.14:23|
 :skolemid |509|
)))
 :qid |unknown.0:0|
 :skolemid |510|
 :pattern ( (_module.MyDt.MakeA_q d@@6))
)))
(assert (forall ((d@@7 T@U) ) (!  (=> (_module.MyDt.MakeB_q d@@7) (exists ((|a#14#0#0| T@U) (|a#14#1#0| T@U) ) (! (= d@@7 (|#_module.MyDt.MakeB| |a#14#0#0| |a#14#1#0|))
 :qid |gitissue133dfy.14:45|
 :skolemid |519|
)))
 :qid |unknown.0:0|
 :skolemid |520|
 :pattern ( (_module.MyDt.MakeB_q d@@7))
)))
(assert (= (Ctor SetType) 5))
(assert (forall ((v@@0 T@U) (t0@@0 T@U) ) (! (= ($Is SetType v@@0 (TSet t0@@0)) (forall ((bx T@U) ) (!  (=> (|Set#IsMember| v@@0 bx) ($IsBox bx t0@@0))
 :qid |DafnyPreludebpl.240:11|
 :skolemid |46|
 :pattern ( (|Set#IsMember| v@@0 bx))
)))
 :qid |DafnyPreludebpl.238:15|
 :skolemid |47|
 :pattern ( ($Is SetType v@@0 (TSet t0@@0)))
)))
(assert (forall ((d@@8 T@U) ) (!  (=> (_module.State.State_q d@@8) (exists ((|a#1#0#0@@0| T@U) ) (! (= d@@8 (|#_module.State.State| |a#1#0#0@@0|))
 :qid |gitissue133dfy.4:24|
 :skolemid |494|
)))
 :qid |unknown.0:0|
 :skolemid |495|
 :pattern ( (_module.State.State_q d@@8))
)))
(assert (forall ((v@@1 T@U) (t T@U) (h T@U) (T@@1 T@T) ) (! (= ($IsAllocBox ($Box T@@1 v@@1) t h) ($IsAlloc T@@1 v@@1 t h))
 :qid |DafnyPreludebpl.217:18|
 :skolemid |39|
 :pattern ( ($IsAllocBox ($Box T@@1 v@@1) t h))
)))
(assert (forall ((bx@@0 T@U) ) (!  (=> ($IsBox bx@@0 Tclass._module.State) (and (= ($Box DatatypeTypeType ($Unbox DatatypeTypeType bx@@0)) bx@@0) ($Is DatatypeTypeType ($Unbox DatatypeTypeType bx@@0) Tclass._module.State)))
 :qid |unknown.0:0|
 :skolemid |479|
 :pattern ( ($IsBox bx@@0 Tclass._module.State))
)))
(assert (forall ((bx@@1 T@U) ) (!  (=> ($IsBox bx@@1 Tclass._module.MyDt) (and (= ($Box DatatypeTypeType ($Unbox DatatypeTypeType bx@@1)) bx@@1) ($Is DatatypeTypeType ($Unbox DatatypeTypeType bx@@1) Tclass._module.MyDt)))
 :qid |unknown.0:0|
 :skolemid |480|
 :pattern ( ($IsBox bx@@1 Tclass._module.MyDt))
)))
(assert (forall ((|a#18#0#0| T@U) (|a#18#1#0| T@U) (d@@9 T@U) ) (!  (=> (> (|MultiSet#Multiplicity| |a#18#0#0| ($Box DatatypeTypeType d@@9)) 0) (< (DtRank d@@9) (DtRank (|#_module.MyDt.MakeB| |a#18#0#0| |a#18#1#0|))))
 :qid |gitissue133dfy.14:45|
 :skolemid |526|
 :pattern ( (|MultiSet#Multiplicity| |a#18#0#0| ($Box DatatypeTypeType d@@9)) (|#_module.MyDt.MakeB| |a#18#0#0| |a#18#1#0|))
)))
(assert  (and (and (forall ((t0@@1 T@T) (t1 T@T) (val T@U) (m T@U) (x0 T@U) ) (! (= (MapType0Select t0@@1 t1 (MapType0Store t0@@1 t1 m x0 val) x0) val)
 :qid |mapAx0:MapType0Select|
 :weight 0
)) (forall ((u0 T@T) (u1 T@T) (val@@0 T@U) (m@@0 T@U) (x0@@0 T@U) (y0 T@U) ) (!  (or (= x0@@0 y0) (= (MapType0Select u0 u1 (MapType0Store u0 u1 m@@0 x0@@0 val@@0) y0) (MapType0Select u0 u1 m@@0 y0)))
 :qid |mapAx1:MapType0Select:0|
 :weight 0
))) (= (Ctor BoxType) 6)))
(assert (forall ((|a#6#0#0| T@U) (bx@@2 T@U) ) (!  (=> (|Set#IsMember| (|Map#Domain| |a#6#0#0|) bx@@2) (< (DtRank ($Unbox DatatypeTypeType (MapType0Select BoxType BoxType (|Map#Elements| |a#6#0#0|) bx@@2))) (DtRank (|#_module.State.State| |a#6#0#0|))))
 :qid |gitissue133dfy.4:24|
 :skolemid |501|
 :pattern ( (|Set#IsMember| (|Map#Domain| |a#6#0#0|) bx@@2) (|#_module.State.State| |a#6#0#0|))
)))
(assert (forall ((m@@1 T@U) (|m'| T@U) ) (! (= (|Map#Equal| m@@1 |m'|)  (and (forall ((u T@U) ) (! (= (|Set#IsMember| (|Map#Domain| m@@1) u) (|Set#IsMember| (|Map#Domain| |m'|) u))
 :qid |DafnyPreludebpl.1498:35|
 :skolemid |297|
)) (forall ((u@@0 T@U) ) (!  (=> (|Set#IsMember| (|Map#Domain| m@@1) u@@0) (= (MapType0Select BoxType BoxType (|Map#Elements| m@@1) u@@0) (MapType0Select BoxType BoxType (|Map#Elements| |m'|) u@@0)))
 :qid |DafnyPreludebpl.1499:35|
 :skolemid |298|
))))
 :qid |DafnyPreludebpl.1496:15|
 :skolemid |299|
 :pattern ( (|Map#Equal| m@@1 |m'|))
)))
(assert (forall ((m@@2 T@U) ) (! (= (= (|Map#Card| m@@2) 0) (= m@@2 |Map#Empty|))
 :qid |DafnyPreludebpl.1382:15|
 :skolemid |272|
 :pattern ( (|Map#Card| m@@2))
)))
(assert (forall ((d@@10 T@U) ) (!  (=> (|$IsA#_module.MyDt| d@@10) (or (_module.MyDt.MakeA_q d@@10) (_module.MyDt.MakeB_q d@@10)))
 :qid |unknown.0:0|
 :skolemid |530|
 :pattern ( (|$IsA#_module.MyDt| d@@10))
)))
(assert (forall ((|a#12#0#0| Int) (|a#12#1#0| Bool) ) (! (= (_module.MyDt._h0 (|#_module.MyDt.MakeA| |a#12#0#0| |a#12#1#0|)) |a#12#1#0|)
 :qid |gitissue133dfy.14:23|
 :skolemid |516|
 :pattern ( (|#_module.MyDt.MakeA| |a#12#0#0| |a#12#1#0|))
)))
(assert (forall ((a@@0 T@U) (b T@U) ) (!  (=> (|MultiSet#Equal| a@@0 b) (= a@@0 b))
 :qid |DafnyPreludebpl.1054:15|
 :skolemid |209|
 :pattern ( (|MultiSet#Equal| a@@0 b))
)))
(assert (forall ((m@@3 T@U) (|m'@@0| T@U) ) (!  (=> (|Map#Equal| m@@3 |m'@@0|) (= m@@3 |m'@@0|))
 :qid |DafnyPreludebpl.1501:15|
 :skolemid |300|
 :pattern ( (|Map#Equal| m@@3 |m'@@0|))
)))
(assert (forall ((v@@2 T@U) (t0@@2 T@U) ) (! (= ($Is MultiSetType v@@2 (TMultiSet t0@@2)) (forall ((bx@@3 T@U) ) (!  (=> (< 0 (|MultiSet#Multiplicity| v@@2 bx@@3)) ($IsBox bx@@3 t0@@2))
 :qid |DafnyPreludebpl.248:11|
 :skolemid |50|
 :pattern ( (|MultiSet#Multiplicity| v@@2 bx@@3))
)))
 :qid |DafnyPreludebpl.246:15|
 :skolemid |51|
 :pattern ( ($Is MultiSetType v@@2 (TMultiSet t0@@2)))
)))
(assert (forall ((d@@11 T@U) ) (!  (=> ($Is DatatypeTypeType d@@11 Tclass._module.MyDt) (or (_module.MyDt.MakeA_q d@@11) (_module.MyDt.MakeB_q d@@11)))
 :qid |unknown.0:0|
 :skolemid |531|
 :pattern ( (_module.MyDt.MakeB_q d@@11) ($Is DatatypeTypeType d@@11 Tclass._module.MyDt))
 :pattern ( (_module.MyDt.MakeA_q d@@11) ($Is DatatypeTypeType d@@11 Tclass._module.MyDt))
)))
(assert (forall ((bx@@4 T@U) ) (!  (=> ($IsBox bx@@4 TInt) (and (= ($Box intType ($Unbox intType bx@@4)) bx@@4) ($Is intType ($Unbox intType bx@@4) TInt)))
 :qid |DafnyPreludebpl.176:15|
 :skolemid |27|
 :pattern ( ($IsBox bx@@4 TInt))
)))
(assert (forall ((bx@@5 T@U) ) (!  (=> ($IsBox bx@@5 TBool) (and (= ($Box boolType ($Unbox boolType bx@@5)) bx@@5) ($Is boolType ($Unbox boolType bx@@5) TBool)))
 :qid |DafnyPreludebpl.182:15|
 :skolemid |29|
 :pattern ( ($IsBox bx@@5 TBool))
)))
(assert (forall ((v@@3 T@U) (t@@0 T@U) (T@@2 T@T) ) (! (= ($IsBox ($Box T@@2 v@@3) t@@0) ($Is T@@2 v@@3 t@@0))
 :qid |DafnyPreludebpl.214:18|
 :skolemid |38|
 :pattern ( ($IsBox ($Box T@@2 v@@3) t@@0))
)))
(assert (forall ((ms T@U) ) (! (= ($IsGoodMultiSet ms) (forall ((bx@@6 T@U) ) (!  (and (<= 0 (|MultiSet#Multiplicity| ms bx@@6)) (<= (|MultiSet#Multiplicity| ms bx@@6) (|MultiSet#Card| ms)))
 :qid |DafnyPreludebpl.921:19|
 :skolemid |183|
 :pattern ( (|MultiSet#Multiplicity| ms bx@@6))
)))
 :qid |DafnyPreludebpl.918:15|
 :skolemid |184|
 :pattern ( ($IsGoodMultiSet ms))
)))
(assert (forall ((s@@1 T@U) ) (! (<= 0 (|Set#Card| s@@1))
 :qid |DafnyPreludebpl.664:15|
 :skolemid |124|
 :pattern ( (|Set#Card| s@@1))
)))
(assert (forall ((s@@2 T@U) ) (! (<= 0 (|MultiSet#Card| s@@2))
 :qid |DafnyPreludebpl.928:15|
 :skolemid |185|
 :pattern ( (|MultiSet#Card| s@@2))
)))
(assert (forall ((m@@4 T@U) ) (! (<= 0 (|Map#Card| m@@4))
 :qid |DafnyPreludebpl.1380:15|
 :skolemid |271|
 :pattern ( (|Map#Card| m@@4))
)))
(assert (forall ((v@@4 T@U) (t0@@3 T@U) (h@@0 T@U) ) (! (= ($IsAlloc SetType v@@4 (TSet t0@@3) h@@0) (forall ((bx@@7 T@U) ) (!  (=> (|Set#IsMember| v@@4 bx@@7) ($IsAllocBox bx@@7 t0@@3 h@@0))
 :qid |DafnyPreludebpl.299:11|
 :skolemid |67|
 :pattern ( (|Set#IsMember| v@@4 bx@@7))
)))
 :qid |DafnyPreludebpl.297:15|
 :skolemid |68|
 :pattern ( ($IsAlloc SetType v@@4 (TSet t0@@3) h@@0))
)))
(assert (forall ((o@@0 T@U) ) (! (= (|MultiSet#Multiplicity| |MultiSet#Empty| o@@0) 0)
 :qid |DafnyPreludebpl.938:15|
 :skolemid |187|
 :pattern ( (|MultiSet#Multiplicity| |MultiSet#Empty| o@@0))
)))
(assert (forall ((t@@1 T@U) (u@@1 T@U) ) (! (= (Inv0_TMap (TMap t@@1 u@@1)) t@@1)
 :qid |DafnyPreludebpl.57:15|
 :skolemid |9|
 :pattern ( (TMap t@@1 u@@1))
)))
(assert (forall ((t@@2 T@U) (u@@2 T@U) ) (! (= (Inv1_TMap (TMap t@@2 u@@2)) u@@2)
 :qid |DafnyPreludebpl.58:15|
 :skolemid |10|
 :pattern ( (TMap t@@2 u@@2))
)))
(assert (forall ((t@@3 T@U) (u@@3 T@U) ) (! (= (Tag (TMap t@@3 u@@3)) TagMap)
 :qid |DafnyPreludebpl.59:15|
 :skolemid |11|
 :pattern ( (TMap t@@3 u@@3))
)))
(assert (forall ((|a#0#0#0| T@U) (|a#0#1#0| T@U) ) (! (= (DatatypeCtorId (|#_System._tuple#2._#Make2| |a#0#0#0| |a#0#1#0|)) |##_System._tuple#2._#Make2|)
 :qid |unknown.0:0|
 :skolemid |448|
 :pattern ( (|#_System._tuple#2._#Make2| |a#0#0#0| |a#0#1#0|))
)))
(assert (forall ((|_System._tuple#2$T0@@2| T@U) (|_System._tuple#2$T1@@2| T@U) ) (! (= (Tclass._System.Tuple2_0 (Tclass._System.Tuple2 |_System._tuple#2$T0@@2| |_System._tuple#2$T1@@2|)) |_System._tuple#2$T0@@2|)
 :qid |unknown.0:0|
 :skolemid |453|
 :pattern ( (Tclass._System.Tuple2 |_System._tuple#2$T0@@2| |_System._tuple#2$T1@@2|))
)))
(assert (forall ((|_System._tuple#2$T0@@3| T@U) (|_System._tuple#2$T1@@3| T@U) ) (! (= (Tclass._System.Tuple2_1 (Tclass._System.Tuple2 |_System._tuple#2$T0@@3| |_System._tuple#2$T1@@3|)) |_System._tuple#2$T1@@3|)
 :qid |unknown.0:0|
 :skolemid |454|
 :pattern ( (Tclass._System.Tuple2 |_System._tuple#2$T0@@3| |_System._tuple#2$T1@@3|))
)))
(assert (forall ((|a#4#0#0| T@U) (|a#4#1#0| T@U) ) (! (= (_System.Tuple2._0 (|#_System._tuple#2._#Make2| |a#4#0#0| |a#4#1#0|)) |a#4#0#0|)
 :qid |unknown.0:0|
 :skolemid |463|
 :pattern ( (|#_System._tuple#2._#Make2| |a#4#0#0| |a#4#1#0|))
)))
(assert (forall ((|a#6#0#0@@0| T@U) (|a#6#1#0| T@U) ) (! (= (_System.Tuple2._1 (|#_System._tuple#2._#Make2| |a#6#0#0@@0| |a#6#1#0|)) |a#6#1#0|)
 :qid |unknown.0:0|
 :skolemid |465|
 :pattern ( (|#_System._tuple#2._#Make2| |a#6#0#0@@0| |a#6#1#0|))
)))
(assert (forall ((|a#7#0#0| Int) (|a#7#1#0| Bool) ) (! (= (DatatypeCtorId (|#_module.MyDt.MakeA| |a#7#0#0| |a#7#1#0|)) |##_module.MyDt.MakeA|)
 :qid |gitissue133dfy.14:23|
 :skolemid |507|
 :pattern ( (|#_module.MyDt.MakeA| |a#7#0#0| |a#7#1#0|))
)))
(assert (forall ((|a#11#0#0| Int) (|a#11#1#0| Bool) ) (! (= (_module.MyDt.x (|#_module.MyDt.MakeA| |a#11#0#0| |a#11#1#0|)) |a#11#0#0|)
 :qid |gitissue133dfy.14:23|
 :skolemid |515|
 :pattern ( (|#_module.MyDt.MakeA| |a#11#0#0| |a#11#1#0|))
)))
(assert (forall ((|a#13#0#0| T@U) (|a#13#1#0| T@U) ) (! (= (DatatypeCtorId (|#_module.MyDt.MakeB| |a#13#0#0| |a#13#1#0|)) |##_module.MyDt.MakeB|)
 :qid |gitissue133dfy.14:45|
 :skolemid |517|
 :pattern ( (|#_module.MyDt.MakeB| |a#13#0#0| |a#13#1#0|))
)))
(assert (forall ((|a#17#0#0| T@U) (|a#17#1#0| T@U) ) (! (= (_module.MyDt.s (|#_module.MyDt.MakeB| |a#17#0#0| |a#17#1#0|)) |a#17#0#0|)
 :qid |gitissue133dfy.14:45|
 :skolemid |525|
 :pattern ( (|#_module.MyDt.MakeB| |a#17#0#0| |a#17#1#0|))
)))
(assert (forall ((|a#19#0#0| T@U) (|a#19#1#0| T@U) ) (! (= (_module.MyDt.t (|#_module.MyDt.MakeB| |a#19#0#0| |a#19#1#0|)) |a#19#1#0|)
 :qid |gitissue133dfy.14:45|
 :skolemid |527|
 :pattern ( (|#_module.MyDt.MakeB| |a#19#0#0| |a#19#1#0|))
)))
(assert (forall ((t@@4 T@U) ) (! (= (Inv0_TSet (TSet t@@4)) t@@4)
 :qid |DafnyPreludebpl.41:15|
 :skolemid |1|
 :pattern ( (TSet t@@4))
)))
(assert (forall ((t@@5 T@U) ) (! (= (Tag (TSet t@@5)) TagSet)
 :qid |DafnyPreludebpl.42:15|
 :skolemid |2|
 :pattern ( (TSet t@@5))
)))
(assert (forall ((t@@6 T@U) ) (! (= (Inv0_TMultiSet (TMultiSet t@@6)) t@@6)
 :qid |DafnyPreludebpl.49:15|
 :skolemid |5|
 :pattern ( (TMultiSet t@@6))
)))
(assert (forall ((t@@7 T@U) ) (! (= (Tag (TMultiSet t@@7)) TagMultiSet)
 :qid |DafnyPreludebpl.50:15|
 :skolemid |6|
 :pattern ( (TMultiSet t@@7))
)))
(assert (forall ((|a#0#0#0@@0| T@U) ) (! (= (DatatypeCtorId (|#_module.State.State| |a#0#0#0@@0|)) |##_module.State.State|)
 :qid |gitissue133dfy.4:24|
 :skolemid |492|
 :pattern ( (|#_module.State.State| |a#0#0#0@@0|))
)))
(assert (forall ((|a#4#0#0@@0| T@U) ) (! (= (_module.State.m (|#_module.State.State| |a#4#0#0@@0|)) |a#4#0#0@@0|)
 :qid |gitissue133dfy.4:24|
 :skolemid |499|
 :pattern ( (|#_module.State.State| |a#4#0#0@@0|))
)))
(assert (forall ((x@@6 T@U) (T@@3 T@T) ) (! (= ($Unbox T@@3 ($Box T@@3 x@@6)) x@@6)
 :qid |DafnyPreludebpl.167:18|
 :skolemid |25|
 :pattern ( ($Box T@@3 x@@6))
)))
(assert (= (Ctor MapType) 7))
(assert (forall ((v@@5 T@U) (t0@@4 T@U) (t1@@0 T@U) (h@@1 T@U) ) (! (= ($IsAlloc MapType v@@5 (TMap t0@@4 t1@@0) h@@1) (forall ((bx@@8 T@U) ) (!  (=> (|Set#IsMember| (|Map#Domain| v@@5) bx@@8) (and ($IsAllocBox (MapType0Select BoxType BoxType (|Map#Elements| v@@5) bx@@8) t1@@0 h@@1) ($IsAllocBox bx@@8 t0@@4 h@@1)))
 :qid |DafnyPreludebpl.318:19|
 :skolemid |75|
 :pattern ( (MapType0Select BoxType BoxType (|Map#Elements| v@@5) bx@@8))
 :pattern ( (|Set#IsMember| (|Map#Domain| v@@5) bx@@8))
)))
 :qid |DafnyPreludebpl.315:15|
 :skolemid |76|
 :pattern ( ($IsAlloc MapType v@@5 (TMap t0@@4 t1@@0) h@@1))
)))
(assert (forall ((|a#5#0#0| T@U) (|a#5#1#0| T@U) ) (! (< (BoxRank |a#5#0#0|) (DtRank (|#_System._tuple#2._#Make2| |a#5#0#0| |a#5#1#0|)))
 :qid |unknown.0:0|
 :skolemid |464|
 :pattern ( (|#_System._tuple#2._#Make2| |a#5#0#0| |a#5#1#0|))
)))
(assert (forall ((|a#7#0#0@@0| T@U) (|a#7#1#0@@0| T@U) ) (! (< (BoxRank |a#7#1#0@@0|) (DtRank (|#_System._tuple#2._#Make2| |a#7#0#0@@0| |a#7#1#0@@0|)))
 :qid |unknown.0:0|
 :skolemid |466|
 :pattern ( (|#_System._tuple#2._#Make2| |a#7#0#0@@0| |a#7#1#0@@0|))
)))
(assert (forall ((|a#20#0#0| T@U) (|a#20#1#0| T@U) ) (! (< (DtRank |a#20#1#0|) (DtRank (|#_module.MyDt.MakeB| |a#20#0#0| |a#20#1#0|)))
 :qid |gitissue133dfy.14:45|
 :skolemid |528|
 :pattern ( (|#_module.MyDt.MakeB| |a#20#0#0| |a#20#1#0|))
)))
(assert (forall ((s@@3 T@U) ) (!  (and (= (= (|MultiSet#Card| s@@3) 0) (= s@@3 |MultiSet#Empty|)) (=> (or (not (= (|MultiSet#Card| s@@3) 0)) (not true)) (exists ((x@@7 T@U) ) (! (< 0 (|MultiSet#Multiplicity| s@@3 x@@7))
 :qid |DafnyPreludebpl.946:20|
 :skolemid |188|
 :pattern ( (|MultiSet#Multiplicity| s@@3 x@@7))
))))
 :qid |DafnyPreludebpl.942:15|
 :skolemid |189|
 :pattern ( (|MultiSet#Card| s@@3))
)))
(assert (forall ((u@@4 T@U) ) (!  (not (|Set#IsMember| (|Map#Domain| |Map#Empty|) u@@4))
 :qid |DafnyPreludebpl.1438:15|
 :skolemid |285|
 :pattern ( (|Set#IsMember| (|Map#Domain| |Map#Empty|) u@@4))
)))
(assert (forall ((bx@@9 T@U) (s@@4 T@U) (t@@8 T@U) ) (!  (=> ($IsBox bx@@9 (TMap s@@4 t@@8)) (and (= ($Box MapType ($Unbox MapType bx@@9)) bx@@9) ($Is MapType ($Unbox MapType bx@@9) (TMap s@@4 t@@8))))
 :qid |DafnyPreludebpl.207:15|
 :skolemid |36|
 :pattern ( ($IsBox bx@@9 (TMap s@@4 t@@8)))
)))
(assert (forall ((|_System._tuple#2$T0@@4| T@U) (|_System._tuple#2$T1@@4| T@U) (bx@@10 T@U) ) (!  (=> ($IsBox bx@@10 (Tclass._System.Tuple2 |_System._tuple#2$T0@@4| |_System._tuple#2$T1@@4|)) (and (= ($Box DatatypeTypeType ($Unbox DatatypeTypeType bx@@10)) bx@@10) ($Is DatatypeTypeType ($Unbox DatatypeTypeType bx@@10) (Tclass._System.Tuple2 |_System._tuple#2$T0@@4| |_System._tuple#2$T1@@4|))))
 :qid |unknown.0:0|
 :skolemid |455|
 :pattern ( ($IsBox bx@@10 (Tclass._System.Tuple2 |_System._tuple#2$T0@@4| |_System._tuple#2$T1@@4|)))
)))
(assert  (and (and (and (and (and (and (forall ((t0@@5 T@T) (t1@@1 T@T) (t2 T@T) (val@@1 T@U) (m@@5 T@U) (x0@@1 T@U) (x1 T@U) ) (! (= (MapType1Select t0@@5 t1@@1 t2 (MapType1Store t0@@5 t1@@1 t2 m@@5 x0@@1 x1 val@@1) x0@@1 x1) val@@1)
 :qid |mapAx0:MapType1Select|
 :weight 0
)) (and (forall ((u0@@0 T@T) (u1@@0 T@T) (u2 T@T) (val@@2 T@U) (m@@6 T@U) (x0@@2 T@U) (x1@@0 T@U) (y0@@0 T@U) (y1 T@U) ) (!  (or (= x0@@2 y0@@0) (= (MapType1Select u0@@0 u1@@0 u2 (MapType1Store u0@@0 u1@@0 u2 m@@6 x0@@2 x1@@0 val@@2) y0@@0 y1) (MapType1Select u0@@0 u1@@0 u2 m@@6 y0@@0 y1)))
 :qid |mapAx1:MapType1Select:0|
 :weight 0
)) (forall ((u0@@1 T@T) (u1@@1 T@T) (u2@@0 T@T) (val@@3 T@U) (m@@7 T@U) (x0@@3 T@U) (x1@@1 T@U) (y0@@1 T@U) (y1@@0 T@U) ) (!  (or (= x1@@1 y1@@0) (= (MapType1Select u0@@1 u1@@1 u2@@0 (MapType1Store u0@@1 u1@@1 u2@@0 m@@7 x0@@3 x1@@1 val@@3) y0@@1 y1@@0) (MapType1Select u0@@1 u1@@1 u2@@0 m@@7 y0@@1 y1@@0)))
 :qid |mapAx1:MapType1Select:1|
 :weight 0
)))) (= (Ctor refType) 8)) (= (Ctor FieldType) 9)) (forall ((arg0@@2 T@T) (arg1 T@T) ) (! (= (Ctor (MapType0Type arg0@@2 arg1)) 10)
 :qid |ctor:MapType0Type|
))) (forall ((arg0@@3 T@T) (arg1@@0 T@T) ) (! (= (MapType0TypeInv0 (MapType0Type arg0@@3 arg1@@0)) arg0@@3)
 :qid |typeInv:MapType0TypeInv0|
 :pattern ( (MapType0Type arg0@@3 arg1@@0))
))) (forall ((arg0@@4 T@T) (arg1@@1 T@T) ) (! (= (MapType0TypeInv1 (MapType0Type arg0@@4 arg1@@1)) arg1@@1)
 :qid |typeInv:MapType0TypeInv1|
 :pattern ( (MapType0Type arg0@@4 arg1@@1))
))))
(assert (forall ((|l#0| T@U) (|l#1| T@U) (|l#2| T@U) (|l#3| Bool) ($o T@U) ($f T@U) ) (! (= (U_2_bool (MapType1Select refType FieldType boolType (|lambda#0| |l#0| |l#1| |l#2| |l#3|) $o $f))  (=> (and (or (not (= $o |l#0|)) (not true)) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) |l#1| $o) |l#2|)))) |l#3|))
 :qid |DafnyPreludebpl.156:1|
 :skolemid |603|
 :pattern ( (MapType1Select refType FieldType boolType (|lambda#0| |l#0| |l#1| |l#2| |l#3|) $o $f))
)))
(assert (forall ((|_System._tuple#2$T0@@5| T@U) (|_System._tuple#2$T1@@5| T@U) ) (!  (and (= (Tag (Tclass._System.Tuple2 |_System._tuple#2$T0@@5| |_System._tuple#2$T1@@5|)) Tagclass._System.Tuple2) (= (TagFamily (Tclass._System.Tuple2 |_System._tuple#2$T0@@5| |_System._tuple#2$T1@@5|)) |tytagFamily$_tuple#2|))
 :qid |unknown.0:0|
 :skolemid |452|
 :pattern ( (Tclass._System.Tuple2 |_System._tuple#2$T0@@5| |_System._tuple#2$T1@@5|))
)))
(assert (forall ((d@@12 T@U) ($h@@0 T@U) ) (!  (=> (and ($IsGoodHeap $h@@0) (and (_module.MyDt.MakeA_q d@@12) ($IsAlloc DatatypeTypeType d@@12 Tclass._module.MyDt $h@@0))) ($IsAlloc intType (int_2_U (_module.MyDt.x d@@12)) TInt $h@@0))
 :qid |unknown.0:0|
 :skolemid |512|
 :pattern ( ($IsAlloc intType (int_2_U (_module.MyDt.x d@@12)) TInt $h@@0))
)))
(assert (forall ((d@@13 T@U) ($h@@1 T@U) ) (!  (=> (and ($IsGoodHeap $h@@1) (and (_module.MyDt.MakeA_q d@@13) ($IsAlloc DatatypeTypeType d@@13 Tclass._module.MyDt $h@@1))) ($IsAlloc boolType (bool_2_U (_module.MyDt._h0 d@@13)) TBool $h@@1))
 :qid |unknown.0:0|
 :skolemid |513|
 :pattern ( ($IsAlloc boolType (bool_2_U (_module.MyDt._h0 d@@13)) TBool $h@@1))
)))
(assert (forall ((m@@8 T@U) ) (!  (or (= m@@8 |Map#Empty|) (exists ((k T@U) ) (! (|Set#IsMember| (|Map#Domain| m@@8) k)
 :qid |DafnyPreludebpl.1388:31|
 :skolemid |273|
)))
 :qid |DafnyPreludebpl.1386:15|
 :skolemid |274|
 :pattern ( (|Map#Domain| m@@8))
)))
(assert (forall ((m@@9 T@U) ) (!  (or (= m@@9 |Map#Empty|) (exists ((v@@6 T@U) ) (! (|Set#IsMember| (|Map#Values| m@@9) v@@6)
 :qid |DafnyPreludebpl.1391:31|
 :skolemid |275|
)))
 :qid |DafnyPreludebpl.1389:15|
 :skolemid |276|
 :pattern ( (|Map#Values| m@@9))
)))
(assert (forall ((m@@10 T@U) (item T@U) ) (! (= (|Set#IsMember| (|Map#Items| m@@10) item)  (and (|Set#IsMember| (|Map#Domain| m@@10) (_System.Tuple2._0 ($Unbox DatatypeTypeType item))) (= (MapType0Select BoxType BoxType (|Map#Elements| m@@10) (_System.Tuple2._0 ($Unbox DatatypeTypeType item))) (_System.Tuple2._1 ($Unbox DatatypeTypeType item)))))
 :qid |DafnyPreludebpl.1430:15|
 :skolemid |284|
 :pattern ( (|Set#IsMember| (|Map#Items| m@@10) item))
)))
(assert (forall ((m@@11 T@U) (v@@7 T@U) ) (! (= (|Set#IsMember| (|Map#Values| m@@11) v@@7) (exists ((u@@5 T@U) ) (!  (and (|Set#IsMember| (|Map#Domain| m@@11) u@@5) (= v@@7 (MapType0Select BoxType BoxType (|Map#Elements| m@@11) u@@5)))
 :qid |DafnyPreludebpl.1414:10|
 :skolemid |282|
 :pattern ( (|Set#IsMember| (|Map#Domain| m@@11) u@@5))
 :pattern ( (MapType0Select BoxType BoxType (|Map#Elements| m@@11) u@@5))
)))
 :qid |DafnyPreludebpl.1412:15|
 :skolemid |283|
 :pattern ( (|Set#IsMember| (|Map#Values| m@@11) v@@7))
)))
(assert (forall ((d@@14 T@U) ) (! (= (BoxRank ($Box DatatypeTypeType d@@14)) (DtRank d@@14))
 :qid |DafnyPreludebpl.391:15|
 :skolemid |84|
 :pattern ( (BoxRank ($Box DatatypeTypeType d@@14)))
)))
(assert (forall ((s@@5 T@U) ) (! (= (|MultiSet#Card| (|MultiSet#FromSet| s@@5)) (|Set#Card| s@@5))
 :qid |DafnyPreludebpl.1074:15|
 :skolemid |213|
 :pattern ( (|MultiSet#Card| (|MultiSet#FromSet| s@@5)))
)))
(assert (forall ((m@@12 T@U) ) (! (= (|Set#Card| (|Map#Domain| m@@12)) (|Map#Card| m@@12))
 :qid |DafnyPreludebpl.1396:15|
 :skolemid |279|
 :pattern ( (|Set#Card| (|Map#Domain| m@@12)))
 :pattern ( (|Map#Card| m@@12))
)))
(assert (forall ((m@@13 T@U) ) (! (= (|Set#Card| (|Map#Items| m@@13)) (|Map#Card| m@@13))
 :qid |DafnyPreludebpl.1402:15|
 :skolemid |281|
 :pattern ( (|Set#Card| (|Map#Items| m@@13)))
 :pattern ( (|Map#Card| m@@13))
)))
(assert (forall ((m@@14 T@U) ) (! (<= (|Set#Card| (|Map#Values| m@@14)) (|Map#Card| m@@14))
 :qid |DafnyPreludebpl.1399:15|
 :skolemid |280|
 :pattern ( (|Set#Card| (|Map#Values| m@@14)))
 :pattern ( (|Map#Card| m@@14))
)))
(assert (forall ((bx@@11 T@U) (t@@9 T@U) ) (!  (=> ($IsBox bx@@11 (TSet t@@9)) (and (= ($Box SetType ($Unbox SetType bx@@11)) bx@@11) ($Is SetType ($Unbox SetType bx@@11) (TSet t@@9))))
 :qid |DafnyPreludebpl.195:15|
 :skolemid |32|
 :pattern ( ($IsBox bx@@11 (TSet t@@9)))
)))
(assert (forall ((bx@@12 T@U) (t@@10 T@U) ) (!  (=> ($IsBox bx@@12 (TMultiSet t@@10)) (and (= ($Box MultiSetType ($Unbox MultiSetType bx@@12)) bx@@12) ($Is MultiSetType ($Unbox MultiSetType bx@@12) (TMultiSet t@@10))))
 :qid |DafnyPreludebpl.201:15|
 :skolemid |34|
 :pattern ( ($IsBox bx@@12 (TMultiSet t@@10)))
)))
(assert (forall ((|a#2#0#0@@1| T@U) ) (! (= ($Is DatatypeTypeType (|#_module.State.State| |a#2#0#0@@1|) Tclass._module.State) ($Is MapType |a#2#0#0@@1| (TMap TInt TBool)))
 :qid |gitissue133dfy.4:24|
 :skolemid |496|
 :pattern ( ($Is DatatypeTypeType (|#_module.State.State| |a#2#0#0@@1|) Tclass._module.State))
)))
(assert (forall ((v@@8 T@U) (t0@@6 T@U) (t1@@2 T@U) ) (! (= ($Is MapType v@@8 (TMap t0@@6 t1@@2)) (forall ((bx@@13 T@U) ) (!  (=> (|Set#IsMember| (|Map#Domain| v@@8) bx@@13) (and ($IsBox (MapType0Select BoxType BoxType (|Map#Elements| v@@8) bx@@13) t1@@2) ($IsBox bx@@13 t0@@6)))
 :qid |DafnyPreludebpl.261:19|
 :skolemid |55|
 :pattern ( (MapType0Select BoxType BoxType (|Map#Elements| v@@8) bx@@13))
 :pattern ( (|Set#IsMember| (|Map#Domain| v@@8) bx@@13))
)))
 :qid |DafnyPreludebpl.258:15|
 :skolemid |56|
 :pattern ( ($Is MapType v@@8 (TMap t0@@6 t1@@2)))
)))
(assert (forall ((d@@15 T@U) ($h@@2 T@U) ) (!  (=> (and ($IsGoodHeap $h@@2) (and (_module.State.State_q d@@15) ($IsAlloc DatatypeTypeType d@@15 Tclass._module.State $h@@2))) ($IsAlloc MapType (_module.State.m d@@15) (TMap TInt TBool) $h@@2))
 :qid |unknown.0:0|
 :skolemid |497|
 :pattern ( ($IsAlloc MapType (_module.State.m d@@15) (TMap TInt TBool) $h@@2))
)))
(assert (forall ((d@@16 T@U) ($h@@3 T@U) ) (!  (=> (and ($IsGoodHeap $h@@3) ($Is DatatypeTypeType d@@16 Tclass._module.State)) ($IsAlloc DatatypeTypeType d@@16 Tclass._module.State $h@@3))
 :qid |unknown.0:0|
 :skolemid |502|
 :pattern ( ($IsAlloc DatatypeTypeType d@@16 Tclass._module.State $h@@3))
)))
(assert (forall ((d@@17 T@U) ($h@@4 T@U) ) (!  (=> (and ($IsGoodHeap $h@@4) ($Is DatatypeTypeType d@@17 Tclass._module.MyDt)) ($IsAlloc DatatypeTypeType d@@17 Tclass._module.MyDt $h@@4))
 :qid |unknown.0:0|
 :skolemid |529|
 :pattern ( ($IsAlloc DatatypeTypeType d@@17 Tclass._module.MyDt $h@@4))
)))
(assert (= (Tag Tclass._module.State) Tagclass._module.State))
(assert (= (TagFamily Tclass._module.State) tytagFamily$State))
(assert (= (Tag Tclass._module.MyDt) Tagclass._module.MyDt))
(assert (= (TagFamily Tclass._module.MyDt) tytagFamily$MyDt))
(assert (forall ((d@@18 T@U) ($h@@5 T@U) ) (!  (=> (and ($IsGoodHeap $h@@5) (and (_module.MyDt.MakeB_q d@@18) ($IsAlloc DatatypeTypeType d@@18 Tclass._module.MyDt $h@@5))) ($IsAlloc MultiSetType (_module.MyDt.s d@@18) (TMultiSet TInt) $h@@5))
 :qid |unknown.0:0|
 :skolemid |522|
 :pattern ( ($IsAlloc MultiSetType (_module.MyDt.s d@@18) (TMultiSet TInt) $h@@5))
)))
(assert (forall ((d@@19 T@U) (|_System._tuple#2$T0@@6| T@U) ($h@@6 T@U) ) (!  (=> (and ($IsGoodHeap $h@@6) (and (_System.Tuple2.___hMake2_q d@@19) (exists ((|_System._tuple#2$T1@@6| T@U) ) (! ($IsAlloc DatatypeTypeType d@@19 (Tclass._System.Tuple2 |_System._tuple#2$T0@@6| |_System._tuple#2$T1@@6|) $h@@6)
 :qid |unknown.0:0|
 :skolemid |458|
 :pattern ( ($IsAlloc DatatypeTypeType d@@19 (Tclass._System.Tuple2 |_System._tuple#2$T0@@6| |_System._tuple#2$T1@@6|) $h@@6))
)))) ($IsAllocBox (_System.Tuple2._0 d@@19) |_System._tuple#2$T0@@6| $h@@6))
 :qid |unknown.0:0|
 :skolemid |459|
 :pattern ( ($IsAllocBox (_System.Tuple2._0 d@@19) |_System._tuple#2$T0@@6| $h@@6))
)))
(assert (forall ((d@@20 T@U) (|_System._tuple#2$T1@@7| T@U) ($h@@7 T@U) ) (!  (=> (and ($IsGoodHeap $h@@7) (and (_System.Tuple2.___hMake2_q d@@20) (exists ((|_System._tuple#2$T0@@7| T@U) ) (! ($IsAlloc DatatypeTypeType d@@20 (Tclass._System.Tuple2 |_System._tuple#2$T0@@7| |_System._tuple#2$T1@@7|) $h@@7)
 :qid |unknown.0:0|
 :skolemid |460|
 :pattern ( ($IsAlloc DatatypeTypeType d@@20 (Tclass._System.Tuple2 |_System._tuple#2$T0@@7| |_System._tuple#2$T1@@7|) $h@@7))
)))) ($IsAllocBox (_System.Tuple2._1 d@@20) |_System._tuple#2$T1@@7| $h@@7))
 :qid |unknown.0:0|
 :skolemid |461|
 :pattern ( ($IsAllocBox (_System.Tuple2._1 d@@20) |_System._tuple#2$T1@@7| $h@@7))
)))
(assert (forall ((d@@21 T@U) ($h@@8 T@U) ) (!  (=> (and ($IsGoodHeap $h@@8) (and (_module.MyDt.MakeB_q d@@21) ($IsAlloc DatatypeTypeType d@@21 Tclass._module.MyDt $h@@8))) ($IsAlloc DatatypeTypeType (_module.MyDt.t d@@21) Tclass._module.State $h@@8))
 :qid |unknown.0:0|
 :skolemid |523|
 :pattern ( ($IsAlloc DatatypeTypeType (_module.MyDt.t d@@21) Tclass._module.State $h@@8))
)))
(assert (forall ((|a#3#0#0| T@U) (|a#3#1#0| T@U) ) (! (= (|#_System._tuple#2._#Make2| (Lit BoxType |a#3#0#0|) (Lit BoxType |a#3#1#0|)) (Lit DatatypeTypeType (|#_System._tuple#2._#Make2| |a#3#0#0| |a#3#1#0|)))
 :qid |unknown.0:0|
 :skolemid |462|
 :pattern ( (|#_System._tuple#2._#Make2| (Lit BoxType |a#3#0#0|) (Lit BoxType |a#3#1#0|)))
)))
(assert (forall ((|a#10#0#0| Int) (|a#10#1#0| T@U) ) (! (= (|#_module.MyDt.MakeA| (LitInt |a#10#0#0|) (U_2_bool (Lit boolType |a#10#1#0|))) (Lit DatatypeTypeType (|#_module.MyDt.MakeA| |a#10#0#0| (U_2_bool |a#10#1#0|))))
 :qid |gitissue133dfy.14:23|
 :skolemid |514|
 :pattern ( (|#_module.MyDt.MakeA| (LitInt |a#10#0#0|) (U_2_bool (Lit boolType |a#10#1#0|))))
)))
(assert (forall ((|a#16#0#0| T@U) (|a#16#1#0| T@U) ) (! (= (|#_module.MyDt.MakeB| (Lit MultiSetType |a#16#0#0|) (Lit DatatypeTypeType |a#16#1#0|)) (Lit DatatypeTypeType (|#_module.MyDt.MakeB| |a#16#0#0| |a#16#1#0|)))
 :qid |gitissue133dfy.14:45|
 :skolemid |524|
 :pattern ( (|#_module.MyDt.MakeB| (Lit MultiSetType |a#16#0#0|) (Lit DatatypeTypeType |a#16#1#0|)))
)))
(assert (forall ((x@@8 Int) ) (! (= ($Box intType (int_2_U (LitInt x@@8))) (Lit BoxType ($Box intType (int_2_U x@@8))))
 :qid |DafnyPreludebpl.109:15|
 :skolemid |18|
 :pattern ( ($Box intType (int_2_U (LitInt x@@8))))
)))
(assert (forall ((|a#3#0#0@@0| T@U) ) (! (= (|#_module.State.State| (Lit MapType |a#3#0#0@@0|)) (Lit DatatypeTypeType (|#_module.State.State| |a#3#0#0@@0|)))
 :qid |gitissue133dfy.4:24|
 :skolemid |498|
 :pattern ( (|#_module.State.State| (Lit MapType |a#3#0#0@@0|)))
)))
(assert (forall ((x@@9 T@U) (T@@4 T@T) ) (! (= ($Box T@@4 (Lit T@@4 x@@9)) (Lit BoxType ($Box T@@4 x@@9)))
 :qid |DafnyPreludebpl.103:18|
 :skolemid |16|
 :pattern ( ($Box T@@4 (Lit T@@4 x@@9)))
)))
(assert (forall ((|a#5#0#0@@0| T@U) (d@@22 T@U) ) (!  (=> (|Set#IsMember| (|Map#Domain| |a#5#0#0@@0|) ($Box DatatypeTypeType d@@22)) (< (DtRank d@@22) (DtRank (|#_module.State.State| |a#5#0#0@@0|))))
 :qid |gitissue133dfy.4:24|
 :skolemid |500|
 :pattern ( (|Set#IsMember| (|Map#Domain| |a#5#0#0@@0|) ($Box DatatypeTypeType d@@22)) (|#_module.State.State| |a#5#0#0@@0|))
)))
(assert (forall ((a@@1 T@U) (b@@0 T@U) ) (! (= (|MultiSet#Equal| a@@1 b@@0) (forall ((o@@1 T@U) ) (! (= (|MultiSet#Multiplicity| a@@1 o@@1) (|MultiSet#Multiplicity| b@@0 o@@1))
 :qid |DafnyPreludebpl.1050:19|
 :skolemid |207|
 :pattern ( (|MultiSet#Multiplicity| a@@1 o@@1))
 :pattern ( (|MultiSet#Multiplicity| b@@0 o@@1))
)))
 :qid |DafnyPreludebpl.1047:15|
 :skolemid |208|
 :pattern ( (|MultiSet#Equal| a@@1 b@@0))
)))
(assert (forall ((v@@9 T@U) (t0@@7 T@U) (h@@2 T@U) ) (! (= ($IsAlloc MultiSetType v@@9 (TMultiSet t0@@7) h@@2) (forall ((bx@@14 T@U) ) (!  (=> (< 0 (|MultiSet#Multiplicity| v@@9 bx@@14)) ($IsAllocBox bx@@14 t0@@7 h@@2))
 :qid |DafnyPreludebpl.307:11|
 :skolemid |71|
 :pattern ( (|MultiSet#Multiplicity| v@@9 bx@@14))
)))
 :qid |DafnyPreludebpl.305:15|
 :skolemid |72|
 :pattern ( ($IsAlloc MultiSetType v@@9 (TMultiSet t0@@7) h@@2))
)))
(assert (forall ((h@@3 T@U) (v@@10 T@U) ) (! ($IsAlloc intType v@@10 TInt h@@3)
 :qid |DafnyPreludebpl.289:14|
 :skolemid |61|
 :pattern ( ($IsAlloc intType v@@10 TInt h@@3))
)))
(assert (forall ((h@@4 T@U) (v@@11 T@U) ) (! ($IsAlloc boolType v@@11 TBool h@@4)
 :qid |DafnyPreludebpl.291:14|
 :skolemid |63|
 :pattern ( ($IsAlloc boolType v@@11 TBool h@@4))
)))
(assert (forall ((m@@15 T@U) ) (!  (or (= m@@15 |Map#Empty|) (exists ((k@@0 T@U) (v@@12 T@U) ) (! (|Set#IsMember| (|Map#Items| m@@15) ($Box DatatypeTypeType (|#_System._tuple#2._#Make2| k@@0 v@@12)))
 :qid |DafnyPreludebpl.1394:31|
 :skolemid |277|
)))
 :qid |DafnyPreludebpl.1392:15|
 :skolemid |278|
 :pattern ( (|Map#Items| m@@15))
)))
(assert (forall ((v@@13 T@U) (t0@@8 T@U) (t1@@3 T@U) ) (!  (=> ($Is MapType v@@13 (TMap t0@@8 t1@@3)) (and (and ($Is SetType (|Map#Domain| v@@13) (TSet t0@@8)) ($Is SetType (|Map#Values| v@@13) (TSet t1@@3))) ($Is SetType (|Map#Items| v@@13) (TSet (Tclass._System.Tuple2 t0@@8 t1@@3)))))
 :qid |DafnyPreludebpl.267:15|
 :skolemid |57|
 :pattern ( ($Is MapType v@@13 (TMap t0@@8 t1@@3)))
)))
(assert (forall ((v@@14 T@U) ) (! ($Is intType v@@14 TInt)
 :qid |DafnyPreludebpl.228:14|
 :skolemid |40|
 :pattern ( ($Is intType v@@14 TInt))
)))
(assert (forall ((v@@15 T@U) ) (! ($Is boolType v@@15 TBool)
 :qid |DafnyPreludebpl.230:14|
 :skolemid |42|
 :pattern ( ($Is boolType v@@15 TBool))
)))
(assert (forall ((|a#15#0#0| T@U) (|a#15#1#0| T@U) ) (! (= ($Is DatatypeTypeType (|#_module.MyDt.MakeB| |a#15#0#0| |a#15#1#0|) Tclass._module.MyDt)  (and ($Is MultiSetType |a#15#0#0| (TMultiSet TInt)) ($Is DatatypeTypeType |a#15#1#0| Tclass._module.State)))
 :qid |gitissue133dfy.14:45|
 :skolemid |521|
 :pattern ( ($Is DatatypeTypeType (|#_module.MyDt.MakeB| |a#15#0#0| |a#15#1#0|) Tclass._module.MyDt))
)))
(push 1)
(declare-fun ControlFlow (Int Int) Int)
(declare-fun $_ModifiesFrame@0 () T@U)
(declare-fun null () T@U)
(declare-fun $Heap () T@U)
(declare-fun |a#0| () T@U)
(declare-fun |b#0| () T@U)
(declare-fun $IsHeapAnchor (T@U) Bool)
(declare-fun $FunctionContextHeight () Int)
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.AnotherTest)
(set-option :timeout 10000)
(set-option :rlimit 0)
(set-option :auto_config false)
(set-option :type_check true)
(set-option :smt.qi.eager_threshold 100)
(set-option :smt.delay_units true)
(set-option :model_evaluator.completion true)
(set-option :model.completion true)
(set-option :model.compact false)
(set-option :smt.case_split 3)
(set-option :smt.mbqi false)
(set-option :model.v2 true)
(set-option :pp.bv_literals false)
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 7) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (_module.MyDt.MakeB_q |a#0|) (_module.MyDt.MakeB_q |b#0|)) (and (=> (= (ControlFlow 0 2) (- 0 6)) (_module.MyDt.MakeB_q |a#0|)) (=> (_module.MyDt.MakeB_q |a#0|) (and (=> (= (ControlFlow 0 2) (- 0 5)) (_module.MyDt.MakeB_q |a#0|)) (=> (_module.MyDt.MakeB_q |a#0|) (=> (and (_module.State.State_q (_module.MyDt.t |a#0|)) (|MultiSet#Equal| (_module.MyDt.s |a#0|) (|MultiSet#FromSet| (|Map#Domain| (_module.State.m (_module.MyDt.t |a#0|)))))) (and (=> (= (ControlFlow 0 2) (- 0 4)) (_module.MyDt.MakeB_q |b#0|)) (=> (_module.MyDt.MakeB_q |b#0|) (=> (= (|MultiSet#Card| (_module.MyDt.s |b#0|)) (LitInt 0)) (and (=> (= (ControlFlow 0 2) (- 0 3)) (_module.MyDt.MakeB_q |a#0|)) (=> (_module.MyDt.MakeB_q |a#0|) (=> (_module.State.State_q (_module.MyDt.t |a#0|)) (=> (and (|Map#Equal| (_module.State.m (_module.MyDt.t |a#0|)) |Map#Empty|) (= (ControlFlow 0 2) (- 0 1))) (_module.MyDt.MakeB_q |b#0|)))))))))))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is DatatypeTypeType |a#0| Tclass._module.MyDt) ($IsAlloc DatatypeTypeType |a#0| Tclass._module.MyDt $Heap)) (|$IsA#_module.MyDt| |a#0|)) (=> (and (and (and ($Is DatatypeTypeType |b#0| Tclass._module.MyDt) ($IsAlloc DatatypeTypeType |b#0| Tclass._module.MyDt $Heap)) (|$IsA#_module.MyDt| |b#0|)) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 7) 2))) anon0_correct)))))
PreconditionGeneratedEntry_correct)))
))
(check-sat)
(get-info :rlimit)
(pop 1)
; Valid
(reset)
(set-option :rlimit 0)
; did a full reset
(reset)
