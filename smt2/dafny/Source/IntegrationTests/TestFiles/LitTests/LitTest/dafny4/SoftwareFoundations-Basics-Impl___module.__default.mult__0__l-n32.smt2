...
(set-info :boogie-vc-id Impl$$_module.__default.mult__0__l)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (|$IsA#_module.Nat| |n#0@@7|)) (=> (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap $Heap@0)) (and (forall ((|$ih#n0#0| T@U) ) (!  (=> (and (and ($Is DatatypeTypeType |$ih#n0#0| Tclass._module.Nat) (U_2_bool (Lit boolType (bool_2_U true)))) (< (DtRank |$ih#n0#0|) (DtRank |n#0@@7|))) (|_module.Nat#Equal| (_module.__default.mult ($LS $LZ) (Lit DatatypeTypeType |#_module.Nat.O|) |$ih#n0#0|) |#_module.Nat.O|))
 :qid |SoftwareFoundationsBasicsdfy.357:17|
 :skolemid |743|
 :pattern ( (_module.__default.mult ($LS $LZ) (Lit DatatypeTypeType |#_module.Nat.O|) |$ih#n0#0|))
)) (= (ControlFlow 0 2) (- 0 1)))) (|_module.Nat#Equal| (_module.__default.mult ($LS ($LS $LZ)) (Lit DatatypeTypeType |#_module.Nat.O|) |n#0@@7|) |#_module.Nat.O|)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is DatatypeTypeType |n#0@@7| Tclass._module.Nat) ($IsAlloc DatatypeTypeType |n#0@@7| Tclass._module.Nat $Heap)) (|$IsA#_module.Nat| |n#0@@7|)) (and (= 3 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
