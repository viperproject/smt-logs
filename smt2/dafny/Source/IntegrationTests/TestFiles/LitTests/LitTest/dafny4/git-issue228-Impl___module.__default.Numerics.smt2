...
(set-info :boogie-vc-id Impl$$_module.__default.Numerics)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (= |x#0@0| (+ (+ |x#0@@0| |x#0@@0|) (+ |x#0@@0| |x#0@@0|))) (= (ControlFlow 0 2) (- 0 1))) ($Is intType (int_2_U (+ (+ |n#0| |n#0|) (+ |n#0| |n#0|))) Tclass._System.nat)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (<= (LitInt 0) |n#0|) (= 0 |b#0|)) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
