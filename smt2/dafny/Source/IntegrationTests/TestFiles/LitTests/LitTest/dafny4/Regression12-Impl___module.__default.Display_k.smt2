...
(set-info :boogie-vc-id Impl$$_module.__default.Display_k)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) ($Is SetType (|Set#Difference| |s#0| (|Set#UnionOne| |Set#Empty| ($Box refType |p#0|))) (TSet Tclass._module.Node)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is refType |p#0| Tclass._module.Node?) ($IsAlloc refType |p#0| Tclass._module.Node? $Heap))) (and (and ($Is SetType |s#0| (TSet Tclass._module.Node)) ($IsAlloc SetType |s#0| (TSet Tclass._module.Node) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2)))) anon0_correct)))
