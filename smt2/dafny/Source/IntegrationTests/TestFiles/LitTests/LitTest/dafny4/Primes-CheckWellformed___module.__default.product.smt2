...
(set-info :boogie-vc-id CheckWellformed$$_module.__default.product)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 7) (let ((anon8_Else_correct  (=> (and (not (|Set#Equal| |s#0@@5| |Set#Empty|)) ($IsAlloc SetType |s#0@@5| (TSet TInt) $Heap@@1)) (and (=> (= (ControlFlow 0 4) (- 0 5)) (not (|Set#Equal| |s#0@@5| |Set#Empty|))) (=> (not (|Set#Equal| |s#0@@5| |Set#Empty|)) (=> (and (and (and (|_module.__default.PickLargest#canCall| |s#0@@5|) (= |let#0#0#0| (_module.__default.PickLargest reveal__module._default.PickLargest |s#0@@5|))) (and (|_module.__default.PickLargest#canCall| |s#0@@5|) ($Is intType (int_2_U |let#0#0#0|) TInt))) (and (and (= |a#Z#0@0| |let#0#0#0|) (= |##s#1@0| (|Set#Difference| |s#0@@5| (|Set#UnionOne| |Set#Empty| ($Box intType (int_2_U |a#Z#0@0|)))))) (and ($IsAlloc SetType |##s#1@0| (TSet TInt) $Heap@@1) (= (ControlFlow 0 4) (- 0 3))))) (and (|Set#Subset| |##s#1@0| |s#0@@5|) (not (|Set#Subset| |s#0@@5| |##s#1@0|)))))))))
(let ((anon8_Then_correct true))
(let ((anon7_Then_correct true))
(let ((anon0_correct  (=> (= $_ReadsFrame@0 (|lambda#0| null $Heap@@1 alloc false)) (and (and (=> (= (ControlFlow 0 6) 1) anon7_Then_correct) (=> (= (ControlFlow 0 6) 2) anon8_Then_correct)) (=> (= (ControlFlow 0 6) 4) anon8_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap@@1) ($IsHeapAnchor $Heap@@1)) ($Is SetType |s#0@@5| (TSet TInt))) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 7) 6))) anon0_correct)))
