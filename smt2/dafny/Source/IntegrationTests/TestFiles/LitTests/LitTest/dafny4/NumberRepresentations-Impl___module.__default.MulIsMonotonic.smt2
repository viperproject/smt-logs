...
(set-info :boogie-vc-id Impl$$_module.__default.MulIsMonotonic)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (<= (Mul |a#0| |x#0|) (Mul |a#0| |y#0|)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (= 0 $FunctionContextHeight) (<= (LitInt 0) |a#0|)) (and (<= |x#0| |y#0|) (= (ControlFlow 0 3) 2))) anon0_correct))))
