...
(declare-fun M2.__default.Y () Bool)
(declare-fun M2.C.X () Bool)
...
(assert  (=> (< 0 $FunctionContextHeight) ($Is boolType (bool_2_U M2.__default.Y) TBool)))
(assert  (=> (< 0 $FunctionContextHeight) ($Is boolType (bool_2_U M2.C.X) TBool)))
...
(set-info :boogie-vc-id Impl$$M2.__default.A)
