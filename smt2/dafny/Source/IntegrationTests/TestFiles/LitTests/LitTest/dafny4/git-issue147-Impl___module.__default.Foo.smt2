...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (= (U_2_int ($Unbox intType (|Seq#Index| |s#0| |i#0|))) (U_2_int ($Unbox intType (|Seq#Index| |s#0| (+ |i#0| 1))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is SeqType |s#0| (TSeq TInt)) ($IsAlloc SeqType |s#0| (TSeq TInt) $Heap))) (and (and (= 0 $FunctionContextHeight) (<= (LitInt 0) |i#0|)) (and (< |i#0| (- (|Seq#Length| |s#0|) 1)) (= (ControlFlow 0 3) 2)))) anon0_correct)))
