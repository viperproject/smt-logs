...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.ZeroIsUnique)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) ($IsAlloc SeqType |a#0| (TSeq TInt) $Heap)) (and ($IsAlloc intType (int_2_U |lowDigit#0@@1|) TInt $Heap) ($IsAlloc intType (int_2_U |base#0@@5|) TInt $Heap))) (=> (and (and (and (|_module.__default.IsSkewNumber#canCall| |a#0| |lowDigit#0@@1| |base#0@@5|) (_module.__default.IsSkewNumber |a#0| |lowDigit#0@@1| |base#0@@5|)) (and ($IsAlloc SeqType |a#0| (TSeq TInt) $Heap) (|_module.__default.trim#canCall| |a#0|))) (and (and (|Seq#Equal| |a#0| (_module.__default.trim ($LS $LZ) |a#0|)) ($IsAlloc SeqType |a#0| (TSeq TInt) $Heap)) (and ($IsAlloc intType (int_2_U |base#0@@5|) TInt $Heap) (= (ControlFlow 0 2) (- 0 1))))) (<= (LitInt 2) |base#0@@5|)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is SeqType |a#0| (TSeq TInt)) ($IsAlloc SeqType |a#0| (TSeq TInt) $Heap)) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
