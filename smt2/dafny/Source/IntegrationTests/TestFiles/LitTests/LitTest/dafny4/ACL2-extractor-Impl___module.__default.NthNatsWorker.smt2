...
(set-info :boogie-vc-id Impl$$_module.__default.NthNatsWorker)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap@@0 alloc false)) (=> (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap@@0 $Heap@0)) (and (forall ((|$ih#i0#0| Int) (|$ih#n0#0| Int) ) (!  (=> (and (and (<= (LitInt 0) |$ih#n0#0|) (and (and (<= (LitInt 0) |$ih#i0#0|) (< |$ih#i0#0| |$ih#n0#0|)) (= (_module.__default.length TInt ($LS $LZ) (_module.__default.nats ($LS $LZ) |$ih#n0#0|)) |$ih#n0#0|))) (or (and (<= 0 |$ih#i0#0|) (< |$ih#i0#0| |i#0|)) (and (= |$ih#i0#0| |i#0|) (and (<= 0 |$ih#n0#0|) (< |$ih#n0#0| |n#0@@8|))))) (= (U_2_int ($Unbox intType (_module.__default.nthWorker TInt ($LS $LZ) |$ih#i0#0| (_module.__default.nats ($LS $LZ) |$ih#n0#0|)))) (- (- |$ih#n0#0| 1) |$ih#i0#0|)))
 :qid |ACL2extractordfy.190:21|
 :skolemid |625|
 :pattern ( ($Unbox intType (_module.__default.nthWorker TInt ($LS $LZ) |$ih#i0#0| (_module.__default.nats ($LS $LZ) |$ih#n0#0|))))
)) (= (ControlFlow 0 2) (- 0 1)))) (= (U_2_int ($Unbox intType (_module.__default.nthWorker TInt ($LS ($LS $LZ)) |i#0| (_module.__default.nats ($LS ($LS $LZ)) |n#0@@8|)))) (- (- |n#0@@8| 1) |i#0|))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap@@0) ($IsHeapAnchor $Heap@@0)) (and (<= (LitInt 0) |n#0@@8|) (= 3 $FunctionContextHeight))) (and (and (<= (LitInt 0) |i#0|) (< |i#0| |n#0@@8|)) (and (= (_module.__default.length TInt ($LS ($LS $LZ)) (_module.__default.nats ($LS ($LS $LZ)) |n#0@@8|)) |n#0@@8|) (= (ControlFlow 0 3) 2)))) anon0_correct)))
