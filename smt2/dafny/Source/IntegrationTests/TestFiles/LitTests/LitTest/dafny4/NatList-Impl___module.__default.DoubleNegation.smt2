...
(declare-fun $LZ () T@U)
(set-info :boogie-vc-id Impl$$_module.__default.DoubleNegation)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (|$IsA#_module.List| |xs#0@@3|)) (=> (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap $Heap@0)) (and (forall ((|$ih#xs0#0| T@U) ) (!  (=> (and (and ($Is DatatypeTypeType |$ih#xs0#0| (Tclass._module.List TInt)) (U_2_bool (Lit boolType (bool_2_U true)))) (< (DtRank |$ih#xs0#0|) (DtRank |xs#0@@3|))) (|_module.List#Equal| (_module.__default.Negate ($LS $LZ) (_module.__default.Negate ($LS $LZ) |$ih#xs0#0|)) |$ih#xs0#0|))
 :qid |NatListdfy.44:22|
 :skolemid |557|
 :pattern ( (_module.__default.Negate ($LS $LZ) (_module.__default.Negate ($LS $LZ) |$ih#xs0#0|)))
)) (= (ControlFlow 0 2) (- 0 1)))) (|_module.List#Equal| (_module.__default.Negate ($LS ($LS $LZ)) (_module.__default.Negate ($LS ($LS $LZ)) |xs#0@@3|)) |xs#0@@3|)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is DatatypeTypeType |xs#0@@3| (Tclass._module.List TInt)) ($IsAlloc DatatypeTypeType |xs#0@@3| (Tclass._module.List TInt) $Heap)) (|$IsA#_module.List| |xs#0@@3|)) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
