...
(let ((anon5_Else_correct  (=> (= |f#0@0| (Lit HandleTypeType (AtLayer HandleTypeType (|lambda#5| (Handle1 (|lambda#1| 1) (|lambda#2| TInt) (|lambda#4| (SetRef_to_SetBox (|lambda#3| false))))) ($LS $LZ)))) (=> (and (and ($IsAlloc HandleTypeType |f#0@0| (Tclass._System.___hTotalFunc1 TInt TInt) $Heap) (|_module.__default.five#canCall| |f#0@0|)) (and (|_module.__default.five#canCall| |f#0@0|) (= (ControlFlow 0 2) (- 0 1)))) (= (_module.__default.five |f#0@0|) (LitInt 5))))))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (and (=> (= (ControlFlow 0 6) 5) anon5_Then_correct) (=> (= (ControlFlow 0 6) 2) anon5_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is HandleTypeType |f#0@@5| (Tclass._System.___hTotalFunc1 TInt TInt)) ($IsAlloc HandleTypeType |f#0@@5| (Tclass._System.___hTotalFunc1 TInt TInt) $Heap)) true) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 7) 6))) anon0_correct))))
