...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.TrimResult)
...
(let ((anon4_Else_correct  (=> (and (< |last#Z#0@0| (LitInt 0)) (= (ControlFlow 0 4) 1)) anon3_correct)))
(let ((anon4_Then_correct  (=> (<= (LitInt 0) |last#Z#0@0|) (=> (and ($IsAlloc SeqType |digits#0@@3| (TSeq TInt) $Heap@0) (|_module.__default.trim#canCall| |digits#0@@3|)) (and (=> (= (ControlFlow 0 2) (- 0 3)) (and (<= 0 |last#Z#0@0|) (< |last#Z#0@0| (|Seq#Length| (_module.__default.trim ($LS $LZ) |digits#0@@3|))))) (=> (and (<= 0 |last#Z#0@0|) (< |last#Z#0@0| (|Seq#Length| (_module.__default.trim ($LS $LZ) |digits#0@@3|)))) (=> (= (ControlFlow 0 2) 1) anon3_correct)))))))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap $Heap@0)) (and ($IsAlloc SeqType |digits#0@@3| (TSeq TInt) $Heap@0) (|_module.__default.trim#canCall| |digits#0@@3|))) (and (and (= |let#0#0#0| (- (|Seq#Length| (_module.__default.trim ($LS $LZ) |digits#0@@3|)) 1)) (|_module.__default.trim#canCall| |digits#0@@3|)) (and ($Is intType (int_2_U |let#0#0#0|) TInt) (= |last#Z#0@0| |let#0#0#0|)))) (and (=> (= (ControlFlow 0 5) 2) anon4_Then_correct) (=> (= (ControlFlow 0 5) 4) anon4_Else_correct))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is SeqType |digits#0@@3| (TSeq TInt)) ($IsAlloc SeqType |digits#0@@3| (TSeq TInt) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct))))
