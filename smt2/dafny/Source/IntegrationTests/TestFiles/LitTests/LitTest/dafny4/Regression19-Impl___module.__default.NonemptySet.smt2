...
(assert (forall ((|l#0| T@U) (|l#1| T@U) (|l#2| T@U) (|l#3| Bool) ($o T@U) ($f T@U) ) (! (= (U_2_bool (MapType0Select refType FieldType boolType (|lambda#16| |l#0| |l#1| |l#2| |l#3|) $o $f))  (=> (and (or (not (= $o |l#0|)) (not true)) (U_2_bool ($Unbox boolType (MapType1Select FieldType BoxType (MapType1Select refType (MapType1Type FieldType BoxType) |l#1| $o) |l#2|)))) |l#3|))
 :qid |DafnyPreludebpl.156:1|
 :skolemid |548|
 :pattern ( (MapType0Select refType FieldType boolType (|lambda#16| |l#0| |l#1| |l#2| |l#3|) $o $f))
)))
...
(declare-fun |x#0| () Int)
(set-info :boogie-vc-id Impl$$_module.__default.NonemptySet)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#16| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (or (not (= (|Set#Card| |s#0|) 0)) (not true)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is SetType |s#0| (TSet TInt)) ($IsAlloc SetType |s#0| (TSet TInt) $Heap)) (= 0 $FunctionContextHeight)) (and (|Set#IsMember| |s#0| ($Box intType (int_2_U |x#0|))) (= (ControlFlow 0 3) 2))) anon0_correct))))
