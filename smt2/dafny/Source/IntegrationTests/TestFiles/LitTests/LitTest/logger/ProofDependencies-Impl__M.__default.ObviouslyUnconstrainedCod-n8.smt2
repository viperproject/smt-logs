...
(set-info :boogie-vc-id Impl$$M.__default.ObviouslyUnconstrainedCodeMethod)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (and (= |a#0@0| (+ |x#0| 1)) (= |b#0@0| (- |x#0| 1))) (and (= |r#0@0| (|#_System._tuple#2._#Make2| ($Box intType (int_2_U |a#0@0|)) ($Box intType (int_2_U |b#0@0|)))) (= (ControlFlow 0 2) (- 0 1)))) (> (U_2_int ($Unbox intType (_System.Tuple2._0 |r#0@0|))) 10)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is DatatypeTypeType |r#0| (Tclass._System.Tuple2 TInt TInt)) ($IsAlloc DatatypeTypeType |r#0| (Tclass._System.Tuple2 TInt TInt) $Heap)) (= 0 $FunctionContextHeight)) (and (> |x#0| 10) (= (ControlFlow 0 3) 2))) anon0_correct))))
