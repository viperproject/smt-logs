...
(assert (forall ((|c#0| T@U) ) (! (= ($Is refType |c#0| Tclass._module.MyClass)  (and ($Is refType |c#0| Tclass._module.MyClass?) (or (not (= |c#0| null)) (not true))))
 :qid |unknown.0:0|
 :skolemid |763|
 :pattern ( ($Is refType |c#0| Tclass._module.MyClass))
 :pattern ( ($Is refType |c#0| Tclass._module.MyClass?))
)))
...
(set-info :boogie-vc-id CheckWellFormed$$_module.ReallyJustANullableMyClass)
