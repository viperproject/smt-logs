...
(assert (distinct alloc Tagclass.Library.Mixed? Tagclass.Library.Mixed tytagFamily$Mixed)
)
...
(assert (forall ((|c#0| T@U) ($h T@U) ) (! (= ($IsAlloc refType |c#0| Tclass.Library.Mixed $h) ($IsAlloc refType |c#0| Tclass.Library.Mixed? $h))
 :qid |unknown.0:0|
 :skolemid |1157|
 :pattern ( ($IsAlloc refType |c#0| Tclass.Library.Mixed $h))
 :pattern ( ($IsAlloc refType |c#0| Tclass.Library.Mixed? $h))
)))
...
(assert (forall (($o@@0 T@U) ($h@@0 T@U) ) (! (= ($IsAlloc refType $o@@0 Tclass.Library.Mixed? $h@@0)  (or (= $o@@0 null) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@0 $o@@0) alloc)))))
 :qid |unknown.0:0|
 :skolemid |1131|
 :pattern ( ($IsAlloc refType $o@@0 Tclass.Library.Mixed? $h@@0))
)))
...
(assert (forall ((|c#0@@0| T@U) ) (! (= ($Is refType |c#0@@0| Tclass.Library.Mixed)  (and ($Is refType |c#0@@0| Tclass.Library.Mixed?) (or (not (= |c#0@@0| null)) (not true))))
 :qid |unknown.0:0|
 :skolemid |1156|
 :pattern ( ($Is refType |c#0@@0| Tclass.Library.Mixed))
 :pattern ( ($Is refType |c#0@@0| Tclass.Library.Mixed?))
)))
...
(set-info :boogie-vc-id Impl$$Library.Mixed.IM)
