...
procedure {:verboseName "NewtypeRangeRegression.Test (correctness)"} Impl$$NewtypeRangeRegression.__default.Test() returns ($_reverifyPost: bool);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "NewtypeRangeRegression.Test (correctness)"} Impl$$NewtypeRangeRegression.__default.Test() returns ($_reverifyPost: bool)
{
  var $_ModifiesFrame: [ref,Field]bool;
  var defass#k#0: bool;
  var k#0: int where defass#k#0 ==> LitInt(2) <= k#0 && k#0 < 5;
  var k#1: int;
  var newtype$check#0: int;

    // AddMethodImpl: Test, Impl$$NewtypeRangeRegression.__default.Test
    $_ModifiesFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    $_reverifyPost := false;
    // ----- assign-such-that statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/comp/Comprehensions.dfy(529,18)
    havoc k#1;
    if (LitInt(2) <= k#1 && k#1 < 5)
    {
        newtype$check#0 := LitInt(2);
        assert {:id "id54"} LitInt(2) <= newtype$check#0 && newtype$check#0 < 5;
        assume true;
    }

    assert {:id "id55"} ($Is(LitInt(5 - 1), Tclass.NewtypeRangeRegression.MyInt()) && Lit(5 - 1 != 2))
       || 
      ($Is(LitInt(2), Tclass.NewtypeRangeRegression.MyInt()) && Lit(2 != 2))
       || 
      ($Is(LitInt(0), Tclass.NewtypeRangeRegression.MyInt()) && Lit(0 != 2))
       || (exists $as#k0#0: int :: LitInt(2) <= $as#k0#0 && $as#k0#0 < 5 && $as#k0#0 != 2);
    defass#k#0 := true;
    havoc k#0;
    assume {:id "id56"} k#0 != 2;
    // ----- print statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/comp/Comprehensions.dfy(530,5)
    assert {:id "id57"} defass#k#0;
    assume true;
    assume true;
}
...
procedure {:verboseName "NewtypeRangeRegression.MyInt (well-formedness)"} CheckWellFormed$$NewtypeRangeRegression.MyInt(x#0: int);
