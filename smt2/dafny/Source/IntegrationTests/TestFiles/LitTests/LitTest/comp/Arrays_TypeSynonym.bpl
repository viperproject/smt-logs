...
const unique class.TypeSynonym.__default: ClassName;

function Tclass.TypeSynonym.uint8() : Ty
uses {
// Tclass.TypeSynonym.uint8 Tag
axiom Tag(Tclass.TypeSynonym.uint8()) == Tagclass.TypeSynonym.uint8
   && TagFamily(Tclass.TypeSynonym.uint8()) == tytagFamily$uint8;
}

const unique Tagclass.TypeSynonym.uint8: TyTag;

// Box/unbox axiom for Tclass.TypeSynonym.uint8
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.TypeSynonym.uint8()) } 
  $IsBox(bx, Tclass.TypeSynonym.uint8())
     ==> $Box($Unbox(bx): int) == bx && $Is($Unbox(bx): int, Tclass.TypeSynonym.uint8()));

function Tclass.TypeSynonym.buffer(Ty) : Ty;

const unique Tagclass.TypeSynonym.buffer: TyTag;

// Tclass.TypeSynonym.buffer Tag
axiom (forall TypeSynonym.buffer$T: Ty :: 
  { Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T) } 
  Tag(Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T))
       == Tagclass.TypeSynonym.buffer
     && TagFamily(Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T)) == tytagFamily$buffer);

function Tclass.TypeSynonym.buffer_0(Ty) : Ty;

// Tclass.TypeSynonym.buffer injectivity 0
axiom (forall TypeSynonym.buffer$T: Ty :: 
  { Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T) } 
  Tclass.TypeSynonym.buffer_0(Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T))
     == TypeSynonym.buffer$T);

// Box/unbox axiom for Tclass.TypeSynonym.buffer
axiom (forall TypeSynonym.buffer$T: Ty, bx: Box :: 
  { $IsBox(bx, Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T)) } 
  $IsBox(bx, Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T))
     ==> $Box($Unbox(bx): ref) == bx
       && $Is($Unbox(bx): ref, Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T)));

procedure {:verboseName "TypeSynonym.BufferTest (well-formedness)"} CheckWellFormed$$TypeSynonym.__default.BufferTest(b#0: ref
       where $Is(b#0, Tclass.TypeSynonym.buffer(Tclass.TypeSynonym.uint8()))
         && $IsAlloc(b#0, Tclass.TypeSynonym.buffer(Tclass.TypeSynonym.uint8()), $Heap));
  free requires 2 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "TypeSynonym.BufferTest (call)"} Call$$TypeSynonym.__default.BufferTest(b#0: ref
       where $Is(b#0, Tclass.TypeSynonym.buffer(Tclass.TypeSynonym.uint8()))
         && $IsAlloc(b#0, Tclass.TypeSynonym.buffer(Tclass.TypeSynonym.uint8()), $Heap));
  modifies $Heap;
...
procedure {:verboseName "TypeSynonym.BufferTest (correctness)"} Impl$$TypeSynonym.__default.BufferTest(b#0: ref
       where $Is(b#0, Tclass.TypeSynonym.buffer(Tclass.TypeSynonym.uint8()))
         && $IsAlloc(b#0, Tclass.TypeSynonym.buffer(Tclass.TypeSynonym.uint8()), $Heap))
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "TypeSynonym.BufferTest (correctness)"} Impl$$TypeSynonym.__default.BufferTest(b#0: ref) returns ($_reverifyPost: bool)
{
  var $_ModifiesFrame: [ref,Field]bool;
  var t#0: Seq
     where $Is(t#0, TSeq(Tclass.TypeSynonym.uint8()))
       && $IsAlloc(t#0, TSeq(Tclass.TypeSynonym.uint8()), $Heap);

    // AddMethodImpl: BufferTest, Impl$$TypeSynonym.__default.BufferTest
    $_ModifiesFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    $_reverifyPost := false;
    // ----- assignment statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/comp/Arrays.dfy(373,11)
    assume true;
    assert {:id "id0"} b#0 != null;
    assume true;
    t#0 := Seq#FromArray($Heap, b#0);
    // ----- print statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/comp/Arrays.dfy(374,5)
    assume true;
    assume true;
}
...
procedure {:verboseName "TypeSynonym.Test (correctness)"} Impl$$TypeSynonym.__default.Test() returns ($_reverifyPost: bool);
  free requires 3 == $FunctionContextHeight;
  modifies $Heap;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "TypeSynonym.Test (correctness)"} Impl$$TypeSynonym.__default.Test() returns ($_reverifyPost: bool)
{
  var $_ModifiesFrame: [ref,Field]bool;
  var b#0: ref
     where $Is(b#0, Tclass._System.array(Tclass.TypeSynonym.uint8()))
       && $IsAlloc(b#0, Tclass._System.array(Tclass.TypeSynonym.uint8()), $Heap);
  ...
  var b##0: ref;
    ...
    // AddMethodImpl: Test, Impl$$TypeSynonym.__default.Test
    $_ModifiesFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    $_reverifyPost := false;
    // ----- assignment statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/comp/Arrays.dfy(378,11)
    assume true;
    assert {:id "id2"} 0 <= LitInt(4);
    assert {:id "id3"} LitInt(4) == 4;
    newtype$check#0 := LitInt(19);
    assert {:id "id4"} LitInt(0) <= newtype$check#0 && newtype$check#0 < 256;
    newtype$check#1 := LitInt(18);
    assert {:id "id5"} LitInt(0) <= newtype$check#1 && newtype$check#1 < 256;
    newtype$check#2 := LitInt(9);
    assert {:id "id6"} LitInt(0) <= newtype$check#2 && newtype$check#2 < 256;
    newtype$check#3 := LitInt(8);
    assert {:id "id7"} LitInt(0) <= newtype$check#3 && newtype$check#3 < 256;
    havoc $nw;
    assume $nw != null && $Is($nw, Tclass._System.array?(Tclass.TypeSynonym.uint8()));
    assume !$Unbox(read($Heap, $nw, alloc)): bool;
    assume _System.array.Length($nw) == LitInt(4);
    assume $Unbox(read($Heap, $nw, IndexField(0))): int == LitInt(19);
    assume $Unbox(read($Heap, $nw, IndexField(1))): int == LitInt(18);
    assume $Unbox(read($Heap, $nw, IndexField(2))): int == LitInt(9);
    assume $Unbox(read($Heap, $nw, IndexField(3))): int == LitInt(8);
    $Heap := update($Heap, $nw, alloc, $Box(true));
    assume $IsGoodHeap($Heap);
    assume $IsHeapAnchor($Heap);
    b#0 := $nw;
    // ----- call statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/comp/Arrays.dfy(379,15)
    // TrCallStmt: Before ProcessCallStmt
    assume true;
    // ProcessCallStmt: CheckSubrange
    assert {:id "id9"} $Is(b#0, Tclass.TypeSynonym.buffer(Tclass.TypeSynonym.uint8()));
    b##0 := b#0;
    call {:id "id10"} Call$$TypeSynonym.__default.BufferTest(b##0);
    // TrCallStmt: After ProcessCallStmt
}



procedure {:verboseName "TypeSynonym.uint8 (well-formedness)"} CheckWellFormed$$TypeSynonym.uint8(i#0: int);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "TypeSynonym.uint8 (well-formedness)"} CheckWellFormed$$TypeSynonym.uint8(i#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    if (*)
    {
        ...
        assume {:id "id11"} LitInt(0) <= i#0 && i#0 < 256;
        assume false;
    }
    ...
    if (*)
    {
        ...
        assume true;
        assert {:id "id12"} {:subsumption 0} LitInt(0) <= LitInt(0);
        assert {:id "id13"} {:subsumption 0} Lit(0 < 256);
        assume false;
    }
}
...
// $Is axiom for newtype TypeSynonym.uint8
axiom (forall i#0: int :: 
  { $Is(i#0, Tclass.TypeSynonym.uint8()) } 
  $Is(i#0, Tclass.TypeSynonym.uint8()) <==> LitInt(0) <= i#0 && i#0 < 256);

// $IsAlloc axiom for newtype TypeSynonym.uint8
axiom (forall i#0: int, $h: Heap :: 
  { $IsAlloc(i#0, Tclass.TypeSynonym.uint8(), $h) } 
  $IsAlloc(i#0, Tclass.TypeSynonym.uint8(), $h));

const unique class.TypeSynonym.uint8: ClassName;

procedure {:verboseName "TypeSynonym.buffer (well-formedness)"} CheckWellFormed$$TypeSynonym.buffer(TypeSynonym.buffer$T: Ty, a#0: ref);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "TypeSynonym.buffer (well-formedness)"} CheckWellFormed$$TypeSynonym.buffer(TypeSynonym.buffer$T: Ty, a#0: ref)
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    if (*)
    {
        ...
        assume $Is(a#0, Tclass._System.array(TypeSynonym.buffer$T));
        assert {:id "id14"} a#0 != null;
        assume {:id "id15"} _System.array.Length(a#0) < 4294967296;
        assume false;
    }
...
// $Is axiom for subset type TypeSynonym.buffer
axiom (forall TypeSynonym.buffer$T: Ty, a#0: ref :: 
  { $Is(a#0, Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T)) } 
  $Is(a#0, Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T))
     <==> $Is(a#0, Tclass._System.array(TypeSynonym.buffer$T))
       && _System.array.Length(a#0) < 4294967296);

// $IsAlloc axiom for subset type TypeSynonym.buffer
axiom (forall TypeSynonym.buffer$T: Ty, a#0: ref, $h: Heap :: 
  { $IsAlloc(a#0, Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T), $h) } 
  $IsAlloc(a#0, Tclass.TypeSynonym.buffer(TypeSynonym.buffer$T), $h)
     <==> $IsAlloc(a#0, Tclass._System.array(TypeSynonym.buffer$T), $h));
