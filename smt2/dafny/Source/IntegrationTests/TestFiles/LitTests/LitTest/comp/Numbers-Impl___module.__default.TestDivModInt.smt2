...
(set-info :boogie-vc-id Impl$$_module.__default.TestDivModInt)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 4) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (and (=> (= (ControlFlow 0 2) (- 0 3)) (or (not (= |b#0| 0)) (not true))) (=> (= (ControlFlow 0 2) (- 0 1)) (or (not (= |b#0| 0)) (not true)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is SeqType |suffix#0| (TSeq TChar)) ($IsAlloc SeqType |suffix#0| (TSeq TChar) $Heap)) (= 0 $FunctionContextHeight)) (and (or (not (= |b#0| 0)) (not true)) (= (ControlFlow 0 4) 2))) anon0_correct))))
