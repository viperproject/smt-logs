...
(declare-fun DefaultValuedExpressions.ST3$T () T@U)
(declare-fun DefaultValuedExpressions.ST3$U () T@U)
...
(set-info :boogie-vc-id CheckWellFormed$$DefaultValuedExpressions.ST3)
...
(let ((anon8_Then_correct  (=> (= |let#0#0#0| (Lit MapType |Map#Empty|)) (=> (and ($Is MapType |let#0#0#0| (TMap DefaultValuedExpressions.ST3$T DefaultValuedExpressions.ST3$U)) (= |m#Z#0@0| |let#0#0#0|)) (and (=> (= (ControlFlow 0 6) 4) anon9_Then_correct) (=> (= (ControlFlow 0 6) 5) anon9_Else_correct))))))
(let ((anon10_Then_correct  (=> (= (ControlFlow 0 2) (- 0 1)) (<= (ite (let ((|m#1| (Lit MapType |Map#Empty|)))
(|Map#Equal| |m#1| |Map#Empty|)) 8 0) (LitInt 8)))))
