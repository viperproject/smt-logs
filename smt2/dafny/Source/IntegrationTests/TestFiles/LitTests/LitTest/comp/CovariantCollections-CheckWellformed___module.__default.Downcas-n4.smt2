...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($Heap T@U) (|a#0@@1| T@U) (|b#0@@1| T@U) ) (!  (=> (and (or (|_module.__default.DowncastFunction#canCall| |a#0@@1| |b#0@@1|) (and (< 1 $FunctionContextHeight) (and (and ($Is refType |a#0@@1| Tclass._module.Integer) ($IsAlloc refType |a#0@@1| Tclass._module.Integer $Heap)) (and ($Is refType |b#0@@1| Tclass._module.Integer) ($IsAlloc refType |b#0@@1| Tclass._module.Integer $Heap))))) ($IsGoodHeap $Heap)) ($IsAlloc SetType (_module.__default.DowncastFunction |a#0@@1| |b#0@@1|) (TSet Tclass._module.Integer) $Heap))
 :qid |CovariantCollectionsdfy.371:10|
 :skolemid |1406|
 :pattern ( ($IsAlloc SetType (_module.__default.DowncastFunction |a#0@@1| |b#0@@1|) (TSet Tclass._module.Integer) $Heap))
))))
...
(set-info :boogie-vc-id CheckWellformed$$_module.__default.DowncastFunction)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon5_Else_correct  (=> (and (and (= |let#0#0#0| (|Set#UnionOne| (|Set#UnionOne| |Set#Empty| ($Box refType |a#0@@3|)) ($Box refType |b#0@@3|))) ($Is SetType |let#0#0#0| (TSet Tclass._module.Number))) (and (= |m#Z#0@0| |let#0#0#0|) (= (ControlFlow 0 3) (- 0 2)))) ($Is SetType |m#Z#0@0| (TSet Tclass._module.Integer)))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap@@0) ($IsHeapAnchor $Heap@@0)) (=> (and (and ($Is refType |a#0@@3| Tclass._module.Integer) ($Is refType |b#0@@3| Tclass._module.Integer)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 5) 4))) anon0_correct))))
