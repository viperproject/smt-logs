...
procedure {:verboseName "TypeDescriptorsInCovariantPositions.Test (correctness)"} Impl$$TypeDescriptorsInCovariantPositions.__default.Test() returns ($_reverifyPost: bool);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "TypeDescriptorsInCovariantPositions.Test (correctness)"} Impl$$TypeDescriptorsInCovariantPositions.__default.Test() returns ($_reverifyPost: bool)
{
  var $_ModifiesFrame: [ref,Field]bool;
  var u#0: DatatypeType
     where $Is(u#0, Tclass.TypeDescriptorsInCovariantPositions.Unit(TReal))
       && $IsAlloc(u#0, Tclass.TypeDescriptorsInCovariantPositions.Unit(TReal), $Heap);
  var r0#0: real;
  var $rhs##0: real;
  var x##0: real;
  var $tmp##0: Box;
  ...
  var $tmp##1: Box;
    ...
    // AddMethodImpl: Test, Impl$$TypeDescriptorsInCovariantPositions.__default.Test
    $_ModifiesFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    $_reverifyPost := false;
    havoc u#0 /* where $Is(u#0, Tclass.TypeDescriptorsInCovariantPositions.Unit(TReal))
       && $IsAlloc(u#0, Tclass.TypeDescriptorsInCovariantPositions.Unit(TReal), $Heap) */;
    // ----- call statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/comp/Datatype.dfy(206,20)
    assume true;
    ...
    x##0 := LitReal(32e-1);
    call {:id "id0"} $tmp##0 := Call$$TypeDescriptorsInCovariantPositions.Unit.Get(TReal, u#0, $Box(x##0));
    havoc $rhs##0;
    assume $rhs##0 == $Unbox($tmp##0): real;
    // TrCallStmt: After ProcessCallStmt
    r0#0 := $rhs##0;
    // ----- call statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/comp/Datatype.dfy(207,20)
    assume true;
    // TrCallStmt: Adding lhs with type real
    // TrCallStmt: Before ProcessCallStmt
    assume true;
    call {:id "id2"} $tmp##1 := Call$$TypeDescriptorsInCovariantPositions.Unit.Het(TReal, u#0);
    havoc $rhs##1;
    assume $rhs##1 == $Unbox($tmp##1): real;
    // TrCallStmt: After ProcessCallStmt
    r1#0 := $rhs##1;
    // ----- print statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/comp/Datatype.dfy(208,5)
    assume true;
    assume true;
    assume true;
    assume true;
...
procedure {:verboseName "TypeDescriptorsInCovariantPositions.Unit.Get (correctness)"} Impl$$TypeDescriptorsInCovariantPositions.Unit.Get(TypeDescriptorsInCovariantPositions.Unit$X: Ty, 
    this: DatatypeType
       where $Is(this, 
          Tclass.TypeDescriptorsInCovariantPositions.Unit(TypeDescriptorsInCovariantPositions.Unit$X))
         && $IsAlloc(this, 
...
procedure {:verboseName "TypeDescriptorsInCovariantPositions.Unit.Het (correctness)"} Impl$$TypeDescriptorsInCovariantPositions.Unit.Het(TypeDescriptorsInCovariantPositions.Unit$X: Ty, 
    this: DatatypeType
       where $Is(this, 
          Tclass.TypeDescriptorsInCovariantPositions.Unit(TypeDescriptorsInCovariantPositions.Unit$X))
         && $IsAlloc(this, 
