...
(declare-fun |abc#Z#0@0| () Int)
(declare-fun |let#1#0#0| () Int)
(declare-fun |def#Z#0@0| () Int)
...
(set-info :boogie-vc-id CheckWellFormed$$Regressions.SoReal)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon6_Then_correct  (=> (= |let#0#0#0| (LitInt 10)) (=> (and ($Is intType (int_2_U |let#0#0#0|) TInt) (= |abc#Z#0@0| |let#0#0#0|)) (=> (and (and (= |let#1#0#0| |abc#Z#0@0|) ($Is intType (int_2_U |let#1#0#0|) TInt)) (and (= |def#Z#0@0| |let#1#0#0|) (= (ControlFlow 0 3) (- 0 2)))) (<= (LitReal 10.0) (+ 3.0 (LitReal 12.0))))))))
