...
(declare-fun DefaultValuedExpressions.ST2$T () T@U)
(declare-fun DefaultValuedExpressions.ST2$U () T@U)
...
(set-info :boogie-vc-id CheckWellFormed$$DefaultValuedExpressions.ST2)
...
(let ((anon8_Then_correct  (=> (= |let#0#0#0| (Lit MapType |Map#Empty|)) (=> (and ($Is MapType |let#0#0#0| (TMap DefaultValuedExpressions.ST2$T DefaultValuedExpressions.ST2$U)) (= |m#Z#0@0| |let#0#0#0|)) (and (=> (= (ControlFlow 0 6) 4) anon9_Then_correct) (=> (= (ControlFlow 0 6) 5) anon9_Else_correct))))))
