...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 7) (let ((anon8_Else_correct  (=> (and (not (|Set#Equal| |s#0@@4| |Set#Empty|)) ($IsAlloc SetType |s#0@@4| (TSet TInt) $Heap@@1)) (and (=> (= (ControlFlow 0 4) (- 0 5)) (not (|Set#Equal| |s#0@@4| |Set#Empty|))) (=> (not (|Set#Equal| |s#0@@4| |Set#Empty|)) (=> (and (and (and (|_module.__default.Pick#canCall| |s#0@@4|) (= |let#0#0#0| (_module.__default.Pick |s#0@@4|))) (and (|_module.__default.Pick#canCall| |s#0@@4|) ($Is intType (int_2_U |let#0#0#0|) TInt))) (and (and (= |x#Z#0@0| |let#0#0#0|) (= |##s#1@0| (|Set#Difference| |s#0@@4| (|Set#UnionOne| |Set#Empty| ($Box intType (int_2_U |x#Z#0@0|)))))) (and ($IsAlloc SetType |##s#1@0| (TSet TInt) $Heap@@1) (= (ControlFlow 0 4) (- 0 3))))) (and (|Set#Subset| |##s#1@0| |s#0@@4|) (not (|Set#Subset| |s#0@@4| |##s#1@0|)))))))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap@@1) ($IsHeapAnchor $Heap@@1)) ($Is SetType |s#0@@4| (TSet TInt))) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 7) 6))) anon0_correct)))
