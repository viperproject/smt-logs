...
(declare-fun Tagclass._System.array () T@U)
(declare-fun Tagclass.TypeSynonym.uint8 () T@U)
(declare-fun Tagclass.TypeSynonym.buffer () T@U)
...
(declare-fun Tclass.TypeSynonym.buffer_0 (T@U) T@U)
...
(assert (distinct TInt TagInt TagSeq alloc Tagclass._System.array? Tagclass._System.array Tagclass.TypeSynonym.uint8 Tagclass.TypeSynonym.buffer tytagFamily$array tytagFamily$uint8 tytagFamily$buffer)
)
(assert (= (FDim alloc) 0))
...
(assert (forall ((TypeSynonym.buffer$T@@1 T@U) ) (!  (and (= (Tag (Tclass.TypeSynonym.buffer TypeSynonym.buffer$T@@1)) Tagclass.TypeSynonym.buffer) (= (TagFamily (Tclass.TypeSynonym.buffer TypeSynonym.buffer$T@@1)) tytagFamily$buffer))
 :qid |unknown.0:0|
 :skolemid |713|
 :pattern ( (Tclass.TypeSynonym.buffer TypeSynonym.buffer$T@@1))
)))
(assert (forall ((bx T@U) ) (!  (=> ($IsBox bx Tclass.TypeSynonym.uint8) (and (= ($Box intType ($Unbox intType bx)) bx) ($Is intType ($Unbox intType bx) Tclass.TypeSynonym.uint8)))
 :qid |unknown.0:0|
 :skolemid |712|
 :pattern ( ($IsBox bx Tclass.TypeSynonym.uint8))
)))
...
(assert (forall ((TypeSynonym.buffer$T@@2 T@U) ) (! (= (Tclass.TypeSynonym.buffer_0 (Tclass.TypeSynonym.buffer TypeSynonym.buffer$T@@2)) TypeSynonym.buffer$T@@2)
 :qid |unknown.0:0|
 :skolemid |714|
 :pattern ( (Tclass.TypeSynonym.buffer TypeSynonym.buffer$T@@2))
)))
...
(assert (forall ((TypeSynonym.buffer$T@@3 T@U) (bx@@4 T@U) ) (!  (=> ($IsBox bx@@4 (Tclass.TypeSynonym.buffer TypeSynonym.buffer$T@@3)) (and (= ($Box refType ($Unbox refType bx@@4)) bx@@4) ($Is refType ($Unbox refType bx@@4) (Tclass.TypeSynonym.buffer TypeSynonym.buffer$T@@3))))
 :qid |unknown.0:0|
 :skolemid |715|
 :pattern ( ($IsBox bx@@4 (Tclass.TypeSynonym.buffer TypeSynonym.buffer$T@@3)))
)))
(assert (= (Tag Tclass.TypeSynonym.uint8) Tagclass.TypeSynonym.uint8))
(assert (= (TagFamily Tclass.TypeSynonym.uint8) tytagFamily$uint8))
(assert (forall ((|i#0@@0| T@U) ) (! (= ($Is intType |i#0@@0| Tclass.TypeSynonym.uint8)  (and (<= (LitInt 0) (U_2_int |i#0@@0|)) (< (U_2_int |i#0@@0|) 256)))
 :qid |unknown.0:0|
 :skolemid |723|
 :pattern ( ($Is intType |i#0@@0| Tclass.TypeSynonym.uint8))
)))
...
(set-info :boogie-vc-id Impl$$TypeSynonym.__default.BufferTest)
...
(let ((PreconditionGeneratedEntry_correct  (=> (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is refType |b#0| (Tclass.TypeSynonym.buffer Tclass.TypeSynonym.uint8)) ($IsAlloc refType |b#0| (Tclass.TypeSynonym.buffer Tclass.TypeSynonym.uint8) $Heap))) (=> (and (and (and ($Is SeqType |t#0| (TSeq Tclass.TypeSynonym.uint8)) ($IsAlloc SeqType |t#0| (TSeq Tclass.TypeSynonym.uint8) $Heap)) true) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
