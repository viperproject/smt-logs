...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.SetData_split2)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 2) (let ((anon0_correct  (=> (and ($IsGoodHeap $Heap@@44) ($IsHeapAnchor $Heap@@44)) (=> (and (and (and (and ($Is refType |universe#0| Tclass._module.Universe) ($IsAlloc refType |universe#0| Tclass._module.Universe $Heap@@44)) (and ($Is refType |running#0| Tclass._module.Thread) ($IsAlloc refType |running#0| Tclass._module.Thread $Heap@@44))) (and (and ($Is refType |mutex#0| Tclass._module.Mutex) ($IsAlloc refType |mutex#0| Tclass._module.Mutex $Heap@@44)) (= 21 $FunctionContextHeight))) (and (and (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap@@44 alloc |universe#0| ($Unbox SetType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap@@44 |universe#0|) _module.Universe.content)))) (or (not (= |universe#0| null)) (not true))) (and ($IsAllocBox ($Box refType |universe#0|) Tclass._module.Universe? $Heap@@44) (|_module.Universe.globalInv#canCall| $Heap@@44 |universe#0|))) (and (and (_module.Universe.globalInv $Heap@@44 |universe#0|) (or (not (= |universe#0| null)) (not true))) (and (|Set#IsMember| ($Unbox SetType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap@@44 |universe#0|) _module.Universe.content)) ($Box refType |running#0|)) (= (ControlFlow 0 2) (- 0 1)))))) (or (not (= |universe#0| null)) (not true))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
