...
(assert (distinct TagSet alloc allocName Tagclass._module.Universe Tagclass._module.Thread Tagclass._module.Mutex Tagclass._module.Universe? Tagclass._module.Object class._module.Universe? Tagclass._module.Object? Tagclass._module.OwnedObject Tagclass._module.Thread? Tagclass._module.OwnedObject? Tagclass._module.Mutex? tytagFamily$Universe tytagFamily$Thread tytagFamily$Mutex tytagFamily$Object tytagFamily$OwnedObject field$content)
)
(assert (= (FDim alloc) 0))
...
(assert (forall ((|c#0@@3| T@U) ($h@@6 T@U) ) (! (= ($IsAlloc refType |c#0@@3| Tclass._module.Mutex $h@@6) ($IsAlloc refType |c#0@@3| Tclass._module.Mutex? $h@@6))
 :qid |unknown.0:0|
 :skolemid |2164|
 :pattern ( ($IsAlloc refType |c#0@@3| Tclass._module.Mutex $h@@6))
 :pattern ( ($IsAlloc refType |c#0@@3| Tclass._module.Mutex? $h@@6))
)))
...
(assert (forall ((|c#0@@8| T@U) ) (! (= ($Is refType |c#0@@8| Tclass._module.Mutex)  (and ($Is refType |c#0@@8| Tclass._module.Mutex?) (or (not (= |c#0@@8| null)) (not true))))
 :qid |unknown.0:0|
 :skolemid |2163|
 :pattern ( ($Is refType |c#0@@8| Tclass._module.Mutex))
 :pattern ( ($Is refType |c#0@@8| Tclass._module.Mutex?))
)))
...
(assert (forall (($o@@9 T@U) ($heap@@1 T@U) ) (!  (=> ($IsAlloc refType $o@@9 Tclass._module.Mutex? $heap@@1) ($IsAlloc refType $o@@9 Tclass._module.OwnedObject? $heap@@1))
 :qid |unknown.0:0|
 :skolemid |2449|
 :pattern ( ($IsAlloc refType $o@@9 Tclass._module.Mutex? $heap@@1))
)))
...
(assert (forall (($o@@12 T@U) ) (!  (=> ($Is refType $o@@12 Tclass._module.Mutex?) ($Is refType $o@@12 Tclass._module.OwnedObject?))
 :qid |unknown.0:0|
 :skolemid |2448|
 :pattern ( ($Is refType $o@@12 Tclass._module.Mutex?))
)))
(assert (forall ((bx@@17 T@U) ) (!  (=> ($IsBox bx@@17 Tclass._module.Mutex?) ($IsBox bx@@17 Tclass._module.OwnedObject?))
 :qid |unknown.0:0|
 :skolemid |2446|
...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.SetData_split0)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 2) (let ((anon0_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is refType |universe#0| Tclass._module.Universe) ($IsAlloc refType |universe#0| Tclass._module.Universe $Heap)) (and ($Is refType |running#0| Tclass._module.Thread) ($IsAlloc refType |running#0| Tclass._module.Thread $Heap))) (=> (and (and (and ($Is refType |mutex#0| Tclass._module.Mutex) ($IsAlloc refType |mutex#0| Tclass._module.Mutex $Heap)) (= 21 $FunctionContextHeight)) (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc |universe#0| ($Unbox SetType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap |universe#0|) _module.Universe.content)))) (= (ControlFlow 0 2) (- 0 1)))) (or (not (= |universe#0| null)) (not true)))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
