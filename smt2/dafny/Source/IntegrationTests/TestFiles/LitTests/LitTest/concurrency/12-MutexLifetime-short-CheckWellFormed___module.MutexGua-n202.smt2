...
(set-info :boogie-vc-id CheckWellFormed$$_module.MutexGuardU32.__ctor_split2)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 2) (let ((anon0_correct  (=> (and ($IsGoodHeap $Heap@@80) ($IsHeapAnchor $Heap@@80)) (=> (and (and ($Is refType |universe#0| Tclass._module.Universe) ($IsAlloc refType |universe#0| Tclass._module.Universe $Heap@@80)) (and ($Is refType |running#0| Tclass._module.Thread) ($IsAlloc refType |running#0| Tclass._module.Thread $Heap@@80))) (=> (and (and (and (and ($Is refType |scope#0| Tclass._module.Lifetime) ($IsAlloc refType |scope#0| Tclass._module.Lifetime $Heap@@80)) (and ($Is refType |mutex#0| Tclass._module.Mutex) ($IsAlloc refType |mutex#0| Tclass._module.Mutex $Heap@@80))) (and (and ($Is refType |mutexScope#0| Tclass._module.Lifetime) ($IsAlloc refType |mutexScope#0| Tclass._module.Lifetime $Heap@@80)) (= 23 $FunctionContextHeight))) (and (and (and (= $_ModifiesFrame@0 (|lambda#250| null $Heap@@80 alloc |universe#0| ($Unbox SetType (MapType1Select FieldType BoxType (MapType1Select refType (MapType1Type FieldType BoxType) $Heap@@80 |universe#0|) _module.Universe.content)) Tclass._module.Lifetime ($Unbox SetType (MapType1Select FieldType BoxType (MapType1Select refType (MapType1Type FieldType BoxType) $Heap@@80 |universe#0|) _module.Universe.content)) $Heap@@80 _module.Lifetime.owner |running#0| |scope#0| |mutexScope#0|)) (or (not (= |universe#0| null)) (not true))) (and ($IsAllocBox ($Box refType |universe#0|) Tclass._module.Universe? $Heap@@80) (|_module.Universe.globalInv#canCall| $Heap@@80 |universe#0|))) (and (and (_module.Universe.globalInv $Heap@@80 |universe#0|) (or (not (= |universe#0| null)) (not true))) (and (|Set#Subset| (|Set#UnionOne| (|Set#UnionOne| (|Set#UnionOne| (|Set#UnionOne| |Set#Empty| ($Box refType |running#0|)) ($Box refType |scope#0|)) ($Box refType |mutex#0|)) ($Box refType |mutexScope#0|)) ($Unbox SetType (MapType1Select FieldType BoxType (MapType1Select refType (MapType1Type FieldType BoxType) $Heap@@80 |universe#0|) _module.Universe.content))) (= (ControlFlow 0 2) (- 0 1)))))) (or (not (= |scope#0| null)) (not true)))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
