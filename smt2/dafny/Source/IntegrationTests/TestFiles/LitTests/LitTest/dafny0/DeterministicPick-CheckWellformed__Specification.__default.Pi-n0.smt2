...
(declare-fun Specification.__default.Pick (T@U) Int)
(declare-fun |Specification.__default.Pick#canCall| (T@U) Bool)
...
(assert  (=> (<= 0 $FunctionContextHeight) (forall ((|s#0| T@U) ) (!  (=> (or (|Specification.__default.Pick#canCall| |s#0|) (and (< 0 $FunctionContextHeight) (and ($Is SetType |s#0| (TSet TInt)) (not (|Set#Equal| |s#0| |Set#Empty|))))) (|Set#IsMember| |s#0| ($Box intType (int_2_U (Specification.__default.Pick |s#0|)))))
 :qid |DeterministicPickdfy.5:17|
 :skolemid |528|
 :pattern ( (Specification.__default.Pick |s#0|))
))))
...
(set-info :boogie-vc-id CheckWellformed$$Specification.__default.Pick)
...
(let ((anon4_Then_correct  (=> ($IsAlloc SetType |s#0@@0| (TSet TInt) $Heap) (and (=> (= (ControlFlow 0 2) (- 0 3)) (not (|Set#Equal| |s#0@@0| |Set#Empty|))) (=> (and (not (|Set#Equal| |s#0@@0| |Set#Empty|)) (= (ControlFlow 0 2) (- 0 1))) (or (|Set#Equal| |s#0@@0| |s#0@@0|) (and (|Set#Subset| |s#0@@0| |s#0@@0|) (not (|Set#Subset| |s#0@@0| |s#0@@0|)))))))))
(let ((anon0_correct  (=> (and (= $_ReadsFrame@0 (|lambda#0| null $Heap alloc false)) (not (|Set#Equal| |s#0@@0| |Set#Empty|))) (and (=> (= (ControlFlow 0 5) 2) anon4_Then_correct) (=> (= (ControlFlow 0 5) 4) anon4_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) ($Is SetType |s#0@@0| (TSet TInt))) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct)))
