...
(set-info :boogie-vc-id CheckWellFormed$$AssumeTypeAssumptions.__default.Q1)
...
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#4| null $Heap alloc |a#0|)) (and (=> (= (ControlFlow 0 5) 2) anon4_Then_correct) (=> (= (ControlFlow 0 5) 4) anon4_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is refType |a#0| (Tclass._System.array (TSeq TInt))) ($IsAlloc refType |a#0| (Tclass._System.array (TSeq TInt)) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct))))
