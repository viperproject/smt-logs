...
(set-info :boogie-vc-id Impl$$_module.__default.MultisetCardinalityC)
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is MultiSetType |s#0| (TMultiSet TInt)) ($IsAlloc MultiSetType |s#0| (TMultiSet TInt) $Heap)) (= 0 $FunctionContextHeight)) (and (exists ((|x#1| Int) ) (! (> (|MultiSet#Multiplicity| |s#0| ($Box intType (int_2_U |x#1|))) 0)
 :qid |MultiSetsdfy.205:19|
 :skolemid |556|
 :pattern ( (|MultiSet#Multiplicity| |s#0| ($Box intType (int_2_U |x#1|))))
