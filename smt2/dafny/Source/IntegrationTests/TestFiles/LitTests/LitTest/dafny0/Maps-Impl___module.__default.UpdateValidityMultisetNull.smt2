...
(assert (distinct TagMultiSet alloc Tagclass._module.Elem? Tagclass._module.Elem tytagFamily$Elem)
)
...
(assert (forall ((bx T@U) ) (!  (=> ($IsBox bx Tclass._module.Elem?) (and (= ($Box refType ($Unbox refType bx)) bx) ($Is refType ($Unbox refType bx) Tclass._module.Elem?)))
 :qid |unknown.0:0|
 :skolemid |763|
 :pattern ( ($IsBox bx Tclass._module.Elem?))
)))
...
(set-info :boogie-vc-id Impl$$_module.__default.UpdateValidityMultisetNull)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 7) (let ((anon3_Else_correct  (=> (= (ControlFlow 0 5) (- 0 4)) (<= 0 (LitInt (- 0 5))))))
(let ((anon3_Then_correct  (and (=> (= (ControlFlow 0 2) (- 0 3)) ($Is refType |d#0| Tclass._module.Elem)) (=> ($Is refType |d#0| Tclass._module.Elem) (=> (= (ControlFlow 0 2) (- 0 1)) (<= 0 (LitInt 5)))))))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and ($Is MultiSetType |s#0@0| (TMultiSet Tclass._module.Elem)) ($IsAlloc MultiSetType |s#0@0| (TMultiSet Tclass._module.Elem) $Heap)) (and (=> (= (ControlFlow 0 6) 2) anon3_Then_correct) (=> (= (ControlFlow 0 6) 5) anon3_Else_correct))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is refType |d#0| Tclass._module.Elem?) ($IsAlloc refType |d#0| Tclass._module.Elem? $Heap)) (and ($Is refType |e#0| Tclass._module.Elem) ($IsAlloc refType |e#0| Tclass._module.Elem $Heap))) (=> (and (and (and ($Is MultiSetType |s#0| (TMultiSet Tclass._module.Elem)) ($IsAlloc MultiSetType |s#0| (TMultiSet Tclass._module.Elem) $Heap)) true) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 7) 6))) anon0_correct)))))
