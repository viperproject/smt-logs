...
const unique class.TerminationCheck__tricky_k.__default: ClassName;

procedure {:verboseName "TerminationCheck_tricky'.False (well-formedness)"} CheckWellFormed$$TerminationCheck__tricky_k.__default.False();
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "TerminationCheck_tricky'.False (call)"} Call$$TerminationCheck__tricky_k.__default.False();
  modifies $Heap;
  // user-defined postconditions
  free ensures true;
  ensures {:id "id729"} Lit(false);
  // frame condition
  free ensures old($Heap) == $Heap;



procedure {:verboseName "TerminationCheck_tricky'.False (correctness)"} Impl$$TerminationCheck__tricky_k.__default.False() returns ($_reverifyPost: bool);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;
  // user-defined postconditions
  free ensures true;
  ensures {:id "id730"} Lit(false);
  // frame condition
  free ensures old($Heap) == $Heap;



function Tclass.TerminationCheck__tricky_k.R() : Ty
uses {
// Tclass.TerminationCheck__tricky_k.R Tag
axiom Tag(Tclass.TerminationCheck__tricky_k.R())
     == Tagclass.TerminationCheck__tricky_k.R
   && TagFamily(Tclass.TerminationCheck__tricky_k.R()) == tytagFamily$R;
}

const unique Tagclass.TerminationCheck__tricky_k.R: TyTag;

// Box/unbox axiom for Tclass.TerminationCheck__tricky_k.R
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.TerminationCheck__tricky_k.R()) } 
  $IsBox(bx, Tclass.TerminationCheck__tricky_k.R())
     ==> $Box($Unbox(bx): DatatypeType) == bx
       && $Is($Unbox(bx): DatatypeType, Tclass.TerminationCheck__tricky_k.R()));

implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "TerminationCheck_tricky'.False (correctness)"} Impl$$TerminationCheck__tricky_k.__default.False() returns ($_reverifyPost: bool)
{
  var $_ModifiesFrame: [ref,Field]bool;
  var r#0: DatatypeType
     where $Is(r#0, Tclass.TerminationCheck__tricky_k.R())
       && $IsAlloc(r#0, Tclass.TerminationCheck__tricky_k.R(), $Heap);
  var n##0: int;

    // AddMethodImpl: False, Impl$$TerminationCheck__tricky_k.__default.False
    $_ModifiesFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    $_reverifyPost := false;
    // ----- assignment statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/DefaultParameters.dfy(476,11)
    assume true;
    assume true;
    r#0 := Lit(#TerminationCheck_tricky'.R.R(LitInt(-3)));
    // ----- assert statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/DefaultParameters.dfy(477,5)
    assume TerminationCheck__tricky_k.R.R_q(r#0);
    assume TerminationCheck__tricky_k.R.R_q(r#0);
    assert {:id "id732"} TerminationCheck__tricky_k.R.x(r#0) == LitInt(-3);
    // ----- call statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/DefaultParameters.dfy(478,8)
    // TrCallStmt: Before ProcessCallStmt
    assume TerminationCheck__tricky_k.R.R_q(r#0);
    assume TerminationCheck__tricky_k.R.R_q(r#0);
    // ProcessCallStmt: CheckSubrange
    n##0 := TerminationCheck__tricky_k.R.x(r#0);
    call {:id "id733"} Call$$TerminationCheck__tricky_k.__default.Nat(n##0);
    // TrCallStmt: After ProcessCallStmt
}



procedure {:verboseName "TerminationCheck_tricky'.Nat (well-formedness)"} CheckWellFormed$$TerminationCheck__tricky_k.__default.Nat(n#0: int where LitInt(0) <= n#0);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "TerminationCheck_tricky'.Nat (call)"} Call$$TerminationCheck__tricky_k.__default.Nat(n#0: int where LitInt(0) <= n#0);
  modifies $Heap;
  // user-defined postconditions
  free ensures true;
  ensures {:id "id735"} LitInt(0) <= n#0;
  // frame condition
  free ensures old($Heap) == $Heap;



procedure {:verboseName "TerminationCheck_tricky'.Nat (correctness)"} Impl$$TerminationCheck__tricky_k.__default.Nat(n#0: int where LitInt(0) <= n#0) returns ($_reverifyPost: bool);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;
  // user-defined postconditions
  free ensures true;
  ensures {:id "id736"} LitInt(0) <= n#0;
  // frame condition
  free ensures old($Heap) == $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "TerminationCheck_tricky'.Nat (correctness)"} Impl$$TerminationCheck__tricky_k.__default.Nat(n#0: int) returns ($_reverifyPost: bool)
{
  var $_ModifiesFrame: [ref,Field]bool;

    // AddMethodImpl: Nat, Impl$$TerminationCheck__tricky_k.__default.Nat
    $_ModifiesFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    $_reverifyPost := false;
}
...
// Constructor function declaration
function #TerminationCheck_tricky'.R.R(int) : DatatypeType;

const unique ##TerminationCheck_tricky'.R.R: DtCtorId
uses {
// Constructor identifier
axiom (forall a#0#0#0: int :: 
  { #TerminationCheck_tricky'.R.R(a#0#0#0) } 
  DatatypeCtorId(#TerminationCheck_tricky'.R.R(a#0#0#0))
     == ##TerminationCheck_tricky'.R.R);
}

function TerminationCheck__tricky_k.R.R_q(DatatypeType) : bool;

// Questionmark and identifier
axiom (forall d: DatatypeType :: 
  { TerminationCheck__tricky_k.R.R_q(d) } 
  TerminationCheck__tricky_k.R.R_q(d)
     <==> DatatypeCtorId(d) == ##TerminationCheck_tricky'.R.R);

procedure {:verboseName "#TerminationCheck_tricky'.R.R (well-formedness)"} CheckWellformed$$#TerminationCheck_tricky'.R.R(x#0: int where LitInt(0) <= x#0);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "#TerminationCheck_tricky'.R.R (well-formedness)"} CheckWellformed$$#TerminationCheck_tricky'.R.R(x#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    $_ReadsFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    // ----- call statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/DefaultParameters.dfy(481,33)
    // TrCallStmt: Before ProcessCallStmt
    assert {:id "id737"} false;
    call {:id "id738"} Call$$TerminationCheck__tricky_k.__default.False();
    // TrCallStmt: After ProcessCallStmt
    assume true;
    assert {:id "id739"} $Is(LitInt(-3), Tclass._System.nat());
}
...
axiom (forall d: DatatypeType :: 
  { TerminationCheck__tricky_k.R.R_q(d) } 
  TerminationCheck__tricky_k.R.R_q(d)
     ==> (exists a#0#0#0: int :: d == #TerminationCheck_tricky'.R.R(a#0#0#0)));

// Constructor $Is
axiom (forall a#1#0#0: int :: 
  { $Is(#TerminationCheck_tricky'.R.R(a#1#0#0), Tclass.TerminationCheck__tricky_k.R()) } 
  $Is(#TerminationCheck_tricky'.R.R(a#1#0#0), Tclass.TerminationCheck__tricky_k.R())
     <==> $Is(a#1#0#0, Tclass._System.nat()));

// Destructor $IsAlloc
axiom (forall d: DatatypeType, $h: Heap :: 
  { $IsAlloc(TerminationCheck__tricky_k.R.x(d), Tclass._System.nat(), $h) } 
  $IsGoodHeap($h)
       && 
      TerminationCheck__tricky_k.R.R_q(d)
       && $IsAlloc(d, Tclass.TerminationCheck__tricky_k.R(), $h)
     ==> $IsAlloc(TerminationCheck__tricky_k.R.x(d), Tclass._System.nat(), $h));

// Constructor literal
axiom (forall a#2#0#0: int :: 
  { #TerminationCheck_tricky'.R.R(LitInt(a#2#0#0)) } 
  #TerminationCheck_tricky'.R.R(LitInt(a#2#0#0))
     == Lit(#TerminationCheck_tricky'.R.R(a#2#0#0)));

function TerminationCheck__tricky_k.R.x(DatatypeType) : int;

// Constructor injectivity
axiom (forall a#3#0#0: int :: 
  { #TerminationCheck_tricky'.R.R(a#3#0#0) } 
  TerminationCheck__tricky_k.R.x(#TerminationCheck_tricky'.R.R(a#3#0#0))
     == a#3#0#0);

// Datatype $IsAlloc
axiom (forall d: DatatypeType, $h: Heap :: 
  { $IsAlloc(d, Tclass.TerminationCheck__tricky_k.R(), $h) } 
  $IsGoodHeap($h) && $Is(d, Tclass.TerminationCheck__tricky_k.R())
     ==> $IsAlloc(d, Tclass.TerminationCheck__tricky_k.R(), $h));

// Depth-one case-split function
function $IsA#TerminationCheck__tricky_k.R(DatatypeType) : bool;

// Depth-one case-split axiom
axiom (forall d: DatatypeType :: 
  { $IsA#TerminationCheck__tricky_k.R(d) } 
  $IsA#TerminationCheck__tricky_k.R(d) ==> TerminationCheck__tricky_k.R.R_q(d));

// Questionmark data type disjunctivity
axiom (forall d: DatatypeType :: 
  { TerminationCheck__tricky_k.R.R_q(d), $Is(d, Tclass.TerminationCheck__tricky_k.R()) } 
  $Is(d, Tclass.TerminationCheck__tricky_k.R())
     ==> TerminationCheck__tricky_k.R.R_q(d));

// Datatype extensional equality declaration
function TerminationCheck__tricky_k.R#Equal(DatatypeType, DatatypeType) : bool;

// Datatype extensional equality definition: #TerminationCheck_tricky'.R.R
axiom (forall a: DatatypeType, b: DatatypeType :: 
  { TerminationCheck__tricky_k.R#Equal(a, b) } 
  TerminationCheck__tricky_k.R#Equal(a, b)
     <==> TerminationCheck__tricky_k.R.x(a) == TerminationCheck__tricky_k.R.x(b));

// Datatype extensionality axiom: TerminationCheck_tricky'.R
axiom (forall a: DatatypeType, b: DatatypeType :: 
  { TerminationCheck__tricky_k.R#Equal(a, b) } 
  TerminationCheck__tricky_k.R#Equal(a, b) <==> a == b);

const unique class.TerminationCheck__tricky_k.R: ClassName;
