...
(assert (distinct alloc Tagclass._module.Node Tagclass._module.Node? tytagFamily$Node)
)
...
(set-info :boogie-vc-id Impl$$_module.Node.Max0)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 6) (let ((GeneratedUnifiedExit_correct  (=> (= (ControlFlow 0 2) (- 0 1)) (= |r#0@0| (ite (< |x#0| |y#0|) |y#0| |x#0|)))))
(let ((anon3_Else_correct  (=> (<= |y#0| |x#0|) (=> (and (= |r#0@0| |x#0|) (= (ControlFlow 0 4) 2)) GeneratedUnifiedExit_correct))))
(let ((anon3_Then_correct  (=> (< |x#0| |y#0|) (=> (and (= |r#0@0| |y#0|) (= (ControlFlow 0 3) 2)) GeneratedUnifiedExit_correct))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (or (not (= this null)) (not true)) (and ($Is refType this Tclass._module.Node) ($IsAlloc refType this Tclass._module.Node $Heap))) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct))))
