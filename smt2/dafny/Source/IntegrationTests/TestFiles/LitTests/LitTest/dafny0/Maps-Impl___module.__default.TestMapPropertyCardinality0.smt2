...
(set-info :boogie-vc-id Impl$$_module.__default.TestMapPropertyCardinality0)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (= (|Map#Card| |m#0|) (|Set#Card| (|Map#Domain| |m#0|))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is MapType |m#0| (TMap Tclass._module.MyClass (TBitvector 3))) ($IsAlloc MapType |m#0| (TMap Tclass._module.MyClass (TBitvector 3)) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
