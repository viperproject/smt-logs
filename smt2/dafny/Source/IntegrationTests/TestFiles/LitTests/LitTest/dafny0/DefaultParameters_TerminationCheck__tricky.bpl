...
const unique class.TerminationCheck__tricky.__default: ClassName;

procedure {:verboseName "TerminationCheck_tricky.False (well-formedness)"} CheckWellFormed$$TerminationCheck__tricky.__default.False();
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "TerminationCheck_tricky.False (call)"} Call$$TerminationCheck__tricky.__default.False();
  modifies $Heap;
  // user-defined postconditions
  free ensures true;
  ensures {:id "id710"} Lit(false);
  // frame condition
  free ensures old($Heap) == $Heap;



procedure {:verboseName "TerminationCheck_tricky.False (correctness)"} Impl$$TerminationCheck__tricky.__default.False() returns ($_reverifyPost: bool);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;
  // user-defined postconditions
  free ensures true;
  ensures {:id "id711"} Lit(false);
  // frame condition
  free ensures old($Heap) == $Heap;



function Tclass.TerminationCheck__tricky.S() : Ty
uses {
// Tclass.TerminationCheck__tricky.S Tag
axiom Tag(Tclass.TerminationCheck__tricky.S()) == Tagclass.TerminationCheck__tricky.S
   && TagFamily(Tclass.TerminationCheck__tricky.S()) == tytagFamily$S;
}

const unique Tagclass.TerminationCheck__tricky.S: TyTag;

// Box/unbox axiom for Tclass.TerminationCheck__tricky.S
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.TerminationCheck__tricky.S()) } 
  $IsBox(bx, Tclass.TerminationCheck__tricky.S())
     ==> $Box($Unbox(bx): DatatypeType) == bx
       && $Is($Unbox(bx): DatatypeType, Tclass.TerminationCheck__tricky.S()));

implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "TerminationCheck_tricky.False (correctness)"} Impl$$TerminationCheck__tricky.__default.False() returns ($_reverifyPost: bool)
{
  var $_ModifiesFrame: [ref,Field]bool;
  var s#0: DatatypeType
     where $Is(s#0, Tclass.TerminationCheck__tricky.S())
       && $IsAlloc(s#0, Tclass.TerminationCheck__tricky.S(), $Heap);
  var ##s#0: DatatypeType;

    // AddMethodImpl: False, Impl$$TerminationCheck__tricky.__default.False
    $_ModifiesFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    $_reverifyPost := false;
    // ----- assignment statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/DefaultParameters.dfy(457,11)
    assume true;
    assume true;
    s#0 := Lit(#TerminationCheck_tricky.S.S(LitInt(-3)));
    // ----- assert statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/DefaultParameters.dfy(458,5)
    assume TerminationCheck__tricky.S.S_q(s#0);
    assume TerminationCheck__tricky.S.S_q(s#0);
    assert {:id "id713"} TerminationCheck__tricky.S.x(s#0) == LitInt(-3);
    // ----- assert statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/DefaultParameters.dfy(459,5)
    ##s#0 := s#0;
    // assume allocatedness for argument to function
    assume $IsAlloc(##s#0, Tclass.TerminationCheck__tricky.S(), $Heap);
    assert {:id "id714"} true;
    assume TerminationCheck__tricky.__default.Id#canCall(s#0);
    assume TerminationCheck__tricky.S.S_q(TerminationCheck__tricky.__default.Id($LS($LZ), s#0));
    assume TerminationCheck__tricky.S.S_q(TerminationCheck__tricky.__default.Id($LS($LZ), s#0));
    assume TerminationCheck__tricky.__default.Id#canCall(s#0)
       && TerminationCheck__tricky.S.S_q(TerminationCheck__tricky.__default.Id($LS($LZ), s#0));
    assert {:id "id715"} {:subsumption 0} LitInt(0)
       <= TerminationCheck__tricky.S.x(TerminationCheck__tricky.__default.Id($LS($LS($LZ)), s#0));
    assume {:id "id716"} LitInt(0)
       <= TerminationCheck__tricky.S.x(TerminationCheck__tricky.__default.Id($LS($LZ), s#0));
}



// function declaration for TerminationCheck_tricky._default.Id
function TerminationCheck__tricky.__default.Id($ly: LayerType, s#0: DatatypeType) : DatatypeType
uses {
// consequence axiom for TerminationCheck__tricky.__default.Id
axiom 0 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, s#0: DatatypeType :: 
    { TerminationCheck__tricky.__default.Id($ly, s#0) } 
    TerminationCheck__tricky.__default.Id#canCall(s#0)
         || (0 < $FunctionContextHeight && $Is(s#0, Tclass.TerminationCheck__tricky.S()))
       ==> LitInt(0)
           <= TerminationCheck__tricky.S.x(TerminationCheck__tricky.__default.Id($ly, s#0))
         && $Is(TerminationCheck__tricky.__default.Id($ly, s#0), 
          Tclass.TerminationCheck__tricky.S()));
// definition axiom for TerminationCheck__tricky.__default.Id (revealed)
axiom {:id "id717"} 0 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, s#0: DatatypeType :: 
    { TerminationCheck__tricky.__default.Id($LS($ly), s#0) } 
    TerminationCheck__tricky.__default.Id#canCall(s#0)
         || (0 < $FunctionContextHeight && $Is(s#0, Tclass.TerminationCheck__tricky.S()))
       ==> TerminationCheck__tricky.S.S_q(s#0)
         && TerminationCheck__tricky.__default.Id($LS($ly), s#0)
           == (var n#0 := TerminationCheck__tricky.S.x(s#0); s#0));
// definition axiom for TerminationCheck__tricky.__default.Id for all literals (revealed)
axiom {:id "id718"} 0 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, s#0: DatatypeType :: 
    {:weight 3} { TerminationCheck__tricky.__default.Id($LS($ly), Lit(s#0)) } 
    TerminationCheck__tricky.__default.Id#canCall(Lit(s#0))
         || (0 < $FunctionContextHeight && $Is(s#0, Tclass.TerminationCheck__tricky.S()))
       ==> TerminationCheck__tricky.S.S_q(Lit(s#0))
         && TerminationCheck__tricky.__default.Id($LS($ly), Lit(s#0))
           == (var n#1 := LitInt(TerminationCheck__tricky.S.x(Lit(s#0))); Lit(s#0)));
}

function TerminationCheck__tricky.__default.Id#canCall(s#0: DatatypeType) : bool;
...
// fuel synonym axiom
axiom (forall $ly: LayerType, s#0: DatatypeType :: 
  { TerminationCheck__tricky.__default.Id(AsFuelBottom($ly), s#0) } 
  TerminationCheck__tricky.__default.Id($ly, s#0)
     == TerminationCheck__tricky.__default.Id($LZ, s#0));

function TerminationCheck__tricky.__default.Id#requires(LayerType, DatatypeType) : bool;

// #requires axiom for TerminationCheck__tricky.__default.Id
axiom (forall $ly: LayerType, s#0: DatatypeType :: 
  { TerminationCheck__tricky.__default.Id#requires($ly, s#0) } 
  $Is(s#0, Tclass.TerminationCheck__tricky.S())
     ==> TerminationCheck__tricky.__default.Id#requires($ly, s#0) == true);

procedure {:verboseName "TerminationCheck_tricky.Id (well-formedness)"} CheckWellformed$$TerminationCheck__tricky.__default.Id(s#0: DatatypeType where $Is(s#0, Tclass.TerminationCheck__tricky.S()));
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;
  ensures {:id "id719"} LitInt(0)
     <= TerminationCheck__tricky.S.x(TerminationCheck__tricky.__default.Id($LS($LS($LZ)), s#0));



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "TerminationCheck_tricky.Id (well-formedness)"} CheckWellformed$$TerminationCheck__tricky.__default.Id(s#0: DatatypeType)
{
  ...
  var let#0#0#0: int;
    ...
    if (*)
    {
        ...
        assume $Is(TerminationCheck__tricky.__default.Id($LS($LZ), s#0), 
          Tclass.TerminationCheck__tricky.S());
        ##s#0 := s#0;
        // assume allocatedness for argument to function
        assume $IsAlloc(##s#0, Tclass.TerminationCheck__tricky.S(), $Heap);
        assert {:id "id720"} TerminationCheck__tricky.S#Equal(s#0, s#0) || DtRank(##s#0) < DtRank(s#0);
        assume TerminationCheck__tricky.S#Equal(s#0, s#0)
           || TerminationCheck__tricky.__default.Id#canCall(s#0);
        assume TerminationCheck__tricky.S.S_q(TerminationCheck__tricky.__default.Id($LS($LZ), s#0));
        assume TerminationCheck__tricky.S.S_q(TerminationCheck__tricky.__default.Id($LS($LZ), s#0));
        assume {:id "id721"} LitInt(0)
           <= TerminationCheck__tricky.S.x(TerminationCheck__tricky.__default.Id($LS($LZ), s#0));
        assume false;
    }
    else
    {
        ...
        havoc n#Z#0;
        assume TerminationCheck__tricky.S.S_q(s#0);
        assume {:id "id722"} let#0#0#0 == TerminationCheck__tricky.S.x(s#0);
        assume TerminationCheck__tricky.S.S_q(s#0);
        // CheckWellformedWithResult: any expression
        assume $Is(let#0#0#0, Tclass._System.nat());
        assume {:id "id723"} n#Z#0 == let#0#0#0;
        assume {:id "id724"} TerminationCheck__tricky.__default.Id($LS($LZ), s#0) == s#0;
        assume true;
        // CheckWellformedWithResult: any expression
        assume $Is(TerminationCheck__tricky.__default.Id($LS($LZ), s#0), 
          Tclass.TerminationCheck__tricky.S());
...
// Constructor function declaration
function #TerminationCheck_tricky.S.S(int) : DatatypeType;

const unique ##TerminationCheck_tricky.S.S: DtCtorId
uses {
// Constructor identifier
axiom (forall a#0#0#0: int :: 
  { #TerminationCheck_tricky.S.S(a#0#0#0) } 
  DatatypeCtorId(#TerminationCheck_tricky.S.S(a#0#0#0))
     == ##TerminationCheck_tricky.S.S);
}

function TerminationCheck__tricky.S.S_q(DatatypeType) : bool;

// Questionmark and identifier
axiom (forall d: DatatypeType :: 
  { TerminationCheck__tricky.S.S_q(d) } 
  TerminationCheck__tricky.S.S_q(d)
     <==> DatatypeCtorId(d) == ##TerminationCheck_tricky.S.S);

procedure {:verboseName "#TerminationCheck_tricky.S.S (well-formedness)"} CheckWellformed$$#TerminationCheck_tricky.S.S(x#0: int where LitInt(0) <= x#0);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "#TerminationCheck_tricky.S.S (well-formedness)"} CheckWellformed$$#TerminationCheck_tricky.S.S(x#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    $_ReadsFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    // ----- call statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/DefaultParameters.dfy(453,33)
    // TrCallStmt: Before ProcessCallStmt
    assert {:id "id725"} false;
    call {:id "id726"} Call$$TerminationCheck__tricky.__default.False();
    // TrCallStmt: After ProcessCallStmt
    assume true;
    assert {:id "id727"} $Is(LitInt(-3), Tclass._System.nat());
}
...
axiom (forall d: DatatypeType :: 
  { TerminationCheck__tricky.S.S_q(d) } 
  TerminationCheck__tricky.S.S_q(d)
     ==> (exists a#0#0#0: int :: d == #TerminationCheck_tricky.S.S(a#0#0#0)));

// Constructor $Is
axiom (forall a#1#0#0: int :: 
  { $Is(#TerminationCheck_tricky.S.S(a#1#0#0), Tclass.TerminationCheck__tricky.S()) } 
  $Is(#TerminationCheck_tricky.S.S(a#1#0#0), Tclass.TerminationCheck__tricky.S())
     <==> $Is(a#1#0#0, Tclass._System.nat()));

// Destructor $IsAlloc
axiom (forall d: DatatypeType, $h: Heap :: 
  { $IsAlloc(TerminationCheck__tricky.S.x(d), Tclass._System.nat(), $h) } 
  $IsGoodHeap($h)
       && 
      TerminationCheck__tricky.S.S_q(d)
       && $IsAlloc(d, Tclass.TerminationCheck__tricky.S(), $h)
     ==> $IsAlloc(TerminationCheck__tricky.S.x(d), Tclass._System.nat(), $h));

// Constructor literal
axiom (forall a#2#0#0: int :: 
  { #TerminationCheck_tricky.S.S(LitInt(a#2#0#0)) } 
  #TerminationCheck_tricky.S.S(LitInt(a#2#0#0))
     == Lit(#TerminationCheck_tricky.S.S(a#2#0#0)));

function TerminationCheck__tricky.S.x(DatatypeType) : int;

// Constructor injectivity
axiom (forall a#3#0#0: int :: 
  { #TerminationCheck_tricky.S.S(a#3#0#0) } 
  TerminationCheck__tricky.S.x(#TerminationCheck_tricky.S.S(a#3#0#0)) == a#3#0#0);

// Datatype $IsAlloc
axiom (forall d: DatatypeType, $h: Heap :: 
  { $IsAlloc(d, Tclass.TerminationCheck__tricky.S(), $h) } 
  $IsGoodHeap($h) && $Is(d, Tclass.TerminationCheck__tricky.S())
     ==> $IsAlloc(d, Tclass.TerminationCheck__tricky.S(), $h));

// Depth-one case-split function
function $IsA#TerminationCheck__tricky.S(DatatypeType) : bool;

// Depth-one case-split axiom
axiom (forall d: DatatypeType :: 
  { $IsA#TerminationCheck__tricky.S(d) } 
  $IsA#TerminationCheck__tricky.S(d) ==> TerminationCheck__tricky.S.S_q(d));

// Questionmark data type disjunctivity
axiom (forall d: DatatypeType :: 
  { TerminationCheck__tricky.S.S_q(d), $Is(d, Tclass.TerminationCheck__tricky.S()) } 
  $Is(d, Tclass.TerminationCheck__tricky.S())
     ==> TerminationCheck__tricky.S.S_q(d));

// Datatype extensional equality declaration
function TerminationCheck__tricky.S#Equal(DatatypeType, DatatypeType) : bool;

// Datatype extensional equality definition: #TerminationCheck_tricky.S.S
axiom (forall a: DatatypeType, b: DatatypeType :: 
  { TerminationCheck__tricky.S#Equal(a, b) } 
  TerminationCheck__tricky.S#Equal(a, b)
     <==> TerminationCheck__tricky.S.x(a) == TerminationCheck__tricky.S.x(b));

// Datatype extensionality axiom: TerminationCheck_tricky.S
axiom (forall a: DatatypeType, b: DatatypeType :: 
  { TerminationCheck__tricky.S#Equal(a, b) } 
  TerminationCheck__tricky.S#Equal(a, b) <==> a == b);

const unique class.TerminationCheck__tricky.S: ClassName;
