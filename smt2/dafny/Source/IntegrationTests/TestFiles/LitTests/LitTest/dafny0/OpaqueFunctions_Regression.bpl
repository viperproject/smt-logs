...
axiom 1 <= $FunctionContextHeight
   ==> (forall Regression._default.Empty$A: Ty :: 
    ...
    Regression.__default.Empty#canCall(Regression._default.Empty$A)
...
axiom 1 <= $FunctionContextHeight
   ==> (forall $Heap: Heap, Regression._default.Empty$A: Ty :: 
...
axiom {:id "id282"} 1 <= $FunctionContextHeight
   ==> (forall Regression._default.Empty$A: Ty :: 
...
procedure {:verboseName "Regression.Empty (well-formedness)"} CheckWellformed$$Regression.__default.Empty(Regression._default.Empty$A: Ty);
...
axiom 1 <= $FunctionContextHeight
   ==> (forall Regression._default.Length$A: Ty, 
      $ly: LayerType, 
      $reveal: bool, 
      s#0: DatatypeType :: 
...
axiom {:id "id284"} 1 <= $FunctionContextHeight
...
axiom {:id "id285"} 1 <= $FunctionContextHeight
   ==> (forall Regression._default.Length$A: Ty, $ly: LayerType, s#0: DatatypeType :: 
...
procedure {:verboseName "Regression.Length (well-formedness)"} CheckWellformed$$Regression.__default.Length(Regression._default.Length$A: Ty, 
    s#0: DatatypeType
       where $Is(s#0, Tclass.Regression.List(Regression._default.Length$A)));
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;
  ensures {:id "id286"} LitInt(0)
     <= Regression.__default.Length(Regression._default.Length$A, 
      $LS($LS($LZ)), 
      reveal_Regression._default.Length, 
      s#0);
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Regression.Length (well-formedness)"} CheckWellformed$$Regression.__default.Length(Regression._default.Length$A: Ty, s#0: DatatypeType)
{
  ...
  var ##s#1: DatatypeType;
    ...
    if (*)
    {
        ...
        assume $IsAlloc(##s#0, Tclass.Regression.List(Regression._default.Length$A), $Heap);
        assert {:id "id287"} Regression.List#Equal(s#0, s#0) || DtRank(##s#0) < DtRank(s#0);
        assume Regression.List#Equal(s#0, s#0)
           || Regression.__default.Length#canCall(Regression._default.Length$A, s#0);
        assume {:id "id288"} LitInt(0)
           <= Regression.__default.Length(Regression._default.Length$A, $LS($LZ), reveal_Regression._default.Length, s#0);
        assume false;
    }
    else
    {
        ...
        if (Regression.List.Cons_q(s#0))
        {
            assert {:id "id289"} Regression.List.Cons_q(s#0);
            ##s#1 := Regression.List.tl(s#0);
            // assume allocatedness for argument to function
            assume $IsAlloc(##s#1, Tclass.Regression.List(Regression._default.Length$A), $Heap);
            assert {:id "id290"} DtRank(##s#1) < DtRank(s#0);
            assume Regression.__default.Length#canCall(Regression._default.Length$A, Regression.List.tl(s#0));
            assume {:id "id291"} Regression.__default.Length(Regression._default.Length$A, $LS($LZ), reveal_Regression._default.Length, s#0)
               == 1
                 + Regression.__default.Length(Regression._default.Length$A, 
                  $LS($LZ), 
                  reveal_Regression._default.Length, 
            ...
            assume $Is(Regression.__default.Length(Regression._default.Length$A, $LS($LZ), reveal_Regression._default.Length, s#0), 
              TInt);
            return;
        }
        else
        {
            assume {:id "id292"} Regression.__default.Length(Regression._default.Length$A, $LS($LZ), reveal_Regression._default.Length, s#0)
               == LitInt(0);
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(Regression.__default.Length(Regression._default.Length$A, $LS($LZ), reveal_Regression._default.Length, s#0), 
...
procedure {:verboseName "Regression.Empty_ToZero (well-formedness)"} CheckWellFormed$$Regression.__default.Empty__ToZero(Regression._default.Empty_ToZero$A: Ty);
  free requires 2 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "Regression.Empty_ToZero (call)"} Call$$Regression.__default.Empty__ToZero(Regression._default.Empty_ToZero$A: Ty);
  modifies $Heap;
  // user-defined postconditions
  free ensures Regression.__default.Empty#canCall(Regression._default.Empty_ToZero$A)
     && Regression.__default.Length#canCall(Regression._default.Empty_ToZero$A, 
      Lit(Regression.__default.Empty(Regression._default.Empty_ToZero$A)));
  ensures {:id "id294"} Regression.__default.Length(Regression._default.Empty_ToZero$A, 
      $LS($LS($LZ)), 
      reveal_Regression._default.Length, 
      Lit(Regression.__default.Empty(Regression._default.Empty_ToZero$A)))
     == LitInt(0);
...
procedure {:verboseName "Regression.Empty_ToZero (correctness)"} Impl$$Regression.__default.Empty__ToZero(Regression._default.Empty_ToZero$A: Ty) returns ($_reverifyPost: bool);
  free requires 2 == $FunctionContextHeight;
  modifies $Heap;
  // user-defined postconditions
  free ensures Regression.__default.Empty#canCall(Regression._default.Empty_ToZero$A)
     && Regression.__default.Length#canCall(Regression._default.Empty_ToZero$A, 
      Lit(Regression.__default.Empty(Regression._default.Empty_ToZero$A)));
  ensures {:id "id295"} Regression.__default.Length(Regression._default.Empty_ToZero$A, 
      $LS($LS($LZ)), 
      reveal_Regression._default.Length, 
      Lit(Regression.__default.Empty(Regression._default.Empty_ToZero$A)))
     == LitInt(0);
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Regression.Empty_ToZero (correctness)"} Impl$$Regression.__default.Empty__ToZero(Regression._default.Empty_ToZero$A: Ty) returns ($_reverifyPost: bool)
{
  var $_ModifiesFrame: [ref,Field]bool;

    // AddMethodImpl: Empty_ToZero, Impl$$Regression.__default.Empty__ToZero
    $_ModifiesFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    $_reverifyPost := false;
    // ----- hide/reveal statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/OpaqueFunctions.dfy(282,5)
    // ----- call statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/OpaqueFunctions.dfy(282,12)
    // TrCallStmt: Before ProcessCallStmt
    call {:id "id296"} Call$$Regression.__default.reveal__Length();
    // TrCallStmt: After ProcessCallStmt
}



procedure {:verboseName "Regression.reveal_Length (well-formedness)"} {:verify false} CheckWellFormed$$Regression.__default.reveal__Length();
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "Regression.reveal_Length (call)"} {:verify false} Call$$Regression.__default.reveal__Length();
  modifies $Heap;
  // frame condition
  free ensures old($Heap) == $Heap;
  free ensures reveal_Regression._default.Length;
