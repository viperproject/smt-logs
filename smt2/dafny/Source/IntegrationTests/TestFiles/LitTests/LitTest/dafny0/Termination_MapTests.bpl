...
axiom {:id "id17"} 0 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, a#0: Map, n#0: int :: 
    { MapTests.__default.F($LS($ly), a#0, n#0) } 
    MapTests.__default.F#canCall(a#0, n#0)
         || (0 < $FunctionContextHeight && $Is(a#0, TMap(TInt, TInt)) && LitInt(0) <= n#0)
...
procedure {:verboseName "MapTests.F (well-formedness)"} CheckWellformed$$MapTests.__default.F(a#0: Map where $Is(a#0, TMap(TInt, TInt)), n#0: int where LitInt(0) <= n#0);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "MapTests.F (well-formedness)"} CheckWellformed$$MapTests.__default.F(a#0: Map, n#0: int)
{
  ...
  var ##n#0: int;
    ...
    else
    {
        ...
        if (n#0 == LitInt(0))
        {
            assume {:id "id19"} MapTests.__default.F($LS($LZ), a#0, n#0) == LitInt(0);
            assume true;
...
procedure {:verboseName "MapTests.F' (well-formedness)"} CheckWellformed$$MapTests.__default.F_k(a#0: Map where $Is(a#0, TMap(TInt, TInt)), n#0: int where LitInt(0) <= n#0);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "MapTests.F' (well-formedness)"} CheckWellformed$$MapTests.__default.F_k(a#0: Map, n#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
  var ##a#0: Map;
  var ##n#0: int;
    ...
    else
    {
        ...
        if (n#0 == LitInt(0))
        {
            assume {:id "id26"} MapTests.__default.F_k($LS($LZ), a#0, n#0) == LitInt(0);
            assume true;
            // CheckWellformedWithResult: any expression
        ...
        else
        {
            ...
            // assume allocatedness for argument to function
            assume $IsAlloc(##n#0, Tclass._System.nat(), $Heap);
            assert {:id "id28"} 0 <= n#0
               || (Set#Subset(Map#Domain(##a#0), Map#Domain(a#0))
                 && !Set#Subset(Map#Domain(a#0), Map#Domain(##a#0)))
               || ##n#0 == n#0;
            assert {:id "id29"} (Set#Subset(Map#Domain(##a#0), Map#Domain(a#0))
                 && !Set#Subset(Map#Domain(a#0), Map#Domain(##a#0)))
               || (Set#Equal(Map#Domain(##a#0), Map#Domain(a#0)) && ##n#0 < n#0);
            assume MapTests.__default.F_k#canCall(a#0, n#0 - 1);
            assume {:id "id30"} MapTests.__default.F_k($LS($LZ), a#0, n#0)
...
procedure {:verboseName "MapTests.M (well-formedness)"} CheckWellFormed$$MapTests.__default.M(n#0: int where LitInt(0) <= n#0, 
    b#0: Map
...
procedure {:verboseName "MapTests.M (call)"} Call$$MapTests.__default.M(n#0: int where LitInt(0) <= n#0, 
    b#0: Map
       where $Is(b#0, TMap(TInt, TInt)) && $IsAlloc(b#0, TMap(TInt, TInt), $Heap));
  modifies $Heap;
  // user-defined postconditions
  free ensures MapTests.__default.F#canCall(b#0, n#0);
  ensures {:id "id32"} MapTests.__default.F($LS($LS($LZ)), b#0, n#0) == LitInt(0);
  // frame condition
  free ensures old($Heap) == $Heap;



procedure {:verboseName "MapTests.M (correctness)"} Impl$$MapTests.__default.M(n#0: int where LitInt(0) <= n#0, 
    b#0: Map
       where $Is(b#0, TMap(TInt, TInt)) && $IsAlloc(b#0, TMap(TInt, TInt), $Heap))
  ...
  // user-defined postconditions
  free ensures MapTests.__default.F#canCall(b#0, n#0);
  ensures {:id "id33"} MapTests.__default.F($LS($LS($LZ)), b#0, n#0) == LitInt(0);
  // frame condition
  free ensures old($Heap) == $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "MapTests.M (correctness)"} Impl$$MapTests.__default.M(n#0: int, b#0: Map) returns ($_reverifyPost: bool)
{
  ...
  var $initHeapForallStmt#0: Heap;
    ...
    assume (forall $ih#n0#0: int, $ih#b0#0: Map :: 
      { MapTests.__default.F($LS($LZ), $ih#b0#0, $ih#n0#0) } 
      LitInt(0) <= $ih#n0#0
