...
(declare-fun |y#0| () T@U)
(declare-fun TwoStateAt.Cell.IdentityLemma$Y () T@U)
(declare-fun $FunctionContextHeight () Int)
(set-info :boogie-vc-id Impl$$TwoStateAt.Cell.IdentityLemma)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#3| null current$Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (= (U_2_int ($Unbox intType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) previous$Heap |c#0@@1|) TwoStateAt.Cell.data))) (U_2_int ($Unbox intType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) current$Heap |c#0@@1|) TwoStateAt.Cell.data)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is refType |c#0@@1| Tclass.TwoStateAt.Cell) ($IsAlloc refType |c#0@@1| Tclass.TwoStateAt.Cell previous$Heap))) (and (and ($IsBox |y#0| TwoStateAt.Cell.IdentityLemma$Y) ($IsAllocBox |y#0| TwoStateAt.Cell.IdentityLemma$Y previous$Heap)) (= 1 $FunctionContextHeight))) (and (and (and (= previous$Heap $Heap) (and ($HeapSucc previous$Heap current$Heap) ($IsGoodHeap current$Heap))) ($IsAlloc refType |c#0@@1| Tclass.TwoStateAt.Cell previous$Heap)) (and ($IsAllocBox |y#0| TwoStateAt.Cell.IdentityLemma$Y previous$Heap) (= (ControlFlow 0 3) 2)))) anon0_correct)))
