...
(set-info :boogie-vc-id Impl$$_module.__default.FwdBug)
...
(let ((anon7_Then_correct  (=> (= |f#0| |#_module.Fwd.FwdNil|) (and (=> (= (ControlFlow 0 7) 5) anon9_Then_correct) (=> (= (ControlFlow 0 7) 6) anon9_Else_correct)))))
(let ((anon8_Else_correct true))
(let ((anon8_Then_correct  (=> (and (= |f#0| (|#_module.Fwd.FwdCons| |a#0#0#0@0| |a#0#1#0@0|)) (= (ControlFlow 0 2) (- 0 1))) false)))
(let ((anon7_Else_correct  (=> (or (not (= |f#0| |#_module.Fwd.FwdNil|)) (not true)) (and (=> (= (ControlFlow 0 4) 2) anon8_Then_correct) (=> (= (ControlFlow 0 4) 3) anon8_Else_correct)))))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (and (=> (= (ControlFlow 0 8) 7) anon7_Then_correct) (=> (= (ControlFlow 0 8) 4) anon7_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and (and ($Is DatatypeTypeType |f#0| Tclass._module.Fwd) ($IsAlloc DatatypeTypeType |f#0| Tclass._module.Fwd $Heap)) (|$IsA#_module.Fwd| |f#0|)) (= 1 $FunctionContextHeight)) (and (not (_module.Fwd.FwdCons_q |f#0|)) (= (ControlFlow 0 9) 8))) anon0_correct))))
