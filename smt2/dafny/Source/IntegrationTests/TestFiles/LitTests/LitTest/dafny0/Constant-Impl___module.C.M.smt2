...
(assert  (=> (< 0 $FunctionContextHeight) (forall (($h@@1 T@U) ) (!  (=> ($IsGoodHeap $h@@1) ($IsAlloc intType (int_2_U _module.C.y) TInt $h@@1))
 :qid |DafnyPreludebpl.593:12|
 :skolemid |2008|
 :pattern ( ($IsAlloc intType (int_2_U _module.C.y) TInt $h@@1))
))))
(assert  (=> (< 1 $FunctionContextHeight) (forall (($h@@2 T@U) ) (!  (=> ($IsGoodHeap $h@@2) ($IsAlloc intType (int_2_U _module.C.x) TInt $h@@2))
 :qid |DafnyPreludebpl.593:12|
 :skolemid |2007|
 :pattern ( ($IsAlloc intType (int_2_U _module.C.x) TInt $h@@2))
))))
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (= |r#0@0| (+ _module.C.x _module.C.y)) (= (ControlFlow 0 2) (- 0 1))) (= |r#0@0| (LitInt 11))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (or (not (= this null)) (not true)) (and ($Is refType this Tclass._module.C) ($IsAlloc refType this Tclass._module.C $Heap))) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
