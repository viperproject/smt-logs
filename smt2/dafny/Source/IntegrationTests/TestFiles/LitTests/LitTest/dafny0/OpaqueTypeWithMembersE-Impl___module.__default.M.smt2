...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 6) (let ((anon2_correct  (=> (= (ControlFlow 0 2) (- 0 1)) (U_2_bool (Lit boolType (bool_2_U true))))))
(let ((anon3_Else_correct  (=> (and (not (and ($IsBox |r#1@0| Tclass._module.FailureCompatible) ($IsAllocBox |r#1@0| Tclass._module.FailureCompatible $Heap))) (= (ControlFlow 0 4) 2)) anon2_correct)))
(let ((anon3_Then_correct  (=> (and (and ($IsBox |r#1@0| Tclass._module.FailureCompatible) ($IsAllocBox |r#1@0| Tclass._module.FailureCompatible $Heap)) (= (ControlFlow 0 3) 2)) anon2_correct)))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($IsBox |r#0| Tclass._module.FailureCompatible) ($IsAllocBox |r#0| Tclass._module.FailureCompatible $Heap)) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct))))
