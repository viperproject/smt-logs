...
(set-info :boogie-vc-id Impl$$_module.__default.MultisetCardinalityC_k)
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is MultiSetType |s#0| (TMultiSet TInt)) ($IsAlloc MultiSetType |s#0| (TMultiSet TInt) $Heap)) (= 0 $FunctionContextHeight)) (and (forall ((|x#1| T@U) ) (!  (not (> (|MultiSet#Multiplicity| |s#0| ($Box intType |x#1|)) 0))
 :qid |MultiSetsdfy.238:19|
 :skolemid |579|
 :pattern ( (|MultiSet#Multiplicity| |s#0| ($Box intType |x#1|)))
