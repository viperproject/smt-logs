...
(assert (distinct TInt TagInt alloc allocName Tagclass.XJ.Cell Tagclass.XJ.Cell? class.XJ.Cell? tytagFamily$Cell field$data)
)
(assert (= (FDim alloc) 0))
...
(set-info :boogie-vc-id CheckWellFormed$$XJ.__default.D)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (or (not (= |seven#0| null)) (not true)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is refType |seven#0| Tclass.XJ.Cell) ($IsAlloc refType |seven#0| Tclass.XJ.Cell $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
