...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 8) (let ((anon3_Else_correct  (=> (<= 10 (_module.FailureCompatible.c |t#0|)) (and (=> (= (ControlFlow 0 5) (- 0 6)) (not |b#0@0|)) (=> (not |b#0@0|) (=> (and ($IsAllocBox |t#0| Tclass._module.FailureCompatible $Heap) (= (ControlFlow 0 5) (- 0 4))) (not (_module.FailureCompatible.IsFailure |t#0|))))))))
(let ((anon3_Then_correct  (=> (< (_module.FailureCompatible.c |t#0|) 10) (and (=> (= (ControlFlow 0 2) (- 0 3)) |b#0@0|) (=> |b#0@0| (=> (and (and ($IsAllocBox |t#0| Tclass._module.FailureCompatible $Heap) (= (ControlFlow 0 2) (- 0 1))) (|_module.FailureCompatible.IsFailure#canCall| |t#0|)) (or (_module.FailureCompatible.IsFailure |t#0|) (< (_module.FailureCompatible.c |t#0|) 10))))))))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (and ($IsAllocBox |t#0| Tclass._module.FailureCompatible $Heap) (|_module.FailureCompatible.IsFailure#canCall| |t#0|)) (and (|_module.FailureCompatible.IsFailure#canCall| |t#0|) (= |b#0@0| (_module.FailureCompatible.IsFailure |t#0|)))) (and (=> (= (ControlFlow 0 7) 2) anon3_Then_correct) (=> (= (ControlFlow 0 7) 5) anon3_Else_correct))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($IsBox |t#0| Tclass._module.FailureCompatible) ($IsAllocBox |t#0| Tclass._module.FailureCompatible $Heap)) (and (= 3 $FunctionContextHeight) (= (ControlFlow 0 8) 7))) anon0_correct))))
