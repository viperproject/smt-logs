...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.SeqRank1)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (not (|Seq#Equal| |s#0| |Seq#Empty|))) (=> (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (and (= $Heap $Heap@0) (= (ControlFlow 0 2) (- 0 1)))) (and (<= 0 (LitInt 0)) (< (LitInt 0) (|Seq#Length| |s#0|)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is SeqType |s#0| (TSeq Tclass._module.Abc)) ($IsAlloc SeqType |s#0| (TSeq Tclass._module.Abc) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
