...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (and (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= |let#0#0#0| (LitInt 54))) (and ($Is intType (int_2_U |let#0#0#0|) TInt) (= |f#Z#0@0| |let#0#0#0|))) (and (and (= |let#1#0#0| (+ |f#Z#0@0| 1)) ($Is intType (int_2_U |let#1#0#0|) TInt)) (and (= |g#Z#0@0| |let#1#0#0|) (= (ControlFlow 0 2) (- 0 1))))) (let ((|f#0| (LitInt 54)))
(let ((|g#0| (LitInt (+ |f#0| 1))))
(= |g#0| (LitInt 55)))))))
