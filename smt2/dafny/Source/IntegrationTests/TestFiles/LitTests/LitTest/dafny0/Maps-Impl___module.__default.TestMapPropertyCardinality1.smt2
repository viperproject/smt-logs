...
(set-info :boogie-vc-id Impl$$_module.__default.TestMapPropertyCardinality1)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 4) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (and (=> (= (ControlFlow 0 2) (- 0 3)) (<= (|Set#Card| (|Map#Values| |m#0|)) (|Set#Card| (|Map#Domain| |m#0|)))) (=> (<= (|Set#Card| (|Map#Values| |m#0|)) (|Set#Card| (|Map#Domain| |m#0|))) (=> (= (ControlFlow 0 2) (- 0 1)) (= (|Set#Card| (|Map#Domain| |m#0|)) (|Set#Card| (|Map#Values| |m#0|)))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is MapType |m#0| (TMap Tclass._module.MyClass (TBitvector 3))) ($IsAlloc MapType |m#0| (TMap Tclass._module.MyClass (TBitvector 3)) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 4) 2))) anon0_correct))))
