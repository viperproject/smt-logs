...
procedure {:verboseName "MatchAcrossMultipleLines.M (well-formedness)"} CheckWellFormed$$MatchAcrossMultipleLines.__default.M(s#0: Set
       where $Is(s#0, TSet(Tclass.MatchAcrossMultipleLines.PQ()))
         && $IsAlloc(s#0, TSet(Tclass.MatchAcrossMultipleLines.PQ()), $Heap));
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "MatchAcrossMultipleLines.M (call)"} Call$$MatchAcrossMultipleLines.__default.M(s#0: Set
       where $Is(s#0, TSet(Tclass.MatchAcrossMultipleLines.PQ()))
         && $IsAlloc(s#0, TSet(Tclass.MatchAcrossMultipleLines.PQ()), $Heap));
  // user-defined preconditions
  requires {:id "id28"} (forall pq#1: DatatypeType :: 
    { Set#IsMember(s#0, $Box(pq#1)) } 
    $Is(pq#1, Tclass.MatchAcrossMultipleLines.PQ()) && Set#IsMember(s#0, $Box(pq#1))
       ==> (if MatchAcrossMultipleLines.PQ.P_q(pq#1)
         then (var x#1 := MatchAcrossMultipleLines.PQ._h0(pq#1); Lit(true))
         else (var y#1 := MatchAcrossMultipleLines.PQ._h1(pq#1); y#1 == Lit(false))));
  modifies $Heap;
...
procedure {:verboseName "MatchAcrossMultipleLines.M (correctness)"} Impl$$MatchAcrossMultipleLines.__default.M(s#0: Set
       where $Is(s#0, TSet(Tclass.MatchAcrossMultipleLines.PQ()))
         && $IsAlloc(s#0, TSet(Tclass.MatchAcrossMultipleLines.PQ()), $Heap))
   returns ($_reverifyPost: bool);
  free requires 1 == $FunctionContextHeight;
  // user-defined preconditions
  requires {:id "id29"} (forall pq#1: DatatypeType :: 
    { Set#IsMember(s#0, $Box(pq#1)) } 
    $Is(pq#1, Tclass.MatchAcrossMultipleLines.PQ()) && Set#IsMember(s#0, $Box(pq#1))
       ==> (if MatchAcrossMultipleLines.PQ.P_q(pq#1)
         then (var x#2 := MatchAcrossMultipleLines.PQ._h0(pq#1); Lit(true))
         else (var y#2 := MatchAcrossMultipleLines.PQ._h1(pq#1); y#2 == Lit(false))));
  modifies $Heap;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
    { $Heap[$o] } 
...
axiom {:id "id30"} 1 <= $FunctionContextHeight
   ==> (forall A#0: bool, B#0: int, C#0: DatatypeType :: 
...
procedure {:verboseName "MatchAcrossMultipleLines.F (well-formedness)"} CheckWellformed$$MatchAcrossMultipleLines.__default.F(A#0: bool, 
    B#0: int, 
    C#0: DatatypeType where $Is(C#0, Tclass.MatchAcrossMultipleLines.YZ()));
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "MatchAcrossMultipleLines.F (well-formedness)"} CheckWellformed$$MatchAcrossMultipleLines.__default.F(A#0: bool, B#0: int, C#0: DatatypeType)
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    assume {:id "id32"} !MatchAcrossMultipleLines.YZ#Equal(C#0, #MatchAcrossMultipleLines.YZ.Y());
    ...
    else
    {
        ...
        if (A#0)
        {
            assume {:id "id33"} MatchAcrossMultipleLines.__default.F(A#0, B#0, C#0) == B#0;
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(MatchAcrossMultipleLines.__default.F(A#0, B#0, C#0), TInt);
            return;
        }
        else
        {
            if (C#0 == #MatchAcrossMultipleLines.YZ.Z())
            {
                assume {:id "id35"} MatchAcrossMultipleLines.__default.F(A#0, B#0, C#0) == LitInt(3);
                assume true;
                // CheckWellformedWithResult: any expression
                assume $Is(MatchAcrossMultipleLines.__default.F(A#0, B#0, C#0), TInt);
                return;
            }
            else if (C#0 == #MatchAcrossMultipleLines.YZ.Y())
            {
