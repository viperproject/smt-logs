...
const unique class.TickRegressions.__default: ClassName;

procedure {:verboseName "TickRegressions.X (well-formedness)"} CheckWellFormed$$TickRegressions.__default.X();
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "TickRegressions.X (call)"} Call$$TickRegressions.__default.X();
  modifies $Heap;
  // frame condition
...
function TickRegressions.__default.u() : int
uses {
axiom TickRegressions.__default.u(): int == LitInt(-3);
// _default.u: Type axiom
axiom 1 < $FunctionContextHeight
   ==> $Is(TickRegressions.__default.u(), Tclass._System.nat());
}

procedure {:verboseName "TickRegressions.u (well-formedness)"} CheckWellFormed$$TickRegressions.__default.u();
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "TickRegressions.u (well-formedness)"} CheckWellFormed$$TickRegressions.__default.u()
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    // ----- call statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/DefaultParameters.dfy(493,21)
    // TrCallStmt: Before ProcessCallStmt
    call {:id "id740"} Call$$TickRegressions.__default.X();
    // TrCallStmt: After ProcessCallStmt
    assume true;
    assert {:id "id741"} $Is(LitInt(-3), Tclass._System.nat());
}



// _default.u: Allocation axiom
axiom 1 < $FunctionContextHeight
   ==> (forall $h: Heap :: 
    { $IsAlloc(TickRegressions.__default.u(), Tclass._System.nat(), $h) } 
    $IsGoodHeap($h)
       ==> $IsAlloc(TickRegressions.__default.u(), Tclass._System.nat(), $h));

// Constructor function declaration
function #TickRegressions.S.S(int) : DatatypeType;

const unique ##TickRegressions.S.S: DtCtorId
uses {
// Constructor identifier
axiom (forall a#0#0#0: int :: 
  { #TickRegressions.S.S(a#0#0#0) } 
  DatatypeCtorId(#TickRegressions.S.S(a#0#0#0)) == ##TickRegressions.S.S);
}

function TickRegressions.S.S_q(DatatypeType) : bool;

// Questionmark and identifier
axiom (forall d: DatatypeType :: 
  { TickRegressions.S.S_q(d) } 
  TickRegressions.S.S_q(d) <==> DatatypeCtorId(d) == ##TickRegressions.S.S);

procedure {:verboseName "#TickRegressions.S.S (well-formedness)"} CheckWellformed$$#TickRegressions.S.S(x#0: int where LitInt(0) <= x#0);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "#TickRegressions.S.S (well-formedness)"} CheckWellformed$$#TickRegressions.S.S(x#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    $_ReadsFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    // ----- call statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/DefaultParameters.dfy(494,29)
    // TrCallStmt: Before ProcessCallStmt
    call {:id "id742"} Call$$TickRegressions.__default.X();
    // TrCallStmt: After ProcessCallStmt
    assume true;
    assert {:id "id743"} $Is(LitInt(-3), Tclass._System.nat());
}
...
axiom (forall d: DatatypeType :: 
  { TickRegressions.S.S_q(d) } 
  TickRegressions.S.S_q(d)
     ==> (exists a#0#0#0: int :: d == #TickRegressions.S.S(a#0#0#0)));

function Tclass.TickRegressions.S() : Ty
uses {
// Tclass.TickRegressions.S Tag
axiom Tag(Tclass.TickRegressions.S()) == Tagclass.TickRegressions.S
   && TagFamily(Tclass.TickRegressions.S()) == tytagFamily$S;
}

const unique Tagclass.TickRegressions.S: TyTag;

// Box/unbox axiom for Tclass.TickRegressions.S
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.TickRegressions.S()) } 
  $IsBox(bx, Tclass.TickRegressions.S())
     ==> $Box($Unbox(bx): DatatypeType) == bx
       && $Is($Unbox(bx): DatatypeType, Tclass.TickRegressions.S()));

// Constructor $Is
axiom (forall a#1#0#0: int :: 
  { $Is(#TickRegressions.S.S(a#1#0#0), Tclass.TickRegressions.S()) } 
  $Is(#TickRegressions.S.S(a#1#0#0), Tclass.TickRegressions.S())
     <==> $Is(a#1#0#0, Tclass._System.nat()));

// Destructor $IsAlloc
axiom (forall d: DatatypeType, $h: Heap :: 
  { $IsAlloc(TickRegressions.S.x(d), Tclass._System.nat(), $h) } 
  $IsGoodHeap($h)
       && 
      TickRegressions.S.S_q(d)
       && $IsAlloc(d, Tclass.TickRegressions.S(), $h)
     ==> $IsAlloc(TickRegressions.S.x(d), Tclass._System.nat(), $h));

// Constructor literal
axiom (forall a#2#0#0: int :: 
  { #TickRegressions.S.S(LitInt(a#2#0#0)) } 
  #TickRegressions.S.S(LitInt(a#2#0#0)) == Lit(#TickRegressions.S.S(a#2#0#0)));

function TickRegressions.S.x(DatatypeType) : int;

// Constructor injectivity
axiom (forall a#3#0#0: int :: 
  { #TickRegressions.S.S(a#3#0#0) } 
  TickRegressions.S.x(#TickRegressions.S.S(a#3#0#0)) == a#3#0#0);

// Datatype $IsAlloc
axiom (forall d: DatatypeType, $h: Heap :: 
  { $IsAlloc(d, Tclass.TickRegressions.S(), $h) } 
  $IsGoodHeap($h) && $Is(d, Tclass.TickRegressions.S())
     ==> $IsAlloc(d, Tclass.TickRegressions.S(), $h));

// Depth-one case-split function
function $IsA#TickRegressions.S(DatatypeType) : bool;

// Depth-one case-split axiom
axiom (forall d: DatatypeType :: 
  { $IsA#TickRegressions.S(d) } 
  $IsA#TickRegressions.S(d) ==> TickRegressions.S.S_q(d));

// Questionmark data type disjunctivity
axiom (forall d: DatatypeType :: 
  { TickRegressions.S.S_q(d), $Is(d, Tclass.TickRegressions.S()) } 
  $Is(d, Tclass.TickRegressions.S()) ==> TickRegressions.S.S_q(d));

// Datatype extensional equality declaration
function TickRegressions.S#Equal(DatatypeType, DatatypeType) : bool;

// Datatype extensional equality definition: #TickRegressions.S.S
axiom (forall a: DatatypeType, b: DatatypeType :: 
  { TickRegressions.S#Equal(a, b) } 
  TickRegressions.S#Equal(a, b)
     <==> TickRegressions.S.x(a) == TickRegressions.S.x(b));

// Datatype extensionality axiom: TickRegressions.S
axiom (forall a: DatatypeType, b: DatatypeType :: 
  { TickRegressions.S#Equal(a, b) } 
  TickRegressions.S#Equal(a, b) <==> a == b);

const unique class.TickRegressions.S: ClassName;
