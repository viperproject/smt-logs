...
(assert (distinct alloc Tagclass.TwoStateAt.Cell Tagclass.TwoStateAt.Cell? tytagFamily$Cell)
)
...
(set-info :boogie-vc-id Impl$$TwoStateAt.__default.LemmaMoreParams)
...
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#3| null current$Heap alloc false)) (and (=> (= (ControlFlow 0 5) 3) anon4_Then_correct) (=> (= (ControlFlow 0 5) 4) anon4_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is refType |c#0@@1| Tclass.TwoStateAt.Cell) ($IsAlloc refType |c#0@@1| Tclass.TwoStateAt.Cell previous$Heap))) (=> (and (and (and ($Is refType |d#0| Tclass.TwoStateAt.Cell) ($IsAlloc refType |d#0| Tclass.TwoStateAt.Cell current$Heap)) (and (=> |defass#e#0| (and ($Is refType |e#0| Tclass.TwoStateAt.Cell) ($IsAlloc refType |e#0| Tclass.TwoStateAt.Cell current$Heap))) (= 1 $FunctionContextHeight))) (and (and (= previous$Heap $Heap) (and ($HeapSucc previous$Heap current$Heap) ($IsGoodHeap current$Heap))) (and ($IsAlloc refType |c#0@@1| Tclass.TwoStateAt.Cell previous$Heap) (= (ControlFlow 0 6) 5)))) anon0_correct))))
