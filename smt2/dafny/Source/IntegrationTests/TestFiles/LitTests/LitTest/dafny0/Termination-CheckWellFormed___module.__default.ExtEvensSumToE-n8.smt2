...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.ExtEvensSumToEven)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 7) (let ((anon4_correct  (=> (forall ((|u#1| T@U) ) (!  (=> (|Set#IsMember| (_module.Tree.Elements ($LS $LZ) |t#0|) ($Box intType |u#1|)) (= (Mod (U_2_int |u#1|) (LitInt 2)) (LitInt 0)))
 :qid |Terminationdfy.518:19|
 :skolemid |3070|
 :pattern ( (|Set#IsMember| (_module.Tree.Elements ($LS $LZ) |t#0|) ($Box intType |u#1|)))
)) (=> (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (=> (and (and (= $Heap $Heap@0) ($IsAllocBox ($Box DatatypeTypeType |t#0|) Tclass._module.Tree $Heap@0)) (and (|_module.Tree.Sum#canCall| |t#0|) (= (ControlFlow 0 2) (- 0 1)))) (or (not (= (LitInt 2) 0)) (not true)))))))
(let ((anon5_Else_correct  (=> (and (=> (|Set#IsMember| (_module.Tree.Elements ($LS $LZ) |t#0|) ($Box intType (int_2_U |u#0@0|))) (= (Mod |u#0@0| (LitInt 2)) (LitInt 0))) (= (ControlFlow 0 5) 2)) anon4_correct)))
(let ((anon5_Then_correct  (=> ($IsAllocBox ($Box DatatypeTypeType |t#0|) Tclass._module.Tree $Heap) (=> (and (|_module.Tree.Elements#canCall| |t#0|) (|Set#IsMember| (_module.Tree.Elements ($LS $LZ) |t#0|) ($Box intType (int_2_U |u#0@0|)))) (and (=> (= (ControlFlow 0 3) (- 0 4)) (or (not (= (LitInt 2) 0)) (not true))) (=> (or (not (= (LitInt 2) 0)) (not true)) (=> (and (= (Mod |u#0@0| (LitInt 2)) (LitInt 0)) (= (ControlFlow 0 3) 2)) anon4_correct)))))))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (and (=> (= (ControlFlow 0 6) 3) anon5_Then_correct) (=> (= (ControlFlow 0 6) 5) anon5_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is DatatypeTypeType |t#0| Tclass._module.Tree) ($IsAlloc DatatypeTypeType |t#0| Tclass._module.Tree $Heap)) (|$IsA#_module.Tree| |t#0|)) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 7) 6))) anon0_correct))))
