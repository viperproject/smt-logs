...
(assert (distinct TInt TagInt alloc Tagclass._System.array? Tagclass._System.array Tagclass._module.MyClass Tagclass._module.MyClass? tytagFamily$array tytagFamily$MyClass)
)
...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.AllocationBusiness0)
...
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (and (=> (= (ControlFlow 0 5) 2) anon4_Then_correct) (=> (= (ControlFlow 0 5) 4) anon4_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is refType |a#0| (Tclass._System.array Tclass._module.MyClass)) ($IsAlloc refType |a#0| (Tclass._System.array Tclass._module.MyClass) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct))))
