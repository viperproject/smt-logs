...
(declare-fun _module.__default.F () T@U)
...
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |_module.__default.F#canCall| (< 1 $FunctionContextHeight)) ($Is refType _module.__default.F Tclass._module.C))))
...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($Heap T@U) ) (!  (=> (and (or |_module.__default.F#canCall| (< 1 $FunctionContextHeight)) ($IsGoodHeap $Heap)) ($IsAlloc refType _module.__default.F Tclass._module.C $Heap))
 :qid |DefiniteAssignmentdfy.72:10|
 :skolemid |3546|
 :pattern ( ($IsAlloc refType _module.__default.F Tclass._module.C $Heap))
))))
...
(set-info :boogie-vc-id CheckWellFormed$$_module.NonNullC)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon6_Then_correct  (=> |_module.__default.F#canCall| (=> (and (and (= |newtype$check#1@0| _module.__default.F) (= |newtype$check#2@0| _module.__default.F)) (and |_module.__default.F#canCall| (= (ControlFlow 0 3) (- 0 2)))) (or (not (= _module.__default.F null)) (not true))))))
