...
(set-info :boogie-vc-id Impl$$_module.__default.TestIMapPropertyMembership3)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 6) (let ((anon2_correct  (=> (= (ControlFlow 0 2) (- 0 1)) (exists ((|v#1| T@U) (|k#1| T@U) ) (!  (and (and ($Is refType |k#1| Tclass._module.MyClass) ($IsAlloc refType |k#1| Tclass._module.MyClass $Heap)) (U_2_bool (MapType0Select BoxType boolType (|IMap#Items| |m#0|) ($Box DatatypeTypeType (|#_System._tuple#2._#Make2| ($Box refType |k#1|) ($Box bv3Type |v#1|))))))
 :qid |Mapsdfy.637:20|
 :skolemid |898|
 :pattern ( (|#_System._tuple#2._#Make2| ($Box refType |k#1|) ($Box bv3Type |v#1|)))
)))))
