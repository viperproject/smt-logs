...
axiom {:id "id238"} 0 <= $FunctionContextHeight
   ==> (forall x#0: int :: 
    ...
    OpaqueTest.__default.bar#canCall(x#0) || (0 < $FunctionContextHeight && x#0 > 7)
...
procedure {:verboseName "OpaqueTest.bar (well-formedness)"} CheckWellformed$$OpaqueTest.__default.bar(x#0: int);
...
axiom {:id "id242"} 1 <= $FunctionContextHeight
   ==> (forall x#0: int :: 
    ...
    OpaqueTest.__default.foo#canCall(x#0) || (1 < $FunctionContextHeight && x#0 > 7)
...
procedure {:verboseName "OpaqueTest.foo (well-formedness)"} CheckWellformed$$OpaqueTest.__default.foo(x#0: int);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "OpaqueTest.foo (well-formedness)"} CheckWellformed$$OpaqueTest.__default.foo(x#0: int)
{
  ...
  var ##x#0: int;
    ...
    else
    {
        ...
        assert {:id "id245"} {:subsumption 0} ##x#0 > 7;
        assume ##x#0 > 7;
        assume OpaqueTest.__default.bar#canCall(x#0);
        assume {:id "id246"} OpaqueTest.__default.foo(reveal_OpaqueTest._default.foo, x#0)
           == OpaqueTest.__default.bar(x#0);
        assume OpaqueTest.__default.bar#canCall(x#0);
        // CheckWellformedWithResult: any expression
        assume $Is(OpaqueTest.__default.foo(reveal_OpaqueTest._default.foo, x#0), TInt);
        return;
...
procedure {:verboseName "OpaqueTest.reveal_foo (well-formedness)"} {:verify false} CheckWellFormed$$OpaqueTest.__default.reveal__foo();
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "OpaqueTest.reveal_foo (call)"} {:verify false} Call$$OpaqueTest.__default.reveal__foo();
  modifies $Heap;
  // frame condition
  free ensures old($Heap) == $Heap;
  free ensures reveal_OpaqueTest._default.foo;
