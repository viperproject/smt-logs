   ...
   ==> (forall $Heap: Heap, source#0: ref, sink#0: ref, S#0: Set :: 
    ...
    Reachable5.__default.Reachable#canCall($Heap, source#0, sink#0, S#0)
       ...
       ==> (forall via#0: DatatypeType :: 
          { Reachable5.__default.ReachableVia($LS($LZ), $Heap, source#0, via#0, sink#0, S#0, S#0) } 
          $Is(via#0, Tclass.Reachable5.Path(Tclass.Reachable5.Node()))
             ==> Reachable5.__default.ReachableVia#canCall($Heap, source#0, via#0, sink#0, S#0, S#0))
...
procedure {:verboseName "Reachable5.Reachable (well-formedness)"} CheckWellformed$$Reachable5.__default.Reachable(source#0: ref where $Is(source#0, Tclass.Reachable5.Node()), 
    sink#0: ref where $Is(sink#0, Tclass.Reachable5.Node()), 
    S#0: Set where $Is(S#0, TSet(Tclass.Reachable5.Node())));
  free requires 2 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Reachable5.Reachable (well-formedness)"} CheckWellformed$$Reachable5.__default.Reachable(source#0: ref, sink#0: ref, S#0: Set)
{
  ...
  var b$reqreads#0: bool;
    ...
    else
    {
        ...
        havoc via#1;
        if ($Is(via#1, Tclass.Reachable5.Path(Tclass.Reachable5.Node()))
           && $IsAlloc(via#1, Tclass.Reachable5.Path(Tclass.Reachable5.Node()), $Heap))
        {
            ##source#0 := source#0;
            // assume allocatedness for argument to function
            assume $IsAlloc(##source#0, Tclass.Reachable5.Node(), $Heap);
            ##p#0 := via#1;
            // assume allocatedness for argument to function
            assume $IsAlloc(##p#0, Tclass.Reachable5.Path(Tclass.Reachable5.Node()), $Heap);
            ##sink#0 := sink#0;
            // assume allocatedness for argument to function
            assume $IsAlloc(##sink#0, Tclass.Reachable5.Node(), $Heap);
            ##S#0 := S#0;
            // assume allocatedness for argument to function
            assume $IsAlloc(##S#0, TSet(Tclass.Reachable5.Node()), $Heap);
            ##T#0 := S#0;
            // assume allocatedness for argument to function
            assume $IsAlloc(##T#0, TSet(Tclass.Reachable5.Node()), $Heap);
            b$reqreads#0 := (forall $o: ref, $f: Field :: 
              $o != null
                   && $Unbox(read($Heap, $o, alloc)): bool
                   && (Set#IsMember(##S#0, $Box($o)) || Set#IsMember(##T#0, $Box($o)))
                 ==> $_ReadsFrame[$o, $f]);
            assume Reachable5.__default.ReachableVia#canCall($Heap, source#0, via#1, sink#0, S#0, S#0);
        }

        // End Comprehension WF check
        assume {:id "id104"} Reachable5.__default.Reachable($Heap, source#0, sink#0, S#0)
           == (exists via#2: DatatypeType :: 
            { Reachable5.__default.ReachableVia($LS($LZ), $Heap, source#0, via#2, sink#0, S#0, S#0) } 
            $Is(via#2, Tclass.Reachable5.Path(Tclass.Reachable5.Node()))
               && Reachable5.__default.ReachableVia($LS($LZ), $Heap, source#0, via#2, sink#0, S#0, S#0));
        assume (forall via#2: DatatypeType :: 
          { Reachable5.__default.ReachableVia($LS($LZ), $Heap, source#0, via#2, sink#0, S#0, S#0) } 
          $Is(via#2, Tclass.Reachable5.Path(Tclass.Reachable5.Node()))
             ==> Reachable5.__default.ReachableVia#canCall($Heap, source#0, via#2, sink#0, S#0, S#0));
        // CheckWellformedWithResult: any expression
        assume $Is(Reachable5.__default.Reachable($Heap, source#0, sink#0, S#0), TBool);
   ...
   ==> (forall $ly: LayerType, 
    ...
    Reachable5.__default.ReachableVia#canCall($Heap, source#0, p#0, sink#0, S#0, T#0)
       ...
       ==> (!Reachable5.Path.Empty_q(p#0)
           ==> (var n#1 := $Unbox(Reachable5.Path._h14(p#0)): ref; 
            (var prefix#1 := Reachable5.Path._h13(p#0); 
              Set#IsMember(S#0, $Box(n#1))
                 ==> 
                Seq#Contains($Unbox(read($Heap, n#1, Reachable5.Node.children)): Seq, $Box(sink#0))
                 ==> Reachable5.__default.ReachableVia#canCall($Heap, source#0, prefix#1, n#1, S#0, T#0))))
         && Reachable5.__default.ReachableVia($LS($ly), $Heap, source#0, p#0, sink#0, S#0, T#0)
           == (if Reachable5.Path.Empty_q(p#0)
             ...
             else (var n#0 := $Unbox(Reachable5.Path._h14(p#0)): ref; 
              (var prefix#0 := Reachable5.Path._h13(p#0); 
                Set#IsMember(S#0, $Box(n#0))
                   && Seq#Contains($Unbox(read($Heap, n#0, Reachable5.Node.children)): Seq, $Box(sink#0))
                   && Reachable5.__default.ReachableVia($ly, $Heap, source#0, prefix#0, n#0, S#0, T#0)))));
}
...
procedure {:verboseName "Reachable5.ReachableVia (well-formedness)"} CheckWellformed$$Reachable5.__default.ReachableVia(source#0: ref where $Is(source#0, Tclass.Reachable5.Node()), 
    p#0: DatatypeType
       where $Is(p#0, Tclass.Reachable5.Path(Tclass.Reachable5.Node())), 
    sink#0: ref where $Is(sink#0, Tclass.Reachable5.Node()), 
    S#0: Set where $Is(S#0, TSet(Tclass.Reachable5.Node())), 
    T#0: Set where $Is(T#0, TSet(Tclass.Reachable5.Node())));
  ...
  ensures (forall $olderHeap: Heap :: 
    ...
    $IsGoodHeap($olderHeap)
         && $OlderTag($olderHeap)
         && 
        Reachable5.__default.ReachableVia($LS($LZ), $Heap, source#0, p#0, sink#0, S#0, T#0)
         && 
        $IsAlloc(source#0, Tclass.Reachable5.Node(), $olderHeap)
         && $IsAlloc(sink#0, Tclass.Reachable5.Node(), $olderHeap)
         && $IsAlloc(S#0, TSet(Tclass.Reachable5.Node()), $olderHeap)
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Reachable5.ReachableVia (well-formedness)"} CheckWellformed$$Reachable5.__default.ReachableVia(source#0: ref, p#0: DatatypeType, sink#0: ref, S#0: Set, T#0: Set)
{
  var $_ReadsFrame: [ref,Field]bool;
  ...
  var b$reqreads#1: bool;
    ...
    {
        // Check well-formedness of body and result subset type constraint
        if (p#0 == #Reachable5.Path.Empty())
        {
            assume {:id "id116"} Reachable5.__default.ReachableVia($LS($LZ), $Heap, source#0, p#0, sink#0, S#0, T#0)
               == 
              (source#0
               == sink#0);
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(Reachable5.__default.ReachableVia($LS($LZ), $Heap, source#0, p#0, sink#0, S#0, T#0), 
              TBool);
            assert {:id "id117"} b$reqreads#0;
            assert {:id "id118"} b$reqreads#1;
            return;
        }
        else if (p#0 == #Reachable5.Path.Extend(_mcc#0#0, $Box(_mcc#1#0)))
        {
            assume $Is(_mcc#0#0, Tclass.Reachable5.Path(Tclass.Reachable5.Node()));
            assume $Is(_mcc#1#0, Tclass.Reachable5.Node());
            havoc n#Z#0;
            assume {:id "id107"} let#0#0#0 == _mcc#1#0;
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(let#0#0#0, Tclass.Reachable5.Node());
            assume {:id "id108"} n#Z#0 == let#0#0#0;
            havoc prefix#Z#0;
            assume {:id "id109"} let#1#0#0 == _mcc#0#0;
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(let#1#0#0, Tclass.Reachable5.Path(Tclass.Reachable5.Node()));
            assume {:id "id110"} prefix#Z#0 == let#1#0#0;
            if (Set#IsMember(S#0, $Box(n#Z#0)))
            {
                assert {:id "id111"} n#Z#0 != null;
                b$reqreads#0 := $_ReadsFrame[n#Z#0, Reachable5.Node.children];
            }

            if (Set#IsMember(S#0, $Box(n#Z#0))
               && Seq#Contains($Unbox(read($Heap, n#Z#0, Reachable5.Node.children)): Seq, $Box(sink#0)))
            {
                ##source#0 := source#0;
                // assume allocatedness for argument to function
                assume $IsAlloc(##source#0, Tclass.Reachable5.Node(), $Heap);
                ...
                b$reqreads#1 := (forall $o: ref, $f: Field :: 
                  $o != null
                       ...
                       && (Set#IsMember(##S#0, $Box($o)) || Set#IsMember(##T#0, $Box($o)))
                     ==> $_ReadsFrame[$o, $f]);
                assert {:id "id112"} DtRank(##p#0) < DtRank(p#0);
                assume Reachable5.__default.ReachableVia#canCall($Heap, source#0, prefix#Z#0, n#Z#0, S#0, T#0);
            }

            assume {:id "id113"} Reachable5.__default.ReachableVia($LS($LZ), $Heap, source#0, p#0, sink#0, S#0, T#0)
               == (
                Set#IsMember(S#0, $Box(n#Z#0))
                 && Seq#Contains($Unbox(read($Heap, n#Z#0, Reachable5.Node.children)): Seq, $Box(sink#0))
                 && Reachable5.__default.ReachableVia($LS($LZ), $Heap, source#0, prefix#Z#0, n#Z#0, S#0, T#0));
            assume Set#IsMember(S#0, $Box(n#Z#0))
               ==> 
              Seq#Contains($Unbox(read($Heap, n#Z#0, Reachable5.Node.children)): Seq, $Box(sink#0))
               ==> Reachable5.__default.ReachableVia#canCall($Heap, source#0, prefix#Z#0, n#Z#0, S#0, T#0);
            // CheckWellformedWithResult: any expression
            assume $Is(Reachable5.__default.ReachableVia($LS($LZ), $Heap, source#0, p#0, sink#0, S#0, T#0), 
              TBool);
