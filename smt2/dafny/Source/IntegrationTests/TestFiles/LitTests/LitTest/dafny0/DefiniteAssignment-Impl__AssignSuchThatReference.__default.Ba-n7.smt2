...
(declare-fun |d#0| () T@U)
(set-info :boogie-vc-id Impl$$AssignSuchThatReference.__default.BadCompiled1)
...
(assert (not
 ...
 :qid |DefiniteAssignmentdfy.341:5|
 :skolemid |982|
))))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (=> |defass#d#0| (and ($Is refType |d#0| Tclass.AssignSuchThatReference.D) ($IsAlloc refType |d#0| Tclass.AssignSuchThatReference.D $Heap))) true) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 8) 7))) anon0_correct))))
