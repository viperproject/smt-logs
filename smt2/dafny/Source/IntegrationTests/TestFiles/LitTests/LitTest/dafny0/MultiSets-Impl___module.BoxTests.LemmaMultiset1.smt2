...
(assert (distinct TInt TagInt TagMultiSet alloc Tagclass._module.BoxTests? Tagclass._module.BoxTests tytagFamily$BoxTests)
)
...
(set-info :boogie-vc-id Impl$$_module.BoxTests.LemmaMultiset1)
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (or (not (= this null)) (not true)) (and ($Is refType this (Tclass._module.BoxTests _module.BoxTests$G@@9)) ($IsAlloc refType this (Tclass._module.BoxTests _module.BoxTests$G@@9) $Heap))) (and ($Is MultiSetType |a#0| (TMultiSet TInt)) ($IsAlloc MultiSetType |a#0| (TMultiSet TInt) $Heap))) (=> (and (and (and ($Is MultiSetType |b#0| (TMultiSet TInt)) ($IsAlloc MultiSetType |b#0| (TMultiSet TInt) $Heap)) (= 0 $FunctionContextHeight)) (and (forall ((|x#1| T@U) ) (!  (=> (> (|MultiSet#Multiplicity| |a#0| ($Box intType |x#1|)) 0) (> (|MultiSet#Multiplicity| |b#0| ($Box intType |x#1|)) 0))
 :qid |MultiSetsdfy.162:21|
 :skolemid |652|
 :pattern ( (|MultiSet#Multiplicity| |b#0| ($Box intType |x#1|)))
 :pattern ( (|MultiSet#Multiplicity| |a#0| ($Box intType |x#1|)))
