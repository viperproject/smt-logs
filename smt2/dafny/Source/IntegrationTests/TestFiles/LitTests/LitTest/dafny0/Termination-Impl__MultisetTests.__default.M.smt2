...
(declare-fun MultisetTests.__default.F (T@U T@U Int) Int)
(declare-fun $LS (T@U) T@U)
(declare-fun |MultisetTests.__default.F#canCall| (T@U Int) Bool)
...
(assert  (=> (<= 0 $FunctionContextHeight) (forall (($ly T@U) (|a#0| T@U) (|n#0| Int) ) (!  (=> (or (|MultisetTests.__default.F#canCall| (Lit MultiSetType |a#0|) (LitInt |n#0|)) (and (< 0 $FunctionContextHeight) (and ($Is MultiSetType |a#0| (TMultiSet TInt)) (<= (LitInt 0) |n#0|)))) (and (=> (or (not (= (LitInt |n#0|) (LitInt 0))) (not true)) (|MultisetTests.__default.F#canCall| (Lit MultiSetType |a#0|) (LitInt (- |n#0| 1)))) (= (MultisetTests.__default.F ($LS $ly) (Lit MultiSetType |a#0|) (LitInt |n#0|)) (ite (= (LitInt |n#0|) (LitInt 0)) 0 (MultisetTests.__default.F ($LS $ly) (Lit MultiSetType |a#0|) (LitInt (- |n#0| 1)))))))
 :qid |Terminationdfy.384:18|
 :weight 3
 :skolemid |590|
 :pattern ( (MultisetTests.__default.F ($LS $ly) (Lit MultiSetType |a#0|) (LitInt |n#0|)))
))))
(assert (forall (($ly@@0 T@U) (|a#0@@0| T@U) (|n#0@@0| Int) ) (! (= (MultisetTests.__default.F ($LS $ly@@0) |a#0@@0| |n#0@@0|) (MultisetTests.__default.F $ly@@0 |a#0@@0| |n#0@@0|))
 :qid |Terminationdfy.384:18|
 :skolemid |585|
 :pattern ( (MultisetTests.__default.F ($LS $ly@@0) |a#0@@0| |n#0@@0|))
)))
...
(assert  (=> (<= 0 $FunctionContextHeight) (forall (($ly@@1 T@U) (|a#0@@1| T@U) (|n#0@@1| Int) ) (!  (=> (or (|MultisetTests.__default.F#canCall| |a#0@@1| |n#0@@1|) (and (< 0 $FunctionContextHeight) (and ($Is MultiSetType |a#0@@1| (TMultiSet TInt)) (<= (LitInt 0) |n#0@@1|)))) (and (=> (or (not (= |n#0@@1| (LitInt 0))) (not true)) (|MultisetTests.__default.F#canCall| |a#0@@1| (- |n#0@@1| 1))) (= (MultisetTests.__default.F ($LS $ly@@1) |a#0@@1| |n#0@@1|) (ite (= |n#0@@1| (LitInt 0)) 0 (MultisetTests.__default.F $ly@@1 |a#0@@1| (- |n#0@@1| 1))))))
 :qid |Terminationdfy.384:18|
 :skolemid |589|
 :pattern ( (MultisetTests.__default.F ($LS $ly@@1) |a#0@@1| |n#0@@1|))
))))
...
(declare-fun |b#0| () T@U)
(set-info :boogie-vc-id Impl$$MultisetTests.__default.M)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap $Heap@0)) (and (forall ((|$ih#n0#0| Int) (|$ih#b0#0| T@U) ) (!  (=> (and (and (and (<= (LitInt 0) |$ih#n0#0|) ($Is MultiSetType |$ih#b0#0| (TMultiSet TInt))) (U_2_bool (Lit boolType (bool_2_U true)))) (or (and (<= 0 |$ih#n0#0|) (< |$ih#n0#0| |n#0@@2|)) (and (= |$ih#n0#0| |n#0@@2|) (and (|MultiSet#Subset| |$ih#b0#0| |b#0|) (not (|MultiSet#Equal| |$ih#b0#0| |b#0|)))))) (= (MultisetTests.__default.F ($LS $LZ) |$ih#b0#0| |$ih#n0#0|) (LitInt 0)))
 :qid |Terminationdfy.395:11|
 :skolemid |599|
 :pattern ( (MultisetTests.__default.F ($LS $LZ) |$ih#b0#0| |$ih#n0#0|))
)) (= (ControlFlow 0 2) (- 0 1)))) (= (MultisetTests.__default.F ($LS ($LS $LZ)) |b#0| |n#0@@2|) (LitInt 0))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (<= (LitInt 0) |n#0@@2|)) (and (and ($Is MultiSetType |b#0| (TMultiSet TInt)) ($IsAlloc MultiSetType |b#0| (TMultiSet TInt) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2)))) anon0_correct)))
