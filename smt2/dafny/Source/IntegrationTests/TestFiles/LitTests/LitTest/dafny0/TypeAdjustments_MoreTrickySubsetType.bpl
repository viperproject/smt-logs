...
procedure {:verboseName "MoreTrickySubsetType.M0 (well-formedness)"} CheckWellFormed$$MoreTrickySubsetType.__default.M0(n#0: int where LitInt(0) <= n#0, b#0: bool);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "MoreTrickySubsetType.M0 (call)"} Call$$MoreTrickySubsetType.__default.M0(n#0: int where LitInt(0) <= n#0, b#0: bool);
  modifies $Heap;
...
procedure {:verboseName "MoreTrickySubsetType.M0 (correctness)"} Impl$$MoreTrickySubsetType.__default.M0(n#0: int where LitInt(0) <= n#0, b#0: bool) returns ($_reverifyPost: bool);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
...
procedure {:verboseName "MoreTrickySubsetType.Nat (well-formedness)"} CheckWellFormed$$MoreTrickySubsetType.Nat(x#0: int);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "MoreTrickySubsetType.Nat (well-formedness)"} CheckWellFormed$$MoreTrickySubsetType.Nat(x#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;


    // AddWellformednessCheck for subset type Nat
    $_ReadsFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    if (*)
    {
        // check well-formedness of subset type constraint
        assume {:id "id306"} LitInt(0) <= x#0;
        assume false;
    }
    ...
    if (*)
    {
        ...
        assume true;
        assert {:id "id307"} LitInt(0) <= LitInt(0);
        assume false;
    }
}
