...
axiom 1 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, $Heap: Heap, n#0: int :: 
    ...
    CoRecursion.__default.AscendingChainAndRead#canCall($Heap, n#0)
         || (1 < $FunctionContextHeight && $IsGoodHeap($Heap) && LitInt(0) <= n#0)
...
procedure {:verboseName "CoRecursion.AscendingChainAndRead (well-formedness)"} CheckWellformed$$CoRecursion.__default.AscendingChainAndRead(n#0: int where LitInt(0) <= n#0);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "CoRecursion.AscendingChainAndRead (well-formedness)"} CheckWellformed$$CoRecursion.__default.AscendingChainAndRead(n#0: int)
{
  ...
  var b$reqreads#0: bool;
    ...
    $_ReadsFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> $o == null);
    ...
    else
    {
        ...
        b$reqreads#0 := (forall $o: ref, $f: Field :: 
          $o != null && $Unbox(read($Heap, $o, alloc)): bool && $o == null
             ==> $_ReadsFrame[$o, $f]);
        assert {:id "id4"} 0 <= n#0
           || (Set#Subset(Set#UnionOne(Set#Empty(): Set, $Box(null)), 
              Set#UnionOne(Set#Empty(): Set, $Box(null)))
             && !Set#Subset(Set#UnionOne(Set#Empty(): Set, $Box(null)), 
              Set#UnionOne(Set#Empty(): Set, $Box(null))))
           || ##n#0 == n#0;
        assert {:id "id5"} (Set#Subset(Set#UnionOne(Set#Empty(): Set, $Box(null)), 
              Set#UnionOne(Set#Empty(): Set, $Box(null)))
             && !Set#Subset(Set#UnionOne(Set#Empty(): Set, $Box(null)), 
              Set#UnionOne(Set#Empty(): Set, $Box(null))))
           || (Set#Equal(Set#UnionOne(Set#Empty(): Set, $Box(null)), 
              Set#UnionOne(Set#Empty(): Set, $Box(null)))
             && ##n#0 < n#0);
        ...
        assume {:id "id6"} CoRecursion.__default.AscendingChainAndRead($LS($LZ), $Heap, n#0)
...
procedure {:verboseName "CoRecursion.AscendingChainAndPostcondition (well-formedness)"} CheckWellformed$$CoRecursion.__default.AscendingChainAndPostcondition(n#0: int where LitInt(0) <= n#0);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "CoRecursion.AscendingChainAndPostcondition (well-formedness)"} CheckWellformed$$CoRecursion.__default.AscendingChainAndPostcondition(n#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
  var ##n#0: int;
    ...
    else
    {
        ...
        // assume allocatedness for argument to function
        assume $IsAlloc(##n#0, Tclass._System.nat(), $Heap);
        assert {:id "id13"} 0 <= n#0 || ##n#0 == n#0;
        assert {:id "id14"} ##n#0 < n#0;
        ...
        assume {:id "id15"} CoRecursion.__default.AscendingChainAndPostcondition($LS($LZ), n#0)
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "CoRecursion.Prefix (well-formedness)"} CheckWellformed$$CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0: Ty, n#0: int, s#0: DatatypeType)
{
  ...
  var ##s#0: DatatypeType;
    ...
    else
    {
        ...
        if (n#0 == LitInt(0))
        {
            assume {:id "id19"} CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0, $LS($LZ), n#0, s#0)
        ...
        else
        {
            ...
            assert {:id "id21"} 0 <= n#0 || ##n#0 == n#0;
            assert {:id "id22"} ##n#0 < n#0;
            assume CoRecursion.__default.Prefix#canCall(CoRecursion._default.Prefix$_T0, n#0 - 1, CoRecursion.Stream.rest(s#0));
            assume {:id "id23"} CoRecursion.__default.Prefix(CoRecursion._default.Prefix$_T0, $LS($LZ), n#0, s#0)
...
axiom (forall a#13#0#0: Box, a#13#1#0: DatatypeType :: 
  ...
  CoRecursion.List._h0(#CoRecursion.List.Cons(a#13#0#0, a#13#1#0)) == a#13#0#0);
...
axiom (forall a#15#0#0: Box, a#15#1#0: DatatypeType :: 
  ...
  CoRecursion.List._h1(#CoRecursion.List.Cons(a#15#0#0, a#15#1#0)) == a#15#1#0);
