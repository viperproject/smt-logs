...
(assert (distinct TInt TagInt alloc Tagclass._module.Tree |##_module.Tree.Empty| |##_module.Tree.Node| tytagFamily$Tree)
)
...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($ly T@U) (this T@U) ) (!  (=> (or (|_module.Tree.Sum#canCall| this) (and (< 1 $FunctionContextHeight) ($Is DatatypeTypeType this Tclass._module.Tree))) (and (=> (not (_module.Tree.Empty_q this)) (let ((|right#1| (_module.Tree.right this)))
(let ((|left#1| (_module.Tree.left this)))
 (and (|_module.Tree.Sum#canCall| |left#1|) (|_module.Tree.Sum#canCall| |right#1|))))) (= (_module.Tree.Sum ($LS $ly) this) (ite (_module.Tree.Empty_q this) 0 (let ((|right#0| (_module.Tree.right this)))
(let ((|left#0| (_module.Tree.left this)))
(let ((|x#0| (_module.Tree.root this)))
(+ (+ |x#0| (_module.Tree.Sum $ly |left#0|)) (_module.Tree.Sum $ly |right#0|)))))))))
 :qid |Terminationdfy.466:18|
 :skolemid |3648|
 :pattern ( (_module.Tree.Sum ($LS $ly) this))
))))
...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($ly@@0 T@U) (this@@0 T@U) ) (!  (=> (or (|_module.Tree.Sum#canCall| (Lit DatatypeTypeType this@@0)) (and (< 1 $FunctionContextHeight) ($Is DatatypeTypeType this@@0 Tclass._module.Tree))) (and (=> (not (U_2_bool (Lit boolType (bool_2_U (_module.Tree.Empty_q (Lit DatatypeTypeType this@@0)))))) (let ((|right#3| (Lit DatatypeTypeType (_module.Tree.right (Lit DatatypeTypeType this@@0)))))
(let ((|left#3| (Lit DatatypeTypeType (_module.Tree.left (Lit DatatypeTypeType this@@0)))))
 (and (|_module.Tree.Sum#canCall| |left#3|) (|_module.Tree.Sum#canCall| |right#3|))))) (= (_module.Tree.Sum ($LS $ly@@0) (Lit DatatypeTypeType this@@0)) (ite (_module.Tree.Empty_q (Lit DatatypeTypeType this@@0)) 0 (let ((|right#2| (Lit DatatypeTypeType (_module.Tree.right (Lit DatatypeTypeType this@@0)))))
...
(LitInt (+ (+ |x#2| (_module.Tree.Sum ($LS $ly@@0) |left#2|)) (_module.Tree.Sum ($LS $ly@@0) |right#2|))))))))))
 :qid |Terminationdfy.466:18|
 :weight 3
 :skolemid |3649|
 :pattern ( (_module.Tree.Sum ($LS $ly@@0) (Lit DatatypeTypeType this@@0)))
))))
...
(assert (forall (($ly@@1 T@U) (this@@1 T@U) ) (! (= (_module.Tree.Sum ($LS $ly@@1) this@@1) (_module.Tree.Sum $ly@@1 this@@1))
 :qid |Terminationdfy.466:18|
 :skolemid |3644|
 :pattern ( (_module.Tree.Sum ($LS $ly@@1) this@@1))
)))
...
(set-info :boogie-vc-id CheckWellformed$$_module.Tree.Sum)
...
(let ((anon10_Then_correct  (=> (= this@@2 (|#_module.Tree.Node| |_mcc#0#0| |_mcc#1#0| |_mcc#2#0|)) (=> (and (and ($Is DatatypeTypeType |_mcc#1#0| Tclass._module.Tree) ($Is DatatypeTypeType |_mcc#2#0| Tclass._module.Tree)) (and (= |let#0#0#0| |_mcc#2#0|) ($Is DatatypeTypeType |let#0#0#0| Tclass._module.Tree))) (=> (and (and (and (= |right#Z#0@0| |let#0#0#0|) (= |let#1#0#0| |_mcc#1#0|)) (and ($Is DatatypeTypeType |let#1#0#0| Tclass._module.Tree) (= |left#Z#0@0| |let#1#0#0|))) (and (and (= |let#2#0#0| |_mcc#0#0|) ($Is intType (int_2_U |let#2#0#0|) TInt)) (and (= |x#Z#0@0| |let#2#0#0|) ($IsAllocBox ($Box DatatypeTypeType |left#Z#0@0|) Tclass._module.Tree $Heap)))) (and (=> (= (ControlFlow 0 4) (- 0 5)) (< (DtRank |left#Z#0@0|) (DtRank this@@2))) (=> (< (DtRank |left#Z#0@0|) (DtRank this@@2)) (=> (|_module.Tree.Sum#canCall| |left#Z#0@0|) (=> (and ($IsAllocBox ($Box DatatypeTypeType |right#Z#0@0|) Tclass._module.Tree $Heap) (= (ControlFlow 0 4) (- 0 3))) (< (DtRank |right#Z#0@0|) (DtRank this@@2)))))))))))
(let ((anon9_Else_correct  (=> (or (not (= this@@2 |#_module.Tree.Empty|)) (not true)) (and (=> (= (ControlFlow 0 7) 4) anon10_Then_correct) (=> (= (ControlFlow 0 7) 6) anon10_Else_correct)))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is DatatypeTypeType this@@2 Tclass._module.Tree) ($IsAlloc DatatypeTypeType this@@2 Tclass._module.Tree $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 9) 8))) anon0_correct))))
