...
(set-info :boogie-vc-id Impl$$_module.__default.AssignSuchThat6)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 6) (let ((anon2_correct  (=> (=> true (and ($Is refType |n#0@1| Tclass._module.Node) ($IsAlloc refType |n#0@1| Tclass._module.Node $Heap))) (=> (and (and (or (not (= |n#0@1| null)) (not true)) (not (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap |n#0@1|) alloc))))) (= (ControlFlow 0 2) (- 0 1))) (U_2_bool (Lit boolType (bool_2_U false)))))))
(let ((anon3_Else_correct  (=> (and (not (and ($Is refType |n#1@0| Tclass._module.Node) ($IsAlloc refType |n#1@0| Tclass._module.Node $Heap))) (= (ControlFlow 0 4) 2)) anon2_correct)))
(let ((anon3_Then_correct  (=> (and (and ($Is refType |n#1@0| Tclass._module.Node) ($IsAlloc refType |n#1@0| Tclass._module.Node $Heap)) (= (ControlFlow 0 3) 2)) anon2_correct)))
(let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> |defass#n#0| (and ($Is refType |n#0@0| Tclass._module.Node) ($IsAlloc refType |n#0@0| Tclass._module.Node $Heap)))) (and (=> (= (ControlFlow 0 5) 3) anon3_Then_correct) (=> (= (ControlFlow 0 5) 4) anon3_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (=> |defass#n#0| (and ($Is refType |n#0| Tclass._module.Node) ($IsAlloc refType |n#0| Tclass._module.Node $Heap))) true) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct))))
