...
(declare-fun Constraints.__default.IsEven (Int) Bool)
(declare-fun |Constraints.__default.IsEven#canCall| (Int) Bool)
...
(assert  (=> (<= 0 $FunctionContextHeight) (forall ((|x#0| Int) ) (!  (=> (or (|Constraints.__default.IsEven#canCall| |x#0|) (< 0 $FunctionContextHeight)) (= (Constraints.__default.IsEven |x#0|) (= (Mod |x#0| (LitInt 2)) (LitInt 0))))
 :qid |Newtypesdfy.56:26|
 :skolemid |548|
 :pattern ( (Constraints.__default.IsEven |x#0|))
))))
(assert  (=> (<= 0 $FunctionContextHeight) (forall ((|x#0@@0| Int) ) (!  (=> (or (|Constraints.__default.IsEven#canCall| (LitInt |x#0@@0|)) (< 0 $FunctionContextHeight)) (= (Constraints.__default.IsEven (LitInt |x#0@@0|)) (= (LitInt (Mod |x#0@@0| (LitInt 2))) (LitInt 0))))
 :qid |Newtypesdfy.56:26|
 :weight 3
 :skolemid |549|
 :pattern ( (Constraints.__default.IsEven (LitInt |x#0@@0|)))
))))
...
(set-info :boogie-vc-id CheckWellformed$$Constraints.__default.IsEven)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon5_Else_correct  (=> (= (ControlFlow 0 3) (- 0 2)) (or (not (= (LitInt 2) 0)) (not true)))))
