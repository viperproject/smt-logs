   ...
   ==> (forall $Heap: Heap, source#0: ref, sink#0: ref, S#0: Set :: 
    ...
    Reachable3.__default.Reachable#canCall($Heap, source#0, sink#0, S#0)
       ...
       ==> (forall via#0: DatatypeType :: 
          { Reachable3.__default.ReachableVia($LS($LZ), $Heap, source#0, via#0, sink#0, S#0, 5) } 
          $Is(via#0, Tclass.Reachable3.Path(Tclass.Reachable3.Node()))
             ==> Reachable3.__default.ReachableVia#canCall($Heap, source#0, via#0, sink#0, S#0, LitInt(5)))
         && Reachable3.__default.Reachable($Heap, source#0, sink#0, S#0)
           == (exists via#0: DatatypeType :: 
            { Reachable3.__default.ReachableVia($LS($LZ), $Heap, source#0, via#0, sink#0, S#0, 5) } 
            $Is(via#0, Tclass.Reachable3.Path(Tclass.Reachable3.Node()))
               && Reachable3.__default.ReachableVia($LS($LZ), $Heap, source#0, via#0, sink#0, S#0, LitInt(5))));
}
...
procedure {:verboseName "Reachable3.Reachable (well-formedness)"} CheckWellformed$$Reachable3.__default.Reachable(source#0: ref where $Is(source#0, Tclass.Reachable3.Node()), 
    sink#0: ref where $Is(sink#0, Tclass.Reachable3.Node()), 
    S#0: Set where $Is(S#0, TSet(Tclass.Reachable3.Node())));
  free requires 2 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Reachable3.Reachable (well-formedness)"} CheckWellformed$$Reachable3.__default.Reachable(source#0: ref, sink#0: ref, S#0: Set)
{
  ...
  var b$reqreads#0: bool;
    ...
    else
    {
        ...
        havoc via#1;
        if ($Is(via#1, Tclass.Reachable3.Path(Tclass.Reachable3.Node()))
           && $IsAlloc(via#1, Tclass.Reachable3.Path(Tclass.Reachable3.Node()), $Heap))
        {
            ##source#0 := source#0;
            // assume allocatedness for argument to function
            assume $IsAlloc(##source#0, Tclass.Reachable3.Node(), $Heap);
            ##p#0 := via#1;
            // assume allocatedness for argument to function
            assume $IsAlloc(##p#0, Tclass.Reachable3.Path(Tclass.Reachable3.Node()), $Heap);
            ##sink#0 := sink#0;
            // assume allocatedness for argument to function
            assume $IsAlloc(##sink#0, Tclass.Reachable3.Node(), $Heap);
            ##S#0 := S#0;
            // assume allocatedness for argument to function
            assume $IsAlloc(##S#0, TSet(Tclass.Reachable3.Node()), $Heap);
            assert {:id "id53"} $Is(LitInt(5), Tclass._System.nat());
            ##bound#0 := LitInt(5);
            // assume allocatedness for argument to function
            assume $IsAlloc(##bound#0, Tclass._System.nat(), $Heap);
            b$reqreads#0 := (forall $o: ref, $f: Field :: 
              $o != null
                   && $Unbox(read($Heap, $o, alloc)): bool
                   && Set#IsMember(##S#0, $Box($o))
                 ==> $_ReadsFrame[$o, $f]);
            assume Reachable3.__default.ReachableVia#canCall($Heap, source#0, via#1, sink#0, S#0, LitInt(5));
        }

        // End Comprehension WF check
        assume {:id "id54"} Reachable3.__default.Reachable($Heap, source#0, sink#0, S#0)
           == (exists via#2: DatatypeType :: 
            { Reachable3.__default.ReachableVia($LS($LZ), $Heap, source#0, via#2, sink#0, S#0, 5) } 
            $Is(via#2, Tclass.Reachable3.Path(Tclass.Reachable3.Node()))
               && Reachable3.__default.ReachableVia($LS($LZ), $Heap, source#0, via#2, sink#0, S#0, LitInt(5)));
        assume (forall via#2: DatatypeType :: 
          { Reachable3.__default.ReachableVia($LS($LZ), $Heap, source#0, via#2, sink#0, S#0, 5) } 
          $Is(via#2, Tclass.Reachable3.Path(Tclass.Reachable3.Node()))
             ==> Reachable3.__default.ReachableVia#canCall($Heap, source#0, via#2, sink#0, S#0, LitInt(5)));
        // CheckWellformedWithResult: any expression
        assume $Is(Reachable3.__default.Reachable($Heap, source#0, sink#0, S#0), TBool);
        assert {:id "id55"} b$reqreads#0;
        return;
    ...
    Reachable3.__default.ReachableVia#canCall($Heap, source#0, p#0, sink#0, S#0, bound#0)
       ...
       ==> (bound#0 != 0
          ...
          !Reachable3.Path.Empty_q(p#0)
           ==> (var n#1 := $Unbox(Reachable3.Path._h9(p#0)): ref; 
            (var prefix#1 := Reachable3.Path._h8(p#0); 
              Set#IsMember(S#0, $Box(n#1))
                 ==> 
                Seq#Contains($Unbox(read($Heap, n#1, Reachable3.Node.children)): Seq, $Box(sink#0))
                 ==> Reachable3.__default.ReachableVia#canCall($Heap, source#0, prefix#1, n#1, S#0, bound#0 - 1))))
         && Reachable3.__default.ReachableVia($LS($ly), $Heap, source#0, p#0, sink#0, S#0, bound#0)
           == (bound#0 != 0
             ==> (if Reachable3.Path.Empty_q(p#0)
               ...
               else (var n#0 := $Unbox(Reachable3.Path._h9(p#0)): ref; 
                (var prefix#0 := Reachable3.Path._h8(p#0); 
                  Set#IsMember(S#0, $Box(n#0))
                     && Seq#Contains($Unbox(read($Heap, n#0, Reachable3.Node.children)): Seq, $Box(sink#0))
                     && Reachable3.__default.ReachableVia($ly, $Heap, source#0, prefix#0, n#0, S#0, bound#0 - 1))))));
}
...
procedure {:verboseName "Reachable3.ReachableVia (well-formedness)"} CheckWellformed$$Reachable3.__default.ReachableVia(source#0: ref where $Is(source#0, Tclass.Reachable3.Node()), 
    p#0: DatatypeType
       where $Is(p#0, Tclass.Reachable3.Path(Tclass.Reachable3.Node())), 
    sink#0: ref where $Is(sink#0, Tclass.Reachable3.Node()), 
    S#0: Set where $Is(S#0, TSet(Tclass.Reachable3.Node())), 
    bound#0: int where LitInt(0) <= bound#0);
  ...
  ensures (forall $olderHeap: Heap :: 
    ...
    $IsGoodHeap($olderHeap)
         && $OlderTag($olderHeap)
         && 
        Reachable3.__default.ReachableVia($LS($LZ), $Heap, source#0, p#0, sink#0, S#0, bound#0)
         && 
        $IsAlloc(source#0, Tclass.Reachable3.Node(), $olderHeap)
         && $IsAlloc(sink#0, Tclass.Reachable3.Node(), $olderHeap)
         && $IsAlloc(S#0, TSet(Tclass.Reachable3.Node()), $olderHeap)
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Reachable3.ReachableVia (well-formedness)"} CheckWellformed$$Reachable3.__default.ReachableVia(source#0: ref, p#0: DatatypeType, sink#0: ref, S#0: Set, bound#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
  ...
  var b$reqreads#1: bool;
    ...
    else
    {
        ...
        if (bound#0 != 0)
        {
            if (p#0 == #Reachable3.Path.Empty())
            {
            }
            else if (p#0 == #Reachable3.Path.Extend(_mcc#0#0, $Box(_mcc#1#0)))
            {
                assume $Is(_mcc#0#0, Tclass.Reachable3.Path(Tclass.Reachable3.Node()));
                assume $Is(_mcc#1#0, Tclass.Reachable3.Node());
                havoc n#Z#0;
                assume {:id "id57"} let#0#0#0 == _mcc#1#0;
                assume true;
                // CheckWellformedWithResult: any expression
                assume $Is(let#0#0#0, Tclass.Reachable3.Node());
                assume {:id "id58"} n#Z#0 == let#0#0#0;
                havoc prefix#Z#0;
                assume {:id "id59"} let#1#0#0 == _mcc#0#0;
                assume true;
                // CheckWellformedWithResult: any expression
                assume $Is(let#1#0#0, Tclass.Reachable3.Path(Tclass.Reachable3.Node()));
                assume {:id "id60"} prefix#Z#0 == let#1#0#0;
                if (Set#IsMember(S#0, $Box(n#Z#0)))
                {
                    assert {:id "id61"} n#Z#0 != null;
                    b$reqreads#0 := $_ReadsFrame[n#Z#0, Reachable3.Node.children];
                }

                if (Set#IsMember(S#0, $Box(n#Z#0))
                   && Seq#Contains($Unbox(read($Heap, n#Z#0, Reachable3.Node.children)): Seq, $Box(sink#0)))
                {
                    ##source#0 := source#0;
                    // assume allocatedness for argument to function
                    assume $IsAlloc(##source#0, Tclass.Reachable3.Node(), $Heap);
                    ...
                    // assume allocatedness for argument to function
                    assume $IsAlloc(##S#0, TSet(Tclass.Reachable3.Node()), $Heap);
                    assert {:id "id62"} $Is(bound#0 - 1, Tclass._System.nat());
                    ##bound#0 := bound#0 - 1;
                    // assume allocatedness for argument to function
                    assume $IsAlloc(##bound#0, Tclass._System.nat(), $Heap);
                    b$reqreads#1 := (forall $o: ref, $f: Field :: 
                      $o != null
                           ...
                           && Set#IsMember(##S#0, $Box($o))
                         ==> $_ReadsFrame[$o, $f]);
                    assert {:id "id63"} DtRank(##p#0) < DtRank(p#0);
                    assume Reachable3.__default.ReachableVia#canCall($Heap, source#0, prefix#Z#0, n#Z#0, S#0, bound#0 - 1);
                }
            }
            else
            {
        ...
        assume {:id "id64"} Reachable3.__default.ReachableVia($LS($LZ), $Heap, source#0, p#0, sink#0, S#0, bound#0)
           == (bound#0 != 0
             ==> (if Reachable3.Path.Empty_q(p#0)
               then source#0 == sink#0
               else (var n#3 := $Unbox(Reachable3.Path._h9(p#0)): ref; 
                (var prefix#3 := Reachable3.Path._h8(p#0); 
                  Set#IsMember(S#0, $Box(n#3))
                     && Seq#Contains($Unbox(read($Heap, n#3, Reachable3.Node.children)): Seq, $Box(sink#0))
                     && Reachable3.__default.ReachableVia($LS($LZ), $Heap, source#0, prefix#3, n#3, S#0, bound#0 - 1)))));
        assume bound#0 != 0
           ==> 
          !Reachable3.Path.Empty_q(p#0)
           ==> (var n#4 := $Unbox(Reachable3.Path._h9(p#0)): ref; 
            (var prefix#4 := Reachable3.Path._h8(p#0); 
              Set#IsMember(S#0, $Box(n#4))
                 ==> 
                Seq#Contains($Unbox(read($Heap, n#4, Reachable3.Node.children)): Seq, $Box(sink#0))
                 ==> Reachable3.__default.ReachableVia#canCall($Heap, source#0, prefix#4, n#4, S#0, bound#0 - 1)));
        // CheckWellformedWithResult: any expression
        assume $Is(Reachable3.__default.ReachableVia($LS($LZ), $Heap, source#0, p#0, sink#0, S#0, bound#0), 
          TBool);
        assert {:id "id65"} b$reqreads#0;
        assert {:id "id66"} b$reqreads#1;
        return;
