...
(set-info :boogie-vc-id CheckWellformed$$Nested.__default.A)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (and (= $_ReadsFrame@0 (|lambda#0| null $Heap alloc false)) (= |##x#0@0| (LitInt (Nested.__default.C ($LS $LZ))))) (and ($IsAlloc intType (int_2_U |##x#0@0|) Tclass._System.nat $Heap) (= (ControlFlow 0 2) (- 0 1)))) false)))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (<= (LitInt 0) |x#0@@4|)) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct)))
