...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (<= (LitInt 0) |j#0|)) (and (=> (= (ControlFlow 0 2) (- 0 4)) (<= (LitInt 0) |j#0|)) (=> (<= (LitInt 0) |j#0|) (and (=> (= (ControlFlow 0 2) (- 0 3)) ($Is intType (int_2_U (+ |j#0| |j#0|)) Tclass._System.nat)) (=> ($Is intType (int_2_U (+ |j#0| |j#0|)) Tclass._System.nat) (=> (and (= |n#0@0| (+ |j#0| |j#0|)) (= (ControlFlow 0 2) (- 0 1))) (= |n#0@0| (Mul (LitInt 2) |j#0|))))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (<= (LitInt 0) |n#0|)) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 5) 2))) anon0_correct)))
