...
axiom {:id "id21"} 0 <= $FunctionContextHeight
   ==> (forall $Heap: Heap :: 
    ...
    LetSuchThat.__default.Bad#canCall()
         || (0 < $FunctionContextHeight && $IsGoodHeap($Heap))
       ==> $let#0$canCall()
         && LetSuchThat.__default.Bad() == (var c#1 := $let#0_c(); LitInt(5)));
}
...
procedure {:verboseName "LetSuchThat.Bad (well-formedness)"} CheckWellformed$$LetSuchThat.__default.Bad();
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "LetSuchThat.Bad (well-formedness)"} CheckWellformed$$LetSuchThat.__default.Bad()
{
  ...
  var c#2: Box;
    ...
    else
    {
        ...
        havoc c#2;
        if ($IsBox(c#2, Tclass.LetSuchThat.C())
           && $IsAllocBox(c#2, Tclass.LetSuchThat.C(), $Heap))
        {
        }

        assert {:id "id22"} (exists c#3: Box :: $IsBox(c#3, Tclass.LetSuchThat.C()) && Lit(true));
        assume $IsBox(c#2, Tclass.LetSuchThat.C())
           && $IsAllocBox(c#2, Tclass.LetSuchThat.C(), $Heap);
        assume true;
        assume $let#0$canCall();
        assume {:id "id23"} LetSuchThat.__default.Bad() == LitInt(5);
        assume true;
   ...
   ==> (forall $Heap: Heap, y#0: Box :: 
    ...
    LetSuchThat.__default.Good#canCall(y#0)
         || (0 < $FunctionContextHeight
           && 
          $IsGoodHeap($Heap)
           && $IsBox(y#0, Tclass.LetSuchThat.C()))
       ==> $let#3$canCall()
         && LetSuchThat.__default.Good(y#0) == (var c#0 := $let#3_c(); LitInt(5)));
    ...
    LetSuchThat.__default.Good#canCall(Lit(y#0))
         || (0 < $FunctionContextHeight
          ...
          $IsGoodHeap($Heap)
           && $IsBox(y#0, Tclass.LetSuchThat.C()))
       ==> $let#3$canCall()
         && LetSuchThat.__default.Good(Lit(y#0)) == (var c#1 := $let#3_c(); LitInt(5)));
}
...
procedure {:verboseName "LetSuchThat.Good (well-formedness)"} CheckWellformed$$LetSuchThat.__default.Good(y#0: Box where $IsBox(y#0, Tclass.LetSuchThat.C()));
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "LetSuchThat.Good (well-formedness)"} CheckWellformed$$LetSuchThat.__default.Good(y#0: Box)
{
  var $_ReadsFrame: [ref,Field]bool;
  var c#2: Box;
    ...
    else
    {
        ...
        assert {:id "id26"} (exists c#3: Box :: $IsBox(c#3, Tclass.LetSuchThat.C()) && Lit(true));
        assume $IsBox(c#2, Tclass.LetSuchThat.C())
           && $IsAllocBox(c#2, Tclass.LetSuchThat.C(), $Heap);
        assume true;
        assume $let#3$canCall();
        assume {:id "id27"} LetSuchThat.__default.Good(y#0) == LitInt(5);
        assume true;
        // CheckWellformedWithResult: any expression
