...
(assert (distinct TagMultiSet alloc Tagclass._System.nat tytagFamily$nat)
)
...
(set-info :boogie-vc-id Impl$$_module.__default.UpdateValidityMultiset)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 8) (let ((anon3_Else_correct  (and (=> (= (ControlFlow 0 5) (- 0 6)) ($Is intType (int_2_U (LitInt 2)) Tclass._System.nat)) (=> ($Is intType (int_2_U (LitInt 2)) Tclass._System.nat) (=> (= (ControlFlow 0 5) (- 0 4)) (<= 0 (LitInt (- 0 5))))))))
(let ((anon3_Then_correct  (and (=> (= (ControlFlow 0 2) (- 0 3)) ($Is intType (int_2_U (LitInt (- 0 2))) Tclass._System.nat)) (=> ($Is intType (int_2_U (LitInt (- 0 2))) Tclass._System.nat) (=> (= (ControlFlow 0 2) (- 0 1)) (<= 0 (LitInt 5)))))))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and ($Is MultiSetType |s#0@0| (TMultiSet Tclass._System.nat)) ($IsAlloc MultiSetType |s#0@0| (TMultiSet Tclass._System.nat) $Heap)) (and (=> (= (ControlFlow 0 7) 2) anon3_Then_correct) (=> (= (ControlFlow 0 7) 5) anon3_Else_correct))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is MultiSetType |s#0| (TMultiSet Tclass._System.nat)) ($IsAlloc MultiSetType |s#0| (TMultiSet Tclass._System.nat) $Heap)) true) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 8) 7))) anon0_correct))))
