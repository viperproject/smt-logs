...
(set-info :boogie-vc-id Impl$$_module.__default.TestIMapPropertyMembership2)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 6) (let ((anon2_correct  (=> (= (ControlFlow 0 2) (- 0 1)) (exists ((|kv#1| T@U) ) (!  (and ($Is DatatypeTypeType |kv#1| (Tclass._System.Tuple2 Tclass._module.MyClass (TBitvector 3))) (U_2_bool (MapType0Select BoxType boolType (|IMap#Items| |m#0|) ($Box DatatypeTypeType |kv#1|))))
 :qid |Mapsdfy.631:17|
 :skolemid |893|
 :pattern ( (MapType0Select BoxType boolType (|IMap#Items| |m#0|) ($Box DatatypeTypeType |kv#1|)))
)))))
