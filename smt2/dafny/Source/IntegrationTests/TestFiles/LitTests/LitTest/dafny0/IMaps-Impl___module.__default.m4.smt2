...
(let ((anon3_correct  (=> (= |a#0@0| (|IMap#Glue| (|lambda#2| TInt (LitInt 3)) (|lambda#3| ($Box intType (int_2_U (LitInt 9)))) (TIMap TInt TInt))) (and (=> (= (ControlFlow 0 4) (- 0 5)) (U_2_bool (MapType0Select BoxType boolType (|IMap#Domain| |a#0@0|) ($Box intType (int_2_U (LitInt 4)))))) (=> (U_2_bool (MapType0Select BoxType boolType (|IMap#Domain| |a#0@0|) ($Box intType (int_2_U (LitInt 4))))) (and (=> (= (ControlFlow 0 4) 2) anon8_Then_correct) (=> (= (ControlFlow 0 4) 3) anon8_Else_correct)))))))
(let ((anon7_Else_correct  (=> (and (or (not (= |i#0@0| (LitInt 3))) (not true)) (= (ControlFlow 0 7) 4)) anon3_correct)))
(let ((anon7_Then_correct  (=> (and (= |i#0@0| (LitInt 3)) (= (ControlFlow 0 6) 4)) anon3_correct)))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is IMapType |a#0| (TIMap TInt TInt)) ($IsAlloc IMapType |a#0| (TIMap TInt TInt) $Heap)) true) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 10) 9))) anon0_correct))))
