...
const unique class.Module0.__default: ClassName;

procedure {:verboseName "Module0.M (well-formedness)"} CheckWellFormed$$Module0.__default.M(x#0: int) returns (n#0: int where Lit(true));
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "Module0.M (call)"} Call$$Module0.__default.M(x#0: int) returns (n#0: int where Lit(true));
  modifies $Heap;
...
procedure {:verboseName "Module0.M (correctness)"} Impl$$Module0.__default.M(x#0: int) returns (n#0: int where Lit(true), $_reverifyPost: bool);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
...
function Tclass.Module1.N9() : Ty
uses {
// Tclass.Module1.N9 Tag
axiom Tag(Tclass.Module1.N9()) == Tagclass.Module1.N9
   && TagFamily(Tclass.Module1.N9()) == tytagFamily$N9;
}

const unique Tagclass.Module1.N9: TyTag;

// Box/unbox axiom for Tclass.Module1.N9
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.Module1.N9()) } 
  $IsBox(bx, Tclass.Module1.N9())
     ==> $Box($Unbox(bx): int) == bx && $Is($Unbox(bx): int, Tclass.Module1.N9()));

// $Is axiom for newtype Module1.N9
axiom (forall c0#1: int :: 
  { $Is(c0#1, Tclass.Module1.N9()) } 
  $Is(c0#1, Tclass.Module1.N9()) <==> Lit(true));

// $IsAlloc axiom for newtype Module1.N9
axiom (forall c1#1: int, $h: Heap :: 
  { $IsAlloc(c1#1, Tclass.Module1.N9(), $h) } 
  $IsAlloc(c1#1, Tclass.Module1.N9(), $h));

const unique class.Module1.N9: ClassName;
...
const unique tytagFamily$N9: TyTagFamily;
