...
(assert (forall ((s@@0 T@U) ) (!  (=> (|$let#0$canCall| s@@0) (|Set#IsMember| s@@0 ($Box intType (int_2_U (|$let#0_x| s@@0)))))
 :qid |Basicsdfy.373:26|
 :skolemid |574|
 :pattern ( (|$let#0_x| s@@0))
)))
...
(set-info :boogie-vc-id CheckWellFormed$$SetCardinality.__default.LetSuchThatExpression)
...
(let ((anon7_Then_correct  (=> (= (ControlFlow 0 3) (- 0 2)) (or (and ($Is intType (int_2_U (LitInt 0)) TInt) (|Set#IsMember| |s#0| ($Box intType (int_2_U (LitInt 0))))) (exists ((|x#1| T@U) ) (! (|Set#IsMember| |s#0| ($Box intType |x#1|))
 :qid |Basicsdfy.373:30|
 :skolemid |573|
))))))
(let ((anon6_Then_correct  (=> (or (not (= (|Set#Card| |s#0|) 0)) (not true)) (and (=> (= (ControlFlow 0 5) 3) anon7_Then_correct) (=> (= (ControlFlow 0 5) 4) anon7_Else_correct)))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is SetType |s#0| (TSet TInt)) ($IsAlloc SetType |s#0| (TSet TInt) $Heap)) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 7) 6))) anon0_correct))))
