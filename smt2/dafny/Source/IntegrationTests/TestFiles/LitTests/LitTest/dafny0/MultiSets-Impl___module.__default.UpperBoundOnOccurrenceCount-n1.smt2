...
(set-info :boogie-vc-id Impl$$_module.__default.UpperBoundOnOccurrenceCount)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 4) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (and (=> (= (ControlFlow 0 2) (- 0 3)) (<= (LitInt 0) (|MultiSet#Multiplicity| |s#0| ($Box intType (int_2_U |x#0|))))) (=> (<= (LitInt 0) (|MultiSet#Multiplicity| |s#0| ($Box intType (int_2_U |x#0|)))) (=> (= (ControlFlow 0 2) (- 0 1)) (<= (|MultiSet#Multiplicity| |s#0| ($Box intType (int_2_U |x#0|))) (|MultiSet#Card| |s#0|))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is MultiSetType |s#0| (TMultiSet TInt)) ($IsAlloc MultiSetType |s#0| (TMultiSet TInt) $Heap)) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 4) 2))) anon0_correct))))
