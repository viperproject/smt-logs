...
(let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= |a#0@0| (Lit MapType (|Map#Build| |Map#Empty| ($Box intType (int_2_U (LitInt 3))) ($Box intType (int_2_U (LitInt 9))))))) (and (=> (= (ControlFlow 0 3) (- 0 4)) (|Set#IsMember| (|Map#Domain| |a#0@0|) ($Box intType (int_2_U (LitInt 4))))) (=> (|Set#IsMember| (|Map#Domain| |a#0@0|) ($Box intType (int_2_U (LitInt 4)))) (and (=> (= (ControlFlow 0 3) 1) anon3_Then_correct) (=> (= (ControlFlow 0 3) 2) anon3_Else_correct)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is MapType |a#0| (TMap TInt TInt)) ($IsAlloc MapType |a#0| (TMap TInt TInt) $Heap)) true) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 5) 3))) anon0_correct))))
