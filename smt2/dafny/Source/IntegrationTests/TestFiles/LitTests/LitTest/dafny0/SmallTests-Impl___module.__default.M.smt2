...
(assert (forall ((h@@1 T@U) (v@@5 T@U) ) (! ($IsAlloc boolType v@@5 TBool h@@1)
 :qid |DafnyPreludebpl.291:14|
 :skolemid |2697|
 :pattern ( ($IsAlloc boolType v@@5 TBool h@@1))
)))
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= |x#0@0| (|Seq#Build| |Seq#Empty| ($Box boolType (bool_2_U |Z#0|))))) (and (=> (= (ControlFlow 0 2) (- 0 4)) (and (<= 0 (LitInt 0)) (<= (LitInt 0) (|Seq#Length| |zeros#0|)))) (and (=> (= (ControlFlow 0 2) (- 0 3)) (and (<= (LitInt 0) (LitInt 1)) (<= (LitInt 1) (|Seq#Length| |zeros#0|)))) (=> (= (ControlFlow 0 2) (- 0 1)) (|Seq#Equal| (|Seq#Drop| (|Seq#Take| |zeros#0| (LitInt 1)) (LitInt 0)) (|Seq#Build| |Seq#Empty| ($Box boolType (bool_2_U |Z#0|))))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is SeqType |zeros#0| (TSeq TBool)) ($IsAlloc SeqType |zeros#0| (TSeq TBool) $Heap))) (=> (and (and (and (and ($Is SeqType |x#0| (TSeq TBool)) ($IsAlloc SeqType |x#0| (TSeq TBool) $Heap)) true) (= 0 $FunctionContextHeight)) (and (and (<= (LitInt 1) (|Seq#Length| |zeros#0|)) (= |Z#0| (U_2_bool (Lit boolType (bool_2_U false))))) (and (forall ((|k#1| Int) ) (!  (=> (and (<= (LitInt 0) |k#1|) (< |k#1| (|Seq#Length| |zeros#0|))) (= (U_2_bool ($Unbox boolType (|Seq#Index| |zeros#0| |k#1|))) |Z#0|))
 :qid |SmallTestsdfy.347:19|
 :skolemid |3154|
 :pattern ( ($Unbox boolType (|Seq#Index| |zeros#0| |k#1|)))
