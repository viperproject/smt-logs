...
procedure {:verboseName "Lets.f1 (well-formedness)"} CheckWellformed$$Lets.__default.f1(x#0: int);
...
axiom {:id "id154"} 1 <= $FunctionContextHeight
   ==> (forall x'#0: int :: 
    { Lets.__default.test1(x'#0) } 
    Lets.__default.test1#canCall(x'#0)
         || (1 < $FunctionContextHeight && (var y'#1 := Mul(LitInt(3), x'#0); y'#1 > 3))
       ==> (var y'#2 := Mul(LitInt(3), x'#0); Lets.__default.f1#canCall(y'#2))
         && Lets.__default.test1(x'#0)
           == (var y'#2 := Mul(LitInt(3), x'#0); Lets.__default.f1(y'#2)));
// definition axiom for Lets.__default.test1 for all literals (revealed)
axiom {:id "id155"} 1 <= $FunctionContextHeight
   ==> (forall x'#0: int :: 
    {:weight 3} { Lets.__default.test1(LitInt(x'#0)) } 
    Lets.__default.test1#canCall(LitInt(x'#0))
         || (1 < $FunctionContextHeight
           && (var y'#3 := LitInt(Mul(LitInt(3), LitInt(x'#0))); Lit(y'#3 > 3)))
       ==> (var y'#4 := LitInt(Mul(LitInt(3), LitInt(x'#0))); 
          Lets.__default.f1#canCall(y'#4))
         && Lets.__default.test1(LitInt(x'#0))
           == (var y'#4 := LitInt(Mul(LitInt(3), LitInt(x'#0))); Lets.__default.f1(y'#4)));
}
...
axiom (forall x'#0: int :: 
  ...
  Lets.__default.test1#requires(x'#0)
     == (var y'#1 := Mul(LitInt(3), x'#0); y'#1 > 3));

procedure {:verboseName "Lets.test1 (well-formedness)"} CheckWellformed$$Lets.__default.test1(x'#0: int);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Lets.test1 (well-formedness)"} CheckWellformed$$Lets.__default.test1(x'#0: int)
{
  ...
  var ##x#0: int;
    ...
    havoc y'#Z#0;
    assume {:id "id156"} let#0#0#0 == Mul(LitInt(3), x'#0);
    assume true;
    // CheckWellformedWithResult: any expression
    assume $Is(let#0#0#0, TInt);
    assume {:id "id157"} y'#Z#0 == let#0#0#0;
    assume {:id "id158"} (var y'#0 := Mul(LitInt(3), x'#0); y'#0 > 3);
    ...
    else
    {
        ...
        havoc y'#Z#1;
        assume {:id "id159"} let#1#0#0 == Mul(LitInt(3), x'#0);
        assume true;
        // CheckWellformedWithResult: any expression
        assume $Is(let#1#0#0, TInt);
        assume {:id "id160"} y'#Z#1 == let#1#0#0;
        ##x#0 := y'#Z#1;
        // assume allocatedness for argument to function
        assume $IsAlloc(##x#0, TInt, $Heap);
        assert {:id "id161"} {:subsumption 0} ##x#0 > 3;
        assume ##x#0 > 3;
        assume Lets.__default.f1#canCall(y'#Z#1);
        assume {:id "id162"} Lets.__default.test1(x'#0) == Lets.__default.f1(y'#Z#1);
        assume Lets.__default.f1#canCall(y'#Z#1);
        // CheckWellformedWithResult: any expression
        assume $Is(Lets.__default.test1(x'#0), TBool);
        return;
