...
function A.__default.F(x#0: int) : int
uses {
// consequence axiom for A.__default.F
axiom 0 <= $FunctionContextHeight
   ==> (forall x#0: int :: 
    { A.__default.F(x#0) } 
    A.__default.F#canCall(x#0) || 0 < $FunctionContextHeight
       ==> A.__default.F(x#0) == x#0);
}

function A.__default.F#canCall(x#0: int) : bool;

function A.__default.F#requires(int) : bool;

// #requires axiom for A.__default.F
axiom (forall x#0: int :: 
  { A.__default.F#requires(x#0) } 
  A.__default.F#requires(x#0) == true);

procedure {:verboseName "A.F (well-formedness)"} CheckWellformed$$A.__default.F(x#0: int) returns (r#0: int);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;
  ensures {:id "id102"} r#0 == x#0;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "A.F (well-formedness)"} CheckWellformed$$A.__default.F(x#0: int) returns (r#0: int)
{
...
procedure {:verboseName "A.M (well-formedness)"} CheckWellFormed$$A.__default.M(x#0: int) returns (r#0: int);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "A.M (call)"} Call$$A.__default.M(x#0: int) returns (r#0: int);
  modifies $Heap;
  // user-defined postconditions
  free ensures true;
  ensures {:id "id105"} r#0 == x#0;
...
procedure {:verboseName "A.Lemma0 (well-formedness)"} CheckWellFormed$$A.__default.Lemma0();
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "A.Lemma0 (call)"} Call$$A.__default.Lemma0();
  modifies $Heap;
  // user-defined postconditions
  free ensures A.__default.F#canCall(LitInt(5));
  ensures {:id "id107"} A.__default.F(LitInt(5)) == LitInt(5);
  // frame condition
  free ensures old($Heap) == $Heap;



procedure {:verboseName "A.Lemma0 (correctness)"} Impl$$A.__default.Lemma0() returns ($_reverifyPost: bool);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;
  // user-defined postconditions
  free ensures A.__default.F#canCall(LitInt(5));
  ensures {:id "id108"} A.__default.F(LitInt(5)) == LitInt(5);
  // frame condition
  free ensures old($Heap) == $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "A.Lemma0 (correctness)"} Impl$$A.__default.Lemma0() returns ($_reverifyPost: bool)
{
  var $_ModifiesFrame: [ref,Field]bool;

    // AddMethodImpl: Lemma0, Impl$$A.__default.Lemma0
