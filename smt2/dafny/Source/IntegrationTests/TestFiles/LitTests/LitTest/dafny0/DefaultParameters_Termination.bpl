...
const unique class.Termination.__default: ClassName;

// function declaration for Termination._default.R
function Termination.__default.R($ly: LayerType, n#0: int) : int
uses {
// consequence axiom for Termination.__default.R
axiom 0 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    { Termination.__default.R($ly, n#0) } 
    Termination.__default.R#canCall(n#0)
         || (0 < $FunctionContextHeight && LitInt(0) <= n#0)
       ==> LitInt(0) <= Termination.__default.R($ly, n#0));
// definition axiom for Termination.__default.R (revealed)
axiom {:id "id93"} 0 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    { Termination.__default.R($LS($ly), n#0) } 
    Termination.__default.R#canCall(n#0)
         || (0 < $FunctionContextHeight && LitInt(0) <= n#0)
       ==> Termination.__default.R($LS($ly), n#0) == n#0);
// definition axiom for Termination.__default.R for all literals (revealed)
axiom {:id "id94"} 0 <= $FunctionContextHeight
   ==> (forall $ly: LayerType, n#0: int :: 
    {:weight 3} { Termination.__default.R($LS($ly), LitInt(n#0)) } 
    Termination.__default.R#canCall(LitInt(n#0))
         || (0 < $FunctionContextHeight && LitInt(0) <= n#0)
       ==> Termination.__default.R($LS($ly), LitInt(n#0)) == LitInt(n#0));
}

function Termination.__default.R#canCall(n#0: int) : bool;

// layer synonym axiom
axiom (forall $ly: LayerType, n#0: int :: 
  { Termination.__default.R($LS($ly), n#0) } 
  Termination.__default.R($LS($ly), n#0) == Termination.__default.R($ly, n#0));

// fuel synonym axiom
axiom (forall $ly: LayerType, n#0: int :: 
  { Termination.__default.R(AsFuelBottom($ly), n#0) } 
  Termination.__default.R($ly, n#0) == Termination.__default.R($LZ, n#0));

function Termination.__default.R#requires(LayerType, int) : bool;

// #requires axiom for Termination.__default.R
axiom (forall $ly: LayerType, n#0: int :: 
  { Termination.__default.R#requires($ly, n#0) } 
  LitInt(0) <= n#0 ==> Termination.__default.R#requires($ly, n#0) == true);

procedure {:verboseName "Termination.R (well-formedness)"} CheckWellformed$$Termination.__default.R(n#0: int where LitInt(0) <= n#0);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Termination.R (well-formedness)"} CheckWellformed$$Termination.__default.R(n#0: int)
{
  ...
  var ##n#0: int;
    ...
    assume Termination.__default.R#canCall(LitInt(0));
    assume Termination.__default.R#canCall(LitInt(0));
    ...
    if (*)
    {
        ...
        assume LitInt(0) <= Termination.__default.R($LS($LZ), n#0);
        assume false;
    }
    else
    {
        // Check well-formedness of body and result subset type constraint
        assume {:id "id97"} Termination.__default.R($LS($LZ), n#0) == n#0;
        assume true;
        // CheckWellformedWithResult: any expression
        assume $Is(Termination.__default.R($LS($LZ), n#0), Tclass._System.nat());
        return;
