...
procedure {:verboseName "IteratorClient_Reveals.M (well-formedness)"} CheckWellFormed$$IteratorClient__Reveals.__default.M();
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "IteratorClient_Reveals.M (call)"} Call$$IteratorClient__Reveals.__default.M();
  modifies $Heap;
...
procedure {:verboseName "IteratorClient_Reveals.M (correctness)"} Impl$$IteratorClient__Reveals.__default.M() returns ($_reverifyPost: bool);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "IteratorClient_Reveals.M (correctness)"} Impl$$IteratorClient__Reveals.__default.M() returns ($_reverifyPost: bool)
{
  var $_ModifiesFrame: [ref,Field]bool;
  var defass#iter#0: bool;
  var iter#0: ref
     where defass#iter#0
       ==> $Is(iter#0, Tclass.ModuleWithIterator.Iter())
  ...
  var x##0: int;
    ...
    $_reverifyPost := false;
    // ----- assignment statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/Iterators.dfy(395,14)
    assume true;
    // ----- init call statement ----- /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/dafny0/Iterators.dfy(395,17)
    // TrCallStmt: Before ProcessCallStmt
    assume true;
    // ProcessCallStmt: CheckSubrange
    x##0 := LitInt(5);
    call {:id "id449"} $nw := Call$$ModuleWithIterator.Iter.__ctor(x##0);
...
procedure {:verboseName "ModuleWithIterator.Iter._ctor (call)"} Call$$ModuleWithIterator.Iter.__ctor(x#0: int)
  ...
  free ensures true;
  ensures {:id "id451"} ModuleWithIterator.Iter.x(this) == x#0;
  free ensures true;
  ensures {:id "id452"} Seq#Equal($Unbox(read($Heap, this, ModuleWithIterator.Iter.ys)): Seq, Seq#Empty(): Seq);
  free ensures ModuleWithIterator.Iter.Valid#canCall($Heap, this);
  ensures {:id "id453"} ModuleWithIterator.Iter.Valid($Heap, this);
  free ensures true;
  ensures {:id "id454"} Set#Equal(ModuleWithIterator.Iter.__reads(this), Set#Empty(): Set);
  free ensures true;
  ensures {:id "id455"} Set#Equal(ModuleWithIterator.Iter.__modifies(this), Set#Empty(): Set);
  free ensures true;
  ensures {:id "id456"} Set#Equal($Unbox(read($Heap, this, ModuleWithIterator.Iter.__new)): Set, Set#Empty(): Set);
  free ensures true;
  ensures {:id "id457"} ModuleWithIterator.Iter.__decreases0(this) == x#0;
