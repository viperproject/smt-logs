...
(set-info :boogie-vc-id Impl$$AssumeTypeAssumptions.__default.P1)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 6) (let ((anon2_correct  (=> (and (forall ((|path#2| T@U) ) (!  (=> ($Is SeqType |path#2| (TSeq TInt)) (|AssumeTypeAssumptions.__default.f#canCall| |path#2|))
 :qid |Basicsdfy.505:19|
 :skolemid |1137|
 :pattern ( (AssumeTypeAssumptions.__default.f |path#2|))
)) (= (ControlFlow 0 2) (- 0 1))) (exists ((|path#2@@0| T@U) ) (!  (and ($Is SeqType |path#2@@0| (TSeq TInt)) (AssumeTypeAssumptions.__default.f |path#2@@0|))
 :qid |Basicsdfy.505:19|
 :skolemid |1139|
 :pattern ( (AssumeTypeAssumptions.__default.f |path#2@@0|))
)))))
(let ((anon3_Else_correct  (=> (and (not (and ($Is SeqType |path#1@0| (TSeq TInt)) ($IsAlloc SeqType |path#1@0| (TSeq TInt) $Heap))) (= (ControlFlow 0 4) 2)) anon2_correct)))
(let ((anon3_Then_correct  (=> (and (and (and ($Is SeqType |path#1@0| (TSeq TInt)) ($IsAlloc SeqType |path#1@0| (TSeq TInt) $Heap)) ($IsAlloc SeqType |path#1@0| (TSeq TInt) $Heap)) (and (|AssumeTypeAssumptions.__default.f#canCall| |path#1@0|) (= (ControlFlow 0 3) 2))) anon2_correct)))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and ($Is SeqType |path#0@0| (TSeq TInt)) ($IsAlloc SeqType |path#0@0| (TSeq TInt) $Heap)) (=> (and (and ($IsAlloc SeqType |path#0@0| (TSeq TInt) $Heap) (|AssumeTypeAssumptions.__default.f#canCall| |path#0@0|)) (and (|AssumeTypeAssumptions.__default.f#canCall| |path#0@0|) (AssumeTypeAssumptions.__default.f |path#0@0|))) (and (=> (= (ControlFlow 0 5) 3) anon3_Then_correct) (=> (= (ControlFlow 0 5) 4) anon3_Else_correct)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is SeqType |path#0| (TSeq TInt)) ($IsAlloc SeqType |path#0| (TSeq TInt) $Heap)) true) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct))))
