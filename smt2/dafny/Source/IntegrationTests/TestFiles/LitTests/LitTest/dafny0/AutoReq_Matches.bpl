...
procedure {:verboseName "Matches.f1 (well-formedness)"} CheckWellformed$$Matches.__default.f1(x#0: int);
         ...
         || (1 < $FunctionContextHeight
          ...
          $Is(t#0, Tclass.Matches.TheType())
           && (if Matches.TheType.TheType__builder_q(t#0)
             ...
             else (var t#2 := Matches.TheType.t(t#0); Lit(true))))
       ==> (Matches.TheType.TheType__builder_q(t#0)
         ...
         && Matches.__default.basic__test(t#0)
           == (if Matches.TheType.TheType__builder_q(t#0)
             ...
             else (var t#3 := Matches.TheType.t(t#0); Lit(true))));
         ...
         || (1 < $FunctionContextHeight
          ...
          $Is(t#0, Tclass.Matches.TheType())
           && (if Matches.TheType.TheType__builder_q(Lit(t#0))
             ...
             else (var t#5 := Lit(Matches.TheType.t(Lit(t#0))); Lit(true))))
       ==> (Lit(Matches.TheType.TheType__builder_q(Lit(t#0)))
         ...
         && Matches.__default.basic__test(Lit(t#0))
           == (if Matches.TheType.TheType__builder_q(Lit(t#0))
             ...
             else (var t#6 := Lit(Matches.TheType.t(Lit(t#0))); Lit(true))));
}
...
procedure {:verboseName "Matches.basic_test (well-formedness)"} CheckWellformed$$Matches.__default.basic__test(t#0: DatatypeType where $Is(t#0, Tclass.Matches.TheType()));
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Matches.basic_test (well-formedness)"} CheckWellformed$$Matches.__default.basic__test(t#0: DatatypeType)
{
  ...
  var ##x#0: int;
    ...
    {
        havoc x#Z#0;
        assume {:id "id204"} let#1#0#0 == _mcc#0#0;
        assume true;
        // CheckWellformedWithResult: any expression
        assume $Is(let#1#0#0, TInt);
        assume {:id "id205"} x#Z#0 == let#1#0#0;
    }
    else if (t#0 == #Matches.TheType.TheType_copier(_mcc#1#0))
    {
        ...
        assume true;
        // CheckWellformedWithResult: any expression
        assume $Is(let#0#0#0, Tclass.Matches.TheType());
        assume {:id "id203"} t#Z#0 == let#0#0#0;
    }
    else
    {
        assume false;
    ...
    assume {:id "id206"} (if Matches.TheType.TheType__builder_q(t#0)
       then (var x#7 := Matches.TheType.x(t#0); x#7 > 3)
       else (var t#8 := Matches.TheType.t(t#0); Lit(true)));
    ...
    else
    {
        ...
        if (t#0 == #Matches.TheType.TheType_builder(_mcc#2#0))
        {
            havoc x#Z#1;
            assume {:id "id210"} let#3#0#0 == _mcc#2#0;
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(let#3#0#0, TInt);
            assume {:id "id211"} x#Z#1 == let#3#0#0;
            ##x#0 := x#Z#1;
            // assume allocatedness for argument to function
            assume $IsAlloc(##x#0, TInt, $Heap);
            assert {:id "id212"} {:subsumption 0} ##x#0 > 3;
            assume ##x#0 > 3;
            assume Matches.__default.f1#canCall(x#Z#1);
            assume {:id "id213"} Matches.__default.basic__test(t#0) == Matches.__default.f1(x#Z#1);
            assume Matches.__default.f1#canCall(x#Z#1);
            // CheckWellformedWithResult: any expression
            assume $Is(Matches.__default.basic__test(t#0), TBool);
            return;
        }
        else if (t#0 == #Matches.TheType.TheType_copier(_mcc#3#0))
        {
            assume $Is(_mcc#3#0, Tclass.Matches.TheType());
            havoc t#Z#1;
            assume {:id "id207"} let#2#0#0 == _mcc#3#0;
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(let#2#0#0, Tclass.Matches.TheType());
            assume {:id "id208"} t#Z#1 == let#2#0#0;
            assume {:id "id209"} Matches.__default.basic__test(t#0) == Lit(true);
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(Matches.__default.basic__test(t#0), TBool);
            return;
