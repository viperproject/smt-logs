...
(set-info :boogie-vc-id CheckWellformed$$_module.__default.build__rec_split1)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon28_Then_correct  (=> (and (not (|Seq#Equal| |s#0| |Seq#Empty|)) (= (ControlFlow 0 2) (- 0 1))) (and (<= 0 (LitInt 0)) (< (LitInt 0) (|Seq#Length| |s#0|))))))
(let ((anon24_Else_correct  (=> (not (|Seq#Equal| |s#0| |Seq#Empty|)) (=> (and (and (<= 0 (LitInt 0)) (< (LitInt 0) (|Seq#Length| |s#0|))) (= (ControlFlow 0 4) 2)) anon28_Then_correct))))
(let ((anon24_Then_correct  (=> (and (|Seq#Equal| |s#0| |Seq#Empty|) (= (ControlFlow 0 3) 2)) anon28_Then_correct)))
(let ((anon0_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) ($Is SeqType |s#0| (TSeq TInt))) (and (= 2 $FunctionContextHeight) (= $_ReadsFrame@0 (|lambda#0| null $Heap alloc false)))) (and (=> (= (ControlFlow 0 5) 3) anon24_Then_correct) (=> (= (ControlFlow 0 5) 4) anon24_Else_correct)))))
anon0_correct)))))
))
(check-sat)
(get-info :rlimit)
