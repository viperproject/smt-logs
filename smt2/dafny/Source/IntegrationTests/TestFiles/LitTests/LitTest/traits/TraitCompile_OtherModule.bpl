...
axiom {:id "id0"} 0 <= $FunctionContextHeight
   ==> (forall x#0: int :: 
    { OtherModule.Y.F(x#0) } 
    OtherModule.Y.F#canCall(x#0) || 0 < $FunctionContextHeight
...
axiom {:id "id1"} 0 <= $FunctionContextHeight
   ==> (forall x#0: int :: 
    ...
    OtherModule.Y.F#canCall(LitInt(x#0)) || 0 < $FunctionContextHeight
...
procedure {:verboseName "OtherModule.Y.F (well-formedness)"} CheckWellformed$$OtherModule.Y.F(x#0: int);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "OtherModule.Y.F (well-formedness)"} CheckWellformed$$OtherModule.Y.F(x#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    else
    {
        ...
        assume {:id "id3"} OtherModule.Y.F(x#0) == Div(x#0, LitInt(2));
        assume true;
        // CheckWellformedWithResult: any expression
        assume $Is(OtherModule.Y.F(x#0), TInt);
        return;
...
procedure {:verboseName "OtherModule.Y.P (correctness)"} Impl$$OtherModule.Y.P(t#0: real) returns (f#0: bool, $_reverifyPost: bool);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
