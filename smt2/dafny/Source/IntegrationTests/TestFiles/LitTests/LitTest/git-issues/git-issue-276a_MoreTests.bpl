...
function MoreTests.__default.bv() : bv19
uses {
axiom MoreTests.__default.bv(): bv19 == Lit(203bv19);
// _default.bv: Type axiom
axiom 0 < $FunctionContextHeight ==> $Is(MoreTests.__default.bv(), TBitvector(19));
}

procedure {:verboseName "MoreTests.bv (well-formedness)"} CheckWellFormed$$MoreTests.__default.bv();
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



// _default.bv: Allocation axiom
axiom 0 < $FunctionContextHeight
   ==> (forall $h: Heap :: 
    { $IsAlloc(MoreTests.__default.bv(), TBitvector(19), $h) } 
    $IsGoodHeap($h) ==> $IsAlloc(MoreTests.__default.bv(), TBitvector(19), $h));

procedure {:verboseName "MoreTests.EmptyFitsIntoUint8 (well-formedness)"} CheckWellFormed$$MoreTests.EmptyFitsIntoUint8(i#0: int);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



function Tclass.MoreTests.EmptyFitsIntoUint8() : Ty
uses {
// Tclass.MoreTests.EmptyFitsIntoUint8 Tag
axiom Tag(Tclass.MoreTests.EmptyFitsIntoUint8())
     == Tagclass.MoreTests.EmptyFitsIntoUint8
   && TagFamily(Tclass.MoreTests.EmptyFitsIntoUint8())
     == tytagFamily$EmptyFitsIntoUint8;
}

const unique Tagclass.MoreTests.EmptyFitsIntoUint8: TyTag;

// Box/unbox axiom for Tclass.MoreTests.EmptyFitsIntoUint8
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.MoreTests.EmptyFitsIntoUint8()) } 
  $IsBox(bx, Tclass.MoreTests.EmptyFitsIntoUint8())
     ==> $Box($Unbox(bx): int) == bx
       && $Is($Unbox(bx): int, Tclass.MoreTests.EmptyFitsIntoUint8()));

// $Is axiom for newtype MoreTests.EmptyFitsIntoUint8
axiom (forall i#0: int :: 
  { $Is(i#0, Tclass.MoreTests.EmptyFitsIntoUint8()) } 
  $Is(i#0, Tclass.MoreTests.EmptyFitsIntoUint8())
     <==> LitInt(-2147483648) <= i#0 && i#0 < (if !Lit(true) then 128 else -4294967296));

// $IsAlloc axiom for newtype MoreTests.EmptyFitsIntoUint8
axiom (forall i#0: int, $h: Heap :: 
  { $IsAlloc(i#0, Tclass.MoreTests.EmptyFitsIntoUint8(), $h) } 
  $IsAlloc(i#0, Tclass.MoreTests.EmptyFitsIntoUint8(), $h));

const unique class.MoreTests.EmptyFitsIntoUint8: ClassName;

procedure {:verboseName "MoreTests.int8 (well-formedness)"} CheckWellFormed$$MoreTests.int8(i#0: int);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "MoreTests.int8 (well-formedness)"} CheckWellFormed$$MoreTests.int8(i#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    if (*)
    {
        ...
        assume {:id "id110"} LitInt(-128) <= i#0 && i#0 < (if !Lit(true) then -200 else 128);
        assume false;
    }
    ...
    if (*)
    {
        ...
        assume true;
        assert {:id "id111"} {:subsumption 0} LitInt(-128) <= LitInt(0);
        assert {:id "id112"} {:subsumption 0} 0 < (if !Lit(true) then -200 else 128);
        assume false;
    }
}
...
function Tclass.MoreTests.int8() : Ty
uses {
// Tclass.MoreTests.int8 Tag
axiom Tag(Tclass.MoreTests.int8()) == Tagclass.MoreTests.int8
   && TagFamily(Tclass.MoreTests.int8()) == tytagFamily$int8;
}

const unique Tagclass.MoreTests.int8: TyTag;

// Box/unbox axiom for Tclass.MoreTests.int8
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.MoreTests.int8()) } 
  $IsBox(bx, Tclass.MoreTests.int8())
     ==> $Box($Unbox(bx): int) == bx && $Is($Unbox(bx): int, Tclass.MoreTests.int8()));

// $Is axiom for newtype MoreTests.int8
axiom (forall i#0: int :: 
  { $Is(i#0, Tclass.MoreTests.int8()) } 
  $Is(i#0, Tclass.MoreTests.int8())
     <==> LitInt(-128) <= i#0 && i#0 < (if !Lit(true) then -200 else 128));

// $IsAlloc axiom for newtype MoreTests.int8
axiom (forall i#0: int, $h: Heap :: 
  { $IsAlloc(i#0, Tclass.MoreTests.int8(), $h) } 
  $IsAlloc(i#0, Tclass.MoreTests.int8(), $h));

const unique class.MoreTests.int8: ClassName;

procedure {:verboseName "MoreTests.AnotherInt8 (well-formedness)"} CheckWellFormed$$MoreTests.AnotherInt8(i#0: int);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "MoreTests.AnotherInt8 (well-formedness)"} CheckWellFormed$$MoreTests.AnotherInt8(i#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;


    // AddWellformednessCheck for newtype AnotherInt8
    $_ReadsFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    if (*)
    {
        ...
        if (LitInt(-128) <= i#0)
        {
            ...
            if (true ==> MoreTests.__default.bv() == Lit(203bv19))
            {
            }
            else
            {
        ...
        assume {:id "id113"} LitInt(-128) <= i#0
           && i#0 < (if true ==> MoreTests.__default.bv() == Lit(203bv19) then 128 else -200);
        assume false;
    }
    ...
    if (*)
    {
        // check well-formedness of newtype witness, and that it satisfies the constraint
        assume true;
        assert {:id "id114"} {:subsumption 0} LitInt(-128) <= LitInt(0);
        assert {:id "id115"} {:subsumption 0} 0 < (if true ==> MoreTests.__default.bv() == Lit(203bv19) then 128 else -200);
        assume false;
    }
}
...
function Tclass.MoreTests.AnotherInt8() : Ty
uses {
// Tclass.MoreTests.AnotherInt8 Tag
axiom Tag(Tclass.MoreTests.AnotherInt8()) == Tagclass.MoreTests.AnotherInt8
   && TagFamily(Tclass.MoreTests.AnotherInt8()) == tytagFamily$AnotherInt8;
}

const unique Tagclass.MoreTests.AnotherInt8: TyTag;

// Box/unbox axiom for Tclass.MoreTests.AnotherInt8
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.MoreTests.AnotherInt8()) } 
  $IsBox(bx, Tclass.MoreTests.AnotherInt8())
     ==> $Box($Unbox(bx): int) == bx
       && $Is($Unbox(bx): int, Tclass.MoreTests.AnotherInt8()));

// $Is axiom for newtype MoreTests.AnotherInt8
axiom (forall i#0: int :: 
  { $Is(i#0, Tclass.MoreTests.AnotherInt8()) } 
  $Is(i#0, Tclass.MoreTests.AnotherInt8())
     <==> LitInt(-128) <= i#0
       && i#0 < (if true ==> MoreTests.__default.bv() == Lit(203bv19) then 128 else -200));

// $IsAlloc axiom for newtype MoreTests.AnotherInt8
axiom (forall i#0: int, $h: Heap :: 
  { $IsAlloc(i#0, Tclass.MoreTests.AnotherInt8(), $h) } 
  $IsAlloc(i#0, Tclass.MoreTests.AnotherInt8(), $h));

const unique class.MoreTests.AnotherInt8: ClassName;

procedure {:verboseName "MoreTests.Int16 (well-formedness)"} CheckWellFormed$$MoreTests.Int16(i#0: int);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "MoreTests.Int16 (well-formedness)"} CheckWellFormed$$MoreTests.Int16(i#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    if (*)
    {
        ...
        if (LitInt(-128) <= i#0)
        {
            ...
            if (true ==> MoreTests.__default.bv() == Lit(204bv19))
            {
            }
            else
            {
        ...
        assume {:id "id116"} LitInt(-128) <= i#0
           && i#0 < (if true ==> MoreTests.__default.bv() == Lit(204bv19) then 128 else 1234);
        assume false;
    }
    ...
    if (*)
    {
        // check well-formedness of newtype witness, and that it satisfies the constraint
        assume true;
        assert {:id "id117"} {:subsumption 0} LitInt(-128) <= LitInt(0);
        assert {:id "id118"} {:subsumption 0} 0 < (if true ==> MoreTests.__default.bv() == Lit(204bv19) then 128 else 1234);
        assume false;
    }
}
...
function Tclass.MoreTests.Int16() : Ty
uses {
// Tclass.MoreTests.Int16 Tag
axiom Tag(Tclass.MoreTests.Int16()) == Tagclass.MoreTests.Int16
   && TagFamily(Tclass.MoreTests.Int16()) == tytagFamily$Int16;
}

const unique Tagclass.MoreTests.Int16: TyTag;

// Box/unbox axiom for Tclass.MoreTests.Int16
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.MoreTests.Int16()) } 
  $IsBox(bx, Tclass.MoreTests.Int16())
     ==> $Box($Unbox(bx): int) == bx && $Is($Unbox(bx): int, Tclass.MoreTests.Int16()));

// $Is axiom for newtype MoreTests.Int16
axiom (forall i#0: int :: 
  { $Is(i#0, Tclass.MoreTests.Int16()) } 
  $Is(i#0, Tclass.MoreTests.Int16())
     <==> LitInt(-128) <= i#0
       && i#0 < (if true ==> MoreTests.__default.bv() == Lit(204bv19) then 128 else 1234));

// $IsAlloc axiom for newtype MoreTests.Int16
axiom (forall i#0: int, $h: Heap :: 
  { $IsAlloc(i#0, Tclass.MoreTests.Int16(), $h) } 
  $IsAlloc(i#0, Tclass.MoreTests.Int16(), $h));

const unique class.MoreTests.Int16: ClassName;
...
const unique tytagFamily$EmptyFitsIntoUint8: TyTagFamily;

const unique tytagFamily$int8: TyTagFamily;

const unique tytagFamily$AnotherInt8: TyTagFamily;
