...
(declare-fun Tagclass.StringsOfLength2.ShortString () T@U)
...
(declare-fun Tclass.StringsOfLength2.ShortString () T@U)
...
(assert (distinct TChar TagChar TagSeq alloc Tagclass.StringsOfLength2.ShortString tytagFamily$ShortString)
)
(assert (= (Tag TChar) TagChar))
(assert (= (Ctor SeqType) 3))
(assert (forall ((|s#0| T@U) ($h T@U) ) (! (= ($IsAlloc SeqType |s#0| Tclass.StringsOfLength2.ShortString $h) ($IsAlloc SeqType |s#0| (TSeq TChar) $h))
 :qid |unknown.0:0|
 :skolemid |972|
 :pattern ( ($IsAlloc SeqType |s#0| Tclass.StringsOfLength2.ShortString $h))
)))
...
(assert (forall ((bx@@0 T@U) ) (!  (=> ($IsBox bx@@0 Tclass.StringsOfLength2.ShortString) (and (= ($Box SeqType ($Unbox SeqType bx@@0)) bx@@0) ($Is SeqType ($Unbox SeqType bx@@0) Tclass.StringsOfLength2.ShortString)))
 :qid |unknown.0:0|
 :skolemid |970|
 :pattern ( ($IsBox bx@@0 Tclass.StringsOfLength2.ShortString))
)))
...
(assert (forall ((|s#0@@0| T@U) ) (! (= ($Is SeqType |s#0@@0| Tclass.StringsOfLength2.ShortString)  (and ($Is SeqType |s#0@@0| (TSeq TChar)) (<= (|Seq#Length| |s#0@@0|) (LitInt 2))))
 :qid |unknown.0:0|
 :skolemid |971|
 :pattern ( ($Is SeqType |s#0@@0| Tclass.StringsOfLength2.ShortString))
)))
...
(assert (= (Tag Tclass.StringsOfLength2.ShortString) Tagclass.StringsOfLength2.ShortString))
(assert (= (TagFamily Tclass.StringsOfLength2.ShortString) tytagFamily$ShortString))
...
(set-info :boogie-vc-id Impl$$StringsOfLength2.__default.M)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) ($Is SeqType (|Seq#Append| (|Seq#Build| (|Seq#Build| |Seq#Empty| ($Box charType (|char#FromInt| 49))) ($Box charType (|char#FromInt| 50))) (|Seq#Build| (|Seq#Build| |Seq#Empty| ($Box charType (|char#FromInt| 51))) ($Box charType (|char#FromInt| 52)))) Tclass.StringsOfLength2.ShortString))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is SeqType |x2#0| Tclass.StringsOfLength2.ShortString) ($IsAlloc SeqType |x2#0| Tclass.StringsOfLength2.ShortString $Heap)) true) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
