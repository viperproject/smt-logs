...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($ly T@U) (|n#0| T@U) ) (!  (=> (or (|_module.__default.ExternalToInt#canCall| (Lit DatatypeTypeType |n#0|)) (and (< 1 $FunctionContextHeight) ($Is DatatypeTypeType |n#0| Tclass._module.Nat))) (and (=> (not (U_2_bool (Lit boolType (bool_2_U (_module.Nat.Zero_q (Lit DatatypeTypeType |n#0|)))))) (let ((|p#3| (Lit DatatypeTypeType (_module.Nat.pred (Lit DatatypeTypeType |n#0|)))))
(|_module.__default.ExternalToInt#canCall| |p#3|))) (= (_module.__default.ExternalToInt ($LS $ly) (Lit DatatypeTypeType |n#0|)) (ite (_module.Nat.Zero_q (Lit DatatypeTypeType |n#0|)) 0 (let ((|p#2| (Lit DatatypeTypeType (_module.Nat.pred (Lit DatatypeTypeType |n#0|)))))
(LitInt (+ 1 (_module.__default.ExternalToInt ($LS $ly) |p#2|))))))))
 :qid |gitissue1143dfy.13:10|
 :weight 3
 :skolemid |534|
 :pattern ( (_module.__default.ExternalToInt ($LS $ly) (Lit DatatypeTypeType |n#0|)))
))))
...
(assert (forall (($ly@@0 T@U) (|n#0@@0| T@U) ) (! (= (_module.__default.ExternalToInt ($LS $ly@@0) |n#0@@0|) (_module.__default.ExternalToInt $ly@@0 |n#0@@0|))
 :qid |gitissue1143dfy.13:10|
 :skolemid |528|
 :pattern ( (_module.__default.ExternalToInt ($LS $ly@@0) |n#0@@0|))
)))
...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($ly@@1 T@U) (|n#0@@1| T@U) ) (!  (=> (or (|_module.__default.ExternalToInt#canCall| |n#0@@1|) (and (< 1 $FunctionContextHeight) ($Is DatatypeTypeType |n#0@@1| Tclass._module.Nat))) (and (=> (not (_module.Nat.Zero_q |n#0@@1|)) (let ((|p#1| (_module.Nat.pred |n#0@@1|)))
(|_module.__default.ExternalToInt#canCall| |p#1|))) (= (_module.__default.ExternalToInt ($LS $ly@@1) |n#0@@1|) (ite (_module.Nat.Zero_q |n#0@@1|) 0 (let ((|p#0| (_module.Nat.pred |n#0@@1|)))
(+ 1 (_module.__default.ExternalToInt $ly@@1 |p#0|)))))))
 :qid |gitissue1143dfy.13:10|
 :skolemid |533|
 :pattern ( (_module.__default.ExternalToInt ($LS $ly@@1) |n#0@@1|))
))))
...
(set-info :boogie-vc-id CheckWellformed$$_module.__default.ExternalToInt)
...
(let ((anon10_Then_correct  (=> (= |n#0@@2| (|#_module.Nat.Succ| |_mcc#0#0|)) (=> (and ($Is DatatypeTypeType |_mcc#0#0| Tclass._module.Nat) (= |let#0#0#0| |_mcc#0#0|)) (=> (and (and ($Is DatatypeTypeType |let#0#0#0| Tclass._module.Nat) (= |p#Z#0@0| |let#0#0#0|)) (and ($IsAlloc DatatypeTypeType |p#Z#0@0| Tclass._module.Nat $Heap) (= (ControlFlow 0 4) (- 0 3)))) (< (DtRank |p#Z#0@0|) (DtRank |n#0@@2|)))))))
(let ((anon9_Else_correct  (=> (or (not (= |n#0@@2| |#_module.Nat.Zero|)) (not true)) (and (=> (= (ControlFlow 0 6) 4) anon10_Then_correct) (=> (= (ControlFlow 0 6) 5) anon10_Else_correct)))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) ($Is DatatypeTypeType |n#0@@2| Tclass._module.Nat)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 8) 7))) anon0_correct)))
