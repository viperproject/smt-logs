...
(declare-fun Tagclass.M1.MyClass () T@U)
(declare-fun Tagclass.M1.MyClass? () T@U)
...
(declare-fun Tclass.M1.MyClass? () T@U)
...
(assert (distinct TBool TagBool alloc Tagclass._System.___hFunc1 Tagclass.M1.MyClass Tagclass.M1.MyClass? |tytagFamily$_#Func1| tytagFamily$MyClass)
)
...
(assert (forall (($o T@U) ) (! (= ($Is refType $o Tclass.M1.MyClass?)  (or (= $o null) (= (dtype $o) Tclass.M1.MyClass?)))
 :qid |unknown.0:0|
 :skolemid |533|
 :pattern ( ($Is refType $o Tclass.M1.MyClass?))
)))
...
(assert (forall ((|c#0| T@U) ($h T@U) ) (! (= ($IsAlloc refType |c#0| Tclass.M1.MyClass $h) ($IsAlloc refType |c#0| Tclass.M1.MyClass? $h))
 :qid |unknown.0:0|
 :skolemid |536|
 :pattern ( ($IsAlloc refType |c#0| Tclass.M1.MyClass $h))
 :pattern ( ($IsAlloc refType |c#0| Tclass.M1.MyClass? $h))
)))
...
(assert (forall (($o@@0 T@U) ($h@@0 T@U) ) (! (= ($IsAlloc refType $o@@0 Tclass.M1.MyClass? $h@@0)  (or (= $o@@0 null) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@0 $o@@0) alloc)))))
 :qid |unknown.0:0|
 :skolemid |534|
 :pattern ( ($IsAlloc refType $o@@0 Tclass.M1.MyClass? $h@@0))
)))
...
(assert (forall ((bx@@1 T@U) ) (!  (=> ($IsBox bx@@1 Tclass.M1.MyClass) (and (= ($Box refType ($Unbox refType bx@@1)) bx@@1) ($Is refType ($Unbox refType bx@@1) Tclass.M1.MyClass)))
 :qid |unknown.0:0|
 :skolemid |528|
 :pattern ( ($IsBox bx@@1 Tclass.M1.MyClass))
)))
(assert (forall ((bx@@2 T@U) ) (!  (=> ($IsBox bx@@2 Tclass.M1.MyClass?) (and (= ($Box refType ($Unbox refType bx@@2)) bx@@2) ($Is refType ($Unbox refType bx@@2) Tclass.M1.MyClass?)))
 :qid |unknown.0:0|
 :skolemid |532|
 :pattern ( ($IsBox bx@@2 Tclass.M1.MyClass?))
)))
(assert (forall ((|c#0@@0| T@U) ) (! (= ($Is refType |c#0@@0| Tclass.M1.MyClass)  (and ($Is refType |c#0@@0| Tclass.M1.MyClass?) (or (not (= |c#0@@0| null)) (not true))))
 :qid |unknown.0:0|
 :skolemid |535|
 :pattern ( ($Is refType |c#0@@0| Tclass.M1.MyClass))
 :pattern ( ($Is refType |c#0@@0| Tclass.M1.MyClass?))
)))
...
(assert (= (Tag Tclass.M1.MyClass) Tagclass.M1.MyClass))
(assert (= (TagFamily Tclass.M1.MyClass) tytagFamily$MyClass))
(assert (= (Tag Tclass.M1.MyClass?) Tagclass.M1.MyClass?))
(assert (= (TagFamily Tclass.M1.MyClass?) tytagFamily$MyClass))
...
(set-info :boogie-vc-id CheckWellFormed$$M1.__default.M)
...
(let ((anon5_Else_correct  (=> (and (=> (Requires1 Tclass.M1.MyClass TBool $Heap |f#0| ($Box refType |t#0@0|)) (U_2_bool ($Unbox boolType (Apply1 Tclass.M1.MyClass TBool $Heap |f#0| ($Box refType |t#0@0|))))) (= (ControlFlow 0 4) 1)) anon4_correct)))
(let ((anon5_Then_correct  (=> ($IsAllocBox ($Box HandleTypeType |f#0|) (Tclass._System.___hFunc1 Tclass.M1.MyClass TBool) $Heap) (=> (and ($IsAlloc refType |t#0@0| Tclass.M1.MyClass $Heap) (Requires1 Tclass.M1.MyClass TBool $Heap |f#0| ($Box refType |t#0@0|))) (and (=> (= (ControlFlow 0 2) (- 0 3)) (Requires1 Tclass.M1.MyClass TBool $Heap |f#0| ($Box refType |t#0@0|))) (=> (Requires1 Tclass.M1.MyClass TBool $Heap |f#0| ($Box refType |t#0@0|)) (=> (and (U_2_bool ($Unbox boolType (Apply1 Tclass.M1.MyClass TBool $Heap |f#0| ($Box refType |t#0@0|)))) (= (ControlFlow 0 2) 1)) anon4_correct)))))))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and ($Is refType |t#0@0| Tclass.M1.MyClass) ($IsAlloc refType |t#0@0| Tclass.M1.MyClass $Heap)) (and (=> (= (ControlFlow 0 5) 2) anon5_Then_correct) (=> (= (ControlFlow 0 5) 4) anon5_Else_correct))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is HandleTypeType |f#0| (Tclass._System.___hFunc1 Tclass.M1.MyClass TBool)) ($IsAlloc HandleTypeType |f#0| (Tclass._System.___hFunc1 Tclass.M1.MyClass TBool) $Heap)) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct))))
