...
(assert (distinct TInt TagInt TagSeq alloc Tagclass._module.A tytagFamily$A)
)
(assert (= (Tag TInt) TagInt))
(assert (= (Ctor SeqType) 3))
(assert (forall ((|s#0| T@U) ($h T@U) ) (! (= ($IsAlloc SeqType |s#0| Tclass._module.A $h) ($IsAlloc SeqType |s#0| (TSeq TInt) $h))
 :qid |unknown.0:0|
 :skolemid |484|
 :pattern ( ($IsAlloc SeqType |s#0| Tclass._module.A $h))
)))
...
(assert (forall ((|s#0@@0| T@U) ) (! (= ($Is SeqType |s#0@@0| Tclass._module.A)  (and ($Is SeqType |s#0@@0| (TSeq TInt)) (< (|Seq#Length| |s#0@@0|) 10)))
 :qid |unknown.0:0|
 :skolemid |483|
 :pattern ( ($Is SeqType |s#0@@0| Tclass._module.A))
)))
(assert (forall ((bx T@U) ) (!  (=> ($IsBox bx Tclass._module.A) (and (= ($Box SeqType ($Unbox SeqType bx)) bx) ($Is SeqType ($Unbox SeqType bx) Tclass._module.A)))
 :qid |unknown.0:0|
 :skolemid |479|
 :pattern ( ($IsBox bx Tclass._module.A))
)))
(assert (forall ((s@@0 T@U) (x@@3 T@U) ) (! (= (exists ((i Int) ) (!  (and (and (<= 0 i) (< i (|Seq#Length| s@@0))) (= x@@3 (|Seq#Index| s@@0 i)))
 :qid |DafnyPreludebpl.1108:11|
 :skolemid |219|
 :pattern ( (|Seq#Index| s@@0 i))
)) (< 0 (|MultiSet#Multiplicity| (|MultiSet#FromSeq| s@@0) x@@3)))
 :qid |DafnyPreludebpl.1106:15|
 :skolemid |220|
 :pattern ( (|MultiSet#Multiplicity| (|MultiSet#FromSeq| s@@0) x@@3))
)))
...
(assert (forall ((s@@3 T@U) ) (!  (and (= (= (|MultiSet#Card| s@@3) 0) (= s@@3 |MultiSet#Empty|)) (=> (or (not (= (|MultiSet#Card| s@@3) 0)) (not true)) (exists ((x@@5 T@U) ) (! (< 0 (|MultiSet#Multiplicity| s@@3 x@@5))
 :qid |DafnyPreludebpl.946:20|
 :skolemid |188|
 :pattern ( (|MultiSet#Multiplicity| s@@3 x@@5))
))))
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (|MultiSet#Equal| (|MultiSet#FromSeq| |a#0|) (|MultiSet#FromSeq| |a#0|)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is SeqType |a#0| (TSeq Tclass._module.A)) ($IsAlloc SeqType |a#0| (TSeq Tclass._module.A) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
