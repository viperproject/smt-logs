...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon3_Else_correct  (=> (and (not (_module.Status.IsFailure |call2formal@r#0@0|)) (= (ControlFlow 0 3) (- 0 2))) (U_2_bool (Lit boolType (bool_2_U false))))))
(let ((anon3_Then_correct true))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (and (and ($Is DatatypeTypeType |valueOrError0#0@0| Tclass._module.Status) ($IsAlloc DatatypeTypeType |valueOrError0#0@0| Tclass._module.Status $Heap)) (= |i##0@0| (LitInt 2))) (and (and ($Is DatatypeTypeType |call2formal@r#0| Tclass._module.Status) ($IsAlloc DatatypeTypeType |call2formal@r#0| Tclass._module.Status $Heap)) (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)))) (=> (and (and (and (and ($Is DatatypeTypeType |call2formal@r#0@0| Tclass._module.Status) ($IsAlloc DatatypeTypeType |call2formal@r#0@0| Tclass._module.Status $Heap@0)) (=> (= |i##0@0| (LitInt 1)) (not (_module.Status.Failure_q |call2formal@r#0@0|)))) (and (=> (= |i##0@0| (LitInt 2)) (_module.Status.Failure_q |call2formal@r#0@0|)) (forall (($o@@0 T@U) ) (!  (=> (and (or (not (= $o@@0 null)) (not true)) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap $o@@0) alloc)))) (= (MapType0Select refType (MapType0Type FieldType BoxType) $Heap@0 $o@@0) (MapType0Select refType (MapType0Type FieldType BoxType) $Heap $o@@0)))
 :qid |gitissue936dfy.10:8|
 :skolemid |481|
 :pattern ( (MapType0Select refType (MapType0Type FieldType BoxType) $Heap@0 $o@@0))
)))) (and (and ($HeapSucc $Heap $Heap@0) ($IsAllocBox ($Box DatatypeTypeType |call2formal@r#0@0|) Tclass._module.Status $Heap@0)) (and (|_module.Status.IsFailure#canCall| |call2formal@r#0@0|) (|_module.Status.IsFailure#canCall| |call2formal@r#0@0|)))) (and (=> (= (ControlFlow 0 4) 1) anon3_Then_correct) (=> (= (ControlFlow 0 4) 3) anon3_Else_correct)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is DatatypeTypeType |valueOrError0#0| Tclass._module.Status) ($IsAlloc DatatypeTypeType |valueOrError0#0| Tclass._module.Status $Heap)) true) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 5) 4))) anon0_correct))))
