...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (_module.Dt.Dt_q |e#0|) (= (ControlFlow 0 2) (- 0 1))) (or (not (= ($Unbox refType (_module.Dt.u |e#0|)) null)) (not true))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is DatatypeTypeType |e#0| (Tclass._module.Dt Tclass._module.Cl)) ($IsAlloc DatatypeTypeType |e#0| (Tclass._module.Dt Tclass._module.Cl) $Heap)) (|$IsA#_module.Dt| |e#0|)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
