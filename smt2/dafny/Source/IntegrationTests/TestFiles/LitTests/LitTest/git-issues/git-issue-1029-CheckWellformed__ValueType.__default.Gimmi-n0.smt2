...
(assert (distinct TBool TagBool TagSeq alloc Tagclass.ValueType.Value tytagFamily$Value)
)
(assert (= (Tag TBool) TagBool))
(assert (= (|Seq#Length| |Seq#Empty|) 0))
(assert (= (Ctor SeqType) 3))
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |ValueType.__default.Gimmie#canCall| (< 1 $FunctionContextHeight)) ($Is SeqType ValueType.__default.Gimmie Tclass.ValueType.Value))))
...
(assert (forall ((bx@@0 T@U) ) (!  (=> ($IsBox bx@@0 Tclass.ValueType.Value) (and (= ($Box SeqType ($Unbox SeqType bx@@0)) bx@@0) ($Is SeqType ($Unbox SeqType bx@@0) Tclass.ValueType.Value)))
 :qid |unknown.0:0|
 :skolemid |479|
 :pattern ( ($IsBox bx@@0 Tclass.ValueType.Value))
)))
...
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |ValueType.__default.Gimmie#canCall| (< 1 $FunctionContextHeight)) (= ValueType.__default.Gimmie (Lit SeqType (|Seq#Build| (|Seq#Build| (|Seq#Build| |Seq#Empty| ($Box boolType (Lit boolType (bool_2_U true)))) ($Box boolType (Lit boolType (bool_2_U true)))) ($Box boolType (Lit boolType (bool_2_U false)))))))))
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |ValueType.__default.Gimmie#canCall| (< 1 $FunctionContextHeight)) (= ValueType.__default.Gimmie (Lit SeqType (|Seq#Build| (|Seq#Build| (|Seq#Build| |Seq#Empty| ($Box boolType (Lit boolType (bool_2_U true)))) ($Box boolType (Lit boolType (bool_2_U true)))) ($Box boolType (Lit boolType (bool_2_U false)))))))))
(assert (forall ((|s#0| T@U) ) (! (= ($Is SeqType |s#0| Tclass.ValueType.Value)  (and ($Is SeqType |s#0| (TSeq TBool)) (<= (|Seq#Length| |s#0|) (LitInt 10))))
 :qid |unknown.0:0|
 :skolemid |480|
 :pattern ( ($Is SeqType |s#0| Tclass.ValueType.Value))
)))
...
(set-info :boogie-vc-id CheckWellformed$$ValueType.__default.Gimmie)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon5_Else_correct  (=> (= (ControlFlow 0 3) (- 0 2)) ($Is SeqType (Lit SeqType (|Seq#Build| (|Seq#Build| (|Seq#Build| |Seq#Empty| ($Box boolType (Lit boolType (bool_2_U true)))) ($Box boolType (Lit boolType (bool_2_U true)))) ($Box boolType (Lit boolType (bool_2_U false))))) Tclass.ValueType.Value))))
