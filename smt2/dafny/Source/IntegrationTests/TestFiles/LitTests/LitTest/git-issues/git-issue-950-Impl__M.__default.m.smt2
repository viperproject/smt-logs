...
(assert  (=> (< 0 $FunctionContextHeight) ($Is intType (int_2_U M.__default.x) TInt)))
(assert (forall ((x@@5 T@U) (T@@1 T@T) ) (! (= ($Unbox T@@1 ($Box T@@1 x@@5)) x@@5)
 :qid |DafnyPreludebpl.167:18|
 :skolemid |25|
 :pattern ( ($Box T@@1 x@@5))
)))
(assert (= M.__default.x (LitInt 5)))
...
(set-info :boogie-vc-id Impl$$M.__default.m)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (= M.__default.x (LitInt 5)))))
