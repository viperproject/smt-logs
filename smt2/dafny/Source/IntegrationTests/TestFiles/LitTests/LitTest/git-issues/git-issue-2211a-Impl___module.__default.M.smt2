...
(declare-fun _module.__default.P3 (Int) Bool)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (|_module.__default.Q#canCall| |s#0@@1|)) (or (_module.__default.Q |s#0@@1|) (exists ((|i#3| Int) ) (!  (and (and (<= (LitInt 0) |i#3|) (< |i#3| (|Seq#Length| |s#0@@1|))) (or (_module.__default.P1 (U_2_int ($Unbox intType (|Seq#Index| |s#0@@1| |i#3|)))) (_module.__default.P2 (U_2_int ($Unbox intType (|Seq#Index| |s#0@@1| |i#3|))))))
 :qid |gitissue2211adfy.9:10|
 :skolemid |504|
 :pattern ( ($Unbox intType (|Seq#Index| |s#0@@1| |i#3|)))
))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is SeqType |s#0@@1| (TSeq TInt)) ($IsAlloc SeqType |s#0@@1| (TSeq TInt) $Heap)) (= 2 $FunctionContextHeight)) (and (exists ((|i#1@@1| Int) ) (!  (and (and (<= (LitInt 0) |i#1@@1|) (< |i#1@@1| (|Seq#Length| |s#0@@1|))) (or (or (_module.__default.P1 (U_2_int ($Unbox intType (|Seq#Index| |s#0@@1| |i#1@@1|)))) (_module.__default.P2 (U_2_int ($Unbox intType (|Seq#Index| |s#0@@1| |i#1@@1|))))) (_module.__default.P3 (U_2_int ($Unbox intType (|Seq#Index| |s#0@@1| |i#1@@1|))))))
 :qid |gitissue2211adfy.15:19|
 :skolemid |503|
