...
(declare-fun _module.__default.WITNESS2 () T@U)
(declare-fun $IsGoodHeap (T@U) Bool)
(declare-fun |_module.__default.WITNESS2#canCall| () Bool)
...
(assert  (=> (<= 0 $FunctionContextHeight) (forall (($Heap T@U) ) (!  (=> (or |_module.__default.WITNESS2#canCall| (and (< 0 $FunctionContextHeight) ($IsGoodHeap $Heap))) (= _module.__default.WITNESS2 (|Seq#Create| TInt $Heap (LitInt 10) (Lit HandleTypeType (AtLayer HandleTypeType (|lambda#4| (Handle1 (|lambda#0| ($Box intType (int_2_U (LitInt 0)))) (|lambda#1| TInt) (|lambda#3| (SetRef_to_SetBox (|lambda#2| false))))) ($LS $LZ))))))
 :qid |gitissue405dfy.7:10|
 :skolemid |484|
 :pattern ( _module.__default.WITNESS2 ($IsGoodHeap $Heap))
))))
(assert  (=> (<= 0 $FunctionContextHeight) (forall (($Heap@@0 T@U) ) (!  (=> (or |_module.__default.WITNESS2#canCall| (and (< 0 $FunctionContextHeight) ($IsGoodHeap $Heap@@0))) (= _module.__default.WITNESS2 (|Seq#Create| TInt $Heap@@0 (LitInt 10) (Lit HandleTypeType (AtLayer HandleTypeType (|lambda#4| (Handle1 (|lambda#0| ($Box intType (int_2_U (LitInt 0)))) (|lambda#1| TInt) (|lambda#3| (SetRef_to_SetBox (|lambda#2| false))))) ($LS $LZ))))))
 :qid |gitissue405dfy.7:10|
 :weight 3
 :skolemid |485|
 :pattern ( _module.__default.WITNESS2 ($IsGoodHeap $Heap@@0))
))))
...
(assert  (=> (<= 0 $FunctionContextHeight) (=> (or |_module.__default.WITNESS2#canCall| (< 0 $FunctionContextHeight)) ($Is SeqType _module.__default.WITNESS2 (TSeq TInt)))))
...
(set-info :boogie-vc-id CheckWellFormed$$_module.Seq10a)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon6_Then_correct  (=> |_module.__default.WITNESS2#canCall| (=> (and |_module.__default.WITNESS2#canCall| (= (ControlFlow 0 3) (- 0 2))) (= (|Seq#Length| (Lit SeqType _module.__default.WITNESS2)) (LitInt 10))))))
(let ((anon5_Then_correct true))
(let ((anon0_correct  (=> (= $_ReadsFrame@0 (|lambda#5| null $Heap@@1 alloc false)) (and (=> (= (ControlFlow 0 4) 1) anon5_Then_correct) (=> (= (ControlFlow 0 4) 3) anon6_Then_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and ($IsGoodHeap $Heap@@1) ($IsHeapAnchor $Heap@@1)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 5) 4))) anon0_correct)))
