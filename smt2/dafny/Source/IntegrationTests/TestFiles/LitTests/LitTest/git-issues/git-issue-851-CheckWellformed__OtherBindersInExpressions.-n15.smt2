...
(declare-fun |OtherBindersInExpressions.__default.Forall3#canCall| () Bool)
(declare-fun OtherBindersInExpressions.__default.Forall3 () Int)
...
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |OtherBindersInExpressions.__default.Forall3#canCall| (< 1 $FunctionContextHeight)) (= OtherBindersInExpressions.__default.Forall3 (LitInt (Div 40 (LitInt 0)))))))
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |OtherBindersInExpressions.__default.Forall3#canCall| (< 1 $FunctionContextHeight)) (= OtherBindersInExpressions.__default.Forall3 (LitInt (Div 40 (LitInt 0)))))))
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 8) (let ((anon5_correct  (=> (and (and (= |let#0#0#0| (forall ((|c#3| T@U) )  (=> ($IsBox |c#3| Tclass.OtherBindersInExpressions.C) (= (LitInt (Div 5 (LitInt 0))) (LitInt 19))))) ($Is boolType (bool_2_U |let#0#0#0|) TBool)) (and (= |b#Z#0@0| |let#0#0#0|) (= (ControlFlow 0 3) (- 0 2)))) (or (not (= (LitInt 0) 0)) (not true)))))
