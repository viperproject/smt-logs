...
(declare-fun Tagclass.D.real1 () T@U)
...
(declare-fun Tclass.D.real1 () T@U)
...
(assert (distinct alloc Tagclass.D.real1 tytagFamily$real1)
)
...
(assert (forall ((|x#0| T@U) ) (! (= ($Is realType |x#0| Tclass.D.real1) (U_2_bool (Lit boolType (bool_2_U true))))
 :qid |unknown.0:0|
 :skolemid |1950|
 :pattern ( ($Is realType |x#0| Tclass.D.real1))
)))
...
(assert (= (Tag Tclass.D.real1) Tagclass.D.real1))
(assert (= (TagFamily Tclass.D.real1) tytagFamily$real1))
...
(set-info :boogie-vc-id CheckWellFormed$$D.__default.z)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ReadsFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (= |newtype$check#0@0| (LitReal 0.0)) (= (ControlFlow 0 2) (- 0 1))) ($Is realType (real_2_U (LitReal 0.0)) Tclass.D.real1)))))
