...
(assert (forall ((s@@1 T@U) (n@@1 Int) (j Int) ) (!  (=> (and (and (<= 0 n@@1) (<= 0 j)) (< j (- (|Seq#Length| s@@1) n@@1))) (= (|Seq#Index| (|Seq#Drop| s@@1 n@@1) j) (|Seq#Index| s@@1 (+ j n@@1))))
 :qid |DafnyPreludebpl.1253:15|
 :weight 25
 :skolemid |247|
 :pattern ( (|Seq#Index| (|Seq#Drop| s@@1 n@@1) j))
)))
...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.Workaround)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 6) (let ((anon3_correct  (=> (and (and (< |i#0| |n#0|) (<= |n#0| (|Seq#Length| |s#0|))) (= (ControlFlow 0 2) (- 0 1))) (and (<= 0 |n#0|) (<= |n#0| (|Seq#Length| |s#0|))))))
(let ((anon4_Else_correct  (=> (and (<= |n#0| |i#0|) (= (ControlFlow 0 4) 2)) anon3_correct)))
(let ((anon4_Then_correct  (=> (and (< |i#0| |n#0|) (= (ControlFlow 0 3) 2)) anon3_correct)))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is SeqType |s#0| (TSeq TInt)) ($IsAlloc SeqType |s#0| (TSeq TInt) $Heap))) (and (and (<= (LitInt 0) |i#0|) (<= (LitInt 0) |n#0|)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 6) 5)))) anon0_correct)))
