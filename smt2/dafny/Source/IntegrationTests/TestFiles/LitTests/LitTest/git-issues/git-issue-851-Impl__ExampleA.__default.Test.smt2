...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($Heap T@U) ) (!  (=> (or |ExampleA.__default.BadGhost#canCall| (and (< 1 $FunctionContextHeight) ($IsGoodHeap $Heap))) (and (and |$let#0$canCall| (|ExampleA.C.F#canCall| |$let#0_c|)) (= ExampleA.__default.BadGhost (let ((|c#0| |$let#0_c|))
(ExampleA.C.F |c#0|)))))
...
(assert (= (Tag Tclass.ExampleA.C?) Tagclass.ExampleA.C?))
(assert (= (TagFamily Tclass.ExampleA.C?) tytagFamily$C))
(assert (forall (($o@@0 T@U) ) (! (= ($Is refType $o@@0 Tclass.ExampleA.C?)  (or (= $o@@0 null) (implements$ExampleA.C (dtype $o@@0))))
 :qid |unknown.0:0|
 :skolemid |489|
 :pattern ( ($Is refType $o@@0 Tclass.ExampleA.C?))
)))
...
(set-info :boogie-vc-id Impl$$ExampleA.__default.Test)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap@@1 alloc false)) (=> (and (and |ExampleA.__default.BadGhost#canCall| |ExampleA.__default.BadGhost#canCall|) (and (= |p#0@0| (U_2_bool (Lit boolType (bool_2_U ExampleA.__default.BadGhost)))) (= (ControlFlow 0 2) (- 0 1)))) (or (not (= (LitInt 0) 0)) (not true))))))
