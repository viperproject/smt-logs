...
axiom {:id "id46"} 0 <= $FunctionContextHeight
   ==> (forall s#0: Seq :: 
    ...
    AnotherTest.__default.P#canCall(s#0)
...
axiom {:id "id47"} 0 <= $FunctionContextHeight
   ==> (forall s#0: Seq :: 
    ...
    AnotherTest.__default.P#canCall(Lit(s#0))
         || (0 < $FunctionContextHeight && $Is(s#0, TSeq(TInt)))
...
procedure {:verboseName "AnotherTest.P (well-formedness)"} CheckWellformed$$AnotherTest.__default.P(s#0: Seq where $Is(s#0, TSeq(TInt)));
...
procedure {:verboseName "AnotherTest.Test (correctness)"} Impl$$AnotherTest.__default.Test() returns ($_reverifyPost: bool);
  free requires 2 == $FunctionContextHeight;
  modifies $Heap;
  // frame condition: object granularity
  free ensures (forall $o: ref :: 
...
procedure {:verboseName "AnotherTest.SST (well-formedness)"} CheckWellFormed$$AnotherTest.SST(s#0: Seq);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "AnotherTest.SST (well-formedness)"} CheckWellFormed$$AnotherTest.SST(s#0: Seq)
{
  var $_ReadsFrame: [ref,Field]bool;
  var ##s#0: Seq;


    // AddWellformednessCheck for subset type SST
    $_ReadsFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    if (*)
    {
        // check well-formedness of subset type constraint
        assume $Is(s#0, TSeq(TInt));
        ##s#0 := s#0;
        // assume allocatedness for argument to function
        assume $IsAlloc(##s#0, TSeq(TInt), $Heap);
        assume AnotherTest.__default.P#canCall(s#0);
        assume {:id "id55"} AnotherTest.__default.P(s#0);
        assume false;
    }
    ...
    if (*)
    {
        ...
        assume AnotherTest.__default.P#canCall(Lit(Seq#Empty(): Seq));
        assert {:id "id56"} {:subsumption 0} AnotherTest.__default.P#canCall(Lit(Seq#Empty(): Seq))
           ==> Lit(AnotherTest.__default.P(Lit(Seq#Empty(): Seq)))
             || Seq#Length(Lit(Seq#Empty(): Seq)) <= LitInt(1);
        assume false;
    }
}
