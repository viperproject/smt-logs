...
(declare-fun |OtherBindersInExpressions.__default.MapComprehension1#canCall| () Bool)
(declare-fun OtherBindersInExpressions.__default.MapComprehension1 () Int)
...
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |OtherBindersInExpressions.__default.MapComprehension1#canCall| (< 1 $FunctionContextHeight)) (= OtherBindersInExpressions.__default.MapComprehension1 (LitInt (Div 40 (LitInt 0)))))))
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |OtherBindersInExpressions.__default.MapComprehension1#canCall| (< 1 $FunctionContextHeight)) (= OtherBindersInExpressions.__default.MapComprehension1 (LitInt (Div 40 (LitInt 0)))))))
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 10) (let ((anon6_correct  (=> (and (and (= |let#0#0#0| (|IMap#Glue| (|lambda#5| Tclass.OtherBindersInExpressions.C (U_2_bool (Lit boolType (bool_2_U true)))) (|lambda#14| ($Box boolType (bool_2_U (= (LitInt (Div 5 (LitInt 0))) (LitInt 19))))) (TIMap Tclass.OtherBindersInExpressions.C TBool))) ($Is IMapType |let#0#0#0| (TIMap Tclass.OtherBindersInExpressions.C TBool))) (and (= |b#Z#0@0| |let#0#0#0|) (= (ControlFlow 0 3) (- 0 2)))) (or (not (= (LitInt 0) 0)) (not true)))))
...
(let ((anon9_Then_correct  (=> (and ($IsBox |c#1@0| Tclass.OtherBindersInExpressions.C) ($IsAllocBox |c#1@0| Tclass.OtherBindersInExpressions.C $Heap)) (and (=> (= (ControlFlow 0 8) 5) anon10_Then_correct) (=> (= (ControlFlow 0 8) 7) anon10_Else_correct)))))
