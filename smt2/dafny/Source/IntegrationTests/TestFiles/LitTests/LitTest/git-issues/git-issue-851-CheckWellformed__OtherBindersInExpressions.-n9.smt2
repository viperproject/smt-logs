...
(declare-fun |OtherBindersInExpressions.__default.MapComprehension0#canCall| () Bool)
(declare-fun OtherBindersInExpressions.__default.MapComprehension0 () Int)
...
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |OtherBindersInExpressions.__default.MapComprehension0#canCall| (< 1 $FunctionContextHeight)) (= OtherBindersInExpressions.__default.MapComprehension0 (LitInt (Div 40 (LitInt 0)))))))
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |OtherBindersInExpressions.__default.MapComprehension0#canCall| (< 1 $FunctionContextHeight)) (= OtherBindersInExpressions.__default.MapComprehension0 (LitInt (Div 40 (LitInt 0)))))))
...
(assert (forall ((|l#0@@0| T@U) (|$w#0| T@U) ) (! (= (MapType0Select BoxType BoxType (|lambda#14| |l#0@@0|) |$w#0|) |l#0@@0|)
 :qid |gitissue851dfy.253:14|
 :skolemid |4112|
 :pattern ( (MapType0Select BoxType BoxType (|lambda#14| |l#0@@0|) |$w#0|))
)))
...
(assert (forall ((v@@8 T@U) (t0@@3 T@U) (t1@@1 T@U) ) (! (= ($Is IMapType v@@8 (TIMap t0@@3 t1@@1)) (forall ((bx@@3 T@U) ) (!  (=> (U_2_bool (MapType0Select BoxType boolType (|IMap#Domain| v@@8) bx@@3)) (and ($IsBox (MapType0Select BoxType BoxType (|IMap#Elements| v@@8) bx@@3) t1@@1) ($IsBox bx@@3 t0@@3)))
...
(set-info :boogie-vc-id CheckWellformed$$OtherBindersInExpressions.__default.MapComprehension0)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 10) (let ((anon6_correct  (=> (and (and (= |let#0#0#0| (|IMap#Glue| (|lambda#5| Tclass.OtherBindersInExpressions.C (= (LitInt (Div 5 (LitInt 0))) (LitInt 19))) (|lambda#14| ($Box intType (int_2_U (LitInt 102)))) (TIMap Tclass.OtherBindersInExpressions.C TInt))) ($Is IMapType |let#0#0#0| (TIMap Tclass.OtherBindersInExpressions.C TInt))) (and (= |b#Z#0@0| |let#0#0#0|) (= (ControlFlow 0 3) (- 0 2)))) (or (not (= (LitInt 0) 0)) (not true)))))
...
(let ((anon9_Then_correct  (=> (and ($IsBox |c#1@0| Tclass.OtherBindersInExpressions.C) ($IsAllocBox |c#1@0| Tclass.OtherBindersInExpressions.C $Heap)) (=> (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (and (U_2_bool (Lit boolType (bool_2_U false))) (= $Heap $Heap@0))) (and (=> (= (ControlFlow 0 7) (- 0 8)) (or (not (= (LitInt 0) 0)) (not true))) (=> (or (not (= (LitInt 0) 0)) (not true)) (and (=> (= (ControlFlow 0 7) 5) anon10_Then_correct) (=> (= (ControlFlow 0 7) 6) anon10_Else_correct))))))))
(let ((anon9_Else_correct  (=> (and (not (and ($IsBox |c#1@0| Tclass.OtherBindersInExpressions.C) ($IsAllocBox |c#1@0| Tclass.OtherBindersInExpressions.C $Heap))) (= (ControlFlow 0 4) 3)) anon6_correct)))
