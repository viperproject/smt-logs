...
const unique class.ConcreteFoo.__default: ClassName;

function Tclass.ConcreteFoo.Message() : Ty
uses {
// Tclass.ConcreteFoo.Message Tag
axiom Tag(Tclass.ConcreteFoo.Message()) == Tagclass.ConcreteFoo.Message
   && TagFamily(Tclass.ConcreteFoo.Message()) == tytagFamily$Message;
}

const unique Tagclass.ConcreteFoo.Message: TyTag;

// Box/unbox axiom for Tclass.ConcreteFoo.Message
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.ConcreteFoo.Message()) } 
  $IsBox(bx, Tclass.ConcreteFoo.Message())
     ==> $Box($Unbox(bx): DatatypeType) == bx
       && $Is($Unbox(bx): DatatypeType, Tclass.ConcreteFoo.Message()));

procedure {:verboseName "ConcreteFoo.someLemma (well-formedness)"} CheckWellFormed$$ConcreteFoo.__default.someLemma(a#0: DatatypeType
       where $Is(a#0, Tclass.ConcreteFoo.Message())
         && $IsAlloc(a#0, Tclass.ConcreteFoo.Message(), $Heap)
         && $IsA#ConcreteFoo.Message(a#0), 
    b#0: DatatypeType
       where $Is(b#0, Tclass.ConcreteFoo.Message())
         && $IsAlloc(b#0, Tclass.ConcreteFoo.Message(), $Heap)
         && $IsA#ConcreteFoo.Message(b#0), 
    c#0: DatatypeType
       where $Is(c#0, Tclass.ConcreteFoo.Message())
         && $IsAlloc(c#0, Tclass.ConcreteFoo.Message(), $Heap)
         && $IsA#ConcreteFoo.Message(c#0));
  free requires 2 == $FunctionContextHeight;
  modifies $Heap;



procedure {:verboseName "ConcreteFoo.someLemma (call)"} Call$$ConcreteFoo.__default.someLemma(a#0: DatatypeType
       where $Is(a#0, Tclass.ConcreteFoo.Message())
         && $IsAlloc(a#0, Tclass.ConcreteFoo.Message(), $Heap)
         && $IsA#ConcreteFoo.Message(a#0), 
    b#0: DatatypeType
...
procedure {:verboseName "ConcreteFoo.someLemma (correctness)"} Impl$$ConcreteFoo.__default.someLemma(a#0: DatatypeType
       where $Is(a#0, Tclass.ConcreteFoo.Message())
         && $IsAlloc(a#0, Tclass.ConcreteFoo.Message(), $Heap)
         && $IsA#ConcreteFoo.Message(a#0), 
    b#0: DatatypeType
...
function Tclass.ByteDefinition.byte() : Ty
uses {
// Tclass.ByteDefinition.byte Tag
axiom Tag(Tclass.ByteDefinition.byte()) == Tagclass.ByteDefinition.byte
   && TagFamily(Tclass.ByteDefinition.byte()) == tytagFamily$byte;
}

const unique Tagclass.ByteDefinition.byte: TyTag;

// Box/unbox axiom for Tclass.ByteDefinition.byte
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.ByteDefinition.byte()) } 
  $IsBox(bx, Tclass.ByteDefinition.byte())
     ==> $Box($Unbox(bx): int) == bx
       && $Is($Unbox(bx): int, Tclass.ByteDefinition.byte()));

// Constructor function declaration
function #ConcreteFoo.Message.M(int) : DatatypeType;

const unique ##ConcreteFoo.Message.M: DtCtorId
uses {
// Constructor identifier
axiom (forall a#0#0#0: int :: 
  { #ConcreteFoo.Message.M(a#0#0#0) } 
  DatatypeCtorId(#ConcreteFoo.Message.M(a#0#0#0)) == ##ConcreteFoo.Message.M);
}

function ConcreteFoo.Message.M_q(DatatypeType) : bool;

// Questionmark and identifier
axiom (forall d: DatatypeType :: 
  { ConcreteFoo.Message.M_q(d) } 
  ConcreteFoo.Message.M_q(d) <==> DatatypeCtorId(d) == ##ConcreteFoo.Message.M);

// Constructor questionmark has arguments
axiom (forall d: DatatypeType :: 
  { ConcreteFoo.Message.M_q(d) } 
  ConcreteFoo.Message.M_q(d)
     ==> (exists a#1#0#0: int :: d == #ConcreteFoo.Message.M(a#1#0#0)));

// Constructor $Is
axiom (forall a#2#0#0: int :: 
  { $Is(#ConcreteFoo.Message.M(a#2#0#0), Tclass.ConcreteFoo.Message()) } 
  $Is(#ConcreteFoo.Message.M(a#2#0#0), Tclass.ConcreteFoo.Message())
     <==> $Is(a#2#0#0, Tclass.ByteDefinition.byte()));

// Destructor $IsAlloc
axiom (forall d: DatatypeType, $h: Heap :: 
  { $IsAlloc(ConcreteFoo.Message.value(d), Tclass.ByteDefinition.byte(), $h) } 
  $IsGoodHeap($h)
       && 
      ConcreteFoo.Message.M_q(d)
       && $IsAlloc(d, Tclass.ConcreteFoo.Message(), $h)
     ==> $IsAlloc(ConcreteFoo.Message.value(d), Tclass.ByteDefinition.byte(), $h));

// Constructor literal
axiom (forall a#3#0#0: int :: 
  { #ConcreteFoo.Message.M(LitInt(a#3#0#0)) } 
  #ConcreteFoo.Message.M(LitInt(a#3#0#0)) == Lit(#ConcreteFoo.Message.M(a#3#0#0)));

function ConcreteFoo.Message.value(DatatypeType) : int;

// Constructor injectivity
axiom (forall a#4#0#0: int :: 
  { #ConcreteFoo.Message.M(a#4#0#0) } 
  ConcreteFoo.Message.value(#ConcreteFoo.Message.M(a#4#0#0)) == a#4#0#0);

// Datatype $IsAlloc
axiom (forall d: DatatypeType, $h: Heap :: 
  { $IsAlloc(d, Tclass.ConcreteFoo.Message(), $h) } 
  $IsGoodHeap($h) && $Is(d, Tclass.ConcreteFoo.Message())
     ==> $IsAlloc(d, Tclass.ConcreteFoo.Message(), $h));

// Depth-one case-split function
function $IsA#ConcreteFoo.Message(DatatypeType) : bool;

// Depth-one case-split axiom
axiom (forall d: DatatypeType :: 
  { $IsA#ConcreteFoo.Message(d) } 
  $IsA#ConcreteFoo.Message(d) ==> ConcreteFoo.Message.M_q(d));

// Questionmark data type disjunctivity
axiom (forall d: DatatypeType :: 
  { ConcreteFoo.Message.M_q(d), $Is(d, Tclass.ConcreteFoo.Message()) } 
  $Is(d, Tclass.ConcreteFoo.Message()) ==> ConcreteFoo.Message.M_q(d));

// Datatype extensional equality declaration
function ConcreteFoo.Message#Equal(DatatypeType, DatatypeType) : bool;

// Datatype extensional equality definition: #ConcreteFoo.Message.M
axiom (forall a: DatatypeType, b: DatatypeType :: 
  { ConcreteFoo.Message#Equal(a, b) } 
  ConcreteFoo.Message#Equal(a, b)
     <==> ConcreteFoo.Message.value(a) == ConcreteFoo.Message.value(b));

// Datatype extensionality axiom: ConcreteFoo.Message
axiom (forall a: DatatypeType, b: DatatypeType :: 
  { ConcreteFoo.Message#Equal(a, b) } 
  ConcreteFoo.Message#Equal(a, b) <==> a == b);

const unique class.ConcreteFoo.Message: ClassName;

const unique class.ByteDefinition.__default: ClassName;

// $Is axiom for newtype ByteDefinition.byte
axiom (forall i#0: int :: 
  { $Is(i#0, Tclass.ByteDefinition.byte()) } 
  $Is(i#0, Tclass.ByteDefinition.byte()) <==> LitInt(0) <= i#0 && i#0 < 256);

// $IsAlloc axiom for newtype ByteDefinition.byte
axiom (forall i#0: int, $h: Heap :: 
  { $IsAlloc(i#0, Tclass.ByteDefinition.byte(), $h) } 
  $IsAlloc(i#0, Tclass.ByteDefinition.byte(), $h));

const unique class.ByteDefinition.byte: ClassName;
