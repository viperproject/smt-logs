...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.StmtExpr)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#3| null $Heap alloc |d#0|)) (= (ControlFlow 0 2) (- 0 1))) (or (not (= |d#0| null)) (not true)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is refType |d#0| Tclass._module.D) ($IsAlloc refType |d#0| Tclass._module.D $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
