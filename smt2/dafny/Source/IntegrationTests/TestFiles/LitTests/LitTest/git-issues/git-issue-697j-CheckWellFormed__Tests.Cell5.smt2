...
(assert (distinct TInt TagInt alloc Tagclass.Tests.Cell Tagclass.Tests.Cell? tytagFamily$Cell)
)
...
(assert (forall ((|c#0| T@U) ($h@@0 T@U) ) (! (= ($IsAlloc refType |c#0| Tclass.Tests.Cell $h@@0) ($IsAlloc refType |c#0| Tclass.Tests.Cell? $h@@0))
 :qid |unknown.0:0|
 :skolemid |522|
 :pattern ( ($IsAlloc refType |c#0| Tclass.Tests.Cell $h@@0))
 :pattern ( ($IsAlloc refType |c#0| Tclass.Tests.Cell? $h@@0))
)))
(assert (forall (($o@@1 T@U) ($h@@1 T@U) ) (! (= ($IsAlloc refType $o@@1 Tclass.Tests.Cell? $h@@1)  (or (= $o@@1 null) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@1 $o@@1) alloc)))))
 :qid |unknown.0:0|
 :skolemid |515|
 :pattern ( ($IsAlloc refType $o@@1 Tclass.Tests.Cell? $h@@1))
)))
...
(assert (forall ((|c#0@@0| T@U) ) (! (= ($Is refType |c#0@@0| Tclass.Tests.Cell)  (and ($Is refType |c#0@@0| Tclass.Tests.Cell?) (or (not (= |c#0@@0| null)) (not true))))
 :qid |unknown.0:0|
 :skolemid |521|
 :pattern ( ($Is refType |c#0@@0| Tclass.Tests.Cell))
 :pattern ( ($Is refType |c#0@@0| Tclass.Tests.Cell?))
)))
...
(set-info :boogie-vc-id CheckWellFormed$$Tests.Cell5)
...
(let ((anon5_Then_correct  (=> (and ($Is refType |c#0@@1| Tclass.Tests.Cell) (= (ControlFlow 0 2) (- 0 1))) (or (not (= |c#0@@1| null)) (not true)))))
