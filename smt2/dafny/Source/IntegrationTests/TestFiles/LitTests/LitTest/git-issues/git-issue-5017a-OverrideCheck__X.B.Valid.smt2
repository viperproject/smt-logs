...
(assert (distinct alloc Tagclass.X.B Tagclass.X.B? Tagclass.X.A Tagclass.X.A? tytagFamily$B tytagFamily$A)
)
(assert (= (Ctor refType) 3))
(assert (forall ((|c#0| T@U) ($h T@U) ) (! (= ($IsAlloc refType |c#0| Tclass.X.A $h) ($IsAlloc refType |c#0| Tclass.X.A? $h))
 :qid |unknown.0:0|
 :skolemid |1005|
 :pattern ( ($IsAlloc refType |c#0| Tclass.X.A $h))
 :pattern ( ($IsAlloc refType |c#0| Tclass.X.A? $h))
)))
(assert (forall ((|c#0@@0| T@U) ($h@@0 T@U) ) (! (= ($IsAlloc refType |c#0@@0| Tclass.X.B $h@@0) ($IsAlloc refType |c#0@@0| Tclass.X.B? $h@@0))
 :qid |unknown.0:0|
 :skolemid |1016|
 :pattern ( ($IsAlloc refType |c#0@@0| Tclass.X.B $h@@0))
 :pattern ( ($IsAlloc refType |c#0@@0| Tclass.X.B? $h@@0))
)))
...
(assert (forall (($o T@U) ($h@@1 T@U) ) (! (= ($IsAlloc refType $o Tclass.X.A? $h@@1)  (or (= $o null) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@1 $o) alloc)))))
 :qid |unknown.0:0|
 :skolemid |1001|
 :pattern ( ($IsAlloc refType $o Tclass.X.A? $h@@1))
)))
(assert (forall (($o@@0 T@U) ($h@@2 T@U) ) (! (= ($IsAlloc refType $o@@0 Tclass.X.B? $h@@2)  (or (= $o@@0 null) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@2 $o@@0) alloc)))))
 :qid |unknown.0:0|
 :skolemid |1007|
 :pattern ( ($IsAlloc refType $o@@0 Tclass.X.B? $h@@2))
)))
...
(set-info :boogie-vc-id OverrideCheck$$X.B.Valid)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 4) (let ((anon0_correct  (and (=> (= (ControlFlow 0 2) (- 0 3)) (=> |x#0| |x#0|)) (=> (=> |x#0| |x#0|) (=> (and (= B_$_ReadsFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall (($o@@6 T@U) ($f@@0 T@U) ) (! true
 :qid |gitissue5017adfy.33:28|
 :skolemid |1014|
)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (or (not (= this null)) (not true)) (and ($Is refType this Tclass.X.B) ($IsAlloc refType this Tclass.X.B $Heap))) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 4) 2))) anon0_correct))))
