...
(declare-fun |OtherBindersInExpressions.__default.Exists#canCall| () Bool)
(declare-fun OtherBindersInExpressions.__default.Exists () Int)
...
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |OtherBindersInExpressions.__default.Exists#canCall| (< 1 $FunctionContextHeight)) (= OtherBindersInExpressions.__default.Exists (LitInt (Div 40 (LitInt 0)))))))
(assert  (=> (<= 1 $FunctionContextHeight) (=> (or |OtherBindersInExpressions.__default.Exists#canCall| (< 1 $FunctionContextHeight)) (= OtherBindersInExpressions.__default.Exists (LitInt (Div 40 (LitInt 0)))))))
...
(set-info :boogie-vc-id CheckWellformed$$OtherBindersInExpressions.__default.Exists)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 8) (let ((anon5_correct  (=> (and (and (= |let#0#0#0| (exists ((|c#3| T@U) ) (!  (and ($IsBox |c#3| Tclass.OtherBindersInExpressions.C) (= (LitInt (Div 5 (LitInt 0))) (LitInt 19)))
 :qid |gitissue851dfy.235:21|
 :skolemid |3513|
