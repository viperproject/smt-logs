...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 7) (let ((GeneratedUnifiedExit_correct  (and (=> (= (ControlFlow 0 2) (- 0 3)) (=> (= |i#0| (LitInt 1)) (not (_module.Status.Failure_q |r#0@2|)))) (=> (=> (= |i#0| (LitInt 1)) (not (_module.Status.Failure_q |r#0@2|))) (=> (= (ControlFlow 0 2) (- 0 1)) (=> (= |i#0| (LitInt 2)) (_module.Status.Failure_q |r#0@2|)))))))
(let ((anon5_Else_correct  (=> (and (and (or (not (= |i#0| (LitInt 2))) (not true)) (= |r#0@1| (Lit DatatypeTypeType |#_module.Status.Success|))) (and (= |r#0@2| |r#0@1|) (= (ControlFlow 0 5) 2))) GeneratedUnifiedExit_correct)))
(let ((anon5_Then_correct  (=> (and (and (= |i#0| (LitInt 2)) (= |r#0@0| (Lit DatatypeTypeType (|#_module.Status.Failure| (Lit SeqType (|Seq#Build| |Seq#Empty| ($Box charType (|char#FromInt| 90)))))))) (and (= |r#0@2| |r#0@0|) (= (ControlFlow 0 4) 2))) GeneratedUnifiedExit_correct)))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (and (=> (= (ControlFlow 0 6) 4) anon5_Then_correct) (=> (= (ControlFlow 0 6) 5) anon5_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is DatatypeTypeType |r#0| Tclass._module.Status) ($IsAlloc DatatypeTypeType |r#0| Tclass._module.Status $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 7) 6))) anon0_correct))))
