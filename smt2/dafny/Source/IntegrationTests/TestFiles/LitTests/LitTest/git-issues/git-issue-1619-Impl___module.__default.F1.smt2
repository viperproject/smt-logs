...
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and ($Is SeqType |p#0@0| (TSeq TInt)) ($IsAlloc SeqType |p#0@0| (TSeq TInt) $Heap)) (=> (and (and ($IsAlloc SeqType |p#0@0| (TSeq TInt) $Heap) (|_module.__default.F#canCall| |p#0@0|)) (and (|_module.__default.F#canCall| |p#0@0|) (_module.__default.F |p#0@0|))) (and (=> (= (ControlFlow 0 5) 3) anon3_Then_correct) (=> (= (ControlFlow 0 5) 4) anon3_Else_correct)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is SeqType |p#0| (TSeq TInt)) ($IsAlloc SeqType |p#0| (TSeq TInt) $Heap)) true) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct))))
