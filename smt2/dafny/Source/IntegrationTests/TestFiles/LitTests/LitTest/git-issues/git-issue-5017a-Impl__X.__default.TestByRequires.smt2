...
(declare-fun |a#0| () T@U)
(set-info :boogie-vc-id Impl$$X.__default.TestByRequires)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (and (=> (= (ControlFlow 0 2) (- 0 4)) true) (and (=> (= (ControlFlow 0 2) (- 0 3)) (or (not (= |b#0| null)) (not true))) (=> ($IsAllocBox ($Box refType |b#0|) Tclass.X.A? $Heap) (=> (and (and ($IsAlloc boolType (bool_2_U |y#0|) TBool $Heap) (|X.A.Valid#canCall| |b#0| |y#0|)) (and (|X.A.Valid#canCall| |b#0| |y#0|) (= (ControlFlow 0 2) (- 0 1)))) (X.A.Valid |b#0| |y#0|))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is refType |b#0| Tclass.X.B) ($IsAlloc refType |b#0| Tclass.X.B $Heap))) (=> (and (and (and (=> |defass#a#0| (and ($Is refType |a#0| Tclass.X.A) ($IsAlloc refType |a#0| Tclass.X.A $Heap))) true) (= 2 $FunctionContextHeight)) (and (X.B.Valid reveal_X.B.Valid |b#0| |y#0|) (= (ControlFlow 0 5) 2))) anon0_correct))))
