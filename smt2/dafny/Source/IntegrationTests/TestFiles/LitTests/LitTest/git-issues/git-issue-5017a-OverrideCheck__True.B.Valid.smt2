...
(assert (forall ((|c#0| T@U) ($h T@U) ) (! (= ($IsAlloc refType |c#0| Tclass.True.A $h) ($IsAlloc refType |c#0| Tclass.True.A? $h))
 :qid |unknown.0:0|
 :skolemid |495|
 :pattern ( ($IsAlloc refType |c#0| Tclass.True.A $h))
 :pattern ( ($IsAlloc refType |c#0| Tclass.True.A? $h))
)))
(assert (forall ((|c#0@@0| T@U) ($h@@0 T@U) ) (! (= ($IsAlloc refType |c#0@@0| Tclass.True.B $h@@0) ($IsAlloc refType |c#0@@0| Tclass.True.B? $h@@0))
 :qid |unknown.0:0|
 :skolemid |505|
 :pattern ( ($IsAlloc refType |c#0@@0| Tclass.True.B $h@@0))
 :pattern ( ($IsAlloc refType |c#0@@0| Tclass.True.B? $h@@0))
)))
...
(assert (forall (($o T@U) ($h@@1 T@U) ) (! (= ($IsAlloc refType $o Tclass.True.A? $h@@1)  (or (= $o null) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@1 $o) alloc)))))
 :qid |unknown.0:0|
 :skolemid |491|
 :pattern ( ($IsAlloc refType $o Tclass.True.A? $h@@1))
)))
(assert (forall (($o@@0 T@U) ($h@@2 T@U) ) (! (= ($IsAlloc refType $o@@0 Tclass.True.B? $h@@2)  (or (= $o@@0 null) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@2 $o@@0) alloc)))))
 :qid |unknown.0:0|
 :skolemid |497|
 :pattern ( ($IsAlloc refType $o@@0 Tclass.True.B? $h@@2))
)))
...
(set-info :boogie-vc-id OverrideCheck$$True.B.Valid)
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (or (not (= this null)) (not true)) (and ($Is refType this Tclass.True.B) ($IsAlloc refType this Tclass.True.B $Heap))) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 4) 2))) anon0_correct))))
