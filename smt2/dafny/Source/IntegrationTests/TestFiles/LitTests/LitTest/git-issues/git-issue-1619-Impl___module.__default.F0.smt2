...
(set-info :boogie-vc-id Impl$$_module.__default.F0)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 6) (let ((anon2_correct  (=> (and (forall ((|p#2| T@U) ) (!  (=> ($Is SeqType |p#2| (TSeq TInt)) (|_module.__default.F#canCall| |p#2|))
 :qid |gitissue1619dfy.157:17|
 :skolemid |579|
 :pattern ( (_module.__default.F |p#2|))
)) (= (ControlFlow 0 2) (- 0 1))) (exists ((|p#2@@0| T@U) ) (!  (and ($Is SeqType |p#2@@0| (TSeq TInt)) (_module.__default.F |p#2@@0|))
 :qid |gitissue1619dfy.157:17|
 :skolemid |581|
 :pattern ( (_module.__default.F |p#2@@0|))
)))))
(let ((anon3_Else_correct  (=> (and (not (and ($Is SeqType |p#1@0| (TSeq TInt)) ($IsAlloc SeqType |p#1@0| (TSeq TInt) $Heap))) (= (ControlFlow 0 4) 2)) anon2_correct)))
(let ((anon3_Then_correct  (=> (and (and (and ($Is SeqType |p#1@0| (TSeq TInt)) ($IsAlloc SeqType |p#1@0| (TSeq TInt) $Heap)) ($IsAlloc SeqType |p#1@0| (TSeq TInt) $Heap)) (and (|_module.__default.F#canCall| |p#1@0|) (= (ControlFlow 0 3) 2))) anon2_correct)))
(let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) ($Is SeqType |$rhs#0@0| (TSeq TInt))) (=> (and (and ($IsAlloc SeqType |$rhs#0@0| (TSeq TInt) $Heap) (|_module.__default.F#canCall| |$rhs#0@0|)) (and (|_module.__default.F#canCall| |$rhs#0@0|) (_module.__default.F |$rhs#0@0|))) (and (=> (= (ControlFlow 0 5) 3) anon3_Then_correct) (=> (= (ControlFlow 0 5) 4) anon3_Else_correct))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is SeqType |p#0| (TSeq TInt)) ($IsAlloc SeqType |p#0| (TSeq TInt) $Heap)) true) (and (and ($Is SeqType |$rhs#0| (TSeq TInt)) true) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 6) 5)))) anon0_correct))))
