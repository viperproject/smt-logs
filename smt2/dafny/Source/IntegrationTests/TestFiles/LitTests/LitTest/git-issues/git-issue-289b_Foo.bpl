...
function Tclass.Foo.Message() : Ty
uses {
// Tclass.Foo.Message Tag
axiom Tag(Tclass.Foo.Message()) == Tagclass.Foo.Message
   && TagFamily(Tclass.Foo.Message()) == tytagFamily$Message;
}

const unique Tagclass.Foo.Message: TyTag;

// Box/unbox axiom for Tclass.Foo.Message
axiom (forall bx: Box :: 
  { $IsBox(bx, Tclass.Foo.Message()) } 
  $IsBox(bx, Tclass.Foo.Message())
     ==> $Box($Unbox(bx): DatatypeType) == bx
       && $Is($Unbox(bx): DatatypeType, Tclass.Foo.Message()));

procedure {:verboseName "Foo.someLemma (well-formedness)"} CheckWellFormed$$Foo.__default.someLemma(a#0: DatatypeType
       where $Is(a#0, Tclass.Foo.Message())
         && $IsAlloc(a#0, Tclass.Foo.Message(), $Heap)
         && $IsA#Foo.Message(a#0), 
    b#0: DatatypeType
       where $Is(b#0, Tclass.Foo.Message())
         && $IsAlloc(b#0, Tclass.Foo.Message(), $Heap)
         && $IsA#Foo.Message(b#0), 
    c#0: DatatypeType
       where $Is(c#0, Tclass.Foo.Message())
         && $IsAlloc(c#0, Tclass.Foo.Message(), $Heap)
         && $IsA#Foo.Message(c#0));
...
procedure {:verboseName "Foo.someLemma (call)"} Call$$Foo.__default.someLemma(a#0: DatatypeType
       where $Is(a#0, Tclass.Foo.Message())
         && $IsAlloc(a#0, Tclass.Foo.Message(), $Heap)
         && $IsA#Foo.Message(a#0), 
    b#0: DatatypeType
...
procedure {:verboseName "Foo.someLemma (correctness)"} Impl$$Foo.__default.someLemma(a#0: DatatypeType
       where $Is(a#0, Tclass.Foo.Message())
         && $IsAlloc(a#0, Tclass.Foo.Message(), $Heap)
         && $IsA#Foo.Message(a#0), 
    b#0: DatatypeType
...
function Tclass.Foo.Value() : Ty;

const unique class.Foo.Value: ClassName;

// Constructor function declaration
function #Foo.Message.M(Box) : DatatypeType;

const unique ##Foo.Message.M: DtCtorId
uses {
// Constructor identifier
axiom (forall a#0#0#0: Box :: 
  { #Foo.Message.M(a#0#0#0) } 
  DatatypeCtorId(#Foo.Message.M(a#0#0#0)) == ##Foo.Message.M);
}

function Foo.Message.M_q(DatatypeType) : bool;

// Questionmark and identifier
axiom (forall d: DatatypeType :: 
  { Foo.Message.M_q(d) } 
  Foo.Message.M_q(d) <==> DatatypeCtorId(d) == ##Foo.Message.M);

// Constructor questionmark has arguments
axiom (forall d: DatatypeType :: 
  { Foo.Message.M_q(d) } 
  Foo.Message.M_q(d) ==> (exists a#1#0#0: Box :: d == #Foo.Message.M(a#1#0#0)));

// Constructor $Is
axiom (forall a#2#0#0: Box :: 
  { $Is(#Foo.Message.M(a#2#0#0), Tclass.Foo.Message()) } 
  $Is(#Foo.Message.M(a#2#0#0), Tclass.Foo.Message())
     <==> $IsBox(a#2#0#0, Tclass.Foo.Value()));

// Constructor $IsAlloc
axiom (forall a#2#0#0: Box, $h: Heap :: 
  { $IsAlloc(#Foo.Message.M(a#2#0#0), Tclass.Foo.Message(), $h) } 
  $IsGoodHeap($h)
     ==> ($IsAlloc(#Foo.Message.M(a#2#0#0), Tclass.Foo.Message(), $h)
       <==> $IsAllocBox(a#2#0#0, Tclass.Foo.Value(), $h)));

// Destructor $IsAlloc
axiom (forall d: DatatypeType, $h: Heap :: 
  { $IsAllocBox(Foo.Message.value(d), Tclass.Foo.Value(), $h) } 
  $IsGoodHeap($h) && Foo.Message.M_q(d) && $IsAlloc(d, Tclass.Foo.Message(), $h)
     ==> $IsAllocBox(Foo.Message.value(d), Tclass.Foo.Value(), $h));

// Constructor literal
axiom (forall a#3#0#0: Box :: 
  { #Foo.Message.M(Lit(a#3#0#0)) } 
  #Foo.Message.M(Lit(a#3#0#0)) == Lit(#Foo.Message.M(a#3#0#0)));

function Foo.Message.value(DatatypeType) : Box;

// Constructor injectivity
axiom (forall a#4#0#0: Box :: 
  { #Foo.Message.M(a#4#0#0) } 
  Foo.Message.value(#Foo.Message.M(a#4#0#0)) == a#4#0#0);

// Depth-one case-split function
function $IsA#Foo.Message(DatatypeType) : bool;

// Depth-one case-split axiom
axiom (forall d: DatatypeType :: 
  { $IsA#Foo.Message(d) } 
  $IsA#Foo.Message(d) ==> Foo.Message.M_q(d));

// Questionmark data type disjunctivity
axiom (forall d: DatatypeType :: 
  { Foo.Message.M_q(d), $Is(d, Tclass.Foo.Message()) } 
  $Is(d, Tclass.Foo.Message()) ==> Foo.Message.M_q(d));

// Datatype extensional equality declaration
function Foo.Message#Equal(DatatypeType, DatatypeType) : bool;

// Datatype extensional equality definition: #Foo.Message.M
axiom (forall a: DatatypeType, b: DatatypeType :: 
  { Foo.Message#Equal(a, b) } 
  Foo.Message#Equal(a, b) <==> Foo.Message.value(a) == Foo.Message.value(b));

// Datatype extensionality axiom: Foo.Message
axiom (forall a: DatatypeType, b: DatatypeType :: 
  { Foo.Message#Equal(a, b) } 
  Foo.Message#Equal(a, b) <==> a == b);

const unique class.Foo.Message: ClassName;
...
const unique tytagFamily$Message: TyTagFamily;
