...
(declare-fun |M.D.f#canCall| () Bool)
(declare-fun M.D.f () Int)
...
(assert  (=> (<= 0 $FunctionContextHeight) (=> (or |M.D.f#canCall| (< 0 $FunctionContextHeight)) (= M.D.f (LitInt 5)))))
(assert  (=> (<= 0 $FunctionContextHeight) (=> (or |M.D.f#canCall| (< 0 $FunctionContextHeight)) (= M.D.f (LitInt 5)))))
...
(set-info :boogie-vc-id Impl$$M.D.f)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= |res#0@0| (LitInt 5))) (and |M.D.f#canCall| (= (ControlFlow 0 2) (- 0 1)))) (= |res#0@0| (LitInt M.D.f)))))
