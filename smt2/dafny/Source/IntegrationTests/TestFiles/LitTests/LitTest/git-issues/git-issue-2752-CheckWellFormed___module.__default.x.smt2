...
(assert (distinct alloc Tagclass._System.object? Tagclass._System.object Tagclass._module.Empty tytagFamily$object tytagFamily$Empty)
)
...
(assert (forall ((|x#0| T@U) ) (! (= ($Is refType |x#0| Tclass._module.Empty)  (and ($Is refType |x#0| Tclass._System.object) (U_2_bool (Lit boolType (bool_2_U false)))))
 :qid |unknown.0:0|
 :skolemid |487|
 :pattern ( ($Is refType |x#0| Tclass._module.Empty))
)))
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ReadsFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) ($Is refType null Tclass._module.Empty))))
