...
(assert (forall (($prevHeap T@U) ($h0 T@U) ($h1 T@U) (|d#0| T@U) (|selector#0| Int) ) (!  (=> (and (and (and ($IsGoodHeap $h0) ($IsGoodHeap $h1)) (or (|_module.__default.CalcStmtExprFunction#canCall| $prevHeap $h0 |d#0| |selector#0|) ($Is refType |d#0| Tclass._module.D))) (and ($IsHeapAnchor $h0) ($HeapSucc $h0 $h1))) (=> (forall (($o@@2 T@U) ($f T@U) ) (!  (=> (and (or (not (= $o@@2 null)) (not true)) (= $o@@2 |d#0|)) (= (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h0 $o@@2) $f) (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h1 $o@@2) $f)))
 :qid |unknown.0:0|
 :skolemid |1146|
)) (= (_module.__default.CalcStmtExprFunction $prevHeap $h0 |d#0| |selector#0|) (_module.__default.CalcStmtExprFunction $prevHeap $h1 |d#0| |selector#0|))))
 :qid |DafnyPreludebpl.593:12|
 :skolemid |1147|
...
(set-info :boogie-vc-id CheckWellformed$$_module.__default.CalcStmtExprFunction)
