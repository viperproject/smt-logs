...
(assert (distinct TagSet alloc Tagclass._module.Memory Tagclass._module.Memory? tytagFamily$Memory)
)
...
(assert (forall (($o T@U) ) (! (= ($Is refType $o Tclass._module.Memory?)  (or (= $o null) (= (dtype $o) Tclass._module.Memory?)))
 :qid |unknown.0:0|
 :skolemid |824|
 :pattern ( ($Is refType $o Tclass._module.Memory?))
)))
...
(assert (forall ((|c#0| T@U) ($h T@U) ) (! (= ($IsAlloc refType |c#0| Tclass._module.Memory $h) ($IsAlloc refType |c#0| Tclass._module.Memory? $h))
 :qid |unknown.0:0|
 :skolemid |1378|
 :pattern ( ($IsAlloc refType |c#0| Tclass._module.Memory $h))
 :pattern ( ($IsAlloc refType |c#0| Tclass._module.Memory? $h))
)))
...
(assert (forall (($o@@0 T@U) ($h@@0 T@U) ) (! (= ($IsAlloc refType $o@@0 Tclass._module.Memory? $h@@0)  (or (= $o@@0 null) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@0 $o@@0) alloc)))))
 :qid |unknown.0:0|
 :skolemid |825|
 :pattern ( ($IsAlloc refType $o@@0 Tclass._module.Memory? $h@@0))
)))
...
(assert (forall ((bx@@0 T@U) ) (!  (=> ($IsBox bx@@0 Tclass._module.Memory) (and (= ($Box refType ($Unbox refType bx@@0)) bx@@0) ($Is refType ($Unbox refType bx@@0) Tclass._module.Memory)))
 :qid |unknown.0:0|
 :skolemid |600|
 :pattern ( ($IsBox bx@@0 Tclass._module.Memory))
)))
(assert (forall ((bx@@1 T@U) ) (!  (=> ($IsBox bx@@1 Tclass._module.Memory?) (and (= ($Box refType ($Unbox refType bx@@1)) bx@@1) ($Is refType ($Unbox refType bx@@1) Tclass._module.Memory?)))
 :qid |unknown.0:0|
 :skolemid |601|
 :pattern ( ($IsBox bx@@1 Tclass._module.Memory?))
)))
(assert (forall ((|c#0@@0| T@U) ) (! (= ($Is refType |c#0@@0| Tclass._module.Memory)  (and ($Is refType |c#0@@0| Tclass._module.Memory?) (or (not (= |c#0@@0| null)) (not true))))
 :qid |unknown.0:0|
 :skolemid |1377|
 :pattern ( ($Is refType |c#0@@0| Tclass._module.Memory))
 :pattern ( ($Is refType |c#0@@0| Tclass._module.Memory?))
)))
...
(declare-fun _module.Memory.BiggerIsBigger$T () T@U)
(declare-fun |xs#0| () T@U)
(declare-fun $FunctionContextHeight () Int)
(set-info :boogie-vc-id Impl$$_module.Memory.BiggerIsBigger)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 4) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= |xs#0@0| (|Set#Difference| |aa#0| |bb#0|))) (and (=> (= (ControlFlow 0 2) (- 0 3)) (= (|Set#Card| |aa#0|) (+ (|Set#Card| |bb#0|) (|Set#Card| |xs#0@0|)))) (=> (= (|Set#Card| |aa#0|) (+ (|Set#Card| |bb#0|) (|Set#Card| |xs#0@0|))) (=> (= (ControlFlow 0 2) (- 0 1)) (>= (|Set#Card| |aa#0|) (|Set#Card| |bb#0|))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and (or (not (= this null)) (not true)) (and ($Is refType this Tclass._module.Memory) ($IsAlloc refType this Tclass._module.Memory $Heap)))) (and (and ($Is SetType |aa#0| (TSet _module.Memory.BiggerIsBigger$T)) ($IsAlloc SetType |aa#0| (TSet _module.Memory.BiggerIsBigger$T) $Heap)) (and ($Is SetType |bb#0| (TSet _module.Memory.BiggerIsBigger$T)) ($IsAlloc SetType |bb#0| (TSet _module.Memory.BiggerIsBigger$T) $Heap)))) (=> (and (and (and (and ($Is SetType |xs#0| (TSet _module.Memory.BiggerIsBigger$T)) ($IsAlloc SetType |xs#0| (TSet _module.Memory.BiggerIsBigger$T) $Heap)) true) (= 0 $FunctionContextHeight)) (and (|Set#Subset| |bb#0| |aa#0|) (= (ControlFlow 0 4) 2))) anon0_correct))))
