...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (and (= |y#0@0| (LitInt 0)) (= (ControlFlow 0 2) (- 0 1))) (and (<= (LitInt 0) |y#0@0|) (< |y#0@0| (|Seq#Length| |test#0|)))) (U_2_bool ($Unbox boolType (|Seq#Index| |test#0| |y#0@0|)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is SeqType |test#0| (TSeq TBool)) ($IsAlloc SeqType |test#0| (TSeq TBool) $Heap)) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
