...
(assert (distinct alloc Tagclass._System.array? Tagclass._System.array Tagclass._module.uint32 tytagFamily$array tytagFamily$uint32)
)
...
(set-info :boogie-vc-id Impl$$_module.__default.returnANullArray)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (= null null))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is refType |a#0| (Tclass._System.array? Tclass._module.uint32)) ($IsAlloc refType |a#0| (Tclass._System.array? Tclass._module.uint32) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
