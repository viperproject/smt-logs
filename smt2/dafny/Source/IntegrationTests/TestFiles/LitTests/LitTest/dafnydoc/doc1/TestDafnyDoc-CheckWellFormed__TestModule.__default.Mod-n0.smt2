...
(set-info :boogie-vc-id CheckWellFormed$$TestModule.__default.ModifyA)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#4| null $Heap alloc |a#0| |a#0| TestModule.A.j)) (= (ControlFlow 0 2) (- 0 1))) (or (not (= |a#0| null)) (not true)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is refType |a#0| Tclass.TestModule.A) ($IsAlloc refType |a#0| Tclass.TestModule.A $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
