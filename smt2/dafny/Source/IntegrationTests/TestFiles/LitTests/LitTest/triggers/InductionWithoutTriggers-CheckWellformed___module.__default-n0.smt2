...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 8) (let ((anon8_Else_correct  (=> (or (not (= |n#0@@3| (LitInt 0))) (not true)) (and (=> (= (ControlFlow 0 4) (- 0 6)) ($Is intType (int_2_U (- |n#0@@3| 1)) Tclass._System.nat)) (=> ($Is intType (int_2_U (- |n#0@@3| 1)) Tclass._System.nat) (=> (= |##n#0@0| (- |n#0@@3| 1)) (=> (and ($IsAlloc intType (int_2_U |##n#0@0|) Tclass._System.nat $Heap@@2) ($IsAlloc HandleTypeType |f#0@@7| (Tclass._System.___hTotalFunc1 TInt TInt) $Heap@@2)) (and (=> (= (ControlFlow 0 4) (- 0 5)) (or (<= 0 |n#0@@3|) (= |##n#0@0| |n#0@@3|))) (=> (or (<= 0 |n#0@@3|) (= |##n#0@0| |n#0@@3|)) (=> (= (ControlFlow 0 4) (- 0 3)) (< |##n#0@0| |n#0@@3|)))))))))))
(let ((anon8_Then_correct true))
(let ((anon7_Then_correct true))
(let ((anon0_correct  (=> (= $_ReadsFrame@0 (|lambda#0| null $Heap@@2 alloc false)) (and (and (=> (= (ControlFlow 0 7) 1) anon7_Then_correct) (=> (= (ControlFlow 0 7) 2) anon8_Then_correct)) (=> (= (ControlFlow 0 7) 4) anon8_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap@@2) ($IsHeapAnchor $Heap@@2)) (=> (and (and (<= (LitInt 0) |n#0@@3|) ($Is HandleTypeType |f#0@@7| (Tclass._System.___hTotalFunc1 TInt TInt))) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 8) 7))) anon0_correct))))
