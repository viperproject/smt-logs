...
(assert (forall (($h@@1 T@U) ($o@@1 T@U) ) (!  (=> (and (and ($IsGoodHeap $h@@1) (and (or (not (= $o@@1 null)) (not true)) (= (dtype $o@@1) Tclass._module.T?))) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@1 $o@@1) alloc)))) ($IsAlloc HandleTypeType ($Unbox HandleTypeType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@1 $o@@1) _module.T.h)) (Tclass._System.___hFunc1 TInt TInt) $h@@1))
 :qid |DafnyPreludebpl.593:12|
 :skolemid |516|
 :pattern ( ($Unbox HandleTypeType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@1 $o@@1) _module.T.h)))
)))
...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($Heap T@U) (|t#0| T@U) ) (!  (=> (or (|_module.__default.B#canCall| $Heap |t#0|) (and (< 1 $FunctionContextHeight) (and ($IsGoodHeap $Heap) (and ($Is refType |t#0| Tclass._module.T) ($IsAlloc refType |t#0| Tclass._module.T $Heap))))) ($Is HandleTypeType (_module.__default.B $Heap |t#0|) (Tclass._System.___hFunc1 TInt TInt)))
 :qid |Classesdfy.25:16|
 :skolemid |489|
 :pattern ( (_module.__default.B $Heap |t#0|))
))))
...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($Heap@@0 T@U) (|t#0@@0| T@U) ) (!  (=> (or (|_module.__default.B#canCall| $Heap@@0 |t#0@@0|) (and (< 1 $FunctionContextHeight) (and ($IsGoodHeap $Heap@@0) ($Is refType |t#0@@0| Tclass._module.T)))) (= (_module.__default.B $Heap@@0 |t#0@@0|) ($Unbox HandleTypeType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap@@0 |t#0@@0|) _module.T.h))))
 :qid |Classesdfy.25:16|
 :skolemid |491|
 :pattern ( (_module.__default.B $Heap@@0 |t#0@@0|) ($IsGoodHeap $Heap@@0))
))))
...
(set-info :boogie-vc-id CheckWellformed$$_module.__default.B)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 6) (let ((anon5_Else_correct  (and (=> (= (ControlFlow 0 3) (- 0 4)) (or (not (= |t#0@@1| null)) (not true))) (=> (or (not (= |t#0@@1| null)) (not true)) (=> (and (and (= |b$reqreads#0@0| (U_2_bool (MapType1Select refType FieldType boolType $_ReadsFrame@0 |t#0@@1| _module.T.h))) (= (_module.__default.B $Heap@@1 |t#0@@1|) ($Unbox HandleTypeType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap@@1 |t#0@@1|) _module.T.h)))) (and ($Is HandleTypeType (_module.__default.B $Heap@@1 |t#0@@1|) (Tclass._System.___hFunc1 TInt TInt)) (= (ControlFlow 0 3) (- 0 2)))) |b$reqreads#0@0|)))))
(let ((anon5_Then_correct true))
(let ((anon0_correct  (=> (= $_ReadsFrame@0 (|lambda#1| null $Heap@@1 alloc |t#0@@1|)) (and (=> (= (ControlFlow 0 5) 1) anon5_Then_correct) (=> (= (ControlFlow 0 5) 3) anon5_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap@@1) ($IsHeapAnchor $Heap@@1)) ($Is refType |t#0@@1| Tclass._module.T)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct)))
