...
(set-info :boogie-vc-id Impl$$TwoStateFunctions.__default.Apply)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (= |y#0@0| (U_2_int ($Unbox intType (Apply1 TInt TInt $Heap |f#0@@3| ($Box intType (int_2_U |x#0|)))))) (= (ControlFlow 0 2) (- 0 1))) (= |y#0@0| (U_2_int ($Unbox intType (Apply1 TInt TInt $Heap |f#0@@3| ($Box intType (int_2_U |x#0|))))))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is HandleTypeType |f#0@@3| (Tclass._System.___hTotalFunc1 TInt TInt)) ($IsAlloc HandleTypeType |f#0@@3| (Tclass._System.___hTotalFunc1 TInt TInt) $Heap)) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
