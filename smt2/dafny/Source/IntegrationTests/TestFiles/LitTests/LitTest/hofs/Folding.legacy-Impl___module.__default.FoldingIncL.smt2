...
(set-info :boogie-vc-id Impl$$_module.__default.FoldingIncL)
...
(let ((anon9_Else_correct  (=> (and (forall ((|$l#12#ys#0| T@U) ) (!  (=> ($Is DatatypeTypeType |$l#12#ys#0| (Tclass._module.List TInt)) (and (|_module.__default.length#canCall| TInt |$l#12#ys#0|) (|_module.__default.length#canCall| TInt |xs#0@@9|)))
 :qid |unknown.0:0|
 :skolemid |761|
)) (= |inv#0@0| (Lit HandleTypeType (AtLayer HandleTypeType (|lambda#135| TInt TInt |xs#0@@9| (|lambda#7| TInt (Tclass._module.List TInt)) (|lambda#9| (SetRef_to_SetBox (|lambda#8| false)))) ($LS $LZ))))) (and (=> (= (ControlFlow 0 9) 8) anon11_Then_correct) (=> (= (ControlFlow 0 9) 3) anon11_Else_correct)))))
