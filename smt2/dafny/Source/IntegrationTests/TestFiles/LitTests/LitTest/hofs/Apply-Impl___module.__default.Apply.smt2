...
(set-info :boogie-vc-id Impl$$_module.__default.Apply)
...
(let ((GeneratedUnifiedExit_correct  (=> (= (ControlFlow 0 2) (- 0 1)) (= |i#0@1| |x#0|))))
(let ((anon5_Else_correct  (=> (= |i#0@0| (U_2_int ($Unbox intType (Apply1 TInt TInt $Heap (Lit HandleTypeType (AtLayer HandleTypeType (|lambda#5| (Handle1 |lambda#1| (|lambda#2| TInt) (|lambda#4| (SetRef_to_SetBox (|lambda#3| false))))) ($LS $LZ))) ($Box intType (int_2_U |x#0|)))))) (=> (and (= |i#0@1| |i#0@0|) (= (ControlFlow 0 3) 2)) GeneratedUnifiedExit_correct))))
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (and (=> (= (ControlFlow 0 7) 6) anon5_Then_correct) (=> (= (ControlFlow 0 7) 3) anon5_Else_correct)))))
