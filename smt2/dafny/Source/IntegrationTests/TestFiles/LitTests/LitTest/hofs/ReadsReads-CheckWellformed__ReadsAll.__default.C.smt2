...
(declare-fun ReadsAll.__default.C (T@U T@U) Int)
(declare-fun |ReadsAll.__default.C#canCall| (T@U T@U) Bool)
...
(assert  (=> (<= 0 $FunctionContextHeight) (forall (($Heap T@U) (|f#0| T@U) ) (!  (=> (or (|ReadsAll.__default.C#canCall| $Heap |f#0|) (and (< 0 $FunctionContextHeight) (and (and ($IsGoodHeap $Heap) ($Is HandleTypeType |f#0| (Tclass._System.___hFunc1 TInt TInt))) (forall ((|x#1| Int) ) (! (Requires1 TInt TInt $Heap |f#0| ($Box intType (int_2_U |x#1|)))
 :qid |ReadsReadsdfy.129:21|
 :skolemid |2066|
 :pattern ( (Requires1 TInt TInt $Heap |f#0| ($Box intType (int_2_U |x#1|))))
))))) (= (ReadsAll.__default.C $Heap |f#0|) (+ (+ (U_2_int ($Unbox intType (Apply1 TInt TInt $Heap |f#0| ($Box intType (int_2_U (LitInt 0)))))) (U_2_int ($Unbox intType (Apply1 TInt TInt $Heap |f#0| ($Box intType (int_2_U (LitInt 1))))))) (U_2_int ($Unbox intType (Apply1 TInt TInt $Heap |f#0| ($Box intType (int_2_U (LitInt 2)))))))))
 :qid |ReadsReadsdfy.127:18|
 :skolemid |2068|
 :pattern ( (ReadsAll.__default.C $Heap |f#0|) ($IsGoodHeap $Heap))
))))
...
(set-info :boogie-vc-id CheckWellformed$$ReadsAll.__default.C)
