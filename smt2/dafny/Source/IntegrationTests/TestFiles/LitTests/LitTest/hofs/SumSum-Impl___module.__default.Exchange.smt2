...
(set-info :boogie-vc-id Impl$$_module.__default.Exchange)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap@@2 alloc false)) (=> (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap@@2 $Heap@0)) (and (forall ((|$ih#n0#0| Int) ) (!  (=> (and (and (<= (LitInt 0) |$ih#n0#0|) (forall ((|i#2| Int) ) (!  (=> (and (<= (LitInt 0) |i#2|) (< |i#2| |$ih#n0#0|)) (= (U_2_int ($Unbox intType (Apply1 TInt TInt $Heap@@2 |f#0@@7| ($Box intType (int_2_U |i#2|))))) (U_2_int ($Unbox intType (Apply1 TInt TInt $Heap@@2 |g#0| ($Box intType (int_2_U |i#2|)))))))
 :qid |SumSumdfy.14:19|
 :skolemid |539|
 :pattern ( ($Unbox intType (Apply1 TInt TInt $Heap@@2 |g#0| ($Box intType (int_2_U |i#2|)))))
 :pattern ( ($Unbox intType (Apply1 TInt TInt $Heap@@2 |f#0@@7| ($Box intType (int_2_U |i#2|)))))
))) (and (<= 0 |$ih#n0#0|) (< |$ih#n0#0| |n#0@@3|))) (= (_module.__default.Sum ($LS $LZ) |$ih#n0#0| |f#0@@7|) (_module.__default.Sum ($LS $LZ) |$ih#n0#0| |g#0|)))
 :qid |SumSumdfy.13:16|
 :skolemid |540|
 :pattern ( (_module.__default.Sum ($LS $LZ) |$ih#n0#0| |g#0|))
 :pattern ( (_module.__default.Sum ($LS $LZ) |$ih#n0#0| |f#0@@7|))
)) (= (ControlFlow 0 2) (- 0 1)))) (= (_module.__default.Sum ($LS ($LS $LZ)) |n#0@@3| |f#0@@7|) (_module.__default.Sum ($LS ($LS $LZ)) |n#0@@3| |g#0|))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and (and ($IsGoodHeap $Heap@@2) ($IsHeapAnchor $Heap@@2)) (<= (LitInt 0) |n#0@@3|)) (and ($Is HandleTypeType |f#0@@7| (Tclass._System.___hTotalFunc1 TInt TInt)) ($IsAlloc HandleTypeType |f#0@@7| (Tclass._System.___hTotalFunc1 TInt TInt) $Heap@@2))) (and (and (and ($Is HandleTypeType |g#0| (Tclass._System.___hTotalFunc1 TInt TInt)) ($IsAlloc HandleTypeType |g#0| (Tclass._System.___hTotalFunc1 TInt TInt) $Heap@@2)) (= 1 $FunctionContextHeight)) (and (forall ((|i#1| T@U) ) (!  (=> (and (<= (LitInt 0) (U_2_int |i#1|)) (< (U_2_int |i#1|) |n#0@@3|)) (= (U_2_int ($Unbox intType (Apply1 TInt TInt $Heap@@2 |f#0@@7| ($Box intType |i#1|)))) (U_2_int ($Unbox intType (Apply1 TInt TInt $Heap@@2 |g#0| ($Box intType |i#1|))))))
 :qid |SumSumdfy.14:19|
 :skolemid |538|
 :pattern ( ($Unbox intType (Apply1 TInt TInt $Heap@@2 |g#0| ($Box intType |i#1|))))
 :pattern ( ($Unbox intType (Apply1 TInt TInt $Heap@@2 |f#0@@7| ($Box intType |i#1|))))
