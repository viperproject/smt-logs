...
axiom 0 <= $FunctionContextHeight
   ==> (forall s#0: ref :: 
    ...
    CSharpUtils.StringUtils.StringAsDafnyString#canCall(s#0)
         || (0 < $FunctionContextHeight && $Is(s#0, Tclass.System.String()))
...
procedure {:verboseName "CSharpUtils.StringUtils.StringAsDafnyString (well-formedness)"} {:extern} CheckWellformed$$CSharpUtils.StringUtils.StringAsDafnyString(s#0: ref where $Is(s#0, Tclass.System.String()));
...
axiom 0 <= $FunctionContextHeight
   ==> (forall CSharpUtils.ListUtils.FoldR$A: Ty, 
      CSharpUtils.ListUtils.FoldR$B: Ty, 
      f#0: HandleType, 
      b0#0: Box, 
      l#0: ref :: 
...
procedure {:verboseName "CSharpUtils.ListUtils.FoldR (well-formedness)"} {:extern} CheckWellformed$$CSharpUtils.ListUtils.FoldR(CSharpUtils.ListUtils.FoldR$A: Ty, 
    CSharpUtils.ListUtils.FoldR$B: Ty, 
    f#0: HandleType
       where $Is(f#0, 
        Tclass._System.___hTotalFunc2(CSharpUtils.ListUtils.FoldR$A, 
          CSharpUtils.ListUtils.FoldR$B, 
          CSharpUtils.ListUtils.FoldR$B)), 
    b0#0: Box where $IsBox(b0#0, CSharpUtils.ListUtils.FoldR$B), 
    l#0: ref
       where $Is(l#0, Tclass.System_mCollections_mGeneric.List(CSharpUtils.ListUtils.FoldR$A)));
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;
...
axiom {:id "id530"} (forall LinkedList._default.Concat$T: Ty, 
    $ly: LayerType, 
    l1#0: DatatypeType, 
    l2#0: DatatypeType :: 
  { LinkedList.__default.Concat(LinkedList._default.Concat$T, $LS($ly), l1#0, l2#0) } 
  LinkedList.__default.Concat#canCall(LinkedList._default.Concat$T, l1#0, l2#0)
     ...
     ==> (LinkedList.List.Cons_q(l1#0)
         ==> (var t#13 := LinkedList.List.tl(l1#0); 
          LinkedList.__default.Concat#canCall(LinkedList._default.Concat$T, t#13, l2#0)))
       && LinkedList.__default.Concat(LinkedList._default.Concat$T, $LS($ly), l1#0, l2#0)
         == (if LinkedList.List.Cons_q(l1#0)
           then (var t#12 := LinkedList.List.tl(l1#0); 
            (var h#12 := LinkedList.List.hd(l1#0); 
              #LinkedList.List.Cons(h#12, LinkedList.__default.Concat(LinkedList._default.Concat$T, $ly, t#12, l2#0))))
           else l2#0));

// definition axiom for LinkedList.__default.Concat for all literals (revealed)
axiom {:id "id531"} (forall LinkedList._default.Concat$T: Ty, 
    $ly: LayerType, 
    l1#0: DatatypeType, 
    l2#0: DatatypeType :: 
  ...
  LinkedList.__default.Concat#canCall(LinkedList._default.Concat$T, Lit(l1#0), Lit(l2#0))
     ...
     ==> (Lit(LinkedList.List.Cons_q(Lit(l1#0)))
         ==> (var t#15 := Lit(LinkedList.List.tl(Lit(l1#0))); 
          LinkedList.__default.Concat#canCall(LinkedList._default.Concat$T, t#15, Lit(l2#0))))
       && LinkedList.__default.Concat(LinkedList._default.Concat$T, $LS($ly), Lit(l1#0), Lit(l2#0))
         == (if LinkedList.List.Cons_q(Lit(l1#0))
           then (var t#14 := Lit(LinkedList.List.tl(Lit(l1#0))); 
            (var h#14 := Lit(LinkedList.List.hd(Lit(l1#0))); 
              Lit(#LinkedList.List.Cons(h#14, 
                  Lit(LinkedList.__default.Concat(LinkedList._default.Concat$T, $LS($ly), t#14, Lit(l2#0)))))))
           else l2#0));
