...
(declare-fun |m1#0| () Int)
(declare-fun |m2#0| () Int)
(declare-fun $IsGoodHeap (T@U) Bool)
(declare-fun $IsHeapAnchor (T@U) Bool)
(declare-fun |values#0| () T@U)
(declare-fun $FunctionContextHeight () Int)
(set-info :boogie-vc-id Impl$$_module.__default.MaximumIsUnique)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (= |m1#0| |m2#0|))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is SeqType |values#0| (TSeq TInt)) ($IsAlloc SeqType |values#0| (TSeq TInt) $Heap)) (and (= 0 $FunctionContextHeight) (|Seq#Contains| |values#0| ($Box intType (int_2_U |m1#0|))))) (and (and (forall ((|i#1| Int) ) (!  (=> (and (<= (LitInt 0) |i#1|) (< |i#1| (|Seq#Length| |values#0|))) (<= (U_2_int ($Unbox intType (|Seq#Index| |values#0| |i#1|))) |m1#0|))
 :qid |maximumdfy.25:35|
 :skolemid |494|
 :pattern ( ($Unbox intType (|Seq#Index| |values#0| |i#1|)))
)) (|Seq#Contains| |values#0| ($Box intType (int_2_U |m2#0|)))) (and (forall ((|i#3| Int) ) (!  (=> (and (<= (LitInt 0) |i#3|) (< |i#3| (|Seq#Length| |values#0|))) (<= (U_2_int ($Unbox intType (|Seq#Index| |values#0| |i#3|))) |m2#0|))
 :qid |maximumdfy.26:35|
 :skolemid |495|
 :pattern ( ($Unbox intType (|Seq#Index| |values#0| |i#3|)))
