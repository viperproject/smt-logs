...
procedure {:verboseName "NativeTypes.nativeint (well-formedness)"} CheckWellFormed$$NativeTypes.nativeint(n#0: int);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "NativeTypes.nativeint (well-formedness)"} CheckWellFormed$$NativeTypes.nativeint(n#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;


    // AddWellformednessCheck for newtype nativeint
    $_ReadsFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    if (*)
    {
        // check well-formedness of newtype constraint
        if (LitInt(-2147483648) <= n#0)
        {
        }

        assume {:id "id527"} LitInt(-2147483648) <= n#0 && n#0 < 2147483648;
        assume false;
    }
    ...
    if (*)
    {
        ...
        assume true;
        assert {:id "id528"} {:subsumption 0} LitInt(-2147483648) <= LitInt(0);
        assert {:id "id529"} {:subsumption 0} Lit(0 < 2147483648);
        assume false;
    }
}
