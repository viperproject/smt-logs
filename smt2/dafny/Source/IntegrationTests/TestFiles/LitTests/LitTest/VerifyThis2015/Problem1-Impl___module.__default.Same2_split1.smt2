...
(set-info :boogie-vc-id Impl$$_module.__default.Same2_split1)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon4_correct  (=> (not (and (not (|Seq#Equal| |a#0@@4| |Seq#Empty|)) (= (|Seq#Index| |pat#0@@4| (LitInt 0)) (|Seq#Index| |a#0@@4| (LitInt 0))))) (=> (and (<= 0 (LitInt 1)) (<= (LitInt 1) (|Seq#Length| |pat#0@@4|))) (=> (and (and (= |pat##1_1_0@0| (|Seq#Drop| |pat#0@@4| (LitInt 1))) (= (ControlFlow 0 2) (- 0 1))) (|_module.__default.IsRelaxedPrefixAux#canCall| _module._default.Same2$T |pat##1_1_0@0| |a#0@@4| (LitInt 0))) (or (_module.__default.IsRelaxedPrefixAux _module._default.Same2$T ($LS $LZ) |pat##1_1_0@0| |a#0@@4| (LitInt 0)) (=> (|Seq#Equal| |pat##1_1_0@0| |Seq#Empty|) (U_2_bool (Lit boolType (bool_2_U true))))))))))
