...
(set-info :boogie-vc-id Impl$$_module.__default.Same2_split58)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 2) (let ((anon0_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (and ($Is SeqType |pat#0@@4| (TSeq _module._default.Same2$T)) ($IsAlloc SeqType |pat#0@@4| (TSeq _module._default.Same2$T) $Heap)) (and ($Is SeqType |a#0@@4| (TSeq _module._default.Same2$T)) ($IsAlloc SeqType |a#0@@4| (TSeq _module._default.Same2$T) $Heap))) (and (and (=> |defass#x#1_0_1_0| (and ($IsBox |x#1_0_1_0| _module._default.Same2$T) ($IsAllocBox |x#1_0_1_0| _module._default.Same2$T $Heap))) true) (= 2 $FunctionContextHeight))) (=> (and (and (and (and (|_module.__default.IsRelaxedPrefixAux#canCall| _module._default.Same2$T |pat#0@@4| |a#0@@4| (LitInt 1)) (and (_module.__default.IsRelaxedPrefixAux _module._default.Same2$T ($LS $LZ) |pat#0@@4| |a#0@@4| (LitInt 1)) (ite (|Seq#Equal| |pat#0@@4| |Seq#Empty|) true (ite  (and (not (|Seq#Equal| |a#0@@4| |Seq#Empty|)) (= (|Seq#Index| |pat#0@@4| (LitInt 0)) (|Seq#Index| |a#0@@4| (LitInt 0)))) (_module.__default.IsRelaxedPrefixAux _module._default.Same2$T ($LS $LZ) (|Seq#Drop| |pat#0@@4| (LitInt 1)) (|Seq#Drop| |a#0@@4| (LitInt 1)) (LitInt 1))  (and (> 1 0) (_module.__default.IsRelaxedPrefixAux _module._default.Same2$T ($LS $LZ) (|Seq#Drop| |pat#0@@4| (LitInt 1)) |a#0@@4| (LitInt (- 1 1)))))))) (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false))) (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0))) (and (and (and (= $Heap $Heap@0) (forall ((|$ih#pat0#0| T@U) (|$ih#a0#0| T@U) ) (!  (=> (and (and (and ($Is SeqType |$ih#pat0#0| (TSeq _module._default.Same2$T)) ($Is SeqType |$ih#a0#0| (TSeq _module._default.Same2$T))) (_module.__default.IsRelaxedPrefixAux _module._default.Same2$T ($LS $LZ) |$ih#pat0#0| |$ih#a0#0| (LitInt 1))) (or (< (|Seq#Rank| |$ih#pat0#0|) (|Seq#Rank| |pat#0@@4|)) (and (= (|Seq#Rank| |$ih#pat0#0|) (|Seq#Rank| |pat#0@@4|)) (< (|Seq#Rank| |$ih#a0#0|) (|Seq#Rank| |a#0@@4|))))) (_module.__default.IRP__Alt _module._default.Same2$T |$ih#pat0#0| |$ih#a0#0|))
...
))) (and (not (|Seq#Equal| |pat#0@@4| |Seq#Empty|)) (not (|Seq#Equal| |a#0@@4| |Seq#Empty|)))) (and (and (<= 0 (LitInt 0)) (< (LitInt 0) (|Seq#Length| |pat#0@@4|))) (= (ControlFlow 0 2) (- 0 1))))) (and (<= 0 (LitInt 0)) (< (LitInt 0) (|Seq#Length| |a#0@@4|))))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
