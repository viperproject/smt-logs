...
(set-info :boogie-vc-id CheckWellformed$$AST.Expr__Raw.WellFormed)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 5) (let ((anon5_Else_correct  (=> (and ($IsAlloc DatatypeTypeType this@@1 Tclass.AST.Expr__Raw $Heap@@4) (= (ControlFlow 0 3) (- 0 2))) (or (not (= 2 $FunctionContextHeight)) (not true)))))
(let ((anon5_Then_correct true))
(let ((anon0_correct  (=> (= $_ReadsFrame@0 (|lambda#70| null $Heap@@4 alloc false)) (and (=> (= (ControlFlow 0 4) 1) anon5_Then_correct) (=> (= (ControlFlow 0 4) 3) anon5_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap@@4) ($IsHeapAnchor $Heap@@4)) (=> (and (and ($Is DatatypeTypeType this@@1 Tclass.AST.Expr__Raw) ($IsAlloc DatatypeTypeType this@@1 Tclass.AST.Expr__Raw $Heap@@4)) (and (= 3 $FunctionContextHeight) (= (ControlFlow 0 5) 4))) anon0_correct))))
