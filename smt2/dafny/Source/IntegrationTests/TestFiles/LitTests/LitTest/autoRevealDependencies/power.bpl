// dafny 4.9.2.0
// Command Line Options: /deprecation:0 /compile:0 /timeLimit:10 /print:/home/runner/work/smt-logs/smt-logs/dafny/../smt2/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/autoRevealDependencies/power.bpl /vcsCores:1 /proverLog:/home/runner/work/smt-logs/smt-logs/dafny/../smt2/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/autoRevealDependencies/power-@PROC@.smt2 /normalizeNames:0 /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/autoRevealDependencies/power.dfy
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "power (well-formedness)"} CheckWellformed$$_module.__default.power(b#0: int, n#0: int)
{
  ...
  var ##n#0: int;
    ...
    else
    {
        ...
        if (n#0 == LitInt(0))
        {
            assume {:id "id2"} _module.__default.power($LS($LZ), b#0, n#0) == LitInt(1);
            assume true;
        ...
        else
        {
            ...
            assert {:id "id3"} $Is(n#0 - 1, Tclass._System.nat());
            ##n#0 := n#0 - 1;
            // assume allocatedness for argument to function
            assume $IsAlloc(##n#0, Tclass._System.nat(), $Heap);
            assert {:id "id4"} 0 <= b#0 || ##b#0 == b#0;
            assert {:id "id5"} 0 <= n#0 || ##b#0 < b#0 || ##n#0 == n#0;
            assert {:id "id6"} ##b#0 < b#0 || (##b#0 == b#0 && ##n#0 < n#0);
            assume _module.__default.power#canCall(b#0, n#0 - 1);
            assume {:id "id7"} _module.__default.power($LS($LZ), b#0, n#0)
...
implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "test_power (well-formedness)"} CheckWellFormed$$_module.__default.test__power()
{
  ...
  var ##n#1: int;
    ...
    assert {:id "id8"} $Is(LitInt(2), Tclass._System.nat());
    ##n#0 := LitInt(2);
    // assume allocatedness for argument to function
    assume $IsAlloc(##n#0, Tclass._System.nat(), $Heap);
    assume _module.__default.power#canCall(LitInt(2), LitInt(2));
    ##b#0 := LitInt(_module.__default.power($LS($LZ), LitInt(2), LitInt(2)));
    // assume allocatedness for argument to function
    assume $IsAlloc(##b#0, TInt, $Heap);
    assert {:id "id9"} $Is(LitInt(2), Tclass._System.nat());
    ##n#1 := LitInt(2);
    // assume allocatedness for argument to function
    assume $IsAlloc(##n#1, Tclass._System.nat(), $Heap);
    assume _module.__default.power#canCall(LitInt(_module.__default.power($LS($LZ), LitInt(2), LitInt(2))), LitInt(2));
    assume {:id "id10"} LitInt(_module.__default.power($LS($LZ), 
          LitInt(_module.__default.power($LS($LZ), LitInt(2), LitInt(2))), 
          LitInt(2)))
       == LitInt(16);
}
...
procedure {:verboseName "test_power (call)"} Call$$_module.__default.test__power();
  ...
  free ensures _module.__default.power#canCall(LitInt(2), LitInt(2))
     && _module.__default.power#canCall(LitInt(_module.__default.power($LS($LZ), LitInt(2), LitInt(2))), LitInt(2));
  ensures {:id "id11"} LitInt(_module.__default.power($LS($LS($LZ)), 
        LitInt(_module.__default.power($LS($LS($LZ)), LitInt(2), LitInt(2))), 
        LitInt(2)))
     == LitInt(16);
...
procedure {:verboseName "test_power (correctness)"} Impl$$_module.__default.test__power() returns ($_reverifyPost: bool);
  ...
  free ensures _module.__default.power#canCall(LitInt(2), LitInt(2))
     && _module.__default.power#canCall(LitInt(_module.__default.power($LS($LZ), LitInt(2), LitInt(2))), LitInt(2));
  ensures {:id "id12"} LitInt(_module.__default.power($LS($LS($LZ)), 
        LitInt(_module.__default.power($LS($LS($LZ)), LitInt(2), LitInt(2))), 
        LitInt(2)))
     == LitInt(16);
  // frame condition: object granularity
