...
(assert (distinct TInt TagInt TagSeq alloc allocName class._module.Queue? Tagclass._module.Queue? Tagclass._module.Queue Tagclass._module.Client? Tagclass._module.Client tytagFamily$Queue tytagFamily$Client field$contents)
)
(assert (= (FDim alloc) 0))
...
(assert (forall (($o T@U) ) (! (= ($Is refType $o Tclass._module.Client?)  (or (= $o null) (= (dtype $o) Tclass._module.Client?)))
 :qid |unknown.0:0|
 :skolemid |572|
 :pattern ( ($Is refType $o Tclass._module.Client?))
)))
...
(assert (forall ((|c#0@@0| T@U) ($h@@0 T@U) ) (! (= ($IsAlloc refType |c#0@@0| Tclass._module.Client $h@@0) ($IsAlloc refType |c#0@@0| Tclass._module.Client? $h@@0))
 :qid |unknown.0:0|
 :skolemid |624|
 :pattern ( ($IsAlloc refType |c#0@@0| Tclass._module.Client $h@@0))
 :pattern ( ($IsAlloc refType |c#0@@0| Tclass._module.Client? $h@@0))
)))
...
(assert (forall (($o@@1 T@U) ($h@@2 T@U) ) (! (= ($IsAlloc refType $o@@1 Tclass._module.Client? $h@@2)  (or (= $o@@1 null) (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $h@@2 $o@@1) alloc)))))
 :qid |unknown.0:0|
 :skolemid |573|
 :pattern ( ($IsAlloc refType $o@@1 Tclass._module.Client? $h@@2))
)))
...
(assert (forall ((bx@@0 T@U) ) (!  (=> ($IsBox bx@@0 Tclass._module.Client?) (and (= ($Box refType ($Unbox refType bx@@0)) bx@@0) ($Is refType ($Unbox refType bx@@0) Tclass._module.Client?)))
 :qid |unknown.0:0|
 :skolemid |571|
 :pattern ( ($IsBox bx@@0 Tclass._module.Client?))
)))
(assert (forall ((bx@@1 T@U) ) (!  (=> ($IsBox bx@@1 Tclass._module.Client) (and (= ($Box refType ($Unbox refType bx@@1)) bx@@1) ($Is refType ($Unbox refType bx@@1) Tclass._module.Client)))
 :qid |unknown.0:0|
 :skolemid |574|
 :pattern ( ($IsBox bx@@1 Tclass._module.Client))
)))
...
(assert (forall ((|c#0@@2| T@U) ) (! (= ($Is refType |c#0@@2| Tclass._module.Client)  (and ($Is refType |c#0@@2| Tclass._module.Client?) (or (not (= |c#0@@2| null)) (not true))))
 :qid |unknown.0:0|
 :skolemid |623|
 :pattern ( ($Is refType |c#0@@2| Tclass._module.Client))
 :pattern ( ($Is refType |c#0@@2| Tclass._module.Client?))
)))
...
(declare-fun |r#0| () T@U)
(set-info :boogie-vc-id CheckWellFormed$$_module.Client.Sort)
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap@@2) ($IsHeapAnchor $Heap@@2)) (=> (and (and (and (or (not (= this@@3 null)) (not true)) (and ($Is refType this@@3 Tclass._module.Client) ($IsAlloc refType this@@3 Tclass._module.Client $Heap@@2))) (and ($Is refType |q#0| (Tclass._module.Queue TInt)) ($IsAlloc refType |q#0| (Tclass._module.Queue TInt) $Heap@@2))) (and (and ($Is refType |r#0| (Tclass._module.Queue TInt)) ($IsAlloc refType |r#0| (Tclass._module.Queue TInt) $Heap@@2)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 22) 18)))) anon0_correct))))
