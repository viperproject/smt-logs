...
(set-info :boogie-vc-id Impl$$Client__ProvideTypes.__default.M1)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and (=> |defass#d#0| (and ($IsBox |d#0@0| Tclass.Lib.D) ($IsAllocBox |d#0@0| Tclass.Lib.D $Heap))) (= (ControlFlow 0 2) (- 0 1))) |defass#d#0|))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and (=> |defass#d#0| (and ($IsBox |d#0| Tclass.Lib.D) ($IsAllocBox |d#0| Tclass.Lib.D $Heap))) true) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
