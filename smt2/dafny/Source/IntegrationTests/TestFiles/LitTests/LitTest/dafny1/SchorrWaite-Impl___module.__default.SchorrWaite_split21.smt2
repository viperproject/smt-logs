...
(set-info :boogie-vc-id Impl$$_module.__default.SchorrWaite_split21)
...
))) (= (ControlFlow 0 2) (- 0 1))) |$w$loop#0@0|))))) ($IsAllocBox ($Box DatatypeTypeType |path#0@0|) Tclass._module.Path $Heap)))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
