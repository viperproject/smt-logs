...
(set-info :boogie-vc-id Impl$$_module.__default.P46)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|ys#1@@0| T@U) ) (!  (=> ($Is DatatypeTypeType |ys#1@@0| Tclass._module.List) (|_module.PList#Equal| (_module.__default.zip ($LS ($LS $LZ)) (Lit DatatypeTypeType |#_module.List.Nil|) |ys#1@@0|) |#_module.PList.PNil|))
 :qid |Ripplinglegacydfy.535:18|
 :skolemid |1136|
 :pattern ( (_module.__default.zip ($LS ($LS $LZ)) |#_module.List.Nil| |ys#1@@0|))
)))))
