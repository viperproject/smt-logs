...
(set-info :boogie-vc-id CheckWellformed$$_module.__default.XInc)
...
(let ((anon8_Then_correct  (=> (and (= |t#0@@3| (|#_module.OneTree.Node| |_mcc#0#0| |_mcc#1#0|)) ($Is DatatypeTypeType |_mcc#1#0| Tclass._module.TreeList)) (=> (and (and (and (= |let#0#0#0| |_mcc#1#0|) ($Is DatatypeTypeType |let#0#0#0| Tclass._module.TreeList)) (and (= |children#Z#0@0| |let#0#0#0|) (= |let#1#0#0| |_mcc#0#0|))) (and (and ($Is intType (int_2_U |let#1#0#0|) TInt) (= |n#Z#0@0| |let#1#0#0|)) (and ($IsAlloc DatatypeTypeType |children#Z#0@0| Tclass._module.TreeList $Heap) (= (ControlFlow 0 3) (- 0 2))))) (< (DtRank |children#Z#0@0|) (DtRank |t#0@@3|))))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) ($Is DatatypeTypeType |t#0@@3| Tclass._module.OneTree)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 6) 5))) anon0_correct)))
