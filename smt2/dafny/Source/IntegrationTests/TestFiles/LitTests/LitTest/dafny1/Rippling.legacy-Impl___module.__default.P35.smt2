...
(set-info :boogie-vc-id Impl$$_module.__default.P35)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap@@2 alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|xs#1| T@U) ) (!  (=> ($Is DatatypeTypeType |xs#1| Tclass._module.List) (|_module.List#Equal| (_module.__default.dropWhileAlways ($LS ($LS $LZ)) (Lit HandleTypeType _module.__default.AlwaysFalseFunction) |xs#1|) |xs#1|))
 :qid |Ripplinglegacydfy.477:18|
 :skolemid |1059|
 :pattern ( (_module.__default.dropWhileAlways ($LS ($LS $LZ)) _module.__default.AlwaysFalseFunction |xs#1|))
)))))
