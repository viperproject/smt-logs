...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|n#1| T@U) (|xs#1| T@U) ) (!  (=> (and ($Is DatatypeTypeType |n#1| Tclass._module.Nat) ($Is DatatypeTypeType |xs#1| Tclass._module.List)) (|_module.Nat#Equal| (_module.__default.add ($LS ($LS $LZ)) (Lit DatatypeTypeType (|#_module.Nat.Suc| (Lit DatatypeTypeType |#_module.Nat.Zero|))) (_module.__default.count ($LS ($LS $LZ)) |n#1| |xs#1|)) (_module.__default.count ($LS ($LS $LZ)) |n#1| (|#_module.List.Cons| |n#1| |xs#1|))))
 :qid |Ripplinglegacydfy.318:18|
 :skolemid |838|
 :pattern ( (|#_module.List.Cons| |n#1| |xs#1|))
 :pattern ( (_module.__default.add ($LS ($LS $LZ)) (|#_module.Nat.Suc| |#_module.Nat.Zero|) (_module.__default.count ($LS ($LS $LZ)) |n#1| |xs#1|)))
)))))
