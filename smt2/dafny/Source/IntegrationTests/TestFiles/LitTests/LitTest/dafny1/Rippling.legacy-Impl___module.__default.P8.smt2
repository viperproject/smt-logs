...
(set-info :boogie-vc-id Impl$$_module.__default.P8)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|k#1| T@U) (|m#1| T@U) (|n#1| T@U) ) (!  (=> (and (and ($Is DatatypeTypeType |k#1| Tclass._module.Nat) ($Is DatatypeTypeType |m#1| Tclass._module.Nat)) ($Is DatatypeTypeType |n#1| Tclass._module.Nat)) (|_module.Nat#Equal| (_module.__default.minus ($LS ($LS $LZ)) (_module.__default.add ($LS ($LS $LZ)) |k#1| |m#1|) (_module.__default.add ($LS ($LS $LZ)) |k#1| |n#1|)) (_module.__default.minus ($LS ($LS $LZ)) |m#1| |n#1|)))
 :qid |Ripplinglegacydfy.340:18|
 :skolemid |866|
 :pattern ( (_module.__default.add ($LS ($LS $LZ)) |k#1| |n#1|) (_module.__default.add ($LS ($LS $LZ)) |k#1| |m#1|))
)))))
