...
(set-info :boogie-vc-id CheckWellformed$$_module.__default.GForestInc)
...
(let ((anon10_Then_correct  (=> (and (= |forest#0@@3| (|#_module.List.Cons| ($Box DatatypeTypeType |_mcc#0#0|) |_mcc#1#0|)) ($Is DatatypeTypeType |_mcc#0#0| (Tclass._module.GTree TInt))) (=> (and (and (and ($Is DatatypeTypeType |_mcc#1#0| (Tclass._module.List (Tclass._module.GTree TInt))) (= |let#0#0#0| |_mcc#1#0|)) (and ($Is DatatypeTypeType |let#0#0#0| (Tclass._module.List (Tclass._module.GTree TInt))) (= |tail#Z#0@0| |let#0#0#0|))) (and (and (= |let#1#0#0| |_mcc#0#0|) ($Is DatatypeTypeType |let#1#0#0| (Tclass._module.GTree TInt))) (and (= |tree#Z#0@0| |let#1#0#0|) ($IsAlloc DatatypeTypeType |tree#Z#0@0| (Tclass._module.GTree TInt) $Heap)))) (and (=> (= (ControlFlow 0 4) (- 0 5)) (< (DtRank |tree#Z#0@0|) (DtRank |forest#0@@3|))) (=> (< (DtRank |tree#Z#0@0|) (DtRank |forest#0@@3|)) (=> (and (and (|_module.__default.GInc#canCall| |tree#Z#0@0|) (_module.GTree.Node_q (_module.__default.GInc ($LS $LZ) |tree#Z#0@0|))) (and ($IsAlloc DatatypeTypeType |tail#Z#0@0| (Tclass._module.List (Tclass._module.GTree TInt)) $Heap) (= (ControlFlow 0 4) (- 0 3)))) (< (DtRank |tail#Z#0@0|) (DtRank |forest#0@@3|)))))))))
(let ((anon9_Else_correct  (=> (or (not (= |forest#0@@3| |#_module.List.Nil|)) (not true)) (and (=> (= (ControlFlow 0 7) 4) anon10_Then_correct) (=> (= (ControlFlow 0 7) 6) anon10_Else_correct)))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) ($Is DatatypeTypeType |forest#0@@3| (Tclass._module.List (Tclass._module.GTree TInt)))) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 9) 8))) anon0_correct)))
