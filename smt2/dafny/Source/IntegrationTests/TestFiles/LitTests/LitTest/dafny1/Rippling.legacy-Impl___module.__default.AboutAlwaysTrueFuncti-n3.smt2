...
(set-info :boogie-vc-id Impl$$_module.__default.AboutAlwaysTrueFunction)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|n#1| T@U) ) (!  (=> ($Is DatatypeTypeType |n#1| Tclass._module.Nat) (not (|_module.Nat#Equal| ($Unbox DatatypeTypeType (Apply1 Tclass._module.Nat Tclass._module.Nat $Heap (Lit HandleTypeType _module.__default.AlwaysTrueFunction) ($Box DatatypeTypeType |n#1|))) |#_module.Nat.Zero|)))
 :qid |Ripplinglegacydfy.294:18|
 :skolemid |811|
 :pattern ( ($Unbox DatatypeTypeType (Apply1 Tclass._module.Nat Tclass._module.Nat $Heap _module.__default.AlwaysTrueFunction ($Box DatatypeTypeType |n#1|))))
)))))
