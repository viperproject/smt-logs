...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|n#1| T@U) (|xs#1| T@U) (|ys#1@@0| T@U) ) (!  (=> (and (and ($Is DatatypeTypeType |n#1| Tclass._module.Nat) ($Is DatatypeTypeType |xs#1| Tclass._module.List)) ($Is DatatypeTypeType |ys#1@@0| Tclass._module.List)) (|_module.Nat#Equal| (_module.__default.add ($LS ($LS $LZ)) (_module.__default.count ($LS ($LS $LZ)) |n#1| |xs#1|) (_module.__default.count ($LS ($LS $LZ)) |n#1| |ys#1@@0|)) (_module.__default.count ($LS ($LS $LZ)) |n#1| (_module.__default.concat ($LS ($LS $LZ)) |xs#1| |ys#1@@0|))))
 :qid |Ripplinglegacydfy.308:18|
 :skolemid |824|
 :pattern ( (_module.__default.count ($LS ($LS $LZ)) |n#1| (_module.__default.concat ($LS ($LS $LZ)) |xs#1| |ys#1@@0|)))
 :pattern ( (_module.__default.add ($LS ($LS $LZ)) (_module.__default.count ($LS ($LS $LZ)) |n#1| |xs#1|) (_module.__default.count ($LS ($LS $LZ)) |n#1| |ys#1@@0|)))
)))))
