...
(set-info :boogie-vc-id Impl$$_module.__default.P18)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|i#1| T@U) (|m#1| T@U) ) (!  (=> (and ($Is DatatypeTypeType |i#1| Tclass._module.Nat) ($Is DatatypeTypeType |m#1| Tclass._module.Nat)) (|_module.Bool#Equal| (_module.__default.less ($LS ($LS $LZ)) |i#1| (|#_module.Nat.Suc| (_module.__default.add ($LS ($LS $LZ)) |i#1| |m#1|))) |#_module.Bool.True|))
 :qid |Ripplinglegacydfy.390:18|
 :skolemid |936|
 :pattern ( (_module.__default.add ($LS ($LS $LZ)) |i#1| |m#1|))
)))))
