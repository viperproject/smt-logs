...
(set-info :boogie-vc-id Impl$$_module.__default.P28)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|x#1| T@U) (|xs#1| T@U) ) (!  (=> (and ($Is DatatypeTypeType |x#1| Tclass._module.Nat) ($Is DatatypeTypeType |xs#1| Tclass._module.List)) (|_module.Bool#Equal| (_module.__default.mem ($LS ($LS $LZ)) |x#1| (_module.__default.concat ($LS ($LS $LZ)) |xs#1| (|#_module.List.Cons| |x#1| (Lit DatatypeTypeType |#_module.List.Nil|)))) |#_module.Bool.True|))
 :qid |Ripplinglegacydfy.442:18|
 :skolemid |1010|
 :pattern ( (_module.__default.concat ($LS ($LS $LZ)) |xs#1| (|#_module.List.Cons| |x#1| |#_module.List.Nil|)))
)))))
