...
(declare-fun $LZ () T@U)
(set-info :boogie-vc-id Impl$$_module.__default.SchorrWaite_split22)
...
)))) (and (and (and (forall ((|n#3| T@U) ) (!  (=> ($Is refType |n#3| Tclass._module.Node) (and (=> (|Set#IsMember| |S#0@@3| ($Box refType |n#3|)) (not (U_2_bool ($Unbox boolType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap@@2 |n#3|) _module.Node.marked))))) (=> (|Set#IsMember| |S#0@@3| ($Box refType |n#3|)) (= (U_2_int ($Unbox intType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap@@2 |n#3|) _module.Node.childrenVisited))) (LitInt 0)))))
...
)))))) (and (and (and (=> |$w$loop#0@0| ($IsAllocBox ($Box DatatypeTypeType |path#0@0|) Tclass._module.Path $Heap@@2)) (= (ControlFlow 0 2) (- 0 1))) |$w$loop#0@0|) (|_module.__default.ReachableVia#canCall| $Heap@@2 |root#0| |path#0@0| |root#0| |S#0@@3|))))) (or (_module.__default.ReachableVia ($LS $LZ) $Heap@@2 |root#0| |path#0@0| |root#0| |S#0@@3|) (=> (_module.Path.Empty_q |path#0@0|) (= |root#0| |root#0|))))))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
