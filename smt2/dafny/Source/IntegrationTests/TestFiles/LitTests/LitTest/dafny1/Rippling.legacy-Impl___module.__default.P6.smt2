...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|m#1| T@U) (|n#1| T@U) ) (!  (=> (and ($Is DatatypeTypeType |m#1| Tclass._module.Nat) ($Is DatatypeTypeType |n#1| Tclass._module.Nat)) (|_module.Nat#Equal| (_module.__default.minus ($LS ($LS $LZ)) |n#1| (_module.__default.add ($LS ($LS $LZ)) |n#1| |m#1|)) |#_module.Nat.Zero|))
 :qid |Ripplinglegacydfy.330:18|
 :skolemid |852|
 :pattern ( (_module.__default.add ($LS ($LS $LZ)) |n#1| |m#1|))
)))))
