...
(set-info :boogie-vc-id Impl$$_module.__default.P42)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|n#1| T@U) (|x#1| T@U) (|xs#1| T@U) ) (!  (=> (and (and ($Is DatatypeTypeType |n#1| Tclass._module.Nat) ($Is DatatypeTypeType |x#1| Tclass._module.Nat)) ($Is DatatypeTypeType |xs#1| Tclass._module.List)) (|_module.List#Equal| (_module.__default.take ($LS ($LS $LZ)) (|#_module.Nat.Suc| |n#1|) (|#_module.List.Cons| |x#1| |xs#1|)) (|#_module.List.Cons| |x#1| (_module.__default.take ($LS ($LS $LZ)) |n#1| |xs#1|))))
 :qid |Ripplinglegacydfy.513:18|
 :skolemid |1108|
 :pattern ( (|#_module.List.Cons| |x#1| (_module.__default.take ($LS ($LS $LZ)) |n#1| |xs#1|)))
 :pattern ( (_module.__default.take ($LS ($LS $LZ)) (|#_module.Nat.Suc| |n#1|) (|#_module.List.Cons| |x#1| |xs#1|)))
)))))
