...
(set-info :boogie-vc-id Impl$$_module.__default.P21)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|n#1| T@U) (|m#1| T@U) ) (!  (=> (and ($Is DatatypeTypeType |n#1| Tclass._module.Nat) ($Is DatatypeTypeType |m#1| Tclass._module.Nat)) (|_module.Bool#Equal| (_module.__default.leq ($LS ($LS $LZ)) |n#1| (_module.__default.add ($LS ($LS $LZ)) |n#1| |m#1|)) |#_module.Bool.True|))
 :qid |Ripplinglegacydfy.407:18|
 :skolemid |961|
