...
(set-info :boogie-vc-id Impl$$_module.__default.P9)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (forall ((|i#1| T@U) (|j#1| T@U) (|k#1| T@U) ) (!  (=> (and (and ($Is DatatypeTypeType |i#1| Tclass._module.Nat) ($Is DatatypeTypeType |j#1| Tclass._module.Nat)) ($Is DatatypeTypeType |k#1| Tclass._module.Nat)) (|_module.Nat#Equal| (_module.__default.minus ($LS ($LS $LZ)) (_module.__default.minus ($LS ($LS $LZ)) |i#1| |j#1|) |k#1|) (_module.__default.minus ($LS ($LS $LZ)) |i#1| (_module.__default.add ($LS ($LS $LZ)) |j#1| |k#1|))))
 :qid |Ripplinglegacydfy.345:18|
 :skolemid |873|
 :pattern ( (_module.__default.minus ($LS ($LS $LZ)) |i#1| (_module.__default.add ($LS ($LS $LZ)) |j#1| |k#1|)))
 :pattern ( (_module.__default.minus ($LS ($LS $LZ)) (_module.__default.minus ($LS ($LS $LZ)) |i#1| |j#1|) |k#1|))
)))))
