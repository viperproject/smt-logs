...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($Heap T@U) ) (!  (=> (and (or |M2.__default.EmptyEnv#canCall| (< 1 $FunctionContextHeight)) ($IsGoodHeap $Heap)) ($IsAllocBox M2.__default.EmptyEnv Tclass.M2.Env $Heap))
 :qid |CloudMakeParallelBuildsdfy.59:18|
 :skolemid |4355|
 :pattern ( ($IsAllocBox M2.__default.EmptyEnv Tclass.M2.Env $Heap))
))))
...
(set-info :boogie-vc-id CheckWellformed$$M2.__default.EmptyEnv)
...
(let ((anon4_Then_correct  (=> (and ($IsBox M2.__default.EmptyEnv Tclass.M2.Env) (= (ControlFlow 0 2) (- 0 1))) (U_2_bool (Lit boolType (bool_2_U true))))))
(let ((anon0_correct  (=> (= $_ReadsFrame@0 (|lambda#4| null $Heap@@0 alloc false)) (and (=> (= (ControlFlow 0 4) 2) anon4_Then_correct) (=> (= (ControlFlow 0 4) 3) anon4_Else_correct)))))
