...
(assert  (=> (<= 1 $FunctionContextHeight) (forall ((|p#0| T@U) ) (!  (=> (or (|M3.__default.LocInv__Exp#canCall| |p#0|) (and (< 1 $FunctionContextHeight) ($Is DatatypeTypeType |p#0| Tclass.M3.Path))) ($Is SeqType (M3.__default.LocInv__Exp |p#0|) (TSeq TChar)))
 :qid |CloudMakeParallelBuildsdfy.638:29|
 :skolemid |5691|
 :pattern ( (M3.__default.LocInv__Exp |p#0|))
))))
...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($Heap T@U) (|p#0@@0| T@U) ) (!  (=> (or (|M3.__default.LocInv__Exp#canCall| |p#0@@0|) (and (< 1 $FunctionContextHeight) (and ($IsGoodHeap $Heap) ($Is DatatypeTypeType |p#0@@0| Tclass.M3.Path)))) (and (=> (not (M3.Path.InternalPath_q |p#0@@0|)) |$let#17$canCall|) (= (M3.__default.LocInv__Exp |p#0@@0|) (ite (M3.Path.InternalPath_q |p#0@@0|) (let ((|exp#0| (M3.Path.exp |p#0@@0|)))
|exp#0|) (let ((|exp#1| |$let#17_exp|))
|exp#1|)))))
 :qid |CloudMakeParallelBuildsdfy.809:18|
 :skolemid |5693|
 :pattern ( (M3.__default.LocInv__Exp |p#0@@0|) ($IsGoodHeap $Heap))
))))
...
(assert  (=> (<= 1 $FunctionContextHeight) (forall (($Heap@@0 T@U) (|p#0@@1| T@U) ) (!  (=> (or (|M3.__default.LocInv__Exp#canCall| (Lit DatatypeTypeType |p#0@@1|)) (and (< 1 $FunctionContextHeight) (and ($IsGoodHeap $Heap@@0) ($Is DatatypeTypeType |p#0@@1| Tclass.M3.Path)))) (and (=> (not (U_2_bool (Lit boolType (bool_2_U (M3.Path.InternalPath_q (Lit DatatypeTypeType |p#0@@1|)))))) |$let#17$canCall|) (= (M3.__default.LocInv__Exp (Lit DatatypeTypeType |p#0@@1|)) (ite (M3.Path.InternalPath_q (Lit DatatypeTypeType |p#0@@1|)) (let ((|exp#3| (Lit SeqType (M3.Path.exp (Lit DatatypeTypeType |p#0@@1|)))))
|exp#3|) (let ((|exp#4| |$let#17_exp|))
|exp#4|)))))
 :qid |CloudMakeParallelBuildsdfy.809:18|
 :weight 3
 :skolemid |5694|
 :pattern ( (M3.__default.LocInv__Exp (Lit DatatypeTypeType |p#0@@1|)) ($IsGoodHeap $Heap@@0))
))))
...
(set-info :boogie-vc-id CheckWellformed$$M3.__default.LocInv__Exp)
...
(let ((anon13_Else_correct  (=> (and (not (and ($Is SeqType |exp#6@0| (TSeq TChar)) ($IsAlloc SeqType |exp#6@0| (TSeq TChar) $Heap@@1))) (= (ControlFlow 0 7) 5)) anon7_correct)))
(let ((anon13_Then_correct  (=> (and (and ($Is SeqType |exp#6@0| (TSeq TChar)) ($IsAlloc SeqType |exp#6@0| (TSeq TChar) $Heap@@1)) (= (ControlFlow 0 6) 5)) anon7_correct)))
