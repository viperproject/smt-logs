...
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.AssignArray)
...
(let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc |a#0|)) (and (=> (= (ControlFlow 0 9) 6) anon6_Then_correct) (=> (= (ControlFlow 0 9) 8) anon6_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and (<= (LitInt 0) |i#0|) (<= (LitInt 0) |j#0|))) (and (and ($Is refType |a#0| (Tclass._System.array TInt)) ($IsAlloc refType |a#0| (Tclass._System.array TInt) $Heap)) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 10) 9)))) anon0_correct)))
