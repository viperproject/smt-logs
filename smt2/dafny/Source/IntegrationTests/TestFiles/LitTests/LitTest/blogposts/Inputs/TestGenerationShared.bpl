// dafny 4.9.2.0
// Command Line Options: /deprecation:0 /compile:0 /timeLimit:10 /print:/home/runner/work/smt-logs/smt-logs/dafny/../smt2/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/blogposts/Inputs/TestGenerationShared.bpl /vcsCores:1 /proverLog:/home/runner/work/smt-logs/smt-logs/dafny/../smt2/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/blogposts/Inputs/TestGenerationShared-@PROC@.smt2 /normalizeNames:0 /home/runner/work/smt-logs/smt-logs/dafny/dafny/Source/IntegrationTests/TestFiles/LitTests/LitTest/blogposts/Inputs/TestGenerationShared.dfy
...
axiom 0 <= $FunctionContextHeight
   ==> (forall n#0: int :: 
    ...
    Shared.__default.abs#canCall(n#0) || 0 < $FunctionContextHeight
...
axiom {:id "id1"} 0 <= $FunctionContextHeight
   ==> (forall n#0: int :: 
    ...
    Shared.__default.abs#canCall(LitInt(n#0)) || 0 < $FunctionContextHeight
...
procedure {:verboseName "Shared.abs (well-formedness)"} CheckWellformed$$Shared.__default.abs(n#0: int);
  free requires 0 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Shared.abs (well-formedness)"} CheckWellformed$$Shared.__default.abs(n#0: int)
{
  var $_ReadsFrame: [ref,Field]bool;
    ...
    if (*)
    {
        ...
        assume LitInt(0) <= Shared.__default.abs(n#0);
        assume false;
    }
    else
    {
        ...
        if (n#0 > 0)
        {
            assert {:id "id2"} $Is(n#0, Tclass._System.nat());
            assume {:id "id3"} Shared.__default.abs(n#0) == n#0;
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(Shared.__default.abs(n#0), Tclass._System.nat());
            return;
        }
        else
        {
            assert {:id "id4"} $Is(0 - n#0, Tclass._System.nat());
            assume {:id "id5"} Shared.__default.abs(n#0) == 0 - n#0;
            assume true;
            // CheckWellformedWithResult: any expression
            assume $Is(Shared.__default.abs(n#0), Tclass._System.nat());
            return;
...
procedure {:verboseName "Shared.ChessPos (well-formedness)"} CheckWellFormed$$Shared.ChessPos(pos#0: DatatypeType);
  free requires 1 == $FunctionContextHeight;
  modifies $Heap;



implementation {:smt_option "smt.arith.solver", "2"} {:verboseName "Shared.ChessPos (well-formedness)"} CheckWellFormed$$Shared.ChessPos(pos#0: DatatypeType)
{
  var $_ReadsFrame: [ref,Field]bool;


    // AddWellformednessCheck for subset type ChessPos
    $_ReadsFrame := (lambda $o: ref, $f: Field :: 
      $o != null && $Unbox(read($Heap, $o, alloc)): bool ==> false);
    if (*)
    {
        // check well-formedness of subset type constraint
        assume $Is(pos#0, Tclass.Shared.Pos());
        assume Shared.Pos.Pos_q(pos#0);
        if (LitInt(0) <= Shared.Pos.row(pos#0))
        {
            assume Shared.Pos.Pos_q(pos#0);
        }

        assume {:id "id6"} LitInt(0) <= Shared.Pos.row(pos#0) && Shared.Pos.row(pos#0) < 8;
        assume Shared.Pos.Pos_q(pos#0);
        if (LitInt(0) <= Shared.Pos.col(pos#0))
        {
            assume Shared.Pos.Pos_q(pos#0);
        }

        assume {:id "id7"} LitInt(0) <= Shared.Pos.col(pos#0) && Shared.Pos.col(pos#0) < 8;
        assume false;
    }
    ...
    if (*)
    {
        ...
        assume Shared.Pos.Pos_q(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0))))
           && (LitInt(0) <= LitInt(Shared.Pos.row(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0)))))
             ==> Shared.Pos.Pos_q(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0))))
               && (Lit(Shared.Pos.row(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0)))) < 8)
                 ==> Shared.Pos.Pos_q(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0))))
                   && (LitInt(0) <= LitInt(Shared.Pos.col(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0)))))
                     ==> Shared.Pos.Pos_q(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0)))))));
        assert {:id "id8"} {:subsumption 0} LitInt(0) <= LitInt(Shared.Pos.row(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0)))));
        assert {:id "id9"} {:subsumption 0} Lit(Shared.Pos.row(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0)))) < 8);
        assert {:id "id10"} {:subsumption 0} LitInt(0) <= LitInt(Shared.Pos.col(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0)))));
        assert {:id "id11"} {:subsumption 0} Lit(Shared.Pos.col(Lit(#Shared.Pos.Pos(LitInt(0), LitInt(0)))) < 8);
        assume false;
    }
}
...
axiom (forall a#0#0#0: DatatypeType, a#0#1#0: DatatypeType :: 
  { #Shared.Piece.Piece(a#0#0#0, a#0#1#0) } 
  DatatypeCtorId(#Shared.Piece.Piece(a#0#0#0, a#0#1#0)) == ##Shared.Piece.Piece);
}
...
axiom (forall d: DatatypeType :: 
  ...
  Shared.Piece.Piece_q(d)
     ==> (exists a#1#0#0: DatatypeType, a#1#1#0: DatatypeType :: 
      d == #Shared.Piece.Piece(a#1#0#0, a#1#1#0)));
...
// Constructor $Is
axiom (forall a#2#0#0: DatatypeType, a#2#1#0: DatatypeType :: 
  { $Is(#Shared.Piece.Piece(a#2#0#0, a#2#1#0), Tclass.Shared.Piece()) } 
  $Is(#Shared.Piece.Piece(a#2#0#0, a#2#1#0), Tclass.Shared.Piece())
     <==> $Is(a#2#0#0, Tclass.Shared.PieceKind())
       && $Is(a#2#1#0, Tclass.Shared.ChessPos()));
...
axiom (forall a#3#0#0: DatatypeType, a#3#1#0: DatatypeType :: 
  { #Shared.Piece.Piece(Lit(a#3#0#0), Lit(a#3#1#0)) } 
  #Shared.Piece.Piece(Lit(a#3#0#0), Lit(a#3#1#0))
     == Lit(#Shared.Piece.Piece(a#3#0#0, a#3#1#0)));

function Shared.Piece.kind(DatatypeType) : DatatypeType;

// Constructor injectivity
axiom (forall a#4#0#0: DatatypeType, a#4#1#0: DatatypeType :: 
  { #Shared.Piece.Piece(a#4#0#0, a#4#1#0) } 
  Shared.Piece.kind(#Shared.Piece.Piece(a#4#0#0, a#4#1#0)) == a#4#0#0);

// Inductive rank
axiom (forall a#5#0#0: DatatypeType, a#5#1#0: DatatypeType :: 
  { #Shared.Piece.Piece(a#5#0#0, a#5#1#0) } 
  DtRank(a#5#0#0) < DtRank(#Shared.Piece.Piece(a#5#0#0, a#5#1#0)));

function Shared.Piece.at(DatatypeType) : DatatypeType;

// Constructor injectivity
axiom (forall a#6#0#0: DatatypeType, a#6#1#0: DatatypeType :: 
  { #Shared.Piece.Piece(a#6#0#0, a#6#1#0) } 
  Shared.Piece.at(#Shared.Piece.Piece(a#6#0#0, a#6#1#0)) == a#6#1#0);

// Inductive rank
axiom (forall a#7#0#0: DatatypeType, a#7#1#0: DatatypeType :: 
  { #Shared.Piece.Piece(a#7#0#0, a#7#1#0) } 
  DtRank(a#7#1#0) < DtRank(#Shared.Piece.Piece(a#7#0#0, a#7#1#0)));
...
procedure {:verboseName "Shared.Piece.Attacks (well-formedness)"} CheckWellformed$$Shared.Piece.Attacks(this: DatatypeType
       where $Is(this, Tclass.Shared.Piece()) && $IsAlloc(this, Tclass.Shared.Piece(), $Heap), 
    pos#0: DatatypeType where $Is(pos#0, Tclass.Shared.ChessPos()));
