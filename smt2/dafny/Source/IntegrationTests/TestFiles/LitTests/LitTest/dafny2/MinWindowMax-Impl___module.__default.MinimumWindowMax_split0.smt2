...
(set-info :boogie-vc-id Impl$$_module.__default.MinimumWindowMax_split0)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 2) (let ((anon0_correct  (=> (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is refType |a#0| (Tclass._System.array TInt)) ($IsAlloc refType |a#0| (Tclass._System.array TInt) $Heap))) (=> (and (and (and (and ($Is refType |b#0| (Tclass._System.array TInt)) ($IsAlloc refType |b#0| (Tclass._System.array TInt) $Heap)) true) (= 3 $FunctionContextHeight)) (and (and (<= (LitInt 1) |W#0|) (<= |W#0| (_System.array.Length |a#0|))) (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))))) (or (not (= |a#0| null)) (not true))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
