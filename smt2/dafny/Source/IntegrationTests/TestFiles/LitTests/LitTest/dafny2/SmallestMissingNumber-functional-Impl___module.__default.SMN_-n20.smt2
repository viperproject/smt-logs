...
(set-info :boogie-vc-id Impl$$_module.__default.SMN_k_k__Correct_split0)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 2) (let ((anon0_correct  (=> (and ($IsGoodHeap $Heap@@0) ($IsHeapAnchor $Heap@@0)) (=> (and (and (and ($Is DatatypeTypeType |xs#0@@11| (Tclass._module.List Tclass._System.nat)) ($IsAlloc DatatypeTypeType |xs#0@@11| (Tclass._module.List Tclass._System.nat) $Heap@@0)) (|$IsA#_module.List| |xs#0@@11|)) (and (<= (LitInt 0) |n#0|) (<= (LitInt 0) |len#0|))) (=> (and (and (and (and ($Is SetType |bound#1_0| (TSet Tclass._System.nat)) ($IsAlloc SetType |bound#1_0| (TSet Tclass._System.nat) $Heap@@0)) true) (= 4 $FunctionContextHeight)) (and (and (and (|_module.__default.NoDuplicates#canCall| Tclass._System.nat |xs#0@@11|) (and (_module.__default.NoDuplicates Tclass._System.nat ($LS $LZ) |xs#0@@11|) (ite (_module.List.Nil_q |xs#0@@11|) true (let ((|tail#3@@2| (_module.List.tail |xs#0@@11|)))
(let ((|x#7| (U_2_int ($Unbox intType (_module.List.head |xs#0@@11|)))))
 (and (not (|Set#IsMember| (_module.__default.Elements Tclass._System.nat ($LS $LZ) |tail#3@@2|) ($Box intType (int_2_U |x#7|)))) (_module.__default.NoDuplicates Tclass._System.nat ($LS $LZ) |tail#3@@2|))))))) (and (forall ((|x#1@@0| T@U) ) (!  (=> (|Set#IsMember| (_module.__default.Elements Tclass._System.nat ($LS $LZ) |xs#0@@11|) ($Box intType |x#1@@0|)) (<= |n#0| (U_2_int |x#1@@0|)))
...
)) (= |len#0| (_module.__default.Length Tclass._System.nat ($LS ($LS $LZ)) |xs#0@@11|)))) (and (and (= $_ModifiesFrame@0 (|lambda#0| null $Heap@@0 alloc false)) (|$IsA#_module.List| |xs#0@@11|)) (and (not (|_module.List#Equal| |xs#0@@11| |#_module.List.Nil|)) (= (ControlFlow 0 2) (- 0 1)))))) (or (not (= (LitInt 2) 0)) (not true)))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
