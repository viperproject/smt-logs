...
(set-info :boogie-vc-id Impl$$SnapTree.Tree.IsSortedProperty)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#1| null $Heap alloc false)) (=> (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap $Heap@0)) (and reveal_SnapTree.Tree.IsSorted (= (ControlFlow 0 2) (- 0 1)))) (= (SnapTree.Tree.IsSorted reveal_SnapTree.Tree.IsSorted |c#0@@1|) (forall ((|i#1@@0| Int) (|j#1@@0| Int) ) (!  (=> (and (and (<= (LitInt 0) |i#1@@0|) (< |i#1@@0| |j#1@@0|)) (< |j#1@@0| (|Seq#Length| |c#0@@1|))) (< (U_2_int ($Unbox intType (|Seq#Index| |c#0@@1| |i#1@@0|))) (U_2_int ($Unbox intType (|Seq#Index| |c#0@@1| |j#1@@0|)))))
 :qid |SnapshotableTreesdfy.115:39|
 :skolemid |694|
 :pattern ( ($Unbox intType (|Seq#Index| |c#0@@1| |j#1@@0|)) ($Unbox intType (|Seq#Index| |c#0@@1| |i#1@@0|)))
)))))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is SeqType |c#0@@1| (TSeq TInt)) ($IsAlloc SeqType |c#0@@1| (TSeq TInt) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
