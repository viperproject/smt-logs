...
(set-info :boogie-vc-id Impl$$_module.__default.MinimumWindowMax_split74)
...
))))))) (and (and (and (and ($IsGoodHeap $Heap@3) ($IsHeapAnchor $Heap@3)) (|_module.__default.Max#canCall| $Heap@3 |a#0@@4| |lo##0@0| (- |n#0@1| |lo##0@0|))) (and (= (U_2_int ($Unbox intType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap@3 |a#0@@4|) (IndexField |x##0@0|)))) (_module.__default.Max ($LS ($LS $LZ)) $Heap@3 |a#0@@4| |lo##0@0| (- |n#0@1| |lo##0@0|))) (= $Heap@2 $Heap@3))) (and (and (and (= |start#0@0| (LitInt 0)) (= |$decr_init$loop#10@0| (- (_System.array.Length |a#0@@4|) |n#0@1|))) (and (=> |$w$loop#1@0| (<= (LitInt 0) |$rhs#1@0|)) (=> |$w$loop#1@0| (< |$rhs#1@0| |l#0@0|)))) (and (and (=> |$w$loop#1@0| (<= |l#0@0| |n#0@1|)) (= (ControlFlow 0 2) (- 0 1))) |$w$loop#1@0|)))))) (<= |n#0@1| (_System.array.Length |a#0@@4|))))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
