...
(declare-fun |W#0| () Int)
(set-info :boogie-vc-id CheckWellFormed$$_module.__default.MinimumWindowMax_split8)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 2) (let ((anon0_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is refType |a#0| (Tclass._System.array TInt)) ($IsAlloc refType |a#0| (Tclass._System.array TInt) $Heap))) (and (and (= 3 $FunctionContextHeight) (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false))) (and (<= (LitInt 1) |W#0|) (= (ControlFlow 0 2) (- 0 1))))) (or (not (= |a#0| null)) (not true)))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
