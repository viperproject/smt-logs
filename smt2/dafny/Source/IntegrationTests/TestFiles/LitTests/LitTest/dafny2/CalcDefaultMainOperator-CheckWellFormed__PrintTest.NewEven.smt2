...
(declare-fun |ww#Z#0@0| () Int)
(declare-fun |let#0#0#0| () Int)
(declare-fun |rr#Z#0@0| () Int)
...
(set-info :boogie-vc-id CheckWellFormed$$PrintTest.NewEven)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 6) (let ((anon6_Then_correct  (=> (and (and (= |let#1#0#0| (LitInt 2)) ($Is intType (int_2_U |let#1#0#0|) TInt)) (and (= |ww#Z#0@0| |let#1#0#0|) (= (ControlFlow 0 4) (- 0 3)))) (let ((|rr#1| (LitInt 2)))
(= (Mod (let ((|ww#0| (LitInt 2)))
(LitInt (+ |ww#0| 8))) |rr#1|) (LitInt 0))))))
(let ((anon5_Then_correct  (=> (and (and (= |let#0#0#0| (LitInt 2)) ($Is intType (int_2_U |let#0#0#0|) TInt)) (and (= |rr#Z#0@0| |let#0#0#0|) (= (ControlFlow 0 2) (- 0 1)))) (or (not (= |rr#Z#0@0| 0)) (not true)))))
