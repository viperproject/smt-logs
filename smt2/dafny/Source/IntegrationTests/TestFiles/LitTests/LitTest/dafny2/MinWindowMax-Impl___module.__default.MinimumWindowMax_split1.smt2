...
(set-info :boogie-vc-id Impl$$_module.__default.MinimumWindowMax_split1)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 2) (let ((anon0_correct  (=> (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and ($Is refType |a#0| (Tclass._System.array TInt)) ($IsAlloc refType |a#0| (Tclass._System.array TInt) $Heap))) (=> (and (and (and (and ($Is refType |b#0| (Tclass._System.array TInt)) ($IsAlloc refType |b#0| (Tclass._System.array TInt) $Heap)) true) (and (= 3 $FunctionContextHeight) (<= (LitInt 1) |W#0|))) (and (and (<= |W#0| (_System.array.Length |a#0|)) (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false))) (and (or (not (= |a#0| null)) (not true)) (= (ControlFlow 0 2) (- 0 1))))) (<= 0 (_System.array.Length |a#0|))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
