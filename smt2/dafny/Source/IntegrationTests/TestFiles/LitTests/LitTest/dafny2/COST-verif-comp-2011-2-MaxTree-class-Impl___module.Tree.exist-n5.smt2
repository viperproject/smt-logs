...
(set-info :boogie-vc-id Impl$$_module.Tree.exists__intro)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (and (= $_ModifiesFrame@0 (|lambda#2| null $Heap alloc false)) (= (ControlFlow 0 2) (- 0 1))) (exists ((|y#1| T@U) ) (!  (and (and ($IsBox |y#1| _module.Tree.exists_intro$T) ($IsAllocBox |y#1| _module.Tree.exists_intro$T $Heap)) (and (Requires1 _module.Tree.exists_intro$T TBool $Heap |P#0| |y#1|) (U_2_bool ($Unbox boolType (Apply1 _module.Tree.exists_intro$T TBool $Heap |P#0| |y#1|)))))
 :qid |COSTverifcomp20112MaxTreeclassdfy.121:20|
 :skolemid |517|
 :pattern ( ($Unbox boolType (Apply1 _module.Tree.exists_intro$T TBool $Heap |P#0| |y#1|)))
 :pattern ( (Requires1 _module.Tree.exists_intro$T TBool $Heap |P#0| |y#1|))
)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (and (or (not (= this null)) (not true)) (and ($Is refType this Tclass._module.Tree) ($IsAlloc refType this Tclass._module.Tree $Heap)))) (=> (and (and (and ($Is HandleTypeType |P#0| (Tclass._System.___hFunc1 _module.Tree.exists_intro$T TBool)) ($IsAlloc HandleTypeType |P#0| (Tclass._System.___hFunc1 _module.Tree.exists_intro$T TBool) $Heap)) (and ($IsBox |x#0| _module.Tree.exists_intro$T) ($IsAllocBox |x#0| _module.Tree.exists_intro$T $Heap))) (and (and (= 0 $FunctionContextHeight) (Requires1 _module.Tree.exists_intro$T TBool $Heap |P#0| |x#0|)) (and (U_2_bool ($Unbox boolType (Apply1 _module.Tree.exists_intro$T TBool $Heap |P#0| |x#0|))) (= (ControlFlow 0 3) 2)))) anon0_correct))))
