...
(set-info :boogie-vc-id Impl$$_module.__default.SMN_k_k__Correct_split35)
...
(let ((|x#4| (U_2_int ($Unbox intType (_module.List.head |L#1_0@0|)))))
 (not (|Set#IsMember| (_module.__default.Elements Tclass._System.nat ($LS ($LS $LZ)) |tail#0@@7|) ($Box intType (int_2_U |x#4|))))))))) (= (ControlFlow 0 2) (- 0 1))) (|_module.__default.NoDuplicates#canCall| Tclass._System.nat |L#1_0@0|)))))) (or (_module.__default.NoDuplicates Tclass._System.nat ($LS $LZ) |L#1_0@0|) (=> (not (_module.List.Nil_q |L#1_0@0|)) (let ((|tail#0@@8| (_module.List.tail |L#1_0@0|)))
(_module.__default.NoDuplicates Tclass._System.nat ($LS ($LS $LZ)) |tail#0@@8|)))))))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
