...
(declare-fun |s#0| () T@U)
(set-info :boogie-vc-id Impl$$SnapTree.Tree.SmallIsSorted)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#1| null $Heap alloc false)) (=> (and (and (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (= $Heap $Heap@0)) (and (and reveal_SnapTree.Tree.IsSorted (= (ControlFlow 0 2) (- 0 1))) (<= (|Seq#Length| |s#0|) (LitInt 1)))) (SnapTree.Tree.IsSorted reveal_SnapTree.Tree.IsSorted |s#0|)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) (=> (and (and ($Is SeqType |s#0| (TSeq TInt)) ($IsAlloc SeqType |s#0| (TSeq TInt) $Heap)) (and (= 1 $FunctionContextHeight) (= (ControlFlow 0 3) 2))) anon0_correct))))
