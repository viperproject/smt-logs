...
(assert  (=> (<= 0 $FunctionContextHeight) (forall ((|d#0| Int) (|s#0| T@U) ) (!  (=> (or (|SnapTree.Tree.AllAbove#canCall| |d#0| |s#0|) (and (< 0 $FunctionContextHeight) ($Is SeqType |s#0| (TSeq TInt)))) (= (SnapTree.Tree.AllAbove |d#0| |s#0|) (forall ((|i#0| Int) ) (!  (=> (and (<= (LitInt 0) |i#0|) (< |i#0| (|Seq#Length| |s#0|))) (< |d#0| (U_2_int ($Unbox intType (|Seq#Index| |s#0| |i#0|)))))
...
(assert  (=> (<= 0 $FunctionContextHeight) (forall ((|d#0@@0| Int) (|s#0@@0| T@U) ) (!  (=> (or (|SnapTree.Tree.AllAbove#canCall| (LitInt |d#0@@0|) (Lit SeqType |s#0@@0|)) (and (< 0 $FunctionContextHeight) ($Is SeqType |s#0@@0| (TSeq TInt)))) (= (SnapTree.Tree.AllAbove (LitInt |d#0@@0|) (Lit SeqType |s#0@@0|)) (forall ((|i#1| Int) ) (!  (=> (and (<= (LitInt 0) |i#1|) (< |i#1| (|Seq#Length| (Lit SeqType |s#0@@0|)))) (< |d#0@@0| (U_2_int ($Unbox intType (|Seq#Index| (Lit SeqType |s#0@@0|) |i#1|)))))
 :qid |SnapshotableTreesdfy.130:14|
 :skolemid |709|
...
(set-info :boogie-vc-id CheckWellformed$$SnapTree.Tree.AllAbove)
...
(let ((anon13_Then_correct  (=> (and (<= (LitInt 0) |i#2@0|) (< |i#2@0| (|Seq#Length| |s#0@@1|))) (and (=> (= (ControlFlow 0 4) (- 0 5)) (and (<= 0 |i#2@0|) (< |i#2@0| (|Seq#Length| |s#0@@1|)))) (=> (and (<= 0 |i#2@0|) (< |i#2@0| (|Seq#Length| |s#0@@1|))) (=> (= (ControlFlow 0 4) 3) anon8_correct))))))
...
(let ((anon0_correct  (=> (= $_ReadsFrame@0 (|lambda#1| null $Heap alloc false)) (and (and (=> (= (ControlFlow 0 10) 1) anon10_Then_correct) (=> (= (ControlFlow 0 10) 9) anon11_Then_correct)) (=> (= (ControlFlow 0 10) 2) anon11_Else_correct)))))
(let ((PreconditionGeneratedEntry_correct  (=> (and (and (and ($IsGoodHeap $Heap) ($IsHeapAnchor $Heap)) ($Is SeqType |s#0@@1| (TSeq TInt))) (and (= 0 $FunctionContextHeight) (= (ControlFlow 0 11) 10))) anon0_correct)))
