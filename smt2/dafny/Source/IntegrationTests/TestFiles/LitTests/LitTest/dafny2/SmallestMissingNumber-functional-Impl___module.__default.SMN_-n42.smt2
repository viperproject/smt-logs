...
(declare-fun |n##1_1_0@0| () Int)
(declare-fun |len##1_1_0@0| () Int)
(set-info :boogie-vc-id Impl$$_module.__default.SMN_k_k__Correct_split22)
...
(let ((|x#7@@0| (U_2_int ($Unbox intType (_module.List.head ($Unbox DatatypeTypeType (_System.Tuple2._0 |r#0@@3|)))))))
 (and (not (|Set#IsMember| (_module.__default.Elements Tclass._System.nat ($LS $LZ) |tail#3@@4|) ($Box intType (int_2_U |x#7@@0|)))) (_module.__default.NoDuplicates Tclass._System.nat ($LS $LZ) |tail#3@@4|)))))))))) (and (and (and (let ((|r#0@@4| (_module.__default.Split ($LS ($LS $LZ)) reveal__module._default.Split |xs#0@@20| |b##1_0@0|)))
...
(_module.__default.NoDuplicates Tclass._System.nat ($LS ($LS $LZ)) |tail#0@@8|)))))) (and (forall ((|x#1@@1| T@U) ) (!  (=> (|Set#IsMember| (_module.__default.Elements Tclass._System.nat ($LS ($LS $LZ)) |R#1_0@0|) ($Box intType |x#1@@1|)) (<= |n##1_1_0@0| (U_2_int |x#1@@1|)))
 :qid |SmallestMissingNumberfunctionaldfy.274:19|
 :skolemid |739|
 :pattern ( (|Set#IsMember| (_module.__default.Elements Tclass._System.nat ($LS ($LS $LZ)) |R#1_0@0|) ($Box intType |x#1@@1|)))
)) (= (ControlFlow 0 2) (- 0 1)))))))) (= |len##1_1_0@0| (_module.__default.Length Tclass._System.nat ($LS ($LS $LZ)) |R#1_0@0|)))))))))
anon0_correct))
))
(check-sat)
(get-info :rlimit)
