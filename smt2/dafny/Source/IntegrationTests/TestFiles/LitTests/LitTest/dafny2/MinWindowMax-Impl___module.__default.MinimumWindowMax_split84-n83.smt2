...
(set-info :boogie-vc-id Impl$$_module.__default.MinimumWindowMax_split84)
...
))) (=> |$w$loop#1@0| (= (U_2_int ($Unbox intType (MapType0Select FieldType BoxType (MapType0Select refType (MapType0Type FieldType BoxType) $Heap@3 $nw@0) (IndexField (- |l#0@0| 1))))) (- |n#0@1| 1)))) (and (=> |$w$loop#1@0| (<= (LitInt 0) |start#0@0|)) (=> |$w$loop#1@0| (<= |start#0@0| (- (_System.array.Length |a#0@@4|) |W#0|))))) (and (and (=> |$w$loop#1@0| (= |m#0@0| (_module.__default.Max ($LS ($LS $LZ)) $Heap@3 |a#0@@4| |start#0@0| |W#0|))) (= (ControlFlow 0 2) (- 0 1))) |$w$loop#1@0|))))) (forall ((|s#3| Int) ) (!  (=> (and (<= (LitInt 0) |s#3|) (<= |s#3| (- |n#0@1| |W#0|))) (<= |m#0@0| (_module.__default.Max ($LS ($LS $LZ)) $Heap@3 |a#0@@4| |s#3| |W#0|)))
