...
(declare-fun $LZ () T@U)
(set-info :boogie-vc-id Impl$$_module.__default.OhOnes__Correct)
...
(set-option :smt.arith.solver 2)
(assert (not
 (=> (= (ControlFlow 0 0) 3) (let ((anon0_correct  (=> (= $_ModifiesFrame@0 (|lambda#0| null $Heap alloc false)) (=> (and ($IsGoodHeap $Heap@0) ($IsHeapAnchor $Heap@0)) (=> (and (and (and (and (|$IsA#_module.Stream| (Lit DatatypeTypeType (_module.Stream.tail (Lit DatatypeTypeType (_module.__default.OhOnes ($LS $LZ)))))) (|$IsA#_module.Stream| (Lit DatatypeTypeType (_module.__default.ones ($LS $LZ))))) (and (and |_module.__default.OhOnes#canCall| (_module.Stream.Cons_q (Lit DatatypeTypeType (_module.__default.OhOnes ($LS $LZ))))) |_module.__default.ones#canCall|)) (|$Eq#_module.Stream| TInt TInt ($LS ($LS $LZ)) (_module.Stream.tail (Lit DatatypeTypeType (_module.__default.OhOnes ($LS ($LS $LZ))))) (_module.__default.ones ($LS ($LS $LZ))))) (and (= $Heap $Heap@0) (= (ControlFlow 0 2) (- 0 1)))) (|$Eq#_module.Stream| TInt TInt ($LS ($LS $LZ)) (_module.__default.OhOnes ($LS ($LS $LZ))) (|#_module.Stream.Cons| ($Box intType (int_2_U (LitInt 0))) (Lit DatatypeTypeType (_module.__default.ones ($LS ($LS $LZ)))))))))))
