...
(assert (distinct TBool TInt TagBool TagInt alloc Tagclass._System.nat Tagclass._module.Stream Tagclass._module.Tree |##_module.Stream.Nil| |##_module.Stream.Cons| |##_module.Tree.Node| tytagFamily$nat tytagFamily$Stream tytagFamily$Tree)
)
...
(set-info :boogie-vc-id CheckWellformed$$_module.__default.ValidPath)
...
(let ((anon15_Else_correct  (=> (and (not (_module.Stream.Cons_q |ch#Z#0@0|)) (= (ControlFlow 0 8) 4)) anon8_correct)))
(let ((anon15_Then_correct  (=> (_module.Stream.Cons_q |ch#Z#0@0|) (and (=> (= (ControlFlow 0 6) (- 0 7)) (_module.Stream.Cons_q |ch#Z#0@0|)) (=> (_module.Stream.Cons_q |ch#Z#0@0|) (=> (= |##t#0@0| ($Unbox DatatypeTypeType (_module.Stream.head |ch#Z#0@0|))) (=> (and (and ($IsAlloc DatatypeTypeType |##t#0@0| Tclass._module.Tree $Heap@@0) ($IsAlloc DatatypeTypeType |tail#Z#0@0| (Tclass._module.Stream TInt) $Heap@@0)) (and (|_module.__default.ValidPath#canCall| ($Unbox DatatypeTypeType (_module.Stream.head |ch#Z#0@0|)) |tail#Z#0@0|) (= (ControlFlow 0 6) 4))) anon8_correct)))))))
(let ((anon14_Then_correct  (=> (and (and (<= (LitInt 0) |index#Z#0@0|) (_module.Tree.Node_q |t#0@@10|)) (and (= |##s#0@0| (_module.Tree.children |t#0@@10|)) ($IsAlloc DatatypeTypeType |##s#0@0| (Tclass._module.Stream Tclass._module.Tree) $Heap@@0))) (and (=> (= (ControlFlow 0 9) (- 0 10)) ($Is intType (int_2_U |index#Z#0@0|) Tclass._System.nat)) (=> ($Is intType (int_2_U |index#Z#0@0|) Tclass._System.nat) (=> ($IsAlloc intType (int_2_U |index#Z#0@0|) Tclass._System.nat $Heap@@0) (=> (and (|_module.__default.Tail#canCall| Tclass._module.Tree (_module.Tree.children |t#0@@10|) |index#Z#0@0|) (= |let#2#0#0| (_module.__default.Tail Tclass._module.Tree ($LS $LZ) (_module.Tree.children |t#0@@10|) |index#Z#0@0|))) (=> (and (and (_module.Tree.Node_q |t#0@@10|) (|_module.__default.Tail#canCall| Tclass._module.Tree (_module.Tree.children |t#0@@10|) |index#Z#0@0|)) (and ($Is DatatypeTypeType |let#2#0#0| (Tclass._module.Stream Tclass._module.Tree)) (= |ch#Z#0@0| |let#2#0#0|))) (and (=> (= (ControlFlow 0 9) 6) anon15_Then_correct) (=> (= (ControlFlow 0 9) 8) anon15_Else_correct))))))))))
(let ((anon14_Else_correct  (=> (and (< |index#Z#0@0| (LitInt 0)) (= (ControlFlow 0 5) 4)) anon8_correct)))
(let ((anon13_Then_correct  (=> (and (and (and (= |p#0@@8| (|#_module.Stream.Cons| ($Box intType (int_2_U |_mcc#0#0|)) |_mcc#1#0|)) ($Is DatatypeTypeType |_mcc#1#0| (Tclass._module.Stream TInt))) (and (= |let#0#0#0| |_mcc#1#0|) ($Is DatatypeTypeType |let#0#0#0| (Tclass._module.Stream TInt)))) (and (and (= |tail#Z#0@0| |let#0#0#0|) (= |let#1#0#0| |_mcc#0#0|)) (and ($Is intType (int_2_U |let#1#0#0|) TInt) (= |index#Z#0@0| |let#1#0#0|)))) (and (=> (= (ControlFlow 0 11) 9) anon14_Then_correct) (=> (= (ControlFlow 0 11) 5) anon14_Else_correct)))))
(let ((anon13_Else_correct true))
(let ((anon12_Else_correct  (=> (or (not (= |p#0@@8| |#_module.Stream.Nil|)) (not true)) (and (=> (= (ControlFlow 0 12) 11) anon13_Then_correct) (=> (= (ControlFlow 0 12) 3) anon13_Else_correct)))))
...
(let ((PreconditionGeneratedEntry_correct  (=> (and ($IsGoodHeap $Heap@@0) ($IsHeapAnchor $Heap@@0)) (=> (and (and ($Is DatatypeTypeType |t#0@@10| Tclass._module.Tree) ($Is DatatypeTypeType |p#0@@8| (Tclass._module.Stream TInt))) (and (= 2 $FunctionContextHeight) (= (ControlFlow 0 14) 13))) anon0_correct))))
