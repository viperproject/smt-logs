...
; encoding sigelt let xxxxxx


; <Start encoding let xxxxxx>

(declare-fun Integration.xxxxxx (Dummy_sort) Term)

; </end encoding let xxxxxx>
...
; encoding sigelt let yyyyyy


; <Start encoding let yyyyyy>

(declare-fun Integration.yyyyyy (Dummy_sort) Term)

; </end encoding let yyyyyy>
...
; encoding sigelt let aaaaaa


; <Start encoding let aaaaaa>

(declare-fun Integration.aaaaaa (Dummy_sort) Term)

; </end encoding let aaaaaa>
...
; encoding sigelt let aaaaaaaaaaaaaaaaaaaaaaaa


; <Start encoding let aaaaaaaaaaaaaaaaaaaaaaaa>

(declare-fun Integration.aaaaaaaaaaaaaaaaaaaaaaaa (Dummy_sort) Term)

; </end encoding let aaaaaaaaaaaaaaaaaaaaaaaa>
...
; Starting query at <input>(11,0-11,17)
...
; While typechecking the top-level declaration `let xx`
...
; encoding sigelt let xx


; <Start encoding let xx>

(declare-fun Integration.xx (Dummy_sort) Term)
...
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Integration.xx; Namespace Integration
(assert (! 
;; def=<input>(11,4-11,6); use=<input>(11,4-11,6)
(forall ((@u0 Dummy_sort))
 (! (HasType (Integration.xx @u0)
Prims.nat)
 

:pattern ((Integration.xx @u0))
:qid typing_Integration.xx))

:named typing_Integration.xx))
;;;;;;;;;;;;;;;;refinement kinding
...
:qid equation_Integration.xx))

:named equation_Integration.xx))
...
; Starting query at <input>(13,0-13,23)

(declare-fun label_1 () Bool)

; Encoding query formula : Integration.xx = 2


; Context: While encoding a query
; While checking for top-level effects
; While typechecking the top-level declaration `let y`
...
;;; Fact-ids: 
(assert (! (not 
;; def=<input>(13,15-13,23); use=<input>(13,8-13,14)
(or label_1

;; def=<input>(13,15-13,23); use=<input>(13,8-13,14)
(= (Integration.xx Dummy_value)
(BoxInt 2))
)
)
:named @query))
...
; Starting query at <input>(13,0-13,24)

(declare-fun label_1 () Bool)

; Encoding query formula : Integration.xx = -1
...
;;; Fact-ids: 
(assert (! (not 
;; def=<input>(13,15-13,24); use=<input>(13,8-13,14)
(or label_1
...
; QUERY ID: (Integration.y, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @query, equation_Integration.xx
