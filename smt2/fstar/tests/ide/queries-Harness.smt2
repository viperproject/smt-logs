...
; Internals for Harness


; Externals for module Harness


; <Start encoding let bar>

(declare-fun Harness.bar (Term Term) Term)
;;;;;;;;;;;;;;;;x: nat -> z: nat -> Prims.logical
(declare-fun Tm_arrow_28a88df0045605b5a62bcbe72d91aa3b () Term)
(declare-fun Harness.bar@tok () Term)

; </end encoding let bar>


; <Start encoding let aux>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun Harness.aux (Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun Harness.aux@tok () Term)

; </end encoding let aux>


; <Skipped val Harness.foo/>


; <Start encoding let foo>

(declare-fun Harness.foo (Term) Term)
;;;;;;;;;;;;;;;;x: nat -> prop
(declare-fun Tm_arrow_1742ee5bbc9c9e74913cc041d85062b6 () Term)
(declare-fun Harness.foo@tok () Term)

; </end encoding let foo>


; End Externals for module Harness
...
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Harness.foo; Namespace Harness
(assert (! 
;; def=Harness.fst(11,4-11,7); use=Harness.fst(11,4-11,7)
(forall ((@x0 Term))
 (! (implies (HasType @x0
Prims.nat)
(HasType (Harness.foo @x0)
Prims.prop))
 

:pattern ((Harness.foo @x0))
:qid typing_Harness.foo))

:named typing_Harness.foo))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Harness.bar; Namespace Harness
(assert (! 
;; def=Harness.fst(6,4-6,7); use=Harness.fst(6,4-6,7)
(forall ((@x0 Term) (@x1 Term))
 (! (implies (and (HasType @x0
Prims.nat)
(HasType @x1
Prims.nat))
(HasType (Harness.bar @x0
@x1)
Prims.logical))
 

:pattern ((Harness.bar @x0
@x1))
:qid typing_Harness.bar))

:named typing_Harness.bar))
;;;;;;;;;;;;;;;;True interpretation
;;; Fact-ids: Name Prims.l_True; Namespace Prims
...
;;;;;;;;;;;;;;;;Equation for Harness.foo
;;; Fact-ids: Name Harness.foo; Namespace Harness
(assert (! 
;; def=Harness.fst(11,4-11,7); use=Harness.fst(11,4-11,7)
(forall ((@x0 Term))
 (! (= (Valid (Harness.foo @x0))

;; def=Harness.fst(11,27-11,44); use=Harness.fst(11,27-11,44)
(forall ((@x1 Term))
 (! (implies (HasType @x1
Prims.nat)

;; def=Harness.fst(11,37-11,44); use=Harness.fst(11,37-11,44)
(Valid 
;; def=Harness.fst(11,37-11,44); use=Harness.fst(11,37-11,44)
(Harness.bar @x0
@x1)
...
;;no pats
:qid equation_Harness.foo.1))
)
 

:pattern ((Harness.foo @x0))
:qid equation_Harness.foo))

:named equation_Harness.foo))
;;;;;;;;;;;;;;;;Equation for Harness.bar
;;; Fact-ids: Name Harness.bar; Namespace Harness
(assert (! 
;; def=Harness.fst(6,4-6,7); use=Harness.fst(6,4-6,7)
(forall ((@x0 Term) (@x1 Term))
 (! (= (Valid (Harness.bar @x0
@x1))

;; def=Harness.fst(6,22-6,55); use=Harness.fst(6,22-6,55)
...
;; def=Harness.fst(6,46-6,55); use=Harness.fst(6,46-6,55)
...
;;no pats
:qid equation_Harness.bar.1))
)
 

:pattern ((Harness.bar @x0
@x1))
:qid equation_Harness.bar))

:named equation_Harness.bar))
;;;;;;;;;;;;;;;;equality for proxy
;;; Fact-ids: Name Prims.trivial; Namespace Prims; Name Prims.T; Namespace Prims
(assert (! (= Prims.T@tok
Prims.T)
...
(forall ((@x0 Term))
 (! (implies (HasType @x0
Prims.nat)
(Valid (Prims.subtype_of (Harness.foo @x0)
Prims.unit)))
 

:pattern ((Prims.subtype_of (Harness.foo @x0)
Prims.unit))
:qid defn_equation_Harness.foo))

:named defn_equation_Harness.foo))
;;;;;;;;;;;;;;;;Prop-typing for Harness.bar
;;; Fact-ids: Name Harness.bar; Namespace Harness
(assert (! 
;; def=Harness.fst(6,4-6,7); use=Harness.fst(6,4-6,7)
(forall ((@x0 Term) (@x1 Term))
...
(HasType @x1
Prims.nat))
(Valid (Prims.subtype_of (Harness.bar @x0
@x1)
Prims.unit)))
 

:pattern ((Prims.subtype_of (Harness.bar @x0
@x1)
Prims.unit))
:qid defn_equation_Harness.bar))

:named defn_equation_Harness.bar))
...
; Starting query at <input>(1,35-1,37)
...
;     (forall (pure_result: Prims.unit). Harness.foo x ==> p pure_result) ==>
;     (forall (any_result: Prims.unit). p any_result)


; Context: While encoding a query
; While typechecking the top-level declaration `let always_foo`
...
;; def=Prims.fst(441,36-441,97); use=<input>(1,35-1,37)
(forall ((@x2 Term))
 (! (implies (and (or label_1
(HasType @x2
Prims.unit))

;; def=<input>(1,25-1,32); use=<input>(1,35-1,37)
(or label_2
...
;; def=Prims.fst(441,83-441,96); use=<input>(1,35-1,37)
(Valid 
;; def=Prims.fst(441,83-441,96); use=<input>(1,35-1,37)
(ApplyTT @x1
@x2)
