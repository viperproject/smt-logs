...
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Lib.Sequence.map_blocks_a; Namespace Lib.Sequence
(assert (! 
;; def=Lib.Sequence.fsti(378,4-378,16); use=Lib.Sequence.fsti(378,4-378,16)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(HasType (Lib.Sequence.map_blocks_a @x0
@x1
@x2
@x3)
Tm_type))
...
:pattern ((Lib.Sequence.map_blocks_a @x0
@x1
@x2
@x3))
:qid typing_Lib.Sequence.map_blocks_a))

:named typing_Lib.Sequence.map_blocks_a))
;;;;;;;;;;;;;;;;free var typing
...
;;;;;;;;;;;;;;;;Equation for Lib.Sequence.map_blocks_a
;;; Fact-ids: Name Lib.Sequence.map_blocks_a; Namespace Lib.Sequence
(assert (! 
;; def=Lib.Sequence.fsti(378,4-378,16); use=Lib.Sequence.fsti(378,4-378,16)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (= (Lib.Sequence.map_blocks_a @x0
@x1
@x2
@x3)
...
@x2
@x3))
:qid equation_Lib.Sequence.map_blocks_a))

:named equation_Lib.Sequence.map_blocks_a))
...
; Starting query at Lib.Sequence.fsti(389,2-391,28)
...
;;; Fact-ids: 
(assert (! 
;; def=FStar.Seq.Base.fsti(64,43-64,73); use=Lib.Sequence.fsti(390,18-390,23)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
...
;           return_val == i * bs ==>
;           (i + 1) * bs >= 0 /\ i * bs <= (i + 1) * bs && (i + 1) * bs <= FStar.Seq.Base.length inp /\
;           (forall (return_val: j: Prims.nat{i * bs <= j && j <= FStar.Seq.Base.length inp}).
;               return_val == (i + 1) * bs ==>
;               (forall (any_result: FStar.Seq.Base.seq a).
;                   FStar.Seq.Base.slice inp (i * bs) ((i + 1) * bs) == any_result ==>
;                   FStar.Seq.Base.length (FStar.Seq.Base.slice inp (i * bs) ((i + 1) * bs)) == bs /\
;                   (forall (any_result: FStar.Seq.Base.seq a).
;                       FStar.Seq.Base.slice inp (i * bs) ((i + 1) * bs) == any_result ==>
;                       (forall (any_result: Lib.Sequence.lseq a bs).
;                           f i (FStar.Seq.Base.slice inp (i * bs) ((i + 1) * bs)) == any_result ==>
;                           (forall (return_val: Lib.Sequence.lseq a bs).
;                               return_val == f i (FStar.Seq.Base.slice inp (i * bs) ((i + 1) * bs)) ==>
;                               f i (FStar.Seq.Base.slice inp (i * bs) ((i + 1) * bs)) == return_val ==>
;                               (forall (any_result: FStar.Seq.Base.seq a).
;                                   FStar.Seq.Base.append acc
;                                     (f i (FStar.Seq.Base.slice inp (i * bs) ((i + 1) * bs))) ==
;                                   any_result ==>
;                                   Lib.Sequence.length (FStar.Seq.Base.append acc
;                                         (f i (FStar.Seq.Base.slice inp (i * bs) ((i + 1) * bs)))) ==
;                                   (i + 1) * bs))))))))


; Context: While encoding a query
; While typechecking the top-level declaration `let map_blocks_f`
...
;; def=Prims.fst(441,29-441,97); use=Lib.Sequence.fsti(387,56-389,33)
(and 
;; def=Prims.fst(680,18-680,24); use=Lib.Sequence.fsti(387,56-387,63)
(or label_1

;; def=Prims.fst(680,18-680,24); use=Lib.Sequence.fsti(389,2-391,28)
...
;; def=Lib.Sequence.fsti(378,57-378,65); use=Lib.Sequence.fsti(387,56-387,63)
(or label_2

;; def=Lib.Sequence.fsti(378,57-378,65); use=Lib.Sequence.fsti(389,2-391,28)
(<= (BoxInt_proj_0 (Prims.op_Addition @x5
(BoxInt 1)))
(BoxInt_proj_0 @x2))
)
...
;; def=FStar.Math.Lemmas.fsti(33,12-33,20); use=Lib.Sequence.fsti(389,14-389,33)
(or label_3
...
;; def=Prims.fst(441,36-441,97); use=Lib.Sequence.fsti(389,14-389,33)
(forall ((@x7 Term))
 (! (implies (and (HasType @x7
Prims.unit)

;; def=FStar.Math.Lemmas.fsti(34,12-34,28); use=Lib.Sequence.fsti(389,14-389,33)
(<= (BoxInt_proj_0 (Prims.op_Multiply (Prims.op_Addition @x5
(BoxInt 1))
@x1))
...
;; def=Prims.fst(459,77-459,89); use=Lib.Sequence.fsti(389,2-391,28)
(and 
;; def=Prims.fst(680,18-680,24); use=Lib.Sequence.fsti(390,28-390,34)
(or label_4
...
;; def=Prims.fst(356,2-356,58); use=Lib.Sequence.fsti(389,2-391,28)
(forall ((@x8 Term))
 (! (implies (and (HasType @x8
Prims.nat)

;; def=Prims.fst(356,26-356,41); use=Lib.Sequence.fsti(389,2-391,28)
(= @x8
...
;; def=Prims.fst(459,77-459,89); use=Lib.Sequence.fsti(389,2-391,28)
(and 
;; def=Prims.fst(680,18-680,24); use=Lib.Sequence.fsti(390,35-390,45)
(or label_5
...
;; def=Lib.Sequence.fsti(390,35-390,45); use=Lib.Sequence.fsti(390,35-390,45)
(or label_6
(BoxBool_proj_0 (Prims.op_LessThanOrEqual (Prims.op_Multiply @x5
@x1)
(Prims.op_Multiply (Prims.op_Addition @x5
(BoxInt 1))
@x1))))
...
;; def=Lib.Sequence.fsti(390,35-390,45); use=Lib.Sequence.fsti(390,35-390,45)
(or label_7
(BoxBool_proj_0 (Prims.op_LessThanOrEqual (Prims.op_Multiply (Prims.op_Addition @x5
(BoxInt 1))
@x1)
(FStar.Seq.Base.length @x0
@x3))))
...
(Tm_refine_1c325641987cce6783228428bd15a869 @x5
@x1
@x0
@x3))
...
;; def=Prims.fst(451,66-451,102); use=Lib.Sequence.fsti(389,2-391,28)
(forall ((@x10 Term))
 (! (implies (and (HasType @x10
(FStar.Seq.Base.seq @x0))

;; def=Lib.Sequence.fsti(390,6-390,45); use=Lib.Sequence.fsti(390,6-390,45)
(= (FStar.Seq.Base.slice @x0
@x3
(Prims.op_Multiply @x5
@x1)
...
;; def=Prims.fst(459,77-459,89); use=Lib.Sequence.fsti(389,2-391,28)
(and 
;; def=Lib.Sequence.fsti(27,44-27,63); use=Lib.Sequence.fsti(391,22-391,27)
(or label_8
...
;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(389,2-391,28)
(= (FStar.Seq.Base.slice @x0
@x3
(Prims.op_Multiply @x5
@x1)
...
;; def=Lib.Sequence.fsti(385,37-391,28); use=Lib.Sequence.fsti(389,2-391,28)
(= (ApplyTT (ApplyTT @x4
@x5)
(FStar.Seq.Base.slice @x0
@x3
...
;; def=Lib.Sequence.fsti(387,34-391,28); use=Lib.Sequence.fsti(389,2-391,28)
(= (FStar.Seq.Base.append @x0
@x6
(ApplyTT (ApplyTT @x4
@x5)
...
;; def=Lib.Sequence.fsti(378,78-378,96); use=Lib.Sequence.fsti(391,2-391,28)
(or label_9
