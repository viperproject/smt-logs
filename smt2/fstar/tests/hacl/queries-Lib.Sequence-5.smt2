...
; Starting query at Lib.Sequence.fsti(71,0-74,67)
...
; Encoding query formula : forall (a: Type0) (l: Prims.list a {FStar.List.Tot.Base.length l <= Prims.pow2 32 - 1}).
;   (*  - Subtyping check failed
;   - Expected type Type0 got type Type0
; *)
;   FStar.List.Tot.Base.length l <= Prims.pow2 32 - 1 /\
;   (forall (s: Lib.Sequence.lseq a (FStar.List.Tot.Base.length l)).
;       (*  - Subtyping check failed
;   - Expected type Lib.IntTypes.size_nat got type Prims.nat
; *)
;       FStar.List.Tot.Base.length l <= Prims.pow2 32 - 1)


; Context: While encoding a query
; While typechecking the top-level declaration `val Lib.Sequence.of_list`
...
;;; Fact-ids: 
(assert (! (not 
;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(71,0-74,67)
(forall ((@x0 Term) (@x1 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(72,7-74,67)
(and 
;; def=Lib.IntTypes.fsti(308,22-308,37); use=Lib.Sequence.fsti(74,16-74,35)
(or label_1
...
;; def=Lib.IntTypes.fsti(308,22-308,37); use=Lib.Sequence.fsti(74,43-74,44)
(or label_2
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (Lib.Sequence.of_list, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @MaxIFuel_assumption, @query, refinement_interpretation_Tm_refine_56609cb097df748006aafa90a98ed73d
