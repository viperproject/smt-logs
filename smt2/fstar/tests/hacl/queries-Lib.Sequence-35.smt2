...
; Starting query at Lib.Sequence.fsti(373,0-375,38)
...
; Encoding query formula : forall (a: Prims.int) (b: Prims.int) (c: Prims.pos) (d: Prims.pos).
;   (d > 0 ==> d <> 0) /\
...
;                               d == any_result ==>
;                               (forall (any_result: Prims.int).
;                                   a / d == any_result ==> d > 0 ==> d <> 0)) /\
;                           (forall (_: Prims.squash (a / d = b / d)).
...
;                                   return_val == c * d ==>
;                                   (forall (any_result: Prims.int).
;                                       a / (c * d) == any_result ==> c * d <> 0)))))))))


; Context: While encoding a query
; While typechecking the top-level declaration `val Lib.Sequence.div_mul_l`
...
;;; Fact-ids: 
(assert (! (not 
;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(373,0-375,38)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (implies (and (HasType @x0
Prims.int)
(HasType @x1
...
;; def=Prims.fst(459,77-459,89); use=Lib.Sequence.fsti(373,17-375,38)
(and (implies 
;; def=Prims.fst(683,18-683,23); use=Lib.Sequence.fsti(374,16-374,17)
(> (BoxInt_proj_0 @x3)
(BoxInt_proj_0 (BoxInt 0)))


;; def=Prims.fst(686,22-686,28); use=Lib.Sequence.fsti(374,16-374,17)
(or label_1
...
;; def=Prims.fst(451,66-451,102); use=Lib.Sequence.fsti(373,17-375,38)
(forall ((@x4 Term))
 (! (implies (and (HasType @x4
Prims.pos)

;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(373,17-375,38)
(= @x3
@x4)
)
...
;; def=Prims.fst(459,77-459,89); use=Lib.Sequence.fsti(373,17-375,38)
(and (implies 
;; def=Prims.fst(683,18-683,23); use=Lib.Sequence.fsti(374,24-374,25)
(> (BoxInt_proj_0 @x3)
(BoxInt_proj_0 (BoxInt 0)))


;; def=Prims.fst(686,22-686,28); use=Lib.Sequence.fsti(374,24-374,25)
(or label_2
...
;; def=Prims.fst(180,10-180,11); use=Lib.Sequence.fsti(373,17-375,38)
...
;; def=FStar.Pervasives.fsti(112,28-112,31); use=Lib.Sequence.fsti(373,17-375,38)
...
;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(373,17-375,38)
(and (implies 
;; def=Prims.fst(683,18-683,23); use=Lib.Sequence.fsti(375,12-375,37)
(> (BoxInt_proj_0 @x3)
(BoxInt_proj_0 (BoxInt 0)))
...
;; def=Prims.fst(686,22-686,28); use=Lib.Sequence.fsti(375,12-375,37)
(not (= @x3
(BoxInt 0)))
)
)

;; def=Prims.fst(451,66-451,102); use=Lib.Sequence.fsti(375,12-375,37)
(forall ((@x10 Term))
 (! (implies (and (HasType @x10
Prims.pos)

;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(375,12-375,37)
(= @x3
@x10)
)
...
;; def=Prims.fst(459,77-459,89); use=Lib.Sequence.fsti(375,12-375,37)
(and 
;; def=Prims.fst(686,22-686,28); use=Lib.Sequence.fsti(375,16-375,23)
(or label_5

;; def=Prims.fst(686,22-686,28); use=Lib.Sequence.fsti(375,16-375,23)
(not (= (Prims.op_Multiply @x2
@x3)
(BoxInt 0)))
)


;; def=Prims.fst(356,2-356,58); use=Lib.Sequence.fsti(375,12-375,37)
(forall ((@x11 Term))
 (! (implies (and (HasType @x11
Prims.nonzero)

;; def=Prims.fst(356,26-356,41); use=Lib.Sequence.fsti(375,12-375,37)
(= @x11
...
;; def=Prims.fst(686,22-686,28); use=Lib.Sequence.fsti(375,30-375,37)
(or label_6
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (Lib.Sequence.div_mul_l, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @MaxIFuel_assumption, @query, equation_Prims.pos, int_inversion, primitive_Prims.op_Multiply, projection_inverse_BoxInt_proj_0, refinement_interpretation_Tm_refine_774ba3f728d91ead8ef40be66c9802e5
