...
; Starting query at Lib.Sequence.fsti(212,0-216,61)
...
; Encoding query formula : forall (a: Type0) (len: n: Prims.nat{n <= Prims.pow2 32 - 1}) (init: (i: Prims.nat{i < len} -> a)).
;   (*  - Subtyping check failed
;   - Expected type Type0 got type Type0
; *)
;   forall (s: Lib.Sequence.lseq a len) (i: Prims.nat).
;     i < len ==>
;     i <= Prims.pow2 32 - 1 /\ i < len /\
;     (forall (any_result: Prims.nat).
;         i == any_result ==>
;         (forall (any_result: r: a{r == FStar.Seq.Base.index (Lib.Sequence.to_seq s) i}).
;             Lib.Sequence.index s i == any_result ==>
;             (forall (return_val: r: a{r == FStar.Seq.Base.index (Lib.Sequence.to_seq s) i}).
;                 return_val == Lib.Sequence.index s i ==>
;                 Lib.Sequence.index s i == return_val ==> i < len)))


; Context: While encoding a query
; While typechecking the top-level declaration `val Lib.Sequence.createi`
...
;;; Fact-ids: 
(assert (! (not 
;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(212,0-216,61)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(212,16-216,61)
...
;; def=Lib.Sequence.fsti(216,27-216,34); use=Lib.Sequence.fsti(216,27-216,34)
(< (BoxInt_proj_0 @x4)
(BoxInt_proj_0 @x1))
)

;; def=Prims.fst(459,77-459,89); use=Lib.Sequence.fsti(216,27-216,58)
(and 
;; def=Lib.IntTypes.fsti(308,22-308,37); use=Lib.Sequence.fsti(216,47-216,48)
(or label_1
...
;; def=Lib.Sequence.fsti(40,16-40,23); use=Lib.Sequence.fsti(216,47-216,48)
(or label_2
...
;; def=Prims.fst(451,66-451,102); use=Lib.Sequence.fsti(216,27-216,58)
(forall ((@x5 Term))
 (! (implies (and (HasType @x5
Prims.nat)

;; def=Lib.Sequence.fsti(40,5-215,30); use=Lib.Sequence.fsti(216,27-216,58)
(= @x4
@x5)
)
...
;; def=Lib.Sequence.fsti(41,6-216,48); use=Lib.Sequence.fsti(216,39-216,48)
(= (Lib.Sequence.index @x0
@x1
@x3
@x4)
...
;; def=Prims.fst(356,2-356,58); use=Lib.Sequence.fsti(216,27-216,58)
(forall ((@x7 Term))
...
;; def=Prims.fst(356,26-356,41); use=Lib.Sequence.fsti(216,27-216,58)
(= @x7
(Lib.Sequence.index @x0
@x1
@x3
...
;; def=Prims.fst(175,35-175,36); use=Lib.Sequence.fsti(216,27-216,58)
(= (Lib.Sequence.index @x0
@x1
@x3
@x4)
...
;; def=Lib.Sequence.fsti(214,17-214,24); use=Lib.Sequence.fsti(216,57-216,58)
(or label_3
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (Lib.Sequence.createi, 1)
; STATUS: unsat
