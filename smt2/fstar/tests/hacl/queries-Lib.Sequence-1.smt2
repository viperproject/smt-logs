...
; Starting query at Lib.Sequence.fsti(36,0-41,40)
...
;   (i: (n: Prims.nat{n <= Prims.pow2 32 - 1}){i < len}).
;   (*  - Subtyping check failed
;   - Expected type Type0 got type Type0
; *)
;   forall (r: a).
;     (*  - Subtyping check failed
;   - Expected type i: Prims.nat{i < FStar.Seq.Base.length (to_seq s)}
;     got type i: Lib.IntTypes.size_nat{i < len}
; *)
;     i <= Prims.pow2 32 - 1 /\ i < len ==> i < FStar.Seq.Base.length (Lib.Sequence.to_seq s)


; Context: While encoding a query
; While typechecking the top-level declaration `val Lib.Sequence.index`
...
;;; Fact-ids: 
(assert (! (not 
;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(36,0-41,40)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
;; def=dummy(0,0-0,0); use=Lib.Sequence.fsti(37,7-41,40)
(forall ((@x4 Term))
 (! (implies (and (HasType @x4
@x0)

;; def=Lib.IntTypes.fsti(308,22-308,37); use=Lib.Sequence.fsti(41,37-41,38)
(<= (BoxInt_proj_0 @x3)
(BoxInt_proj_0 (Prims.op_Subtraction (Prims.pow2 (BoxInt 32))
(BoxInt 1))))


;; def=Lib.Sequence.fsti(40,16-40,23); use=Lib.Sequence.fsti(41,37-41,38)
(< (BoxInt_proj_0 @x3)
(BoxInt_proj_0 @x1))
)

;; def=FStar.Seq.Base.fsti(32,40-32,52); use=Lib.Sequence.fsti(41,37-41,38)
(or label_1
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (Lib.Sequence.index, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @MaxIFuel_assumption, @query, equation_Lib.Sequence.lseq, equation_Lib.Sequence.to_seq, refinement_interpretation_Tm_refine_d8d83307254a8900dd20598654272e42
