...
; Internals for SMTPatSymbols


; encoding sigelt val SMTPatSymbols.lem


; <Skipped val SMTPatSymbols.lem/>
...
; Starting query at SMTPatSymbols.fst(4,51-4,53)
...
;     (forall (pure_result: Prims.unit). x > x - 1 ==> p pure_result) ==>
;     (forall (any_result: Prims.unit). p any_result)


; Context: While encoding a query
; While typechecking the top-level declaration `let lem`
...
;; def=Prims.fst(441,36-441,97); use=SMTPatSymbols.fst(4,51-4,53)
(forall ((@x2 Term))
 (! (implies (and (or label_1
(HasType @x2
Prims.unit))

;; def=SMTPatSymbols.fst(4,24-4,33); use=SMTPatSymbols.fst(4,51-4,53)
(or label_2
...
;; def=Prims.fst(441,83-441,96); use=SMTPatSymbols.fst(4,51-4,53)
(Valid 
;; def=Prims.fst(441,83-441,96); use=SMTPatSymbols.fst(4,51-4,53)
(ApplyTT @x1
@x2)
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (SMTPatSymbols.lem, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @query, primitive_Prims.op_Subtraction, projection_inverse_BoxInt_proj_0
