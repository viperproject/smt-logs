...
(declare-fun Tm_abs_Lens_378 (Term Term Term) Term)
;;;;;;;;;;;;;;;;Imprecise function encoding
(declare-fun Tm_abs_Lens_379 (Term Term Term) Term)

; </end encoding let st>


; <Start encoding let op_Bar_Colon_Dot_Dot>

(declare-fun Lens.op_Bar_Colon_Dot_Dot (Term Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;sl: stlens l -> m: lens b c -> stlens (l |.. m)
(declare-fun Tm_arrow_1f2d5ded9fad49738d673aa1aa8fa915 () Term)
(declare-fun Lens.op_Bar_Colon_Dot_Dot@tok () Term)
;;;;;;;;;;;;;;;;Imprecise function encoding
(declare-fun Tm_abs_Lens_382 (Term Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Imprecise function encoding
(declare-fun Tm_abs_Lens_383 (Term Term Term Term Term Term) Term)

; </end encoding let op_Bar_Colon_Dot_Dot>


; <Start encoding let op_Bar_Colon_Dot>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun Lens.op_Bar_Colon_Dot (Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun Lens.op_Bar_Colon_Dot@tok () Term)

; </end encoding let op_Bar_Colon_Dot>


; <Start encoding let op_Bar_Colon_Colon_Equals>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun Lens.op_Bar_Colon_Colon_Equals (Term Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun Lens.op_Bar_Colon_Colon_Equals@tok () Term)

; </end encoding let op_Bar_Colon_Colon_Equals>


; <Start encoding let op_Array_Assignment>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun Lens.op_Array_Assignment (Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun Lens.op_Array_Assignment@tok () Term)

; </end encoding let op_Array_Assignment>


; <Start encoding let op_Array_Access>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun Lens.op_Array_Access (Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun Lens.op_Array_Access@tok () Term)

; </end encoding let op_Array_Access>


; <Start encoding let mutate>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun Lens.mutate (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun Lens.mutate@tok () Term)

; </end encoding let mutate>


; <Start encoding let mutate2>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun Lens.mutate2 (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun Lens.mutate2@tok () Term)

; </end encoding let mutate2>


; <Start encoding let mutate3>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun Lens.mutate3 (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun Lens.mutate3@tok () Term)

; </end encoding let mutate3>


; End Externals for module Lens

(push) ;; push{1

; Internals for StatefulLens


; encoding sigelt val StatefulLens.upd_ref


; <Start encoding val StatefulLens.upd_ref>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun StatefulLens.upd_ref (Term Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun StatefulLens.upd_ref@tok () Term)

; </end encoding val StatefulLens.upd_ref>


; encoding sigelt type StatefulLens.hlens


; <Start encoding type StatefulLens.hlens>

;;;;;;;;;;;;;;;;Constructor
(declare-fun StatefulLens.hlens (Term Term) Term)
;;;;;;;;;;;;;;;;token
(declare-fun StatefulLens.hlens@tok () Term)
;;;;;;;;;;;;;;;;Constructor
(declare-fun StatefulLens.Mkhlens (Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun StatefulLens.Mkhlens_a (Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun StatefulLens.Mkhlens_b (Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun StatefulLens.Mkhlens_get (Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun StatefulLens.Mkhlens_put (Term) Term)
;;;;;;;;;;;;;;;;data constructor proxy: StatefulLens.Mkhlens
(declare-fun StatefulLens.Mkhlens@tok () Term)
;;;;;;;;;;;;;;;;_: FStar.Monotonic.Heap.heap & a -> Prims.GTot b
(declare-fun Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b (Term Term) Term)
;;;;;;;;;;;;;;;;_: b -> _: FStar.Monotonic.Heap.heap & a -> Prims.GTot (FStar.Monotonic.Heap.heap & a)
(declare-fun Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 (Term Term) Term)


;;;;;;;;;;;;;;;;get: (_: FStar.Monotonic.Heap.heap & a -> Prims.GTot b) ->     put: (_: b -> _: FStar.Monotonic.Heap.heap & a -> Prims.GTot (FStar.Monotonic.Heap.heap & a))   -> hlens a b
(declare-fun Tm_arrow_516b5fc3413d5fcb5b8f04cf3a1629ac () Term)

; <start constructor StatefulLens.hlens>

;;;;;;;;;;;;;;;;Discriminator definition
(define-fun is-StatefulLens.hlens ((__@x0 Term)) Bool
 (and (= (Term_constr_id __@x0)
104)
(exists ((@x0 Term) (@x1 Term))
 (! (= __@x0
(StatefulLens.hlens @x0
@x1))
 
;;no pats
:qid is-StatefulLens.hlens))))

; </end constructor StatefulLens.hlens>


; <start constructor StatefulLens.Mkhlens>

;;;;;;;;;;;;;;;;Discriminator definition
(define-fun is-StatefulLens.Mkhlens ((__@x0 Term)) Bool
 (and (= (Term_constr_id __@x0)
115)
(= __@x0
(StatefulLens.Mkhlens (StatefulLens.Mkhlens_a __@x0)
(StatefulLens.Mkhlens_b __@x0)
(StatefulLens.Mkhlens_get __@x0)
(StatefulLens.Mkhlens_put __@x0)))))

; </end constructor StatefulLens.Mkhlens>


; </end encoding type StatefulLens.hlens>


; encoding sigelt val StatefulLens.__proj__Mkhlens__item__get


; <Start encoding val StatefulLens.__proj__Mkhlens__item__get>

(declare-fun StatefulLens.__proj__Mkhlens__item__get (Term Term Term) Term)

;;;;;;;;;;;;;;;;projectee: hlens a b -> _: FStar.Monotonic.Heap.heap & a -> Prims.GTot b
(declare-fun Tm_arrow_3361558004d257244693421d62990cea () Term)
(declare-fun StatefulLens.__proj__Mkhlens__item__get@tok () Term)


; </end encoding val StatefulLens.__proj__Mkhlens__item__get>


; encoding sigelt let __proj__Mkhlens__item__get


; <Skipped let __proj__Mkhlens__item__get/>


; encoding sigelt val StatefulLens.__proj__Mkhlens__item__put


; <Start encoding val StatefulLens.__proj__Mkhlens__item__put>

(declare-fun StatefulLens.__proj__Mkhlens__item__put (Term Term Term) Term)

;;;;;;;;;;;;;;;;projectee: hlens a b -> _: b -> _: FStar.Monotonic.Heap.heap & a   -> Prims.GTot (FStar.Monotonic.Heap.heap & a)
(declare-fun Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7 () Term)
(declare-fun StatefulLens.__proj__Mkhlens__item__put@tok () Term)


; </end encoding val StatefulLens.__proj__Mkhlens__item__put>


; encoding sigelt let __proj__Mkhlens__item__put


; <Skipped let __proj__Mkhlens__item__put/>


; encoding sigelt let as_hlens


; <Start encoding let as_hlens>

(declare-fun StatefulLens.as_hlens (Term Term Term) Term)
;;;;;;;;;;;;;;;;l: Lens.lens 'a 'b -> hlens 'a 'b
(declare-fun Tm_arrow_1cc40ae7ebe28a14177d7b0a488e9c59 () Term)
(declare-fun StatefulLens.as_hlens@tok () Term)

(declare-fun Tm_abs_67044bd756546113eab810f29d691656 (Term Term Term) Term)

(declare-fun Tm_abs_4c574a44c59a8826c9f20f8889049f16 (Term Term Term) Term)

; </end encoding let as_hlens>


; encoding sigelt let compose_hlens


; <Start encoding let compose_hlens>

(declare-fun StatefulLens.compose_hlens (Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;l: hlens a b -> m: hlens b c -> hlens a c
(declare-fun Tm_arrow_b0cf705e4543bc03e6e28fb2316f739b () Term)
(declare-fun StatefulLens.compose_hlens@tok () Term)


(declare-fun Tm_abs_eb58a5c7378d916320d8f69adb3470f7 (Term Term Term Term Term) Term)



(declare-fun Tm_abs_106f73abf9d4751ff515c66ee21e5216 (Term Term Term Term Term) Term)

; </end encoding let compose_hlens>


; encoding sigelt type StatefulLens.stlens


; <Start encoding type StatefulLens.stlens>

;;;;;;;;;;;;;;;;Constructor
(declare-fun StatefulLens.stlens (Term Term Term) Term)
;;;;;;;;;;;;;;;;token
(declare-fun StatefulLens.stlens@tok () Term)
;;;;;;;;;;;;;;;;Constructor
(declare-fun StatefulLens.Mkstlens (Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun StatefulLens.Mkstlens_st_get (Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun StatefulLens.Mkstlens_st_put (Term) Term)
;;;;;;;;;;;;;;;;Constructor base
(declare-fun StatefulLens.Mkstlens@base (Term Term) Term)
;;;;;;;;;;;;;;;;data constructor proxy: StatefulLens.Mkstlens
(declare-fun StatefulLens.Mkstlens@tok () Term)
(declare-fun Non_total_Tm_arrow_2fd0425a15b041bc68405011b81aa070 (Term Term Term) Term)
(declare-fun Non_total_Tm_arrow_6f94488d6278900545315db3af6b01bf (Term Term Term) Term)


;;;;;;;;;;;;;;;;st_get: (x: a -> FStar.ST.ST b) -> st_put: (y: b -> x: a -> FStar.ST.ST a) -> stlens l
(declare-fun Tm_arrow_aad6081304f5c0a614aa16cd12ce81a6 () Term)

; <start constructor StatefulLens.stlens>

;;;;;;;;;;;;;;;;Discriminator definition
(define-fun is-StatefulLens.stlens ((__@x0 Term)) Bool
 (and (= (Term_constr_id __@x0)
147)
(exists ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= __@x0
(StatefulLens.stlens @x0
@x1
@x2))
 
;;no pats
:qid is-StatefulLens.stlens))))

; </end constructor StatefulLens.stlens>


; <start constructor StatefulLens.Mkstlens>

;;;;;;;;;;;;;;;;Discriminator definition
(define-fun is-StatefulLens.Mkstlens ((__@x0 Term)) Bool
 (and (= (Term_constr_id __@x0)
174)
(exists ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= __@x0
(StatefulLens.Mkstlens @x0
@x1
@x2
(StatefulLens.Mkstlens_st_get __@x0)
(StatefulLens.Mkstlens_st_put __@x0)))
 
;;no pats
:qid is-StatefulLens.Mkstlens))))

; </end constructor StatefulLens.Mkstlens>


; </end encoding type StatefulLens.stlens>


; encoding sigelt val StatefulLens.__proj__Mkstlens__item__st_get


; <Start encoding val StatefulLens.__proj__Mkstlens__item__st_get>

(declare-fun StatefulLens.__proj__Mkstlens__item__st_get (Term Term Term Term) Term)

;;;;;;;;;;;;;;;;projectee: stlens l -> x: a -> FStar.ST.ST b
(declare-fun Tm_arrow_aa212c5cb4f9979c6ed18168c7993229 () Term)
(declare-fun StatefulLens.__proj__Mkstlens__item__st_get@tok () Term)


; </end encoding val StatefulLens.__proj__Mkstlens__item__st_get>
...
; encoding sigelt val StatefulLens.__proj__Mkstlens__item__st_put


; <Start encoding val StatefulLens.__proj__Mkstlens__item__st_put>

(declare-fun StatefulLens.__proj__Mkstlens__item__st_put (Term Term Term Term) Term)

;;;;;;;;;;;;;;;;projectee: stlens l -> y: b -> x: a -> FStar.ST.ST a
(declare-fun Tm_arrow_1f1cc1aa7eb682efc2fcb4e0f85c2229 () Term)
(declare-fun StatefulLens.__proj__Mkstlens__item__st_put@tok () Term)


; </end encoding val StatefulLens.__proj__Mkstlens__item__st_put>
...
;;;;;;;;;;;;;;;;typing_Tm_abs_eb58a5c7378d916320d8f69adb3470f7
;;; Fact-ids: Name StatefulLens.compose_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(51,27-51,52); use=StatefulLens.fst(51,27-51,52)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (HasType (Tm_abs_eb58a5c7378d916320d8f69adb3470f7 @x0
@x1
@x2
@x3
@x4)
(Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b @x3
@x4))
 

:pattern ((Tm_abs_eb58a5c7378d916320d8f69adb3470f7 @x0
@x1
@x2
@x3
@x4))
:qid typing_Tm_abs_eb58a5c7378d916320d8f69adb3470f7))

:named typing_Tm_abs_eb58a5c7378d916320d8f69adb3470f7))
;;;;;;;;;;;;;;;;typing_Tm_abs_106f73abf9d4751ff515c66ee21e5216
;;; Fact-ids: Name StatefulLens.compose_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(52,32-52,93); use=StatefulLens.fst(52,32-52,93)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (HasType (Tm_abs_106f73abf9d4751ff515c66ee21e5216 @x0
@x1
@x2
@x3
@x4)
(Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 @x3
@x4))
 

:pattern ((Tm_abs_106f73abf9d4751ff515c66ee21e5216 @x0
@x1
@x2
@x3
@x4))
:qid typing_Tm_abs_106f73abf9d4751ff515c66ee21e5216))

:named typing_Tm_abs_106f73abf9d4751ff515c66ee21e5216))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.compose_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(50,4-50,17); use=StatefulLens.fst(50,4-50,17)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
...
@x1))
(HasType @x4
(StatefulLens.hlens @x1
@x2)))
(HasType (StatefulLens.compose_hlens @x0
@x1
@x2
@x3
@x4)
...
:pattern ((StatefulLens.compose_hlens @x0
@x1
@x2
@x3
@x4))
:qid typing_StatefulLens.compose_hlens))

:named typing_StatefulLens.compose_hlens))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__put; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(39,5-39,8); use=StatefulLens.fst(39,5-39,8)
...
(StatefulLens.hlens @x0
@x1)))
(HasType (StatefulLens.__proj__Mkhlens__item__put @x0
@x1
@x2)
(Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 @x0
@x1)))
 

:pattern ((StatefulLens.__proj__Mkhlens__item__put @x0
@x1
@x2))
:qid typing_StatefulLens.__proj__Mkhlens__item__put))

:named typing_StatefulLens.__proj__Mkhlens__item__put))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__get; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(38,5-38,8); use=StatefulLens.fst(38,5-38,8)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(StatefulLens.hlens @x0
@x1)))
(HasType (StatefulLens.__proj__Mkhlens__item__get @x0
@x1
@x2)
(Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b @x1
@x0)))
 

:pattern ((StatefulLens.__proj__Mkhlens__item__get @x0
@x1
@x2))
:qid typing_StatefulLens.__proj__Mkhlens__item__get))

:named typing_StatefulLens.__proj__Mkhlens__item__get))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Prims.unit; Namespace Prims
...
;;; Fact-ids: Name StatefulLens.stlens; Namespace StatefulLens; Name StatefulLens.Mkstlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(58,5-58,11); use=StatefulLens.fst(58,5-58,11)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (ApplyTT (ApplyTT (ApplyTT StatefulLens.stlens@tok
@x0)
@x1)
@x2)
(StatefulLens.stlens @x0
...
:pattern ((ApplyTT (ApplyTT (ApplyTT StatefulLens.stlens@tok
@x0)
@x1)
@x2))

:pattern ((StatefulLens.stlens @x0
@x1
@x2))
:qid token_correspondence_StatefulLens.stlens@tok))

:named token_correspondence_StatefulLens.stlens@tok))
;;;;;;;;;;;;;;;;name-token correspondence
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ApplyTT (ApplyTT StatefulLens.hlens@tok
@x0)
@x1)
(StatefulLens.hlens @x0
@x1))
...
:pattern ((ApplyTT (ApplyTT StatefulLens.hlens@tok
@x0)
@x1))

:pattern ((StatefulLens.hlens @x0
@x1))
:qid token_correspondence_StatefulLens.hlens@tok))

:named token_correspondence_StatefulLens.hlens@tok))
;;;;;;;;;;;;;;;;Name-token correspondence
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__put; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(39,5-39,8); use=StatefulLens.fst(39,5-39,8)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__put@tok
@x0)
@x1)
@x2)
(StatefulLens.__proj__Mkhlens__item__put @x0
@x1
@x2))
 

:pattern ((ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__put@tok
@x0)
@x1)
@x2))
:qid token_correspondence_StatefulLens.__proj__Mkhlens__item__put))

:named token_correspondence_StatefulLens.__proj__Mkhlens__item__put))
;;;;;;;;;;;;;;;;Name-token correspondence
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__get; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(38,5-38,8); use=StatefulLens.fst(38,5-38,8)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
@x0)
@x1)
@x2)
(StatefulLens.__proj__Mkhlens__item__get @x0
@x1
@x2))
 

:pattern ((ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
@x0)
@x1)
@x2))
:qid token_correspondence_StatefulLens.__proj__Mkhlens__item__get))

:named token_correspondence_StatefulLens.__proj__Mkhlens__item__get))
;;;;;;;;;;;;;;;;name-token correspondence
...
(StatefulLens.Mkstlens @x1
@x2
@x3
@x4
@x5)
(StatefulLens.stlens @x6
@x7
@x8))
...
@x7
@x8)))
:qid subterm_ordering_StatefulLens.Mkstlens))

:named subterm_ordering_StatefulLens.Mkstlens))
;;;;;;;;;;;;;;;;subterm ordering
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term) (@x6 Term))
 (! (implies (HasTypeFuel (SFuel @u0)
(StatefulLens.Mkhlens @x1
@x2
@x3
@x4)
(StatefulLens.hlens @x5
@x6))
(and (Valid (Prims.precedes Prims.lex_t
Prims.lex_t
@x3
...
(StatefulLens.hlens @x5
@x6)))
:qid subterm_ordering_StatefulLens.Mkhlens))

:named subterm_ordering_StatefulLens.Mkhlens))
;;;;;;;;;;;;;;;;subterm ordering
;;; Fact-ids: Name FStar.Pervasives.Native.tuple2; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.Mktuple2; Namespace FStar.Pervasives.Native
(assert (! 
...
;; def=StatefulLens.fst(58,5-58,11); use=StatefulLens.fst(58,5-58,11)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (= (StatefulLens.Mkstlens_st_put (StatefulLens.Mkstlens @x0
@x1
@x2
@x3
@x4))
...
:pattern ((StatefulLens.Mkstlens @x0
@x1
@x2
@x3
@x4))
:qid projection_inverse_StatefulLens.Mkstlens_st_put))

:named projection_inverse_StatefulLens.Mkstlens_st_put))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name StatefulLens.stlens; Namespace StatefulLens; Name StatefulLens.Mkstlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(58,5-58,11); use=StatefulLens.fst(58,5-58,11)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (= (StatefulLens.Mkstlens_st_get (StatefulLens.Mkstlens @x0
@x1
@x2
@x3
@x4))
...
@x3
@x4))
:qid projection_inverse_StatefulLens.Mkstlens_st_get))

:named projection_inverse_StatefulLens.Mkstlens_st_get))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (= (StatefulLens.Mkhlens_put (StatefulLens.Mkhlens @x0
@x1
@x2
@x3))
@x3)
...
:pattern ((StatefulLens.Mkhlens @x0
@x1
@x2
@x3))
:qid projection_inverse_StatefulLens.Mkhlens_put))

:named projection_inverse_StatefulLens.Mkhlens_put))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (= (StatefulLens.Mkhlens_get (StatefulLens.Mkhlens @x0
@x1
@x2
@x3))
@x2)
...
@x2
@x3))
:qid projection_inverse_StatefulLens.Mkhlens_get))

:named projection_inverse_StatefulLens.Mkhlens_get))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (= (StatefulLens.Mkhlens_b (StatefulLens.Mkhlens @x0
@x1
@x2
@x3))
@x1)
...
@x2
@x3))
:qid projection_inverse_StatefulLens.Mkhlens_b))

:named projection_inverse_StatefulLens.Mkhlens_b))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (= (StatefulLens.Mkhlens_a (StatefulLens.Mkhlens @x0
@x1
@x2
@x3))
@x0)
...
@x2
@x3))
:qid projection_inverse_StatefulLens.Mkhlens_a))

:named projection_inverse_StatefulLens.Mkhlens_a))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name FStar.Pervasives.Native.tuple2; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.Mktuple2; Namespace FStar.Pervasives.Native
(assert (! 
;; def=FStar.Pervasives.Native.fst(59,22-59,30); use=FStar.Pervasives.Native.fst(59,22-59,30)
...
@x1
@x2)
(StatefulLens.Mkhlens_put @x2))
 

:pattern ((StatefulLens.__proj__Mkhlens__item__put @x0
@x1
@x2))
:qid proj_equation_StatefulLens.Mkhlens_put))

:named proj_equation_StatefulLens.Mkhlens_put))
;;;;;;;;;;;;;;;;Projector equation
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__get; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(38,5-38,8); use=StatefulLens.fst(38,5-38,8)
...
@x1
@x2)
(StatefulLens.Mkhlens_get @x2))
 

:pattern ((StatefulLens.__proj__Mkhlens__item__get @x0
@x1
@x2))
:qid proj_equation_StatefulLens.Mkhlens_get))

:named proj_equation_StatefulLens.Mkhlens_get))
;;;;;;;;;;;;;;;;kinding
;;; Fact-ids: Name StatefulLens.stlens; Namespace StatefulLens; Name StatefulLens.Mkstlens; Namespace StatefulLens
(assert (! (is-Tm_arrow (PreType StatefulLens.stlens@tok))
:named pre_kinding_StatefulLens.stlens@tok))
;;;;;;;;;;;;;;;;kinding
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! (is-Tm_arrow (PreType StatefulLens.hlens@tok))
:named pre_kinding_StatefulLens.hlens@tok))
;;;;;;;;;;;;;;;;kinding
;;; Fact-ids: Name FStar.Pervasives.Native.tuple2; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.Mktuple2; Namespace FStar.Pervasives.Native
...
;;; Fact-ids: Name StatefulLens.stlens; Namespace StatefulLens; Name StatefulLens.Mkstlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(58,14-58,25); use=StatefulLens.fst(62,16-62,32)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(StatefulLens.hlens @x1
@x0)))
(HasType (Non_total_Tm_arrow_6f94488d6278900545315db3af6b01bf @x0
@x1
@x2)
Tm_type))
 

:pattern ((HasType (Non_total_Tm_arrow_6f94488d6278900545315db3af6b01bf @x0
@x1
@x2)
Tm_type))
:qid non_total_function_typing_Non_total_Tm_arrow_6f94488d6278900545315db3af6b01bf))

:named non_total_function_typing_Non_total_Tm_arrow_6f94488d6278900545315db3af6b01bf))
;;;;;;;;;;;;;;;;Typing for non-total arrows
;;; Fact-ids: Name StatefulLens.stlens; Namespace StatefulLens; Name StatefulLens.Mkstlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(58,14-58,25); use=StatefulLens.fst(59,16-59,25)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(StatefulLens.hlens @x0
@x1)))
(HasType (Non_total_Tm_arrow_2fd0425a15b041bc68405011b81aa070 @x0
@x1
@x2)
Tm_type))
 

:pattern ((HasType (Non_total_Tm_arrow_2fd0425a15b041bc68405011b81aa070 @x0
@x1
@x2)
Tm_type))
:qid non_total_function_typing_Non_total_Tm_arrow_2fd0425a15b041bc68405011b81aa070))

:named non_total_function_typing_Non_total_Tm_arrow_2fd0425a15b041bc68405011b81aa070))
;;;;;;;;;;;;;;;;kinding_Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,13-39,44); use=StatefulLens.fst(39,10-39,44)
(forall ((@x0 Term) (@x1 Term))
 (! (HasType (Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 @x0
@x1)
Tm_type)
 

:pattern ((HasType (Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 @x0
@x1)
Tm_type))
:qid kinding_Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7))

:named kinding_Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7))
;;;;;;;;;;;;;;;;kinding_Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,13-38,20); use=StatefulLens.fst(38,10-38,30)
(forall ((@x0 Term) (@x1 Term))
 (! (HasType (Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b @x0
@x1)
Tm_type)
 

:pattern ((HasType (Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b @x0
@x1)
Tm_type))
:qid kinding_Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b))

:named kinding_Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b))
...
;;;;;;;;;;;;;;;;kinding_Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__put; Namespace StatefulLens
(assert (! (HasType Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7
Tm_type)
:named kinding_Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_3361558004d257244693421d62990cea
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__get; Namespace StatefulLens
(assert (! (HasType Tm_arrow_3361558004d257244693421d62990cea
Tm_type)
:named kinding_Tm_arrow_3361558004d257244693421d62990cea))
...
;;; Fact-ids: Name StatefulLens.stlens; Namespace StatefulLens; Name StatefulLens.Mkstlens; Namespace StatefulLens
(assert (! (and (IsTotFun StatefulLens.stlens@tok)

;; def=StatefulLens.fst(58,5-58,11); use=StatefulLens.fst(58,5-58,11)
(forall ((@x0 Term))
 (! (IsTotFun (ApplyTT StatefulLens.stlens@tok
@x0))
 

:pattern ((ApplyTT StatefulLens.stlens@tok
@x0))
:qid kinding_StatefulLens.stlens@tok))


;; def=StatefulLens.fst(58,5-58,11); use=StatefulLens.fst(58,5-58,11)
(forall ((@x0 Term) (@x1 Term))
 (! (IsTotFun (ApplyTT (ApplyTT StatefulLens.stlens@tok
@x0)
@x1))
 

:pattern ((ApplyTT (ApplyTT StatefulLens.stlens@tok
@x0)
@x1))
:qid kinding_StatefulLens.stlens@tok.1))
...
(StatefulLens.hlens @x0
@x1)))
(HasType (StatefulLens.stlens @x0
@x1
@x2)
Tm_type))
...
@x1
@x2))
:qid kinding_StatefulLens.stlens@tok.2))
)
:named kinding_StatefulLens.stlens@tok))
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! (and (IsTotFun StatefulLens.hlens@tok)

;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@x0 Term))
 (! (IsTotFun (ApplyTT StatefulLens.hlens@tok
@x0))
 

:pattern ((ApplyTT StatefulLens.hlens@tok
@x0))
:qid kinding_StatefulLens.hlens@tok))
...
:pattern ((StatefulLens.hlens @x0
@x1))
:qid kinding_StatefulLens.hlens@tok.1))
)
:named kinding_StatefulLens.hlens@tok))
;;; Fact-ids: Name Prims.trivial; Namespace Prims; Name Prims.T; Namespace Prims
...
;;;;;;;;;;;;;;;;interpretation_Tm_abs_eb58a5c7378d916320d8f69adb3470f7
;;; Fact-ids: Name StatefulLens.compose_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(51,27-51,52); use=StatefulLens.fst(51,27-51,52)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term))
 (! (= (ApplyTT (Tm_abs_eb58a5c7378d916320d8f69adb3470f7 @x1
@x2
@x3
@x4
@x5)
@x0)
(let ((@lb6 @x0))
(ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
@x1)
@x4)
@x2)
(FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
@x1
...
:pattern ((ApplyTT (Tm_abs_eb58a5c7378d916320d8f69adb3470f7 @x1
@x2
@x3
@x4
@x5)
@x0))
:qid interpretation_Tm_abs_eb58a5c7378d916320d8f69adb3470f7))

:named interpretation_Tm_abs_eb58a5c7378d916320d8f69adb3470f7))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_106f73abf9d4751ff515c66ee21e5216
;;; Fact-ids: Name StatefulLens.compose_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(52,32-52,93); use=StatefulLens.fst(52,32-52,93)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term) (@x6 Term))
 (! (= (ApplyTT (ApplyTT (Tm_abs_106f73abf9d4751ff515c66ee21e5216 @x2
@x3
@x4
@x5
@x6)
...
@x1)
(let ((@lb7 @x1))
(let ((@lb8 (ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__put@tok
@x2)
@x6)
@x4)
@x0)
...
(ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__put@tok
@x5)
@x2)
@x3)
...
:pattern ((ApplyTT (ApplyTT (Tm_abs_106f73abf9d4751ff515c66ee21e5216 @x2
@x3
@x4
@x5
@x6)
@x0)
@x1))
:qid interpretation_Tm_abs_106f73abf9d4751ff515c66ee21e5216))

:named interpretation_Tm_abs_106f73abf9d4751ff515c66ee21e5216))
...
(forall ((@x0 Term))
 (! (and (NoHoist @x0
(HasType StatefulLens.__proj__Mkhlens__item__put@tok
Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7))
...
@x2)
@x3)
(StatefulLens.__proj__Mkhlens__item__put @x1
@x2
@x3))
 

:pattern ((StatefulLens.__proj__Mkhlens__item__put @x1
@x2
@x3))
:qid function_token_typing_StatefulLens.__proj__Mkhlens__item__put.1))
)
 

:pattern ((ApplyTT @x0
StatefulLens.__proj__Mkhlens__item__put@tok))
:qid function_token_typing_StatefulLens.__proj__Mkhlens__item__put))

:named function_token_typing_StatefulLens.__proj__Mkhlens__item__put))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__get; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(38,5-38,8); use=StatefulLens.fst(38,5-38,8)
(forall ((@x0 Term))
 (! (and (NoHoist @x0
(HasType StatefulLens.__proj__Mkhlens__item__get@tok
Tm_arrow_3361558004d257244693421d62990cea))
...
@x2)
@x3)
(StatefulLens.__proj__Mkhlens__item__get @x1
@x2
@x3))
 

:pattern ((StatefulLens.__proj__Mkhlens__item__get @x1
@x2
@x3))
:qid function_token_typing_StatefulLens.__proj__Mkhlens__item__get.1))
)
 

:pattern ((ApplyTT @x0
StatefulLens.__proj__Mkhlens__item__get@tok))
:qid function_token_typing_StatefulLens.__proj__Mkhlens__item__get))

:named function_token_typing_StatefulLens.__proj__Mkhlens__item__get))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name Prims.unit; Namespace Prims
(assert (! (HasType Prims.unit
Prims.eqtype)
...
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (implies (HasTypeFuel @u0
@x1
(StatefulLens.stlens @x2
@x3
@x4))
(is-StatefulLens.Mkstlens @x1))
...
@x3
@x4)))
:qid fuel_guarded_inversion_StatefulLens.stlens))

:named fuel_guarded_inversion_StatefulLens.stlens))
;;;;;;;;;;;;;;;;inversion axiom
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (implies (HasTypeFuel @u0
@x1
(StatefulLens.hlens @x2
@x3))
(and (is-StatefulLens.Mkhlens @x1)
(= @x2
(StatefulLens.Mkhlens_a @x1))
(= @x3
(StatefulLens.Mkhlens_b @x1))))
...
(StatefulLens.hlens @x2
@x3)))
:qid fuel_guarded_inversion_StatefulLens.hlens))

:named fuel_guarded_inversion_StatefulLens.hlens))
;;;;;;;;;;;;;;;;inversion axiom
;;; Fact-ids: Name Prims.trivial; Namespace Prims; Name Prims.T; Namespace Prims
(assert (! 
...
;;; Fact-ids: Name StatefulLens.stlens; Namespace StatefulLens; Name StatefulLens.Mkstlens; Namespace StatefulLens
(assert (! (= 148
(Term_constr_id StatefulLens.stlens@tok))
:named fresh_token_StatefulLens.stlens@tok))
;;;;;;;;;;;;;;;;fresh token
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! (= 105
(Term_constr_id StatefulLens.hlens@tok))
:named fresh_token_StatefulLens.hlens@tok))
;;;;;;;;;;;;;;;;fresh token
;;; Fact-ids: Name FStar.Pervasives.Native.tuple2; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.Mktuple2; Namespace FStar.Pervasives.Native
(assert (! (= 126
(Term_constr_id FStar.Pervasives.Native.tuple2@tok))
:named fresh_token_FStar.Pervasives.Native.tuple2@tok))
;;;;;;;;;;;;;;;;Equation for StatefulLens.compose_hlens
;;; Fact-ids: Name StatefulLens.compose_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(50,4-50,17); use=StatefulLens.fst(50,4-50,17)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (= (StatefulLens.compose_hlens @x0
@x1
@x2
@x3
@x4)
(StatefulLens.Mkhlens @x0
@x2
(Tm_abs_eb58a5c7378d916320d8f69adb3470f7 @x1
@x4
@x3
@x2
@x0)
(Tm_abs_106f73abf9d4751ff515c66ee21e5216 @x1
@x3
@x4
@x0
@x2)))
...
@x3
@x4))
:qid equation_StatefulLens.compose_hlens))

:named equation_StatefulLens.compose_hlens))
...
(HasTypeFuel @u0
@x4
(Non_total_Tm_arrow_2fd0425a15b041bc68405011b81aa070 @x1
@x2
@x3))
(HasTypeFuel @u0
@x5
(Non_total_Tm_arrow_6f94488d6278900545315db3af6b01bf @x2
@x1
@x3)))
(HasTypeFuel @u0
(StatefulLens.Mkstlens @x1
...
@x2
@x3)))
:qid data_typing_intro_StatefulLens.Mkstlens@tok))

:named data_typing_intro_StatefulLens.Mkstlens@tok))
;;;;;;;;;;;;;;;;data constructor typing intro
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
...
(HasTypeFuel @u0
@x3
(Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b @x2
@x1))
(HasTypeFuel @u0
@x4
(Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 @x1
@x2)))
(HasTypeFuel @u0
(StatefulLens.Mkhlens @x1
@x2
...
(StatefulLens.hlens @x1
@x2)))
:qid data_typing_intro_StatefulLens.Mkhlens@tok))

:named data_typing_intro_StatefulLens.Mkhlens@tok))
;;;;;;;;;;;;;;;;data constructor typing intro
;;; Fact-ids: Name Prims.trivial; Namespace Prims; Name Prims.T; Namespace Prims
(assert (! 
...
(HasTypeFuel @u0
@x8
(StatefulLens.hlens @x6
@x7))
(HasTypeFuel @u0
@x4
(Non_total_Tm_arrow_2fd0425a15b041bc68405011b81aa070 @x6
@x7
@x8))
(HasTypeFuel @u0
@x5
(Non_total_Tm_arrow_6f94488d6278900545315db3af6b01bf @x7
@x6
@x8))))
...
@x7
@x8)))
:qid data_elim_StatefulLens.Mkstlens))

:named data_elim_StatefulLens.Mkstlens))
;;;;;;;;;;;;;;;;data constructor typing elim
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
...
(HasTypeFuel @u0
@x3
(Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b @x6
@x5))
(HasTypeFuel @u0
@x4
(Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 @x5
@x6))))
...
(StatefulLens.hlens @x5
@x6)))
:qid data_elim_StatefulLens.Mkhlens))

:named data_elim_StatefulLens.Mkhlens))
;;;;;;;;;;;;;;;;data constructor typing elim
;;; Fact-ids: Name FStar.Pervasives.Native.tuple2; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.Mktuple2; Namespace FStar.Pervasives.Native
(assert (! 
;; def=FStar.Pervasives.Native.fst(59,22-59,30); use=FStar.Pervasives.Native.fst(59,22-59,30)
...
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= 147
(Term_constr_id (StatefulLens.stlens @x0
@x1
@x2)))
...
@x1
@x2))
:qid constructor_distinct_StatefulLens.stlens))

:named constructor_distinct_StatefulLens.stlens))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@x0 Term) (@x1 Term))
 (! (= 104
(Term_constr_id (StatefulLens.hlens @x0
@x1)))
 

:pattern ((StatefulLens.hlens @x0
@x1))
:qid constructor_distinct_StatefulLens.hlens))

:named constructor_distinct_StatefulLens.hlens))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name StatefulLens.stlens; Namespace StatefulLens; Name StatefulLens.Mkstlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(58,5-58,11); use=StatefulLens.fst(58,5-58,11)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (= 174
(Term_constr_id (StatefulLens.Mkstlens @x0
@x1
@x2
@x3
@x4)))
...
@x3
@x4))
:qid constructor_distinct_StatefulLens.Mkstlens))

:named constructor_distinct_StatefulLens.Mkstlens))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (= 115
(Term_constr_id (StatefulLens.Mkhlens @x0
@x1
@x2
@x3)))
...
@x2
@x3))
:qid constructor_distinct_StatefulLens.Mkhlens))

:named constructor_distinct_StatefulLens.Mkhlens))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name Prims.unit; Namespace Prims
(assert (! (= 125
...
;; def=StatefulLens.fst(58,5-58,11); use=StatefulLens.fst(58,5-58,11)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (implies (is-StatefulLens.Mkstlens (StatefulLens.Mkstlens @x0
@x1
@x2
@x3
@x4))
...
@x3
@x4))
:qid constructor_base_StatefulLens.Mkstlens))

:named constructor_base_StatefulLens.Mkstlens))
...
@x3
@x4))
(= (Term_constr_id (StatefulLens.stlens @x2
@x3
@x4))
...
@x3
@x4)))
:qid StatefulLens_pretyping_9a31ce84ead86c6edfd9120bec5344be))

:named StatefulLens_pretyping_9a31ce84ead86c6edfd9120bec5344be))
;;;;;;;;;;;;;;;;pretyping
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
...
(StatefulLens.hlens @x2
@x3)))
:qid StatefulLens_pretyping_7af44ca0a7072977ef875cbbfce78db1))

:named StatefulLens_pretyping_7af44ca0a7072977ef875cbbfce78db1))
...
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (implies (HasTypeFuel @u0
@x1
(Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 @x2
@x3))
...
(Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 @x2
@x3)))
:qid StatefulLens_pre_typing_Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7))

:named StatefulLens_pre_typing_Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,13-38,20); use=StatefulLens.fst(38,10-38,30)
...
(Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b @x2
@x3)))
:qid StatefulLens_pre_typing_Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b))

:named StatefulLens_pre_typing_Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__put; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
...
@x1
Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7))
:qid StatefulLens_pre_typing_Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7))

:named StatefulLens_pre_typing_Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__get; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
...
@x1
Tm_arrow_3361558004d257244693421d62990cea))
:qid StatefulLens_pre_typing_Tm_arrow_3361558004d257244693421d62990cea))

:named StatefulLens_pre_typing_Tm_arrow_3361558004d257244693421d62990cea))
;;;;;;;;;;;;;;;;interpretation_Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,13-39,44); use=StatefulLens.fst(39,10-39,44)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
...
@x2)
(HasType @x4
(FStar.Pervasives.Native.tuple2 FStar.Monotonic.Heap.heap
@x1)))
...
@x3)
@x4))
:qid StatefulLens_interpretation_Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7.1))
...
(Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 @x1
@x2)))
:qid StatefulLens_interpretation_Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7))

:named StatefulLens_interpretation_Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7))
;;;;;;;;;;;;;;;;interpretation_Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b
;;; Fact-ids: Name StatefulLens.hlens; Namespace StatefulLens; Name StatefulLens.Mkhlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,13-38,20); use=StatefulLens.fst(38,10-38,30)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
...
:pattern ((ApplyTT @x0
@x3))
:qid StatefulLens_interpretation_Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b.1))
)
...
(Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b @x1
@x2)))
:qid StatefulLens_interpretation_Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b))

:named StatefulLens_interpretation_Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__put; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@x0 Term))
...
@x2)
@x3))
:qid StatefulLens_interpretation_Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7.1))
...
:pattern ((ApplyTT @x0
@x1))
:qid StatefulLens_interpretation_Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7.2))
...
@x1)
@x2))
:qid StatefulLens_interpretation_Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7.3))
))
 

:pattern ((HasTypeZ @x0
Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7))
:qid StatefulLens_interpretation_Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7))

:named StatefulLens_interpretation_Tm_arrow_4521b3799bb370301cd7cb5f8aae39f7))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_3361558004d257244693421d62990cea
;;; Fact-ids: Name StatefulLens.__proj__Mkhlens__item__get; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(37,5-37,10); use=StatefulLens.fst(37,5-37,10)
(forall ((@x0 Term))
...
@x2)
@x3))
:qid StatefulLens_interpretation_Tm_arrow_3361558004d257244693421d62990cea.1))
...
:pattern ((ApplyTT @x0
@x1))
:qid StatefulLens_interpretation_Tm_arrow_3361558004d257244693421d62990cea.2))
...
@x1)
@x2))
:qid StatefulLens_interpretation_Tm_arrow_3361558004d257244693421d62990cea.3))
))
 

:pattern ((HasTypeZ @x0
Tm_arrow_3361558004d257244693421d62990cea))
:qid StatefulLens_interpretation_Tm_arrow_3361558004d257244693421d62990cea))

:named StatefulLens_interpretation_Tm_arrow_3361558004d257244693421d62990cea))
;;;;;;;;;;;;;;;;pretyping
;;; Fact-ids: Name Prims.unit; Namespace Prims
(assert (! 
...
StatefulLens.__proj__Mkhlens__item__get@tok))
:named @kick_partial_app_9e94b5a4e7c82251b9be981b735dc0bd))
;;;;;;;;;;;;;;;;kick_partial_app
;;; Fact-ids: Name StatefulLens.compose_hlens; Namespace StatefulLens
(assert (! (Valid (ApplyTT __uu__PartialApp
StatefulLens.__proj__Mkhlens__item__put@tok))
:named @kick_partial_app_0b0a9c1ce6136149d770efb0fa9de9de))
(push) ;; push{2

; Starting query at StatefulLens.fst(72,6-73,75)
...
(declare-fun Non_total_Tm_arrow_6e02226308028d8afff3f61d76f889c8 (Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Typing for non-total arrows
;;; Fact-ids: 
(assert (! 
;; def=StatefulLens.fst(69,20-69,27); use=StatefulLens.fst(69,55-72,24)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(StatefulLens.hlens @x2
@x1)))
(HasType (Non_total_Tm_arrow_6e02226308028d8afff3f61d76f889c8 @x0
@x1
@x2
@x3
@x4)
...
:pattern ((HasType (Non_total_Tm_arrow_6e02226308028d8afff3f61d76f889c8 @x0
@x1
@x2
@x3
@x4)
Tm_type))
:qid non_total_function_typing_Non_total_Tm_arrow_6e02226308028d8afff3f61d76f889c8))

:named non_total_function_typing_Non_total_Tm_arrow_6e02226308028d8afff3f61d76f889c8))
...
;   (b: Type)
;   (c: Type)
;   (l: StatefulLens.hlens a b)
;   (m: StatefulLens.hlens b c)
;   (sl: StatefulLens.stlens l)
;   (sm: StatefulLens.stlens m).
...
;             h == h1 /\ (StatefulLens.compose_hlens l m).get (h, x) == a ==> p a h1) ==>
;         (forall (a: b) (h1: FStar.Monotonic.Heap.heap).
;             h == h1 /\ l.get (h, x) == a ==>
;             (forall (a: c) (h1: FStar.Monotonic.Heap.heap).
;                 h1 == h1 /\ m.get (h1, a) == a ==> p a h1))) /\
;   (forall (any_result: (x: a -> FStar.ST.ST c)) (z: c) (x: a).
;       (*  - Could not prove post-condition
; *)
;       forall (p: FStar.Pervasives.st_post_h FStar.Monotonic.Heap.heap a)
;         (h: FStar.Monotonic.Heap.heap).
;         (forall (a: a) (h1: FStar.Monotonic.Heap.heap).
;             (h1, a) == (StatefulLens.compose_hlens l m).put z (h, x) ==> p a h1) ==>
;         (forall (a: b) (h1: FStar.Monotonic.Heap.heap).
;             h == h1 /\ l.get (h, x) == a ==>
;             (forall (a: b) (h1: FStar.Monotonic.Heap.heap).
;                 (h1, a) == m.put z (h1, a) ==>
;                 (forall (a: a) (h1: FStar.Monotonic.Heap.heap).
;                     (h1, a) == l.put a (h1, x) ==> p a h1))))


; Context: While encoding a query
; While typechecking the top-level declaration `let compose_stlens`
...
;; def=Prims.fst(459,77-459,89); use=StatefulLens.fst(72,6-73,75)
(and 
;; def=dummy(0,0-0,0); use=StatefulLens.fst(72,6-73,75)
(forall ((@x7 Term))
 (! (implies (HasType @x7
@x0)

;; def=FStar.Pervasives.fsti(484,2-484,63); use=StatefulLens.fst(72,6-73,75)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(72,29-72,52)
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(72,29-72,52)
(or label_3
...
;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(72,29-72,52)
(or label_4
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(72,29-72,52)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(72,29-72,52)
(ApplyTT (ApplyTT @x8
@x10)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(72,43-72,49)
(forall ((@x10 Term) (@x11 Term))
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(72,43-72,49)
(= @x9
@x11)


;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(72,43-72,49)
(= (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
@x0)
@x1)
@x3)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(72,32-72,38)
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(72,32-72,38)
(= @x11
@x13)


;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(72,32-72,38)
(= (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
@x1)
@x2)
@x4)
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(72,32-72,38)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(72,32-72,38)
(ApplyTT (ApplyTT @x8
@x12)
...
;; def=Prims.fst(451,66-451,102); use=StatefulLens.fst(72,6-73,75)
(forall ((@x7 Term))
 (! (implies (HasType @x7
(Non_total_Tm_arrow_6e02226308028d8afff3f61d76f889c8 @x0
@x2
@x1
@x3
@x4))
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(73,35-73,74)
(forall ((@x12 Term) (@x13 Term))
...
;; def=StatefulLens.fst(64,37-64,66); use=StatefulLens.fst(73,35-73,74)
(or label_7
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(73,35-73,74)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(73,35-73,74)
(ApplyTT (ApplyTT @x10
@x12)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(73,62-73,68)
(forall ((@x12 Term) (@x13 Term))
 (! (implies (and (HasType @x12
@x1)
(HasType @x13
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(73,62-73,68)
(= @x11
@x13)


;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(73,62-73,68)
(= (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
@x0)
@x1)
@x3)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(73,49-73,55)
...
;; def=StatefulLens.fst(64,37-64,66); use=StatefulLens.fst(73,49-73,55)
(= (FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
@x1
@x15
@x14)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(73,38-73,44)
...
;; def=StatefulLens.fst(64,37-64,66); use=StatefulLens.fst(73,38-73,44)
(= (FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
@x0
@x17
@x16)
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(73,38-73,44)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(73,38-73,44)
(ApplyTT (ApplyTT @x10
@x16)
...
; encoding sigelt let compose_stlens


; <Start encoding let compose_stlens>

(declare-fun StatefulLens.compose_stlens (Term Term Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;sl: stlens l -> sm: stlens m -> stlens (compose_hlens l m)
(declare-fun Tm_arrow_c875932dc001902997da3c84a1d6c14f () Term)
(declare-fun StatefulLens.compose_stlens@tok () Term)
;;;;;;;;;;;;;;;;Imprecise function encoding
(declare-fun Tm_abs_StatefulLens_254 (Term Term Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Imprecise function encoding
(declare-fun Tm_abs_StatefulLens_255 (Term Term Term Term Term Term Term) Term)

; </end encoding let compose_stlens>

;;;;;;;;;;;;;;;;typing_Tm_abs_67044bd756546113eab810f29d691656
;;; Fact-ids: Name StatefulLens.as_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(45,25-45,30); use=StatefulLens.fst(45,25-45,30)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (HasType (Tm_abs_67044bd756546113eab810f29d691656 @x0
@x1
@x2)
(Tm_ghost_arrow_7fa68e03f9ead717b8c201739cf85d2b @x1
@x2))
...
:pattern ((Tm_abs_67044bd756546113eab810f29d691656 @x0
@x1
@x2))
:qid typing_Tm_abs_67044bd756546113eab810f29d691656))

:named typing_Tm_abs_67044bd756546113eab810f29d691656))
;;;;;;;;;;;;;;;;typing_Tm_abs_4c574a44c59a8826c9f20f8889049f16
;;; Fact-ids: Name StatefulLens.as_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(46,27-46,42); use=StatefulLens.fst(46,27-46,42)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (HasType (Tm_abs_4c574a44c59a8826c9f20f8889049f16 @x0
@x1
@x2)
(Tm_ghost_arrow_e18a0c1c7ad560d0351136e0611744d7 @x1
@x2))
...
:pattern ((Tm_abs_4c574a44c59a8826c9f20f8889049f16 @x0
@x1
@x2))
:qid typing_Tm_abs_4c574a44c59a8826c9f20f8889049f16))

:named typing_Tm_abs_4c574a44c59a8826c9f20f8889049f16))
...
;;; Fact-ids: Name StatefulLens.as_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(44,4-44,12); use=StatefulLens.fst(44,4-44,12)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(Lens.lens @x0
@x1)))
(HasType (StatefulLens.as_hlens @x0
@x1
@x2)
(StatefulLens.hlens @x0
@x1)))
...
:pattern ((StatefulLens.as_hlens @x0
@x1
@x2))
:qid typing_StatefulLens.as_hlens))

:named typing_StatefulLens.as_hlens))
;;;;;;;;;;;;;;;;free var typing
...
;;;;;;;;;;;;;;;;interpretation_Tm_abs_67044bd756546113eab810f29d691656
;;; Fact-ids: Name StatefulLens.as_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(45,25-45,30); use=StatefulLens.fst(45,25-45,30)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (= (ApplyTT (Tm_abs_67044bd756546113eab810f29d691656 @x1
@x2
@x3)
@x0)
(let ((@lb4 @x0))
(Lens.op_String_Access @x3
@x2
(FStar.Pervasives.Native.Mktuple2__2 @lb4)
@x1)))
 

:pattern ((ApplyTT (Tm_abs_67044bd756546113eab810f29d691656 @x1
@x2
@x3)
@x0))
:qid interpretation_Tm_abs_67044bd756546113eab810f29d691656))

:named interpretation_Tm_abs_67044bd756546113eab810f29d691656))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_4c574a44c59a8826c9f20f8889049f16
;;; Fact-ids: Name StatefulLens.as_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(46,27-46,42); use=StatefulLens.fst(46,27-46,42)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (= (ApplyTT (ApplyTT (Tm_abs_4c574a44c59a8826c9f20f8889049f16 @x2
@x3
@x4)
@x0)
@x1)
...
:pattern ((ApplyTT (ApplyTT (Tm_abs_4c574a44c59a8826c9f20f8889049f16 @x2
@x3
@x4)
@x0)
@x1))
:qid interpretation_Tm_abs_4c574a44c59a8826c9f20f8889049f16))

:named interpretation_Tm_abs_4c574a44c59a8826c9f20f8889049f16))
...
;;;;;;;;;;;;;;;;Equation for StatefulLens.as_hlens
;;; Fact-ids: Name StatefulLens.as_hlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(44,4-44,12); use=StatefulLens.fst(44,4-44,12)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (StatefulLens.as_hlens @x0
@x1
@x2)
(StatefulLens.Mkhlens @x0
@x1
(Tm_abs_67044bd756546113eab810f29d691656 @x2
@x1
@x0)
(Tm_abs_4c574a44c59a8826c9f20f8889049f16 @x2
@x0
@x1)))
...
@x1
@x2))
:qid equation_StatefulLens.as_hlens))

:named equation_StatefulLens.as_hlens))
...
; Starting query at StatefulLens.fst(80,4-81,46)
...
(declare-fun Non_total_Tm_arrow_c90ac54976f656fb24e0e2dab8d22c57 (Term Term Term) Term)
;;;;;;;;;;;;;;;;Typing for non-total arrows
;;; Fact-ids: 
(assert (! 
;; def=StatefulLens.fst(79,22-79,27); use=StatefulLens.fst(79,25-80,23)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(Lens.lens @x0
@x1)))
(HasType (Non_total_Tm_arrow_c90ac54976f656fb24e0e2dab8d22c57 @x0
@x1
@x2)
Tm_type))
 

:pattern ((HasType (Non_total_Tm_arrow_c90ac54976f656fb24e0e2dab8d22c57 @x0
@x1
@x2)
Tm_type))
:qid non_total_function_typing_Non_total_Tm_arrow_c90ac54976f656fb24e0e2dab8d22c57))

:named non_total_function_typing_Non_total_Tm_arrow_c90ac54976f656fb24e0e2dab8d22c57))
...
;         (h: FStar.Monotonic.Heap.heap).
;         (forall (a: 'b) (h1: FStar.Monotonic.Heap.heap).
;             h == h1 /\ (StatefulLens.as_hlens l).get (h, x) == a ==> p a h1) ==>
;         (forall (return_val: 'b). return_val == x.[ l ] ==> p return_val h)) /\
;   (forall (any_result: (x: 'a -> FStar.ST.ST 'b)) (y: 'b) (x: 'a).
;       (*  - Could not prove post-condition
; *)
;       forall (p: FStar.Pervasives.st_post_h FStar.Monotonic.Heap.heap 'a)
;         (h: FStar.Monotonic.Heap.heap).
;         (forall (a: 'a) (h1: FStar.Monotonic.Heap.heap).
;             (h1, a) == (StatefulLens.as_hlens l).put y (h, x) ==> p a h1) ==>
;         (forall (return_val: 'a). return_val == (x.[ l ] <- y) ==> p return_val h))


; Context: While encoding a query
; While typechecking the top-level declaration `let as_stlens`
...
;; def=Prims.fst(459,77-459,89); use=StatefulLens.fst(80,4-81,46)
(and 
;; def=dummy(0,0-0,0); use=StatefulLens.fst(80,4-81,46)
(forall ((@x3 Term))
 (! (implies (HasType @x3
@x0)

;; def=FStar.Pervasives.fsti(484,2-484,63); use=StatefulLens.fst(80,4-81,46)
(forall ((@x4 Term) (@x5 Term))
 (! (implies (and (HasType @x4
(FStar.Pervasives.st_post_h FStar.Monotonic.Heap.heap
@x1))
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(80,28-80,33)
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(80,28-80,33)
(or label_3
...
;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(80,28-80,33)
(or label_4
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(80,28-80,33)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(80,28-80,33)
(ApplyTT (ApplyTT @x4
@x6)
...
;; def=Prims.fst(356,2-356,58); use=StatefulLens.fst(80,4-81,46)
(forall ((@x6 Term))
 (! (implies (and (HasType @x6
@x1)

;; def=Prims.fst(356,26-356,41); use=StatefulLens.fst(80,4-81,46)
(= @x6
(Lens.op_String_Access @x0
@x1
@x3
...
;; def=FStar.ST.fst(33,87-33,92); use=StatefulLens.fst(80,4-81,46)
(Valid 
;; def=FStar.ST.fst(33,87-33,92); use=StatefulLens.fst(80,4-81,46)
(ApplyTT (ApplyTT @x4
@x6)
...
;; def=Prims.fst(451,66-451,102); use=StatefulLens.fst(80,4-81,46)
(forall ((@x3 Term))
 (! (implies (HasType @x3
(Non_total_Tm_arrow_c90ac54976f656fb24e0e2dab8d22c57 @x0
@x1
@x2))
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(81,35-81,45)
(forall ((@x8 Term) (@x9 Term))
 (! (implies (and (or label_5
(HasType @x8
@x0))
...
;; def=StatefulLens.fst(64,37-64,66); use=StatefulLens.fst(81,35-81,45)
(or label_7
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(81,35-81,45)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(81,35-81,45)
(ApplyTT (ApplyTT @x6
@x8)
...
; encoding sigelt let as_stlens


; <Start encoding let as_stlens>

(declare-fun StatefulLens.as_stlens (Term Term Term) Term)
;;;;;;;;;;;;;;;;l: Lens.lens 'a 'b -> stlens (as_hlens l)
(declare-fun Tm_arrow_9afc543e9c761ff52a712f7334a85e19 () Term)
(declare-fun StatefulLens.as_stlens@tok () Term)
;;;;;;;;;;;;;;;;Imprecise function encoding
(declare-fun Tm_abs_StatefulLens_268 (Term Term Term) Term)
;;;;;;;;;;;;;;;;Imprecise function encoding
(declare-fun Tm_abs_StatefulLens_269 (Term Term Term) Term)

; </end encoding let as_stlens>


; encoding sigelt let hlens_ref


; <Start encoding let hlens_ref>

(declare-fun StatefulLens.hlens_ref (Term) Term)
;;;;;;;;;;;;;;;;hlens (FStar.ST.ref a) a
(declare-fun Tm_arrow_884a93777972a0ef41c1cc68d2d461da () Term)
(declare-fun StatefulLens.hlens_ref@tok () Term)
;;;;;;;;;;;;;;;;_: FStar.Monotonic.Heap.heap & FStar.ST.ref a -> Prims.GTot a
(declare-fun Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913 (Term) Term)
(declare-fun Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220 (Term) Term)
;;;;;;;;;;;;;;;;y: a -> _: FStar.Monotonic.Heap.heap & FStar.ST.ref a   -> Prims.GTot (FStar.Monotonic.Heap.heap & FStar.ST.ref a)
(declare-fun Tm_ghost_arrow_e781a315e344b695435caeddfbaad871 (Term) Term)
(declare-fun Tm_abs_7ce64cbdc33523e52445d94bdd42f107 (Term) Term)

; </end encoding let hlens_ref>
...
;;;;;;;;;;;;;;;;typing_Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(86,26-86,33); use=StatefulLens.fst(86,26-86,33)
(forall ((@x0 Term))
 (! (HasType (Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220 @x0)
(Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913 @x0))
 

:pattern ((Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220 @x0))
:qid typing_Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220))

:named typing_Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220))
;;;;;;;;;;;;;;;;typing_Tm_abs_7ce64cbdc33523e52445d94bdd42f107
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(87,28-87,42); use=StatefulLens.fst(87,28-87,42)
(forall ((@x0 Term))
 (! (HasType (Tm_abs_7ce64cbdc33523e52445d94bdd42f107 @x0)
(Tm_ghost_arrow_e781a315e344b695435caeddfbaad871 @x0))
 

:pattern ((Tm_abs_7ce64cbdc33523e52445d94bdd42f107 @x0))
:qid typing_Tm_abs_7ce64cbdc33523e52445d94bdd42f107))

:named typing_Tm_abs_7ce64cbdc33523e52445d94bdd42f107))
...
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(85,4-85,13); use=StatefulLens.fst(85,4-85,13)
(forall ((@x0 Term))
 (! (implies (HasType @x0
Tm_type)
(HasType (StatefulLens.hlens_ref @x0)
(StatefulLens.hlens (FStar.ST.ref @x0)
@x0)))
 

:pattern ((StatefulLens.hlens_ref @x0))
:qid typing_StatefulLens.hlens_ref))

:named typing_StatefulLens.hlens_ref))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Prims.prop; Namespace Prims
...
;;;;;;;;;;;;;;;;kinding_Tm_ghost_arrow_e781a315e344b695435caeddfbaad871
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(39,15-85,17); use=StatefulLens.fst(85,37-87,43)
(forall ((@x0 Term))
 (! (HasType (Tm_ghost_arrow_e781a315e344b695435caeddfbaad871 @x0)
Tm_type)
 

:pattern ((HasType (Tm_ghost_arrow_e781a315e344b695435caeddfbaad871 @x0)
Tm_type))
:qid kinding_Tm_ghost_arrow_e781a315e344b695435caeddfbaad871))

:named kinding_Tm_ghost_arrow_e781a315e344b695435caeddfbaad871))
;;;;;;;;;;;;;;;;kinding_Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(38,10-85,17); use=StatefulLens.fst(85,37-87,43)
(forall ((@x0 Term))
 (! (HasType (Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913 @x0)
Tm_type)
 

:pattern ((HasType (Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913 @x0)
Tm_type))
:qid kinding_Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913))

:named kinding_Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913))
...
;;;;;;;;;;;;;;;;interpretation_Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(86,26-86,33); use=StatefulLens.fst(86,26-86,33)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ApplyTT (Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220 @x1)
@x0)
(let ((@lb2 @x0))
(FStar.Monotonic.Heap.sel @x1
(FStar.Heap.trivial_preorder @x1)
(FStar.Pervasives.Native.Mktuple2__1 @lb2)
(FStar.Pervasives.Native.Mktuple2__2 @lb2))))
 

:pattern ((ApplyTT (Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220 @x1)
@x0))
:qid interpretation_Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220))

:named interpretation_Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_7ce64cbdc33523e52445d94bdd42f107
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(87,28-87,42); use=StatefulLens.fst(87,28-87,42)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (ApplyTT (ApplyTT (Tm_abs_7ce64cbdc33523e52445d94bdd42f107 @x2)
@x0)
@x1)
(let ((@lb3 @x1))
(FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
(FStar.ST.ref @x2)
(FStar.Monotonic.Heap.upd @x2
...
:pattern ((ApplyTT (ApplyTT (Tm_abs_7ce64cbdc33523e52445d94bdd42f107 @x2)
@x0)
@x1))
:qid interpretation_Tm_abs_7ce64cbdc33523e52445d94bdd42f107))

:named interpretation_Tm_abs_7ce64cbdc33523e52445d94bdd42f107))
...
;;;;;;;;;;;;;;;;Equation for StatefulLens.hlens_ref
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(85,4-85,13); use=StatefulLens.fst(85,4-85,13)
(forall ((@x0 Term))
 (! (= (StatefulLens.hlens_ref @x0)
(StatefulLens.Mkhlens (FStar.ST.ref @x0)
@x0
(Tm_abs_9f4f7acfd7a439677b2ba928cc1d7220 @x0)
(Tm_abs_7ce64cbdc33523e52445d94bdd42f107 @x0)))
 

:pattern ((StatefulLens.hlens_ref @x0))
:qid equation_StatefulLens.hlens_ref))

:named equation_StatefulLens.hlens_ref))
...
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term))
 (! (implies (HasTypeFuel @u0
@x1
(Tm_ghost_arrow_e781a315e344b695435caeddfbaad871 @x2))
(is-Tm_arrow (PreType @x1)))
...
@x1
(Tm_ghost_arrow_e781a315e344b695435caeddfbaad871 @x2)))
:qid StatefulLens_pre_typing_Tm_ghost_arrow_e781a315e344b695435caeddfbaad871))

:named StatefulLens_pre_typing_Tm_ghost_arrow_e781a315e344b695435caeddfbaad871))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(38,10-85,17); use=StatefulLens.fst(85,37-87,43)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term))
 (! (implies (HasTypeFuel @u0
@x1
(Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913 @x2))
(is-Tm_arrow (PreType @x1)))
...
@x1
(Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913 @x2)))
:qid StatefulLens_pre_typing_Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913))

:named StatefulLens_pre_typing_Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913))
;;;;;;;;;;;;;;;;interpretation_Tm_ghost_arrow_e781a315e344b695435caeddfbaad871
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(39,15-85,17); use=StatefulLens.fst(85,37-87,43)
(forall ((@x0 Term) (@x1 Term))
 (! (iff (HasTypeZ @x0
(Tm_ghost_arrow_e781a315e344b695435caeddfbaad871 @x1))
(and 
;; def=StatefulLens.fst(39,15-85,17); use=StatefulLens.fst(85,37-87,43)
(forall ((@x2 Term) (@x3 Term))
 (! (implies (and (HasType @x2
...
@x2)
@x3))
:qid StatefulLens_interpretation_Tm_ghost_arrow_e781a315e344b695435caeddfbaad871.1))
...
:pattern ((HasTypeZ @x0
(Tm_ghost_arrow_e781a315e344b695435caeddfbaad871 @x1)))
:qid StatefulLens_interpretation_Tm_ghost_arrow_e781a315e344b695435caeddfbaad871))

:named StatefulLens_interpretation_Tm_ghost_arrow_e781a315e344b695435caeddfbaad871))
;;;;;;;;;;;;;;;;interpretation_Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913
;;; Fact-ids: Name StatefulLens.hlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(38,10-85,17); use=StatefulLens.fst(85,37-87,43)
(forall ((@x0 Term) (@x1 Term))
 (! (iff (HasTypeZ @x0
(Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913 @x1))
...
:pattern ((ApplyTT @x0
@x2))
:qid StatefulLens_interpretation_Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913.1))
)
 

:pattern ((HasTypeZ @x0
(Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913 @x1)))
:qid StatefulLens_interpretation_Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913))

:named StatefulLens_interpretation_Tm_ghost_arrow_15b4c759ddecd18753e419cc85cdd913))
;;;;;;;;;;;;;;;;pretyping
;;; Fact-ids: Name Prims.bool; Namespace Prims
(assert (! 
...
; Starting query at StatefulLens.fst(92,5-93,53)
...
(declare-fun Non_total_Tm_arrow_8ff6e77f49d857298be34dc019f39fa8 (Term) Term)
;;;;;;;;;;;;;;;;Typing for non-total arrows
;;; Fact-ids: 
(assert (! 
;; def=StatefulLens.fst(91,17-92,27); use=StatefulLens.fst(92,5-93,53)
(forall ((@x0 Term))
 (! (implies (HasType @x0
Tm_type)
(HasType (Non_total_Tm_arrow_8ff6e77f49d857298be34dc019f39fa8 @x0)
Tm_type))
 

:pattern ((HasType (Non_total_Tm_arrow_8ff6e77f49d857298be34dc019f39fa8 @x0)
Tm_type))
:qid non_total_function_typing_Non_total_Tm_arrow_8ff6e77f49d857298be34dc019f39fa8))

:named non_total_function_typing_Non_total_Tm_arrow_8ff6e77f49d857298be34dc019f39fa8))


; Encoding query formula : forall (a: Type).
;   (*  - Could not prove post-condition
; *)
;   (forall (x: FStar.ST.ref a).
;       (*  - Could not prove post-condition
; *)
;       forall (p: FStar.Pervasives.st_post_h FStar.Monotonic.Heap.heap a)
;         (h: FStar.Monotonic.Heap.heap).
;         (forall (a: a) (h1: FStar.Monotonic.Heap.heap).
;             h == h1 /\ StatefulLens.hlens_ref.get (h, x) == a ==> p a h1) ==>
;         p (FStar.Monotonic.Heap.sel h x) h) /\
;   (forall (any_result: (x: FStar.ST.ref a -> FStar.ST.ST a)) (y: a) (x: FStar.ST.ref a).
;       (*  - Could not prove post-condition
; *)
...
;             (h1, a) == StatefulLens.hlens_ref.put y (h, x) ==> p a h1) ==>
;         (forall (a: Prims.unit) (h1: FStar.Monotonic.Heap.heap).
;             h1 == FStar.Monotonic.Heap.upd h x y ==> p x h1))


; Context: While encoding a query
; While typechecking the top-level declaration `let stlens_ref`
...
;; def=Prims.fst(459,77-459,89); use=StatefulLens.fst(92,5-93,53)
(and 
;; def=dummy(0,0-0,0); use=StatefulLens.fst(92,5-93,53)
(forall ((@x1 Term))
 (! (implies (HasType @x1
(FStar.ST.ref @x0))

;; def=FStar.Pervasives.fsti(484,2-484,63); use=StatefulLens.fst(92,5-93,53)
(forall ((@x2 Term) (@x3 Term))
 (! (implies (and (HasType @x2
(FStar.Pervasives.st_post_h FStar.Monotonic.Heap.heap
@x0))
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(92,32-92,34)
(forall ((@x4 Term) (@x5 Term))
 (! (implies (and (or label_1
(HasType @x4
@x0))
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(92,32-92,34)
(or label_3
...
;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(92,32-92,34)
(or label_4
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(92,32-92,34)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(92,32-92,34)
(ApplyTT (ApplyTT @x2
@x4)
...
;; def=FStar.Ref.fst(69,54-69,67); use=StatefulLens.fst(92,32-92,33)
(Valid 
;; def=FStar.Ref.fst(69,54-69,67); use=StatefulLens.fst(92,32-92,33)
(ApplyTT (ApplyTT @x2
(FStar.Monotonic.Heap.sel @x0
...
;; def=Prims.fst(451,66-451,102); use=StatefulLens.fst(92,5-93,53)
(forall ((@x1 Term))
 (! (implies (HasType @x1
(Non_total_Tm_arrow_8ff6e77f49d857298be34dc019f39fa8 @x0))
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(93,38-93,52)
(forall ((@x6 Term) (@x7 Term))
 (! (implies (and (or label_5
(HasType @x6
(FStar.ST.ref @x0)))
...
;; def=StatefulLens.fst(64,37-64,66); use=StatefulLens.fst(93,38-93,52)
(or label_7
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(93,38-93,52)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(93,38-93,52)
(ApplyTT (ApplyTT @x4
@x6)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(93,38-93,45)
(forall ((@x6 Term) (@x7 Term))
 (! (implies (and (HasType @x6
Prims.unit)
(HasType @x7
...
;; def=StatefulLens.fst(30,59-30,75); use=StatefulLens.fst(93,38-93,45)
(= @x7
(FStar.Monotonic.Heap.upd @x0
(FStar.Heap.trivial_preorder @x0)
@x5
...
;; def=FStar.Pervasives.fsti(463,24-463,34); use=StatefulLens.fst(93,38-93,52)
(Valid 
;; def=FStar.Pervasives.fsti(463,24-463,34); use=StatefulLens.fst(93,38-93,52)
(ApplyTT (ApplyTT @x4
@x3)
...
; encoding sigelt let stlens_ref


; <Start encoding let stlens_ref>

(declare-fun StatefulLens.stlens_ref (Term) Term)
;;;;;;;;;;;;;;;;stlens hlens_ref
(declare-fun Tm_arrow_192493b779883cedbe439d20ede935d7 () Term)
(declare-fun StatefulLens.stlens_ref@tok () Term)
;;;;;;;;;;;;;;;;Imprecise function encoding
(declare-fun Tm_abs_StatefulLens_288 (Term) Term)
;;;;;;;;;;;;;;;;Imprecise function encoding
(declare-fun Tm_abs_StatefulLens_289 (Term) Term)

; </end encoding let stlens_ref>

(push) ;; push{2

; Starting query at StatefulLens.fst(105,4-105,51)
...
; Encoding query formula : forall (c: FStar.ST.ref (FStar.ST.ref Prims.int)).
;   (*  - Could not prove post-condition
; *)
;   forall (p: FStar.Pervasives.st_post_h FStar.Monotonic.Heap.heap Prims.int)
;     (h: FStar.Monotonic.Heap.heap).
;     (forall (a: Prims.int) (h1: FStar.Monotonic.Heap.heap).
;         h == h1 /\ a == FStar.Monotonic.Heap.sel h1 (FStar.Monotonic.Heap.sel h1 c) ==> p a h1) ==>
;     (forall (a: Prims.int) (h1: FStar.Monotonic.Heap.heap).
;         h == h1 /\
;         (StatefulLens.compose_hlens StatefulLens.hlens_ref StatefulLens.hlens_ref).get (h, c) == a ==>
;         p a h1)
...
(assert (! (not (forall ((@x0 Term))
 (! (implies (HasType @x0
(FStar.ST.ref (FStar.ST.ref Prims.int)))
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(105,4-105,51)
(forall ((@x3 Term) (@x4 Term))
 (! (implies (and (or label_1
(HasType @x3
Prims.int))
...
;; def=StatefulLens.fst(104,35-104,43); use=StatefulLens.fst(105,4-105,51)
(or label_3
...
;; def=StatefulLens.fst(104,47-104,69); use=StatefulLens.fst(105,4-105,51)
(or label_4
...
@x4
(FStar.Monotonic.Heap.sel (FStar.ST.ref Prims.int)
(FStar.Heap.trivial_preorder (FStar.ST.ref Prims.int))
@x4
@x0)))
)
)

;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(105,4-105,51)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(105,4-105,51)
(ApplyTT (ApplyTT @x1
@x3)
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(105,43-105,49)
(= @x2
@x4)


;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(105,43-105,49)
(= (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
(FStar.ST.ref (FStar.ST.ref Prims.int)))
Prims.int)
(StatefulLens.compose_hlens (FStar.ST.ref (FStar.ST.ref Prims.int))
(FStar.ST.ref Prims.int)
Prims.int
(StatefulLens.hlens_ref (FStar.ST.ref Prims.int))
(StatefulLens.hlens_ref Prims.int)))
(FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
(FStar.ST.ref (FStar.ST.ref Prims.int))
@x2
@x0))
...
;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun StatefulLens.test0 (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun StatefulLens.test0@tok () Term)
...
; Starting query at StatefulLens.fst(116,5-116,54)
...
(declare-fun Tm_abs_226ef2c50b4e410e169f061746e2661f (Term) Term)
;;;;;;;;;;;;;;;;typing_Tm_abs_226ef2c50b4e410e169f061746e2661f
;;; Fact-ids: 
(assert (! 
;; def=StatefulLens.fst(110,30-110,60); use=StatefulLens.fst(116,5-116,54)
(forall ((@x0 Term))
 (! (HasType (Tm_abs_226ef2c50b4e410e169f061746e2661f @x0)
Tm_arrow_c4185f52507383203d0d6335fd0332a8)
 

:pattern ((Tm_abs_226ef2c50b4e410e169f061746e2661f @x0))
:qid typing_Tm_abs_226ef2c50b4e410e169f061746e2661f))

:named typing_Tm_abs_226ef2c50b4e410e169f061746e2661f))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_226ef2c50b4e410e169f061746e2661f
;;; Fact-ids: 
(assert (! 
;; def=StatefulLens.fst(110,30-110,60); use=StatefulLens.fst(116,5-116,54)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ApplyTT (Tm_abs_226ef2c50b4e410e169f061746e2661f @x1)
@x0)
(Prims.b2t (Prims.op_disEquality Prims.nat
(FStar.Monotonic.Heap.addr_of Prims.int
(FStar.Heap.trivial_preorder Prims.int)
(FStar.Monotonic.Heap.sel (FStar.ST.ref Prims.int)
(FStar.Heap.trivial_preorder (FStar.ST.ref Prims.int))
@x0
@x1))
(FStar.Monotonic.Heap.addr_of (FStar.ST.ref Prims.int)
(FStar.Heap.trivial_preorder (FStar.ST.ref Prims.int))
@x1))))
 

:pattern ((ApplyTT (Tm_abs_226ef2c50b4e410e169f061746e2661f @x1)
@x0))
:qid interpretation_Tm_abs_226ef2c50b4e410e169f061746e2661f))

:named interpretation_Tm_abs_226ef2c50b4e410e169f061746e2661f))

(declare-fun Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088 (Term Term) Term)
;;;;;;;;;;;;;;;;refinement kinding
;;; Fact-ids: 
(assert (! 
;; def=FStar.ST.fst(75,66-75,73); use=StatefulLens.fst(116,5-116,54)
(forall ((@x0 Term) (@x1 Term))
 (! (HasType (Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088 @x0
@x1)
Tm_type)
 

:pattern ((HasType (Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088 @x0
@x1)
Tm_type))
:qid refinement_kinding_Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088))

:named refinement_kinding_Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088))
;;;;;;;;;;;;;;;;refinement_interpretation
;;; Fact-ids: 
(assert (! 
;; def=FStar.ST.fst(75,66-75,73); use=StatefulLens.fst(116,5-116,54)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (iff (HasTypeFuel @u0
@x1
(Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088 @x2
@x3))
(and (HasTypeFuel @u0
@x1
FStar.Monotonic.Heap.heap)
...
;; def=StatefulLens.fst(110,30-110,60); use=StatefulLens.fst(116,5-116,54)
(not (= (FStar.Monotonic.Heap.addr_of Prims.int
(FStar.Heap.trivial_preorder Prims.int)
(FStar.Monotonic.Heap.sel (FStar.ST.ref Prims.int)
(FStar.Heap.trivial_preorder (FStar.ST.ref Prims.int))
@x2
...
(Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088 @x2
@x3)))
:qid refinement_interpretation_Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088))

:named refinement_interpretation_Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088))
;;;;;;;;;;;;;;;;haseq for Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088
;;; Fact-ids: 
(assert (! 
;; def=FStar.ST.fst(75,66-75,73); use=StatefulLens.fst(116,5-116,54)
(forall ((@x0 Term) (@x1 Term))
 (! (iff (Valid (Prims.hasEq (Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088 @x0
@x1)))
(Valid (Prims.hasEq FStar.Monotonic.Heap.heap)))
 

:pattern ((Valid (Prims.hasEq (Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088 @x0
@x1))))
:qid haseqTm_refine_2a4aff828cfe8cabe6c5d05bc88b6088))

:named haseqTm_refine_2a4aff828cfe8cabe6c5d05bc88b6088))


; Encoding query formula : forall (c: FStar.ST.ref (FStar.ST.ref Prims.int)).
;   (forall (h: FStar.Monotonic.Heap.heap).
;       (*  - Could not prove post-condition
; *) Prims.hasEq Prims.nat) /\
;   (forall (any_result: FStar.ST.st_pre).
;       (fun h ->
;           FStar.Monotonic.Heap.addr_of (FStar.Monotonic.Heap.sel h c) <>
;           FStar.Monotonic.Heap.addr_of c) ==
;       any_result ==>
;       (forall (h0: FStar.Monotonic.Heap.heap)
;           (d: FStar.ST.ref (FStar.ST.ref Prims.int))
...
;           Prims.hasEq Prims.nat)) /\
;   (forall (p:
;       FStar.Pervasives.st_post_h FStar.Monotonic.Heap.heap (FStar.ST.ref (FStar.ST.ref Prims.int)))
;       (h: FStar.Monotonic.Heap.heap).
;       FStar.Monotonic.Heap.addr_of (FStar.Monotonic.Heap.sel h c) <> FStar.Monotonic.Heap.addr_of c /\
;       (forall (a: FStar.ST.ref (FStar.ST.ref Prims.int))
;           (h1:
;           _:
;           FStar.Monotonic.Heap.heap
;             { FStar.Monotonic.Heap.addr_of (FStar.Monotonic.Heap.sel h c) <>
...
;           (h1,
;           a) ==
;           (StatefulLens.compose_hlens StatefulLens.hlens_ref StatefulLens.hlens_ref).put 0 (h, c) /\
;           h1 ==
;           FStar.Monotonic.Heap.upd (FStar.Monotonic.Heap.upd h (FStar.Monotonic.Heap.sel h c) 0)
;             c
;             (FStar.Monotonic.Heap.sel h c) /\
;           FStar.Monotonic.Heap.sel h c == FStar.Monotonic.Heap.sel h1 c /\
;           FStar.Monotonic.Heap.sel h1 (FStar.Monotonic.Heap.sel h1 c) = 0 ==>
;           p a h1) ==>
;       (forall (a: FStar.ST.ref (FStar.ST.ref Prims.int)) (h1: FStar.Monotonic.Heap.heap).
;           (h1,
;           a) ==
;           (StatefulLens.compose_hlens StatefulLens.hlens_ref StatefulLens.hlens_ref).put 0 (h, c) ==>
;           p a h1))
...
;; def=FStar.Pervasives.fsti(484,2-484,63); use=FStar.Pervasives.fsti(503,15-503,26)
(and 
;; def=dummy(0,0-0,0); use=StatefulLens.fst(116,5-116,54)
(forall ((@x1 Term))
 (! (implies (HasType @x1
FStar.Monotonic.Heap.heap)

;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(110,30-110,37)
(or label_1

;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(116,5-116,54)
(Valid 
;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(116,5-116,54)
(Prims.hasEq Prims.nat)
)
...
;; def=Prims.fst(451,66-451,102); use=StatefulLens.fst(116,5-116,54)
(forall ((@x1 Term))
 (! (implies (and (HasType @x1
FStar.ST.st_pre)

;; def=FStar.ST.fst(75,20-75,23); use=StatefulLens.fst(116,5-116,54)
(= (Tm_abs_226ef2c50b4e410e169f061746e2661f @x0)
@x1)
)
...
;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(109,30-109,32)
(or label_2
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(116,5-116,54)
(forall ((@x3 Term) (@x4 Term))
...
;; def=StatefulLens.fst(112,22-112,28); use=StatefulLens.fst(116,5-116,54)
(or label_5
...
;; def=StatefulLens.fst(113,22-113,82); use=StatefulLens.fst(116,5-116,54)
(or label_6
...
;; def=StatefulLens.fst(114,22-114,66); use=StatefulLens.fst(116,5-116,54)
(or label_7

;; def=StatefulLens.fst(114,22-114,66); use=StatefulLens.fst(116,5-116,54)
(= @x4
(FStar.Monotonic.Heap.upd (FStar.ST.ref Prims.int)
(FStar.Heap.trivial_preorder (FStar.ST.ref Prims.int))
...
;; def=StatefulLens.fst(115,22-115,42); use=StatefulLens.fst(116,5-116,54)
(or label_8
...
;; def=StatefulLens.fst(115,46-115,67); use=StatefulLens.fst(116,5-116,54)
(or label_9
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(116,5-116,54)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(116,5-116,54)
(ApplyTT (ApplyTT @x1
@x3)
...
;; def=StatefulLens.fst(64,37-64,66); use=StatefulLens.fst(116,44-116,50)
(= (FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
(FStar.ST.ref (FStar.ST.ref Prims.int))
@x4
@x3)
...
;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun StatefulLens.test1 (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun StatefulLens.test1@tok () Term)

; </end encoding let test1>

(push) ;; push{2

; Starting query at StatefulLens.fst(122,86-124,54)
...
;;; Fact-ids: 
(assert (! 
;; def=StatefulLens.fst(121,30-121,60); use=StatefulLens.fst(122,86-124,54)
(forall ((@x0 Term))
 (! (HasType (Tm_abs_226ef2c50b4e410e169f061746e2661f @x0)
Tm_arrow_c4185f52507383203d0d6335fd0332a8)
...
;;; Fact-ids: 
(assert (! 
;; def=FStar.ST.fst(75,66-75,73); use=StatefulLens.fst(122,86-124,54)
(forall ((@x0 Term) (@x1 Term))
 (! (HasType (Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088 @x0
@x1)
Tm_type)
...
;               FStar.Monotonic.Heap.addr_of c }).
;           c == a /\
;           FStar.Monotonic.Heap.sel h1 (FStar.Monotonic.Heap.sel h1 c) =
;           FStar.Monotonic.Heap.sel h (FStar.Monotonic.Heap.sel h c) ==>
;           p a h1) ==>
;       (forall (a: Prims.int) (h1: FStar.Monotonic.Heap.heap).
;           h == h1 /\
;           (StatefulLens.compose_hlens StatefulLens.hlens_ref StatefulLens.hlens_ref).get (h, c) == a ==>
...
;               (h1,
;               a) ==
;               (StatefulLens.compose_hlens StatefulLens.hlens_ref StatefulLens.hlens_ref).put a
;                 (h1,
;                 c) ==>
;               p a h1)))
...
;; def=FStar.Pervasives.fsti(484,2-484,63); use=FStar.Pervasives.fsti(503,15-503,26)
(and 
;; def=dummy(0,0-0,0); use=StatefulLens.fst(122,86-124,54)
(forall ((@x1 Term))
 (! (implies (HasType @x1
FStar.Monotonic.Heap.heap)

;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(121,30-121,37)
(or label_1

;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(122,86-124,54)
(Valid 
;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(122,86-124,54)
(Prims.hasEq Prims.nat)
)
...
;; def=Prims.fst(451,66-451,102); use=StatefulLens.fst(122,86-124,54)
(forall ((@x1 Term))
 (! (implies (and (HasType @x1
FStar.ST.st_pre)

;; def=FStar.ST.fst(75,20-75,23); use=StatefulLens.fst(122,86-124,54)
(= (Tm_abs_226ef2c50b4e410e169f061746e2661f @x0)
@x1)
)
...
;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(120,30-120,32)
(or label_2
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(122,86-124,54)
(forall ((@x3 Term) (@x4 Term))
 (! (implies (and (or label_3
(HasType @x3
(FStar.ST.ref (FStar.ST.ref Prims.int))))
...
;; def=StatefulLens.fst(122,35-122,41); use=StatefulLens.fst(122,86-124,54)
(or label_5
...
;; def=StatefulLens.fst(122,45-122,82); use=StatefulLens.fst(122,86-124,54)
(or label_6
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(122,86-124,54)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(122,86-124,54)
(ApplyTT (ApplyTT @x1
@x3)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(123,52-123,58)
(forall ((@x3 Term) (@x4 Term))
 (! (implies (and (HasType @x3
Prims.int)
(HasType @x4
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(123,52-123,58)
(= @x2
@x4)


;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(123,52-123,58)
(= (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
(FStar.ST.ref (FStar.ST.ref Prims.int)))
Prims.int)
(StatefulLens.compose_hlens (FStar.ST.ref (FStar.ST.ref Prims.int))
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(124,44-124,50)
...
;; def=StatefulLens.fst(64,37-64,66); use=StatefulLens.fst(124,44-124,50)
(= (FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
(FStar.ST.ref (FStar.ST.ref Prims.int))
@x6
@x5)
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(124,44-124,50)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(124,44-124,50)
(ApplyTT (ApplyTT @x1
@x5)
...
;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun StatefulLens.test2 (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun StatefulLens.test2@tok () Term)
...
(declare-fun StatefulLens.op_Bar_Dot_Dot (Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;sl: stlens l -> sm: stlens m -> stlens (compose_hlens l m)
(declare-fun Tm_arrow_c28512dabc02359241dca227a47192ea (Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;sl: stlens l -> sm: stlens m -> stlens (compose_hlens l m)
(declare-fun Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e () Term)
(declare-fun StatefulLens.op_Bar_Dot_Dot@tok () Term)


; </end encoding let op_Bar_Dot_Dot>


; encoding sigelt let op_Tilde_Dot


; <Start encoding let op_Tilde_Dot>

(declare-fun StatefulLens.op_Tilde_Dot (Term Term Term) Term)

(declare-fun StatefulLens.op_Tilde_Dot@tok () Term)

; </end encoding let op_Tilde_Dot>


; encoding sigelt let op_Bar_Dot_Dot_Dot


; <Start encoding let op_Bar_Dot_Dot_Dot>

(declare-fun StatefulLens.op_Bar_Dot_Dot_Dot (Term Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;sl: stlens l -> m: Lens.lens b c -> stlens (compose_hlens l (as_hlens m))
(declare-fun Tm_arrow_9cb22f4fe54540747ec8a9525bc77fc6 () Term)
(declare-fun StatefulLens.op_Bar_Dot_Dot_Dot@tok () Term)

; </end encoding let op_Bar_Dot_Dot_Dot>
...
;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun StatefulLens.op_String_Access (Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun StatefulLens.op_String_Access@tok () Term)
...
;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun StatefulLens.op_String_Assignment (Term Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun StatefulLens.op_String_Assignment@tok () Term)
...
(declare-fun StatefulLens.v (Term) Term)

(declare-fun StatefulLens.v@tok () Term)

; </end encoding let v>


; encoding sigelt let deref


; <Start encoding let deref>

(declare-fun StatefulLens.deref (Term) Term)

(declare-fun StatefulLens.deref@tok () Term)
...
;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun StatefulLens.test3 (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun StatefulLens.test3@tok () Term)

; </end encoding let test3>

(push) ;; push{2

; Starting query at StatefulLens.fst(151,5-151,31)
...
;;; Fact-ids: 
(assert (! 
;; def=StatefulLens.fst(149,30-149,60); use=StatefulLens.fst(151,5-151,31)
(forall ((@x0 Term))
 (! (HasType (Tm_abs_226ef2c50b4e410e169f061746e2661f @x0)
Tm_arrow_c4185f52507383203d0d6335fd0332a8)
...
;;; Fact-ids: 
(assert (! 
;; def=FStar.ST.fst(75,66-75,73); use=StatefulLens.fst(151,5-151,31)
(forall ((@x0 Term) (@x1 Term))
 (! (HasType (Tm_refine_2a4aff828cfe8cabe6c5d05bc88b6088 @x0
@x1)
Tm_type)
...
;           FStar.Monotonic.Heap.addr_of c) ==
;       any_result ==>
;       (forall (h0: FStar.Monotonic.Heap.heap) (d: Prims.unit) (_: FStar.Monotonic.Heap.heap).
;           (*  - Subtyping check failed
;   - Expected type Prims.eqtype got type Type0
; *)
;           Prims.hasEq Prims.nat)) /\
...
;                 (h1,
;                 c) ==>
;               (forall (any_result: Prims.unit). p any_result h1))))
...
;; def=FStar.Pervasives.fsti(484,2-484,63); use=FStar.Pervasives.fsti(503,15-503,26)
(and 
;; def=dummy(0,0-0,0); use=StatefulLens.fst(151,5-151,31)
(forall ((@x1 Term))
 (! (implies (HasType @x1
FStar.Monotonic.Heap.heap)

;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(149,30-149,37)
(or label_1

;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(151,5-151,31)
(Valid 
;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(151,5-151,31)
(Prims.hasEq Prims.nat)
)
...
;; def=Prims.fst(451,66-451,102); use=StatefulLens.fst(151,5-151,31)
(forall ((@x1 Term))
 (! (implies (and (HasType @x1
FStar.ST.st_pre)

;; def=FStar.ST.fst(75,20-75,23); use=StatefulLens.fst(151,5-151,31)
(= (Tm_abs_226ef2c50b4e410e169f061746e2661f @x0)
@x1)
)
...
;; def=Prims.fst(73,23-73,30); use=StatefulLens.fst(148,30-148,32)
(or label_2
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(151,5-151,31)
(forall ((@x3 Term) (@x4 Term))
 (! (implies (and (or label_3
(HasType @x3
Prims.unit))
...
;; def=StatefulLens.fst(150,35-150,72); use=StatefulLens.fst(151,5-151,31)
(or label_5
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(151,5-151,31)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(151,5-151,31)
(ApplyTT (ApplyTT @x1
@x3)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(151,21-151,23)
(forall ((@x3 Term) (@x4 Term))
 (! (implies (and (HasType @x3
Prims.int)
(HasType @x4
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(151,21-151,23)
(= @x2
@x4)


;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(151,21-151,23)
(= (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
(FStar.ST.ref (FStar.ST.ref Prims.int)))
Prims.int)
(StatefulLens.compose_hlens (FStar.ST.ref (FStar.ST.ref Prims.int))
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(151,6-151,8)
(forall ((@x5 Term) (@x6 Term))
 (! (implies (and (HasType @x5
(FStar.ST.ref (FStar.ST.ref Prims.int)))
(HasType @x6
...
;; def=StatefulLens.fst(64,37-64,66); use=StatefulLens.fst(151,6-151,8)
(= (FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
(FStar.ST.ref (FStar.ST.ref Prims.int))
@x6
@x5)
...
;; def=Prims.fst(451,66-451,102); use=StatefulLens.fst(151,6-151,8)
(forall ((@x7 Term))
 (! (implies (HasType @x7
Prims.unit)

;; def=FStar.ST.fst(33,87-33,92); use=StatefulLens.fst(151,6-151,8)
(Valid 
;; def=FStar.ST.fst(33,87-33,92); use=StatefulLens.fst(151,6-151,8)
(ApplyTT (ApplyTT @x1
@x7)
...
;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun StatefulLens.test4 (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun StatefulLens.test4@tok () Term)
...
;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun StatefulLens.test5 (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun StatefulLens.test5@tok () Term)

; </end encoding let test5>


; encoding sigelt type StatefulLens.point


; <Start encoding type StatefulLens.point>

;;;;;;;;;;;;;;;;Constructor
(declare-fun StatefulLens.point () Term)
;;;;;;;;;;;;;;;;Constructor
(declare-fun StatefulLens.Mkpoint (Term Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun StatefulLens.Mkpoint_x (Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun StatefulLens.Mkpoint_y (Term) Term)
;;;;;;;;;;;;;;;;data constructor proxy: StatefulLens.Mkpoint
(declare-fun StatefulLens.Mkpoint@tok () Term)
;;;;;;;;;;;;;;;;x: FStar.ST.ref Prims.int -> y: FStar.ST.ref Prims.int -> point
(declare-fun Tm_arrow_32bd23c9c209c6899790fd6fb86c2fd1 () Term)

; <start constructor StatefulLens.point>

;;;;;;;;;;;;;;;;Discriminator definition
(define-fun is-StatefulLens.point ((__@x0 Term)) Bool
 (and (= (Term_constr_id __@x0)
313)
(= __@x0
StatefulLens.point)))

; </end constructor StatefulLens.point>


; <start constructor StatefulLens.Mkpoint>

;;;;;;;;;;;;;;;;Discriminator definition
(define-fun is-StatefulLens.Mkpoint ((__@x0 Term)) Bool
 (and (= (Term_constr_id __@x0)
319)
(= __@x0
(StatefulLens.Mkpoint (StatefulLens.Mkpoint_x __@x0)
(StatefulLens.Mkpoint_y __@x0)))))

; </end constructor StatefulLens.Mkpoint>


; </end encoding type StatefulLens.point>


; encoding sigelt val StatefulLens.__proj__Mkpoint__item__x


; <Start encoding val StatefulLens.__proj__Mkpoint__item__x>

(declare-fun StatefulLens.__proj__Mkpoint__item__x (Term) Term)
;;;;;;;;;;;;;;;;projectee: point -> FStar.ST.ref Prims.int
(declare-fun Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678 () Term)
(declare-fun StatefulLens.__proj__Mkpoint__item__x@tok () Term)

; </end encoding val StatefulLens.__proj__Mkpoint__item__x>
...
; encoding sigelt val StatefulLens.__proj__Mkpoint__item__y


; <Start encoding val StatefulLens.__proj__Mkpoint__item__y>

(declare-fun StatefulLens.__proj__Mkpoint__item__y (Term) Term)

(declare-fun StatefulLens.__proj__Mkpoint__item__y@tok () Term)

; </end encoding val StatefulLens.__proj__Mkpoint__item__y>
...
; encoding sigelt type StatefulLens.circle


; <Start encoding type StatefulLens.circle>

;;;;;;;;;;;;;;;;Constructor
(declare-fun StatefulLens.circle () Term)
;;;;;;;;;;;;;;;;Constructor
(declare-fun StatefulLens.Mkcircle (Term Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun StatefulLens.Mkcircle_center (Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun StatefulLens.Mkcircle_radius (Term) Term)
;;;;;;;;;;;;;;;;data constructor proxy: StatefulLens.Mkcircle
(declare-fun StatefulLens.Mkcircle@tok () Term)
;;;;;;;;;;;;;;;;center: FStar.ST.ref point -> radius: FStar.ST.ref Prims.nat -> circle
(declare-fun Tm_arrow_0c981adf958a658c1e90d2349591792b () Term)

; <start constructor StatefulLens.circle>

;;;;;;;;;;;;;;;;Discriminator definition
(define-fun is-StatefulLens.circle ((__@x0 Term)) Bool
 (and (= (Term_constr_id __@x0)
327)
(= __@x0
StatefulLens.circle)))

; </end constructor StatefulLens.circle>


; <start constructor StatefulLens.Mkcircle>

;;;;;;;;;;;;;;;;Discriminator definition
(define-fun is-StatefulLens.Mkcircle ((__@x0 Term)) Bool
 (and (= (Term_constr_id __@x0)
333)
(= __@x0
(StatefulLens.Mkcircle (StatefulLens.Mkcircle_center __@x0)
(StatefulLens.Mkcircle_radius __@x0)))))

; </end constructor StatefulLens.Mkcircle>


; </end encoding type StatefulLens.circle>


; encoding sigelt val StatefulLens.__proj__Mkcircle__item__center


; <Start encoding val StatefulLens.__proj__Mkcircle__item__center>

(declare-fun StatefulLens.__proj__Mkcircle__item__center (Term) Term)
;;;;;;;;;;;;;;;;projectee: circle -> FStar.ST.ref point
(declare-fun Tm_arrow_19423d5e9698c2151fb9174130d2b021 () Term)
(declare-fun StatefulLens.__proj__Mkcircle__item__center@tok () Term)

; </end encoding val StatefulLens.__proj__Mkcircle__item__center>
...
; encoding sigelt val StatefulLens.__proj__Mkcircle__item__radius


; <Start encoding val StatefulLens.__proj__Mkcircle__item__radius>

(declare-fun StatefulLens.__proj__Mkcircle__item__radius (Term) Term)
;;;;;;;;;;;;;;;;projectee: circle -> FStar.ST.ref Prims.nat
(declare-fun Tm_arrow_903357c02d0222feec42b104a1e146d1 () Term)
(declare-fun StatefulLens.__proj__Mkcircle__item__radius@tok () Term)

; </end encoding val StatefulLens.__proj__Mkcircle__item__radius>
...
(declare-fun StatefulLens.center (Dummy_sort) Term)

(declare-fun Tm_abs_75f91c1b4f08d624c8341c967bb881e0 () Term)
;;;;;;;;;;;;;;;;p: FStar.ST.ref point -> c: circle -> circle
(declare-fun Tm_arrow_255f5a3fee68155d196533d257f0db91 () Term)
(declare-fun Tm_abs_1256fa2ccc4c41723429bd87797d151c () Term)
...
(declare-fun StatefulLens.x (Dummy_sort) Term)

(declare-fun Tm_abs_6fa1761532061cff8000658ed76d6d8c () Term)
;;;;;;;;;;;;;;;;x: FStar.ST.ref Prims.int -> p: point -> point
(declare-fun Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c () Term)
(declare-fun Tm_abs_e775c6111ea25fcdc9dbe08c104fef8c () Term)

; </end encoding let x>


; encoding sigelt let op_Bar_Hat_Dot


; <Start encoding let op_Bar_Hat_Dot>

(declare-fun StatefulLens.op_Bar_Hat_Dot (Term Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;m: Lens.lens a b -> sl: stlens l -> stlens (compose_hlens (as_hlens m) l)
(declare-fun Tm_arrow_b5c62d707bb736f46b14cca49bda81fd () Term)
(declare-fun StatefulLens.op_Bar_Hat_Dot@tok () Term)


; </end encoding let op_Bar_Hat_Dot>


; encoding sigelt let op_Bar_Dot_Hat


; <Start encoding let op_Bar_Dot_Hat>

(declare-fun StatefulLens.op_Bar_Dot_Hat (Term Term Term Term Term Term) Term)

(declare-fun StatefulLens.op_Bar_Dot_Hat@tok () Term)


; </end encoding let op_Bar_Dot_Hat>
...
(declare-fun StatefulLens.op_Bar_Dot (Term Term Term Term Term) Term)

(declare-fun StatefulLens.op_Bar_Dot@tok () Term)

; </end encoding let op_Bar_Dot>


; encoding sigelt let op_Array_Access


; <Start encoding let op_Array_Access>

(declare-fun StatefulLens.op_Array_Access (Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;x: a -> $hs: FStar.Monotonic.Heap.heap & stlens l -> Prims.GTot b
(declare-fun Tm_ghost_arrow_afe2eca863fe48687ca6cc1af8db5e10 () Term)
(declare-fun StatefulLens.op_Array_Access@tok () Term)


; </end encoding let op_Array_Access>


; encoding sigelt let op_Array_Assignment


; <Start encoding let op_Array_Assignment>

(declare-fun StatefulLens.op_Array_Assignment (Term Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;x: a -> $hs: FStar.Monotonic.Heap.heap & stlens l -> y: b   -> Prims.GTot (FStar.Monotonic.Heap.heap & a)
(declare-fun Tm_ghost_arrow_58c7e76177dcc1ff25b27ccd62df63a0 () Term)
(declare-fun StatefulLens.op_Array_Assignment@tok () Term)


; </end encoding let op_Array_Assignment>

;;;;;;;;;;;;;;;;typing_Tm_abs_e775c6111ea25fcdc9dbe08c104fef8c
;;; Fact-ids: Name StatefulLens.x; Namespace StatefulLens
(assert (! (HasType Tm_abs_e775c6111ea25fcdc9dbe08c104fef8c
Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c)
:named typing_Tm_abs_e775c6111ea25fcdc9dbe08c104fef8c))
;;;;;;;;;;;;;;;;typing_Tm_abs_75f91c1b4f08d624c8341c967bb881e0
;;; Fact-ids: Name StatefulLens.center; Namespace StatefulLens
(assert (! (HasType Tm_abs_75f91c1b4f08d624c8341c967bb881e0
Tm_arrow_19423d5e9698c2151fb9174130d2b021)
:named typing_Tm_abs_75f91c1b4f08d624c8341c967bb881e0))
;;;;;;;;;;;;;;;;typing_Tm_abs_6fa1761532061cff8000658ed76d6d8c
;;; Fact-ids: Name StatefulLens.x; Namespace StatefulLens
(assert (! (HasType Tm_abs_6fa1761532061cff8000658ed76d6d8c
Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678)
:named typing_Tm_abs_6fa1761532061cff8000658ed76d6d8c))
;;;;;;;;;;;;;;;;typing_Tm_abs_1256fa2ccc4c41723429bd87797d151c
;;; Fact-ids: Name StatefulLens.center; Namespace StatefulLens
(assert (! (HasType Tm_abs_1256fa2ccc4c41723429bd87797d151c
Tm_arrow_255f5a3fee68155d196533d257f0db91)
:named typing_Tm_abs_1256fa2ccc4c41723429bd87797d151c))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.x; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(177,4-177,5); use=StatefulLens.fst(177,4-177,5)
(forall ((@u0 Dummy_sort))
 (! (HasType (StatefulLens.x @u0)
(Lens.lens StatefulLens.point
(FStar.ST.ref Prims.int)))
 

:pattern ((StatefulLens.x @u0))
:qid typing_StatefulLens.x))

:named typing_StatefulLens.x))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.v; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(141,4-141,5); use=StatefulLens.fst(141,4-141,5)
(forall ((@x0 Term))
 (! (implies (HasType @x0
Tm_type)
(HasType (StatefulLens.v @x0)
(StatefulLens.stlens (FStar.ST.ref @x0)
@x0
(StatefulLens.hlens_ref @x0))))
 

:pattern ((StatefulLens.v @x0))
:qid typing_StatefulLens.v))

:named typing_StatefulLens.v))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.stlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(91,4-91,14); use=StatefulLens.fst(91,4-91,14)
(forall ((@x0 Term))
 (! (implies (HasType @x0
Tm_type)
(HasType (StatefulLens.stlens_ref @x0)
(StatefulLens.stlens (FStar.ST.ref @x0)
@x0
(StatefulLens.hlens_ref @x0))))
 

:pattern ((StatefulLens.stlens_ref @x0))
:qid typing_StatefulLens.stlens_ref))

:named typing_StatefulLens.stlens_ref))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.op_Tilde_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(133,6-133,8); use=StatefulLens.fst(133,6-133,8)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(Lens.lens @x0
@x1)))
(HasType (StatefulLens.op_Tilde_Dot @x0
@x1
@x2)
(StatefulLens.stlens @x0
@x1
...
:pattern ((StatefulLens.op_Tilde_Dot @x0
@x1
@x2))
:qid typing_StatefulLens.op_Tilde_Dot))

:named typing_StatefulLens.op_Tilde_Dot))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.op_Bar_Hat_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(183,6-183,9); use=StatefulLens.fst(183,6-183,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
@x2
@x3)))
(HasType (StatefulLens.op_Bar_Hat_Dot @x0
@x1
@x2
@x3
@x4
...
:pattern ((StatefulLens.op_Bar_Hat_Dot @x0
@x1
@x2
@x3
@x4
@x5))
:qid typing_StatefulLens.op_Bar_Hat_Dot))

:named typing_StatefulLens.op_Bar_Hat_Dot))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Hat; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(185,6-185,9); use=StatefulLens.fst(185,6-185,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(Lens.lens @x1
@x2)))
(HasType (StatefulLens.op_Bar_Dot_Hat @x0
@x1
@x2
@x3
@x4
...
@x2
@x3
(StatefulLens.as_hlens @x1
@x2
@x5)))))
 

:pattern ((StatefulLens.op_Bar_Dot_Hat @x0
@x1
@x2
@x3
@x4
@x5))
:qid typing_StatefulLens.op_Bar_Dot_Hat))

:named typing_StatefulLens.op_Bar_Dot_Hat))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(131,6-131,9); use=StatefulLens.fst(131,6-131,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(StatefulLens.hlens @x1
@x2)))
(HasType (StatefulLens.op_Bar_Dot_Dot @x0
@x1
@x2
@x3
@x4)
(Tm_arrow_c28512dabc02359241dca227a47192ea @x4
@x2
@x3
@x1
@x0)))
...
:pattern ((StatefulLens.op_Bar_Dot_Dot @x0
@x1
@x2
@x3
@x4))
:qid typing_StatefulLens.op_Bar_Dot_Dot))

:named typing_StatefulLens.op_Bar_Dot_Dot))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.op_Array_Assignment; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(189,6-189,11); use=StatefulLens.fst(189,6-189,11)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(HasType @x5
@x1))
(HasType (StatefulLens.op_Array_Assignment @x0
@x1
@x2
@x3
@x4
...
:pattern ((StatefulLens.op_Array_Assignment @x0
@x1
@x2
@x3
@x4
@x5))
:qid typing_StatefulLens.op_Array_Assignment))

:named typing_StatefulLens.op_Array_Assignment))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.op_Array_Access; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(188,6-188,9); use=StatefulLens.fst(188,6-188,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
@x1
@x2))))
(HasType (StatefulLens.op_Array_Access @x0
@x1
@x2
@x3
@x4)
...
:pattern ((StatefulLens.op_Array_Access @x0
@x1
@x2
@x3
@x4))
:qid typing_StatefulLens.op_Array_Access))

:named typing_StatefulLens.op_Array_Access))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.compose_stlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(69,4-69,18); use=StatefulLens.fst(69,4-69,18)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term) (@x6 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
@x2
@x4)))
(HasType (StatefulLens.compose_stlens @x0
@x1
@x2
@x3
@x4
...
:pattern ((StatefulLens.compose_stlens @x0
@x1
@x2
@x3
@x4
@x5
@x6))
:qid typing_StatefulLens.compose_stlens))

:named typing_StatefulLens.compose_stlens))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.center; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(173,4-173,10); use=StatefulLens.fst(173,4-173,10)
(forall ((@u0 Dummy_sort))
 (! (HasType (StatefulLens.center @u0)
(Lens.lens StatefulLens.circle
(FStar.ST.ref StatefulLens.point)))
 

:pattern ((StatefulLens.center @u0))
:qid typing_StatefulLens.center))

:named typing_StatefulLens.center))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.as_stlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(79,4-79,13); use=StatefulLens.fst(79,4-79,13)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(Lens.lens @x0
@x1)))
(HasType (StatefulLens.as_stlens @x0
@x1
@x2)
(StatefulLens.stlens @x0
@x1
...
:pattern ((StatefulLens.as_stlens @x0
@x1
@x2))
:qid typing_StatefulLens.as_stlens))

:named typing_StatefulLens.as_stlens))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.__proj__Mkpoint__item__y; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(164,2-164,3); use=StatefulLens.fst(164,2-164,3)
(forall ((@x0 Term))
 (! (implies (HasType @x0
StatefulLens.point)
(HasType (StatefulLens.__proj__Mkpoint__item__y @x0)
(FStar.ST.ref Prims.int)))
 

:pattern ((StatefulLens.__proj__Mkpoint__item__y @x0))
:qid typing_StatefulLens.__proj__Mkpoint__item__y))

:named typing_StatefulLens.__proj__Mkpoint__item__y))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.__proj__Mkpoint__item__x; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(163,2-163,3); use=StatefulLens.fst(163,2-163,3)
(forall ((@x0 Term))
 (! (implies (HasType @x0
StatefulLens.point)
(HasType (StatefulLens.__proj__Mkpoint__item__x @x0)
(FStar.ST.ref Prims.int)))
 

:pattern ((StatefulLens.__proj__Mkpoint__item__x @x0))
:qid typing_StatefulLens.__proj__Mkpoint__item__x))

:named typing_StatefulLens.__proj__Mkpoint__item__x))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.__proj__Mkcircle__item__radius; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(171,2-171,8); use=StatefulLens.fst(171,2-171,8)
(forall ((@x0 Term))
 (! (implies (HasType @x0
StatefulLens.circle)
(HasType (StatefulLens.__proj__Mkcircle__item__radius @x0)
(FStar.ST.ref Prims.nat)))
 

:pattern ((StatefulLens.__proj__Mkcircle__item__radius @x0))
:qid typing_StatefulLens.__proj__Mkcircle__item__radius))

:named typing_StatefulLens.__proj__Mkcircle__item__radius))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name StatefulLens.__proj__Mkcircle__item__center; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(170,2-170,8); use=StatefulLens.fst(170,2-170,8)
(forall ((@x0 Term))
 (! (implies (HasType @x0
StatefulLens.circle)
(HasType (StatefulLens.__proj__Mkcircle__item__center @x0)
(FStar.ST.ref StatefulLens.point)))
 

:pattern ((StatefulLens.__proj__Mkcircle__item__center @x0))
:qid typing_StatefulLens.__proj__Mkcircle__item__center))

:named typing_StatefulLens.__proj__Mkcircle__item__center))
;;;;;;;;;;;;;;;;free var typing
...
;; def=StatefulLens.fst(131,6-131,9); use=StatefulLens.fst(131,6-131,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (= (ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.op_Bar_Dot_Dot@tok
@x0)
@x1)
@x2)
@x3)
@x4)
(StatefulLens.op_Bar_Dot_Dot @x0
@x1
@x2
@x3
@x4))
...
:pattern ((ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.op_Bar_Dot_Dot@tok
@x0)
@x1)
@x2)
@x3)
@x4))
:qid token_correspondence_StatefulLens.op_Bar_Dot_Dot))

:named token_correspondence_StatefulLens.op_Bar_Dot_Dot))
;;;;;;;;;;;;;;;;Name-token correspondence
;;; Fact-ids: Name StatefulLens.compose_stlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(69,4-69,18); use=StatefulLens.fst(69,4-69,18)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term) (@x6 Term))
 (! (= (ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.compose_stlens@tok
@x0)
@x1)
@x2)
@x3)
...
@x5)
@x6)
(StatefulLens.compose_stlens @x0
@x1
@x2
@x3
@x4
...
:pattern ((ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.compose_stlens@tok
@x0)
@x1)
@x2)
@x3)
...
@x5)
@x6))
:qid token_correspondence_StatefulLens.compose_stlens))

:named token_correspondence_StatefulLens.compose_stlens))
;;;;;;;;;;;;;;;;subterm ordering
;;; Fact-ids: Name StatefulLens.point; Namespace StatefulLens; Name StatefulLens.Mkpoint; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-162,10); use=StatefulLens.fst(162,5-162,10)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term))
 (! (implies (HasTypeFuel (SFuel @u0)
(StatefulLens.Mkpoint @x1
@x2)
StatefulLens.point)
(and (Valid (Prims.precedes Prims.lex_t
Prims.lex_t
...
@x2)
StatefulLens.point))
:qid subterm_ordering_StatefulLens.Mkpoint))

:named subterm_ordering_StatefulLens.Mkpoint))
;;;;;;;;;;;;;;;;subterm ordering
;;; Fact-ids: Name StatefulLens.circle; Namespace StatefulLens; Name StatefulLens.Mkcircle; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-169,11); use=StatefulLens.fst(169,5-169,11)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term))
 (! (implies (HasTypeFuel (SFuel @u0)
(StatefulLens.Mkcircle @x1
@x2)
StatefulLens.circle)
(and (Valid (Prims.precedes Prims.lex_t
Prims.lex_t
...
@x2)
StatefulLens.circle))
:qid subterm_ordering_StatefulLens.Mkcircle))

:named subterm_ordering_StatefulLens.Mkcircle))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name StatefulLens.point; Namespace StatefulLens; Name StatefulLens.Mkpoint; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-162,10); use=StatefulLens.fst(162,5-162,10)
(forall ((@x0 Term) (@x1 Term))
 (! (= (StatefulLens.Mkpoint_y (StatefulLens.Mkpoint @x0
@x1))
@x1)
 

:pattern ((StatefulLens.Mkpoint @x0
@x1))
:qid projection_inverse_StatefulLens.Mkpoint_y))

:named projection_inverse_StatefulLens.Mkpoint_y))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name StatefulLens.point; Namespace StatefulLens; Name StatefulLens.Mkpoint; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-162,10); use=StatefulLens.fst(162,5-162,10)
(forall ((@x0 Term) (@x1 Term))
 (! (= (StatefulLens.Mkpoint_x (StatefulLens.Mkpoint @x0
@x1))
@x0)
...
:pattern ((StatefulLens.Mkpoint @x0
@x1))
:qid projection_inverse_StatefulLens.Mkpoint_x))

:named projection_inverse_StatefulLens.Mkpoint_x))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name StatefulLens.circle; Namespace StatefulLens; Name StatefulLens.Mkcircle; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-169,11); use=StatefulLens.fst(169,5-169,11)
(forall ((@x0 Term) (@x1 Term))
 (! (= (StatefulLens.Mkcircle_radius (StatefulLens.Mkcircle @x0
@x1))
@x1)
 

:pattern ((StatefulLens.Mkcircle @x0
@x1))
:qid projection_inverse_StatefulLens.Mkcircle_radius))

:named projection_inverse_StatefulLens.Mkcircle_radius))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name StatefulLens.circle; Namespace StatefulLens; Name StatefulLens.Mkcircle; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-169,11); use=StatefulLens.fst(169,5-169,11)
(forall ((@x0 Term) (@x1 Term))
 (! (= (StatefulLens.Mkcircle_center (StatefulLens.Mkcircle @x0
@x1))
@x0)
...
:pattern ((StatefulLens.Mkcircle @x0
@x1))
:qid projection_inverse_StatefulLens.Mkcircle_center))

:named projection_inverse_StatefulLens.Mkcircle_center))
;;;;;;;;;;;;;;;;Projector equation
;;; Fact-ids: Name StatefulLens.__proj__Mkpoint__item__y; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(164,2-164,3); use=StatefulLens.fst(164,2-164,3)
(forall ((@x0 Term))
 (! (= (StatefulLens.__proj__Mkpoint__item__y @x0)
(StatefulLens.Mkpoint_y @x0))
 

:pattern ((StatefulLens.__proj__Mkpoint__item__y @x0))
:qid proj_equation_StatefulLens.Mkpoint_y))

:named proj_equation_StatefulLens.Mkpoint_y))
;;;;;;;;;;;;;;;;Projector equation
;;; Fact-ids: Name StatefulLens.__proj__Mkpoint__item__x; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(163,2-163,3); use=StatefulLens.fst(163,2-163,3)
(forall ((@x0 Term))
 (! (= (StatefulLens.__proj__Mkpoint__item__x @x0)
(StatefulLens.Mkpoint_x @x0))
 

:pattern ((StatefulLens.__proj__Mkpoint__item__x @x0))
:qid proj_equation_StatefulLens.Mkpoint_x))

:named proj_equation_StatefulLens.Mkpoint_x))
;;;;;;;;;;;;;;;;Projector equation
;;; Fact-ids: Name StatefulLens.__proj__Mkcircle__item__radius; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(171,2-171,8); use=StatefulLens.fst(171,2-171,8)
(forall ((@x0 Term))
 (! (= (StatefulLens.__proj__Mkcircle__item__radius @x0)
(StatefulLens.Mkcircle_radius @x0))
 

:pattern ((StatefulLens.__proj__Mkcircle__item__radius @x0))
:qid proj_equation_StatefulLens.Mkcircle_radius))

:named proj_equation_StatefulLens.Mkcircle_radius))
;;;;;;;;;;;;;;;;Projector equation
;;; Fact-ids: Name StatefulLens.__proj__Mkcircle__item__center; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(170,2-170,8); use=StatefulLens.fst(170,2-170,8)
(forall ((@x0 Term))
 (! (= (StatefulLens.__proj__Mkcircle__item__center @x0)
(StatefulLens.Mkcircle_center @x0))
 

:pattern ((StatefulLens.__proj__Mkcircle__item__center @x0))
:qid proj_equation_StatefulLens.Mkcircle_center))

:named proj_equation_StatefulLens.Mkcircle_center))
;;;;;;;;;;;;;;;;Projector equation
;;; Fact-ids: Name FStar.Pervasives.Native.__proj__Mktuple2__item___1; Namespace FStar.Pervasives.Native
(assert (! 
;; def=FStar.Pervasives.Native.fst(59,33-59,35); use=FStar.Pervasives.Native.fst(59,33-59,35)
...
;;;;;;;;;;;;;;;;kinding_Tm_arrow_c875932dc001902997da3c84a1d6c14f
;;; Fact-ids: Name StatefulLens.compose_stlens; Namespace StatefulLens
(assert (! (HasType Tm_arrow_c875932dc001902997da3c84a1d6c14f
Tm_type)
:named kinding_Tm_arrow_c875932dc001902997da3c84a1d6c14f))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_c28512dabc02359241dca227a47192ea
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(70,32-71,32); use=StatefulLens.fst(131,53-131,67)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (HasType (Tm_arrow_c28512dabc02359241dca227a47192ea @x0
@x1
@x2
@x3
@x4)
...
:pattern ((HasType (Tm_arrow_c28512dabc02359241dca227a47192ea @x0
@x1
@x2
@x3
@x4)
Tm_type))
:qid kinding_Tm_arrow_c28512dabc02359241dca227a47192ea))

:named kinding_Tm_arrow_c28512dabc02359241dca227a47192ea))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot; Namespace StatefulLens
(assert (! (HasType Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e
Tm_type)
:named kinding_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c
;;; Fact-ids: Name StatefulLens.x; Namespace StatefulLens
(assert (! (HasType Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c
Tm_type)
:named kinding_Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678
;;; Fact-ids: Name StatefulLens.__proj__Mkpoint__item__x; Namespace StatefulLens
(assert (! (HasType Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678
Tm_type)
:named kinding_Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_255f5a3fee68155d196533d257f0db91
;;; Fact-ids: Name StatefulLens.center; Namespace StatefulLens
(assert (! (HasType Tm_arrow_255f5a3fee68155d196533d257f0db91
Tm_type)
:named kinding_Tm_arrow_255f5a3fee68155d196533d257f0db91))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_19423d5e9698c2151fb9174130d2b021
;;; Fact-ids: Name StatefulLens.__proj__Mkcircle__item__center; Namespace StatefulLens
(assert (! (HasType Tm_arrow_19423d5e9698c2151fb9174130d2b021
Tm_type)
:named kinding_Tm_arrow_19423d5e9698c2151fb9174130d2b021))
;;; Fact-ids: Name StatefulLens.point; Namespace StatefulLens; Name StatefulLens.Mkpoint; Namespace StatefulLens
(assert (! (HasType StatefulLens.point
Tm_type)
:named kinding_StatefulLens.point@tok))
;;; Fact-ids: Name StatefulLens.circle; Namespace StatefulLens; Name StatefulLens.Mkcircle; Namespace StatefulLens
(assert (! (HasType StatefulLens.circle
Tm_type)
:named kinding_StatefulLens.circle@tok))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_e775c6111ea25fcdc9dbe08c104fef8c
;;; Fact-ids: Name StatefulLens.x; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(179,26-179,38); use=StatefulLens.fst(179,26-179,38)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ApplyTT (ApplyTT Tm_abs_e775c6111ea25fcdc9dbe08c104fef8c
@x0)
@x1)
(StatefulLens.Mkpoint @x0
(StatefulLens.__proj__Mkpoint__item__y @x1)))
 

:pattern ((ApplyTT (ApplyTT Tm_abs_e775c6111ea25fcdc9dbe08c104fef8c
@x0)
@x1))
:qid interpretation_Tm_abs_e775c6111ea25fcdc9dbe08c104fef8c))

:named interpretation_Tm_abs_e775c6111ea25fcdc9dbe08c104fef8c))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_75f91c1b4f08d624c8341c967bb881e0
;;; Fact-ids: Name StatefulLens.center; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(174,23-174,31); use=StatefulLens.fst(174,23-174,31)
(forall ((@x0 Term))
 (! (= (ApplyTT Tm_abs_75f91c1b4f08d624c8341c967bb881e0
@x0)
(StatefulLens.__proj__Mkcircle__item__center @x0))
 

:pattern ((ApplyTT Tm_abs_75f91c1b4f08d624c8341c967bb881e0
@x0))
:qid interpretation_Tm_abs_75f91c1b4f08d624c8341c967bb881e0))

:named interpretation_Tm_abs_75f91c1b4f08d624c8341c967bb881e0))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_6fa1761532061cff8000658ed76d6d8c
;;; Fact-ids: Name StatefulLens.x; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(178,23-178,26); use=StatefulLens.fst(178,23-178,26)
(forall ((@x0 Term))
 (! (= (ApplyTT Tm_abs_6fa1761532061cff8000658ed76d6d8c
@x0)
(StatefulLens.__proj__Mkpoint__item__x @x0))
 

:pattern ((ApplyTT Tm_abs_6fa1761532061cff8000658ed76d6d8c
@x0))
:qid interpretation_Tm_abs_6fa1761532061cff8000658ed76d6d8c))

:named interpretation_Tm_abs_6fa1761532061cff8000658ed76d6d8c))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_1256fa2ccc4c41723429bd87797d151c
;;; Fact-ids: Name StatefulLens.center; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(175,26-175,43); use=StatefulLens.fst(175,26-175,43)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ApplyTT (ApplyTT Tm_abs_1256fa2ccc4c41723429bd87797d151c
@x0)
@x1)
(StatefulLens.Mkcircle @x0
(StatefulLens.__proj__Mkcircle__item__radius @x1)))
 

:pattern ((ApplyTT (ApplyTT Tm_abs_1256fa2ccc4c41723429bd87797d151c
@x0)
@x1))
:qid interpretation_Tm_abs_1256fa2ccc4c41723429bd87797d151c))

:named interpretation_Tm_abs_1256fa2ccc4c41723429bd87797d151c))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(131,6-131,9); use=StatefulLens.fst(131,6-131,9)
(forall ((@x0 Term))
 (! (and (NoHoist @x0
(HasType StatefulLens.op_Bar_Dot_Dot@tok
Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e))
...
@x4)
@x5)
(StatefulLens.op_Bar_Dot_Dot @x1
@x2
@x3
@x4
@x5))
...
:pattern ((StatefulLens.op_Bar_Dot_Dot @x1
@x2
@x3
@x4
@x5))
:qid function_token_typing_StatefulLens.op_Bar_Dot_Dot.1))
)
 

:pattern ((ApplyTT @x0
StatefulLens.op_Bar_Dot_Dot@tok))
:qid function_token_typing_StatefulLens.op_Bar_Dot_Dot))

:named function_token_typing_StatefulLens.op_Bar_Dot_Dot))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name StatefulLens.compose_stlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(69,4-69,18); use=StatefulLens.fst(69,4-69,18)
(forall ((@x0 Term))
 (! (and (NoHoist @x0
(HasType StatefulLens.compose_stlens@tok
Tm_arrow_c875932dc001902997da3c84a1d6c14f))
...
@x6)
@x7)
(StatefulLens.compose_stlens @x1
@x2
@x3
@x4
@x5
...
:pattern ((StatefulLens.compose_stlens @x1
@x2
@x3
@x4
@x5
@x6
@x7))
:qid function_token_typing_StatefulLens.compose_stlens.1))
)
 

:pattern ((ApplyTT @x0
StatefulLens.compose_stlens@tok))
:qid function_token_typing_StatefulLens.compose_stlens))

:named function_token_typing_StatefulLens.compose_stlens))
;;;;;;;;;;;;;;;;inversion axiom
;;; Fact-ids: Name StatefulLens.point; Namespace StatefulLens; Name StatefulLens.Mkpoint; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-162,10); use=StatefulLens.fst(162,5-162,10)
...
@x1
StatefulLens.point)
(is-StatefulLens.Mkpoint @x1))
 

:pattern ((HasTypeFuel @u0
@x1
StatefulLens.point))
:qid fuel_guarded_inversion_StatefulLens.point))

:named fuel_guarded_inversion_StatefulLens.point))
;;;;;;;;;;;;;;;;inversion axiom
;;; Fact-ids: Name StatefulLens.circle; Namespace StatefulLens; Name StatefulLens.Mkcircle; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-169,11); use=StatefulLens.fst(169,5-169,11)
...
@x1
StatefulLens.circle)
(is-StatefulLens.Mkcircle @x1))
 

:pattern ((HasTypeFuel @u0
@x1
StatefulLens.circle))
:qid fuel_guarded_inversion_StatefulLens.circle))

:named fuel_guarded_inversion_StatefulLens.circle))
;;;;;;;;;;;;;;;;Equation for StatefulLens.x
;;; Fact-ids: Name StatefulLens.x; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(177,4-177,5); use=StatefulLens.fst(177,4-177,5)
(forall ((@u0 Dummy_sort))
...
(Lens.Mklens StatefulLens.point
(FStar.ST.ref Prims.int)
Tm_abs_6fa1761532061cff8000658ed76d6d8c
Tm_abs_e775c6111ea25fcdc9dbe08c104fef8c))
...
:qid equation_StatefulLens.x))

:named equation_StatefulLens.x))
;;;;;;;;;;;;;;;;Equation for StatefulLens.v
;;; Fact-ids: Name StatefulLens.v; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(141,4-141,5); use=StatefulLens.fst(141,4-141,5)
(forall ((@x0 Term))
 (! (= (StatefulLens.v @x0)
(StatefulLens.stlens_ref @x0))
 

:pattern ((StatefulLens.v @x0))
:qid equation_StatefulLens.v))

:named equation_StatefulLens.v))
;;;;;;;;;;;;;;;;Equation for StatefulLens.stlens_ref
;;; Fact-ids: Name StatefulLens.stlens_ref; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(91,4-91,14); use=StatefulLens.fst(91,4-91,14)
(forall ((@x0 Term))
 (! (= (StatefulLens.stlens_ref @x0)
(StatefulLens.Mkstlens (FStar.ST.ref @x0)
@x0
(StatefulLens.hlens_ref @x0)
(Tm_abs_StatefulLens_288 @x0)
(Tm_abs_StatefulLens_289 @x0)))
 

:pattern ((StatefulLens.stlens_ref @x0))
:qid equation_StatefulLens.stlens_ref))

:named equation_StatefulLens.stlens_ref))
;;;;;;;;;;;;;;;;Equation for StatefulLens.op_Tilde_Dot
;;; Fact-ids: Name StatefulLens.op_Tilde_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(133,6-133,8); use=StatefulLens.fst(133,6-133,8)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (StatefulLens.op_Tilde_Dot @x0
@x1
@x2)
(StatefulLens.as_stlens @x0
@x1
@x2))
...
@x1
@x2))
:qid equation_StatefulLens.op_Tilde_Dot))

:named equation_StatefulLens.op_Tilde_Dot))
;;;;;;;;;;;;;;;;Equation for StatefulLens.op_Bar_Hat_Dot
;;; Fact-ids: Name StatefulLens.op_Bar_Hat_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(183,6-183,9); use=StatefulLens.fst(183,6-183,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term))
 (! (= (StatefulLens.op_Bar_Hat_Dot @x0
@x1
@x2
@x3
@x4
@x5)
(ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.op_Bar_Dot_Dot@tok
@x0)
@x1)
@x2)
(StatefulLens.as_hlens @x0
...
@x4
@x5))
:qid equation_StatefulLens.op_Bar_Hat_Dot))

:named equation_StatefulLens.op_Bar_Hat_Dot))
;;;;;;;;;;;;;;;;Equation for StatefulLens.op_Bar_Dot_Hat
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Hat; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(185,6-185,9); use=StatefulLens.fst(185,6-185,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term))
 (! (= (StatefulLens.op_Bar_Dot_Hat @x0
@x1
@x2
@x3
@x4
...
@x4
@x5))
:qid equation_StatefulLens.op_Bar_Dot_Hat))

:named equation_StatefulLens.op_Bar_Dot_Hat))
;;;;;;;;;;;;;;;;Equation for StatefulLens.op_Bar_Dot_Dot
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(131,6-131,9); use=StatefulLens.fst(131,6-131,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
...
@x3
@x4)
(ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.compose_stlens@tok
@x0)
@x1)
@x2)
@x3)
...
@x3
@x4))
:qid equation_StatefulLens.op_Bar_Dot_Dot))

:named equation_StatefulLens.op_Bar_Dot_Dot))
;;;;;;;;;;;;;;;;Equation for StatefulLens.op_Array_Assignment
;;; Fact-ids: Name StatefulLens.op_Array_Assignment; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(189,6-189,11); use=StatefulLens.fst(189,6-189,11)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term))
 (! (= (StatefulLens.op_Array_Assignment @x0
@x1
@x2
@x3
@x4
...
(FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
@x0
(FStar.Pervasives.Native.fst FStar.Monotonic.Heap.heap
(StatefulLens.stlens @x0
@x1
@x2)
@x4)
...
@x4
@x5))
:qid equation_StatefulLens.op_Array_Assignment))

:named equation_StatefulLens.op_Array_Assignment))
;;;;;;;;;;;;;;;;Equation for StatefulLens.op_Array_Access
;;; Fact-ids: Name StatefulLens.op_Array_Access; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(188,6-188,9); use=StatefulLens.fst(188,6-188,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (= (StatefulLens.op_Array_Access @x0
@x1
@x2
@x3
@x4)
...
@x3
@x4))
:qid equation_StatefulLens.op_Array_Access))

:named equation_StatefulLens.op_Array_Access))
;;;;;;;;;;;;;;;;Equation for StatefulLens.compose_stlens
;;; Fact-ids: Name StatefulLens.compose_stlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(69,4-69,18); use=StatefulLens.fst(69,4-69,18)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term) (@x6 Term))
...
@x5
@x6))
:qid equation_StatefulLens.compose_stlens))

:named equation_StatefulLens.compose_stlens))
;;;;;;;;;;;;;;;;Equation for StatefulLens.center
;;; Fact-ids: Name StatefulLens.center; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(173,4-173,10); use=StatefulLens.fst(173,4-173,10)
(forall ((@u0 Dummy_sort))
 (! (= 
;; def=StatefulLens.fst(173,4-173,10); use=StatefulLens.fst(173,4-173,10)
(StatefulLens.center @u0)

(Lens.Mklens StatefulLens.circle
(FStar.ST.ref StatefulLens.point)
Tm_abs_75f91c1b4f08d624c8341c967bb881e0
Tm_abs_1256fa2ccc4c41723429bd87797d151c))
...
:qid equation_StatefulLens.center))

:named equation_StatefulLens.center))
;;;;;;;;;;;;;;;;Equation for StatefulLens.as_stlens
;;; Fact-ids: Name StatefulLens.as_stlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(79,4-79,13); use=StatefulLens.fst(79,4-79,13)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
...
@x1
@x2)
(Tm_abs_StatefulLens_268 @x0
@x1
@x2)
(Tm_abs_StatefulLens_269 @x1
@x0
@x2)))
...
@x1
@x2))
:qid equation_StatefulLens.as_stlens))

:named equation_StatefulLens.as_stlens))
...
@x2)
StatefulLens.point))
:qid data_typing_intro_StatefulLens.Mkpoint@tok))

:named data_typing_intro_StatefulLens.Mkpoint@tok))
;;;;;;;;;;;;;;;;data constructor typing intro
;;; Fact-ids: Name StatefulLens.circle; Namespace StatefulLens; Name StatefulLens.Mkcircle; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-169,11); use=StatefulLens.fst(169,5-169,11)
...
@x2)
StatefulLens.circle))
:qid data_typing_intro_StatefulLens.Mkcircle@tok))

:named data_typing_intro_StatefulLens.Mkcircle@tok))
;;;;;;;;;;;;;;;;data constructor typing elim
;;; Fact-ids: Name StatefulLens.point; Namespace StatefulLens; Name StatefulLens.Mkpoint; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-162,10); use=StatefulLens.fst(162,5-162,10)
...
@x2)
StatefulLens.point))
:qid data_elim_StatefulLens.Mkpoint))

:named data_elim_StatefulLens.Mkpoint))
;;;;;;;;;;;;;;;;data constructor typing elim
;;; Fact-ids: Name StatefulLens.circle; Namespace StatefulLens; Name StatefulLens.Mkcircle; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-169,11); use=StatefulLens.fst(169,5-169,11)
...
@x2)
StatefulLens.circle))
:qid data_elim_StatefulLens.Mkcircle))

:named data_elim_StatefulLens.Mkcircle))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name StatefulLens.point; Namespace StatefulLens; Name StatefulLens.Mkpoint; Namespace StatefulLens
(assert (! (= 313
(Term_constr_id StatefulLens.point))
:named constructor_distinct_StatefulLens.point))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name StatefulLens.circle; Namespace StatefulLens; Name StatefulLens.Mkcircle; Namespace StatefulLens
(assert (! (= 327
(Term_constr_id StatefulLens.circle))
:named constructor_distinct_StatefulLens.circle))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name StatefulLens.point; Namespace StatefulLens; Name StatefulLens.Mkpoint; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-162,10); use=StatefulLens.fst(162,5-162,10)
(forall ((@x0 Term) (@x1 Term))
 (! (= 319
(Term_constr_id (StatefulLens.Mkpoint @x0
@x1)))
 

:pattern ((StatefulLens.Mkpoint @x0
@x1))
:qid constructor_distinct_StatefulLens.Mkpoint))

:named constructor_distinct_StatefulLens.Mkpoint))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name StatefulLens.circle; Namespace StatefulLens; Name StatefulLens.Mkcircle; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-169,11); use=StatefulLens.fst(169,5-169,11)
(forall ((@x0 Term) (@x1 Term))
 (! (= 333
(Term_constr_id (StatefulLens.Mkcircle @x0
@x1)))
 

:pattern ((StatefulLens.Mkcircle @x0
@x1))
:qid constructor_distinct_StatefulLens.Mkcircle))

:named constructor_distinct_StatefulLens.Mkcircle))
;;;;;;;;;;;;;;;;pretyping
;;; Fact-ids: Name StatefulLens.point; Namespace StatefulLens; Name StatefulLens.Mkpoint; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-162,10); use=StatefulLens.fst(162,5-162,10)
...
@x0
StatefulLens.point))
:qid StatefulLens_pretyping_d2f60ef0b37d90f8e818551e51ee3e6f))

:named StatefulLens_pretyping_d2f60ef0b37d90f8e818551e51ee3e6f))
;;;;;;;;;;;;;;;;pretyping
;;; Fact-ids: Name StatefulLens.circle; Namespace StatefulLens; Name StatefulLens.Mkcircle; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-169,11); use=StatefulLens.fst(169,5-169,11)
...
@x0
StatefulLens.circle))
:qid StatefulLens_pretyping_c2e0b3282eda2816deca149d7bed58e4))

:named StatefulLens_pretyping_c2e0b3282eda2816deca149d7bed58e4))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.compose_stlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(69,32-70,55); use=StatefulLens.fst(69,20-71,32)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_c875932dc001902997da3c84a1d6c14f)
...
@x1
Tm_arrow_c875932dc001902997da3c84a1d6c14f))
:qid StatefulLens_pre_typing_Tm_arrow_c875932dc001902997da3c84a1d6c14f))

:named StatefulLens_pre_typing_Tm_arrow_c875932dc001902997da3c84a1d6c14f))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(70,32-71,32); use=StatefulLens.fst(131,53-131,67)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term) (@x6 Term))
 (! (implies (HasTypeFuel @u0
@x1
(Tm_arrow_c28512dabc02359241dca227a47192ea @x2
@x3
@x4
@x5
@x6))
...
@x5
@x6)))
:qid StatefulLens_pre_typing_Tm_arrow_c28512dabc02359241dca227a47192ea))

:named StatefulLens_pre_typing_Tm_arrow_c28512dabc02359241dca227a47192ea))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(131,25-131,49); use=StatefulLens.fst(131,13-131,67)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e)
...
@x1
Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e))
:qid StatefulLens_pre_typing_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e))

:named StatefulLens_pre_typing_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.x; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-163,11); use=StatefulLens.fst(178,23-178,26)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c)
...
@x1
Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c))
:qid StatefulLens_pre_typing_Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c))

:named StatefulLens_pre_typing_Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.__proj__Mkpoint__item__x; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-163,11); use=StatefulLens.fst(162,5-163,11)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678)
...
@x1
Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678))
:qid StatefulLens_pre_typing_Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678))

:named StatefulLens_pre_typing_Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.center; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-170,19); use=StatefulLens.fst(174,23-174,31)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_255f5a3fee68155d196533d257f0db91)
...
@x1
Tm_arrow_255f5a3fee68155d196533d257f0db91))
:qid StatefulLens_pre_typing_Tm_arrow_255f5a3fee68155d196533d257f0db91))

:named StatefulLens_pre_typing_Tm_arrow_255f5a3fee68155d196533d257f0db91))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name StatefulLens.__proj__Mkcircle__item__center; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-170,19); use=StatefulLens.fst(169,5-170,19)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_19423d5e9698c2151fb9174130d2b021)
...
@x1
Tm_arrow_19423d5e9698c2151fb9174130d2b021))
:qid StatefulLens_pre_typing_Tm_arrow_19423d5e9698c2151fb9174130d2b021))

:named StatefulLens_pre_typing_Tm_arrow_19423d5e9698c2151fb9174130d2b021))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_c875932dc001902997da3c84a1d6c14f
;;; Fact-ids: Name StatefulLens.compose_stlens; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(69,32-70,55); use=StatefulLens.fst(69,20-71,32)
(forall ((@x0 Term))
...
@x6)
@x7))
:qid StatefulLens_interpretation_Tm_arrow_c875932dc001902997da3c84a1d6c14f.1))
...
:pattern ((ApplyTT @x0
@x1))
:qid StatefulLens_interpretation_Tm_arrow_c875932dc001902997da3c84a1d6c14f.2))
...
@x1)
@x2))
:qid StatefulLens_interpretation_Tm_arrow_c875932dc001902997da3c84a1d6c14f.3))
...
@x2)
@x3))
:qid StatefulLens_interpretation_Tm_arrow_c875932dc001902997da3c84a1d6c14f.4))
...
@x3)
@x4))
:qid StatefulLens_interpretation_Tm_arrow_c875932dc001902997da3c84a1d6c14f.5))
...
@x4)
@x5))
:qid StatefulLens_interpretation_Tm_arrow_c875932dc001902997da3c84a1d6c14f.6))
...
@x5)
@x6))
:qid StatefulLens_interpretation_Tm_arrow_c875932dc001902997da3c84a1d6c14f.7))
))
 

:pattern ((HasTypeZ @x0
Tm_arrow_c875932dc001902997da3c84a1d6c14f))
:qid StatefulLens_interpretation_Tm_arrow_c875932dc001902997da3c84a1d6c14f))

:named StatefulLens_interpretation_Tm_arrow_c875932dc001902997da3c84a1d6c14f))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_c28512dabc02359241dca227a47192ea
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(70,32-71,32); use=StatefulLens.fst(131,53-131,67)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term) (@x5 Term))
 (! (iff (HasTypeZ @x0
(Tm_arrow_c28512dabc02359241dca227a47192ea @x1
@x2
@x3
@x4
@x5))
...
(forall ((@x6 Term) (@x7 Term))
 (! (implies (and (HasType @x6
(StatefulLens.stlens @x5
@x4
@x3))
(HasType @x7
(StatefulLens.stlens @x4
@x2
@x1)))
(HasType (ApplyTT (ApplyTT @x0
@x6)
...
@x6)
@x7))
:qid StatefulLens_interpretation_Tm_arrow_c28512dabc02359241dca227a47192ea.1))
...
:pattern ((ApplyTT @x0
@x6))
:qid StatefulLens_interpretation_Tm_arrow_c28512dabc02359241dca227a47192ea.2))
))
...
@x4
@x5)))
:qid StatefulLens_interpretation_Tm_arrow_c28512dabc02359241dca227a47192ea))

:named StatefulLens_interpretation_Tm_arrow_c28512dabc02359241dca227a47192ea))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(131,25-131,49); use=StatefulLens.fst(131,13-131,67)
(forall ((@x0 Term))
...
@x4)
@x5)
(Tm_arrow_c28512dabc02359241dca227a47192ea @x5
@x3
@x4
@x2
@x1)))
...
@x4)
@x5))
:qid StatefulLens_interpretation_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e.1))
...
:pattern ((ApplyTT @x0
@x1))
:qid StatefulLens_interpretation_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e.2))
...
@x1)
@x2))
:qid StatefulLens_interpretation_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e.3))
...
@x2)
@x3))
:qid StatefulLens_interpretation_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e.4))
...
@x3)
@x4))
:qid StatefulLens_interpretation_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e.5))
))
 

:pattern ((HasTypeZ @x0
Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e))
:qid StatefulLens_interpretation_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e))

:named StatefulLens_interpretation_Tm_arrow_a62a24823b6a3b3a1a8571123b43d77e))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c
;;; Fact-ids: Name StatefulLens.x; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-163,11); use=StatefulLens.fst(178,23-178,26)
(forall ((@x0 Term))
...
@x1)
@x2))
:qid StatefulLens_interpretation_Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c.1))
...
:pattern ((ApplyTT @x0
@x1))
:qid StatefulLens_interpretation_Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c.2))
))
 

:pattern ((HasTypeZ @x0
Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c))
:qid StatefulLens_interpretation_Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c))

:named StatefulLens_interpretation_Tm_arrow_47dbc20bf4c1056ffdf2d1e8f409d93c))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678
;;; Fact-ids: Name StatefulLens.__proj__Mkpoint__item__x; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(162,5-163,11); use=StatefulLens.fst(162,5-163,11)
(forall ((@x0 Term))
...
:pattern ((ApplyTT @x0
@x1))
:qid StatefulLens_interpretation_Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678.1))
...
:pattern ((HasTypeZ @x0
Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678))
:qid StatefulLens_interpretation_Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678))

:named StatefulLens_interpretation_Tm_arrow_2ac9f46a6b7bea3477dbcc89d0759678))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_255f5a3fee68155d196533d257f0db91
;;; Fact-ids: Name StatefulLens.center; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-170,19); use=StatefulLens.fst(174,23-174,31)
(forall ((@x0 Term))
...
@x1)
@x2))
:qid StatefulLens_interpretation_Tm_arrow_255f5a3fee68155d196533d257f0db91.1))
...
:pattern ((ApplyTT @x0
@x1))
:qid StatefulLens_interpretation_Tm_arrow_255f5a3fee68155d196533d257f0db91.2))
))
 

:pattern ((HasTypeZ @x0
Tm_arrow_255f5a3fee68155d196533d257f0db91))
:qid StatefulLens_interpretation_Tm_arrow_255f5a3fee68155d196533d257f0db91))

:named StatefulLens_interpretation_Tm_arrow_255f5a3fee68155d196533d257f0db91))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_19423d5e9698c2151fb9174130d2b021
;;; Fact-ids: Name StatefulLens.__proj__Mkcircle__item__center; Namespace StatefulLens
(assert (! 
;; def=StatefulLens.fst(169,5-170,19); use=StatefulLens.fst(169,5-170,19)
(forall ((@x0 Term))
...
:pattern ((ApplyTT @x0
@x1))
:qid StatefulLens_interpretation_Tm_arrow_19423d5e9698c2151fb9174130d2b021.1))
...
:pattern ((HasTypeZ @x0
Tm_arrow_19423d5e9698c2151fb9174130d2b021))
:qid StatefulLens_interpretation_Tm_arrow_19423d5e9698c2151fb9174130d2b021))

:named StatefulLens_interpretation_Tm_arrow_19423d5e9698c2151fb9174130d2b021))
;;;;;;;;;;;;;;;;kick_partial_app
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot; Namespace StatefulLens
(assert (! (Valid (ApplyTT __uu__PartialApp
StatefulLens.compose_stlens@tok))
:named @kick_partial_app_e57e35cf266b8f125076951d04ca63df))
;;;;;;;;;;;;;;;;kick_partial_app
;;; Fact-ids: Name StatefulLens.op_Bar_Dot_Dot_Dot; Namespace StatefulLens
(assert (! (Valid (ApplyTT __uu__PartialApp
StatefulLens.op_Bar_Dot_Dot@tok))
:named @kick_partial_app_6a91dc20eb946173125577f38dacfa3b))
(push) ;; push{2

; Starting query at StatefulLens.fst(196,5-196,73)
...
; Encoding query formula : forall (delta: Prims.int) (c: StatefulLens.circle).
;   (*  - Could not prove post-condition
; *)
;   forall (p: FStar.Pervasives.st_post_h FStar.Monotonic.Heap.heap Prims.unit)
;     (h: FStar.Monotonic.Heap.heap).
...
;         (h1,
;         c) ==
;         (c.(h, StatefulLens.center |^. StatefulLens.v |.^ StatefulLens.x |.. StatefulLens.v) <-
;             c.(h, StatefulLens.center |^. StatefulLens.v |.^ StatefulLens.x |.. StatefulLens.v) +
;             delta) ==>
;         p a h1) ==>
;     (forall (a: Prims.int) (h1: FStar.Monotonic.Heap.heap).
;         h == h1 /\
;         (StatefulLens.compose_hlens (StatefulLens.compose_hlens (StatefulLens.compose_hlens (StatefulLens.as_hlens
;                         StatefulLens.center)
;                     StatefulLens.hlens_ref)
;                 (StatefulLens.as_hlens StatefulLens.x))
;             StatefulLens.hlens_ref)
;           .get (h,
;           c) ==
;         a ==>
;         (forall (a: StatefulLens.circle) (h1: FStar.Monotonic.Heap.heap).
;             (h1,
;             a) ==
;             (StatefulLens.compose_hlens (StatefulLens.compose_hlens (StatefulLens.compose_hlens (StatefulLens.as_hlens
;                             StatefulLens.center)
...
; Context: While encoding a query
; While typechecking the top-level declaration `let move_x`
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(196,5-196,73)
(forall ((@x4 Term) (@x5 Term))
 (! (implies (and (or label_1
(HasType @x4
Prims.unit))
...
;; def=StatefulLens.fst(195,13-195,56); use=StatefulLens.fst(196,5-196,73)
(or label_3
...
@x5
@x1)
(StatefulLens.op_Array_Assignment StatefulLens.circle
Prims.int
(StatefulLens.compose_hlens StatefulLens.circle
(FStar.ST.ref Prims.int)
Prims.int
(StatefulLens.compose_hlens StatefulLens.circle
StatefulLens.point
...
(FStar.ST.ref StatefulLens.point)
StatefulLens.point
(StatefulLens.as_hlens StatefulLens.circle
(FStar.ST.ref StatefulLens.point)
(StatefulLens.center Dummy_value))
(StatefulLens.hlens_ref StatefulLens.point))
(StatefulLens.as_hlens StatefulLens.point
(FStar.ST.ref Prims.int)
(StatefulLens.x Dummy_value)))
(StatefulLens.hlens_ref Prims.int))
@x1
(FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
(StatefulLens.stlens StatefulLens.circle
Prims.int
(StatefulLens.compose_hlens StatefulLens.circle
(FStar.ST.ref Prims.int)
Prims.int
...
(StatefulLens.x Dummy_value))))
(StatefulLens.hlens_ref Prims.int))
(StatefulLens.op_Bar_Dot_Hat StatefulLens.circle
StatefulLens.point
(FStar.ST.ref Prims.int)
(StatefulLens.compose_hlens StatefulLens.circle
(FStar.ST.ref StatefulLens.point)
...
(StatefulLens.center Dummy_value))
(StatefulLens.hlens_ref StatefulLens.point))
(StatefulLens.op_Bar_Hat_Dot StatefulLens.circle
(FStar.ST.ref StatefulLens.point)
StatefulLens.point
(StatefulLens.hlens_ref StatefulLens.point)
(StatefulLens.center Dummy_value)
(StatefulLens.v StatefulLens.point))
(StatefulLens.x Dummy_value)))
(StatefulLens.v Prims.int)))
(Prims.op_Addition (StatefulLens.op_Array_Access StatefulLens.circle
Prims.int
(StatefulLens.compose_hlens StatefulLens.circle
(FStar.ST.ref Prims.int)
Prims.int
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(196,5-196,73)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(196,5-196,73)
(ApplyTT (ApplyTT @x2
@x4)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(196,38-196,40)
(forall ((@x4 Term) (@x5 Term))
 (! (implies (and (HasType @x4
Prims.int)
(HasType @x5
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(196,38-196,40)
(= @x3
@x5)


;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(196,38-196,40)
(= (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
StatefulLens.circle)
Prims.int)
(StatefulLens.compose_hlens StatefulLens.circle
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(196,6-196,8)
(forall ((@x6 Term) (@x7 Term))
 (! (implies (and (HasType @x6
StatefulLens.circle)
(HasType @x7
...
;; def=StatefulLens.fst(64,37-64,66); use=StatefulLens.fst(196,6-196,8)
(= (FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
StatefulLens.circle
@x7
@x6)
...
;; def=Prims.fst(451,66-451,102); use=StatefulLens.fst(196,6-196,8)
(forall ((@x8 Term))
 (! (implies (HasType @x8
Prims.unit)

;; def=FStar.ST.fst(33,87-33,92); use=StatefulLens.fst(196,6-196,8)
(Valid 
;; def=FStar.ST.fst(33,87-33,92); use=StatefulLens.fst(196,6-196,8)
(ApplyTT (ApplyTT @x2
@x8)
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (StatefulLens.move_x, 1)
; STATUS: unsat
...
; encoding sigelt let move_x


; <Start encoding let move_x>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun StatefulLens.move_x (Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun StatefulLens.move_x@tok () Term)

; </end encoding let move_x>

(push) ;; push{2

; Starting query at StatefulLens.fst(208,5-208,75)
...
;     (forall (a: Prims.unit) (h1: FStar.Monotonic.Heap.heap).
;         h1 ==
;         FStar.Monotonic.Heap.upd (FStar.Monotonic.Heap.upd h
;               (FStar.Monotonic.Heap.sel h c.center).x
;               (FStar.Monotonic.Heap.sel h (FStar.Monotonic.Heap.sel h c.center).x + delta))
;           c.center
;           (StatefulLens.Mkpoint (FStar.Monotonic.Heap.sel h c.center).x
;               (FStar.Monotonic.Heap.sel (FStar.Monotonic.Heap.upd h
;                       (FStar.Monotonic.Heap.sel h c.center).x
;                       (FStar.Monotonic.Heap.sel h (FStar.Monotonic.Heap.sel h c.center).x + delta))
;                   c.center)
;                 .y) ==>
...
; Context: While encoding a query
; While typechecking the top-level declaration `let move_x2`
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(208,5-208,75)
(forall ((@x4 Term) (@x5 Term))
 (! (implies (and (or label_1
(HasType @x4
Prims.unit))
...
;; def=StatefulLens.fst(207,13-207,21); use=StatefulLens.fst(208,5-208,75)
(or label_3

;; def=StatefulLens.fst(207,13-207,21); use=StatefulLens.fst(208,5-208,75)
(= @x5
(FStar.Monotonic.Heap.upd StatefulLens.point
(FStar.Heap.trivial_preorder StatefulLens.point)
(FStar.Monotonic.Heap.upd Prims.int
(FStar.Heap.trivial_preorder Prims.int)
@x3
(StatefulLens.__proj__Mkpoint__item__x (FStar.Monotonic.Heap.sel StatefulLens.point
(FStar.Heap.trivial_preorder StatefulLens.point)
@x3
(StatefulLens.__proj__Mkcircle__item__center @x1)))
(Prims.op_Addition (FStar.Monotonic.Heap.sel Prims.int
(FStar.Heap.trivial_preorder Prims.int)
@x3
(StatefulLens.__proj__Mkpoint__item__x (FStar.Monotonic.Heap.sel StatefulLens.point
(FStar.Heap.trivial_preorder StatefulLens.point)
...
@x0))
(StatefulLens.__proj__Mkcircle__item__center @x1)
(StatefulLens.Mkpoint (StatefulLens.__proj__Mkpoint__item__x (FStar.Monotonic.Heap.sel StatefulLens.point
(FStar.Heap.trivial_preorder StatefulLens.point)
@x3
(StatefulLens.__proj__Mkcircle__item__center @x1)))
(StatefulLens.__proj__Mkpoint__item__y (FStar.Monotonic.Heap.sel StatefulLens.point
(FStar.Heap.trivial_preorder StatefulLens.point)
(FStar.Monotonic.Heap.upd Prims.int
(FStar.Heap.trivial_preorder Prims.int)
@x3
...
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(208,5-208,75)
(Valid 
;; def=FStar.ST.fst(76,80-76,86); use=StatefulLens.fst(208,5-208,75)
(ApplyTT (ApplyTT @x2
@x4)
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(208,39-208,41)
(forall ((@x4 Term) (@x5 Term))
 (! (implies (and (HasType @x4
Prims.int)
(HasType @x5
...
;; def=StatefulLens.fst(61,36-61,42); use=StatefulLens.fst(208,39-208,41)
(= @x3
@x5)


;; def=StatefulLens.fst(61,46-61,66); use=StatefulLens.fst(208,39-208,41)
(= (ApplyTT (ApplyTT (ApplyTT (ApplyTT StatefulLens.__proj__Mkhlens__item__get@tok
StatefulLens.circle)
Prims.int)
(StatefulLens.compose_hlens StatefulLens.circle
...
;; def=FStar.ST.fst(76,50-76,87); use=StatefulLens.fst(208,6-208,8)
(forall ((@x6 Term) (@x7 Term))
 (! (implies (and (HasType @x6
StatefulLens.circle)
(HasType @x7
...
;; def=StatefulLens.fst(64,37-64,66); use=StatefulLens.fst(208,6-208,8)
(= (FStar.Pervasives.Native.Mktuple2 FStar.Monotonic.Heap.heap
StatefulLens.circle
@x7
@x6)
...
;; def=Prims.fst(451,66-451,102); use=StatefulLens.fst(208,6-208,8)
(forall ((@x8 Term))
 (! (implies (HasType @x8
Prims.unit)

;; def=FStar.ST.fst(33,87-33,92); use=StatefulLens.fst(208,6-208,8)
(Valid 
;; def=FStar.ST.fst(33,87-33,92); use=StatefulLens.fst(208,6-208,8)
(ApplyTT (ApplyTT @x2
@x8)
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (StatefulLens.move_x2, 1)
; STATUS: unsat
