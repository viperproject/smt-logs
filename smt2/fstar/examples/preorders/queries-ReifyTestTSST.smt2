...
; Internals for ReifyTestTSST


; encoding sigelt val ReifyTestTSST.timestamp


; <Skipped val ReifyTestTSST.timestamp/>


; encoding sigelt let timestamp


; <Start encoding let timestamp>

(declare-fun ReifyTestTSST.timestamp () Term)

; </end encoding let timestamp>


; encoding sigelt val ReifyTestTSST.timestamped_state


; <Skipped val ReifyTestTSST.timestamped_state/>


; encoding sigelt let timestamped_state


; <Start encoding let timestamped_state>

(declare-fun ReifyTestTSST.timestamped_state (Term) Term)

(declare-fun ReifyTestTSST.timestamped_state@tok () Term)

; </end encoding let timestamped_state>


; encoding sigelt val ReifyTestTSST.get_timestamp


; <Skipped val ReifyTestTSST.get_timestamp/>


; encoding sigelt let get_timestamp


; <Start encoding let get_timestamp>

(declare-fun ReifyTestTSST.get_timestamp (Term Term) Term)
;;;;;;;;;;;;;;;;tss: timestamped_state state -> timestamp
(declare-fun Tm_arrow_ad5d011f2e4789bfd2dd4b87e8d35b4e () Term)
(declare-fun ReifyTestTSST.get_timestamp@tok () Term)

; </end encoding let get_timestamp>


; encoding sigelt val ReifyTestTSST.get_state


; <Skipped val ReifyTestTSST.get_state/>


; encoding sigelt let get_state


; <Start encoding let get_state>

(declare-fun ReifyTestTSST.get_state (Term Term) Term)
;;;;;;;;;;;;;;;;tss: timestamped_state state -> state
(declare-fun Tm_arrow_bbdcc092576a0a0f46c5c10a69c01bfd () Term)
(declare-fun ReifyTestTSST.get_state@tok () Term)

; </end encoding let get_state>


; encoding sigelt val ReifyTestTSST.older_than


; <Skipped val ReifyTestTSST.older_than/>


; encoding sigelt let older_than


; <Start encoding let older_than>

(declare-fun ReifyTestTSST.older_than (Dummy_sort) Term)

; </end encoding let older_than>


; encoding sigelt val ReifyTestTSST.older_than_transitive


; <Skipped val ReifyTestTSST.older_than_transitive/>
...
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.timestamp; Namespace ReifyTestTSST
(assert (! (HasType ReifyTestTSST.timestamp
Tm_type)
:named typing_ReifyTestTSST.timestamp))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.older_than; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(33,4-33,14); use=ReifyTestTSST.fst(33,4-33,14)
(forall ((@u0 Dummy_sort))
 (! (HasType (ReifyTestTSST.older_than @u0)
(FStar.Preorder.relation ReifyTestTSST.timestamp))
 

:pattern ((ReifyTestTSST.older_than @u0))
:qid typing_ReifyTestTSST.older_than))

:named typing_ReifyTestTSST.older_than))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Prims.unit; Namespace Prims
...
(assert (! (HasType ReifyTestTSST.timestamp
Tm_type)
:named function_token_typing_ReifyTestTSST.timestamp))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name Prims.unit; Namespace Prims
(assert (! (HasType Prims.unit
Prims.eqtype)
...
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.timestamp
;;; Fact-ids: Name ReifyTestTSST.timestamp; Namespace ReifyTestTSST
(assert (! (= ReifyTestTSST.timestamp
Prims.nat)
:named equation_ReifyTestTSST.timestamp))
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.older_than
;;; Fact-ids: Name ReifyTestTSST.older_than; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(33,4-33,14); use=ReifyTestTSST.fst(33,4-33,14)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x0)
@x1)
(Prims.b2t (Prims.op_LessThan @x0
@x1)))
 

:pattern ((ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x0)
@x1))
:qid equation_ReifyTestTSST.older_than))

:named equation_ReifyTestTSST.older_than))
...
; Starting query at ReifyTestTSST.fst(35,40-35,42)

(declare-fun label_2 () Bool)
(declare-fun label_1 () Bool)

; Encoding query formula : forall (ts0: ReifyTestTSST.timestamp) (ts1: ReifyTestTSST.timestamp) (ts2: ReifyTestTSST.timestamp).
;   (*  - Could not prove post-condition
; *)
;   forall (p: Prims.pure_post Prims.unit).
;     ReifyTestTSST.older_than ts0 ts1 /\ ReifyTestTSST.older_than ts1 ts2 /\
;     (forall (pure_result: Prims.unit). ReifyTestTSST.older_than ts0 ts2 ==> p pure_result) ==>
;     (forall (any_result: Prims.unit). p any_result)


; Context: While encoding a query
; While typechecking the top-level declaration `let older_than_transitive`
...
;; def=ReifyTestTSST.fsti(40,45-40,63); use=ReifyTestTSST.fst(35,40-35,42)
(Valid 
;; def=ReifyTestTSST.fsti(40,45-40,63); use=ReifyTestTSST.fst(35,40-35,42)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x0)
...
;; def=ReifyTestTSST.fsti(40,67-40,85); use=ReifyTestTSST.fst(35,40-35,42)
(Valid 
;; def=ReifyTestTSST.fsti(40,67-40,85); use=ReifyTestTSST.fst(35,40-35,42)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x1)
...
;; def=Prims.fst(441,36-441,97); use=ReifyTestTSST.fst(35,40-35,42)
(forall ((@x4 Term))
 (! (implies (and (or label_1
(HasType @x4
Prims.unit))

;; def=ReifyTestTSST.fsti(41,23-41,43); use=ReifyTestTSST.fst(35,40-35,42)
(or label_2
...
;; def=Prims.fst(441,83-441,96); use=ReifyTestTSST.fst(35,40-35,42)
(Valid 
;; def=Prims.fst(441,83-441,96); use=ReifyTestTSST.fst(35,40-35,42)
(ApplyTT @x3
@x4)
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (ReifyTestTSST.older_than_transitive, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @query, b2t_def, equation_ReifyTestTSST.older_than, primitive_Prims.op_LessThan, projection_inverse_BoxBool_proj_0
...
; encoding sigelt let older_than_transitive


; <Start encoding let older_than_transitive>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun ReifyTestTSST.older_than_transitive (Term Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun ReifyTestTSST.older_than_transitive@tok () Term)

; </end encoding let older_than_transitive>


; encoding sigelt val ReifyTestTSST.older_than_antisymmetric


; <Skipped val ReifyTestTSST.older_than_antisymmetric/>

;;;;;;;;;;;;;;;;Lemma: ReifyTestTSST.older_than_transitive
;;; Fact-ids: Name ReifyTestTSST.older_than_transitive; Namespace ReifyTestTSST
...
;; def=ReifyTestTSST.fsti(40,45-40,63); use=ReifyTestTSST.fst(35,4-35,25)
(Valid 
;; def=ReifyTestTSST.fsti(40,45-40,63); use=ReifyTestTSST.fst(35,4-35,25)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x0)
...
;; def=ReifyTestTSST.fsti(40,67-40,85); use=ReifyTestTSST.fst(35,4-35,25)
(Valid 
;; def=ReifyTestTSST.fsti(40,67-40,85); use=ReifyTestTSST.fst(35,4-35,25)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x1)
...
;; def=ReifyTestTSST.fsti(41,23-41,43); use=ReifyTestTSST.fst(35,4-35,25)
(Valid 
;; def=ReifyTestTSST.fsti(41,23-41,43); use=ReifyTestTSST.fst(35,4-35,25)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x0)
...
:pattern ((ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x0)
@x1) (ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x1)
@x2))
:qid lemma_ReifyTestTSST.older_than_transitive))
:named lemma_ReifyTestTSST.older_than_transitive))
(push) ;; push{2

; Starting query at ReifyTestTSST.fst(37,39-37,41)

(declare-fun label_2 () Bool)
(declare-fun label_1 () Bool)

; Encoding query formula : forall (ts0: ReifyTestTSST.timestamp) (ts1: ReifyTestTSST.timestamp).
;   (*  - Could not prove post-condition
; *)
;   forall (p: Prims.pure_post Prims.unit).
;     ~(ReifyTestTSST.older_than ts0 ts1) /\ ~(ReifyTestTSST.older_than ts1 ts0) /\
;     (forall (pure_result: Prims.unit). ts0 == ts1 ==> p pure_result) ==>
;     (forall (any_result: Prims.unit). p any_result)


; Context: While encoding a query
; While typechecking the top-level declaration `let older_than_antisymmetric`
...
;; def=ReifyTestTSST.fsti(46,41-46,62); use=ReifyTestTSST.fst(37,39-37,41)
(not 
;; def=ReifyTestTSST.fsti(46,42-46,62); use=ReifyTestTSST.fst(37,39-37,41)
(Valid 
;; def=ReifyTestTSST.fsti(46,42-46,62); use=ReifyTestTSST.fst(37,39-37,41)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x0)
...
;; def=ReifyTestTSST.fsti(46,66-46,87); use=ReifyTestTSST.fst(37,39-37,41)
(not 
;; def=ReifyTestTSST.fsti(46,67-46,87); use=ReifyTestTSST.fst(37,39-37,41)
(Valid 
;; def=ReifyTestTSST.fsti(46,67-46,87); use=ReifyTestTSST.fst(37,39-37,41)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x1)
...
;; def=Prims.fst(441,36-441,97); use=ReifyTestTSST.fst(37,39-37,41)
(forall ((@x3 Term))
 (! (implies (and (or label_1
(HasType @x3
Prims.unit))

;; def=ReifyTestTSST.fsti(47,40-47,52); use=ReifyTestTSST.fst(37,39-37,41)
(or label_2
...
;; def=Prims.fst(441,83-441,96); use=ReifyTestTSST.fst(37,39-37,41)
(Valid 
;; def=Prims.fst(441,83-441,96); use=ReifyTestTSST.fst(37,39-37,41)
(ApplyTT @x2
@x3)
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (ReifyTestTSST.older_than_antisymmetric, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @MaxIFuel_assumption, @query, b2t_def, equation_Prims.nat, equation_ReifyTestTSST.older_than, equation_ReifyTestTSST.timestamp, int_inversion, primitive_Prims.op_LessThan, projection_inverse_BoxBool_proj_0, refinement_interpretation_Tm_refine_542f9d4f129664613f2483a6c88bc7c2
...
; encoding sigelt let older_than_antisymmetric


; <Start encoding let older_than_antisymmetric>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun ReifyTestTSST.older_than_antisymmetric (Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun ReifyTestTSST.older_than_antisymmetric@tok () Term)

; </end encoding let older_than_antisymmetric>


; encoding sigelt let tsst_pre


; <Start encoding let tsst_pre>

(declare-fun ReifyTestTSST.tsst_pre (Term) Term)

(declare-fun ReifyTestTSST.tsst_pre@tok () Term)
;;;;;;;;;;;;;;;;_: timestamped_state state -> Type
(declare-fun Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6 (Term) Term)

; </end encoding let tsst_pre>


; encoding sigelt let tsst_post


; <Start encoding let tsst_post>

(declare-fun ReifyTestTSST.tsst_post (Term Term) Term)

(declare-fun ReifyTestTSST.tsst_post@tok () Term)
;;;;;;;;;;;;;;;;_: a -> _: timestamped_state state -> Type
(declare-fun Tm_arrow_8483b8fa8baab11fbc6bf59532304f92 (Term Term) Term)

; </end encoding let tsst_post>


; encoding sigelt let tsst_wp


; <Start encoding let tsst_wp>

(declare-fun ReifyTestTSST.tsst_wp (Term Term) Term)

(declare-fun ReifyTestTSST.tsst_wp@tok () Term)
;;;;;;;;;;;;;;;;_: tsst_post state a -> tsst_pre state
(declare-fun Tm_arrow_1aaab5ad432edfb9f06800245a1886a4 (Term Term) Term)

; </end encoding let tsst_wp>
...
(declare-fun ReifyTestTSST.state () Term)

; </end encoding let state>

;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.state; Namespace ReifyTestTSST
(assert (! (HasType ReifyTestTSST.state
Tm_type)
:named typing_ReifyTestTSST.state))
;;;;;;;;;;;;;;;;free var typing
...
(assert (! (HasType ReifyTestTSST.state
Tm_type)
:named function_token_typing_ReifyTestTSST.state))
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.state
;;; Fact-ids: Name ReifyTestTSST.state; Namespace ReifyTestTSST
(assert (! (= ReifyTestTSST.state
Prims.nat)
:named equation_ReifyTestTSST.state))
...
; Starting query at ReifyTestTSST.fsti(64,0-64,60)

(declare-fun label_1 () Bool)
;;;;;;;;;;;;;;;;s0: Prims.int -> s1: Prims.int -> Prims.logical
(declare-fun Tm_arrow_7418b65d64c02ff38a120101f758e12a () Term)
;;;;;;;;;;;;;;;;kinding_Tm_arrow_7418b65d64c02ff38a120101f758e12a
;;; Fact-ids: 
(assert (! (HasType Tm_arrow_7418b65d64c02ff38a120101f758e12a
Tm_type)
:named kinding_Tm_arrow_7418b65d64c02ff38a120101f758e12a))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: 
(assert (! 
;; def=Prims.fst(180,21-564,34); use=ReifyTestTSST.fsti(64,37-64,56)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_7418b65d64c02ff38a120101f758e12a)
(is-Tm_arrow (PreType @x1)))
...
@x1
Tm_arrow_7418b65d64c02ff38a120101f758e12a))
:qid ReifyTestTSST_pre_typing_Tm_arrow_7418b65d64c02ff38a120101f758e12a))

:named ReifyTestTSST_pre_typing_Tm_arrow_7418b65d64c02ff38a120101f758e12a))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_7418b65d64c02ff38a120101f758e12a
;;; Fact-ids: 
(assert (! 
;; def=Prims.fst(180,21-564,34); use=ReifyTestTSST.fsti(64,37-64,56)
(forall ((@x0 Term))
...
@x1)
@x2))
:qid ReifyTestTSST_interpretation_Tm_arrow_7418b65d64c02ff38a120101f758e12a.1))
...
:pattern ((ApplyTT @x0
@x1))
:qid ReifyTestTSST_interpretation_Tm_arrow_7418b65d64c02ff38a120101f758e12a.2))
))
 

:pattern ((HasTypeZ @x0
Tm_arrow_7418b65d64c02ff38a120101f758e12a))
:qid ReifyTestTSST_interpretation_Tm_arrow_7418b65d64c02ff38a120101f758e12a))

:named ReifyTestTSST_interpretation_Tm_arrow_7418b65d64c02ff38a120101f758e12a))
(declare-fun Tm_abs_577bb0504d99bec50af9e2beb88b9dcb () Term)
;;;;;;;;;;;;;;;;typing_Tm_abs_577bb0504d99bec50af9e2beb88b9dcb
;;; Fact-ids: 
(assert (! (HasType Tm_abs_577bb0504d99bec50af9e2beb88b9dcb
Tm_arrow_7418b65d64c02ff38a120101f758e12a)
:named typing_Tm_abs_577bb0504d99bec50af9e2beb88b9dcb))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_577bb0504d99bec50af9e2beb88b9dcb
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(64,46-64,60); use=ReifyTestTSST.fsti(64,46-64,60)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ApplyTT (ApplyTT Tm_abs_577bb0504d99bec50af9e2beb88b9dcb
@x0)
@x1)
...
:pattern ((ApplyTT (ApplyTT Tm_abs_577bb0504d99bec50af9e2beb88b9dcb
@x0)
@x1))
:qid interpretation_Tm_abs_577bb0504d99bec50af9e2beb88b9dcb))

:named interpretation_Tm_abs_577bb0504d99bec50af9e2beb88b9dcb))

; Encoding query formula : Prims.auto_squash (FStar.Preorder.preorder_rel (fun s0 s1 -> s0 <= s1))


; Context: While encoding a query
; While checking for top-level effects
; While typechecking the top-level declaration `let state_rel`
...
;;; Fact-ids: 
(assert (! (not 
;; def=FStar.Preorder.fst(33,40-33,56); use=ReifyTestTSST.fsti(64,46-64,60)
(or label_1

;; def=FStar.Preorder.fst(33,40-33,56); use=ReifyTestTSST.fsti(64,46-64,60)
(Valid 
;; def=FStar.Preorder.fst(33,40-33,56); use=ReifyTestTSST.fsti(64,46-64,60)
(FStar.Preorder.preorder_rel ReifyTestTSST.state
Tm_abs_577bb0504d99bec50af9e2beb88b9dcb)
)
)
)
:named @query))
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (ReifyTestTSST.state_rel, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @query, b2t_def, equation_FStar.Preorder.preorder_rel, equation_FStar.Preorder.reflexive, equation_FStar.Preorder.transitive, equation_ReifyTestTSST.state, interpretation_Tm_abs_577bb0504d99bec50af9e2beb88b9dcb, primitive_Prims.op_LessThanOrEqual, projection_inverse_BoxBool_proj_0
...
; encoding sigelt let state_rel


; <Start encoding let state_rel>

(declare-fun ReifyTestTSST.state_rel (Dummy_sort) Term)
;;;;;;;;;;;;;;;;s0: Prims.int -> s1: Prims.int -> Prims.logical
(declare-fun Tm_arrow_7418b65d64c02ff38a120101f758e12a () Term)
(declare-fun Tm_abs_577bb0504d99bec50af9e2beb88b9dcb () Term)

; </end encoding let state_rel>


; encoding sigelt new_effect { TSSTATE ... }


; <Skipped new_effect { TSSTATE ... }/>


; encoding sigelt let lift_div_tsstate


; <Start encoding let lift_div_tsstate>

(declare-fun ReifyTestTSST.lift_div_tsstate (Term Term Term Term Term Term) Term)
;;;;;;;;;;;;;;;;state: Type ->     rel: FStar.Preorder.preorder state ->     a: Type ->     wp: Prims.pure_wp a ->     p: tsst_post state a ->     s: timestamped_state state   -> Prims.pure_pre
(declare-fun Tm_arrow_705435317e51c8dd0fa710ef92b4fe53 () Term)
(declare-fun ReifyTestTSST.lift_div_tsstate@tok () Term)



; </end encoding let lift_div_tsstate>


; encoding sigelt sub_effect DIV ~> TSSTATE


; <Skipped sub_effect DIV ~> TSSTATE/>


; encoding sigelt effect TSST a pre post = ReifyTestTSST.TSSTATE a


; <Skipped effect TSST a pre post = ReifyTestTSST.TSSTATE a/>


; encoding sigelt let witnessed


; <Start encoding let witnessed>

(declare-fun Tm_refine_8c7a59d2aae063ddae7f7f455c88be60 () Term)
(declare-fun ReifyTestTSST.witnessed (Term Term) Term)

;;;;;;;;;;;;;;;;ts: timestamp -> p: FStar.Preorder.predicate state {FStar.Preorder.stable p state_rel} -> Type
(declare-fun Tm_arrow_2497436487eb9e025b479d2dbafa78b0 () Term)
(declare-fun ReifyTestTSST.witnessed@tok () Term)


; </end encoding let witnessed>


; encoding sigelt val ReifyTestTSST.get


; <Skipped val ReifyTestTSST.get/>
...
;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun ReifyTestTSST.get (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun ReifyTestTSST.get@tok () Term)

; </end encoding let get>


; encoding sigelt val ReifyTestTSST.put


; <Skipped val ReifyTestTSST.put/>
...
;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun ReifyTestTSST.put (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun ReifyTestTSST.put@tok () Term)

; </end encoding let put>


; encoding sigelt val ReifyTestTSST.witness


; <Skipped val ReifyTestTSST.witness/>


; encoding sigelt let witness


; <Start encoding let witness>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun ReifyTestTSST.witness (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun ReifyTestTSST.witness@tok () Term)

; </end encoding let witness>


; encoding sigelt val ReifyTestTSST.recall


; <Skipped val ReifyTestTSST.recall/>


; encoding sigelt let recall


; <Start encoding let recall>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun ReifyTestTSST.recall (Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun ReifyTestTSST.recall@tok () Term)

; </end encoding let recall>


; encoding sigelt val ReifyTestTSST.reify_


; <Skipped val ReifyTestTSST.reify_/>


; encoding sigelt let reify_


; <Start encoding let reify_>

;;;;;;;;;;;;;;;;_: timestamped_state state -> tsst_post state a
(declare-fun Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a (Term) Term)

(declare-fun Non_total_Tm_arrow_88f057a810db543d1c7c6b6178fead3c (Term Term Term) Term)
(declare-fun ReifyTestTSST.reify_ (Term Term Term Term Term) Term)



(declare-fun Tm_refine_d82b849f9f4e183841cbef932b6fc45f (Term Term Term) Term)
;;;;;;;;;;;;;;;;e: (_: Prims.unit -> TSST a) -> s0: timestamped_state state   -> Prims.Pure (a & timestamped_state state)
(declare-fun Tm_arrow_53e44af7f6e83320c9be36b1f475adcb () Term)
(declare-fun ReifyTestTSST.reify_@tok () Term)





; </end encoding let reify_>
...
;;;;;;;;;;;;;;;;typing_Tm_abs_577bb0504d99bec50af9e2beb88b9dcb
;;; Fact-ids: Name ReifyTestTSST.state_rel; Namespace ReifyTestTSST
(assert (! (HasType Tm_abs_577bb0504d99bec50af9e2beb88b9dcb
Tm_arrow_7418b65d64c02ff38a120101f758e12a)
:named typing_Tm_abs_577bb0504d99bec50af9e2beb88b9dcb))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.witnessed; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(97,4-97,13); use=ReifyTestTSST.fsti(97,4-97,13)
(forall ((@x0 Term) (@x1 Term))
 (! (implies (and (HasType @x0
ReifyTestTSST.timestamp)
(HasType @x1
Tm_refine_8c7a59d2aae063ddae7f7f455c88be60))
(HasType (ReifyTestTSST.witnessed @x0
@x1)
Tm_type))
 

:pattern ((ReifyTestTSST.witnessed @x0
@x1))
:qid typing_ReifyTestTSST.witnessed))

:named typing_ReifyTestTSST.witnessed))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.tsst_pre; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(53,4-53,12); use=ReifyTestTSST.fsti(53,4-53,12)
(forall ((@x0 Term))
 (! (implies (HasType @x0
Tm_type)
(HasType (ReifyTestTSST.tsst_pre @x0)
Tm_type))
 

:pattern ((ReifyTestTSST.tsst_pre @x0))
:qid typing_ReifyTestTSST.tsst_pre))

:named typing_ReifyTestTSST.tsst_pre))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.tsst_post; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(55,4-55,13); use=ReifyTestTSST.fsti(55,4-55,13)
(forall ((@x0 Term) (@x1 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
Tm_type))
(HasType (ReifyTestTSST.tsst_post @x0
@x1)
Tm_type))
 

:pattern ((ReifyTestTSST.tsst_post @x0
@x1))
:qid typing_ReifyTestTSST.tsst_post))

:named typing_ReifyTestTSST.tsst_post))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.timestamped_state; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(27,4-27,21); use=ReifyTestTSST.fst(27,4-27,21)
(forall ((@x0 Term))
 (! (implies (HasType @x0
Tm_type)
(HasType (ReifyTestTSST.timestamped_state @x0)
Tm_type))
 

:pattern ((ReifyTestTSST.timestamped_state @x0))
:qid typing_ReifyTestTSST.timestamped_state))

:named typing_ReifyTestTSST.timestamped_state))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.state_rel; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(64,4-64,13); use=ReifyTestTSST.fsti(64,4-64,13)
(forall ((@u0 Dummy_sort))
 (! (HasType (ReifyTestTSST.state_rel @u0)
(FStar.Preorder.preorder ReifyTestTSST.state))
 

:pattern ((ReifyTestTSST.state_rel @u0))
:qid typing_ReifyTestTSST.state_rel))

:named typing_ReifyTestTSST.state_rel))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(43,4-43,10); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (implies (and 
;; def=ReifyTestTSST.fsti(138,40-138,48); use=ReifyTestTSST.fst(43,4-43,10)
(Valid 
;; def=ReifyTestTSST.fsti(138,40-138,48); use=ReifyTestTSST.fst(43,4-43,10)
(ApplyTT @x1
@x4)
...
Tm_type)
(HasType @x1
(ReifyTestTSST.tsst_pre ReifyTestTSST.state))
(HasType @x2
(Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a @x0))
(HasType @x3
(Non_total_Tm_arrow_88f057a810db543d1c7c6b6178fead3c @x0
@x1
@x2))
(HasType @x4
(ReifyTestTSST.timestamped_state ReifyTestTSST.state)))
(HasType (ReifyTestTSST.reify_ @x0
@x1
@x2
@x3
@x4)
(Tm_refine_d82b849f9f4e183841cbef932b6fc45f @x4
@x0
@x2)))
 

:pattern ((ReifyTestTSST.reify_ @x0
@x1
@x2
@x3
@x4))
:qid typing_ReifyTestTSST.reify_))

:named typing_ReifyTestTSST.reify_))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.get_timestamp; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(29,4-29,17); use=ReifyTestTSST.fst(29,4-29,17)
(forall ((@x0 Term) (@x1 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
(ReifyTestTSST.timestamped_state @x0)))
(HasType (ReifyTestTSST.get_timestamp @x0
@x1)
ReifyTestTSST.timestamp))
 

:pattern ((ReifyTestTSST.get_timestamp @x0
@x1))
:qid typing_ReifyTestTSST.get_timestamp))

:named typing_ReifyTestTSST.get_timestamp))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name ReifyTestTSST.get_state; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(31,4-31,13); use=ReifyTestTSST.fst(31,4-31,13)
(forall ((@x0 Term) (@x1 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
(ReifyTestTSST.timestamped_state @x0)))
(HasType (ReifyTestTSST.get_state @x0
@x1)
@x0))
 

:pattern ((ReifyTestTSST.get_state @x0
@x1))
:qid typing_ReifyTestTSST.get_state))

:named typing_ReifyTestTSST.get_state))
;;;;;;;;;;;;;;;;free var typing
...
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name FStar.Preorder.stable; Namespace FStar.Preorder
(assert (! 
;; def=FStar.Preorder.fst(35,4-35,10); use=FStar.Preorder.fst(35,4-35,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
(FStar.Preorder.predicate @x0))
(HasType @x2
(Tm_refine_bd10f09297e0e7dc08314f7d9211801c @x0)))
(HasType (FStar.Preorder.stable @x0
@x1
@x2)
Prims.logical))
 

:pattern ((FStar.Preorder.stable @x0
@x1
@x2))
:qid typing_FStar.Preorder.stable))

:named typing_FStar.Preorder.stable))
;;;;;;;;;;;;;;;;free var typing
...
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name FStar.Preorder.predicate; Namespace FStar.Preorder
(assert (! 
;; def=FStar.Preorder.fst(22,5-22,14); use=FStar.Preorder.fst(22,5-22,14)
(forall ((@x0 Term))
 (! (implies (HasType @x0
Tm_type)
(HasType (FStar.Preorder.predicate @x0)
Tm_type))
 

:pattern ((FStar.Preorder.predicate @x0))
:qid typing_FStar.Preorder.predicate))

:named typing_FStar.Preorder.predicate))
;;;;;;;;;;;;;;;;free var typing
...
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name FStar.Monotonic.Witnessed.witnessed; Namespace FStar.Monotonic.Witnessed
(assert (! 
;; def=FStar.Monotonic.Witnessed.fsti(35,4-35,13); use=FStar.Monotonic.Witnessed.fsti(35,4-35,13)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(HasType @x2
(Tm_arrow_2eaa01e78f73e9bab5d0955fc1a662da @x0)))
(HasType (FStar.Monotonic.Witnessed.witnessed @x0
@x1
@x2)
Tm_type))
 

:pattern ((FStar.Monotonic.Witnessed.witnessed @x0
@x1
@x2))
:qid typing_FStar.Monotonic.Witnessed.witnessed))

:named typing_FStar.Monotonic.Witnessed.witnessed))
;;;;;;;;;;;;;;;;Name-token correspondence
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(43,4-43,10); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (= (ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT ReifyTestTSST.reify_@tok
@x0)
@x1)
@x2)
@x3)
@x4)
(ReifyTestTSST.reify_ @x0
@x1
@x2
@x3
@x4))
...
:pattern ((ApplyTT (ApplyTT (ApplyTT (ApplyTT (ApplyTT ReifyTestTSST.reify_@tok
@x0)
@x1)
@x2)
@x3)
@x4))
:qid token_correspondence_ReifyTestTSST.reify_))

:named token_correspondence_ReifyTestTSST.reify_))
;;;;;;;;;;;;;;;;name-token correspondence
...
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(138,10-138,39); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (HasType (Tm_refine_d82b849f9f4e183841cbef932b6fc45f @x0
@x1
@x2)
Tm_type)
 

:pattern ((HasType (Tm_refine_d82b849f9f4e183841cbef932b6fc45f @x0
@x1
@x2)
Tm_type))
:qid refinement_kinding_Tm_refine_d82b849f9f4e183841cbef932b6fc45f))

:named refinement_kinding_Tm_refine_d82b849f9f4e183841cbef932b6fc45f))
;;;;;;;;;;;;;;;;refinement kinding
;;; Fact-ids: Name FStar.Preorder.preorder; Namespace FStar.Preorder
(assert (! 
...
;;;;;;;;;;;;;;;;refinement kinding
;;; Fact-ids: Name ReifyTestTSST.witnessed; Namespace ReifyTestTSST
(assert (! (HasType Tm_refine_8c7a59d2aae063ddae7f7f455c88be60
Tm_type)
:named refinement_kinding_Tm_refine_8c7a59d2aae063ddae7f7f455c88be60))
;;;;;;;;;;;;;;;;refinement_interpretation
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(138,10-138,39); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
 (! (iff (HasTypeFuel @u0
@x1
(Tm_refine_d82b849f9f4e183841cbef932b6fc45f @x2
@x3
@x4))
(and (HasTypeFuel @u0
@x1
...
;; def=ReifyTestTSST.fsti(139,52-140,71); use=ReifyTestTSST.fst(43,4-43,10)
(or 
;; def=ReifyTestTSST.fsti(139,53-139,108); use=ReifyTestTSST.fst(43,4-43,10)
(Valid 
;; def=ReifyTestTSST.fsti(139,53-139,108); use=ReifyTestTSST.fst(43,4-43,10)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x2))
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(140,27-140,70); use=ReifyTestTSST.fst(43,4-43,10)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x2)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
(FStar.Pervasives.Native.snd @x3
...
;; def=ReifyTestTSST.fsti(141,24-141,70); use=ReifyTestTSST.fst(43,4-43,10)
(Valid 
;; def=ReifyTestTSST.fsti(141,24-141,70); use=ReifyTestTSST.fst(43,4-43,10)
(ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
@x2))
(ReifyTestTSST.get_state ReifyTestTSST.state
(FStar.Pervasives.Native.snd @x3
(ReifyTestTSST.timestamped_state ReifyTestTSST.state)
...
;; def=ReifyTestTSST.fsti(142,24-142,51); use=ReifyTestTSST.fst(43,4-43,10)
(Valid 
;; def=ReifyTestTSST.fsti(142,24-142,51); use=ReifyTestTSST.fst(43,4-43,10)
...
@x3
@x4)))
:qid refinement_interpretation_Tm_refine_d82b849f9f4e183841cbef932b6fc45f))

:named refinement_interpretation_Tm_refine_d82b849f9f4e183841cbef932b6fc45f))
;;;;;;;;;;;;;;;;refinement_interpretation
;;; Fact-ids: Name FStar.Preorder.preorder; Namespace FStar.Preorder
(assert (! 
;; def=FStar.Preorder.fst(33,25-33,57); use=FStar.Preorder.fst(33,25-33,57)
...
;;; Fact-ids: Name ReifyTestTSST.witnessed; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(97,29-97,68); use=ReifyTestTSST.fsti(97,29-97,68)
(forall ((@u0 Fuel) (@x1 Term))
 (! (iff (HasTypeFuel @u0
@x1
Tm_refine_8c7a59d2aae063ddae7f7f455c88be60)
(and (HasTypeFuel @u0
@x1
(FStar.Preorder.predicate ReifyTestTSST.state))

;; def=ReifyTestTSST.fsti(97,48-97,66); use=ReifyTestTSST.fsti(97,48-97,66)
(Valid 
;; def=ReifyTestTSST.fsti(97,48-97,66); use=ReifyTestTSST.fsti(97,48-97,66)
(FStar.Preorder.stable ReifyTestTSST.state
@x1
(ReifyTestTSST.state_rel Dummy_value))
)
))
...
@x1
Tm_refine_8c7a59d2aae063ddae7f7f455c88be60))
:qid refinement_interpretation_Tm_refine_8c7a59d2aae063ddae7f7f455c88be60))

:named refinement_interpretation_Tm_refine_8c7a59d2aae063ddae7f7f455c88be60))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name Prims.sum; Namespace Prims; Name Prims.Left; Namespace Prims; Name Prims.Right; Namespace Prims
(assert (! 
;; def=Prims.fst(193,4-193,9); use=Prims.fst(193,4-193,9)
...
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(133,14-136,12); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x0
Tm_type)
(HasType @x1
...
(HasType @x2
(Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a @x0)))
(HasType (Non_total_Tm_arrow_88f057a810db543d1c7c6b6178fead3c @x0
@x1
@x2)
Tm_type))
 

:pattern ((HasType (Non_total_Tm_arrow_88f057a810db543d1c7c6b6178fead3c @x0
@x1
@x2)
Tm_type))
:qid non_total_function_typing_Non_total_Tm_arrow_88f057a810db543d1c7c6b6178fead3c))

:named non_total_function_typing_Non_total_Tm_arrow_88f057a810db543d1c7c6b6178fead3c))
;;;;;;;;;;;;;;;;\/ interpretation
;;; Fact-ids: Name Prims.l_or; Namespace Prims
...
:qid l_and-interp))
:named l_and-interp))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6
;;; Fact-ids: Name ReifyTestTSST.tsst_pre; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(53,38-53,72); use=ReifyTestTSST.fsti(53,38-53,72)
(forall ((@x0 Term))
 (! (HasType (Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6 @x0)
Tm_type)
 

:pattern ((HasType (Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6 @x0)
Tm_type))
:qid kinding_Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6))

:named kinding_Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_8483b8fa8baab11fbc6bf59532304f92
;;; Fact-ids: Name ReifyTestTSST.tsst_post; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(55,28-55,77); use=ReifyTestTSST.fsti(55,38-55,77)
(forall ((@x0 Term) (@x1 Term))
 (! (HasType (Tm_arrow_8483b8fa8baab11fbc6bf59532304f92 @x0
@x1)
Tm_type)
 

:pattern ((HasType (Tm_arrow_8483b8fa8baab11fbc6bf59532304f92 @x0
@x1)
Tm_type))
:qid kinding_Tm_arrow_8483b8fa8baab11fbc6bf59532304f92))

:named kinding_Tm_arrow_8483b8fa8baab11fbc6bf59532304f92))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_7418b65d64c02ff38a120101f758e12a
;;; Fact-ids: Name ReifyTestTSST.state_rel; Namespace ReifyTestTSST
(assert (! (HasType Tm_arrow_7418b65d64c02ff38a120101f758e12a
Tm_type)
:named kinding_Tm_arrow_7418b65d64c02ff38a120101f758e12a))
;;;;;;;;;;;;;;;;kinding_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! (HasType Tm_arrow_53e44af7f6e83320c9be36b1f475adcb
Tm_type)
:named kinding_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb))
...
;;;;;;;;;;;;;;;;kinding_Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(135,12-135,64); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@x0 Term))
 (! (HasType (Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a @x0)
Tm_type)
 

:pattern ((HasType (Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a @x0)
Tm_type))
:qid kinding_Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a))

:named kinding_Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a))
...
:named interpretation_Tm_abs_577bb0504d99bec50af9e2beb88b9dcb))
;;;;;;;;;;;;;;;;haseq for Tm_refine_d82b849f9f4e183841cbef932b6fc45f
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(138,10-138,39); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (iff (Valid (Prims.hasEq (Tm_refine_d82b849f9f4e183841cbef932b6fc45f @x0
@x1
@x2)))
...
:pattern ((Valid (Prims.hasEq (Tm_refine_d82b849f9f4e183841cbef932b6fc45f @x0
@x1
@x2))))
:qid haseqTm_refine_d82b849f9f4e183841cbef932b6fc45f))

:named haseqTm_refine_d82b849f9f4e183841cbef932b6fc45f))
...
;;;;;;;;;;;;;;;;haseq for Tm_refine_8c7a59d2aae063ddae7f7f455c88be60
;;; Fact-ids: Name ReifyTestTSST.witnessed; Namespace ReifyTestTSST
(assert (! (iff (Valid (Prims.hasEq Tm_refine_8c7a59d2aae063ddae7f7f455c88be60))
(Valid (Prims.hasEq (FStar.Preorder.predicate ReifyTestTSST.state))))
:named haseqTm_refine_8c7a59d2aae063ddae7f7f455c88be60))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(43,4-43,10); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@x0 Term))
 (! (and (NoHoist @x0
(HasType ReifyTestTSST.reify_@tok
Tm_arrow_53e44af7f6e83320c9be36b1f475adcb))
...
:pattern ((ReifyTestTSST.reify_ @x1
@x2
@x3
@x4
@x5))
:qid function_token_typing_ReifyTestTSST.reify_.1))
)
 

:pattern ((ApplyTT @x0
ReifyTestTSST.reify_@tok))
:qid function_token_typing_ReifyTestTSST.reify_))

:named function_token_typing_ReifyTestTSST.reify_))
;;;;;;;;;;;;;;;;inversion axiom
;;; Fact-ids: Name Prims.sum; Namespace Prims; Name Prims.Left; Namespace Prims; Name Prims.Right; Namespace Prims
(assert (! 
;; def=Prims.fst(191,5-191,8); use=Prims.fst(191,5-191,8)
...
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.witnessed
;;; Fact-ids: Name ReifyTestTSST.witnessed; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(97,4-97,13); use=ReifyTestTSST.fsti(97,4-97,13)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ReifyTestTSST.witnessed @x0
@x1)
(FStar.Monotonic.Witnessed.witnessed ReifyTestTSST.state
(ReifyTestTSST.state_rel Dummy_value)
@x1))
...
:pattern ((ReifyTestTSST.witnessed @x0
@x1))
:qid equation_ReifyTestTSST.witnessed))

:named equation_ReifyTestTSST.witnessed))
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.tsst_pre
;;; Fact-ids: Name ReifyTestTSST.tsst_pre; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(53,4-53,12); use=ReifyTestTSST.fsti(53,4-53,12)
(forall ((@x0 Term))
 (! (= (ReifyTestTSST.tsst_pre @x0)
(Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6 @x0))
 

:pattern ((ReifyTestTSST.tsst_pre @x0))
:qid equation_ReifyTestTSST.tsst_pre))

:named equation_ReifyTestTSST.tsst_pre))
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.tsst_post
;;; Fact-ids: Name ReifyTestTSST.tsst_post; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(55,4-55,13); use=ReifyTestTSST.fsti(55,4-55,13)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ReifyTestTSST.tsst_post @x0
@x1)
(Tm_arrow_8483b8fa8baab11fbc6bf59532304f92 @x0
@x1))
 

:pattern ((ReifyTestTSST.tsst_post @x0
@x1))
:qid equation_ReifyTestTSST.tsst_post))

:named equation_ReifyTestTSST.tsst_post))
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.timestamped_state
;;; Fact-ids: Name ReifyTestTSST.timestamped_state; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(27,4-27,21); use=ReifyTestTSST.fst(27,4-27,21)
(forall ((@x0 Term))
 (! (= (ReifyTestTSST.timestamped_state @x0)
(FStar.Pervasives.Native.tuple2 ReifyTestTSST.timestamp
@x0))
 

:pattern ((ReifyTestTSST.timestamped_state @x0))
:qid equation_ReifyTestTSST.timestamped_state))

:named equation_ReifyTestTSST.timestamped_state))
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.state_rel
;;; Fact-ids: Name ReifyTestTSST.state_rel; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(64,4-64,13); use=ReifyTestTSST.fsti(64,4-64,13)
(forall ((@u0 Dummy_sort))
...
:qid equation_ReifyTestTSST.state_rel))

:named equation_ReifyTestTSST.state_rel))
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.reify_
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(43,4-43,10); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term) (@x3 Term) (@x4 Term))
...
@x3
@x4)
(Prims.admit (FStar.Pervasives.Native.tuple2 @x0
(ReifyTestTSST.timestamped_state ReifyTestTSST.state))
Tm_unit))
...
@x3
@x4))
:qid equation_ReifyTestTSST.reify_))

:named equation_ReifyTestTSST.reify_))
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.get_timestamp
;;; Fact-ids: Name ReifyTestTSST.get_timestamp; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(29,4-29,17); use=ReifyTestTSST.fst(29,4-29,17)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ReifyTestTSST.get_timestamp @x0
@x1)
(FStar.Pervasives.Native.fst ReifyTestTSST.timestamp
@x0
@x1))
...
:pattern ((ReifyTestTSST.get_timestamp @x0
@x1))
:qid equation_ReifyTestTSST.get_timestamp))

:named equation_ReifyTestTSST.get_timestamp))
;;;;;;;;;;;;;;;;Equation for ReifyTestTSST.get_state
;;; Fact-ids: Name ReifyTestTSST.get_state; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fst(31,4-31,13); use=ReifyTestTSST.fst(31,4-31,13)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ReifyTestTSST.get_state @x0
@x1)
(FStar.Pervasives.Native.snd ReifyTestTSST.timestamp
@x0
@x1))
...
:pattern ((ReifyTestTSST.get_state @x0
@x1))
:qid equation_ReifyTestTSST.get_state))

:named equation_ReifyTestTSST.get_state))
...
;;;;;;;;;;;;;;;;Equation for FStar.Preorder.stable
;;; Fact-ids: Name FStar.Preorder.stable; Namespace FStar.Preorder
(assert (! 
;; def=FStar.Preorder.fst(35,4-35,10); use=FStar.Preorder.fst(35,4-35,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (Valid (FStar.Preorder.stable @x0
@x1
@x2))

;; def=FStar.Preorder.fst(36,2-36,46); use=FStar.Preorder.fst(36,2-36,46)
...
;; def=FStar.Preorder.fst(36,23-36,26); use=FStar.Preorder.fst(36,23-36,26)
(Valid 
;; def=FStar.Preorder.fst(36,23-36,26); use=FStar.Preorder.fst(36,23-36,26)
(ApplyTT @x1
@x3)
...
;; def=FStar.Preorder.fst(36,30-36,37); use=FStar.Preorder.fst(36,30-36,37)
(Valid 
;; def=FStar.Preorder.fst(36,30-36,37); use=FStar.Preorder.fst(36,30-36,37)
(ApplyTT (ApplyTT @x2
@x3)
...
;; def=FStar.Preorder.fst(36,43-36,46); use=FStar.Preorder.fst(36,43-36,46)
(Valid 
;; def=FStar.Preorder.fst(36,43-36,46); use=FStar.Preorder.fst(36,43-36,46)
(ApplyTT @x1
@x4)
...
;;no pats
:qid equation_FStar.Preorder.stable.1))
)
...
@x1
@x2))
:qid equation_FStar.Preorder.stable))

:named equation_FStar.Preorder.stable))
...
;;;;;;;;;;;;;;;;Equation for FStar.Preorder.predicate
;;; Fact-ids: Name FStar.Preorder.predicate; Namespace FStar.Preorder
(assert (! 
;; def=FStar.Preorder.fst(22,5-22,14); use=FStar.Preorder.fst(22,5-22,14)
(forall ((@x0 Term))
...
:pattern ((FStar.Preorder.predicate @x0))
:qid equation_FStar.Preorder.predicate))

:named equation_FStar.Preorder.predicate))
...
:qid eq2-interp))
:named eq2-interp))
;;;;;;;;;;;;;;;;Prop-typing for FStar.Preorder.stable
;;; Fact-ids: Name FStar.Preorder.stable; Namespace FStar.Preorder
(assert (! 
;; def=FStar.Preorder.fst(35,4-35,10); use=FStar.Preorder.fst(35,4-35,10)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
...
(HasType @x2
(Tm_refine_bd10f09297e0e7dc08314f7d9211801c @x0)))
(Valid (Prims.subtype_of (FStar.Preorder.stable @x0
@x1
@x2)
Prims.unit)))
 

:pattern ((Prims.subtype_of (FStar.Preorder.stable @x0
@x1
@x2)
Prims.unit))
:qid defn_equation_FStar.Preorder.stable))

:named defn_equation_FStar.Preorder.stable))
;;;;;;;;;;;;;;;;data constructor typing intro
;;; Fact-ids: Name Prims.sum; Namespace Prims; Name Prims.Left; Namespace Prims; Name Prims.Right; Namespace Prims
(assert (! 
;; def=Prims.fst(193,4-193,9); use=Prims.fst(193,4-193,9)
...
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term))
 (! (implies (HasTypeFuel @u0
@x1
(Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6 @x2))
(is-Tm_arrow (PreType @x1)))
...
@x1
(Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6 @x2)))
:qid ReifyTestTSST_pre_typing_Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6))

:named ReifyTestTSST_pre_typing_Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name ReifyTestTSST.tsst_post; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(55,28-55,77); use=ReifyTestTSST.fsti(55,38-55,77)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (implies (HasTypeFuel @u0
@x1
(Tm_arrow_8483b8fa8baab11fbc6bf59532304f92 @x2
@x3))
(is-Tm_arrow (PreType @x1)))
...
(Tm_arrow_8483b8fa8baab11fbc6bf59532304f92 @x2
@x3)))
:qid ReifyTestTSST_pre_typing_Tm_arrow_8483b8fa8baab11fbc6bf59532304f92))

:named ReifyTestTSST_pre_typing_Tm_arrow_8483b8fa8baab11fbc6bf59532304f92))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name ReifyTestTSST.state_rel; Namespace ReifyTestTSST
(assert (! 
;; def=Prims.fst(180,21-564,34); use=ReifyTestTSST.fsti(64,37-64,56)
...
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(133,14-138,39); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_53e44af7f6e83320c9be36b1f475adcb)
...
@x1
Tm_arrow_53e44af7f6e83320c9be36b1f475adcb))
:qid ReifyTestTSST_pre_typing_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb))

:named ReifyTestTSST_pre_typing_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(135,12-135,64); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term))
 (! (implies (HasTypeFuel @u0
@x1
(Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a @x2))
(is-Tm_arrow (PreType @x1)))
...
@x1
(Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a @x2)))
:qid ReifyTestTSST_pre_typing_Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a))

:named ReifyTestTSST_pre_typing_Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6
;;; Fact-ids: Name ReifyTestTSST.tsst_pre; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(53,38-53,72); use=ReifyTestTSST.fsti(53,38-53,72)
(forall ((@x0 Term) (@x1 Term))
 (! (iff (HasTypeZ @x0
(Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6 @x1))
(and 
;; def=ReifyTestTSST.fsti(53,38-53,72); use=ReifyTestTSST.fsti(53,38-53,72)
(forall ((@x2 Term))
 (! (implies (HasType @x2
(ReifyTestTSST.timestamped_state @x1))
(HasType (ApplyTT @x0
@x2)
Tm_type))
...
:pattern ((ApplyTT @x0
@x2))
:qid ReifyTestTSST_interpretation_Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6.1))
...
:pattern ((HasTypeZ @x0
(Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6 @x1)))
:qid ReifyTestTSST_interpretation_Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6))

:named ReifyTestTSST_interpretation_Tm_arrow_ade8e69f1ac045eab1276f0a9a4026d6))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_8483b8fa8baab11fbc6bf59532304f92
;;; Fact-ids: Name ReifyTestTSST.tsst_post; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(55,28-55,77); use=ReifyTestTSST.fsti(55,38-55,77)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (iff (HasTypeZ @x0
(Tm_arrow_8483b8fa8baab11fbc6bf59532304f92 @x1
@x2))
(and 
;; def=ReifyTestTSST.fsti(55,28-55,77); use=ReifyTestTSST.fsti(55,38-55,77)
(forall ((@x3 Term) (@x4 Term))
...
@x3)
@x4))
:qid ReifyTestTSST_interpretation_Tm_arrow_8483b8fa8baab11fbc6bf59532304f92.1))
...
:pattern ((ApplyTT @x0
@x3))
:qid ReifyTestTSST_interpretation_Tm_arrow_8483b8fa8baab11fbc6bf59532304f92.2))
))
...
(Tm_arrow_8483b8fa8baab11fbc6bf59532304f92 @x1
@x2)))
:qid ReifyTestTSST_interpretation_Tm_arrow_8483b8fa8baab11fbc6bf59532304f92))

:named ReifyTestTSST_interpretation_Tm_arrow_8483b8fa8baab11fbc6bf59532304f92))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_7418b65d64c02ff38a120101f758e12a
;;; Fact-ids: Name ReifyTestTSST.state_rel; Namespace ReifyTestTSST
(assert (! 
;; def=Prims.fst(180,21-564,34); use=ReifyTestTSST.fsti(64,37-64,56)
...
:named ReifyTestTSST_interpretation_Tm_arrow_7418b65d64c02ff38a120101f758e12a))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(133,14-138,39); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@x0 Term))
...
(ReifyTestTSST.tsst_pre ReifyTestTSST.state))
(HasType @x3
(Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a @x1))
(HasType @x4
(Non_total_Tm_arrow_88f057a810db543d1c7c6b6178fead3c @x1
@x2
@x3))
(HasType @x5
(ReifyTestTSST.timestamped_state ReifyTestTSST.state)))
...
@x4)
@x5)
(Tm_refine_d82b849f9f4e183841cbef932b6fc45f @x5
@x1
@x3)))
...
@x4)
@x5))
:qid ReifyTestTSST_interpretation_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb.1))
...
:pattern ((ApplyTT @x0
@x1))
:qid ReifyTestTSST_interpretation_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb.2))
...
@x1)
@x2))
:qid ReifyTestTSST_interpretation_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb.3))
...
@x2)
@x3))
:qid ReifyTestTSST_interpretation_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb.4))
...
@x3)
@x4))
:qid ReifyTestTSST_interpretation_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb.5))
))
 

:pattern ((HasTypeZ @x0
Tm_arrow_53e44af7f6e83320c9be36b1f475adcb))
:qid ReifyTestTSST_interpretation_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb))

:named ReifyTestTSST_interpretation_Tm_arrow_53e44af7f6e83320c9be36b1f475adcb))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a
;;; Fact-ids: Name ReifyTestTSST.reify_; Namespace ReifyTestTSST
(assert (! 
;; def=ReifyTestTSST.fsti(135,12-135,64); use=ReifyTestTSST.fst(43,4-43,10)
(forall ((@x0 Term) (@x1 Term))
...
(HasType (ApplyTT @x0
@x2)
(ReifyTestTSST.tsst_post ReifyTestTSST.state
@x1)))
 

:pattern ((ApplyTT @x0
@x2))
:qid ReifyTestTSST_interpretation_Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a.1))
...
:pattern ((HasTypeZ @x0
(Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a @x1)))
:qid ReifyTestTSST_interpretation_Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a))

:named ReifyTestTSST_interpretation_Tm_arrow_2b7f18a91613ab7edbc1a749e6e9019a))
;;;;;;;;;;;;;;;;pretyping
;;; Fact-ids: Name Prims.pair; Namespace Prims; Name Prims.Pair; Namespace Prims
(assert (! 
;; def=Prims.fst(183,5-183,9); use=Prims.fst(183,5-183,9)
...
; Starting query at ReifyTestTSST.fsti(147,88-170,4)
...
(declare-fun label_1 () Bool)
;;;;;;;;;;;;;;;;s: state -> Type0
(declare-fun Tm_arrow_6ec2a0104cf451b3384463b9a4a702db () Term)
;;;;;;;;;;;;;;;;kinding_Tm_arrow_6ec2a0104cf451b3384463b9a4a702db
;;; Fact-ids: 
(assert (! (HasType Tm_arrow_6ec2a0104cf451b3384463b9a4a702db
Tm_type)
:named kinding_Tm_arrow_6ec2a0104cf451b3384463b9a4a702db))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(114,25-114,30); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_6ec2a0104cf451b3384463b9a4a702db)
(is-Tm_arrow (PreType @x1)))
...
@x1
Tm_arrow_6ec2a0104cf451b3384463b9a4a702db))
:qid ReifyTestTSST_pre_typing_Tm_arrow_6ec2a0104cf451b3384463b9a4a702db))

:named ReifyTestTSST_pre_typing_Tm_arrow_6ec2a0104cf451b3384463b9a4a702db))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_6ec2a0104cf451b3384463b9a4a702db
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(114,25-114,30); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@x0 Term))
...
:pattern ((ApplyTT @x0
@x1))
:qid ReifyTestTSST_interpretation_Tm_arrow_6ec2a0104cf451b3384463b9a4a702db.1))
...
:pattern ((HasTypeZ @x0
Tm_arrow_6ec2a0104cf451b3384463b9a4a702db))
:qid ReifyTestTSST_interpretation_Tm_arrow_6ec2a0104cf451b3384463b9a4a702db))

:named ReifyTestTSST_interpretation_Tm_arrow_6ec2a0104cf451b3384463b9a4a702db))
(declare-fun Tm_abs_e528333b9d5eec3dd11db629fcea7508 () Term)
;;;;;;;;;;;;;;;;typing_Tm_abs_e528333b9d5eec3dd11db629fcea7508
;;; Fact-ids: 
(assert (! (HasType Tm_abs_e528333b9d5eec3dd11db629fcea7508
Tm_arrow_6ec2a0104cf451b3384463b9a4a702db)
:named typing_Tm_abs_e528333b9d5eec3dd11db629fcea7508))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_e528333b9d5eec3dd11db629fcea7508
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(154,28-154,33); use=ReifyTestTSST.fsti(154,28-154,33)
(forall ((@x0 Term))
 (! (= (ApplyTT Tm_abs_e528333b9d5eec3dd11db629fcea7508
@x0)
(Prims.b2t (Prims.op_GreaterThan @x0
(BoxInt 0))))
 

:pattern ((ApplyTT Tm_abs_e528333b9d5eec3dd11db629fcea7508
@x0))
:qid interpretation_Tm_abs_e528333b9d5eec3dd11db629fcea7508))

:named interpretation_Tm_abs_e528333b9d5eec3dd11db629fcea7508))
...
(declare-fun Non_total_Tm_arrow_d98eef1c3ea7562ff73800fa6a004c34 () Term)
;;;;;;;;;;;;;;;;Typing for non-total arrows
;;; Fact-ids: 
(assert (! (HasType Non_total_Tm_arrow_d98eef1c3ea7562ff73800fa6a004c34
Tm_type)
:named non_total_function_typing_Non_total_Tm_arrow_d98eef1c3ea7562ff73800fa6a004c34))


;;;;;;;;;;;;;;;;_: timestamp -> Prims.GTot Type
(declare-fun Tm_arrow_b519a73ba534f2602101b33df8a6b55c () Term)
;;;;;;;;;;;;;;;;kinding_Tm_arrow_b519a73ba534f2602101b33df8a6b55c
;;; Fact-ids: 
(assert (! (HasType Tm_arrow_b519a73ba534f2602101b33df8a6b55c
Tm_type)
:named kinding_Tm_arrow_b519a73ba534f2602101b33df8a6b55c))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(35,26-35,35); use=ReifyTestTSST.fsti(160,10-160,16)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_b519a73ba534f2602101b33df8a6b55c)
(is-Tm_arrow (PreType @x1)))
...
@x1
Tm_arrow_b519a73ba534f2602101b33df8a6b55c))
:qid ReifyTestTSST_pre_typing_Tm_arrow_b519a73ba534f2602101b33df8a6b55c))

:named ReifyTestTSST_pre_typing_Tm_arrow_b519a73ba534f2602101b33df8a6b55c))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_b519a73ba534f2602101b33df8a6b55c
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(35,26-35,35); use=ReifyTestTSST.fsti(160,10-160,16)
(forall ((@x0 Term))
...
:pattern ((ApplyTT @x0
@x1))
:qid ReifyTestTSST_interpretation_Tm_arrow_b519a73ba534f2602101b33df8a6b55c.1))
...
:pattern ((HasTypeZ @x0
Tm_arrow_b519a73ba534f2602101b33df8a6b55c))
:qid ReifyTestTSST_interpretation_Tm_arrow_b519a73ba534f2602101b33df8a6b55c))

:named ReifyTestTSST_interpretation_Tm_arrow_b519a73ba534f2602101b33df8a6b55c))





(declare-fun Tm_abs_5491e1e6dca087f89a943a2cf569d3bc (Term) Term)
;;;;;;;;;;;;;;;;typing_Tm_abs_5491e1e6dca087f89a943a2cf569d3bc
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(160,35-163,54); use=ReifyTestTSST.fsti(160,10-160,16)
(forall ((@x0 Term))
 (! (HasType (Tm_abs_5491e1e6dca087f89a943a2cf569d3bc @x0)
Tm_arrow_b519a73ba534f2602101b33df8a6b55c)
 

:pattern ((Tm_abs_5491e1e6dca087f89a943a2cf569d3bc @x0))
:qid typing_Tm_abs_5491e1e6dca087f89a943a2cf569d3bc))

:named typing_Tm_abs_5491e1e6dca087f89a943a2cf569d3bc))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_5491e1e6dca087f89a943a2cf569d3bc
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(160,35-163,54); use=ReifyTestTSST.fsti(160,10-160,16)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ApplyTT (Tm_abs_5491e1e6dca087f89a943a2cf569d3bc @x1)
@x0)
(Prims.l_and (Prims.l_or (ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x0)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x1))
(Prims.eq2 ReifyTestTSST.timestamp
@x0
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x1)))
(ReifyTestTSST.witnessed @x0
Tm_abs_e528333b9d5eec3dd11db629fcea7508)))
 

:pattern ((ApplyTT (Tm_abs_5491e1e6dca087f89a943a2cf569d3bc @x1)
@x0))
:qid interpretation_Tm_abs_5491e1e6dca087f89a943a2cf569d3bc))

:named interpretation_Tm_abs_5491e1e6dca087f89a943a2cf569d3bc))
...
;; def=ReifyTestTSST.fsti(160,35-163,54); use=ReifyTestTSST.fsti(160,10-160,16)
(forall ((@x0 Term))
 (! (iff (Valid (Prims.l_Exists ReifyTestTSST.timestamp
(Tm_abs_5491e1e6dca087f89a943a2cf569d3bc @x0)))
...
;; def=ReifyTestTSST.fsti(161,22-162,34); use=ReifyTestTSST.fsti(160,10-160,16)
(or 
;; def=ReifyTestTSST.fsti(161,23-161,55); use=ReifyTestTSST.fsti(160,10-160,16)
(Valid 
;; def=ReifyTestTSST.fsti(161,23-161,55); use=ReifyTestTSST.fsti(160,10-160,16)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x1)
...
;; def=ReifyTestTSST.fsti(162,11-162,33); use=ReifyTestTSST.fsti(160,10-160,16)
(= @x1
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x0))
)
...
;; def=ReifyTestTSST.fsti(163,25-163,54); use=ReifyTestTSST.fsti(160,10-160,16)
(Valid 
;; def=ReifyTestTSST.fsti(163,25-163,54); use=ReifyTestTSST.fsti(160,10-160,16)
(ReifyTestTSST.witnessed @x1
Tm_abs_e528333b9d5eec3dd11db629fcea7508)
)
)
 
;;no pats
:qid l_quant_interp_f5817b9d445ae7096e75e29ad0f5ca4a.1))
)
 

:pattern ((Valid (Prims.l_Exists ReifyTestTSST.timestamp
(Tm_abs_5491e1e6dca087f89a943a2cf569d3bc @x0))))
:qid l_quant_interp_f5817b9d445ae7096e75e29ad0f5ca4a))

:named l_quant_interp_f5817b9d445ae7096e75e29ad0f5ca4a))
...
(declare-fun Tm_refine_4303e0506cfa49b2fdea568febabcd6d (Term) Term)
;;;;;;;;;;;;;;;;refinement kinding
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(138,10-138,39); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@x0 Term))
 (! (HasType (Tm_refine_4303e0506cfa49b2fdea568febabcd6d @x0)
Tm_type)
 

:pattern ((HasType (Tm_refine_4303e0506cfa49b2fdea568febabcd6d @x0)
Tm_type))
:qid refinement_kinding_Tm_refine_4303e0506cfa49b2fdea568febabcd6d))

:named refinement_kinding_Tm_refine_4303e0506cfa49b2fdea568febabcd6d))
;;;;;;;;;;;;;;;;refinement_interpretation
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(138,10-138,39); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term))
 (! (iff (HasTypeFuel @u0
@x1
(Tm_refine_4303e0506cfa49b2fdea568febabcd6d @x2))
(and (HasTypeFuel @u0
@x1
(FStar.Pervasives.Native.tuple2 Prims.unit
(ReifyTestTSST.timestamped_state ReifyTestTSST.state)))

;; def=ReifyTestTSST.fsti(139,52-140,71); use=ReifyTestTSST.fsti(147,88-170,4)
(or 
;; def=ReifyTestTSST.fsti(139,53-139,108); use=ReifyTestTSST.fsti(147,88-170,4)
(Valid 
;; def=ReifyTestTSST.fsti(139,53-139,108); use=ReifyTestTSST.fsti(147,88-170,4)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(140,27-140,70); use=ReifyTestTSST.fsti(147,88-170,4)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x2)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
(FStar.Pervasives.Native.snd Prims.unit
...
;; def=ReifyTestTSST.fsti(141,24-141,70); use=ReifyTestTSST.fsti(147,88-170,4)
(Valid 
;; def=ReifyTestTSST.fsti(141,24-141,70); use=ReifyTestTSST.fsti(147,88-170,4)
(ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
...
@x1
(Tm_refine_4303e0506cfa49b2fdea568febabcd6d @x2)))
:qid refinement_interpretation_Tm_refine_4303e0506cfa49b2fdea568febabcd6d))

:named refinement_interpretation_Tm_refine_4303e0506cfa49b2fdea568febabcd6d))
;;;;;;;;;;;;;;;;haseq for Tm_refine_4303e0506cfa49b2fdea568febabcd6d
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(138,10-138,39); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@x0 Term))
 (! (iff (Valid (Prims.hasEq (Tm_refine_4303e0506cfa49b2fdea568febabcd6d @x0)))
(Valid (Prims.hasEq (FStar.Pervasives.Native.tuple2 Prims.unit
(ReifyTestTSST.timestamped_state ReifyTestTSST.state)))))
 

:pattern ((Valid (Prims.hasEq (Tm_refine_4303e0506cfa49b2fdea568febabcd6d @x0))))
:qid haseqTm_refine_4303e0506cfa49b2fdea568febabcd6d))

:named haseqTm_refine_4303e0506cfa49b2fdea568febabcd6d))


;;;;;;;;;;;;;;;;s0: timestamped_state state -> Prims.Pure (Prims.unit & timestamped_state state)
(declare-fun Tm_arrow_542fab4e08fc235ca028273a257143cf () Term)
;;;;;;;;;;;;;;;;kinding_Tm_arrow_542fab4e08fc235ca028273a257143cf
;;; Fact-ids: 
(assert (! (HasType Tm_arrow_542fab4e08fc235ca028273a257143cf
Tm_type)
:named kinding_Tm_arrow_542fab4e08fc235ca028273a257143cf))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(137,8-138,39); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_542fab4e08fc235ca028273a257143cf)
(is-Tm_arrow (PreType @x1)))
...
@x1
Tm_arrow_542fab4e08fc235ca028273a257143cf))
:qid ReifyTestTSST_pre_typing_Tm_arrow_542fab4e08fc235ca028273a257143cf))

:named ReifyTestTSST_pre_typing_Tm_arrow_542fab4e08fc235ca028273a257143cf))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_542fab4e08fc235ca028273a257143cf
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(137,8-138,39); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@x0 Term))
...
(forall ((@x1 Term))
 (! (implies (and 
;; def=ReifyTestTSST.fsti(160,35-163,54); use=ReifyTestTSST.fsti(147,88-170,4)
(exists ((@x2 Term))
 (! (and (HasType @x2
ReifyTestTSST.timestamp)

;; def=ReifyTestTSST.fsti(161,22-162,34); use=ReifyTestTSST.fsti(147,88-170,4)
(or 
;; def=ReifyTestTSST.fsti(161,23-161,55); use=ReifyTestTSST.fsti(147,88-170,4)
(Valid 
;; def=ReifyTestTSST.fsti(161,23-161,55); use=ReifyTestTSST.fsti(147,88-170,4)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x2)
...
;; def=ReifyTestTSST.fsti(162,11-162,33); use=ReifyTestTSST.fsti(147,88-170,4)
(= @x2
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x1))
)
...
;; def=ReifyTestTSST.fsti(163,25-163,54); use=ReifyTestTSST.fsti(147,88-170,4)
(Valid 
;; def=ReifyTestTSST.fsti(163,25-163,54); use=ReifyTestTSST.fsti(147,88-170,4)
(ReifyTestTSST.witnessed @x2
Tm_abs_e528333b9d5eec3dd11db629fcea7508)
)
)
 
;;no pats
:qid ReifyTestTSST_interpretation_Tm_arrow_542fab4e08fc235ca028273a257143cf.2))

(HasType @x1
(ReifyTestTSST.timestamped_state ReifyTestTSST.state)))
(HasType (ApplyTT @x0
@x1)
(Tm_refine_4303e0506cfa49b2fdea568febabcd6d @x1)))
 

:pattern ((ApplyTT @x0
@x1))
:qid ReifyTestTSST_interpretation_Tm_arrow_542fab4e08fc235ca028273a257143cf.1))
...
:pattern ((HasTypeZ @x0
Tm_arrow_542fab4e08fc235ca028273a257143cf))
:qid ReifyTestTSST_interpretation_Tm_arrow_542fab4e08fc235ca028273a257143cf))

:named ReifyTestTSST_interpretation_Tm_arrow_542fab4e08fc235ca028273a257143cf))
...
;;;;;;;;;;;;;;;;s0: timestamped_state state -> Type0
(declare-fun Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548 () Term)
;;;;;;;;;;;;;;;;kinding_Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548
;;; Fact-ids: 
(assert (! (HasType Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548
Tm_type)
:named kinding_Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(53,38-53,72); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548)
(is-Tm_arrow (PreType @x1)))
...
@x1
Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548))
:qid ReifyTestTSST_pre_typing_Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548))

:named ReifyTestTSST_pre_typing_Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(53,38-53,72); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@x0 Term))
...
:pattern ((ApplyTT @x0
@x1))
:qid ReifyTestTSST_interpretation_Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548.1))
...
:pattern ((HasTypeZ @x0
Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548))
:qid ReifyTestTSST_interpretation_Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548))

:named ReifyTestTSST_interpretation_Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548))
(declare-fun Tm_abs_87897e1227d4c10418dfe7d54680feb0 () Term)
;;;;;;;;;;;;;;;;typing_Tm_abs_87897e1227d4c10418dfe7d54680feb0
;;; Fact-ids: 
(assert (! (HasType Tm_abs_87897e1227d4c10418dfe7d54680feb0
Tm_arrow_0eb0fcbd32600c5f995f9bc9bfb6c548)
:named typing_Tm_abs_87897e1227d4c10418dfe7d54680feb0))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_87897e1227d4c10418dfe7d54680feb0
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(160,35-163,54); use=ReifyTestTSST.fsti(160,35-163,54)
(forall ((@x0 Term))
 (! (= (ApplyTT Tm_abs_87897e1227d4c10418dfe7d54680feb0
@x0)
(Prims.l_Exists ReifyTestTSST.timestamp
(Tm_abs_5491e1e6dca087f89a943a2cf569d3bc @x0)))
 

:pattern ((ApplyTT Tm_abs_87897e1227d4c10418dfe7d54680feb0
@x0))
:qid interpretation_Tm_abs_87897e1227d4c10418dfe7d54680feb0))

:named interpretation_Tm_abs_87897e1227d4c10418dfe7d54680feb0))
;;;;;;;;;;;;;;;;_: timestamped_state state -> _: Prims.unit -> _: timestamped_state state -> Type0
(declare-fun Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9 () Term)
;;;;;;;;;;;;;;;;kinding_Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9
;;; Fact-ids: 
(assert (! (HasType Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9
Tm_type)
:named kinding_Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(55,43-160,22); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9)
(is-Tm_arrow (PreType @x1)))
...
@x1
Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9))
:qid ReifyTestTSST_pre_typing_Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9))

:named ReifyTestTSST_pre_typing_Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(55,43-160,22); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@x0 Term))
...
@x2)
@x3))
:qid ReifyTestTSST_interpretation_Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9.1))
...
:pattern ((ApplyTT @x0
@x1))
:qid ReifyTestTSST_interpretation_Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9.2))
...
@x1)
@x2))
:qid ReifyTestTSST_interpretation_Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9.3))
))
 

:pattern ((HasTypeZ @x0
Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9))
:qid ReifyTestTSST_interpretation_Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9))

:named ReifyTestTSST_interpretation_Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9))
(declare-fun Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd () Term)
;;;;;;;;;;;;;;;;typing_Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd
;;; Fact-ids: 
(assert (! (HasType Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd
Tm_arrow_3b9f2f9d15d98c257419565a7e751ff9)
:named typing_Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd
;;; Fact-ids: 
(assert (! 
;; def=ReifyTestTSST.fsti(164,23-164,27); use=ReifyTestTSST.fsti(164,23-164,27)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (ApplyTT (ApplyTT (ApplyTT Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd
@x0)
@x1)
@x2)
Prims.l_True)
...
:pattern ((ApplyTT (ApplyTT (ApplyTT Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd
@x0)
@x1)
@x2))
:qid interpretation_Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd))

:named interpretation_Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd))
...
ReifyTestTSST.reify_@tok))
:named @kick_partial_app_44256ccffc82d9477f88767cdcc9f79a))
...
; *)
;   forall (p:
;     FStar.Pervasives.st_post_h (ReifyTestTSST.timestamped_state ReifyTestTSST.state) Prims.unit)
;     (h: ReifyTestTSST.timestamped_state ReifyTestTSST.state).
;     (forall (x: Prims.unit) (s1: ReifyTestTSST.timestamped_state ReifyTestTSST.state).
;         ReifyTestTSST.state_rel (ReifyTestTSST.get_state h) (ReifyTestTSST.get_state s1) /\
;         (ReifyTestTSST.older_than (ReifyTestTSST.get_timestamp h) (ReifyTestTSST.get_timestamp s1) \/
;           ReifyTestTSST.get_timestamp h == ReifyTestTSST.get_timestamp s1) ==>
;         p x s1) ==>
;     (forall (x: ReifyTestTSST.timestamped_state ReifyTestTSST.state)
;         (s1: ReifyTestTSST.timestamped_state ReifyTestTSST.state).
;         ReifyTestTSST.state_rel (ReifyTestTSST.get_state h) (ReifyTestTSST.get_state s1) /\
;         (ReifyTestTSST.older_than (ReifyTestTSST.get_timestamp h) (ReifyTestTSST.get_timestamp s1) \/
;           ReifyTestTSST.get_timestamp h == ReifyTestTSST.get_timestamp s1) /\ (h == x) /\ (s1 == x) ==>
;         ReifyTestTSST.get_state x + 1 >= 0 /\
;         (forall (return_val: ReifyTestTSST.state).
;             return_val == ReifyTestTSST.get_state x + 1 ==>
;             (forall (any_result: Type0).
;                 ReifyTestTSST.state_rel (ReifyTestTSST.get_state x) (ReifyTestTSST.get_state x + 1) ==
;                 any_result ==>
;                 (forall (pure_result: Prims.unit).
;                     ReifyTestTSST.state_rel (ReifyTestTSST.get_state x)
;                       (ReifyTestTSST.get_state x + 1) ==>
;                     ReifyTestTSST.get_state x + 1 >= 0 /\
;                     (forall (return_val: ReifyTestTSST.state).
;                         return_val == ReifyTestTSST.get_state x + 1 ==>
;                         ReifyTestTSST.state_rel (ReifyTestTSST.get_state s1)
;                           (ReifyTestTSST.get_state x + 1) /\
;                         (forall (x: Prims.unit)
;                             (s1: ReifyTestTSST.timestamped_state ReifyTestTSST.state).
;                             ReifyTestTSST.state_rel (ReifyTestTSST.get_state s1)
;                               (ReifyTestTSST.get_state x + 1) /\
;                             ReifyTestTSST.state_rel (ReifyTestTSST.get_state s1)
;                               (ReifyTestTSST.get_state s1) /\
;                             (ReifyTestTSST.older_than (ReifyTestTSST.get_timestamp s1)
;                                 (ReifyTestTSST.get_timestamp s1) \/
;                               ReifyTestTSST.get_timestamp s1 == ReifyTestTSST.get_timestamp s1) /\
;                             (ReifyTestTSST.get_state s1 == ReifyTestTSST.get_state x + 1) /\
;                             ReifyTestTSST.older_than (ReifyTestTSST.get_timestamp s1)
;                               (ReifyTestTSST.get_timestamp s1) ==>
;                             Prims.auto_squash (FStar.Preorder.stable (fun s -> s > 0)
;                                   ReifyTestTSST.state_rel) /\
;                             (forall (any_result:
;                                 p:
;                                 FStar.Preorder.predicate ReifyTestTSST.state
;                                   {FStar.Preorder.stable p ReifyTestTSST.state_rel}).
;                                 (fun s -> s > 0) == any_result ==>
;                                 ReifyTestTSST.get_state s1 > 0 /\
;                                 (forall (x: Prims.unit)
;                                     (s1: ReifyTestTSST.timestamped_state ReifyTestTSST.state).
;                                     ReifyTestTSST.get_state s1 > 0 /\
;                                     ReifyTestTSST.state_rel (ReifyTestTSST.get_state s1)
...
;                                       ReifyTestTSST.get_timestamp s1 ==
;                                       ReifyTestTSST.get_timestamp s1) /\
;                                     (ReifyTestTSST.get_state s1 == ReifyTestTSST.get_state s1) /\
;                                     (ReifyTestTSST.get_timestamp s1 ==
;                                       ReifyTestTSST.get_timestamp s1) /\
;                                     ReifyTestTSST.witnessed (ReifyTestTSST.get_timestamp s1)
;                                       (fun s -> s > 0) ==>
;                                     (forall (x: ReifyTestTSST.timestamped_state ReifyTestTSST.state)
;                                         (s1: ReifyTestTSST.timestamped_state ReifyTestTSST.state).
;                                         ReifyTestTSST.state_rel (ReifyTestTSST.get_state s1)
...
;                                             (ReifyTestTSST.get_timestamp s1) \/
;                                           ReifyTestTSST.get_timestamp s1 ==
;                                           ReifyTestTSST.get_timestamp s1) /\ (s1 == x) /\ (s1 == x) ==>
;                                         (forall (x: Prims.unit)
;                                             (s0:
;                                             ReifyTestTSST.timestamped_state ReifyTestTSST.state).
;                                             (*  - Could not prove post-condition
; *)
;                                             forall (ts: ReifyTestTSST.timestamp).
;                                               ReifyTestTSST.older_than ts
;                                                 (ReifyTestTSST.get_timestamp s0) \/
;                                               ts == ReifyTestTSST.get_timestamp s0 ==>
;                                               Prims.auto_squash (FStar.Preorder.stable (fun s ->
;                                                         s > 0)
;                                                     ReifyTestTSST.state_rel)) /\
;                                         (forall (x: Prims.unit).
;                                             (*  - Could not prove post-condition
; *)
;                                             forall (p:
;                                               FStar.Pervasives.st_post_h (ReifyTestTSST.timestamped_state
;                                                     ReifyTestTSST.state)
;                                                 Prims.unit)
;                                               (h:
;                                               ReifyTestTSST.timestamped_state ReifyTestTSST.state).
;                                               (exists (ts: ReifyTestTSST.timestamp).
;                                                   (ReifyTestTSST.older_than ts
;                                                       (ReifyTestTSST.get_timestamp h) \/
;                                                     ts == ReifyTestTSST.get_timestamp h) /\
;                                                   ReifyTestTSST.witnessed ts (fun s -> s > 0)) /\
;                                               (forall (x: Prims.unit)
;                                                   (s1:
;                                                   ReifyTestTSST.timestamped_state ReifyTestTSST.state
;                                                   ).
...
;                                                         ts == ReifyTestTSST.get_timestamp h) /\
;                                                       ReifyTestTSST.witnessed ts (fun s -> s > 0)) /\
;                                                   ReifyTestTSST.state_rel (ReifyTestTSST.get_state h
;                                                     )
;                                                     (ReifyTestTSST.get_state s1) /\
;                                                   (ReifyTestTSST.older_than (ReifyTestTSST.get_timestamp
;                                                           h)
;                                                       (ReifyTestTSST.get_timestamp s1) \/
;                                                     ReifyTestTSST.get_timestamp h ==
;                                                     ReifyTestTSST.get_timestamp s1) /\
;                                                   (ReifyTestTSST.get_state h ==
;                                                     ReifyTestTSST.get_state s1) /\
;                                                   (ReifyTestTSST.get_timestamp h ==
;                                                     ReifyTestTSST.get_timestamp s1) /\
;                                                   (ReifyTestTSST.get_state s1 > 0) ==>
...
;                                                               (ReifyTestTSST.get_timestamp h) \/
;                                                             ts == ReifyTestTSST.get_timestamp h) /\
;                                                           ReifyTestTSST.witnessed ts
;                                                             (fun s -> s > 0)) /\
;                                                       ReifyTestTSST.state_rel (ReifyTestTSST.get_state
;                                                             h)
;                                                         (ReifyTestTSST.get_state s1) /\
;                                                       (ReifyTestTSST.older_than (ReifyTestTSST.get_timestamp
;                                                               h)
...
;                                                       p x s1))) /\
;                                         (forall (any_result:
;                                             (x: Prims.unit -> ReifyTestTSST.TSST Prims.unit)).
;                                             (forall (s0:
;                                                 ReifyTestTSST.timestamped_state ReifyTestTSST.state)
;                                               .
;                                                 (forall (ts: ReifyTestTSST.timestamp).
...
;                                                       (ReifyTestTSST.get_timestamp s0) \/
;                                                     ts == ReifyTestTSST.get_timestamp s0 ==>
;                                                     Prims.auto_squash (FStar.Preorder.stable (fun
;                                                               s
;                                                               ->
;                                                               s > 0)
;                                                           ReifyTestTSST.state_rel)) /\
;                                                 (forall (any_result:
;                                                     (_: ReifyTestTSST.timestamp -> Prims.GTot Type0))
;                                                   .
;                                                     (fun ts ->
;                                                         (ReifyTestTSST.older_than ts
;                                                             (ReifyTestTSST.get_timestamp s0) \/
...
;                                                               (ReifyTestTSST.get_timestamp s0) \/
;                                                             ts == ReifyTestTSST.get_timestamp s0 ==>
;                                                             Prims.auto_squash (FStar.Preorder.stable
;                                                                   (fun s -> s > 0)
;                                                                   ReifyTestTSST.state_rel))))) /\
;                                             (forall (s0:
;                                                 ReifyTestTSST.timestamped_state ReifyTestTSST.state)
...
;                                                             ReifyTestTSST.timestamped_state ReifyTestTSST.state
;                                                             ))).
;                                                     ReifyTestTSST.reify_ any_result == any_result ==>
;                                                     (exists (ts: ReifyTestTSST.timestamp).
;                                                         (ReifyTestTSST.older_than ts
;                                                             (ReifyTestTSST.get_timestamp x) \/
;                                                           ts == ReifyTestTSST.get_timestamp x) /\
;                                                         ReifyTestTSST.witnessed ts (fun s -> s > 0)) /\
;                                                     (forall (pure_result:
;                                                         Prims.unit &
;                                                         ReifyTestTSST.timestamped_state ReifyTestTSST.state
...
;                                                         (ReifyTestTSST.older_than (ReifyTestTSST.get_timestamp
;                                                                 x)
;                                                             (ReifyTestTSST.get_timestamp (FStar.Pervasives.Native.snd
;                                                                     pure_result)) \/
;                                                           ReifyTestTSST.get_timestamp x ==
;                                                           ReifyTestTSST.get_timestamp (FStar.Pervasives.Native.snd
;                                                                 pure_result)) /\
;                                                         ReifyTestTSST.state_rel (ReifyTestTSST.get_state
;                                                               x)
;                                                           (ReifyTestTSST.get_state (FStar.Pervasives.Native.snd
;                                                                   pure_result)) ==>
;                                                         ReifyTestTSST.reify_ any_result x ==
;                                                         pure_result ==>
...
; Context: While encoding a query
; While typechecking the top-level declaration `let reify_recall_test`
...
(forall ((@x1 Term) (@x2 Term))
 (! (implies (and (HasType @x1
(FStar.Pervasives.st_post_h (ReifyTestTSST.timestamped_state ReifyTestTSST.state)
Prims.unit))
(HasType @x2
(ReifyTestTSST.timestamped_state ReifyTestTSST.state))

;; def=ReifyTestTSST.fsti(85,40-92,14); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@x3 Term) (@x4 Term))
 (! (implies (and (or label_1
(HasType @x3
Prims.unit))
...
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(147,88-170,4)
(or label_3
...
;; def=ReifyTestTSST.fsti(88,7-89,47); use=ReifyTestTSST.fsti(147,88-170,4)
(or label_4

;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(147,88-170,4)
(Valid 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(147,88-170,4)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(89,10-89,46); use=ReifyTestTSST.fsti(147,88-170,4)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x2)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x4))
...
;; def=ReifyTestTSST.fsti(92,7-92,13); use=ReifyTestTSST.fsti(147,88-170,4)
(Valid 
;; def=ReifyTestTSST.fsti(92,7-92,13); use=ReifyTestTSST.fsti(147,88-170,4)
(ApplyTT (ApplyTT @x1
@x3)
...
;; def=ReifyTestTSST.fsti(85,40-92,14); use=ReifyTestTSST.fsti(148,11-148,14)
(forall ((@x3 Term) (@x4 Term))
 (! (implies (and (HasType @x3
(ReifyTestTSST.timestamped_state ReifyTestTSST.state))
(HasType @x4
...
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(148,11-148,14)
(Valid 
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(148,11-148,14)
(ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(88,7-89,47); use=ReifyTestTSST.fsti(148,11-148,14)
(or 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(148,11-148,14)
(Valid 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(148,11-148,14)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(89,10-89,46); use=ReifyTestTSST.fsti(148,11-148,14)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x2)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x4))
...
;; def=ReifyTestTSST.fsti(104,53-104,60); use=ReifyTestTSST.fsti(148,11-148,14)
(= @x2
@x3)


;; def=ReifyTestTSST.fsti(105,25-105,32); use=ReifyTestTSST.fsti(148,11-148,14)
(= @x4
@x3)
)

;; def=Prims.fst(459,77-459,89); use=ReifyTestTSST.fsti(147,88-170,4)
(and 
;; def=Prims.fst(680,18-680,24); use=ReifyTestTSST.fsti(150,35-150,53)
(or label_5

;; def=Prims.fst(680,18-680,24); use=ReifyTestTSST.fsti(150,35-150,53)
(>= (BoxInt_proj_0 (Prims.op_Addition (ReifyTestTSST.get_state ReifyTestTSST.state
@x3)
(BoxInt 1)))
(BoxInt_proj_0 (BoxInt 0)))
)
...
;; def=Prims.fst(356,2-356,58); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@x5 Term))
 (! (implies (and (HasType @x5
ReifyTestTSST.state)

;; def=Prims.fst(356,26-356,41); use=ReifyTestTSST.fsti(147,88-170,4)
(= @x5
(Prims.op_Addition (ReifyTestTSST.get_state ReifyTestTSST.state
@x3)
(BoxInt 1)))
)

;; def=Prims.fst(451,66-451,102); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@x6 Term))
 (! (implies (and (HasType @x6
Tm_type)

;; def=Prims.fst(642,13-642,14); use=ReifyTestTSST.fsti(147,88-170,4)
(= (ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
@x3))
(Prims.op_Addition (ReifyTestTSST.get_state ReifyTestTSST.state
...
;; def=Prims.fst(441,36-441,97); use=ReifyTestTSST.fsti(150,2-150,8)
(forall ((@x7 Term))
 (! (implies (and (HasType @x7
Prims.unit)

;; def=ReifyTestTSST.fsti(150,9-150,54); use=ReifyTestTSST.fsti(150,2-150,8)
(Valid 
;; def=ReifyTestTSST.fsti(150,9-150,54); use=ReifyTestTSST.fsti(150,2-150,8)
(ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
...
;; def=Prims.fst(459,77-459,89); use=ReifyTestTSST.fsti(147,88-170,4)
(and 
;; def=Prims.fst(680,18-680,24); use=ReifyTestTSST.fsti(152,14-152,32)
(or label_6
...
;; def=ReifyTestTSST.fsti(85,30-92,14); use=ReifyTestTSST.fsti(152,10-152,13)
(and 
;; def=ReifyTestTSST.fsti(109,27-109,53); use=ReifyTestTSST.fsti(152,10-152,13)
(or label_7
...
;; def=ReifyTestTSST.fsti(85,40-92,14); use=ReifyTestTSST.fsti(152,10-152,13)
...
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(152,10-152,13)
(Valid 
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(152,10-152,13)
(ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(88,7-89,47); use=ReifyTestTSST.fsti(152,10-152,13)
(or 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(152,10-152,13)
(Valid 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(152,10-152,13)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(89,10-89,46); use=ReifyTestTSST.fsti(152,10-152,13)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x4)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x10))
...
;; def=ReifyTestTSST.fsti(110,25-110,42); use=ReifyTestTSST.fsti(152,10-152,13)
(= (ReifyTestTSST.get_state ReifyTestTSST.state
@x10)
(Prims.op_Addition (ReifyTestTSST.get_state ReifyTestTSST.state
@x3)
...
;; def=ReifyTestTSST.fsti(111,11-111,59); use=ReifyTestTSST.fsti(152,10-152,13)
(Valid 
;; def=ReifyTestTSST.fsti(111,11-111,59); use=ReifyTestTSST.fsti(152,10-152,13)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=Prims.fst(459,77-459,89); use=ReifyTestTSST.fsti(147,88-170,4)
(and 
;; def=ReifyTestTSST.fsti(114,31-114,49); use=ReifyTestTSST.fsti(154,28-154,33)
(or label_8
...
;; def=ReifyTestTSST.fsti(114,13-154,33); use=ReifyTestTSST.fsti(147,88-170,4)
(= Tm_abs_e528333b9d5eec3dd11db629fcea7508
@x11)
)

;; def=ReifyTestTSST.fsti(85,30-92,14); use=ReifyTestTSST.fsti(154,10-154,17)
(and 
;; def=ReifyTestTSST.fsti(154,28-154,33); use=ReifyTestTSST.fsti(154,10-154,17)
(or label_9

;; def=ReifyTestTSST.fsti(154,28-154,33); use=ReifyTestTSST.fsti(154,10-154,17)
(> (BoxInt_proj_0 (ReifyTestTSST.get_state ReifyTestTSST.state
@x10))
(BoxInt_proj_0 (BoxInt 0)))
)


;; def=ReifyTestTSST.fsti(85,40-92,14); use=ReifyTestTSST.fsti(154,10-154,17)
...
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(154,10-154,17)
(Valid 
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(154,10-154,17)
(ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(88,7-89,47); use=ReifyTestTSST.fsti(154,10-154,17)
(or 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(154,10-154,17)
(Valid 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(154,10-154,17)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(89,10-89,46); use=ReifyTestTSST.fsti(154,10-154,17)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x10)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x13))
...
;; def=ReifyTestTSST.fsti(116,25-116,53); use=ReifyTestTSST.fsti(154,10-154,17)
(= (ReifyTestTSST.get_state ReifyTestTSST.state
@x10)
(ReifyTestTSST.get_state ReifyTestTSST.state
@x13))
...
;; def=ReifyTestTSST.fsti(117,18-117,54); use=ReifyTestTSST.fsti(154,10-154,17)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x10)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x13))
...
;; def=ReifyTestTSST.fsti(118,11-118,41); use=ReifyTestTSST.fsti(154,10-154,17)
(Valid 
;; def=ReifyTestTSST.fsti(118,11-118,41); use=ReifyTestTSST.fsti(154,10-154,17)
(ReifyTestTSST.witnessed (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x13)
Tm_abs_e528333b9d5eec3dd11db629fcea7508)
)
)

;; def=ReifyTestTSST.fsti(85,40-92,14); use=ReifyTestTSST.fsti(156,11-156,14)
...
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(156,11-156,14)
(Valid 
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(156,11-156,14)
(ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(88,7-89,47); use=ReifyTestTSST.fsti(156,11-156,14)
(or 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(156,11-156,14)
(Valid 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(156,11-156,14)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(89,10-89,46); use=ReifyTestTSST.fsti(156,11-156,14)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x13)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x15))
...
;; def=ReifyTestTSST.fsti(104,53-104,60); use=ReifyTestTSST.fsti(156,11-156,14)
(= @x13
@x14)


;; def=ReifyTestTSST.fsti(105,25-105,32); use=ReifyTestTSST.fsti(156,11-156,14)
(= @x15
@x14)
)
...
;; def=Prims.fst(459,77-459,89); use=ReifyTestTSST.fsti(147,88-170,4)
(and 
;; def=dummy(0,0-0,0); use=ReifyTestTSST.fsti(156,20-170,4)
...
;; def=dummy(0,0-0,0); use=ReifyTestTSST.fsti(158,26-158,32)
(forall ((@x18 Term))
 (! (implies (and (HasType @x18
ReifyTestTSST.timestamp)

;; def=ReifyTestTSST.fsti(123,29-124,34); use=ReifyTestTSST.fsti(158,26-158,32)
(or 
;; def=ReifyTestTSST.fsti(123,30-123,62); use=ReifyTestTSST.fsti(158,26-158,32)
(Valid 
;; def=ReifyTestTSST.fsti(123,30-123,62); use=ReifyTestTSST.fsti(158,26-158,32)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x18)
...
;; def=ReifyTestTSST.fsti(124,11-124,33); use=ReifyTestTSST.fsti(158,26-158,32)
(= @x18
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x17))
)
...
;; def=ReifyTestTSST.fsti(97,48-97,66); use=ReifyTestTSST.fsti(158,26-158,32)
(or label_10
...
;; def=dummy(0,0-0,0); use=ReifyTestTSST.fsti(147,88-170,4)
(forall ((@x16 Term))
 (! (implies (HasType @x16
Prims.unit)

;; def=FStar.Pervasives.fsti(484,2-484,63); use=ReifyTestTSST.fsti(147,88-170,4)
...
;; def=ReifyTestTSST.fsti(122,27-125,46); use=ReifyTestTSST.fsti(158,26-158,32)
(exists ((@x19 Term))
 (! (and (HasType @x19
ReifyTestTSST.timestamp)
...
;; def=ReifyTestTSST.fsti(125,32-125,46); use=ReifyTestTSST.fsti(158,26-158,32)
(Valid 
;; def=ReifyTestTSST.fsti(125,32-125,46); use=ReifyTestTSST.fsti(158,26-158,32)
(ReifyTestTSST.witnessed @x19
Tm_abs_e528333b9d5eec3dd11db629fcea7508)
)
)
...
;; def=ReifyTestTSST.fsti(85,40-92,14); use=ReifyTestTSST.fsti(158,26-158,49)
...
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(158,26-158,49)
(or label_14
...
;; def=ReifyTestTSST.fsti(88,7-89,47); use=ReifyTestTSST.fsti(158,26-158,49)
(or label_15

;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(158,26-158,49)
(Valid 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(158,26-158,49)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(89,10-89,46); use=ReifyTestTSST.fsti(158,26-158,49)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x18)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x20))
...
;; def=ReifyTestTSST.fsti(126,25-126,53); use=ReifyTestTSST.fsti(158,26-158,32)
(or label_16
...
;; def=ReifyTestTSST.fsti(127,11-127,47); use=ReifyTestTSST.fsti(158,26-158,32)
(or label_17
...
;; def=ReifyTestTSST.fsti(158,43-158,48); use=ReifyTestTSST.fsti(158,26-158,32)
(or label_18
...
;; def=ReifyTestTSST.fsti(92,7-92,13); use=ReifyTestTSST.fsti(158,26-158,49)
(Valid 
;; def=ReifyTestTSST.fsti(92,7-92,13); use=ReifyTestTSST.fsti(158,26-158,49)
(ApplyTT (ApplyTT @x17
@x19)
...
;; def=Prims.fst(459,77-459,89); use=ReifyTestTSST.fsti(158,26-158,49)
(and 
;; def=ReifyTestTSST.fsti(121,31-121,49); use=ReifyTestTSST.fsti(158,43-158,48)
(or label_19
...
;; def=Prims.fst(451,66-451,102); use=ReifyTestTSST.fsti(158,26-158,49)
(forall ((@x19 Term))
 (! (implies (and (HasType @x19
Tm_refine_8c7a59d2aae063ddae7f7f455c88be60)

;; def=ReifyTestTSST.fsti(121,13-158,48); use=ReifyTestTSST.fsti(158,26-158,49)
(= Tm_abs_e528333b9d5eec3dd11db629fcea7508
@x19)
)

;; def=ReifyTestTSST.fsti(85,30-92,14); use=ReifyTestTSST.fsti(158,26-158,32)
(and 
;; def=ReifyTestTSST.fsti(122,27-125,46); use=ReifyTestTSST.fsti(158,26-158,32)
(or label_20
...
;; def=ReifyTestTSST.fsti(85,40-92,14); use=ReifyTestTSST.fsti(158,26-158,32)
...
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(158,26-158,32)
(Valid 
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(158,26-158,32)
(ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(88,7-89,47); use=ReifyTestTSST.fsti(158,26-158,32)
(or 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(158,26-158,32)
(Valid 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(158,26-158,32)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(89,10-89,46); use=ReifyTestTSST.fsti(158,26-158,32)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x18)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x21))
...
;; def=ReifyTestTSST.fsti(92,7-92,13); use=ReifyTestTSST.fsti(158,26-158,32)
(Valid 
;; def=ReifyTestTSST.fsti(92,7-92,13); use=ReifyTestTSST.fsti(158,26-158,32)
(ApplyTT (ApplyTT @x17
@x20)
...
(forall ((@x16 Term))
 (! (implies (HasType @x16
Non_total_Tm_arrow_d98eef1c3ea7562ff73800fa6a004c34)

;; def=Prims.fst(459,77-459,89); use=ReifyTestTSST.fsti(147,88-170,4)
(and 
;; def=dummy(0,0-0,0); use=ReifyTestTSST.fsti(158,52-170,4)
(forall ((@x17 Term))
 (! (implies (HasType @x17
(ReifyTestTSST.timestamped_state ReifyTestTSST.state))

;; def=Prims.fst(459,77-459,89); use=ReifyTestTSST.fsti(160,10-160,16)
(and 
;; def=dummy(0,0-0,0); use=ReifyTestTSST.fsti(160,10-160,16)
(forall ((@x18 Term))
 (! (implies (and (HasType @x18
ReifyTestTSST.timestamp)
...
;; def=ReifyTestTSST.fsti(97,48-97,66); use=ReifyTestTSST.fsti(160,10-160,16)
(or label_21
...
;; def=Prims.fst(451,66-451,102); use=ReifyTestTSST.fsti(160,10-160,16)
(forall ((@x18 Term))
 (! (implies (and (HasType @x18
Tm_arrow_b519a73ba534f2602101b33df8a6b55c)

;; def=Prims.fst(509,26-509,27); use=ReifyTestTSST.fsti(160,10-160,16)
(= (Tm_abs_5491e1e6dca087f89a943a2cf569d3bc @x17)
@x18)
)
...
;; def=Prims.fst(439,23-439,26); use=ReifyTestTSST.fsti(160,10-160,16)
(= (Prims.l_Exists ReifyTestTSST.timestamp
(Tm_abs_5491e1e6dca087f89a943a2cf569d3bc @x17))
@x19)
)
...
;; def=dummy(0,0-0,0); use=ReifyTestTSST.fsti(160,35-163,54)
(forall ((@x18 Term))
 (! (implies (and (HasType @x18
ReifyTestTSST.timestamp)

;; def=ReifyTestTSST.fsti(161,22-162,34); use=ReifyTestTSST.fsti(161,22-162,34)
(or 
;; def=ReifyTestTSST.fsti(161,23-161,55); use=ReifyTestTSST.fsti(161,23-161,55)
(Valid 
;; def=ReifyTestTSST.fsti(161,23-161,55); use=ReifyTestTSST.fsti(161,23-161,55)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x18)
...
;; def=ReifyTestTSST.fsti(162,11-162,33); use=ReifyTestTSST.fsti(162,11-162,33)
(= @x18
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x17))
)
...
;; def=ReifyTestTSST.fsti(97,48-97,66); use=ReifyTestTSST.fsti(163,48-163,53)
(or label_23
...
;; def=ReifyTestTSST.fsti(134,6-163,54); use=ReifyTestTSST.fsti(147,88-170,4)
(= Tm_abs_87897e1227d4c10418dfe7d54680feb0
@x17)
)

;; def=Prims.fst(459,77-459,89); use=ReifyTestTSST.fsti(147,88-170,4)
(and 
;; def=dummy(0,0-0,0); use=ReifyTestTSST.fsti(164,29-164,30)
...
;; def=ReifyTestTSST.fsti(160,35-163,54); use=ReifyTestTSST.fsti(164,29-164,30)
(exists ((@x21 Term))
 (! (and (HasType @x21
ReifyTestTSST.timestamp)

;; def=ReifyTestTSST.fsti(161,22-162,34); use=ReifyTestTSST.fsti(164,29-164,30)
(or 
;; def=ReifyTestTSST.fsti(161,23-161,55); use=ReifyTestTSST.fsti(164,29-164,30)
(Valid 
;; def=ReifyTestTSST.fsti(161,23-161,55); use=ReifyTestTSST.fsti(164,29-164,30)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x21)
...
;; def=ReifyTestTSST.fsti(162,11-162,33); use=ReifyTestTSST.fsti(164,29-164,30)
(= @x21
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x20))
)
...
;; def=ReifyTestTSST.fsti(163,25-163,54); use=ReifyTestTSST.fsti(164,29-164,30)
(Valid 
;; def=ReifyTestTSST.fsti(163,25-163,54); use=ReifyTestTSST.fsti(164,29-164,30)
(ReifyTestTSST.witnessed @x21
Tm_abs_e528333b9d5eec3dd11db629fcea7508)
...
;; def=ReifyTestTSST.fsti(85,40-92,14); use=ReifyTestTSST.fsti(164,29-164,30)
...
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(164,29-164,30)
(Valid 
;; def=ReifyTestTSST.fsti(87,7-87,46); use=ReifyTestTSST.fsti(164,29-164,30)
(ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(88,7-89,47); use=ReifyTestTSST.fsti(164,29-164,30)
(or 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(164,29-164,30)
(Valid 
;; def=ReifyTestTSST.fsti(88,8-88,56); use=ReifyTestTSST.fsti(164,29-164,30)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(89,10-89,46); use=ReifyTestTSST.fsti(164,29-164,30)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x20)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x22))
...
;; def=ReifyTestTSST.fsti(92,7-92,13); use=ReifyTestTSST.fsti(164,29-164,30)
(Valid 
;; def=ReifyTestTSST.fsti(92,7-92,13); use=ReifyTestTSST.fsti(164,29-164,30)
(ApplyTT (ApplyTT @x19
@x21)
...
;; def=ReifyTestTSST.fsti(85,30-92,14); use=ReifyTestTSST.fsti(164,29-164,30)
(and 
;; def=ReifyTestTSST.fsti(122,27-125,46); use=ReifyTestTSST.fsti(164,29-164,30)
(or label_24
...
;; def=ReifyTestTSST.fsti(123,29-124,34); use=ReifyTestTSST.fsti(164,29-164,30)
(or 
;; def=ReifyTestTSST.fsti(123,30-123,62); use=ReifyTestTSST.fsti(164,29-164,30)
(Valid 
;; def=ReifyTestTSST.fsti(123,30-123,62); use=ReifyTestTSST.fsti(164,29-164,30)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x21)
...
;; def=ReifyTestTSST.fsti(124,11-124,33); use=ReifyTestTSST.fsti(164,29-164,30)
(= @x21
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x20))
)
...
;; def=ReifyTestTSST.fsti(125,32-125,46); use=ReifyTestTSST.fsti(164,29-164,30)
(Valid 
;; def=ReifyTestTSST.fsti(125,32-125,46); use=ReifyTestTSST.fsti(164,29-164,30)
(ReifyTestTSST.witnessed @x21
Tm_abs_e528333b9d5eec3dd11db629fcea7508)
...
;; def=ReifyTestTSST.fsti(126,25-126,53); use=ReifyTestTSST.fsti(164,29-164,30)
(= (ReifyTestTSST.get_state ReifyTestTSST.state
@x20)
(ReifyTestTSST.get_state ReifyTestTSST.state
@x22))
...
;; def=ReifyTestTSST.fsti(127,11-127,47); use=ReifyTestTSST.fsti(164,29-164,30)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x20)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x22))
...
;; def=ReifyTestTSST.fsti(158,43-158,48); use=ReifyTestTSST.fsti(164,29-164,30)
(> (BoxInt_proj_0 (ReifyTestTSST.get_state ReifyTestTSST.state
@x22))
(BoxInt_proj_0 (BoxInt 0)))
)
...
;; def=ReifyTestTSST.fsti(160,6-164,30); use=ReifyTestTSST.fsti(160,6-164,30)
(= (ApplyTT (ApplyTT (ApplyTT (ApplyTT ReifyTestTSST.reify_@tok
Prims.unit)
Tm_abs_87897e1227d4c10418dfe7d54680feb0)
Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd)
@x16)
@x19)
)

;; def=Prims.fst(441,29-441,97); use=ReifyTestTSST.fsti(166,10-166,11)
(and 
;; def=ReifyTestTSST.fsti(160,35-163,54); use=ReifyTestTSST.fsti(166,10-166,11)
(or label_26
...
;; def=ReifyTestTSST.fsti(161,22-162,34); use=ReifyTestTSST.fsti(166,10-166,11)
(or 
;; def=ReifyTestTSST.fsti(161,23-161,55); use=ReifyTestTSST.fsti(166,10-166,11)
(Valid 
;; def=ReifyTestTSST.fsti(161,23-161,55); use=ReifyTestTSST.fsti(166,10-166,11)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
@x20)
...
;; def=ReifyTestTSST.fsti(162,11-162,33); use=ReifyTestTSST.fsti(166,10-166,11)
(= @x20
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x14))
)
...
;; def=ReifyTestTSST.fsti(163,25-163,54); use=ReifyTestTSST.fsti(166,10-166,11)
(Valid 
;; def=ReifyTestTSST.fsti(163,25-163,54); use=ReifyTestTSST.fsti(166,10-166,11)
(ReifyTestTSST.witnessed @x20
Tm_abs_e528333b9d5eec3dd11db629fcea7508)
...
;; def=Prims.fst(441,36-441,97); use=ReifyTestTSST.fsti(166,10-166,11)
(forall ((@x20 Term))
 (! (implies (and (HasType @x20
(FStar.Pervasives.Native.tuple2 Prims.unit
(ReifyTestTSST.timestamped_state ReifyTestTSST.state)))

;; def=ReifyTestTSST.fsti(139,52-140,71); use=ReifyTestTSST.fsti(166,10-166,11)
(or 
;; def=ReifyTestTSST.fsti(139,53-139,108); use=ReifyTestTSST.fsti(166,10-166,11)
(Valid 
;; def=ReifyTestTSST.fsti(139,53-139,108); use=ReifyTestTSST.fsti(166,10-166,11)
(ApplyTT (ApplyTT (ReifyTestTSST.older_than Dummy_value)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(140,27-140,70); use=ReifyTestTSST.fsti(166,10-166,11)
(= (ReifyTestTSST.get_timestamp ReifyTestTSST.state
@x14)
(ReifyTestTSST.get_timestamp ReifyTestTSST.state
(FStar.Pervasives.Native.snd Prims.unit
...
;; def=ReifyTestTSST.fsti(141,24-141,70); use=ReifyTestTSST.fsti(166,10-166,11)
(Valid 
;; def=ReifyTestTSST.fsti(141,24-141,70); use=ReifyTestTSST.fsti(166,10-166,11)
(ApplyTT (ApplyTT (ReifyTestTSST.state_rel Dummy_value)
(ReifyTestTSST.get_state ReifyTestTSST.state
...
;; def=ReifyTestTSST.fsti(166,6-166,14); use=ReifyTestTSST.fsti(166,6-166,14)
(= (ReifyTestTSST.reify_ Prims.unit
Tm_abs_87897e1227d4c10418dfe7d54680feb0
Tm_abs_bfa972c71b356dc8f159eb4bc4914bfd
@x16
@x14)
...
;; def=ReifyTestTSST.fsti(75,117-75,122); use=ReifyTestTSST.fsti(147,88-170,4)
(Valid 
;; def=ReifyTestTSST.fsti(75,117-75,122); use=ReifyTestTSST.fsti(147,88-170,4)
(ApplyTT (ApplyTT @x1
@x21)
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (ReifyTestTSST.reify_recall_test, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @MaxIFuel_assumption, @query, b2t_def, equation_FStar.Preorder.stable, equation_Prims.nat, equation_ReifyTestTSST.get_timestamp, equation_ReifyTestTSST.older_than, equation_ReifyTestTSST.state, equation_ReifyTestTSST.state_rel, equation_ReifyTestTSST.timestamp, equation_ReifyTestTSST.witnessed, int_inversion, interpretation_Tm_abs_577bb0504d99bec50af9e2beb88b9dcb, interpretation_Tm_abs_e528333b9d5eec3dd11db629fcea7508, primitive_Prims.op_Addition, primitive_Prims.op_GreaterThan, primitive_Prims.op_LessThan, primitive_Prims.op_LessThanOrEqual, projection_inverse_BoxBool_proj_0, projection_inverse_BoxInt_proj_0, refinement_interpretation_Tm_refine_542f9d4f129664613f2483a6c88bc7c2, typing_Prims.nat, typing_ReifyTestTSST.get_state, typing_ReifyTestTSST.get_timestamp
