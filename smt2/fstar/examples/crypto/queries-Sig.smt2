...
(declare-fun Sig.text () Term)

; </end encoding let text>


; encoding sigelt let sigsize


; <Start encoding let sigsize>

(declare-fun Sig.sigsize (Dummy_sort) Term)

; </end encoding let sigsize>


; encoding sigelt let sksize


; <Start encoding let sksize>

(declare-fun Sig.sksize (Dummy_sort) Term)

; </end encoding let sksize>


; encoding sigelt let pksize


; <Start encoding let pksize>

(declare-fun Sig.pksize (Dummy_sort) Term)

; </end encoding let pksize>
...
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.sigsize; Namespace Sig
(assert (! 
;; def=Sig.fst(26,4-26,11); use=Sig.fst(26,4-26,11)
(forall ((@u0 Dummy_sort))
 (! (HasType (Sig.sigsize @u0)
Prims.int)
 

:pattern ((Sig.sigsize @u0))
:qid typing_Sig.sigsize))

:named typing_Sig.sigsize))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Prims.unit; Namespace Prims
...
:qid equation_Sig.sigsize))

:named equation_Sig.sigsize))
...
; Starting query at Sig.fst(29,5-29,10)

(declare-fun label_1 () Bool)

; Encoding query formula : Sig.sigsize >= 0


; Context: While encoding a query
; While checking for top-level effects
; While typechecking the top-level declaration `let sig_t`
...
;;; Fact-ids: 
(assert (! (not 
;; def=Prims.fst(680,18-680,24); use=Sig.fst(29,20-29,27)
(or label_1

;; def=Prims.fst(680,18-680,24); use=Sig.fst(29,20-29,27)
(>= (BoxInt_proj_0 (Sig.sigsize Dummy_value))
(BoxInt_proj_0 (BoxInt 0)))
)
)
:named @query))
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (Sig.sig_t, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @query, equation_Sig.sigsize, projection_inverse_BoxInt_proj_0
...
; encoding sigelt let sig_t


; <Start encoding let sig_t>

(declare-fun Sig.sig_t () Term)

; </end encoding let sig_t>

;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.pksize; Namespace Sig
(assert (! 
;; def=Sig.fst(28,4-28,10); use=Sig.fst(28,4-28,10)
(forall ((@u0 Dummy_sort))
 (! (HasType (Sig.pksize @u0)
Prims.int)
 

:pattern ((Sig.pksize @u0))
:qid typing_Sig.pksize))

:named typing_Sig.pksize))
;;;;;;;;;;;;;;;;Equation for Sig.pksize
;;; Fact-ids: Name Sig.pksize; Namespace Sig
(assert (! 
;; def=Sig.fst(28,4-28,10); use=Sig.fst(28,4-28,10)
(forall ((@u0 Dummy_sort))
...
:qid equation_Sig.pksize))

:named equation_Sig.pksize))
(push) ;; push{2

; Starting query at Sig.fst(30,5-30,7)

(declare-fun label_1 () Bool)

; Encoding query formula : Sig.pksize >= 0


; Context: While encoding a query
; While checking for top-level effects
; While typechecking the top-level declaration `let pk`
...
;;; Fact-ids: 
(assert (! (not 
;; def=Prims.fst(680,18-680,24); use=Sig.fst(30,19-30,25)
(or label_1

;; def=Prims.fst(680,18-680,24); use=Sig.fst(30,19-30,25)
(>= (BoxInt_proj_0 (Sig.pksize Dummy_value))
(BoxInt_proj_0 (BoxInt 0)))
)
)
:named @query))
...
; QUERY ID: (Sig.pk, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @query, equation_Sig.pksize, projection_inverse_BoxInt_proj_0
...
; encoding sigelt let pk


; <Start encoding let pk>

(declare-fun Sig.pk () Term)

; </end encoding let pk>

;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.sksize; Namespace Sig
(assert (! 
;; def=Sig.fst(27,4-27,10); use=Sig.fst(27,4-27,10)
(forall ((@u0 Dummy_sort))
 (! (HasType (Sig.sksize @u0)
Prims.int)
 

:pattern ((Sig.sksize @u0))
:qid typing_Sig.sksize))

:named typing_Sig.sksize))
;;;;;;;;;;;;;;;;Equation for Sig.sksize
;;; Fact-ids: Name Sig.sksize; Namespace Sig
(assert (! 
;; def=Sig.fst(27,4-27,10); use=Sig.fst(27,4-27,10)
(forall ((@u0 Dummy_sort))
...
:qid equation_Sig.sksize))

:named equation_Sig.sksize))
(push) ;; push{2

; Starting query at Sig.fst(31,5-31,7)

(declare-fun label_1 () Bool)

; Encoding query formula : Sig.sksize >= 0


; Context: While encoding a query
; While checking for top-level effects
; While typechecking the top-level declaration `let sk`
...
;;; Fact-ids: 
(assert (! (not 
;; def=Prims.fst(680,18-680,24); use=Sig.fst(31,19-31,25)
(or label_1

;; def=Prims.fst(680,18-680,24); use=Sig.fst(31,19-31,25)
(>= (BoxInt_proj_0 (Sig.sksize Dummy_value))
(BoxInt_proj_0 (BoxInt 0)))
)
)
:named @query))
...
; QUERY ID: (Sig.sk, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @query, equation_Sig.sksize, projection_inverse_BoxInt_proj_0
...
; encoding sigelt let sk


; <Start encoding let sk>

(declare-fun Sig.sk () Term)

; </end encoding let sk>


; encoding sigelt val Sig.fdh_rsa


; <Start encoding val Sig.fdh_rsa>

(declare-fun Sig.fdh_rsa (Term Term) Term)
;;;;;;;;;;;;;;;;_: sk -> _: text -> sig_t
(declare-fun Tm_arrow_5a3207f42d4556c932c05c8220ea6f3a () Term)
(declare-fun Sig.fdh_rsa@tok () Term)

; </end encoding val Sig.fdh_rsa>


; encoding sigelt val Sig.key_prop


; <Start encoding val Sig.key_prop>

(declare-fun Sig.key_prop (Term Term) Term)
;;;;;;;;;;;;;;;;_: pk -> _: text -> Type
(declare-fun Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0 () Term)
(declare-fun Sig.key_prop@tok () Term)

; </end encoding val Sig.key_prop>


; encoding sigelt let prop_pk


; <Start encoding let prop_pk>

;;;;;;;;;;;;;;;;_: text -> Type
(declare-fun Tm_arrow_f4662f20b0d5b0893ed3e22a3522108b () Term)
(declare-fun Sig.prop_pk (Term) Term)

;;;;;;;;;;;;;;;;p: (_: text -> Type) -> Type
(declare-fun Tm_arrow_6469442a14a6fec88fe011ee869dea24 () Term)
(declare-fun Sig.prop_pk@tok () Term)

(declare-fun Tm_refine_acd033073b8ee0ad9837fe1f6b6cc105 (Term) Term)

; </end encoding let prop_pk>


; encoding sigelt val Sig.sk_to_pk


; <Start encoding val Sig.sk_to_pk>

(declare-fun Sig.sk_to_pk (Term) Term)
;;;;;;;;;;;;;;;;_: sk -> pk
(declare-fun Tm_arrow_1c781f458073e421399ec10835567c0b () Term)
(declare-fun Sig.sk_to_pk@tok () Term)

; </end encoding val Sig.sk_to_pk>


; encoding sigelt let pk_sk


; <Start encoding let pk_sk>

(declare-fun Sig.pk_sk (Term) Term)
;;;;;;;;;;;;;;;;p: pk -> Type
(declare-fun Tm_arrow_065b8451d1019bee9a07b8f1c28e50cc () Term)
(declare-fun Sig.pk_sk@tok () Term)
(declare-fun Tm_refine_f674e363187b68a0bd32a671b638c73b (Term) Term)

; </end encoding let pk_sk>


; encoding sigelt val Sig.keygen


; <Start encoding val Sig.keygen>


(declare-fun Sig.keygen (Term) Term)

;;;;;;;;;;;;;;;;k: prop_pk p -> Prims.GTot Type
(declare-fun Tm_arrow_94d45525a7e1d15c42591b6e16c19eec (Term) Term)
(declare-fun Tm_abs_2462277a97e470f0bdec4a2a1cd4f05a (Term) Term)
;;;;;;;;;;;;;;;;p: (_: text -> Type) -> k: prop_pk p & pk_sk k
(declare-fun Tm_arrow_af2804f32bf8ef471876157cd3e1e135 () Term)
(declare-fun Sig.keygen@tok () Term)



; </end encoding val Sig.keygen>
...
; encoding sigelt type Sig.entry


; <Start encoding type Sig.entry>

;;;;;;;;;;;;;;;;Constructor
(declare-fun Sig.entry () Term)
;;;;;;;;;;;;;;;;Constructor
(declare-fun Sig.Entry (Term Term Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun Sig.Entry_k (Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun Sig.Entry_t (Term) Term)
;;;;;;;;;;;;;;;;Projector
(declare-fun Sig.Entry_m (Term) Term)
;;;;;;;;;;;;;;;;data constructor proxy: Sig.Entry
(declare-fun Sig.Entry@tok () Term)
(declare-fun Tm_refine_f7e217a709bf72990519e54b2cca90dc (Term) Term)

;;;;;;;;;;;;;;;;k: pk -> t: text{key_prop k t} -> m: sig_t -> entry
(declare-fun Tm_arrow_278042d6dbf4800c8bee82418a5d252c () Term)

; <start constructor Sig.entry>

;;;;;;;;;;;;;;;;Discriminator definition
(define-fun is-Sig.entry ((__@x0 Term)) Bool
 (and (= (Term_constr_id __@x0)
147)
(= __@x0
Sig.entry)))

; </end constructor Sig.entry>
...
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.text; Namespace Sig
(assert (! (HasType Sig.text
Tm_type)
:named typing_Sig.text))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.sig_t; Namespace Sig
(assert (! (HasType Sig.sig_t
Tm_type)
:named typing_Sig.sig_t))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.pk; Namespace Sig
(assert (! (HasType Sig.pk
Tm_type)
:named typing_Sig.pk))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.key_prop; Namespace Sig
(assert (! 
;; def=Sig.fst(35,16-35,24); use=Sig.fst(35,16-35,24)
(forall ((@x0 Term) (@x1 Term))
 (! (implies (and (HasType @x0
Sig.pk)
(HasType @x1
Sig.text))
(HasType (Sig.key_prop @x0
@x1)
Tm_type))
 

:pattern ((Sig.key_prop @x0
@x1))
:qid typing_Sig.key_prop))

:named typing_Sig.key_prop))
;;;;;;;;;;;;;;;;free var typing
...
;; def=Sig.fst(35,16-35,24); use=Sig.fst(35,16-35,24)
(forall ((@x0 Term) (@x1 Term))
 (! (= (ApplyTT (ApplyTT Sig.key_prop@tok
@x0)
@x1)
(Sig.key_prop @x0
@x1))
...
:pattern ((ApplyTT (ApplyTT Sig.key_prop@tok
@x0)
@x1))
:qid token_correspondence_Sig.key_prop))

:named token_correspondence_Sig.key_prop))
...
;;;;;;;;;;;;;;;;subterm ordering
;;; Fact-ids: Name Sig.entry; Namespace Sig; Name Sig.Entry; Namespace Sig
(assert (! 
;; def=Sig.fst(46,4-46,9); use=Sig.fst(46,4-46,9)
...
@x3)
Sig.entry))
:qid subterm_ordering_Sig.Entry))

:named subterm_ordering_Sig.Entry))
;;;;;;;;;;;;;;;;refinement kinding
;;; Fact-ids: Name Sig.entry; Namespace Sig; Name Sig.Entry; Namespace Sig
(assert (! 
;; def=Sig.fst(47,12-47,32); use=Sig.fst(47,12-47,32)
(forall ((@x0 Term))
 (! (HasType (Tm_refine_f7e217a709bf72990519e54b2cca90dc @x0)
Tm_type)
 

:pattern ((HasType (Tm_refine_f7e217a709bf72990519e54b2cca90dc @x0)
Tm_type))
:qid refinement_kinding_Tm_refine_f7e217a709bf72990519e54b2cca90dc))

:named refinement_kinding_Tm_refine_f7e217a709bf72990519e54b2cca90dc))
;;;;;;;;;;;;;;;;refinement kinding
;;; Fact-ids: Name FStar.UInt.uint_t; Namespace FStar.UInt
(assert (! 
...
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term))
 (! (iff (HasTypeFuel @u0
@x1
(Tm_refine_f7e217a709bf72990519e54b2cca90dc @x2))
(and (HasTypeFuel @u0
@x1
Sig.text)

;; def=Sig.fst(47,19-47,31); use=Sig.fst(47,19-47,31)
(Valid 
;; def=Sig.fst(47,19-47,31); use=Sig.fst(47,19-47,31)
(Sig.key_prop @x2
@x1)
...
@x1
(Tm_refine_f7e217a709bf72990519e54b2cca90dc @x2)))
:qid refinement_interpretation_Tm_refine_f7e217a709bf72990519e54b2cca90dc))

:named refinement_interpretation_Tm_refine_f7e217a709bf72990519e54b2cca90dc))
;;;;;;;;;;;;;;;;refinement_interpretation
;;; Fact-ids: Name FStar.UInt.uint_t; Namespace FStar.UInt
(assert (! 
;; def=FStar.UInt.fsti(54,22-54,37); use=FStar.UInt.fsti(54,22-54,37)
...
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (Sig.Entry_t (Sig.Entry @x0
@x1
@x2))
@x1)
 

:pattern ((Sig.Entry @x0
@x1
@x2))
:qid projection_inverse_Sig.Entry_t))

:named projection_inverse_Sig.Entry_t))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name Sig.entry; Namespace Sig; Name Sig.Entry; Namespace Sig
(assert (! 
;; def=Sig.fst(46,4-46,9); use=Sig.fst(46,4-46,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (Sig.Entry_m (Sig.Entry @x0
@x1
@x2))
@x2)
...
@x1
@x2))
:qid projection_inverse_Sig.Entry_m))

:named projection_inverse_Sig.Entry_m))
;;;;;;;;;;;;;;;;Projection inverse
;;; Fact-ids: Name Sig.entry; Namespace Sig; Name Sig.Entry; Namespace Sig
(assert (! 
;; def=Sig.fst(46,4-46,9); use=Sig.fst(46,4-46,9)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (Sig.Entry_k (Sig.Entry @x0
@x1
@x2))
@x0)
...
@x1
@x2))
:qid projection_inverse_Sig.Entry_k))

:named projection_inverse_Sig.Entry_k))
...
;;;;;;;;;;;;;;;;kinding_Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0
;;; Fact-ids: Name Sig.key_prop; Namespace Sig
(assert (! (HasType Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0
Tm_type)
:named kinding_Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0))
;;; Fact-ids: Name Sig.entry; Namespace Sig; Name Sig.Entry; Namespace Sig
(assert (! (HasType Sig.entry
Tm_type)
:named kinding_Sig.entry@tok))
;;;;;;;;;;;;;;;;haseq for Tm_refine_f7e217a709bf72990519e54b2cca90dc
;;; Fact-ids: Name Sig.entry; Namespace Sig; Name Sig.Entry; Namespace Sig
(assert (! 
;; def=Sig.fst(47,12-47,32); use=Sig.fst(47,12-47,32)
(forall ((@x0 Term))
 (! (iff (Valid (Prims.hasEq (Tm_refine_f7e217a709bf72990519e54b2cca90dc @x0)))
(Valid (Prims.hasEq Sig.text)))
 

:pattern ((Valid (Prims.hasEq (Tm_refine_f7e217a709bf72990519e54b2cca90dc @x0))))
:qid haseqTm_refine_f7e217a709bf72990519e54b2cca90dc))

:named haseqTm_refine_f7e217a709bf72990519e54b2cca90dc))
...
(assert (! (HasType Sig.text
Tm_type)
:named function_token_typing_Sig.text))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name Sig.sig_t; Namespace Sig
(assert (! (HasType Sig.sig_t
Tm_type)
:named function_token_typing_Sig.sig_t))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name Sig.pk; Namespace Sig
(assert (! (HasType Sig.pk
Tm_type)
:named function_token_typing_Sig.pk))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name Sig.key_prop; Namespace Sig
(assert (! 
;; def=Sig.fst(35,16-35,24); use=Sig.fst(35,16-35,24)
(forall ((@x0 Term))
 (! (and (NoHoist @x0
(HasType Sig.key_prop@tok
Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0))
...
:pattern ((Sig.key_prop @x1
@x2))
:qid function_token_typing_Sig.key_prop.1))
)
 

:pattern ((ApplyTT @x0
Sig.key_prop@tok))
:qid function_token_typing_Sig.key_prop))

:named function_token_typing_Sig.key_prop))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name Prims.pos; Namespace Prims
(assert (! (HasType Prims.pos
Tm_type)
...
;;; Fact-ids: Name Sig.entry; Namespace Sig; Name Sig.Entry; Namespace Sig
(assert (! 
;; def=Sig.fst(45,5-45,10); use=Sig.fst(45,5-45,10)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Sig.entry)
...
@x1
Sig.entry))
:qid fuel_guarded_inversion_Sig.entry))

:named fuel_guarded_inversion_Sig.entry))
...
;;;;;;;;;;;;;;;;Equation for Sig.text
;;; Fact-ids: Name Sig.text; Namespace Sig
(assert (! (= Sig.text
Platform.Bytes.bytes)
:named equation_Sig.text))
;;;;;;;;;;;;;;;;Equation for Sig.sig_t
;;; Fact-ids: Name Sig.sig_t; Namespace Sig
(assert (! (= Sig.sig_t
(Platform.Bytes.lbytes (Sig.sigsize Dummy_value)))
:named equation_Sig.sig_t))
;;;;;;;;;;;;;;;;Equation for Sig.pk
;;; Fact-ids: Name Sig.pk; Namespace Sig
(assert (! (= Sig.pk
(Platform.Bytes.lbytes (Sig.pksize Dummy_value)))
:named equation_Sig.pk))
...
(HasTypeFuel @u0
@x2
(Tm_refine_f7e217a709bf72990519e54b2cca90dc @x1))
(HasTypeFuel @u0
@x3
Sig.sig_t))
(HasTypeFuel @u0
...
@x3)
Sig.entry))
:qid data_typing_intro_Sig.Entry@tok))

:named data_typing_intro_Sig.Entry@tok))
...
@x3)
Sig.entry))
:qid data_elim_Sig.Entry))

:named data_elim_Sig.Entry))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name Sig.key_prop; Namespace Sig
(assert (! 
;; def=Sig.fst(35,16-35,24); use=Sig.fst(35,16-35,24)
(forall ((@x0 Term) (@x1 Term))
 (! (= 108
(Term_constr_id (Sig.key_prop @x0
@x1)))
 

:pattern ((Sig.key_prop @x0
@x1))
:qid constructor_distinct_Sig.key_prop))

:named constructor_distinct_Sig.key_prop))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name Sig.entry; Namespace Sig; Name Sig.Entry; Namespace Sig
(assert (! (= 147
(Term_constr_id Sig.entry))
:named constructor_distinct_Sig.entry))
;;;;;;;;;;;;;;;;Constructor distinct
;;; Fact-ids: Name Sig.entry; Namespace Sig; Name Sig.Entry; Namespace Sig
(assert (! 
;; def=Sig.fst(46,4-46,9); use=Sig.fst(46,4-46,9)
...
@x1
@x2))
:qid constructor_distinct_Sig.Entry))
...
@x0
Sig.entry))
:qid Sig_pretyping_ac28a9c0f1d901a3c5a7526fa93e2704))

:named Sig_pretyping_ac28a9c0f1d901a3c5a7526fa93e2704))
;;;;;;;;;;;;;;;;pretyping
;;; Fact-ids: Name Sig.key_prop; Namespace Sig
(assert (! 
;; def=Sig.fst(35,16-35,24); use=Sig.fst(35,16-35,24)
...
(Sig.key_prop @x2
@x3)))
:qid Sig_pretyping_2be4a4731ffe19c69c1700bea21364c9))

:named Sig_pretyping_2be4a4731ffe19c69c1700bea21364c9))
;;;;;;;;;;;;;;;;pre-typing for functions
;;; Fact-ids: Name Sig.key_prop; Namespace Sig
(assert (! 
;; def=Sig.fst(35,27-35,45); use=Sig.fst(35,27-35,45)
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0)
...
@x1
Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0))
:qid Sig_pre_typing_Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0))

:named Sig_pre_typing_Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0
;;; Fact-ids: Name Sig.key_prop; Namespace Sig
(assert (! 
;; def=Sig.fst(35,27-35,45); use=Sig.fst(35,27-35,45)
(forall ((@x0 Term))
...
@x1)
@x2))
:qid Sig_interpretation_Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0.1))
...
:pattern ((ApplyTT @x0
@x1))
:qid Sig_interpretation_Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0.2))
))
 

:pattern ((HasTypeZ @x0
Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0))
:qid Sig_interpretation_Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0))

:named Sig_interpretation_Tm_arrow_282d0a7a02d234fa440e0e3080ef51a0))
;;;;;;;;;;;;;;;;pretyping
;;; Fact-ids: Name FStar.UInt8.t; Namespace FStar.UInt8
(assert (! 
;; def=FStar.UInt8.fsti(50,8-50,9); use=FStar.UInt8.fsti(50,8-50,9)
...
;;;;;;;;;;;;;;;;kick_partial_app
;;; Fact-ids: Name Sig.prop_pk; Namespace Sig
(assert (! (Valid (ApplyTT __uu__PartialApp
Sig.key_prop@tok))
:named @kick_partial_app_a0ef75e86507efca478a49a5b5937441))
;;;;;;;;;;;;;;;;Fuel irrelevance
;;; Fact-ids: Name Prims.pow2; Namespace Prims
(assert (! 
;; def=Prims.fst(708,8-708,12); use=Prims.fst(708,8-708,12)
...
; Starting query at Sig.fst(45,0-49,17)
...
; Encoding query formula : Prims.hasEq Sig.entry ==>
; (forall (k: Sig.pk) (t: Sig.text{Sig.key_prop k t}) (m: Sig.sig_t).
;     Prims.hasEq Sig.pk /\ Prims.hasEq Sig.text /\ Prims.hasEq Sig.sig_t)


; Context: While encoding a query
; While typechecking the top-level declaration `type Sig.entry`
...
;;; Fact-ids: 
(assert (! (not (implies 
;; def=Sig.fst(45,0-49,17); use=Sig.fst(45,0-49,17)
(Valid 
;; def=Sig.fst(45,0-49,17); use=Sig.fst(45,0-49,17)
(Prims.hasEq Sig.entry)
)
...
Sig.pk)
(HasType @x1
(Tm_refine_f7e217a709bf72990519e54b2cca90dc @x0))
(HasType @x2
Sig.sig_t))

;; def=Sig.fst(45,0-49,17); use=Sig.fst(45,0-49,17)
(and 
;; def=Sig.fst(45,0-49,17); use=Sig.fst(46,14-46,16)
(or label_1
...
;; def=Sig.fst(47,14-47,18); use=Sig.fst(47,14-47,18)
(or label_2

;; def=Sig.fst(47,14-47,18); use=Sig.fst(47,14-47,18)
(Valid 
;; def=Sig.fst(47,14-47,18); use=Sig.fst(47,14-47,18)
(Prims.hasEq Sig.text)
)
)


;; def=Sig.fst(45,0-49,17); use=Sig.fst(48,14-48,19)
(or label_3

;; def=Sig.fst(45,0-49,17); use=Sig.fst(45,0-49,17)
(Valid 
;; def=Sig.fst(45,0-49,17); use=Sig.fst(45,0-49,17)
(Prims.hasEq Sig.sig_t)
)
)
)
)
...
(echo "Done!")
(pop) ;; 0}pop
; QUERY ID: (Sig.entry, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @query, assumption_Platform.Bytes.HasEq_bytes, equation_Platform.Bytes.lbytes, equation_Sig.pk, equation_Sig.pksize, equation_Sig.sig_t, equation_Sig.sigsize, equation_Sig.text, haseqTm_refine_b3982a6d174e1d047431ae85c35dc7d8
...
; encoding sigelt assume Sig.entry__uu___haseq


; <Start encoding assume Sig.entry__uu___haseq>


; </end encoding assume Sig.entry__uu___haseq>


; encoding sigelt val Sig.uu___is_Entry


; <Start encoding val Sig.uu___is_Entry>

(declare-fun Sig.uu___is_Entry (Term) Term)
;;;;;;;;;;;;;;;;projectee: entry -> Prims.bool
(declare-fun Tm_arrow_3892df11bf018e17133d2fef9397e3c1 () Term)
(declare-fun Sig.uu___is_Entry@tok () Term)

; </end encoding val Sig.uu___is_Entry>
...
; encoding sigelt val Sig.__proj__Entry__item__k


; <Start encoding val Sig.__proj__Entry__item__k>

(declare-fun Sig.__proj__Entry__item__k (Term) Term)
;;;;;;;;;;;;;;;;projectee: entry -> pk
(declare-fun Tm_arrow_9d529d02a62a29214ed32b9440411d48 () Term)
(declare-fun Sig.__proj__Entry__item__k@tok () Term)

; </end encoding val Sig.__proj__Entry__item__k>
...
; encoding sigelt val Sig.__proj__Entry__item__t


; <Start encoding val Sig.__proj__Entry__item__t>

(declare-fun Sig.__proj__Entry__item__t (Term) Term)
(declare-fun Tm_refine_74b0d1c3beeb7a29ae2a6424b6b75e81 (Term) Term)
;;;;;;;;;;;;;;;;projectee: entry -> t: text{key_prop projectee.k t}
(declare-fun Tm_arrow_828718ecd7ee3df535116e50954fa6bd () Term)
(declare-fun Sig.__proj__Entry__item__t@tok () Term)


; </end encoding val Sig.__proj__Entry__item__t>


; encoding sigelt let __proj__Entry__item__t


; <Skipped let __proj__Entry__item__t/>


; encoding sigelt val Sig.__proj__Entry__item__m


; <Start encoding val Sig.__proj__Entry__item__m>

(declare-fun Sig.__proj__Entry__item__m (Term) Term)
;;;;;;;;;;;;;;;;projectee: entry -> sig_t
(declare-fun Tm_arrow_1910ebfb5525fb3950faaa1abcde8d3b () Term)
(declare-fun Sig.__proj__Entry__item__m@tok () Term)

; </end encoding val Sig.__proj__Entry__item__m>


; encoding sigelt let __proj__Entry__item__m


; <Skipped let __proj__Entry__item__m/>


; encoding sigelt val Sig.log


; <Start encoding val Sig.log>

(declare-fun Sig.log (Dummy_sort) Term)

; </end encoding val Sig.log>


; encoding sigelt val Sig.sign


; <Skipped val Sig.sign/>
...
:named typing_Sig.text))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.sk_to_pk; Namespace Sig
(assert (! 
;; def=Sig.fst(38,11-38,19); use=Sig.fst(38,11-38,19)
(forall ((@x0 Term))
 (! (implies (HasType @x0
Sig.sk)
(HasType (Sig.sk_to_pk @x0)
Sig.pk))
 

:pattern ((Sig.sk_to_pk @x0))
:qid typing_Sig.sk_to_pk))

:named typing_Sig.sk_to_pk))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.sk; Namespace Sig
(assert (! (HasType Sig.sk
Tm_type)
:named typing_Sig.sk))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.sig_t; Namespace Sig
...
:named typing_Sig.sig_t))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.pk_sk; Namespace Sig
(assert (! 
;; def=Sig.fst(41,5-41,10); use=Sig.fst(41,5-41,10)
(forall ((@x0 Term))
 (! (implies (HasType @x0
Sig.pk)
(HasType (Sig.pk_sk @x0)
Tm_type))
 

:pattern ((Sig.pk_sk @x0))
:qid typing_Sig.pk_sk))

:named typing_Sig.pk_sk))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.pk; Namespace Sig
(assert (! (HasType Sig.pk
Tm_type)
:named typing_Sig.pk))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.log; Namespace Sig
(assert (! 
;; def=Sig.fst(51,11-51,14); use=Sig.fst(51,11-51,14)
(forall ((@u0 Dummy_sort))
 (! (HasType (Sig.log @u0)
(FStar.ST.ref (Prims.list Sig.entry)))
 

:pattern ((Sig.log @u0))
:qid typing_Sig.log))

:named typing_Sig.log))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.key_prop; Namespace Sig
(assert (! 
;; def=Sig.fst(35,16-35,24); use=Sig.fst(35,16-35,24)
...
:named typing_Sig.key_prop))
;;;;;;;;;;;;;;;;free var typing
;;; Fact-ids: Name Sig.fdh_rsa; Namespace Sig
(assert (! 
;; def=Sig.fst(33,11-33,18); use=Sig.fst(33,11-33,18)
(forall ((@x0 Term) (@x1 Term))
 (! (implies (and (HasType @x0
Sig.sk)
(HasType @x1
Sig.text))
(HasType (Sig.fdh_rsa @x0
@x1)
Sig.sig_t))
 

:pattern ((Sig.fdh_rsa @x0
@x1))
:qid typing_Sig.fdh_rsa))

:named typing_Sig.fdh_rsa))
;;;;;;;;;;;;;;;;free var typing
...
;;; Fact-ids: Name Sig.pk_sk; Namespace Sig
(assert (! 
;; def=Sig.fst(41,20-41,41); use=Sig.fst(41,20-41,41)
(forall ((@x0 Term))
 (! (HasType (Tm_refine_f674e363187b68a0bd32a671b638c73b @x0)
Tm_type)
 

:pattern ((HasType (Tm_refine_f674e363187b68a0bd32a671b638c73b @x0)
Tm_type))
:qid refinement_kinding_Tm_refine_f674e363187b68a0bd32a671b638c73b))

:named refinement_kinding_Tm_refine_f674e363187b68a0bd32a671b638c73b))
;;;;;;;;;;;;;;;;refinement kinding
;;; Fact-ids: Name FStar.UInt.uint_t; Namespace FStar.UInt
(assert (! 
;; def=FStar.UInt.fsti(54,22-54,37); use=FStar.UInt.fsti(54,22-54,37)
...
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term))
 (! (iff (HasTypeFuel @u0
@x1
(Tm_refine_f674e363187b68a0bd32a671b638c73b @x2))
(and (HasTypeFuel @u0
@x1
Sig.sk)

;; def=Sig.fst(41,25-41,40); use=Sig.fst(41,25-41,40)
(= (Sig.sk_to_pk @x1)
@x2)
))
...
@x1
(Tm_refine_f674e363187b68a0bd32a671b638c73b @x2)))
:qid refinement_interpretation_Tm_refine_f674e363187b68a0bd32a671b638c73b))

:named refinement_interpretation_Tm_refine_f674e363187b68a0bd32a671b638c73b))
;;;;;;;;;;;;;;;;refinement_interpretation
;;; Fact-ids: Name FStar.UInt.uint_t; Namespace FStar.UInt
(assert (! 
;; def=FStar.UInt.fsti(54,22-54,37); use=FStar.UInt.fsti(54,22-54,37)
...
:named haseqTm_refine_f7e217a709bf72990519e54b2cca90dc))
;;;;;;;;;;;;;;;;haseq for Tm_refine_f674e363187b68a0bd32a671b638c73b
;;; Fact-ids: Name Sig.pk_sk; Namespace Sig
(assert (! 
;; def=Sig.fst(41,20-41,41); use=Sig.fst(41,20-41,41)
(forall ((@x0 Term))
 (! (iff (Valid (Prims.hasEq (Tm_refine_f674e363187b68a0bd32a671b638c73b @x0)))
(Valid (Prims.hasEq Sig.sk)))
 

:pattern ((Valid (Prims.hasEq (Tm_refine_f674e363187b68a0bd32a671b638c73b @x0))))
:qid haseqTm_refine_f674e363187b68a0bd32a671b638c73b))

:named haseqTm_refine_f674e363187b68a0bd32a671b638c73b))
;;;;;;;;;;;;;;;;haseq for Tm_refine_f13070840248fced9d9d60d77bdae3ec
;;; Fact-ids: Name FStar.UInt.uint_t; Namespace FStar.UInt
(assert (! 
;; def=FStar.UInt.fsti(54,22-54,37); use=FStar.UInt.fsti(54,22-54,37)
...
(assert (! (HasType Sig.sk
Tm_type)
:named function_token_typing_Sig.sk))
;;;;;;;;;;;;;;;;function token typing
;;; Fact-ids: Name Sig.sig_t; Namespace Sig
(assert (! (HasType Sig.sig_t
Tm_type)
...
;;; Fact-ids: Name Sig.sk; Namespace Sig
(assert (! (= Sig.sk
(Platform.Bytes.lbytes (Sig.sksize Dummy_value)))
:named equation_Sig.sk))
;;;;;;;;;;;;;;;;Equation for Sig.sig_t
;;; Fact-ids: Name Sig.sig_t; Namespace Sig
(assert (! (= Sig.sig_t
(Platform.Bytes.lbytes (Sig.sigsize Dummy_value)))
:named equation_Sig.sig_t))
;;;;;;;;;;;;;;;;Equation for Sig.pk_sk
;;; Fact-ids: Name Sig.pk_sk; Namespace Sig
(assert (! 
;; def=Sig.fst(41,5-41,10); use=Sig.fst(41,5-41,10)
(forall ((@x0 Term))
 (! (= (Sig.pk_sk @x0)
(Tm_refine_f674e363187b68a0bd32a671b638c73b @x0))
 

:pattern ((Sig.pk_sk @x0))
:qid equation_Sig.pk_sk))

:named equation_Sig.pk_sk))
;;;;;;;;;;;;;;;;Equation for Sig.pk
;;; Fact-ids: Name Sig.pk; Namespace Sig
(assert (! (= Sig.pk
(Platform.Bytes.lbytes (Sig.pksize Dummy_value)))
...
:named b2t_def))
;;;;;;;;;;;;;;;;Assumption: Sig.entry__uu___haseq
;;; Fact-ids: Name Sig.entry__uu___haseq; Namespace Sig
(assert (! (Valid (Prims.hasEq Sig.entry))
:named assumption_Sig.entry__uu___haseq))
...
; Starting query at Sig.fst(57,16-60,3)
...
; Encoding query formula : forall (p: Sig.pk) (s: Sig.pk_sk p) (t: Sig.text{Sig.key_prop p t}).
;   (*  - Could not prove post-condition
; *)
;   forall (h0: FStar.Monotonic.Heap.heap).
;     Prims.auto_squash (FStar.Heap.trivial_preorder (Prims.list Sig.entry)
;           (FStar.Monotonic.Heap.sel h0 Sig.log)
;           (Sig.Entry p t (Sig.fdh_rsa s t) :: FStar.Monotonic.Heap.sel h0 Sig.log))


; Context: While encoding a query
; While typechecking the top-level declaration `let sign`
...
;; def=FStar.ST.fst(124,14-124,29); use=Sig.fst(59,6-59,8)
(or label_1
...
(Prims.list Sig.entry))
(FStar.Monotonic.Heap.sel (Prims.list Sig.entry)
(FStar.Heap.trivial_preorder (Prims.list Sig.entry))
@x3
(Sig.log Dummy_value)))
(Prims.Cons Sig.entry
(Sig.Entry @x0
@x2
(Sig.fdh_rsa @x1
@x2))
...
; QUERY ID: (Sig.sign, 1)
; STATUS: unsat
; UNSAT CORE GENERATED: @query, equation_FStar.Heap.trivial_preorder, equation_FStar.Heap.trivial_rel, interpretation_Tm_abs_568747eb5009c1dec504311dee989dc2, token_correspondence_FStar.Heap.trivial_preorder, true_interp
...
; encoding sigelt let sign


; <Start encoding let sign>

;;;;;;;;;;;;;;;;Uninterpreted function symbol for impure function
(declare-fun Sig.sign (Term Term Term) Term)
;;;;;;;;;;;;;;;;Uninterpreted name for impure function
(declare-fun Sig.sign@tok () Term)

; </end encoding let sign>


; encoding sigelt val Sig.verify


; <Skipped val Sig.verify/>
...
(HasType @x2
(Prims.list @x0)))
(HasType (FStar.List.Tot.Base.find @x0
@x1
@x2)
...
@x1
@x2))
:qid typing_FStar.List.Tot.Base.find))

:named typing_FStar.List.Tot.Base.find))
;;;;;;;;;;;;;;;;name-token correspondence
...
(HasType @x3
(Prims.list @x1)))
(HasType (FStar.List.Tot.Base.find.fuel_instrumented @u0
@x1
@x2
@x3)
...
:pattern ((FStar.List.Tot.Base.find.fuel_instrumented @u0
@x1
@x2
@x3))
:qid token_correspondence_FStar.List.Tot.Base.find.fuel_instrumented))

:named token_correspondence_FStar.List.Tot.Base.find.fuel_instrumented))
;;;;;;;;;;;;;;;;subterm ordering
;;; Fact-ids: Name FStar.Pervasives.Native.option; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.None; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.Some; Namespace FStar.Pervasives.Native
(assert (! 
;; def=FStar.Pervasives.Native.fst(33,4-33,8); use=FStar.Pervasives.Native.fst(33,4-33,8)
...
;;; Fact-ids: Name FStar.List.Tot.Base.find; Namespace FStar.List.Tot.Base
(assert (! 
;; def=FStar.List.Tot.Base.fst(268,33-268,43); use=FStar.List.Tot.Base.fst(268,33-268,43)
(forall ((@x0 Term) (@x1 Term))
...
;; def=FStar.List.Tot.Base.fst(268,38-268,41); use=FStar.List.Tot.Base.fst(268,38-268,41)
...
;;;;;;;;;;;;;;;;kinding_Tm_arrow_3892df11bf018e17133d2fef9397e3c1
;;; Fact-ids: Name Sig.uu___is_Entry; Namespace Sig
(assert (! (HasType Tm_arrow_3892df11bf018e17133d2fef9397e3c1
Tm_type)
:named kinding_Tm_arrow_3892df11bf018e17133d2fef9397e3c1))
;;; Fact-ids: Name FStar.Pervasives.Native.option; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.None; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.Some; Namespace FStar.Pervasives.Native
...
;;;;;;;;;;;;;;;;Equation for fuel-instrumented recursive function: FStar.List.Tot.Base.find
;;; Fact-ids: Name FStar.List.Tot.Base.find; Namespace FStar.List.Tot.Base
(assert (! 
;; def=FStar.List.Tot.Base.fst(266,8-266,12); use=FStar.List.Tot.Base.fst(266,8-266,12)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term) (@x3 Term))
...
(FStar.Pervasives.Native.None (Tm_refine_3b1cb9ec3355fed185c658f53954b3fa @x1
@x2))
...
(ite (= @lb5
(BoxBool true))
(FStar.Pervasives.Native.Some (Tm_refine_3b1cb9ec3355fed185c658f53954b3fa @x1
@x2)
(Prims.Cons_hd @lb4))
(FStar.List.Tot.Base.find.fuel_instrumented @u0
@x1
@x2
(Prims.Cons_tl @lb4))))
Tm_unit)))))
...
@x2
@x3))
:qid equation_with_fuel_FStar.List.Tot.Base.find.fuel_instrumented))

:named equation_with_fuel_FStar.List.Tot.Base.find.fuel_instrumented))
...
(forall ((@u0 Fuel) (@x1 Term))
 (! (implies (HasTypeFuel @u0
@x1
Tm_arrow_3892df11bf018e17133d2fef9397e3c1)
(is-Tm_arrow (PreType @x1)))
...
@x1
Tm_arrow_3892df11bf018e17133d2fef9397e3c1))
:qid Sig_pre_typing_Tm_arrow_3892df11bf018e17133d2fef9397e3c1))

:named Sig_pre_typing_Tm_arrow_3892df11bf018e17133d2fef9397e3c1))
;;;;;;;;;;;;;;;;interpretation_Tm_arrow_3892df11bf018e17133d2fef9397e3c1
;;; Fact-ids: Name Sig.uu___is_Entry; Namespace Sig
(assert (! 
;; def=Sig.fst(45,5-45,10); use=Sig.fst(45,5-45,10)
(forall ((@x0 Term))
...
:pattern ((ApplyTT @x0
@x1))
:qid Sig_interpretation_Tm_arrow_3892df11bf018e17133d2fef9397e3c1.1))
...
:pattern ((HasTypeZ @x0
Tm_arrow_3892df11bf018e17133d2fef9397e3c1))
:qid Sig_interpretation_Tm_arrow_3892df11bf018e17133d2fef9397e3c1))

:named Sig_interpretation_Tm_arrow_3892df11bf018e17133d2fef9397e3c1))
;;;;;;;;;;;;;;;;pretyping
;;; Fact-ids: Name FStar.Pervasives.Native.option; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.None; Namespace FStar.Pervasives.Native; Name FStar.Pervasives.Native.Some; Namespace FStar.Pervasives.Native
(assert (! 
;; def=FStar.Pervasives.Native.fst(31,5-31,11); use=FStar.Pervasives.Native.fst(31,5-31,11)
...
; Starting query at Sig.fst(66,18-68,13)
...
(declare-fun Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 (Term Term) Term)
;;;;;;;;;;;;;;;;refinement kinding
;;; Fact-ids: 
(assert (! 
;; def=FStar.List.Tot.Base.fst(265,23-265,33); use=Sig.fst(66,18-68,13)
(forall ((@x0 Term) (@x1 Term))
 (! (HasType (Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 @x0
@x1)
Tm_type)
 

:pattern ((HasType (Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 @x0
@x1)
Tm_type))
:qid refinement_kinding_Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1))

:named refinement_kinding_Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1))
;;;;;;;;;;;;;;;;refinement_interpretation
;;; Fact-ids: 
(assert (! 
;; def=FStar.List.Tot.Base.fst(265,23-265,33); use=Sig.fst(66,18-68,13)
(forall ((@u0 Fuel) (@x1 Term) (@x2 Term) (@x3 Term))
 (! (iff (HasTypeFuel @u0
@x1
(Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 @x2
@x3))
(and (HasTypeFuel @u0
@x1
Sig.entry)

;; def=FStar.List.Tot.Base.fst(265,28-265,31); use=Sig.fst(66,18-68,13)
(BoxBool_proj_0 (let ((@lb4 @x1))
(Prims.op_AmpAmp (Prims.op_Equality Sig.pk
@x2
(Sig.Entry_k @lb4))
(Prims.op_Equality Sig.text
@x3
(Sig.Entry_t @lb4)))))
))
...
(Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 @x2
@x3)))
:qid refinement_interpretation_Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1))

:named refinement_interpretation_Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1))
;;;;;;;;;;;;;;;;haseq for Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1
;;; Fact-ids: 
(assert (! 
;; def=FStar.List.Tot.Base.fst(265,23-265,33); use=Sig.fst(66,18-68,13)
(forall ((@x0 Term) (@x1 Term))
 (! (iff (Valid (Prims.hasEq (Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 @x0
@x1)))
(Valid (Prims.hasEq Sig.entry)))
 

:pattern ((Valid (Prims.hasEq (Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 @x0
@x1))))
:qid haseqTm_refine_fcaf3f241603a1e28a7ee47f5eb025f1))

:named haseqTm_refine_fcaf3f241603a1e28a7ee47f5eb025f1))





(declare-fun Tm_abs_0906a909ab3c422949a080aa3e10ae1f (Term Term) Term)
;;;;;;;;;;;;;;;;typing_Tm_abs_0906a909ab3c422949a080aa3e10ae1f
;;; Fact-ids: 
(assert (! 
;; def=Sig.fst(67,28-67,70); use=Sig.fst(67,28-67,70)
(forall ((@x0 Term) (@x1 Term))
 (! (HasType (Tm_abs_0906a909ab3c422949a080aa3e10ae1f @x0
@x1)
Tm_arrow_3892df11bf018e17133d2fef9397e3c1)
 

:pattern ((Tm_abs_0906a909ab3c422949a080aa3e10ae1f @x0
@x1))
:qid typing_Tm_abs_0906a909ab3c422949a080aa3e10ae1f))

:named typing_Tm_abs_0906a909ab3c422949a080aa3e10ae1f))
;;;;;;;;;;;;;;;;interpretation_Tm_abs_0906a909ab3c422949a080aa3e10ae1f
;;; Fact-ids: 
(assert (! 
;; def=Sig.fst(67,28-67,70); use=Sig.fst(67,28-67,70)
(forall ((@x0 Term) (@x1 Term) (@x2 Term))
 (! (= (ApplyTT (Tm_abs_0906a909ab3c422949a080aa3e10ae1f @x1
@x2)
@x0)
(let ((@lb3 @x0))
(Prims.op_AmpAmp (Prims.op_Equality Sig.pk
...
:pattern ((ApplyTT (Tm_abs_0906a909ab3c422949a080aa3e10ae1f @x1
@x2)
@x0))
:qid interpretation_Tm_abs_0906a909ab3c422949a080aa3e10ae1f))

:named interpretation_Tm_abs_0906a909ab3c422949a080aa3e10ae1f))



; Encoding query formula : forall (p: Sig.pk) (t: Sig.text) (m: Sig.sig_t).
;   (*  - Could not prove post-condition
; *)
;   forall (h0: FStar.Monotonic.Heap.heap).
;     (forall (x: Sig.entry) (b: Sig.pk) (b: t: Sig.text{Sig.key_prop b t}) (b: Sig.sig_t).
;         x == Sig.Entry b b b ==> Prims.hasEq Sig.pk /\ (p = b ==> Prims.hasEq Sig.text)) /\
;     (forall (_: Sig.entry).
;         (*  - Could not prove post-condition
; *)
;         forall (b: Sig.pk) (b: t: Sig.text{Sig.key_prop b t}) (b: Sig.sig_t).
;           _ == Sig.Entry b b b ==> Prims.hasEq Sig.pk /\ (p = b ==> Prims.hasEq Sig.text)) /\
;     (forall (any_result: (_: Sig.entry -> Prims.bool)).
;         (fun _ ->
;             (let Sig.Entry p' t' _ = _ in
;               p = p' && t = t')
;             <:
;             Prims.bool) ==
;         any_result ==>
;         (forall (any_result:
;             FStar.Pervasives.Native.option (x:
;                 Sig.entry
;                   { let Sig.Entry p' t' _ = x in
;                     p = p' && t = t' })).
;             any_result ==
;             FStar.List.Tot.Base.find (fun _ ->
;                   (let Sig.Entry p' t' _ = _ in
;                     p = p' && t = t')
;                   <:
;                   Prims.bool)
...
;                     Some? any_result == any_result ==> Some? any_result ==> Sig.key_prop p t))))


; Context: While encoding a query
; While typechecking the top-level declaration `let verify`
...
;; def=Prims.fst(459,77-459,89); use=Sig.fst(66,18-68,13)
(and 
;; def=dummy(0,0-0,0); use=Sig.fst(66,18-68,13)
...
;; def=Prims.fst(413,99-413,120); use=Sig.fst(67,23-67,27)
(forall ((@x6 Term))
 (! (implies (HasType @x6
(Tm_refine_f7e217a709bf72990519e54b2cca90dc @x5))
...
;; def=Sig.fst(67,39-67,52); use=Sig.fst(67,23-67,27)
(= @x4
(Sig.Entry @x5
@x6
@x7))
...
;; def=Sig.fst(47,14-67,61); use=Sig.fst(67,23-67,27)
(and 
;; def=Prims.fst(73,23-73,30); use=Sig.fst(67,23-67,27)
(or label_1
...
(implies 
;; def=Sig.fst(67,57-67,61); use=Sig.fst(67,23-67,27)
(= @x0
@x5)
...
;; def=Prims.fst(413,99-413,120); use=Sig.fst(67,28-67,70)
(forall ((@x5 Term))
 (! (implies (HasType @x5
Sig.pk)
...
;; def=Sig.fst(67,29-67,69); use=Sig.fst(67,29-67,69)
(= @x4
(Sig.Entry @x5
@x6
@x7))
...
;; def=Sig.fst(47,14-67,61); use=Sig.fst(67,57-67,69)
(and 
;; def=Prims.fst(73,23-73,30); use=Sig.fst(67,57-67,58)
(or label_3
...
(implies 
;; def=Sig.fst(67,57-67,61); use=Sig.fst(67,57-67,61)
(= @x0
@x5)


;; def=Prims.fst(73,23-73,30); use=Sig.fst(67,67-67,69)
(or label_4
...
;; def=Prims.fst(451,66-451,102); use=Sig.fst(66,18-68,13)
(forall ((@x4 Term))
 (! (implies (and (HasType @x4
Tm_arrow_3892df11bf018e17133d2fef9397e3c1)

;; def=FStar.List.Tot.Base.fst(263,11-263,12); use=Sig.fst(66,18-68,13)
(= (Tm_abs_0906a909ab3c422949a080aa3e10ae1f @x0
@x1)
@x4)
)
...
(FStar.Pervasives.Native.option (Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 @x0
@x1)))

;; def=Sig.fst(67,14-67,75); use=Sig.fst(67,14-67,75)
(= @x5
(FStar.List.Tot.Base.find Sig.entry
(Tm_abs_0906a909ab3c422949a080aa3e10ae1f @x0
@x1)
(FStar.Monotonic.Heap.sel (Prims.list Sig.entry)
(FStar.Heap.trivial_preorder (Prims.list Sig.entry))
...
;; def=Prims.fst(413,99-413,120); use=Sig.fst(68,8-68,13)
(forall ((@x8 Term))
 (! (implies (HasType @x8
(Tm_refine_f7e217a709bf72990519e54b2cca90dc @x7))
...
;; def=Sig.fst(67,39-67,52); use=Sig.fst(68,8-68,13)
(= @x6
(Sig.Entry @x7
@x8
@x9))
...
;; def=Sig.fst(47,14-67,61); use=Sig.fst(68,8-68,13)
(and 
;; def=Prims.fst(73,23-73,30); use=Sig.fst(68,8-68,13)
(or label_5
...
(implies 
;; def=Sig.fst(67,57-67,61); use=Sig.fst(68,8-68,13)
(= @x0
@x7)
...
;; def=FStar.Pervasives.Native.fst(31,13-31,14); use=Sig.fst(66,18-68,13)
(= (Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 @x0
@x1)
@x6)
)
...
;; def=Sig.fst(65,15-68,13); use=Sig.fst(66,18-68,13)
(= (FStar.Pervasives.Native.uu___is_Some (Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 @x0
@x1)
@x5)
@x7)


;; def=Sig.fst(65,16-65,17); use=Sig.fst(68,2-68,13)
(BoxBool_proj_0 (FStar.Pervasives.Native.uu___is_Some (Tm_refine_fcaf3f241603a1e28a7ee47f5eb025f1 @x0
@x1)
@x5))
)

;; def=Sig.fst(65,29-65,41); use=Sig.fst(68,2-68,13)
(or label_7
