...
(push) ; 3
(set-option :timeout 10)
(assert (not (< $Perm.Write (+ $k@5@01 $Perm.Write))))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
...
(push) ; 3
(set-option :timeout 10)
(assert (not (< $Perm.Write (+ $Perm.Write $k@5@01))))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
...
(assert (not (and
  ...
  (not
    (=
      ...
      ($SortWrappers.$SnapTo$Ref ($Snap.second $t@4@01)))))))
(check-sat)
; unknown
(pop) ; 3
; 0.00s
...
; 0.00s
; (get-info :all-statistics)
(assert (and (not (= t1@2@01 t2@3@01)) (not (= t2@3@01 t1@2@01))))
; [eval] t1 != t2
(pop) ; 2
(pop) ; 1
