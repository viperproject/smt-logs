...
(check-sat)
; unknown
; [eval] (unfolding acc(Option(self), 1 / 2) in self.discriminant)
(set-option :timeout 0)
(push) ; 2
(assert (Option%trigger s@$ self@0@00))
(assert (not (= self@0@00 $Ref.null)))
(pop) ; 2
; Joined path conditions
(assert (and (Option%trigger s@$ self@0@00) (not (= self@0@00 $Ref.null))))
