...
(assert (=
  ...
  $Snap.unit))
; [eval] x.right > 52
(assert (> ($SortWrappers.$SnapToInt ($Snap.first ($Snap.second $t@6@01))) 52))
...
(assert (=
  ...
  $Snap.unit))
; [eval] x.left == 42
(assert (= ($SortWrappers.$SnapToInt ($Snap.first $t@7@01)) 42))
(pop) ; 2
(set-option :timeout 0)
(push) ; 2
; [exec]
...
(assert (= n@8@01 ($SortWrappers.$SnapToInt ($Snap.first $t@6@01))))
; [exec]
; label ret
; [exec]
; x.left := x.left * 2
; [eval] x.left * 2
(declare-const left@9@01 Int)
(assert (= left@9@01 (* ($SortWrappers.$SnapToInt ($Snap.first $t@6@01)) 2)))
(push) ; 3
(set-option :timeout 10)
...
; 0.00s
; (get-info :all-statistics)
(assert (and (not (= y@4@01 x@3@01)) (not (= x@3@01 y@4@01))))
(assert (and (not (= y@4@01 x@3@01)) (not (= x@3@01 y@4@01))))
; [eval] x.left == 42
(set-option :timeout 0)
(push) ; 3
