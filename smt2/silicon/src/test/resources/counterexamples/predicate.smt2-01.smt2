...
; [eval] a.x == 42
(assert (= ($SortWrappers.$SnapToInt ($Snap.first $t@5@01)) 42))
(pop) ; 2
(push) ; 2
; [exec]
; unfold acc(StructA(a), write)
(assert (=
  ($Snap.first $t@4@01)
  ($Snap.combine
...
(check-sat)
; unknown
(assert (StructA%trigger ($Snap.first $t@4@01) a@2@01))
; [exec]
; unfold acc(StructB(b), write)
(assert (=
  ($Snap.second $t@4@01)
  ($Snap.combine
...
(check-sat)
; unknown
(assert (StructB%trigger ($Snap.second $t@4@01) b@3@01))
; [exec]
; a.x := 3
(set-option :timeout 0)
(push) ; 3
