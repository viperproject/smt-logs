...
(push) ; 2
; [exec]
; assert constant_DoesDeclOrderMatter_pkg_G() &&
;   constant_BoolExprConst_pkg_G()
; [eval] constant_DoesDeclOrderMatter_pkg_G()
(push) ; 3
(assert (not (as constant_DoesDeclOrderMatter_pkg_G<Bool>  Bool)))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
; (get-info :all-statistics)
(assert (as constant_DoesDeclOrderMatter_pkg_G<Bool>  Bool))
; [eval] constant_BoolExprConst_pkg_G()
(push) ; 3
(assert (not (as constant_BoolExprConst_pkg_G<Bool>  Bool)))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
; (get-info :all-statistics)
(assert (as constant_BoolExprConst_pkg_G<Bool>  Bool))
; [exec]
; label returnLabel
(pop) ; 2
(pop) ; 1
