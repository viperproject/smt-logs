...
(check-sat)
; unknown
; [eval] (i <= 0 ? true : (unfolding acc(list(r, i), write) in r.val == v && allSet(r.next, i - 1, v)))
; [eval] i <= 0
(set-option :timeout 0)
(push) ; 2
(push) ; 3
(set-option :timeout 10)
(assert (not (not (<= i@1@00 0))))
(check-sat)
; unknown
(pop) ; 3
; 0.00s
...
; 0.00s
; (get-info :all-statistics)
; [then-branch: 0 | i@1@00 <= 0 | live]
; [else-branch: 0 | !(i@1@00 <= 0) | live]
(set-option :timeout 0)
(push) ; 3
; [then-branch: 0 | i@1@00 <= 0]
(assert (<= i@1@00 0))
(pop) ; 3
(push) ; 3
; [else-branch: 0 | !(i@1@00 <= 0)]
(assert (not (<= i@1@00 0)))
; [eval] (unfolding acc(list(r, i), write) in r.val == v && allSet(r.next, i - 1, v))
(push) ; 4
(assert (list%trigger s@$ r@0@00 i@1@00))
; [eval] i > 0
(push) ; 5
(set-option :timeout 10)
(assert (not (not (> i@1@00 0))))
...
(set-option :timeout 10)
(assert (not (and
  (= r@0@00 ($SortWrappers.$SnapTo$Ref ($Snap.first ($Snap.second s@$))))
  (= i@1@00 (- i@1@00 1)))))
(check-sat)
; unknown
(pop) ; 6
; 0.00s
; (get-info :all-statistics)
; [eval] r.val == v && allSet(r.next, i - 1, v)
; [eval] r.val == v
(set-option :timeout 0)
(push) ; 6
; [then-branch: 2 | First:(s@$) != v@2@00 | live]
; [else-branch: 2 | First:(s@$) == v@2@00 | live]
(push) ; 7
; [then-branch: 2 | First:(s@$) != v@2@00]
(assert (not (= ($SortWrappers.$SnapToInt ($Snap.first s@$)) v@2@00)))
(pop) ; 7
(push) ; 7
; [else-branch: 2 | First:(s@$) == v@2@00]
(assert (= ($SortWrappers.$SnapToInt ($Snap.first s@$)) v@2@00))
; [eval] allSet(r.next, i - 1, v)
; [eval] i - 1
(push) ; 8
(assert (allSet%precondition ($Snap.second ($Snap.second s@$)) ($SortWrappers.$SnapTo$Ref ($Snap.first ($Snap.second s@$))) (-
  i@1@00
  1) v@2@00))
(pop) ; 8
...
(assert (or
  (= ($SortWrappers.$SnapToInt ($Snap.first s@$)) v@2@00)
  (not (= ($SortWrappers.$SnapToInt ($Snap.first s@$)) v@2@00))))
(pop) ; 5
(pop) ; 4
; Joined path conditions
(assert (list%trigger s@$ r@0@00 i@1@00))
...
(assert (=
  ...
  (ite
    ...
    (and
      (= ($SortWrappers.$SnapToInt ($Snap.first s@$)) v@2@00)
      (allSet ($Snap.second ($Snap.second s@$)) ($SortWrappers.$SnapTo$Ref ($Snap.first ($Snap.second s@$))) (-
        i@1@00
        1) v@2@00)))))
(pop) ; 1
(assert (forall ((s@$ $Snap) (r@0@00 $Ref) (i@1@00 Int) (v@2@00 Int)) (!
...
; 0.00s
; (get-info :all-statistics)
; [then-branch: 3 | i@5@00 > 0 | live]
; [else-branch: 3 | !(i@5@00 > 0) | live]
(set-option :timeout 0)
(push) ; 2
; [then-branch: 3 | i@5@00 > 0]
(assert (> i@5@00 0))
