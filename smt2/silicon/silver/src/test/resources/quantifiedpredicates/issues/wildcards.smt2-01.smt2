...
; Definitional axioms for singleton-SM's value
(assert (=
  ($PSF.lookup_P (as sm@16@01  $PSF<P>) ($Snap.combine
    ($SortWrappers.$RefTo$Snap x@13@01)
    ($SortWrappers.IntTo$Snap 0)))
  $t@15@01))
(assert (<= $Perm.No (ite (= 0 0) $k@14@01 $Perm.No)))
; State saturation: after contract
(set-option :timeout 50)
(check-sat)
; unknown
...
(declare-fun inv@20@01 ($Ref Int) $Ref)
(declare-fun img@21@01 ($Ref Int) Bool)
; Nested auxiliary terms: globals
; Nested auxiliary terms: non-globals
; Check receiver injectivity
; Definitional axioms for inverse functions
...
(push) ; 3
; [eval] (r in xs)
(assert (Set_in r@22@01 xs@17@01))
; [eval] perm(P(r, 0))
(declare-const sm@23@01 $PSF<P>)
...
(assert (forall ((s@24@01 $Snap)) (!
  (=>
    (ite
      (and
        (= ($SortWrappers.$SnapTo$Ref ($Snap.first s@24@01)) x@13@01)
        (= ($SortWrappers.$SnapToInt ($Snap.second s@24@01)) 0))
      (< $Perm.No $k@14@01)
      false)
    (and
      (not (= s@24@01 $Snap.unit))
      (=
        ($PSF.lookup_P (as sm@23@01  $PSF<P>) s@24@01)
        ($PSF.lookup_P (as sm@16@01  $PSF<P>) s@24@01))))
  :pattern (($PSF.lookup_P (as sm@23@01  $PSF<P>) s@24@01))
  :pattern (($PSF.lookup_P (as sm@16@01  $PSF<P>) s@24@01))
  :qid |qp.psmValDef1|)))
(declare-const pm@25@01 $PPM)
(assert (forall ((x $Ref) (idx Int)) (!
  (=
    ($PSF.perm_P (as pm@25@01  $PPM) ($Snap.combine
      ($SortWrappers.$RefTo$Snap x)
      ($SortWrappers.IntTo$Snap idx)))
    (ite (and (= x x@13@01) (= idx 0)) $k@14@01 $Perm.No))
  :pattern (($PSF.perm_P (as pm@25@01  $PPM) ($Snap.combine
    ($SortWrappers.$RefTo$Snap x)
    ($SortWrappers.IntTo$Snap idx))))
  :qid |qp.resPrmSumDef3|)))
(pop) ; 3
(declare-fun inv@26@01 ($Ref Int) $Ref)
(declare-fun img@27@01 ($Ref Int) Bool)
; Nested auxiliary terms: globals
(assert (forall ((s@24@01 $Snap)) (!
  (=>
    (ite
...
; Nested auxiliary terms: non-globals
(push) ; 3
(assert (not (forall ((r@22@01 $Ref)) (!
  (=>
    (Set_in r@22@01 xs@17@01)
    (or
      (=
        ($PSF.perm_P (as pm@25@01  $PPM) ($Snap.combine
    ...
    (and
      ...
      (<
        ...
        ($PSF.perm_P (as pm@25@01  $PPM) ($Snap.combine
          ($SortWrappers.$RefTo$Snap r@22@01)
          ($SortWrappers.IntTo$Snap 0)))))
    (and (= (inv@26@01 r@22@01 0) r@22@01) (img@27@01 r@22@01 0)))
  :pattern ((Set_in r@22@01 xs@17@01))
  :pattern ((inv@26@01 r@22@01 0))
  :pattern ((img@27@01 r@22@01 0))
  :qid |P-invOfFct|)))
(assert (forall ((x $Ref) (idx Int)) (!
  (=>
    (and
      (img@27@01 x idx)
      (and
        (Set_in (inv@26@01 x idx) xs@17@01)
        (<
          $Perm.No
          ($PSF.perm_P (as pm@25@01  $PPM) ($Snap.combine
            ($SortWrappers.$RefTo$Snap (inv@26@01 x idx))
            ($SortWrappers.IntTo$Snap 0))))))
    (and (= (inv@26@01 x idx) x) (= 0 idx)))
  :pattern ((inv@26@01 x idx))
  :qid |P-fctOfInv|)))
; Precomputing data for removing quantified permissions
(define-fun pTaken@28@01 ((x $Ref) (idx Int)) $Perm
  (ite
    (and
      (Set_in (inv@26@01 x idx) xs@17@01)
      (img@27@01 x idx)
      (and (= x (inv@26@01 x idx)) (= idx 0)))
    ($Perm.min
      $Perm.No
      ($PSF.perm_P (as pm@25@01  $PPM) ($Snap.combine
        ($SortWrappers.$RefTo$Snap (inv@26@01 x idx))
        ($SortWrappers.IntTo$Snap 0))))
    $Perm.No))
(define-fun pTaken@29@01 ((x $Ref) (idx Int)) $Perm
  (ite
    (and
      (Set_in (inv@26@01 x idx) xs@17@01)
      (img@27@01 x idx)
...
(push) ; 3
(set-option :timeout 500)
(assert (not (= (- (ite (= 0 0) $k@14@01 $Perm.No) (pTaken@29@01 x@13@01 0)) $Perm.No)))
(check-sat)
; unknown
(pop) ; 3
; 0.00s
