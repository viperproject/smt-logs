...
(check-sat)
; unknown
; [eval] (x == null ? 0 : 1 + (unfolding acc(list(x), write) in length(x.next)))
; [eval] x == null
(set-option :timeout 0)
(push) ; 2
(push) ; 3
...
(assert (not (= x@0@00 $Ref.null)))
; [eval] 1 + (unfolding acc(list(x), write) in length(x.next))
; [eval] (unfolding acc(list(x), write) in length(x.next))
(push) ; 4
(assert (list%trigger s@$ x@0@00))
; [eval] x != null
(push) ; 5
(set-option :timeout 10)
(assert (not (= x@0@00 $Ref.null)))
