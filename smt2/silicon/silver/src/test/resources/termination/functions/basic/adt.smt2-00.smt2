...
(check-sat)
; unknown
; [eval] (size(xs) > 0 ? f(tail_Cons(xs)) : 6)
; [eval] size(xs) > 0
; [eval] size(xs)
(set-option :timeout 0)
(push) ; 2
...
(assert (not (not (> (size<Int> xs@0@00) 0))))
(check-sat)
; unknown
(pop) ; 3
; 0.01s
...
(push) ; 3
(set-option :timeout 10)
(assert (not (> (size<Int> xs@0@00) 0)))
(check-sat)
; unknown
(pop) ; 3
; 0.01s
; (get-info :all-statistics)
; [then-branch: 0 | size[Int](xs@0@00) > 0 | live]
; [else-branch: 0 | !(size[Int](xs@0@00) > 0) | live]
(set-option :timeout 0)
(push) ; 3
; [then-branch: 0 | size[Int](xs@0@00) > 0]
(assert (> (size<Int> xs@0@00) 0))
; [eval] f(tail_Cons(xs))
; [eval] tail_Cons(xs)
(push) ; 4
...
; [eval] size(xs)
(push) ; 5
(assert (not (>= (size<Int> (tail_Cons<list> xs@0@00)) 0)))
(check-sat)
; unsat
(pop) ; 5
; 0.00s
; (get-info :all-statistics)
(assert (>= (size<Int> (tail_Cons<list> xs@0@00)) 0))
(assert (f%precondition $Snap.unit (tail_Cons<list> xs@0@00)))
(pop) ; 4
; Joined path conditions
(assert (and
  (>= (size<Int> (tail_Cons<list> xs@0@00)) 0)
  (f%precondition $Snap.unit (tail_Cons<list> xs@0@00))))
(pop) ; 3
(push) ; 3
; [else-branch: 0 | !(size[Int](xs@0@00) > 0)]
(assert (not (> (size<Int> xs@0@00) 0)))
(pop) ; 3
(pop) ; 2
; Joined path conditions
(assert (=>
  ...
  (and
    ...
    (f%precondition $Snap.unit (tail_Cons<list> xs@0@00)))))
; Joined path conditions
(assert (or (not (> (size<Int> xs@0@00) 0)) (> (size<Int> xs@0@00) 0)))
(assert (=
  result@1@00
  (ite (> (size<Int> xs@0@00) 0) (f $Snap.unit (tail_Cons<list> xs@0@00)) 6)))
(pop) ; 1
(assert (forall ((s@$ $Snap) (xs@0@00 list)) (!
  (=>
