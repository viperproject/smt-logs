...
(assert (not (not (= (Seq_length xs@0@00) 0))))
(check-sat)
; unknown
(pop) ; 3
; 0.00s
...
(push) ; 3
(set-option :timeout 10)
(assert (not (= (Seq_length xs@0@00) 0)))
(check-sat)
; unknown
(pop) ; 3
; 0.00s
; (get-info :all-statistics)
; [then-branch: 0 | |xs@0@00| == 0 | live]
; [else-branch: 0 | |xs@0@00| != 0 | live]
(set-option :timeout 0)
(push) ; 3
; [then-branch: 0 | |xs@0@00| == 0]
(assert (= (Seq_length xs@0@00) 0))
(pop) ; 3
(push) ; 3
; [else-branch: 0 | |xs@0@00| != 0]
(assert (not (= (Seq_length xs@0@00) 0)))
; [eval] xs[0] + sum(xs[1..])
; [eval] xs[0]
(push) ; 4
...
; [eval] xs[1..]
(push) ; 4
(assert (sum%precondition $Snap.unit (Seq_drop xs@0@00 1)))
(pop) ; 4
...
; Joined path conditions
(assert (=>
  (not (= (Seq_length xs@0@00) 0))
  (and
    (not (= (Seq_length xs@0@00) 0))
    (sum%precondition $Snap.unit (Seq_drop xs@0@00 1)))))
(assert (or (not (= (Seq_length xs@0@00) 0)) (= (Seq_length xs@0@00) 0)))
(assert (=
  result@1@00
  (ite
    (= (Seq_length xs@0@00) 0)
    0
    (+ (Seq_index xs@0@00 0) (sum $Snap.unit (Seq_drop xs@0@00 1))))))
(pop) ; 1
(assert (forall ((s@$ $Snap) (xs@0@00 Seq<Int>)) (!
  (=>
