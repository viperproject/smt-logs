...
(push) ; 2
; [exec]
; package acc(P(x), write) --* acc(x.f, write) {
...
(check-sat)
; unknown
; Create MagicWandSnapFunction for wand acc(P(x), write) --* acc(x.f, write)
...
(assert true)
; [exec]
; exhale acc(P(x), write) --* acc(x.f, write)
(pop) ; 3
(pop) ; 2
(pop) ; 1
; ---------- t02 ----------
...
; Create MagicWandSnapFunction for wand acc(P(x), write) --* true
