...
; Create MagicWandSnapFunction for wand true --* x.f == 3
...
(push) ; 2
; [exec]
; package acc(x.f, write) && x.f == 0 --* x.f == 0 {
; }
(push) ; 3
...
; [eval] x.f == 0
; Create MagicWandSnapFunction for wand acc(x.f, write) && x.f == 0 --* x.f == 0
...
(push) ; 2
; [exec]
; apply acc(x.f, write) && x.f == 0 --* x.f == 0
; [eval] x.f == 0
(assert (=
  (MWSF_apply ($SortWrappers.$SnapTo$MWSF ($Snap.second ($Snap.second $t@14@01))) ($Snap.combine
    ($Snap.first $t@14@01)
    $Snap.unit))
  $Snap.unit))
; [eval] x.f == 0
