...
(push) ; 2
; [exec]
; package acc(y.f, write) && acc(P(x), write) --*
; acc(y.f, write) && (unfolding acc(P(x), write) in true) {
; }
(push) ; 3
...
(set-option :timeout 0)
(push) ; 4
(assert (P%trigger ($Snap.second $t@4@01) x@2@01))
(push) ; 5
(set-option :timeout 10)
...
(assert (and (P%trigger ($Snap.second $t@4@01) x@2@01) (not (= x@2@01 $Ref.null))))
; Create MagicWandSnapFunction for wand acc(y.f, write) && acc(P(x), write) --* acc(y.f, write) && (unfolding acc(P(x), write) in true)
