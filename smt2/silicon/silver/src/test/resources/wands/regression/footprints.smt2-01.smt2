...
; [eval] x.l != null
; [exec]
; package acc(x.l, write) --* acc(x.l, write) && acc(x.l.v, write) {
; }
(push) ; 3
...
(assert (not (=
  ...
  ($SortWrappers.$SnapTo$Ref $t@3@01))))
(check-sat)
; unknown
(pop) ; 4
; 0.00s
...
; Create MagicWandSnapFunction for wand acc(x.l, write) --* acc(x.l, write) && acc(x.l.v, write)
...
(assert (not (= ($SortWrappers.$SnapTo$Ref ($Snap.first $t@9@01)) $Ref.null)))
(assert (= ($Snap.second ($Snap.second $t@9@01)) $Snap.unit))
(assert ($SortWrappers.$SnapToBool ($Snap.first ($Snap.second $t@9@01))))
; State saturation: after contract
(set-option :timeout 50)
(check-sat)
; unknown
...
; [eval] x.l != null
; [exec]
; package acc(x.l, write) && x.l != null --*
...
(set-option :timeout 10)
(assert (not (=
  ($SortWrappers.$SnapTo$Ref ($Snap.first $t@9@01))
...
; Create MagicWandSnapFunction for wand acc(x.l, write) && x.l != null --* acc(x.l, write) && acc(x.l.v, write)
...
; Create MagicWandSnapFunction for wand true --* x.v
