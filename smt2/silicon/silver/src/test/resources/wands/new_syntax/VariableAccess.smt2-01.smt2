...
; [exec]
; package true --* acc(TrueListNode(x), write) && getNext(x) == n {
;   var tmp: Ref
;   tmp := n
...
(push) ; 4
(set-option :timeout 10)
(assert (not (= x@1@01 n@4@01)))
(check-sat)
; unknown
(pop) ; 4
; 0.00s
; (get-info :all-statistics)
(assert (TrueListNode%trigger ($Snap.combine
  ($SortWrappers.$RefTo$Snap n@4@01)
  ($Snap.combine $Snap.unit ($SortWrappers.$RefTo$Snap next@5@01))) x@1@01))
(set-option :timeout 0)
(push) ; 4
(set-option :timeout 10)
(assert (not false))
...
(set-option :timeout 0)
(push) ; 4
(assert (getNext%precondition ($Snap.combine
  ($SortWrappers.$RefTo$Snap n@4@01)
  ($Snap.combine $Snap.unit ($SortWrappers.$RefTo$Snap next@5@01))) x@1@01))
(pop) ; 4
...
(assert (=
  (getNext ($Snap.combine
    ...
    ($Snap.combine $Snap.unit ($SortWrappers.$RefTo$Snap next@5@01))) x@1@01)
  n@4@01))
; Create MagicWandSnapFunction for wand true --* acc(TrueListNode(x), write) && getNext(x) == n
(declare-const mwsf@8@01 $MWSF)
(assert (forall (($t@6@01 $Snap)) (!
  (=
    (MWSF_apply mwsf@8@01 $t@6@01)
    ($Snap.combine
      ($Snap.combine
        ($SortWrappers.$RefTo$Snap n@4@01)
