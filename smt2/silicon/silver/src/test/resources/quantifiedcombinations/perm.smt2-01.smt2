...
(assert (= ($FVF.lookup_f (as sm@6@01  $FVF<f>) x@1@01) $t@5@01))
...
(assert (forall ((r $Ref)) (!
  (= ($FVF.perm_f (as pm@7@01  $FPM) r) (ite (= r x@1@01) $Perm.Write $Perm.No))
  :pattern (($FVF.perm_f (as pm@7@01  $FPM) r))
  :qid |qp.resPrmSumDef0|)))
; perm(x.f)  ~~>  assume upper permission bound
(assert (<= ($FVF.perm_f (as pm@7@01  $FPM) x@1@01) $Perm.Write))
(set-option :timeout 0)
(push) ; 3
(assert (not (= ($FVF.perm_f (as pm@7@01  $FPM) x@1@01) $Perm.Write)))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
...
(assert (not (= (- $Perm.Write (pTaken@8@01 x@1@01)) $Perm.No)))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
...
(assert (not (forall ((r $Ref)) (!
  (=> (= r x@1@01) (= (- $Perm.Write (pTaken@8@01 r)) $Perm.No))
...
(assert (forall ((r $Ref)) (!
  (= ($FVF.perm_f (as pm@9@01  $FPM) r) $Perm.No)
  :pattern (($FVF.perm_f (as pm@9@01  $FPM) r))
  :qid |qp.resPrmSumDef1|)))
; perm(x.f)  ~~>  assume upper permission bound
(assert (<= ($FVF.perm_f (as pm@9@01  $FPM) x@1@01) $Perm.Write))
(set-option :timeout 0)
(push) ; 3
(assert (not (= ($FVF.perm_f (as pm@9@01  $FPM) x@1@01) $Perm.No)))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
; (get-info :all-statistics)
(assert (= ($FVF.perm_f (as pm@9@01  $FPM) x@1@01) $Perm.No))
(pop) ; 2
(pop) ; 1
; ---------- test02 ----------
...
; Definitional axioms for singleton-SM's value
(assert (=
  ($PSF.lookup_P (as sm@16@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@11@01))
  $t@15@01))
; State saturation: after inhale
(set-option :timeout 20)
(check-sat)
...
(define-fun pTaken@17@01 ((x $Ref)) $Perm
  (ite
    (= x x@11@01)
    ($Perm.min (ite (= x x@11@01) $Perm.Write $Perm.No) $Perm.Write)
    $Perm.No))
; Done precomputing, updating quantified chunks
; State saturation: before repetition
(set-option :timeout 10)
...
(push) ; 3
(set-option :timeout 500)
(assert (not (= (- $Perm.Write (pTaken@17@01 x@11@01)) $Perm.No)))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
...
(assert (not (forall ((x $Ref)) (!
  (=> (= x x@11@01) (= (- $Perm.Write (pTaken@17@01 x)) $Perm.No))
...
(assert (and
  (not (= ($SortWrappers.$RefTo$Snap x@11@01) $Snap.unit))
  (=
    ($PSF.lookup_P (as sm@18@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@11@01))
    ($PSF.lookup_P (as sm@16@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@11@01)))))
(assert (=
  ($PSF.lookup_P (as sm@18@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@11@01))
  ($Snap.combine
    ($Snap.first ($PSF.lookup_P (as sm@18@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@11@01)))
    ($Snap.second ($PSF.lookup_P (as sm@18@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@11@01))))))
...
; [eval] x.f > 0
(assert (>
  ($SortWrappers.$SnapToInt ($Snap.first ($PSF.lookup_P (as sm@18@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@11@01))))
  0))
; State saturation: after unfold
(set-option :timeout 40)
(check-sat)
; unknown
(assert (P%trigger ($PSF.lookup_P (as sm@18@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@11@01)) x@11@01))
; [exec]
; assert perm(x.f) == write
; [eval] perm(x.f) == write
; [eval] perm(x.f)
...
(declare-const pm@22@01 $PPM)
(assert (forall ((x $Ref)) (!
  (= ($PSF.perm_P (as pm@22@01  $PPM) ($SortWrappers.$RefTo$Snap x)) $Perm.No)
  :pattern (($PSF.perm_P (as pm@22@01  $PPM) ($SortWrappers.$RefTo$Snap x)))
  :qid |qp.resPrmSumDef3|)))
(set-option :timeout 0)
(push) ; 3
(assert (not (=
  ($PSF.perm_P (as pm@22@01  $PPM) ($SortWrappers.$RefTo$Snap x@11@01))
  $Perm.No)))
(check-sat)
; unsat
(pop) ; 3
...
; Definitional axioms for singleton-SM's value
(assert (=
  ($PSF.lookup_P (as sm@23@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@11@01))
  ($Snap.combine
    ($Snap.first ($PSF.lookup_P (as sm@18@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@11@01)))
    $Snap.unit)))
; [exec]
...
; [eval] perm(P(x)) == write
; [eval] perm(P(x))
(declare-const sm@24@01 $PSF<P>)
(declare-const s@25@01 $Snap)
; Definitional axioms for snapshot map domain
; Definitional axioms for snapshot map values
(assert (forall ((s@25@01 $Snap)) (!
  (=>
    (= ($SortWrappers.$SnapTo$Ref s@25@01) x@11@01)
    (and
      (not (= s@25@01 $Snap.unit))
      (=
        ($PSF.lookup_P (as sm@24@01  $PSF<P>) s@25@01)
        ($PSF.lookup_P (as sm@23@01  $PSF<P>) s@25@01))))
  :pattern (($PSF.lookup_P (as sm@24@01  $PSF<P>) s@25@01))
  :pattern (($PSF.lookup_P (as sm@23@01  $PSF<P>) s@25@01))
  :qid |qp.psmValDef4|)))
(declare-const pm@26@01 $PPM)
(assert (forall ((x $Ref)) (!
  (=
    ($PSF.perm_P (as pm@26@01  $PPM) ($SortWrappers.$RefTo$Snap x))
    (ite (= x x@11@01) $Perm.Write $Perm.No))
  :pattern (($PSF.perm_P (as pm@26@01  $PPM) ($SortWrappers.$RefTo$Snap x)))
  :qid |qp.resPrmSumDef5|)))
(push) ; 3
(assert (not (=
  ($PSF.perm_P (as pm@26@01  $PPM) ($SortWrappers.$RefTo$Snap x@11@01))
  $Perm.Write)))
(check-sat)
; unsat
(pop) ; 3
