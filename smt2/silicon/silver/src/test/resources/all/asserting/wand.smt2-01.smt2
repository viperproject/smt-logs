...
(push) ; 2
; [exec]
; package acc(x.f, write) && (asserting (acc(x.f, write)) in x.f == 0) --*
; acc(x.f, write) {
; }
(push) ; 3
...
; [eval] (asserting (acc(x.f, write)) in x.f == 0)
; [eval] x.f == 0
...
(check-sat)
; unknown
; Create MagicWandSnapFunction for wand acc(x.f, write) && (asserting (acc(x.f, write)) in x.f == 0) --* acc(x.f, write)
...
(push) ; 2
; [exec]
; package acc(x.f, write) && (asserting (acc(x.g, write)) in x.f == 0) --*
; acc(x.f, write) {
; }
(push) ; 3
...
; [eval] (asserting (acc(x.g, write)) in x.f == 0)
(check-sat)
; unknown
; [state consolidation]
...
; Create MagicWandSnapFunction for wand acc(x.f, write) && (asserting (acc(x.g, write)) in x.f == 0) --* acc(x.f, write)
