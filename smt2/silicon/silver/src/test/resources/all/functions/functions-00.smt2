...
(push) ; 3
(set-option :timeout 10)
(assert (not (= y@3@00 x@2@00)))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
; (get-info :all-statistics)
(assert (P%trigger ($Snap.first ($Snap.second s@$)) x@2@00))
(pop) ; 2
...
(set-option :timeout 0)
(push) ; 2
(assert (Q%trigger s@$ x@16@00))
(assert (= s@$ ($Snap.combine ($Snap.first s@$) ($Snap.second s@$))))
(assert (not (= x@16@00 $Ref.null)))
(assert (=
  ($Snap.second s@$)
  ($Snap.combine
    ($Snap.first ($Snap.second s@$))
...
(assert (=>
  ...
  (=
    (joined_unfolding@23@00 s@$ x@16@00)
    (+
      ($SortWrappers.$SnapToInt ($Snap.first s@$))
...
(assert (or
  ...
  (not
    (= ($SortWrappers.$SnapTo$Ref ($Snap.first ($Snap.second s@$))) $Ref.null))))
(assert (= result@17@00 (joined_unfolding@23@00 s@$ x@16@00)))
(pop) ; 1
(assert (forall ((s@$ $Snap) (x@16@00 $Ref)) (!
  (=>
...
(set-option :timeout 0)
(push) ; 2
(assert (P%trigger s@$ x@18@00))
(assert (not (= x@18@00 $Ref.null)))
(pop) ; 2
; Joined path conditions
(assert (and (P%trigger s@$ x@18@00) (not (= x@18@00 $Ref.null))))
(assert (= result@19@00 ($SortWrappers.$SnapToInt s@$)))
(pop) ; 1
(assert (forall ((s@$ $Snap) (x@18@00 $Ref)) (!
  (=>
