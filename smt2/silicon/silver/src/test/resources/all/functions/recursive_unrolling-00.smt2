...
(check-sat)
; unknown
; [eval] 1 + (unfolding acc(node(this), write) in (this.next == null ? 0 : length(this.next)))
; [eval] (unfolding acc(node(this), write) in (this.next == null ? 0 : length(this.next)))
(set-option :timeout 0)
(push) ; 2
...
; 0.00s
; (get-info :all-statistics)
; [eval] (this.next == null ? 0 : length(this.next))
; [eval] this.next == null
(set-option :timeout 0)
(push) ; 4
...
(assert (=>
  ...
  (= (joined_unfolding@3@00 s@$ this@0@00) 0)))
; Joined path conditions
(assert (and
...
(assert (or
  (= ($SortWrappers.$SnapTo$Ref ($Snap.first s@$)) $Ref.null)
  (not (= ($SortWrappers.$SnapTo$Ref ($Snap.first s@$)) $Ref.null))))
(assert (= result@1@00 (+ 1 (joined_unfolding@3@00 s@$ this@0@00))))
; [eval] result > 0
(push) ; 2
