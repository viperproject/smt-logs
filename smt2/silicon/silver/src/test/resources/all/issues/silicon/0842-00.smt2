...
; [eval] x.f != null
(declare-const sm@5@00 $FVF<f>)
; Definitional axioms for snapshot map values
(declare-const pm@6@00 $FPM)
(assert (forall ((r $Ref)) (!
  (= ($FVF.perm_f (as pm@6@00  $FPM) r) $Perm.No)
  :pattern (($FVF.perm_f (as pm@6@00  $FPM) r))
  :qid |qp.resPrmSumDef0|)))
(push) ; 5
(assert (not (< $Perm.No ($FVF.perm_f (as pm@6@00  $FPM) x@3@00))))
(check-sat)
; unsat
(pop) ; 5
; 0.00s
...
(assert (and
  (= (one $Snap.unit) (two $Snap.unit))
  (not (= ($FVF.lookup_f (as sm@5@00  $FVF<f>) x@3@00) $Ref.null))))
(check-sat)
; unsat
(pop) ; 2
(pop) ; 1
