...
; [eval] length(this)
(push) ; 2
(assert (length%precondition ($Snap.first s@$) this@3@00))
(pop) ; 2
; Joined path conditions
(assert (length%precondition ($Snap.first s@$) this@3@00))
(assert (< i@4@00 (length ($Snap.first s@$) this@3@00)))
(pop) ; 1
...
(set-option :timeout 0)
(push) ; 2
(assert (Node%trigger ($Snap.first s@$) this@3@00))
(assert (=
  ($Snap.first s@$)
  ($Snap.combine
...
(assert (not (= this@3@00 ($SortWrappers.$SnapTo$Ref ($Snap.first ($Snap.first s@$))))))
(check-sat)
; unknown
(pop) ; 3
; 0.00s
...
; Joined path conditions
(assert (and
  (Node%trigger ($Snap.first s@$) this@3@00)
  (=
    ($Snap.first s@$)
    ($Snap.combine
      ($Snap.first ($Snap.first s@$))
