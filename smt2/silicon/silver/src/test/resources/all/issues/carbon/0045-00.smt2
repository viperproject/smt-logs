...
(check-sat)
; unknown
; [eval] (unfolding acc(Pair(p), write) in p.f + p.g)
(set-option :timeout 0)
(push) ; 2
(assert (Pair%trigger s@$ p@0@00))
(assert (= s@$ ($Snap.combine ($Snap.first s@$) ($Snap.second s@$))))
(assert (not (= p@0@00 $Ref.null)))
; [eval] p.f + p.g
(pop) ; 2
; Joined path conditions
(assert (and
  (Pair%trigger s@$ p@0@00)
