...
; Declaring additional sort wrappers
(declare-fun $SortWrappers.SnapTo$Snap (Snap) $Snap)
(declare-fun $SortWrappers.$SnapToSnap ($Snap) Snap)
(assert (forall ((x Snap)) (!
    (= x ($SortWrappers.$SnapToSnap($SortWrappers.SnapTo$Snap x)))
    :pattern (($SortWrappers.SnapTo$Snap x))
    :qid |$Snap.$SnapToSnapTo$Snap|
    )))
(assert (forall ((x $Snap)) (!
    (= x ($SortWrappers.SnapTo$Snap($SortWrappers.$SnapToSnap x)))
    :pattern (($SortWrappers.$SnapToSnap x))
    :qid |$Snap.SnapTo$SnapToSnap|
    )))
; Declaring additional sort wrappers
...
(declare-fun trigger<Bool> (Snap Int) Bool)
...
(declare-fun loopkup ($Snap $Ref) Snap)
(declare-fun loopkup%limited ($Snap $Ref) Snap)
(declare-fun loopkup%stateless ($Ref) Bool)
(declare-fun loopkup%precondition ($Snap $Ref) Bool)
; Snapshot variable to be used during function verification
(declare-fun s@$ () $Snap)
; Declaring predicate trigger functions
(declare-fun P1%trigger ($Snap $Ref) Bool)
(declare-fun P2%trigger ($Snap Int) Bool)
...
; [exec]
; var address: Ref
(declare-const address@0@01 $Ref)
; [exec]
; var marco: Perm
(declare-const marco@1@01 $Perm)
; [exec]
; var marcoAddress: Ref
(declare-const marcoAddress@2@01 $Ref)
; [exec]
; inhale marco >= none
(declare-const $t@3@01 $Snap)
(assert (= $t@3@01 $Snap.unit))
; [eval] marco >= none
(assert (>= marco@1@01 $Perm.No))
...
(set-option :timeout 0)
(push) ; 3
(assert (not (or (= marco@1@01 $Perm.No) (< $Perm.No marco@1@01))))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
...
(declare-const index@6@01 Int)
(set-option :timeout 0)
(push) ; 3
(assert false)
; [eval] trigger(loopkup(address), index)
; [eval] loopkup(address)
(push) ; 4
(declare-const $k@7@01 $Perm)
(assert ($Perm.isReadVar $k@7@01))
(push) ; 5
(set-option :timeout 10)
(assert (not (= marcoAddress@2@01 address@0@01)))
(check-sat)
; unsat
(pop) ; 5
; 0.00s
...
(set-option :timeout 0)
(push) ; 5
(assert (not (=> (< $Perm.No $k@7@01) (< $Perm.No marco@1@01))))
(check-sat)
; unsat
(pop) ; 5
; 0.00s
...
; 0.00s
; (get-info :all-statistics)
(assert (loopkup%precondition $t@4@01 address@0@01))
(pop) ; 4
...
(assert (loopkup%precondition $t@4@01 address@0@01))
(pop) ; 3
(declare-fun inv@8@01 (Int) Int)
(declare-fun img@9@01 (Int) Bool)
