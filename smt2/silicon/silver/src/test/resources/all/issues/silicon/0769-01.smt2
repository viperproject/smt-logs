...
(declare-const values@2@01 Map<Int~_$Ref>)
(assert (not (= x@1@01 x@0@01)))
; [exec]
; x.values := Map[Int,Ref]()
; [eval] Map[Int,Ref]()
; [exec]
; assert x.values[1].value > 0
; [eval] x.values[1].value > 0
; [eval] x.values[1]
(push) ; 3
(assert (not (Set_in 1 (Map_domain (as Map_empty  Map<Int~_$Ref>)))))
(check-sat)
; unknown
(pop) ; 3
; 0.00s
