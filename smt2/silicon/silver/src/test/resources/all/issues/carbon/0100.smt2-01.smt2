...
(declare-fun Main$MonitorInvariant%trigger ($Snap $Ref) Bool)
...
; [eval] perm(m.mustReleaseBounded$) + perm(m.mustReleaseUnbounded$) == none
; [eval] perm(m.mustReleaseBounded$) + perm(m.mustReleaseUnbounded$)
; [eval] perm(m.mustReleaseBounded$)
; [eval] perm(m.mustReleaseUnbounded$)
; [exec]
...
(check-sat)
; unknown
(assert (Main$MonitorInvariant%trigger $t@8@01 m@4@01))
; [exec]
...
; [exec]
; m.Main$f := n
; [eval] perm(m.mustReleaseBounded$) == none
; [eval] perm(m.mustReleaseBounded$)
; [eval] write - perm(m.mustReleaseBounded$) == write
; [eval] write - perm(m.mustReleaseBounded$)
; [eval] perm(m.mustReleaseBounded$)
; [eval] write - perm(m.mustReleaseBounded$)
; [eval] perm(m.mustReleaseBounded$)
; [eval] (forperm r$_15: Ref [r$_15.mustReleaseUnbounded$] :: false)
(pop) ; 2
(pop) ; 1
