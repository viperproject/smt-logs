...
(push) ; 2
; [exec]
; assert LibB(77, false)
; [eval] LibB(77, false)
(push) ; 3
; [eval] foo(1)
(push) ; 4
(assert (not (foo<Bool> 1)))
(check-sat)
; unsat
(pop) ; 4
; 0.00s
...
(assert (foo<Bool> 1))
(assert (LibB%precondition ($Snap.combine
  $Snap.unit
  ($Snap.combine $Snap.unit $Snap.unit)) 77 false))
(pop) ; 3
; Joined path conditions
(assert (and
  (foo<Bool> 1)
  (LibB%precondition ($Snap.combine
    $Snap.unit
    ($Snap.combine $Snap.unit $Snap.unit)) 77 false)))
(push) ; 3
(assert (not (LibB ($Snap.combine $Snap.unit ($Snap.combine $Snap.unit $Snap.unit)) 77 false)))
(check-sat)
; unsat
(pop) ; 3
; 0.00s
; (get-info :all-statistics)
(assert (LibB ($Snap.combine $Snap.unit ($Snap.combine $Snap.unit $Snap.unit)) 77 false))
(pop) ; 2
(pop) ; 1
