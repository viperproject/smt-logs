...
(set-option :pp.bv_literals false)
(assert (not
 (=> (= (ControlFlow 0 0) 8) (let ((anon6_Then_correct  (=> (and (> (select (|PolymorphicMapType_4930_2596_1212#PolymorphicMapType_4930| ZeroMask) x22 f_7) NoPerm) (= (ControlFlow 0 4) (- 0 3))) false)))
(let ((anon4_correct true))
(let ((anon6_Else_correct  (=> (>= NoPerm (select (|PolymorphicMapType_4930_2596_1212#PolymorphicMapType_4930| ZeroMask) x22 f_7)) (=> (and (state Heap@@28 ZeroMask) (= (ControlFlow 0 5) 1)) anon4_correct))))
(let ((anon5_Then_correct  (=> (and $condInEx (select (|PolymorphicMapType_4909_2468_53#PolymorphicMapType_4909| Heap@@28) x22 $allocated)) (and (=> (= (ControlFlow 0 6) 4) anon6_Then_correct) (=> (= (ControlFlow 0 6) 5) anon6_Else_correct)))))
(let ((anon5_Else_correct  (=> (and (not $condInEx) (= (ControlFlow 0 2) 1)) anon4_correct)))
(let ((anon0_correct  (=> (and (and (state Heap@@28 ZeroMask) (= AssumeFunctionsAbove (- 0 1))) (and AssumePermUpperBound (select (|PolymorphicMapType_4909_2468_53#PolymorphicMapType_4909| Heap@@28) x@@7 $allocated))) (and (=> (= (ControlFlow 0 7) 6) anon5_Then_correct) (=> (= (ControlFlow 0 7) 2) anon5_Else_correct)))))
