  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: if (arg > 0) -- example_4.vpr@30.5--37.6
    if (arg > 0) {
      ...
      // -- Translating statement: res := (None(): Option[Int]) -- example_4.vpr@36.9--36.21
        res := (None(): OptionDomainType int);
        assume state(Heap, Mask);
    }
    assume state(Heap, Mask);
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: response := request(1) -- example_4.vpr@43.5--43.27
    ...
    // -- Inhaling postcondition
      assume (Option_tag(response): int) == 1;
      assume response == (Some(2): OptionDomainType int);
      assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Translating statement: if ((Option_tag(response): Int) == 1) -- example_4.vpr@44.5--60.6
  ...
  // -- Translating statement: response := request(-1) -- example_4.vpr@64.5--64.28
    PreCallHeap := Heap;
    PreCallMask := Mask;
    arg_arg := -1;
    ...
    // -- Inhaling postcondition
      if (arg_arg > 0) {
        assume (Option_tag(response): int) == 1;
        assume response == (Some(2 * arg_arg): OptionDomainType int);
      }
      if (arg_arg < 0) {
        assume (Option_tag(response): int) == 0;
      }
      assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  // -- Translating statement: if ((Option_tag(response): Int) == 1) -- example_4.vpr@66.5--75.6
    if ((Option_tag(response): int) == 1) {
