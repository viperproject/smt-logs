...
function  get_List_head(t_3: ListDomainType): int;

// Translation of domain function get_List_tail
function  get_List_tail(t_3: ListDomainType): ListDomainType;

// Translation of domain function List_tag
function  List_tag(t_3: ListDomainType): int;
...
function  len_1(Heap: HeapType, xs: ListDomainType): int;
function  len'(Heap: HeapType, xs: ListDomainType): int;
axiom (forall Heap: HeapType, xs: ListDomainType ::
  { len_1(Heap, xs) }
  len_1(Heap, xs) == len'(Heap, xs) && dummyFunction(len#triggerStateless(xs))
);
axiom (forall Heap: HeapType, xs: ListDomainType ::
  { len'(Heap, xs) }
...
axiom (forall Heap: HeapType, Mask: MaskType, xs: ListDomainType ::
  { state(Heap, Mask), len_1(Heap, xs) }
  state(Heap, Mask) && AssumeFunctionsAbove < 1 ==> len_1(Heap, xs) == (if (List_tag(xs): int) == 1 then 0 else 1 + len'(Heap, (get_List_tail(xs): ListDomainType)))
);
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translate function body
    Result := (if (List_tag(xs): int) == 1 then 0 else 1 + len_1(Heap, (get_List_tail(xs): ListDomainType)));
  
  // -- Exhaling postcondition (with checking)
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    assert {:msg "  Postcondition of len might not hold. Assertion result >= 0 might not hold. (example_3.vpr@15.13--15.24) [103590]"}
      Result >= 0;
}
...
axiom (forall Heap: HeapType, xs: ListDomainType, pos: int ::
  { at(Heap, xs, pos) }
  at(Heap, xs, pos) == at'(Heap, xs, pos) && dummyFunction(at#triggerStateless(xs, pos))
);
axiom (forall Heap: HeapType, xs: ListDomainType, pos: int ::
  { at'(Heap, xs, pos) }
...
axiom (forall Heap: HeapType, Mask: MaskType, xs: ListDomainType, pos: int ::
  ...
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> at(Heap, xs, pos) == (if (List_tag(xs): int) == 1 || pos < 0 then -1 else (if pos == 0 then (get_List_head(xs): int) else at'(Heap, (get_List_tail(xs): ListDomainType), pos - 1)))
);
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translate function body
    Result := (if (List_tag(xs): int) == 1 || pos < 0 then -1 else (if pos == 0 then (get_List_head(xs): int) else at(Heap, (get_List_tail(xs): ListDomainType), pos - 1)));
}
  ...
  modifies Heap, Mask;
{
  ...
  // -- Exhaling postcondition
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    assert {:msg "  Postcondition of prepend might not hold. Assertion get_List_head(ys) == v might not hold. (example_3.vpr@8.13--8.25) [103591]"}
      (get_List_head(ys): int) == v_2;
    assert {:msg "  Postcondition of prepend might not hold. Assertion get_List_tail(ys) == xs might not hold. (example_3.vpr@9.13--9.26) [103592]"}
...
procedure linear_search(xs: ListDomainType, key_2: int) returns (pos: int)
  modifies Heap, Mask;
{
  var oldHeap: HeapType;
  var oldMask: MaskType;
  ...
  if (*) {
    ...
    // -- Check definedness of at(xs, pos) == key || pos == len(xs)
      ...
      if (!(at(PostHeap, xs, pos) == key_2)) {
        if (*) {
          // Stop execution
          assume false;
        }
      }
    assume at(PostHeap, xs, pos) == key_2 || pos == len_1(PostHeap, xs);
    assume state(PostHeap, PostMask);
    ...
    if (pos == len_1(PostHeap, xs)) {
      ...
      // -- Check definedness of (forall k: Int :: { at(xs, k) } 0 <= k && k < len(xs) ==> at(xs, k) != key)
        if (*) {
          ...
          if (0 <= k_16 && k_16 < len_1(PostHeap, xs)) {
            if (*) {
              // Stop execution
              assume false;
            }
      ...
      assume (forall k_1: int ::
        { at#frame(EmptyFrame, xs, k_1) }
        0 <= k_1 && k_1 < len_1(PostHeap, xs) ==> at(PostHeap, xs, k_1) != key_2
      );
    }
    assume state(PostHeap, PostMask);
    // Stop execution
  ...
  // -- Translating statement: while (!(List_tag(ys) == 1) && get_List_head(ys) != key) -- example_3.vpr@43.5--56.6
    ...
    // -- Before loop head
      ...
      // -- Exhale loop invariant before loop
        ExhaleWellDef0Heap := Heap;
        ExhaleWellDef0Mask := Mask;
        assert {:msg "  Loop invariant 0 <= pos might not hold on entry. Assertion 0 <= pos might not hold. (example_3.vpr@44.19--44.27) [103593]"}
          0 <= pos;
        assert {:msg "  Loop invariant pos + len(ys) == len(xs) might not hold on entry. Assertion pos + len(ys) == len(xs) might not hold. (example_3.vpr@45.19--45.43) [103594]"}
          pos + len_1(Heap, ys) == len_1(Heap, xs);
        if ((List_tag(ys): int) == 1) {
          assert {:msg "  Loop invariant List_tag(ys) == 1 ==> pos == len(xs) might not hold on entry. Assertion pos == len(xs) might not hold. (example_3.vpr@46.19--46.46) [103595]"}
            pos == len_1(Heap, xs);
        }
        if (*) {
          if (0 <= k_4_2) {
            assert {:msg "  Loop invariant (forall k: Int :: { at(ys, k) } 0 <= k ==> at(xs, pos + k) == at(ys, k)) might not hold on entry. Assertion at(xs, pos + k) == at(ys, k) might not hold. (example_3.vpr@47.19--47.72) [103596]"}
              at(Heap, xs, pos + k_4_2) == at(Heap, ys, k_4_2);
          }
          assume false;
        }
        assume (forall k_5_1_1: int ::
          { at#frame(EmptyFrame, ys, k_5_1_1) }
          0 <= k_5_1_1 ==> at(Heap, xs, pos + k_5_1_1) == at(Heap, ys, k_5_1_1)
        );
        if (*) {
          if (0 <= k_6_2 && k_6_2 < pos) {
            assert {:msg "  Loop invariant (forall k: Int :: { at(xs, k) } 0 <= k && k < pos ==> at(xs, k) != key) might not hold on entry. Assertion at(xs, k) != key might not hold. (example_3.vpr@48.19--48.73) [103597]"}
              at(Heap, xs, k_6_2) != key_2;
          }
          assume false;
        }
        assume (forall k_7_1_1: int ::
          { at#frame(EmptyFrame, xs, k_7_1_1) }
          0 <= k_7_1_1 && k_7_1_1 < pos ==> at(Heap, xs, k_7_1_1) != key_2
        );
    ...
    // -- Check definedness of invariant
      if (*) {
        ...
        assume pos + len_1(Heap, ys) == len_1(Heap, xs);
        assume state(Heap, Mask);
        if ((List_tag(ys): int) == 1) {
          assume state(Heap, Mask);
        ...
        assume (forall k_9_1: int ::
          { at#frame(EmptyFrame, ys, k_9_1) }
          0 <= k_9_1 ==> at(Heap, xs, pos + k_9_1) == at(Heap, ys, k_9_1)
        );
        assume state(Heap, Mask);
        assume state(Heap, Mask);
        ...
        assume (forall k_11: int ::
          ...
          0 <= k_11 && k_11 < pos ==> at(Heap, xs, k_11) != key_2
        );
        assume state(Heap, Mask);
        assume false;
      }
    ...
    // -- Check the loop body
      if (*) {
        ...
        assume (forall k_12: int ::
          { at#frame(EmptyFrame, ys, k_12) }
          0 <= k_12 ==> at(Heap, xs, pos + k_12) == at(Heap, ys, k_12)
        );
        assume state(Heap, Mask);
        assume (forall k_13: int ::
          { at#frame(EmptyFrame, xs, k_13) }
          0 <= k_13 && k_13 < pos ==> at(Heap, xs, k_13) != key_2
        );
        assume state(Heap, Mask);
        // Check and assume guard
        assume !((List_tag(ys): int) == 1) && (get_List_head(ys): int) != key_2;
        assume state(Heap, Mask);
  ...
  //     0 <= k ==> at(old_ys, k + 1) == at(ys, k)) -- example_3.vpr@53.9--53.69
            ...
            if (*) {
              if (0 <= k_15) {
                assert {:msg "  Assert might fail. Assertion at(old_ys, k + 1) == at(ys, k) might not hold. (example_3.vpr@53.16--53.69) [103598]"}
                  at(Heap, old_ys, k_15 + 1) == at(Heap, ys, k_15);
              }
              assume false;
            }
            assume (forall k_16_1: int ::
              { at#frame(EmptyFrame, ys, k_16_1) }
              0 <= k_16_1 ==> at(Heap, old_ys, k_16_1 + 1) == at(Heap, ys, k_16_1)
            );
            assume state(Heap, Mask);
          ...
          // -- Translating statement: assert at(old_ys, 0) == at(xs, pos) -- example_3.vpr@54.9--54.44
            ...
            assert {:msg "  Assert might fail. Assertion at(old_ys, 0) == at(xs, pos) might not hold. (example_3.vpr@54.16--54.44) [103599]"}
              at(Heap, old_ys, 0) == at(Heap, xs, pos);
            assume state(Heap, Mask);
        ...
        ExhaleWellDef0Heap := Heap;
        ExhaleWellDef0Mask := Mask;
        assert {:msg "  Loop invariant 0 <= pos might not be preserved. Assertion 0 <= pos might not hold. (example_3.vpr@44.19--44.27) [103600]"}
          0 <= pos;
        assert {:msg "  Loop invariant pos + len(ys) == len(xs) might not be preserved. Assertion pos + len(ys) == len(xs) might not hold. (example_3.vpr@45.19--45.43) [103601]"}
          pos + len_1(Heap, ys) == len_1(Heap, xs);
        if ((List_tag(ys): int) == 1) {
          assert {:msg "  Loop invariant List_tag(ys) == 1 ==> pos == len(xs) might not be preserved. Assertion pos == len(xs) might not hold. (example_3.vpr@46.19--46.46) [103602]"}
            pos == len_1(Heap, xs);
        }
        if (*) {
          if (0 <= k_17) {
            assert {:msg "  Loop invariant (forall k: Int :: { at(ys, k) } 0 <= k ==> at(xs, pos + k) == at(ys, k)) might not be preserved. Assertion at(xs, pos + k) == at(ys, k) might not hold. (example_3.vpr@47.19--47.72) [103603]"}
              at(Heap, xs, pos + k_17) == at(Heap, ys, k_17);
          }
          assume false;
        }
        assume (forall k_18_1: int ::
          { at#frame(EmptyFrame, ys, k_18_1) }
          0 <= k_18_1 ==> at(Heap, xs, pos + k_18_1) == at(Heap, ys, k_18_1)
        );
        if (*) {
          if (0 <= k_19_1 && k_19_1 < pos) {
            assert {:msg "  Loop invariant (forall k: Int :: { at(xs, k) } 0 <= k && k < pos ==> at(xs, k) != key) might not be preserved. Assertion at(xs, k) != key might not hold. (example_3.vpr@48.19--48.73) [103604]"}
              at(Heap, xs, k_19_1) != key_2;
          }
          assume false;
        }
        assume (forall k_20_1_1: int ::
          { at#frame(EmptyFrame, xs, k_20_1_1) }
          0 <= k_20_1_1 && k_20_1_1 < pos ==> at(Heap, xs, k_20_1_1) != key_2
        );
        // Terminate execution
        assume false;
      }
    ...
    // -- Inhale loop invariant after loop, and assume guard
      ...
      assume (forall k_21_1: int ::
        ...
        0 <= k_21_1 ==> at(Heap, xs, pos + k_21_1) == at(Heap, ys, k_21_1)
      );
      assume state(Heap, Mask);
      assume (forall k_22_1: int ::
        { at#frame(EmptyFrame, xs, k_22_1) }
        0 <= k_22_1 && k_22_1 < pos ==> at(Heap, xs, k_22_1) != key_2
      );
      assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  // -- Translating statement: assert !(List_tag(ys) == 1) ==> at(ys, 0) == key -- example_3.vpr@58.5--58.42
    ...
    if (!((List_tag(ys): int) == 1)) {
      ...
      assert {:msg "  Assert might fail. Assertion at(ys, 0) == key might not hold. (example_3.vpr@58.12--58.42) [103605]"}
        at(Heap, ys, 0) == key_2;
    }
    assume state(Heap, Mask);
  ...
  // -- Exhaling postcondition
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    assert {:msg "  Postcondition of linear_search might not hold. Assertion 0 <= pos might not hold. (example_3.vpr@35.13--35.39) [103606]"}
      0 <= pos;
    assert {:msg "  Postcondition of linear_search might not hold. Assertion pos <= len(xs) might not hold. (example_3.vpr@35.13--35.39) [103607]"}
      pos <= len_1(Heap, xs);
    assert {:msg "  Postcondition of linear_search might not hold. Assertion at(xs, pos) == key || pos == len(xs) might not hold. (example_3.vpr@36.13--36.49) [103608]"}
      at(Heap, xs, pos) == key_2 || pos == len_1(Heap, xs);
    if (pos == len_1(Heap, xs)) {
      if (*) {
        if (0 <= k_2_2 && k_2_2 < len_1(Heap, xs)) {
          assert {:msg "  Postcondition of linear_search might not hold. Assertion at(xs, k) != key might not hold. (example_3.vpr@37.13--37.90) [103609]"}
            at(Heap, xs, k_2_2) != key_2;
        }
        assume false;
      }
      assume (forall k_3_1: int ::
        { at#frame(EmptyFrame, xs, k_3_1) }
        0 <= k_3_1 && k_3_1 < len_1(Heap, xs) ==> at(Heap, xs, k_3_1) != key_2
      );
    }
}