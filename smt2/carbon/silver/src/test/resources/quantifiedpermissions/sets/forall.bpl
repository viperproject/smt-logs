...
procedure t1(x: Ref, xs: (Set Ref)) returns ()
  modifies Heap, Mask;
{
  var QPMask: MaskType;
  ...
  // -- Checked inhaling of precondition
    ...
    havoc QPMask;
    assert {:msg "  Contract might not be well-formed. Quantified resource y.f might not be injective. (forall.vpr@7.10--7.53) [1200]"}
  ...
  // -- Translating statement: assert acc(x.f, write) -- forall.vpr@11.4--11.27
    ...
    perm := FullPerm;
    if (perm != NoPerm) {
      assert {:msg "  Assert might fail. There might be insufficient permission to access x.f (forall.vpr@11.11--11.26) [1202]"}
        perm <= AssertMask[x, f_7];
    }
    AssertMask := AssertMask[x, f_7:=AssertMask[x, f_7] - perm];
    assume state(Heap, Mask);
  
  // -- Translating statement: x.f := 5 -- forall.vpr@12.4--12.12
    assert {:msg "  Assignment might fail. There might be insufficient permission to access x.f (forall.vpr@12.4--12.12) [1203]"}
      FullPerm == Mask[x, f_7];
    Heap := Heap[x, f_7:=5];
    assume state(Heap, Mask);
}