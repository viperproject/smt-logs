...
procedure t1_2(x: Ref, xs: (Set Ref)) returns ()
  modifies Heap, Mask;
{
  var QPMask: MaskType;
  ...
  // -- Checked inhaling of precondition
    ...
    havoc QPMask;
    assert {:msg "  Contract might not be well-formed. Quantified resource y.f might not be injective. (forall.vpr@7.10--7.53) [182429]"}
  ...
  // -- Translating statement: x.f := 5 -- forall.vpr@12.4--12.12
    assert {:msg "  Assignment might fail. There might be insufficient permission to access x.f (forall.vpr@12.4--12.12) [182432]"}
      FullPerm == Mask[x, f_7];
    Heap := Heap[x, f_7:=5];
    assume state(Heap, Mask);
}