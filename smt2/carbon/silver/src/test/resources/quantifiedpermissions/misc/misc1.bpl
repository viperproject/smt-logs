...
// Translation of domain function up
function  up(x_37: Ref): Ref;

// Translation of domain function down
function  down(x_37: Ref): Ref;

// Translation of domain function up_inv
function  up_inv(x_37: Ref): Ref;

// Translation of domain function down_inv
function  down_inv(x_37: Ref): Ref;
...
axiom (forall r_1: Ref ::
  { (temp(r_1): Ref) }
  (temp_inv((temp(r_1): Ref)): Ref) == r_1 && (if (is_ghost(r_1): bool) then (temp(r_1): Ref) == r_1 else (heap((temp(r_1): Ref)): int) == -3)
);
...
// Translation of domain function rds
function  rds_1(i_6: int): Perm;

// Translation of domain axiom some
axiom (forall i: int ::
  { (rds_1(i): Perm) }
  (rds_1(0): Perm) == NoPerm && ((rds_1(i): Perm) == real(i) * (rd_1(): Perm) && (i >= 0 ==> (rds_1(i): Perm) < FullPerm))
);
...
// ==================================================
// Translation of method clone
// ==================================================

procedure clone(data_1: Ref, count: Ref, ghost: Ref) returns ()
  modifies Heap, Mask;
{
  var perm: Perm;
  ...
  // -- Checked inhaling of precondition
    ...
    perm := (rd_1(): Perm);
    assert {:msg "  Contract might not be well-formed. Fraction rd() might be negative. (misc1.vpr@34.15--39.35) [176208]"}
      perm >= NoPerm;
    assume perm > NoPerm ==> data_1 != null;
    Mask := Mask[data_1, val:=Mask[data_1, val] + perm];
    assume state(Heap, Mask);
    perm := (rd_1(): Perm);
    assert {:msg "  Contract might not be well-formed. Fraction rd() might be negative. (misc1.vpr@34.15--39.35) [176209]"}
      perm >= NoPerm;
  ...
  // -- Translating statement: inhale (forall d: Ref ::false ==> acc(d.val, none)) -- misc1.vpr@43.3--43.53
    ...
    // -- Check definedness of (forall d: Ref ::false ==> acc(d.val, none))
      if (*) {
        assume false;
      }
    havoc QPMask;
  ...
  // -- Translating statement: inhale acc(temp(ghost).val, write - rds(t)) -- misc1.vpr@45.3--45.46
    perm := FullPerm - (rds_1(t_2): Perm);
    assert {:msg "  Inhale might fail. Fraction write - rds(t) might be negative. (misc1.vpr@45.10--45.46) [176211]"}
      perm >= NoPerm;
    assume perm > NoPerm ==> (temp(ghost): Ref) != null;
    Mask := Mask[(temp(ghost): Ref), val:=Mask[(temp(ghost): Ref), val] + perm];
    assume state(Heap, Mask);
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Translating statement: inhale t < 1 -- misc1.vpr@46.10--46.15
    assume t_2 < 1;
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Translating statement: if (t >= 0) -- misc1.vpr@48.3--55.4
    if (t_2 >= 0) {
      
      // -- Translating statement: exhale acc(up(data).val, write - rds(t + 1) -
  //   (write - rds(t + 1) < perm(temp(data).val) ?
  //     write - rds(t + 1) :
  //     perm(temp(data).val))) -- misc1.vpr@49.5--54.47
        ExhaleWellDef0Heap := Heap;
        ExhaleWellDef0Mask := Mask;
        perm := FullPerm - (rds_1(t_2 + 1): Perm) - (if FullPerm - (rds_1(t_2 + 1): Perm) < Mask[(temp(data_1): Ref), val] then FullPerm - (rds_1(t_2 + 1): Perm) else Mask[(temp(data_1): Ref), val]);
        assert {:msg "  Exhale might fail. Fraction write - rds(t + 1) - (write - rds(t + 1) < perm(temp(data).val) ? write - rds(t + 1) : perm(temp(data).val)) might be negative. (misc1.vpr@49.12--54.47) [176212]"}
          perm >= NoPerm;
        if (perm != NoPerm) {
          assert {:msg "  Exhale might fail. There might be insufficient permission to access up(data).val (misc1.vpr@49.12--54.47) [176213]"}
            perm <= Mask[(up(data_1): Ref), val];
        }
        Mask := Mask[(up(data_1): Ref), val:=Mask[(up(data_1): Ref), val] - perm];
        // Finish exhale
        havoc ExhaleHeap;
