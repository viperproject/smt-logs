...
function  rds(i_79: int): Perm;

// Translation of domain axiom some
axiom (forall i: int ::
  { (rds(i): Perm) }
  (rds(0): Perm) == NoPerm && ((rds(i): Perm) == real(i) * (rd(): Perm) && (i >= 0 ==> (rds(i): Perm) < FullPerm))
);
...
procedure clone(data_1: Ref, count_1: Ref, ghost: Ref) returns ()
  modifies Heap, Mask;
{
  var perm: Perm;
  ...
  // -- Checked inhaling of precondition
    ...
    perm := (rd(): Perm);
    assert {:msg "  Contract might not be well-formed. Fraction rd() might be negative. (misc1.vpr@34.15--39.35) [70117]"}
      perm >= NoPerm;
    assume perm > NoPerm ==> data_1 != null;
    Mask := Mask[data_1, val:=Mask[data_1, val] + perm];
    assume state(Heap, Mask);
    perm := (rd(): Perm);
    assert {:msg "  Contract might not be well-formed. Fraction rd() might be negative. (misc1.vpr@34.15--39.35) [70118]"}
      perm >= NoPerm;
  ...
  // -- Translating statement: inhale acc(temp(ghost).val, write - rds(t)) -- misc1.vpr@45.3--45.46
    perm := FullPerm - (rds(t_2): Perm);
    assert {:msg "  Inhale might fail. Fraction write - rds(t) might be negative. (misc1.vpr@45.10--45.46) [70120]"}
      perm >= NoPerm;
  ...
  //     perm(temp(data).val))) -- misc1.vpr@49.5--54.47
        ...
        assert {:msg "  Exhale might fail. Fraction write - rds(t + 1) - (write - rds(t + 1) < perm(temp(data).val) ? write - rds(t + 1) : perm(temp(data).val)) might be negative. (misc1.vpr@49.12--54.47) [70121]"}
          perm >= NoPerm;
        if (perm != NoPerm) {
          assert {:msg "  Exhale might fail. There might be insufficient permission to access up(data).val (misc1.vpr@49.12--54.47) [70122]"}
