...
// 
// Date:         2025-01-27 03:37:50
// Tool:         carbon 1.0
// Arguments: :  --disableCaching --boogieExe /home/runner/.dotnet/tools/boogie --timeout 10 --print /home/runner/work/smt-logs/smt-logs/carbon/../smt2/carbon/silver/src/test/resources/quantifiedpermissions/issues/issue_0062.bpl --boogieOpt /proverLog:/home/runner/work/smt-logs/smt-logs/carbon/../smt2/carbon/silver/src/test/resources/quantifiedpermissions/issues/issue_0062-@PROC@.smt2 --ignoreFile dummy-file-to-prevent-cli-parser-from-complaining-about-missing-file-name.silver
// Dependencies:
...
function  neverTriggered1(q_1_1: Ref): bool;
...
procedure test01(xs: (Set Ref), x: Ref, y: Ref, z: Ref) returns ()
  modifies Heap, Mask;
{
  var perm: Perm;
  ...
  // -- Translating statement: assert x != y -- issue_0062.vpr@10.3--10.16
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    assert {:msg "  Assert might fail. Assertion x != y might not hold. (issue_0062.vpr@10.10--10.16) [175092]"}
      x != y;
    assume state(Heap, Mask);
}
...
procedure test02(xs: (Set Ref), x: Ref, y: Ref, z: Ref) returns ()
  modifies Heap, Mask;
{
  var QPMask: MaskType;
  var oldHeap: HeapType;
  ...
  // -- Checked inhaling of precondition
    ...
    // -- Check definedness of (forall q: Ref :: { (q in xs) } (q in xs) ==> acc(q.f, write))
      if (*) {
        assume false;
      }
    havoc QPMask;
    assert {:msg "  Contract might not be well-formed. Quantified resource q.f might not be injective. (issue_0062.vpr@15.12--15.49) [175093]"}
      (forall q_1_1: Ref, q_1_2: Ref ::
      
      (((q_1_1 != q_1_2 && xs[q_1_1]) && xs[q_1_2]) && NoPerm < FullPerm) && NoPerm < FullPerm ==> q_1_1 != q_1_2
    );
    
    // -- Define Inverse Function
      assume (forall q_1_1: Ref ::
        { Heap[q_1_1, f_7] } { QPMask[q_1_1, f_7] } { xs[q_1_1] }
        xs[q_1_1] && NoPerm < FullPerm ==> qpRange1(q_1_1) && invRecv1(q_1_1) == q_1_1
      );
  ...
  // -- Translating statement: assert x != y -- issue_0062.vpr@18.3--18.16
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    assert {:msg "  Assert might fail. Assertion x != y might not hold. (issue_0062.vpr@18.10--18.16) [175094]"}
      x != y;
    assume state(Heap, Mask);
}
...
procedure m_17(S: (Seq Ref), a_2: int, b_24: int) returns ()
  modifies Heap, Mask;
{
  var i_3: int;
  var j_15: int;
  ...
  // -- Checked inhaling of precondition
    ...
    // -- Check definedness of (forall i: Int, j: Int :: { S[i], S[j] } 0 <= i && (i < j && j < |S|) ==> S[i] != S[j])
      if (*) {
        if (0 <= i_3 && (i_3 < j_15 && j_15 < Seq#Length(S))) {
          assert {:msg "  Contract might not be well-formed. Index S[i] into S might be negative. (issue_0062.vpr@22.12--22.80) [175095]"}
            i_3 >= 0;
          assert {:msg "  Contract might not be well-formed. Index S[i] into S might exceed sequence length. (issue_0062.vpr@22.12--22.80) [175096]"}
            i_3 < Seq#Length(S);
          assert {:msg "  Contract might not be well-formed. Index S[j] into S might be negative. (issue_0062.vpr@22.12--22.80) [175097]"}
            j_15 >= 0;
          assert {:msg "  Contract might not be well-formed. Index S[j] into S might exceed sequence length. (issue_0062.vpr@22.12--22.80) [175098]"}
            j_15 < Seq#Length(S);
        }
        assume false;
      }
    ...
    // -- Check definedness of (forall i: Int :: { (i in [0..|S|)) } { S[i] } (i in [0..|S|)) ==> acc(S[i].f, write))
      if (*) {
        if (Seq#Contains(Seq#Range(0, Seq#Length(S)), i_4)) {
          assert {:msg "  Contract might not be well-formed. Index S[i] into S might be negative. (issue_0062.vpr@23.12--23.57) [175099]"}
            i_4 >= 0;
          assert {:msg "  Contract might not be well-formed. Index S[i] into S might exceed sequence length. (issue_0062.vpr@23.12--23.57) [175100]"}
            i_4 < Seq#Length(S);
        }
        assume false;
      }
    havoc QPMask;
    assert {:msg "  Contract might not be well-formed. Quantified resource S[i].f might not be injective. (issue_0062.vpr@23.12--23.57) [175101]"}
    ...
    // -- Define Inverse Function
      assume (forall i_3_2: int ::
        ...
        Seq#Contains(Seq#Range(0, Seq#Length(S)), i_3_2) && NoPerm < FullPerm ==> qpRange2(Seq#Index(S, i_3_2)) && invRecv2(Seq#Index(S, i_3_2)) == i_3_2
      );
      assume (forall o_9: Ref ::
        { invRecv2(o_9) }
    ...
    // -- Define permissions
      assume (forall o_9: Ref ::
        { QPMask[o_9, f_7] }
        ((Seq#Contains(Seq#Range(0, Seq#Length(S)), invRecv2(o_9)) && NoPerm < FullPerm) && qpRange2(o_9) ==> (NoPerm < FullPerm ==> Seq#Index(S, invRecv2(o_9)) == o_9) && QPMask[o_9, f_7] == Mask[o_9, f_7] + FullPerm) && (!((Seq#Contains(Seq#Range(0, Seq#Length(S)), invRecv2(o_9)) && NoPerm < FullPerm) && qpRange2(o_9)) ==> QPMask[o_9, f_7] == Mask[o_9, f_7])
      );
      assume (forall <A, B> o_9: Ref, f_5: (Field A B) ::
        { Mask[o_9, f_5] } { QPMask[o_9, f_5] }
        f_5 != f_7 ==> Mask[o_9, f_5] == QPMask[o_9, f_5]
  ...
  // -- Translating statement: assert S[a] != S[b] -- issue_0062.vpr@25.3--25.23
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    
    // -- Check definedness of S[a] != S[b]
      assert {:msg "  Assert might fail. Index S[a] into S might be negative. (issue_0062.vpr@25.10--25.22) [175102]"}
        a_2 >= 0;
      assert {:msg "  Assert might fail. Index S[a] into S might exceed sequence length. (issue_0062.vpr@25.10--25.22) [175103]"}
        a_2 < Seq#Length(S);
      assert {:msg "  Assert might fail. Index S[b] into S might be negative. (issue_0062.vpr@25.10--25.22) [175104]"}
        b_24 >= 0;
      assert {:msg "  Assert might fail. Index S[b] into S might exceed sequence length. (issue_0062.vpr@25.10--25.22) [175105]"}
        b_24 < Seq#Length(S);
    assert {:msg "  Assert might fail. Assertion S[a] != S[b] might not hold. (issue_0062.vpr@25.10--25.22) [175106]"}
      Seq#Index(S, a_2) != Seq#Index(S, b_24);
    assume state(Heap, Mask);
}