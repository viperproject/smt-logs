  ...
  modifies Heap, Mask;
{
  ...
  // -- Checked inhaling of precondition
    ...
    // -- Check definedness of (forall i: Int ::0 <= i && (i < M && 0 < N) ==> M * N == |ar|)
      if (*) {
        assume false;
      }
  ...
  // -- Translating statement: inhale false -- testNestedSingleIC.vpr@69.3--69.15
    assume false;
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: Ref__loop_main_20(diz, current_thread_id, ar, N, M) -- testNestedSingleIC.vpr@102.3--102.54
    PreCallHeap := Heap;
    PreCallMask := Mask;
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: diz := new(Ref__Integer_value) -- testNestedSingleIC.vpr@110.3--110.33
    havoc freshObj;
  ...
  // -- Translating statement: sys__result := diz -- testNestedSingleIC.vpr@111.3--111.21
    sys__result := diz;
    assume state(Heap, Mask);
  ...
  // -- Translating statement: inhale false -- testNestedSingleIC.vpr@113.3--113.15
    assume false;
    assume state(Heap, Mask);
    assume state(Heap, Mask);
