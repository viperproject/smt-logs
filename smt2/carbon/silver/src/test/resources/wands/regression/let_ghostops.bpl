  ...
  modifies Heap, Mask;
{
  ...
  // -- Check definedness of function body
    ...
    // -- Check definedness of (unfolding acc(F(x), write) in x.f)
      ...
      assume state(UnfoldingHeap, UnfoldingMask);
      assert {:msg "  Function might not be well-formed. There might be insufficient permission to access x.f (let_ghostops.vpr@11.1--13.31) [126526]"}
  ...
  modifies Heap, Mask;
{
  ...
  // } -- let_ghostops.vpr@16.3--22.6
    ...
    if (b_1_1) {
      ...
      // -- Translating statement: unfold acc(Pair(x), write) -- let_ghostops.vpr@21.9--21.28
        ...
        // -- Transfer of acc(Pair(x), write)
          ...
          assert {:msg "  Unfolding Pair(x) might fail. Fraction acc(Pair(x), write) might be negative. (let_ghostops.vpr@21.9--21.28) [126528]"}
            ...
            if (((b_1_1 && b_2_1) && accVar2) && neededTransfer > 0.000000000) {
              ...
              if (takeTransfer > 0.000000000) {
                ...
                assume (forall <A, B> o_15: Ref, f_20: (Field A B) ::
                  { newPMask[o_15, f_20] }
                  Heap[null, wand#sm(x, x, x, FullPerm)][o_15, f_20] || Heap[null, Pair#sm(x)][o_15, f_20] ==> newPMask[o_15, f_20]
                );
                Heap := Heap[null, wand#sm(x, x, x, FullPerm):=newPMask];
              }
            }
          assert {:msg "  Unfolding Pair(x) might fail. There might be insufficient permission to access Pair(x) (let_ghostops.vpr@21.9--21.28) [126529]"}
    ...
    // -- Transfer of acc(x.f, write)
      ...
      assert {:msg "  Packaging wand might fail. Fraction acc(x.f, write) might be negative. (let_ghostops.vpr@16.3--22.6) [126532]"}
        neededTransfer >= 0.000000000;
      ...
      assert {:msg "  Packaging wand might fail. There might be insufficient permission to access x.f (let_ghostops.vpr@16.3--22.6) [126533]"}
