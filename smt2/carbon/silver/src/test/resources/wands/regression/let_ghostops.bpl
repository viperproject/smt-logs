...
type WandType_wand;
function  wand(arg1: Ref, arg2: Ref, arg3: Ref, arg4: Perm): Field WandType_wand int;
function  wand#sm(arg1: Ref, arg2: Ref, arg3: Ref, arg4: Perm): Field WandType_wand PMaskType;
function  wand#ft(arg1: Ref, arg2: Ref, arg3: Ref, arg4: Perm): Field WandType_wand FrameType;
axiom (forall arg1: Ref, arg2: Ref, arg3: Ref, arg4: Perm ::
...
axiom (forall arg1: Ref, arg2: Ref, arg3: Ref, arg4: Perm, arg1_2: Ref, arg2_2: Ref, arg3_2: Ref, arg4_2: Perm ::
...
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  { state(Heap, Mask), fun(Heap, x) } { state(Heap, Mask), fun#triggerStateless(x), F#trigger(Heap, F(x)) }
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> fun(Heap, x) == Heap[x, f_7]
);
...
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  ...
  state(Heap, Mask) ==> fun'(Heap, x) == fun#frame(Heap[null, F(x)], x)
);
  ...
  modifies Heap, Mask;
{
  ...
  // -- Check definedness of function body
    ...
    // -- Check definedness of (unfolding acc(F(x), write) in x.f)
      ...
      perm := FullPerm;
      assert {:msg "  Function might not be well-formed. There might be insufficient permission to access F(x) (let_ghostops.vpr@11.1--13.31) [208045]"}
      ...
      assume state(UnfoldingHeap, UnfoldingMask);
      assert {:msg "  Function might not be well-formed. There might be insufficient permission to access x.f (let_ghostops.vpr@11.1--13.31) [208046]"}
  ...
  modifies Heap, Mask;
{
  ...
  // } -- let_ghostops.vpr@16.3--22.6
    ...
    if (b_1_1) {
      ...
      // -- Translating statement: unfold acc(Pair(x), write) -- let_ghostops.vpr@21.9--21.28
        ...
        // -- Transfer of acc(Pair(x), write)
          ...
          assert {:msg "  Unfolding Pair(x) might fail. Fraction acc(Pair(x), write) might be negative. (let_ghostops.vpr@21.9--21.28) [208048]"}
            ...
            if (((b_1_1 && b_2_1) && accVar2) && neededTransfer > 0.000000000) {
              ...
              if (takeTransfer > 0.000000000) {
                ...
                assume (forall <A, B> o_5: Ref, f_11: (Field A B) ::
                  { newPMask[o_5, f_11] }
                  Heap[null, wand#sm(x, x, x, FullPerm)][o_5, f_11] || Heap[null, Pair#sm(x)][o_5, f_11] ==> newPMask[o_5, f_11]
                );
                Heap := Heap[null, wand#sm(x, x, x, FullPerm):=newPMask];
              }
            }
          assert {:msg "  Unfolding Pair(x) might fail. There might be insufficient permission to access Pair(x) (let_ghostops.vpr@21.9--21.28) [208049]"}
    ...
    b_1_1 := b_1_1 && xf_1 == Ops_1Heap[x, f_7];
    ...
    // -- Transfer of acc(x.f, write)
      ...
      assert {:msg "  Packaging wand might fail. Fraction acc(x.f, write) might be negative. (let_ghostops.vpr@16.3--22.6) [208052]"}
        neededTransfer >= 0.000000000;
      ...
      // -- transfer code for top state of stack
        ...
        if ((((b_1_1 && b_1_1) && b_4) && accVar2) && neededTransfer > 0.000000000) {
          ...
          if (takeTransfer > 0.000000000) {
            ...
            Heap := Heap[null, wand#sm(x, x, x, FullPerm):=Heap[null, wand#sm(x, x, x, FullPerm)][x, f_7:=true]];
          }
        }
      assert {:msg "  Packaging wand might fail. There might be insufficient permission to access x.f (let_ghostops.vpr@16.3--22.6) [208053]"}
      ...
      // -- Creating state which is the sum of the two previously built up states
        ...
        b_5 := b_5 && state(Result_1Heap, Result_1Mask);
      b_1_1 := b_1_1 && b_5;
    Mask := Mask[null, wand(x, x, x, FullPerm):=Mask[null, wand(x, x, x, FullPerm)] + FullPerm];
    assume state(Heap, Mask);
    assume state(Heap, Mask);
    assume state(Heap, Mask);
}