  ...
  modifies Heap, Mask;
{
  ...
  // -- Check definedness of function body
    ...
    // -- Check definedness of (unfolding acc(Pair(p), write) in p.f + p.g)
      ...
      assume state(UnfoldingHeap, UnfoldingMask);
      assume state(UnfoldingHeap, UnfoldingMask);
      assert {:msg "  Function might not be well-formed. There might be insufficient permission to access p.f (folding.vpr@11.1--15.2) [130244]"}
        HasDirectPerm(UnfoldingMask, p_1, f_7);
      assert {:msg "  Function might not be well-formed. There might be insufficient permission to access p.g (folding.vpr@11.1--15.2) [130245]"}
  ...
  modifies Heap, Mask;
{
  ...
  // } -- folding.vpr@34.3--34.53
    ...
    // -- Transfer of acc(Pair(p), write)
      ...
      assert {:msg "  Packaging wand might fail. Fraction acc(Pair(p), write) might be negative. (folding.vpr@34.3--34.53) [130247]"}
        ...
        if ((((b_1_1 && b_1_1) && b_2_1) && accVar2) && neededTransfer > 0.000000000) {
          ...
          if (takeTransfer > 0.000000000) {
            ...
            assume (forall <A, B> o_15: Ref, f_20: (Field A B) ::
              { newPMask[o_15, f_20] }
              Heap[null, wand#sm(true, p_1, FullPerm, p_1, 0)][o_15, f_20] || Heap[null, Pair#sm(p_1)][o_15, f_20] ==> newPMask[o_15, f_20]
            );
            Heap := Heap[null, wand#sm(true, p_1, FullPerm, p_1, 0):=newPMask];
          }
        }
      assert {:msg "  Packaging wand might fail. There might be insufficient permission to access Pair(p) (folding.vpr@34.3--34.53) [130248]"}
        (b_1_1 && b_1_1) && b_2_1 ==> neededTransfer == 0.000000000 && Used_1Mask[null, Pair(arg_1)] == initNeededTransfer;
    ...
    assert {:msg "  Packaging wand might fail. Assertion sum(p) > 0 might not hold. (folding.vpr@34.3--34.53) [130250]"}
      (b_1_1 && b_1_1) && b_2_1 ==> sum(ResultHeap, p_1) > 0;
    assume state(Heap, Mask);
