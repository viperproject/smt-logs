...
// ==================================================
// Translation of function x$3
// ==================================================

// Uninterpreted function definitions
function  x$3(Heap: HeapType): bool;
function  x$3'(Heap: HeapType): bool;
axiom (forall Heap: HeapType ::
  { x$3(Heap) }
  x$3(Heap) == x$3'(Heap) && dummyFunction(x$3#triggerStateless())
);
axiom (forall Heap: HeapType ::
  { x$3'(Heap) }
  dummyFunction(x$3#triggerStateless())
);

// Definitional axiom
axiom (forall Heap: HeapType, Mask: MaskType ::
  { state(Heap, Mask), x$3(Heap) }
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> x$3(Heap)
);

// Framing axioms
function  x$3#frame(frame: FrameType): bool;
axiom (forall Heap: HeapType, Mask: MaskType ::
  { state(Heap, Mask), x$3'(Heap) }
  state(Heap, Mask) ==> x$3'(Heap) == x$3#frame(EmptyFrame)
);

// Trigger function (controlling recursive postconditions)
function  x$3#trigger(frame: FrameType): bool;

// State-independent trigger function
function  x$3#triggerStateless(): bool;

// Check contract well-formedness and postcondition
procedure x$3#definedness() returns (Result: bool)
  modifies Heap, Mask;
{
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: x := true -- collision.vpr@5.5--5.24
    x := true;
    assume state(Heap, Mask);
  ...
  // -- Translating statement: x$2 := true -- collision.vpr@5.5--5.24
    x$2 := true;
    assume state(Heap, Mask);
