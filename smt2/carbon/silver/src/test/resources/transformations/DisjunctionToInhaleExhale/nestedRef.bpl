...
function  NonDet1(Heap: HeapType, vari: bool): bool;
function  NonDet1'(Heap: HeapType, vari: bool): bool;
axiom (forall Heap: HeapType, vari: bool ::
  { NonDet1(Heap, vari) }
  NonDet1(Heap, vari) == NonDet1'(Heap, vari) && dummyFunction(NonDet1#triggerStateless(vari))
);
axiom (forall Heap: HeapType, vari: bool ::
  { NonDet1'(Heap, vari) }
  dummyFunction(NonDet1#triggerStateless(vari))
);

// Definitional axiom
axiom (forall Heap: HeapType, Mask: MaskType, vari: bool ::
  { state(Heap, Mask), NonDet1(Heap, vari) }
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> NonDet1(Heap, vari)
);

// Framing axioms
function  NonDet1#frame(frame: FrameType, vari: bool): bool;
axiom (forall Heap: HeapType, Mask: MaskType, vari: bool ::
  { state(Heap, Mask), NonDet1'(Heap, vari) }
  state(Heap, Mask) ==> NonDet1'(Heap, vari) == NonDet1#frame(EmptyFrame, vari)
);

// Trigger function (controlling recursive postconditions)
function  NonDet1#trigger(frame: FrameType, vari: bool): bool;

// State-independent trigger function
function  NonDet1#triggerStateless(vari: bool): bool;

// Check contract well-formedness and postcondition
procedure NonDet1#definedness(vari: bool) returns (Result: bool)
  modifies Heap, Mask;
{
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: m := true -- nestedRef.vpr@11.4--11.23
    m_17 := true;
    assume state(Heap, Mask);
  ...
  //     [(NonDet1(k) ? k : m), k || m]) -- nestedRef.vpr@13.4--13.66
    ...
    assert {:msg "  Assert might fail. Assertion (exists k: Bool :: { NonDet1(k) } i && (k || m)) might not hold. (nestedRef.vpr@13.11--13.65) [92883]"}
      (exists k_1_1: bool ::
      { NonDet1#frame(EmptyFrame, k_1_1) }
      i && (k_1_1 || m_17)
    );
    assume state(Heap, Mask);
}