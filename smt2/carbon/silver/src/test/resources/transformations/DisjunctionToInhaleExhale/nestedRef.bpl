  ...
  modifies Heap, Mask;
{
  ...
  //     [(NonDet1(k) ? k : m), k || m]) -- nestedRef.vpr@13.4--13.66
    ...
    assert {:msg "  Assert might fail. Assertion (exists k: Bool :: { NonDet1(k) } i && (k || m)) might not hold. (nestedRef.vpr@13.11--13.65) [189374]"}
      (exists k_1: bool ::
      { NonDet1#frame(EmptyFrame, k_1) }
      i && (k_1 || m_18)
    );
    assume state(Heap, Mask);
}