...
// Uninterpreted function definitions
function  bar(Heap: HeapType, y: int): int;
function  bar'(Heap: HeapType, y: int): int;
axiom (forall Heap: HeapType, y: int ::
  { bar(Heap, y) }
  bar(Heap, y) == bar'(Heap, y) && dummyFunction(bar#triggerStateless(y))
);
axiom (forall Heap: HeapType, y: int ::
  { bar'(Heap, y) }
  dummyFunction(bar#triggerStateless(y))
);

// Definitional axiom
axiom (forall Heap: HeapType, Mask: MaskType, y: int ::
  { state(Heap, Mask), bar(Heap, y) }
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> bar(Heap, y) == y - 2
);

// Framing axioms
function  bar#frame(frame: FrameType, y: int): int;
axiom (forall Heap: HeapType, Mask: MaskType, y: int ::
  { state(Heap, Mask), bar'(Heap, y) }
  state(Heap, Mask) ==> bar'(Heap, y) == bar#frame(EmptyFrame, y)
);

// Trigger function (controlling recursive postconditions)
function  bar#trigger(frame: FrameType, y: int): bool;

// State-independent trigger function
function  bar#triggerStateless(y: int): int;

// Check contract well-formedness and postcondition
procedure bar#definedness(y: int) returns (Result: int)
  modifies Heap, Mask;
{
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: x := 5 -- complex.vpr@8.5--8.20
    x := 5;
    assume state(Heap, Mask);
  
  // -- Translating statement: assert foo(5) > bar(x - 1) -- complex.vpr@9.5--9.35
    ExhaleWellDef0Mask := Mask;
    ExhaleWellDef0Heap := Heap;
    
    // -- Check definedness of foo(5) > bar(x - 1)
      if (*) {
        // Stop execution
        assume false;
      }
