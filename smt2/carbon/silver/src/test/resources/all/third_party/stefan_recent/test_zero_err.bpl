...
const unique zero__x: Field NormalField (Seq Ref);
axiom !IsPredicateField(zero__x);
axiom !IsWandField(zero__x);
const unique Ref__Integer_value: Field NormalField int;
axiom !IsPredicateField(Ref__Integer_value);
axiom !IsWandField(Ref__Integer_value);

// ==================================================
// Translation of method zero__main_main
// ==================================================

procedure zero__main_main(diz: Ref, current_thread_id: int, tcount: int, gsize: int, tid: int, gid: int, lid: int) returns ()
  modifies Heap, Mask;
{
  ...
  // -- Checked inhaling of precondition
    ...
    perm := wildcard;
    assume diz != null;
    Mask := Mask[diz, zero__x:=Mask[diz, zero__x] + perm];
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  if (*) {
    ...
    perm := wildcard;
    assume diz != null;
    PostMask := PostMask[diz, zero__x:=PostMask[diz, zero__x] + perm];
    assume state(PostHeap, PostMask);
    assume state(PostHeap, PostMask);
    ...
    PostMask := PostMask[Seq#Index(PostHeap[diz, zero__x], tid), Ref__Integer_value:=PostMask[Seq#Index(PostHeap[diz, zero__x], tid), Ref__Integer_value] + perm];
    assume state(PostHeap, PostMask);
    assume state(PostHeap, PostMask);
  ...
  // -- Translating statement: __last_barrier := 0 -- test_zero_err.vpr@37.3--37.22
    __last_barrier := 0;
    assume state(Heap, Mask);
  ...
  // -- Translating statement: __flatten_2 := 1 -- test_zero_err.vpr@39.3--39.19
    __flatten_2 := 1;
    assume state(Heap, Mask);
...
// ==================================================
// Translation of method zero__main_barrier
// ==================================================

procedure zero__main_barrier(diz: Ref, current_thread_id: int, tcount: int, gsize: int, tid: int, gid: int, lid: int, this_barrier: int, last_barrier: int) returns (sys__result: int)
  modifies Heap, Mask;
{
  var wildcard: real where wildcard > NoPerm;
  var perm: Perm;
  ...
  // -- Translating statement: inhale false -- test_zero_err.vpr@67.3--67.15
    assume false;
    assume state(Heap, Mask);
    assume state(Heap, Mask);
