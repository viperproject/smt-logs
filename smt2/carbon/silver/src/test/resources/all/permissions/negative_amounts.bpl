...
procedure test01(x: Ref, k: Perm) returns ()
  modifies Heap, Mask;
{
  var perm: Perm;
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: inhale k < write -- negative_amounts.vpr@20.23--20.32
    assume k < FullPerm;
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Translating statement: k := -1 * k -- negative_amounts.vpr@21.3--21.16
    k := real(-1) * k;
    assume state(Heap, Mask);
