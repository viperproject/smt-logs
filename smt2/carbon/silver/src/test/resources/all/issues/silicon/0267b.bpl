...
function  $(Heap: HeapType, n: Ref): NodeDomainType;
function  $'(Heap: HeapType, n: Ref): NodeDomainType;
axiom (forall Heap: HeapType, n: Ref ::
  { $(Heap, n) }
  $(Heap, n) == $'(Heap, n) && dummyFunction($#triggerStateless(n))
);
axiom (forall Heap: HeapType, n: Ref ::
  { $'(Heap, n) }
  dummyFunction($#triggerStateless(n))
);
...
function  $#trigger(frame: FrameType, n: Ref): bool;

// State-independent trigger function
function  $#triggerStateless(n: Ref): NodeDomainType;

// Check contract well-formedness and postcondition
procedure $#definedness(n: Ref) returns (Result: NodeDomainType)
  modifies Heap, Mask;
{
  var perm: Perm;
...
// ==================================================
// Translation of function purify_graph
// ==================================================
  ...
  modifies Heap, Mask;
{
  ...
  // -- Inhaling precondition (with checking)
    ...
    // -- Check definedness of (forall n: Ref :: { (n in nodes) } (n in nodes) ==> acc(n.m, write))
      if (*) {
        assume false;
      }
    havoc QPMask;
