...
function  bar(Heap: HeapType, x: Ref): bool;
function  bar'(Heap: HeapType, x: Ref): bool;
...
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  { state(Heap, Mask), bar(Heap, x) }
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> bar(Heap, x)
);

// Framing axioms
function  bar#frame(frame: FrameType, x: Ref): bool;
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  { state(Heap, Mask), bar'(Heap, x) }
  state(Heap, Mask) ==> bar'(Heap, x) == bar#frame(FrameFragment(Heap[x, f_7]), x)
);
...
function  bar#triggerStateless(x: Ref): bool;

// Check contract well-formedness and postcondition
procedure bar#definedness(x: Ref) returns (Result: bool)
  modifies Heap, Mask;
{
  var perm: Perm;
