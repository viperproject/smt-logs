...
// ==================================================
// Translation of function idx00
// ==================================================

// Uninterpreted function definitions
function  idx00(Heap: HeapType, a_2: int, b_24: int, c: int, n: int): int;
function  idx00'(Heap: HeapType, a_2: int, b_24: int, c: int, n: int): int;
axiom (forall Heap: HeapType, a_2: int, b_24: int, c: int, n: int ::
  { idx00(Heap, a_2, b_24, c, n) }
  idx00(Heap, a_2, b_24, c, n) == idx00'(Heap, a_2, b_24, c, n) && dummyFunction(idx00#triggerStateless(a_2, b_24, c, n))
);
axiom (forall Heap: HeapType, a_2: int, b_24: int, c: int, n: int ::
  { idx00'(Heap, a_2, b_24, c, n) }
  dummyFunction(idx00#triggerStateless(a_2, b_24, c, n))
);

// Framing axioms
function  idx00#frame(frame: FrameType, a_2: int, b_24: int, c: int, n: int): int;
axiom (forall Heap: HeapType, Mask: MaskType, a_2: int, b_24: int, c: int, n: int ::
  { state(Heap, Mask), idx00'(Heap, a_2, b_24, c, n) }
  state(Heap, Mask) ==> idx00'(Heap, a_2, b_24, c, n) == idx00#frame(EmptyFrame, a_2, b_24, c, n)
);

// Postcondition axioms
axiom (forall Heap: HeapType, Mask: MaskType, a_2: int, b_24: int, c: int, n: int ::
  { state(Heap, Mask), idx00'(Heap, a_2, b_24, c, n) }
  state(Heap, Mask) && (AssumeFunctionsAbove < 0 || idx00#trigger(EmptyFrame, a_2, b_24, c, n)) ==> 0 <= idx00'(Heap, a_2, b_24, c, n) && idx00'(Heap, a_2, b_24, c, n) < n
);

// Trigger function (controlling recursive postconditions)
function  idx00#trigger(frame: FrameType, a_2: int, b_24: int, c: int, n: int): bool;

// State-independent trigger function
function  idx00#triggerStateless(a_2: int, b_24: int, c: int, n: int): int;

// Check contract well-formedness and postcondition
procedure idx00#definedness(a_2: int, b_24: int, c: int, n: int) returns (Result: int)
  modifies Heap, Mask;
{
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: inhale n <= |xs| -- 0146.vpr@11.3--11.19
    assume n <= Seq#Length(xs);
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Translating statement: inhale (forall i: Int, j: Int, fresh__414: Int ::
  //     { xs[idx00(i, j, fresh__414, n)] }
  //     0 <= i && (i < n && (0 <= j && j < i)) ==>
  //     xs[idx00(i, j, j + 1, n)] == i + j) -- 0146.vpr@13.3--16.41
    assume state(Heap, Mask);
    
    // -- Check definedness of (forall i: Int, j: Int, fresh__414: Int :: { xs[idx00(i, j, fresh__414, n)] } 0 <= i && (i < n && (0 <= j && j < i)) ==> xs[idx00(i, j, j + 1, n)] == i + j)
      if (*) {
        if (0 <= i_14 && (i_14 < n && (0 <= j_5 && j_5 < i_14))) {
          if (*) {
            // Stop execution
            assume false;
          }
          assert {:msg "  Inhale might fail. Index xs[idx00(i, j, j + 1, n)] into xs might be negative. (0146.vpr@13.10--16.41) [203189]"}
            idx00(Heap, i_14, j_5, j_5 + 1, n) >= 0;
          assert {:msg "  Inhale might fail. Index xs[idx00(i, j, j + 1, n)] into xs might exceed sequence length. (0146.vpr@13.10--16.41) [203190]"}
            idx00(Heap, i_14, j_5, j_5 + 1, n) < Seq#Length(xs);
        }
        assume false;
      }
    assume (forall i_1: int, j_1: int, fresh__414_1: int ::
      { Seq#Index(xs, idx00#frame(EmptyFrame, i_1, j_1, fresh__414_1, n)) }
      0 <= i_1 && (i_1 < n && (0 <= j_1 && j_1 < i_1)) ==> Seq#Index(xs, idx00(Heap, i_1, j_1, j_1 + 1, n)) == i_1 + j_1
    );
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  // -- Translating statement: assert xs[idx00(5, 2, 3, n)] == 7 -- 0146.vpr@18.3--18.36
    ...
    // -- Check definedness of xs[idx00(5, 2, 3, n)] == 7
      ...
      assert {:msg "  Assert might fail. Index xs[idx00(5, 2, 3, n)] into xs might be negative. (0146.vpr@18.10--18.36) [203191]"}
        idx00(Heap, 5, 2, 3, n) >= 0;
      assert {:msg "  Assert might fail. Index xs[idx00(5, 2, 3, n)] into xs might exceed sequence length. (0146.vpr@18.10--18.36) [203192]"}
        idx00(Heap, 5, 2, 3, n) < Seq#Length(xs);
    assert {:msg "  Assert might fail. Assertion xs[idx00(5, 2, 3, n)] == 7 might not hold. (0146.vpr@18.10--18.36) [203193]"}
      Seq#Index(xs, idx00(Heap, 5, 2, 3, n)) == 7;
    assume state(Heap, Mask);
}