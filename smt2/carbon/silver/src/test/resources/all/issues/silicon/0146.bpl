  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: inhale (forall i: Int, j: Int, fresh__220: Int ::
  //     { xs[idx00(i, j, fresh__220, n)] }
  //     0 <= i && (i < n && (0 <= j && j < i)) ==>
  //     xs[idx00(i, j, j + 1, n)] == i + j) -- 0146.vpr@13.3--16.41
    assume state(Heap, Mask);
    
    // -- Check definedness of (forall i: Int, j: Int, fresh__220: Int :: { xs[idx00(i, j, fresh__220, n)] } 0 <= i && (i < n && (0 <= j && j < i)) ==> xs[idx00(i, j, j + 1, n)] == i + j)
      if (*) {
        if (0 <= i_18 && (i_18 < n && (0 <= j_14 && j_14 < i_18))) {
          if (*) {
            // Stop execution
            assume false;
          }
          assert {:msg "  Inhale might fail. Index xs[idx00(i, j, j + 1, n)] into xs might be negative. (0146.vpr@13.10--16.41) [57948]"}
            idx00(Heap, i_18, j_14, j_14 + 1, n) >= 0;
          assert {:msg "  Inhale might fail. Index xs[idx00(i, j, j + 1, n)] into xs might exceed sequence length. (0146.vpr@13.10--16.41) [57949]"}
            idx00(Heap, i_18, j_14, j_14 + 1, n) < Seq#Length(xs);
        }
        assume false;
      }
    assume (forall i_1: int, j_1: int, fresh__220_1: int ::
      { Seq#Index(xs, idx00#frame(EmptyFrame, i_1, j_1, fresh__220_1, n)) }
  ...
  // -- Translating statement: assert xs[idx00(5, 2, 3, n)] == 7 -- 0146.vpr@18.3--18.36
    ...
    // -- Check definedness of xs[idx00(5, 2, 3, n)] == 7
      ...
      assert {:msg "  Assert might fail. Index xs[idx00(5, 2, 3, n)] into xs might be negative. (0146.vpr@18.10--18.36) [57950]"}
        idx00(Heap, 5, 2, 3, n) >= 0;
      assert {:msg "  Assert might fail. Index xs[idx00(5, 2, 3, n)] into xs might exceed sequence length. (0146.vpr@18.10--18.36) [57951]"}
        idx00(Heap, 5, 2, 3, n) < Seq#Length(xs);
    assert {:msg "  Assert might fail. Assertion xs[idx00(5, 2, 3, n)] == 7 might not hold. (0146.vpr@18.10--18.36) [57952]"}
