  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: a := new(val) -- 0293.vpr@9.3--9.14
    havoc freshObj;
  ...
  // -- Translating statement: assert !((a in refSet_b1)) -- 0293.vpr@11.3--11.27
    ...
    assert {:msg "  Assert might fail. Assertion !((a in refSet_b1)) might not hold. (0293.vpr@11.10--11.27) [74019]"}
      !refSet_b1[a_2];
    assume state(Heap, Mask);
  
  // -- Translating statement: inhale (forall r: Ref ::
  //     { (r in refSet_b1) }
  //     (r in refSet_b1) == perm(r.val) > none) -- 0293.vpr@13.10--13.83
    
    // -- Check definedness of (forall r: Ref :: { (r in refSet_b1) } (r in refSet_b1) == perm(r.val) > none)
      if (*) {
        assume false;
      }
  ...
  // -- Translating statement: assert false -- 0293.vpr@14.3--14.15
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    assert {:msg "  Assert might fail. Assertion false might not hold. (0293.vpr@14.10--14.15) [74020]"}
      false;
    assume state(Heap, Mask);
}
...
procedure vhavoc_1() returns (xs: (Set Ref))
  modifies Heap, Mask;
{
  var oldHeap: HeapType;
  var oldMask: MaskType;
