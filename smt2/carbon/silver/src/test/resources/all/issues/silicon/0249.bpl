    ...
    // -- Check definedness of (forall other: Int :: { int___box__(other) } (int___box__(other) == result) == (other == int))
      if (*) {
        if (*) {
          ...
          assume int___box__#trigger(EmptyFrame, other_2);
        }
        assume false;
      }
    assume (forall other_2_1: int ::
      { int___box__#frame(EmptyFrame, other_2_1) }
      (int___box__(Heap, other_2_1) == Result) == (other_2_1 == vint_2)
    );
    assume state(Heap, Mask);
}
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: inhale int___box__(i) == int___box__(j) -- 0249.vpr@9.10--9.42
    ...
    assume int___box__(Heap, i) == int___box__(Heap, j_9);
    assume state(Heap, Mask);
    assume state(Heap, Mask);
