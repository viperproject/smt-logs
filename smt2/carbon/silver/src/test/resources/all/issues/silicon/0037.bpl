  ...
  modifies Heap, Mask;
{
  ...
  if (*) {
    ...
    assume (forall xs_1_2: (ListDomainType AnyDomainType) ::
      { (drop((zero(): NaturalDomainType), xs_1_2): ListDomainType AnyDomainType) }
      (drop((zero(): NaturalDomainType), xs_1_2): ListDomainType AnyDomainType) == xs_1_2
    );
    assume state(PostHeap, PostMask);
    // Stop execution
    assume false;
  ...
  // -- Exhaling postcondition
    ...
    if (*) {
      assert {:msg "  Postcondition of property11 might not hold. Assertion (drop(zero(), xs): List[Any]) == xs might not hold. (0037.vpr@52.11--52.57) [60740]"}
        (drop((zero(): NaturalDomainType), xs_2): ListDomainType AnyDomainType) == xs_2;
      assume false;
    }
    assume (forall xs_3_1: (ListDomainType AnyDomainType) ::
      { (drop((zero(): NaturalDomainType), xs_3_1): ListDomainType AnyDomainType) }
      (drop((zero(): NaturalDomainType), xs_3_1): ListDomainType AnyDomainType) == xs_3_1
    );
}