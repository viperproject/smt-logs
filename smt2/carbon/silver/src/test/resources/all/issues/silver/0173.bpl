...
// 
// Date:         2025-01-13 13:26:59
// Tool:         carbon 1.0
// Arguments: :  --disableCaching --boogieExe /home/runner/.dotnet/tools/boogie --timeout 10 --print /home/runner/work/smt-logs/smt-logs/carbon/../smt2/carbon/silver/src/test/resources/all/issues/silver/0173.bpl --boogieOpt /proverLog:/home/runner/work/smt-logs/smt-logs/carbon/../smt2/carbon/silver/src/test/resources/all/issues/silver/0173-@PROC@.smt2 --ignoreFile dummy-file-to-prevent-cli-parser-from-complaining-about-missing-file-name.silver
// Dependencies:
...
// ==================================================
// Translation of function trigger_issue
// ==================================================

// Uninterpreted function definitions
function  trigger_issue(Heap: HeapType, lo: int, hi: int): bool;
function  trigger_issue'(Heap: HeapType, lo: int, hi: int): bool;
axiom (forall Heap: HeapType, lo: int, hi: int ::
  { trigger_issue(Heap, lo, hi) }
  trigger_issue(Heap, lo, hi) == trigger_issue'(Heap, lo, hi) && dummyFunction(trigger_issue#triggerStateless(lo, hi))
);
axiom (forall Heap: HeapType, lo: int, hi: int ::
  { trigger_issue'(Heap, lo, hi) }
  dummyFunction(trigger_issue#triggerStateless(lo, hi))
);

// Definitional axiom
axiom (forall Heap: HeapType, Mask: MaskType, lo: int, hi: int ::
  { state(Heap, Mask), trigger_issue(Heap, lo, hi) }
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> (forall k: int ::
    { Seq#ContainsTrigger(Seq#Range(lo, hi), k) } { Seq#Contains(Seq#Range(lo, hi), k) }
    Seq#Contains(Seq#Range(lo, hi), k) ==> k < k + 1
  ) ==> trigger_issue(Heap, lo, hi)
);

// Framing axioms
function  trigger_issue#frame(frame: FrameType, lo: int, hi: int): bool;
axiom (forall Heap: HeapType, Mask: MaskType, lo: int, hi: int ::
  { state(Heap, Mask), trigger_issue'(Heap, lo, hi) }
  state(Heap, Mask) ==> trigger_issue'(Heap, lo, hi) == trigger_issue#frame(EmptyFrame, lo, hi)
);

// Trigger function (controlling recursive postconditions)
function  trigger_issue#trigger(frame: FrameType, lo: int, hi: int): bool;

// State-independent trigger function
function  trigger_issue#triggerStateless(lo: int, hi: int): bool;

// Check contract well-formedness and postcondition
procedure trigger_issue#definedness(lo: int, hi: int) returns (Result: bool)
  modifies Heap, Mask;
{
  ...
  // -- Inhaling precondition (with checking)
    ...
    // -- Check definedness of (forall k: Int :: { (k in [lo..hi)) } (k in [lo..hi)) ==> k < k + 1)
      if (*) {
        assume false;
      }
    assume (forall k_3: int ::
      { Seq#ContainsTrigger(Seq#Range(lo, hi), k_3) } { Seq#Contains(Seq#Range(lo, hi), k_3) }
      Seq#Contains(Seq#Range(lo, hi), k_3) ==> k_3 < k_3 + 1
    );
    assume state(Heap, Mask);
...
// ==================================================
// Translation of method problem
// ==================================================

procedure problem(x: int, y: int) returns ()
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: assert trigger_issue(0, x * y) -- 0173.vpr@12.3--12.31
    ExhaleWellDef0Mask := Mask;
    ExhaleWellDef0Heap := Heap;
    
    // -- Check definedness of trigger_issue(0, x * y)
      if (*) {
        // Exhale precondition of function application
        ExhaleWellDef1Mask := ExhaleWellDef0Mask;
        ExhaleWellDef1Heap := ExhaleWellDef0Heap;
        if (*) {
          if (Seq#Contains(Seq#Range(0, x * y), k)) {
            assert {:msg "  Precondition of function trigger_issue might not hold. Assertion k < k + 1 might not hold. (0173.vpr@12.10--12.31) [220126]"}
              k < k + 1;
          }
          assume false;
        }
        assume (forall k_1_1_1: int ::
          { Seq#ContainsTrigger(Seq#Range(0, x * y), k_1_1_1) } { Seq#Contains(Seq#Range(0, x * y), k_1_1_1) }
          Seq#Contains(Seq#Range(0, x * y), k_1_1_1) ==> k_1_1_1 < k_1_1_1 + 1
        );
        // Stop execution
        assume false;
      }
    assert {:msg "  Assert might fail. Assertion trigger_issue(0, x * y) might not hold. (0173.vpr@12.10--12.31) [220127]"}
      trigger_issue(Heap, 0, x * y);
    assume state(Heap, Mask);
}