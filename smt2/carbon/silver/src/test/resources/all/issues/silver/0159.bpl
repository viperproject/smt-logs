...
procedure m_17(e_1: Ref) returns ()
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: assert (forall i: Int ::i < 4 && i > 0 ==> acc(e.x, write)) -- 0159.vpr@11.3--11.54
    ...
    // -- check if receiver e is injective
      assert {:msg "  Assert might fail. Quantified resource e.x might not be injective. (0159.vpr@11.10--11.54) [54776]"}
    ...
    // -- check if sufficient permission is held
      assert {:msg "  Assert might fail. There might be insufficient permission to access e.x (0159.vpr@11.10--11.54) [54777]"}
        (forall i_1: int ::
        
        i_1 < 4 && i_1 > 0 ==> AssertMask[e_1, x_42] >= FullPerm
      );
    ...
    // -- assumptions for inverse of receiver e
      ...
      assume (forall o_9: Ref ::
        { invRecv1(o_9) }
        (invRecv1(o_9) < 4 && invRecv1(o_9) > 0) && (NoPerm < FullPerm && qpRange1(o_9)) ==> e_1 == o_9
      );
    
    // -- assume permission updates for field x
      assume (forall o_9: Ref ::
        { QPMask[o_9, x_42] }
        ((invRecv1(o_9) < 4 && invRecv1(o_9) > 0) && (NoPerm < FullPerm && qpRange1(o_9)) ==> e_1 == o_9 && QPMask[o_9, x_42] == AssertMask[o_9, x_42] - FullPerm) && (!((invRecv1(o_9) < 4 && invRecv1(o_9) > 0) && (NoPerm < FullPerm && qpRange1(o_9))) ==> QPMask[o_9, x_42] == AssertMask[o_9, x_42])
      );
    
    // -- assume permission updates for independent locations
      assume (forall <A, B> o_9: Ref, f_5: (Field A B) ::
        { QPMask[o_9, f_5] }
        f_5 != x_42 ==> AssertMask[o_9, f_5] == QPMask[o_9, f_5]
      );
    AssertMask := QPMask;
    assume state(Heap, Mask);
}