...
type ListWellFoundedOrderDomainType T;

// Translation of anonymous domain axiom
axiom (forall <T> y: (ListDomainType T) ::
  { (bounded(y): bool) }
  (bounded(y): bool)
);

// Translation of anonymous domain axiom
axiom (forall <T> xs: (ListDomainType T), y: T ::
  { (decreasing(xs, (Cons(y, xs): ListDomainType T)): bool) }
  (decreasing(xs, (Cons(y, xs): ListDomainType T)): bool)
);

// Translation of anonymous domain axiom
axiom (forall <T> xs: (ListDomainType T), ys: (ListDomainType T), zs: (ListDomainType T) ::
  { (decreasing(xs, ys): bool), (decreasing(ys, zs): bool) }
  (decreasing(xs, ys): bool) && (decreasing(ys, zs): bool) ==> (decreasing(xs, zs): bool)
);
