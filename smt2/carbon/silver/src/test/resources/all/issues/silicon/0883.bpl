...
procedure foo_3(x: Ref, xs: (Set Ref)) returns ()
  modifies Heap, Mask;
{
  var QPMask: MaskType;
  ...
  // -- Checked inhaling of precondition
    ...
    havoc QPMask;
    assert {:msg "  Contract might not be well-formed. Quantified resource r.f might not be injective. (0883.vpr@9.14--9.63) [73567]"}
  ...
  // -- Translating statement: inhale acc(x.f, 1 / 2) -- 0883.vpr@12.5--12.25
    perm := 1 / 2;
    assert {:msg "  Inhale might fail. Fraction 1 / 2 might be negative. (0883.vpr@12.12--12.25) [73568]"}
      perm >= NoPerm;
  ...
  // -- Translating statement: if (b) -- 0883.vpr@13.5--17.6
    if (b_24) {
      ...
      // -- Translating statement: inhale acc(x.f, 1 / 2) -- 0883.vpr@14.9--14.29
        perm := 1 / 2;
        assert {:msg "  Inhale might fail. Fraction 1 / 2 might be negative. (0883.vpr@14.16--14.29) [73569]"}
          perm >= NoPerm;
        assume perm > NoPerm ==> x != null;
        Mask := Mask[x, f_7:=Mask[x, f_7] + perm];
        assume state(Heap, Mask);
      ...
      // -- Translating statement: inhale acc(x.f, 1 / 2) -- 0883.vpr@16.9--16.29
        perm := 1 / 2;
        assert {:msg "  Inhale might fail. Fraction 1 / 2 might be negative. (0883.vpr@16.16--16.29) [73570]"}
          perm >= NoPerm;
        assume perm > NoPerm ==> x != null;
        Mask := Mask[x, f_7:=Mask[x, f_7] + perm];
        assume state(Heap, Mask);
