...
procedure foo_1(x: Ref, xs: (Set Ref)) returns ()
  modifies Heap, Mask;
{
  var QPMask: MaskType;
  ...
  // -- Checked inhaling of precondition
    ...
    havoc QPMask;
    assert {:msg "  Contract might not be well-formed. Quantified resource r.f might not be injective. (0883.vpr@9.14--9.63) [204609]"}
    ...
    // -- Define Inverse Function
      assume (forall r_1_1: Ref ::
        { Heap[r_1_1, f_7] } { QPMask[r_1_1, f_7] } { xs[r_1_1] }
        xs[r_1_1] && NoPerm < FullPerm ==> qpRange1(r_1_1) && invRecv1(r_1_1) == r_1_1
      );
  ...
  // -- Translating statement: inhale acc(x.f, 1 / 2) -- 0883.vpr@12.5--12.25
    perm := 1 / 2;
    assert {:msg "  Inhale might fail. Fraction 1 / 2 might be negative. (0883.vpr@12.12--12.25) [204610]"}
      perm >= NoPerm;
  ...
  // -- Translating statement: if (b) -- 0883.vpr@13.5--17.6
    if (b_24) {
      
      // -- Translating statement: inhale acc(x.f, 1 / 2) -- 0883.vpr@14.9--14.29
        perm := 1 / 2;
        assert {:msg "  Inhale might fail. Fraction 1 / 2 might be negative. (0883.vpr@14.16--14.29) [204611]"}
          perm >= NoPerm;
        assume perm > NoPerm ==> x != null;
        Mask := Mask[x, f_7:=Mask[x, f_7] + perm];
        assume state(Heap, Mask);
      ...
      // -- Translating statement: inhale acc(x.f, 1 / 2) -- 0883.vpr@16.9--16.29
        perm := 1 / 2;
        assert {:msg "  Inhale might fail. Fraction 1 / 2 might be negative. (0883.vpr@16.16--16.29) [204612]"}
          perm >= NoPerm;
        assume perm > NoPerm ==> x != null;
        Mask := Mask[x, f_7:=Mask[x, f_7] + perm];
        assume state(Heap, Mask);
  ...
  // -- Translating statement: assert acc(x.f, write) -- 0883.vpr@19.5--19.20
    ...
    perm := FullPerm;
    if (perm != NoPerm) {
      assert {:msg "  Assert might fail. There might be insufficient permission to access x.f (0883.vpr@19.12--19.20) [204614]"}
