...
function  f4(Heap: HeapType, j: int): bool;
function  f4'(Heap: HeapType, j: int): bool;
axiom (forall Heap: HeapType, j: int ::
  { f4(Heap, j) }
  f4(Heap, j) == f4'(Heap, j) && dummyFunction(f4#triggerStateless(j))
);
axiom (forall Heap: HeapType, j: int ::
  { f4'(Heap, j) }
  dummyFunction(f4#triggerStateless(j))
);

// Definitional axiom
axiom (forall Heap: HeapType, Mask: MaskType, j: int ::
  { state(Heap, Mask), f4(Heap, j) } { state(Heap, Mask), f4#triggerStateless(j), P#trigger(Heap, P()) }
  state(Heap, Mask) && AssumeFunctionsAbove < 1 ==> f4(Heap, j)
);

// Framing axioms
function  f4#frame(frame: FrameType, j: int): bool;
axiom (forall Heap: HeapType, Mask: MaskType, j: int ::
  { state(Heap, Mask), f4'(Heap, j) }
  state(Heap, Mask) ==> f4'(Heap, j) == f4#frame(Heap[null, P()], j)
);

// Trigger function (controlling recursive postconditions)
function  f4#trigger(frame: FrameType, j: int): bool;

// State-independent trigger function
function  f4#triggerStateless(j: int): bool;

// Check contract well-formedness and postcondition
procedure f4#definedness(j: int) returns (Result: bool)
  modifies Heap, Mask;
{
  var perm: Perm;
...
axiom (forall Heap: HeapType, x: Ref, y: int ::
  { f2(Heap, x, y) }
  f2(Heap, x, y) == f2'(Heap, x, y) && dummyFunction(f2#triggerStateless(x, y))
);
axiom (forall Heap: HeapType, x: Ref, y: int ::
  { f2'(Heap, x, y) }
...
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref, y: int ::
  ...
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> f2(Heap, x, y) == y
);
...
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref, y: int ::
  { state(Heap, Mask), f2'(Heap, x, y) }
  state(Heap, Mask) ==> f2'(Heap, x, y) == f2#frame(FrameFragment(Heap[x, v_35]), x, y)
);
  ...
  modifies Heap, Mask;
{
  ...
  // } -- 0225.vpr@13.3--13.60
    ...
    // -- Transfer of acc(P(), write)
      ...
      assert {:msg "  Packaging wand might fail. Fraction acc(P(), write) might be negative. (0225.vpr@13.3--13.60) [74563]"}
        ...
        if ((((b_1_1 && b_1_1) && b_2_1) && accVar2) && neededTransfer > 0.000000000) {
          ...
          if (takeTransfer > 0.000000000) {
            ...
            assume (forall <A, B> o_73: Ref, f_82: (Field A B) ::
              { newPMask[o_73, f_82] }
              Heap[null, wand#sm()][o_73, f_82] || Heap[null, P#sm()][o_73, f_82] ==> newPMask[o_73, f_82]
            );
            Heap := Heap[null, wand#sm():=newPMask];
          }
        }
      assert {:msg "  Packaging wand might fail. There might be insufficient permission to access P() (0225.vpr@13.3--13.60) [74564]"}
    ...
    assert {:msg "  Packaging wand might fail. Assertion (forall j: Int :: { f4(j) } f4(j)) might not hold. (0225.vpr@13.3--13.60) [74566]"}
      (b_1_1 && b_1_1) && b_2_1 ==> (forall j_3: int ::
      { f4#frame(ResultHeap[null, P()], j_3) }
      f4(ResultHeap, j_3)
    );
    assume state(Heap, Mask);
...
procedure test_1(y: Ref) returns ()
  modifies Heap, Mask;
{
  var perm: Perm;
  var j_2: int;
  ...
  // -- Checked inhaling of precondition
    perm := FullPerm;
    assume y != null;
    Mask := Mask[y, v_35:=Mask[y, v_35] + perm];
    assume state(Heap, Mask);
    assume state(Heap, Mask);
    assume state(Heap, Mask);
    ...
    assume (forall j_1: int ::
      { f2#frame(FrameFragment(Heap[y, v_35]), y, j_1) }
      j_1 > 0 ==> f2(Heap, y, j_1) == j_1
    );
    assume state(Heap, Mask);
  ...
  // } -- 0225.vpr@28.5--28.79
    ...
    if (b_5) {
      perm := FullPerm;
      b_5 := b_5 && y != null;
      Ops_3Mask := Ops_3Mask[y, v_35:=Ops_3Mask[y, v_35] + perm];
      b_5 := b_5 && state(Ops_3Heap, Ops_3Mask);
    }
    b_5 := b_5 && state(Ops_3Heap, Ops_3Mask);
    ...
    // -- Transfer of acc(y.v, write)
      ...
      assert {:msg "  Packaging wand might fail. Fraction acc(y.v, write) might be negative. (0225.vpr@28.5--28.79) [74568]"}
        neededTransfer >= 0.000000000;
      ...
      // -- transfer code for top state of stack
        ...
        if ((((b_5 && b_5) && b_6) && accVar2) && neededTransfer > 0.000000000) {
          ...
          if (takeTransfer > 0.000000000) {
            ...
            Heap := Heap[null, wand_1#sm(y, FullPerm, y, FullPerm, 0, y):=Heap[null, wand_1#sm(y, FullPerm, y, FullPerm, 0, y)][y, v_35:=true]];
          }
        }
      assert {:msg "  Packaging wand might fail. There might be insufficient permission to access y.v (0225.vpr@28.5--28.79) [74569]"}
        (b_5 && b_5) && b_6 ==> neededTransfer == 0.000000000 && Used_3Mask[rcvLocal, v_35] == initNeededTransfer;
    ...
    assert {:msg "  Packaging wand might fail. Assertion (forall j: Int :: { f2(y, j) } j > 0 ==> f2(y, j) == j) might not hold. (0225.vpr@28.5--28.79) [74571]"}
      (b_5 && b_5) && b_6 ==> (forall j_5: int ::
      { f2#frame(FrameFragment(Result_1Heap[y, v_35]), y, j_5) }
      j_5 > 0 ==> f2(Result_1Heap, y, j_5) == j_5
    );
    assume state(Heap, Mask);
