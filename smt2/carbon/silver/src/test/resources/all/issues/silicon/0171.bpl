...
axiom (forall Heap: HeapType, Mask: MaskType, a_2: IArrayDomainType, x: int, y: int ::
  { state(Heap, Mask), f_lcp'(Heap, a_2, x, y) }
  state(Heap, Mask) && (AssumeFunctionsAbove < 0 || f_lcp#trigger(EmptyFrame, a_2, x, y)) ==> (forall k: int, fresh__407: int ::
    { Seq#ContainsTrigger(Seq#Range(x, fresh__407), k) } { Seq#Contains(Seq#Range(x, fresh__407), k) }
    Seq#Contains(Seq#Range(x, x + 1), k) ==> Seq#Contains(Seq#Range(0, (length_1(a_2): int)), y)
  )
);
  ...
  modifies Heap, Mask;
{
  ...
  // -- Checking definedness of postcondition (no body)
    ...
    // -- Check definedness of (forall k: Int, fresh__407: Int :: { (k in [x..fresh__407)) } (k in [x..x + 1)) ==> (y in [0..length(a))))
      if (*) {
        assume false;
      }
    assume (forall k_2_1: int, fresh__407_2: int ::
      { Seq#ContainsTrigger(Seq#Range(x, fresh__407_2), k_2_1) } { Seq#Contains(Seq#Range(x, fresh__407_2), k_2_1) }
      Seq#Contains(Seq#Range(x, x + 1), k_2_1) ==> Seq#Contains(Seq#Range(0, (length_1(a_2): int)), y)
    );
    assume state(Heap, Mask);
}