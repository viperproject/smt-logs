...
// 
// Date:         2025-01-27 03:17:58
// Tool:         carbon 1.0
// Arguments: :  --disableCaching --boogieExe /home/runner/.dotnet/tools/boogie --timeout 10 --print /home/runner/work/smt-logs/smt-logs/carbon/../smt2/carbon/silver/src/test/resources/all/issues/silicon/0314b.bpl --boogieOpt /proverLog:/home/runner/work/smt-logs/smt-logs/carbon/../smt2/carbon/silver/src/test/resources/all/issues/silicon/0314b-@PROC@.smt2 --ignoreFile dummy-file-to-prevent-cli-parser-from-complaining-about-missing-file-name.silver
// Dependencies:
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: inhale acc(y.f, write) -- 0314b.vpr@15.3--15.18
    perm := FullPerm;
    assume y != null;
  ...
  // -- Translating statement: inhale (forall x: Ref, i: Int ::
  //     { foo(i, x.f) }
  //     (x in xs) && 0 < i ==> bar(i)) -- 0314b.vpr@16.3--16.76
    
    // -- Check definedness of (forall x: Ref, i: Int :: { foo(i, x.f) } (x in xs) && 0 < i ==> bar(i))
      if (*) {
        assume false;
      }
    assume (forall x_1: Ref, i_1: int ::
      { (foo_2(i_1, Heap[x_1, f_7]): bool) }
      xs[x_1] && 0 < i_1 ==> (bar_1(i_1): bool)
    );
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Translating statement: inhale (y in xs) -- 0314b.vpr@18.3--18.17
    assume xs[y];
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Translating statement: inhale foo(1, y.f) -- 0314b.vpr@19.3--19.21
    
    // -- Check definedness of foo(1, y.f)
      assert {:msg "  Inhale might fail. There might be insufficient permission to access y.f (0314b.vpr@19.10--19.21) [61909]"}
        HasDirectPerm(Mask, y, f_7);
    assume (foo_2(1, Heap[y, f_7]): bool);
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Translating statement: assert bar(1) -- 0314b.vpr@20.3--20.16
    ExhaleWellDef0Mask := Mask;
    ExhaleWellDef0Heap := Heap;
    assert {:msg "  Assert might fail. Assertion bar(1) might not hold. (0314b.vpr@20.10--20.16) [61910]"}
      (bar_1(1): bool);
    assume state(Heap, Mask);
}