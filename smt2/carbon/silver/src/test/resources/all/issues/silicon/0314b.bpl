  ...
  modifies Heap, Mask;
{
  ...
  //     (x in xs) && 0 < i ==> bar(i)) -- 0314b.vpr@16.3--16.76
    ...
    assume (forall x_1: Ref, i_1: int ::
      { (foo(i_1, Heap[x_1, f_7]): bool) }
      xs[x_1] && 0 < i_1 ==> (bar_3(i_1): bool)
    );
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  // -- Translating statement: inhale foo(1, y.f) -- 0314b.vpr@19.3--19.21
    ...
    // -- Check definedness of foo(1, y.f)
      assert {:msg "  Inhale might fail. There might be insufficient permission to access y.f (0314b.vpr@19.10--19.21) [198654]"}
        HasDirectPerm(Mask, y, f_7);
    assume (foo(1, Heap[y, f_7]): bool);
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  // -- Translating statement: assert bar(1) -- 0314b.vpr@20.3--20.16
    ...
    assert {:msg "  Assert might fail. Assertion bar(1) might not hold. (0314b.vpr@20.10--20.16) [198655]"}
      (bar_3(1): bool);
    assume state(Heap, Mask);
}