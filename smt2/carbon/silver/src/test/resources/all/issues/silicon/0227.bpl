...
procedure test_1(this: Ref, c: Ref, n: int) returns ()
  modifies Heap, Mask;
{
  var perm: Perm;
  ...
  // -- Checked inhaling of precondition
    ...
    assert {:msg "  Contract might not be well-formed. Fraction n * write might be negative. (0227.vpr@11.12--11.62) [57276]"}
      perm >= NoPerm;
  ...
  // -- Translating statement: exhale (n - i) * write >= none && acc(mustSendBounded$(c), (n - i) * write) -- 0227.vpr@16.3--16.80
    ...
    assert {:msg "  Exhale might fail. Assertion (n - i) * write >= none might not hold. (0227.vpr@16.10--16.80) [57277]"}
      NoPerm <= real(n - i) * FullPerm;
    perm := real(n - i) * FullPerm;
    assert {:msg "  Exhale might fail. Fraction (n - i) * write might be negative. (0227.vpr@16.10--16.80) [57278]"}
      perm >= NoPerm;
    if (perm != NoPerm) {
      assert {:msg "  Exhale might fail. There might be insufficient permission to access mustSendBounded$(c) (0227.vpr@16.10--16.80) [57279]"}
  ...
  // -- Translating statement: exhale n - i + 1 > 0 && n - i >= 0 -- 0227.vpr@17.3--17.41
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    assert {:msg "  Exhale might fail. Assertion n - i + 1 > 0 might not hold. (0227.vpr@17.10--17.41) [57280]"}
      n - i + 1 > 0;
    assert {:msg "  Exhale might fail. Assertion n - i >= 0 might not hold. (0227.vpr@17.10--17.41) [57281]"}
      n - i >= 0;
    assume state(Heap, Mask);
      ...
      if (*) {
        ...
        // -- Translate loop body
          ...
          // -- Translating statement: inhale (n - i) * write >= none && acc(mustSendBounded$(c), (n - i) * write) -- 0227.vpr@21.5--21.82
            ...
            perm := real(n - i) * FullPerm;
            assert {:msg "  Inhale might fail. Fraction (n - i) * write might be negative. (0227.vpr@21.12--21.82) [57282]"}
              perm >= NoPerm;
            Mask := Mask[null, mustSendBounded$(c):=Mask[null, mustSendBounded$(c)] + perm];
            assume state(Heap, Mask);
            assume state(Heap, Mask);
          ...
          // -- Translating statement: assert c != null -- 0227.vpr@26.5--26.21
            ExhaleWellDef0Heap := Heap;
            ExhaleWellDef0Mask := Mask;
            assert {:msg "  Assert might fail. Assertion c != null might not hold. (0227.vpr@26.12--26.21) [57283]"}
              c != null;
            assume state(Heap, Mask);
          ...
          // -- Translating statement: exhale acc(mustSendBounded$(c), 1 * write) -- 0227.vpr@28.5--28.47
            ...
            assert {:msg "  Exhale might fail. Fraction 1 * write might be negative. (0227.vpr@28.12--28.47) [57284]"}
              perm >= NoPerm;
            if (perm != NoPerm) {
              assert {:msg "  Exhale might fail. There might be insufficient permission to access mustSendBounded$(c) (0227.vpr@28.12--28.47) [57285]"}
                perm <= Mask[null, mustSendBounded$(c)];
            }
            Mask := Mask[null, mustSendBounded$(c):=Mask[null, mustSendBounded$(c)] - perm];
            // Finish exhale
            ...
            // -- Check definedness of (forperm r$_4: Ref [mustSendBounded$(r$_4)] :: false)
              if (*) {
                if (HasDirectPerm(Mask, null, mustSendBounded$(r$_4))) {
                  assert {:msg "  Exhale might fail. There might be insufficient permission to access mustSendBounded$(r$_4) (0227.vpr@31.13--31.64) [57286]"}
                    HasDirectPerm(ExhaleWellDef0Mask, null, mustSendBounded$(r$_4));
                }
                assume false;
              }
            assert {:msg "  Exhale might fail. Assertion (forperm r$_4: Ref [mustSendBounded$(r$_4)] :: false) might not hold. (0227.vpr@31.13--31.64) [57287]"}
              (forall r$_4_1: Ref ::
