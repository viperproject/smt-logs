...
(let ((anon6_Else_correct  (=> (and x (= (ControlFlow 0 4) 1)) anon4_correct)))
(let ((anon6_Then_correct  (=> (not x) (and (=> (= (ControlFlow 0 2) (- 0 3)) (not (= y@0 123))) (=> (not (= y@0 123)) (=> (= (ControlFlow 0 2) 1) anon4_correct))))))
(let ((anon2_correct  (=> (state Heap@@15 ZeroMask) (and (=> (= (ControlFlow 0 5) 2) anon6_Then_correct) (=> (= (ControlFlow 0 5) 4) anon6_Else_correct)))))
(let ((anon5_Else_correct  (=> x (=> (and (and (state Heap@@15 ZeroMask) (state Heap@@15 ZeroMask)) (and (= y@0 456) (= (ControlFlow 0 7) 5))) anon2_correct))))
(let ((anon5_Then_correct  (=> (and (and (not x) (state Heap@@15 ZeroMask)) (and (= y@0 123) (= (ControlFlow 0 6) 5))) anon2_correct)))
(let ((anon0_correct  (=> (and (state Heap@@15 ZeroMask) AssumePermUpperBound) (and (=> (= (ControlFlow 0 8) 6) anon5_Then_correct) (=> (= (ControlFlow 0 8) 7) anon5_Else_correct)))))
