...
axiom (forall Heap: HeapType, xs: (Seq Ref) ::
  { g'(Heap, xs) }
  dummyFunction(g#triggerStateless(xs))
);

// Framing axioms
function  g#frame(frame: FrameType, xs: (Seq Ref)): Seq Ref;
axiom (forall Heap: HeapType, Mask: MaskType, xs: (Seq Ref) ::
  { state(Heap, Mask), g'(Heap, xs) }
  state(Heap, Mask) ==> g'(Heap, xs) == g#frame(EmptyFrame, xs)
);

// Trigger function (controlling recursive postconditions)
function  g#trigger(frame: FrameType, xs: (Seq Ref)): bool;

// State-independent trigger function
function  g#triggerStateless(xs: (Seq Ref)): Seq Ref;

// Check contract well-formedness and postcondition
procedure g#definedness(xs: (Seq Ref)) returns (Result: (Seq Ref))
  modifies Heap, Mask;
{
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: inhale g(Seq(b) ++ Seq[Ref]()) == s -- 0126.vpr@10.10--10.38
    assume state(Heap, Mask);
    
    // -- Check definedness of g(Seq(b) ++ Seq[Ref]()) == s
      if (*) {
        // Stop execution
        assume false;
      }
