...
axiom (forall Heap: HeapType ::
  { test'(Heap) }
  dummyFunction(test#triggerStateless())
);
...
// Framing axioms
function  test#frame(frame: FrameType): int;
axiom (forall Heap: HeapType, Mask: MaskType ::
  { state(Heap, Mask), test'(Heap) }
  state(Heap, Mask) ==> test'(Heap) == test#frame(EmptyFrame)
);

// Postcondition axioms
axiom (forall Heap: HeapType, Mask: MaskType ::
  { state(Heap, Mask), test'(Heap) }
  state(Heap, Mask) && (AssumeFunctionsAbove < 0 || test#trigger(EmptyFrame)) ==> false
);

// Trigger function (controlling recursive postconditions)
function  test#trigger(frame: FrameType): bool;

// State-independent trigger function
function  test#triggerStateless(): int;

// Check contract well-formedness and postcondition
procedure test#definedness() returns (Result: int)
  modifies Heap, Mask;
{
