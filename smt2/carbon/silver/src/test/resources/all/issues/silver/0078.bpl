...
function  fun(Heap: HeapType, x: Ref): Perm;
function  fun'(Heap: HeapType, x: Ref): Perm;
...
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  ...
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> fun(Heap, x) == FullPerm
);

// Framing axioms
function  fun#frame(frame: FrameType, x: Ref): Perm;
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  { state(Heap, Mask), fun'(Heap, x) }
...
function  fun#triggerStateless(x: Ref): Perm;

// Check contract well-formedness and postcondition
procedure fun#definedness(x: Ref) returns (Result: Perm)
  modifies Heap, Mask;
{
