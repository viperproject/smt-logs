  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: inhale (unfolding acc(P(), write) in true) -- 0127-2.vpr@12.3--12.31
    ...
    // -- Check definedness of (unfolding acc(P(), write) in true)
      UnfoldingHeap := Heap;
      UnfoldingMask := Mask;
      ...
      perm := FullPerm;
      if (perm != NoPerm) {
        assert {:msg "  Inhale might fail. There might be insufficient permission to access P() (0127-2.vpr@12.10--12.31) [54006]"}
          perm <= UnfoldingMask[null, P()];
      }
