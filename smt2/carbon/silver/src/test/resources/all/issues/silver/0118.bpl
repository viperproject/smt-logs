...
// ==================================================
// Translation of function zip
// ==================================================

// Uninterpreted function definitions
function  zip(Heap: HeapType, xs: (Seq Ref), ys: (Seq Ref)): Seq Ref;
function  zip'(Heap: HeapType, xs: (Seq Ref), ys: (Seq Ref)): Seq Ref;
axiom (forall Heap: HeapType, xs: (Seq Ref), ys: (Seq Ref) ::
  { zip(Heap, xs, ys) }
  zip(Heap, xs, ys) == zip'(Heap, xs, ys) && dummyFunction(zip#triggerStateless(xs, ys))
);
axiom (forall Heap: HeapType, xs: (Seq Ref), ys: (Seq Ref) ::
  { zip'(Heap, xs, ys) }
  dummyFunction(zip#triggerStateless(xs, ys))
);

// Definitional axiom
axiom (forall Heap: HeapType, Mask: MaskType, xs: (Seq Ref), ys: (Seq Ref) ::
  { state(Heap, Mask), zip(Heap, xs, ys) }
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> Seq#Length(xs) == Seq#Length(ys) || Seq#Length(xs) == Seq#Length(ys) + 1 ==> zip(Heap, xs, ys) == (if Seq#Equal(ys, (Seq#Empty(): Seq Ref)) then xs else Seq#Append(Seq#Append(Seq#Singleton(Seq#Index(xs, 0)), Seq#Singleton(Seq#Index(ys, 0))), zip'(Heap, Seq#Drop(xs, 1), Seq#Drop(ys, 1))))
);

// Framing axioms
function  zip#frame(frame: FrameType, xs: (Seq Ref), ys: (Seq Ref)): Seq Ref;
axiom (forall Heap: HeapType, Mask: MaskType, xs: (Seq Ref), ys: (Seq Ref) ::
  { state(Heap, Mask), zip'(Heap, xs, ys) }
  state(Heap, Mask) ==> zip'(Heap, xs, ys) == zip#frame(EmptyFrame, xs, ys)
);

// Trigger function (controlling recursive postconditions)
function  zip#trigger(frame: FrameType, xs: (Seq Ref), ys: (Seq Ref)): bool;

// State-independent trigger function
function  zip#triggerStateless(xs: (Seq Ref), ys: (Seq Ref)): Seq Ref;
  ...
  modifies Heap, Mask;
{
  ...
  // -- Inhaling precondition (with checking)
    assume Seq#Length(xs) == Seq#Length(ys) || Seq#Length(xs) == Seq#Length(ys) + 1;
    assume state(Heap, Mask);
    ...
    // -- Check definedness of (ys == Seq[Ref]() ? xs : Seq(xs[0], ys[0]) ++ zip(xs[1..], ys[1..]))
      if (Seq#Equal(ys, (Seq#Empty(): Seq Ref))) {
        ...
        if (*) {
          ...
          assume zip#trigger(EmptyFrame, Seq#Drop(xs, 1), Seq#Drop(ys, 1));
        }
      }
  
  // -- Translate function body
    Result := (if Seq#Equal(ys, (Seq#Empty(): Seq Ref)) then xs else Seq#Append(Seq#Append(Seq#Singleton(Seq#Index(xs, 0)), Seq#Singleton(Seq#Index(ys, 0))), zip(Heap, Seq#Drop(xs, 1), Seq#Drop(ys, 1))));
}
  ...
  modifies Heap, Mask;
{
  ...
  // -- Checked inhaling of precondition
    assume Seq#Equal(xs, Seq#Singleton(x));
    assume state(Heap, Mask);
    assume Seq#Equal(ys, Seq#Singleton(y));
    assume state(Heap, Mask);
  ...
  if (*) {
    ...
    assume Seq#Equal(Seq#Append(Seq#Singleton(x), Seq#Singleton(y)), zip(PostHeap, xs, ys));
    assume state(PostHeap, PostMask);
    // Stop execution
    assume false;
  }
