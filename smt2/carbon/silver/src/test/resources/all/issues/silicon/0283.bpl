...
// 
// Date:         2024-12-29 15:39:48
// Tool:         carbon 1.0
// Arguments: :  --disableCaching --boogieExe /home/runner/.dotnet/tools/boogie --timeout 10 --print /home/runner/work/smt-logs/smt-logs/carbon/../smt2/carbon/silver/src/test/resources/all/issues/silicon/0283.bpl --boogieOpt /proverLog:/home/runner/work/smt-logs/smt-logs/carbon/../smt2/carbon/silver/src/test/resources/all/issues/silicon/0283-@PROC@.smt2 --ignoreFile dummy-file-to-prevent-cli-parser-from-complaining-about-missing-file-name.silver
// Dependencies:
  ...
  modifies Heap, Mask;
{
  ...
  // -- Checked inhaling of precondition
    ...
    // -- Check definedness of (forall i: Int :: { xs[i] } 0 <= i && i <= |xs| ==> xs[i] == 0)
      if (*) {
        if (0 <= i_14 && i_14 <= Seq#Length(xs)) {
          assert {:msg "  Contract might not be well-formed. Index xs[i] into xs might be negative. (0283.vpr@6.12--6.63) [203499]"}
            i_14 >= 0;
          assert {:msg "  Contract might not be well-formed. Index xs[i] into xs might exceed sequence length. (0283.vpr@6.12--6.63) [203500]"}
            i_14 < Seq#Length(xs);
        }
        assume false;
      }
    assume (forall i_1: int ::
      { Seq#Index(xs, i_1) }
      0 <= i_1 && i_1 <= Seq#Length(xs) ==> Seq#Index(xs, i_1) == 0
    );
    assume state(Heap, Mask);
...
procedure test2(xs: (Seq int)) returns ()
  modifies Heap, Mask;
{
  var i_2: int;
  var oldHeap: HeapType;
    ...
    // -- Check definedness of (forall i: Int :: { xs[i] } 0 <= i && i < |xs| ==> xs[i] == 0)
      if (*) {
        if (0 <= i_2 && i_2 < Seq#Length(xs)) {
          assert {:msg "  Contract might not be well-formed. Index xs[i] into xs might be negative. (0283.vpr@9.12--9.62) [203501]"}
            i_2 >= 0;
          assert {:msg "  Contract might not be well-formed. Index xs[i] into xs might exceed sequence length. (0283.vpr@9.12--9.62) [203502]"}
            i_2 < Seq#Length(xs);
        }
        assume false;
      }
