    ...
    // -- Check definedness of (forall i: Int :: { xs[i] } 0 <= i && i <= |xs| ==> xs[i] == 0)
      if (*) {
        if (0 <= i_18 && i_18 <= Seq#Length(xs)) {
          assert {:msg "  Contract might not be well-formed. Index xs[i] into xs might be negative. (0283.vpr@6.12--6.63) [75078]"}
            i_18 >= 0;
          assert {:msg "  Contract might not be well-formed. Index xs[i] into xs might exceed sequence length. (0283.vpr@6.12--6.63) [75079]"}
            i_18 < Seq#Length(xs);
        }
        assume false;
      }
...
procedure test2_1(xs: (Seq int)) returns ()
  modifies Heap, Mask;
{
  var i_3: int;
  var oldMask: MaskType;
    ...
    // -- Check definedness of (forall i: Int :: { xs[i] } 0 <= i && i < |xs| ==> xs[i] == 0)
      if (*) {
        if (0 <= i_3 && i_3 < Seq#Length(xs)) {
          assert {:msg "  Contract might not be well-formed. Index xs[i] into xs might be negative. (0283.vpr@9.12--9.62) [75080]"}
            i_3 >= 0;
          assert {:msg "  Contract might not be well-formed. Index xs[i] into xs might exceed sequence length. (0283.vpr@9.12--9.62) [75081]"}
            i_3 < Seq#Length(xs);
        }
        assume false;
      }
