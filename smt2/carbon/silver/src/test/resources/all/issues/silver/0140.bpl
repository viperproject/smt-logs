...
procedure test_1(b_24: bool) returns ()
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: assert true --* (b ==> true --* true) -- 0140.vpr@6.3--6.40
    ...
    // -- Check definedness of true --* (b ==> true --* true)
      if (*) {
        ...
        if (b_24) {
          ...
          WandDefRHSMask := WandDefRHSMask[null, wand_1(true, true):=WandDefRHSMask[null, wand_1(true, true)] + FullPerm];
          assume state(WandDefRHSHeap, WandDefRHSMask);
        }
        assume state(WandDefRHSHeap, WandDefRHSMask);
        assume false;
    ...
    // permLe
    assert {:msg "  Assert might fail. Magic wand instance not found. (0140.vpr@6.10--6.40) [54612]"}
      FullPerm <= AssertMask[null, wand(true, b_24, true, true)];
    AssertMask := AssertMask[null, wand(true, b_24, true, true):=AssertMask[null, wand(true, b_24, true, true)] - FullPerm];
    assume state(Heap, Mask);
}