...
function  peek_neighbors(Heap: HeapType, graph: (Seq Ref), i: int): bool;
function  peek_neighbors'(Heap: HeapType, graph: (Seq Ref), i: int): bool;
axiom (forall Heap: HeapType, graph: (Seq Ref), i: int ::
  { peek_neighbors(Heap, graph, i) }
  peek_neighbors(Heap, graph, i) == peek_neighbors'(Heap, graph, i) && dummyFunction(peek_neighbors#triggerStateless(graph, i))
);
axiom (forall Heap: HeapType, graph: (Seq Ref), i: int ::
  { peek_neighbors'(Heap, graph, i) }
  dummyFunction(peek_neighbors#triggerStateless(graph, i))
);

// Definitional axiom
axiom (forall Heap: HeapType, Mask: MaskType, graph: (Seq Ref), i: int ::
  { state(Heap, Mask), peek_neighbors(Heap, graph, i) } { state(Heap, Mask), peek_neighbors#triggerStateless(graph, i), Node#trigger(Heap, Node(Seq#Index(graph, i))) }
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> 0 <= i && i < Seq#Length(graph) ==> peek_neighbors(Heap, graph, i)
);

// Framing axioms
function  peek_neighbors#frame(frame: FrameType, graph: (Seq Ref), i: int): bool;
axiom (forall Heap: HeapType, Mask: MaskType, graph: (Seq Ref), i: int ::
  { state(Heap, Mask), peek_neighbors'(Heap, graph, i) }
  state(Heap, Mask) ==> peek_neighbors'(Heap, graph, i) == peek_neighbors#frame(Heap[null, Node(Seq#Index(graph, i))], graph, i)
);

// Trigger function (controlling recursive postconditions)
function  peek_neighbors#trigger(frame: FrameType, graph: (Seq Ref), i: int): bool;

// State-independent trigger function
function  peek_neighbors#triggerStateless(graph: (Seq Ref), i: int): bool;

// Check contract well-formedness and postcondition
procedure peek_neighbors#definedness(graph: (Seq Ref), i: int) returns (Result: bool)
  modifies Heap, Mask;
{
  ...
  // -- Inhaling precondition (with checking)
    ...
    havoc wildcard;
    perm := wildcard;
    Mask := Mask[null, Node(Seq#Index(graph, i)):=Mask[null, Node(Seq#Index(graph, i))] + perm];
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Check definedness of function body
    
    // -- Check definedness of (unfolding acc(Node(graph[i]), wildcard) in true)
      UnfoldingHeap := Heap;
      UnfoldingMask := Mask;
  ...
  modifies Heap, Mask;
{
  ...
  // -- Check definedness of predicate body of Node
    ...
    perm := FullPerm;
    assume self != null;
    Mask := Mask[self, neighbors:=Mask[self, neighbors] + perm];
    assume state(Heap, Mask);
    ...
    assume (forall i_1: int, j_1: int ::
      { Seq#Index(Heap[self, neighbors], i_1), Seq#Index(Heap[self, neighbors], j_1) }
      (0 <= i_1 && i_1 < Seq#Length(Heap[self, neighbors])) && ((0 <= j_1 && j_1 < Seq#Length(Heap[self, neighbors])) && i_1 != j_1) ==> Seq#Index(Heap[self, neighbors], i_1) != Seq#Index(Heap[self, neighbors], j_1)
    );
    assume state(Heap, Mask);
}