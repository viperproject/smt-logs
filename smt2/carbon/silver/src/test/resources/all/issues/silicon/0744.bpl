...
function  fac(Heap: HeapType, i: int): int;
function  fac'(Heap: HeapType, i: int): int;
axiom (forall Heap: HeapType, i: int ::
  { fac(Heap, i) }
  fac(Heap, i) == fac'(Heap, i) && dummyFunction(fac#triggerStateless(i))
);
axiom (forall Heap: HeapType, i: int ::
  { fac'(Heap, i) }
  dummyFunction(fac#triggerStateless(i))
);

// Definitional axiom
axiom (forall Heap: HeapType, Mask: MaskType, i: int ::
  { state(Heap, Mask), fac(Heap, i) }
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> fac(Heap, i) == (if i <= 1 then 1 else i * fac'(Heap, i - 1))
);

// Framing axioms
function  fac#frame(frame: FrameType, i: int): int;
axiom (forall Heap: HeapType, Mask: MaskType, i: int ::
  { state(Heap, Mask), fac'(Heap, i) }
  state(Heap, Mask) ==> fac'(Heap, i) == fac#frame(EmptyFrame, i)
);

// Postcondition axioms
axiom (forall Heap: HeapType, Mask: MaskType, i: int ::
  { state(Heap, Mask), fac'(Heap, i) }
  state(Heap, Mask) && (AssumeFunctionsAbove < 0 || fac#trigger(EmptyFrame, i)) ==> ge(Heap, fac'(Heap, i), 1)
);

// Trigger function (controlling recursive postconditions)
function  fac#trigger(frame: FrameType, i: int): bool;

// State-independent trigger function
function  fac#triggerStateless(i: int): int;

// Check contract well-formedness and postcondition
procedure fac#definedness(i: int) returns (Result: int)
  modifies Heap, Mask;
{
  ...
  // -- Check definedness of function body
    ...
    // -- Check definedness of (i <= 1 ? 1 : i * fac(i - 1))
      if (i <= 1) {
      } else {
        if (*) {
          // Stop execution
          ...
          assume fac#trigger(EmptyFrame, i - 1);
        }
      }
  
  // -- Translate function body
    Result := (if i <= 1 then 1 else i * fac(Heap, i - 1));
  
  // -- Exhaling postcondition (with checking)
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    
    // -- Check definedness of ge(result, 1)
      if (*) {
        // Stop execution
        assume false;
      }
