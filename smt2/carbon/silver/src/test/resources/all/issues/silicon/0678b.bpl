...
// ==================================================
// Translation of method mWand
// ==================================================

procedure mWand(r_1: Ref) returns ()
  modifies Heap, Mask;
{
  ...
  // -- Checked inhaling of precondition
    ...
    // -- Check definedness of acc(r.f, write) --* false
      if (*) {
    ...
    Mask := Mask[null, wand(r_1, FullPerm, false):=Mask[null, wand(r_1, FullPerm, false)] + FullPerm];
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  //     false) >
  //     none) -- 0678b.vpr@10.12--10.83
    
    // -- Check definedness of (exists x: Ref :: { acc(x.f, write) --* false } perm(acc(x.f, write) --* false) > none)
      if (*) {
        if (*) {
          havoc WandDefLHSHeap;
          WandDefLHSMask := ZeroMask;
    ...
    assume (exists x_1: Ref ::
      { Heap[null, wand(x_1, FullPerm, false)] }
      NoPerm < Mask[null, wand(x_1, FullPerm, false)]
    );
    assume state(Heap, Mask);
    assume state(Heap, Mask);
}