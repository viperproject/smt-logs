  ...
  modifies Heap, Mask;
{
  ...
  // -- Check definedness of function body
    ...
    // -- Check definedness of (unfolding acc(cell(x), write) in x.val)
      ...
      assume UnfoldingHeap[null, cell(x)] == FrameFragment(UnfoldingHeap[x, val]);
      ExhaleWellDef0Heap := UnfoldingHeap;
      ExhaleWellDef0Mask := UnfoldingMask;
      perm := FullPerm;
      assert {:msg "  Function might not be well-formed. There might be insufficient permission to access cell(x) (0238.vpr@11.1--15.2) [80559]"}
      ...
      assume state(UnfoldingHeap, UnfoldingMask);
      assert {:msg "  Function might not be well-formed. There might be insufficient permission to access x.val (0238.vpr@11.1--15.2) [80560]"}
  ...
  modifies Heap, Mask;
{
  ...
  // -- Checked inhaling of precondition
    ...
    assume state(Heap, Mask);
    perm := FullPerm;
    Mask := Mask[null, cell(y):=Mask[null, cell(y)] + perm];
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  //         (unfolding acc(cell(w), write) in x.val == w.val)))) -- 0238.vpr@20.13--20.103
    ...
    // -- Check definedness of (unfolding acc(cell(x), write) in (let w == (y) in (unfolding acc(cell(w), write) in x.val == w.val)))
      ...
      perm := FullPerm;
      if (perm != NoPerm) {
        assert {:msg "  Inhale might fail. There might be insufficient permission to access cell(x) (0238.vpr@20.13--20.103) [80561]"}
      ...
      assume cell#trigger(Unfolding1Heap, cell(w_2));
      assume Unfolding1Heap[null, cell(w_2)] == FrameFragment(Unfolding1Heap[w_2, val]);
      ExhaleWellDef0Heap := Unfolding1Heap;
      ExhaleWellDef0Mask := Unfolding1Mask;
      perm := FullPerm;
      if (perm != NoPerm) {
        assert {:msg "  Inhale might fail. There might be insufficient permission to access cell(w) (0238.vpr@20.13--20.103) [80562]"}
          perm <= Unfolding1Mask[null, cell(w_2)];
      }
      Unfolding1Mask := Unfolding1Mask[null, cell(w_2):=Unfolding1Mask[null, cell(w_2)] - perm];
      perm := FullPerm;
      assume w_2 != null;
      Unfolding1Mask := Unfolding1Mask[w_2, val:=Unfolding1Mask[w_2, val] + perm];
      assume state(Unfolding1Heap, Unfolding1Mask);
      assume state(Unfolding1Heap, Unfolding1Mask);
      assert {:msg "  Inhale might fail. There might be insufficient permission to access x.val (0238.vpr@20.13--20.103) [80563]"}
        HasDirectPerm(Unfolding1Mask, x, val);
      assert {:msg "  Inhale might fail. There might be insufficient permission to access w.val (0238.vpr@20.13--20.103) [80564]"}
      ...
      // -- Free assumptions (exp module)
        ...
        assume state(Heap, Mask);
        w_1_1 := y;
        Heap := Heap[null, cell#sm(w_1_1):=Heap[null, cell#sm(w_1_1)][w_1_1, val:=true]];
        assume state(Heap, Mask);
  ...
  // -- Translating statement: inhale (unfolding acc(cell(y), write) in y.val == 3) -- 0238.vpr@21.13--21.44
    ...
    // -- Check definedness of (unfolding acc(cell(y), write) in y.val == 3)
      UnfoldingHeap := Heap;
      UnfoldingMask := Mask;
      assume cell#trigger(UnfoldingHeap, cell(y));
      assume UnfoldingHeap[null, cell(y)] == FrameFragment(UnfoldingHeap[y, val]);
      ExhaleWellDef0Heap := UnfoldingHeap;
      ExhaleWellDef0Mask := UnfoldingMask;
      perm := FullPerm;
      if (perm != NoPerm) {
        assert {:msg "  Inhale might fail. There might be insufficient permission to access cell(y) (0238.vpr@21.13--21.44) [80565]"}
          perm <= UnfoldingMask[null, cell(y)];
      }
      UnfoldingMask := UnfoldingMask[null, cell(y):=UnfoldingMask[null, cell(y)] - perm];
      perm := FullPerm;
      assume y != null;
      UnfoldingMask := UnfoldingMask[y, val:=UnfoldingMask[y, val] + perm];
      assume state(UnfoldingHeap, UnfoldingMask);
      assume state(UnfoldingHeap, UnfoldingMask);
      assert {:msg "  Inhale might fail. There might be insufficient permission to access y.val (0238.vpr@21.13--21.44) [80566]"}
  ...
  // -- Translating statement: exhale acc(cell(y), write) -- 0238.vpr@22.6--22.20
    ...
    perm := FullPerm;
    if (perm != NoPerm) {
      assert {:msg "  Exhale might fail. There might be insufficient permission to access cell(y) (0238.vpr@22.13--22.20) [80568]"}
        perm <= Mask[null, cell(y)];
    }
    Mask := Mask[null, cell(y):=Mask[null, cell(y)] - perm];
    // Finish exhale
    havoc ExhaleHeap;
  ...
  // -- Translating statement: assert get(x) == 3 -- 0238.vpr@23.6--23.24
    ...
    // -- Check definedness of get(x) == 3
      if (*) {
        ...
        perm := FullPerm;
        assert {:msg "  Precondition of function get might not hold. There might be insufficient permission to access cell(x) (0238.vpr@23.13--23.19) [80569]"}
    ...
    assert {:msg "  Assert might fail. Assertion get(x) == 3 might not hold. (0238.vpr@23.13--23.24) [80570]"}
      get(Heap, x) == 3;
    assume state(Heap, Mask);
  ...
  // -- Translating statement: assert get(x) == 3 -- 0238.vpr@25.6--25.24
    ...
    // -- Check definedness of get(x) == 3
      if (*) {
        ...
        ExhaleWellDef1Mask := ExhaleWellDef0Mask;
        perm := FullPerm;
        assert {:msg "  Precondition of function get might not hold. There might be insufficient permission to access cell(x) (0238.vpr@25.13--25.19) [80572]"}
          NoPerm < perm ==> NoPerm < ExhaleWellDef0Mask[null, cell(x)];
        // Finish exhale
        havoc ExhaleHeap;
        assume IdenticalOnKnownLocations(ExhaleWellDef0Heap, ExhaleHeap, ExhaleWellDef0Mask);
    ...
    assert {:msg "  Assert might fail. Assertion get(x) == 3 might not hold. (0238.vpr@25.13--25.24) [80573]"}
      get(Heap, x) == 3;
    assume state(Heap, Mask);
  ...
  // -- Translating statement: assert get(y) == 3 -- 0238.vpr@27.6--27.24
    ...
    // -- Check definedness of get(y) == 3
      if (*) {
        ...
        ExhaleWellDef1Mask := ExhaleWellDef0Mask;
        perm := FullPerm;
        assert {:msg "  Precondition of function get might not hold. There might be insufficient permission to access cell(y) (0238.vpr@27.13--27.19) [80574]"}
          NoPerm < perm ==> NoPerm < ExhaleWellDef0Mask[null, cell(y)];
        // Finish exhale
        havoc ExhaleHeap;
        assume IdenticalOnKnownLocations(ExhaleWellDef0Heap, ExhaleHeap, ExhaleWellDef0Mask);
        ExhaleWellDef0Heap := ExhaleHeap;
    ...
    assert {:msg "  Assert might fail. Assertion get(y) == 3 might not hold. (0238.vpr@27.13--27.24) [80575]"}
      get(Heap, y) == 3;
    assume state(Heap, Mask);
}