  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: inhale (forall x: Ref :: { (x in xs) } (x in xs) ==> acc(x.f, write)) -- triggerNotInBody.vpr@7.5--7.49
  ...
  // -- Translating statement: inhale |ys| >= 1 -- triggerNotInBody.vpr@8.12--8.21
    assume Seq#Length(ys) >= 1;
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  // -- Translating statement: inhale acc(ys[0].f, write) -- triggerNotInBody.vpr@9.5--9.24
    ...
    Mask := Mask[Seq#Index(ys, 0), f_7:=Mask[Seq#Index(ys, 0), f_7] + perm];
    assume state(Heap, Mask);
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Translating statement: inhale (forall x: Ref :: { ys[x.f].f } (x in xs) ==> x != z) -- triggerNotInBody.vpr@10.12--10.59
    
    // -- Check definedness of (forall x: Ref :: { ys[x.f].f } (x in xs) ==> x != z)
      if (*) {
        assume false;
      }
    assume (forall x_3: Ref ::
      { Heap[Seq#Index(ys, Heap[x_3, f_7]), f_7] }
      Seq#Contains(xs, x_3) ==> x_3 != z
    );
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  
  // -- Translating statement: inhale (y in xs) -- triggerNotInBody.vpr@13.12--13.19
