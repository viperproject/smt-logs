  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: as := Seq(x, y) -- testNonQuant.vpr@10.5--10.20
    vas := Seq#Append(Seq#Singleton(x), Seq#Singleton(y));
    assume state(Heap, Mask);
  ...
  // -- Translating statement: inhale (forall a: Ref :: { a.f } (a in as) ==> a.f > 0) -- testNonQuant.vpr@14.5--14.54
    ...
    assume (forall a_3_1: Ref ::
      { Heap[a_3_1, f_7] }
      Seq#Contains(vas, a_3_1) ==> Heap[a_3_1, f_7] > 0
    );
    assume state(Heap, Mask);
    assume state(Heap, Mask);
