  ...
  modifies Heap, Mask;
{
  ...
  // // LoopInfo(None,Set())
  // inhale acc(x.g, 1 / 2) -- jump_out_of_loop.vpr@6.5--6.24
    perm := 1 / 2;
    assert {:msg "  Inhale might fail. Fraction 1 / 2 might be negative. (jump_out_of_loop.vpr@6.12--6.24) [85865]"}
      perm >= NoPerm;
  ...
  // -- Translating statement: while (c > 0) -- jump_out_of_loop.vpr@7.5--17.6
  ...
  // inhale true -- <no position>
          ...
          // -- Translating statement: if (c == 5) -- jump_out_of_loop.vpr@10.9--16.10
            if (c == 5) {
              
              // -- Translating statement: // id = 5
  // inhale acc(x.g, 1 / 3) -- jump_out_of_loop.vpr@11.13--11.33
                perm := 1 / 3;
                assert {:msg "  Inhale might fail. Fraction 1 / 3 might be negative. (jump_out_of_loop.vpr@11.20--11.33) [85866]"}
                  perm >= NoPerm;
                assume perm > NoPerm ==> x != null;
                Mask := Mask[x, g:=Mask[x, g] + perm];
                assume state(Heap, Mask);
  ...
  // // LoopInfo(None,Set(3))
  // goto l -- jump_out_of_loop.vpr@13.13--13.19
  ...
  // c := c - 1 -- jump_out_of_loop.vpr@15.13--15.21
                c := c - 1;
                assume state(Heap, Mask);
            }
            assume state(Heap, Mask);
  ...
  // inhale c == 5 -- jump_out_of_loop.vpr@23.5--23.18
    assume c == 5;
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  // assert perm(x.g) == 1 / 2 + 1 / 3 -- jump_out_of_loop.vpr@25.5--25.32
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    assert {:msg "  Assert might fail. Assertion perm(x.g) == 1 / 2 + 1 / 3 might not hold. (jump_out_of_loop.vpr@25.12--25.32) [85867]"}
      Mask[x, g] == 1 / 2 + 1 / 3;
    assume state(Heap, Mask);
  ...
  // assert false -- jump_out_of_loop.vpr@28.5--28.17
    ExhaleWellDef0Heap := Heap;
    ExhaleWellDef0Mask := Mask;
    assert {:msg "  Assert might fail. Assertion false might not hold. (jump_out_of_loop.vpr@28.12--28.17) [85868]"}
      false;
    assume state(Heap, Mask);
}