  ...
  modifies Heap, Mask;
{
  ...
  // inhale acc(x.g, 1 / 2) -- jump_out_of_loop.vpr@6.5--6.24
    perm := 1 / 2;
    assert {:msg "  Inhale might fail. Fraction 1 / 2 might be negative. (jump_out_of_loop.vpr@6.12--6.24) [141603]"}
      perm >= NoPerm;
  ...
  // inhale acc(x.g, 1 / 3) -- jump_out_of_loop.vpr@11.13--11.33
                perm := 1 / 3;
                assert {:msg "  Inhale might fail. Fraction 1 / 3 might be negative. (jump_out_of_loop.vpr@11.20--11.33) [141604]"}
                  perm >= NoPerm;
                assume perm > NoPerm ==> x != null;
                Mask := Mask[x, g:=Mask[x, g] + perm];
                assume state(Heap, Mask);
