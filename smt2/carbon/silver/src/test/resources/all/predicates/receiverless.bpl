...
function  valid(b_24: bool): Field PredicateType_valid FrameType;
function  valid#sm(b_24: bool): Field PredicateType_valid PMaskType;
axiom (forall b_24: bool ::
  { PredicateMaskField(valid(b_24)) }
  PredicateMaskField(valid(b_24)) == valid#sm(b_24)
);
axiom (forall b_24: bool ::
  { valid(b_24) }
  IsPredicateField(valid(b_24))
);
axiom (forall b_24: bool ::
  { valid(b_24) }
  getPredWandId(valid(b_24)) == 0
);
...
axiom (forall b_24: bool, b2: bool ::
  { valid(b_24), valid(b2) }
  valid(b_24) == valid(b2) ==> b_24 == b2
);
axiom (forall b_24: bool, b2: bool ::
  { valid#sm(b_24), valid#sm(b2) }
  valid#sm(b_24) == valid#sm(b2) ==> b_24 == b2
);

axiom (forall Heap: HeapType, b_24: bool ::
  { valid#trigger(Heap, valid(b_24)) }
  valid#everUsed(valid(b_24))
);

procedure valid#definedness(b_24: bool) returns ()
  modifies Heap, Mask;
{
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: unfold acc(valid(false), write) -- receiverless.vpr@18.5--18.36
    ...
    // -- Update version of predicate
      if (!HasDirectPerm(Mask, null, valid(false))) {
        havoc newVersion;
        Heap := Heap[null, valid(false):=newVersion];
      }
    assume false;
    assume state(Heap, Mask);
    assume state(Heap, Mask);
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: unfold acc(valid(true), write) -- receiverless.vpr@25.5--25.35
    ...
    // -- Update version of predicate
      if (!HasDirectPerm(Mask, null, valid(true))) {
        havoc newVersion;
        Heap := Heap[null, valid(true):=newVersion];
      }
    assume state(Heap, Mask);
    assume state(Heap, Mask);
