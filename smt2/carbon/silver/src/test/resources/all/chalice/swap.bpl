...
const unique F: Field NormalField int;
axiom !IsPredicateField(F);
axiom !IsWandField(F);
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: x := a -- swap.vpr@7.5--7.11
    x := a_2;
    assume state(Heap, Mask);
  
  // -- Translating statement: y := b -- swap.vpr@8.5--8.11
    y := b_24;
    assume state(Heap, Mask);
  ...
  modifies Heap, Mask;
{
  ...
  if (*) {
    ...
    perm := FullPerm;
    assume this != null;
    PostMask := PostMask[this, F:=PostMask[this, F] + perm];
    assume state(PostHeap, PostMask);
    perm := FullPerm;
    assume this != null;
    PostMask := PostMask[this, G:=PostMask[this, G] + perm];
    assume state(PostHeap, PostMask);
    assume state(PostHeap, PostMask);
