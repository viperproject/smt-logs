...
// Framing axioms
function  foo#frame(frame: FrameType, this: Ref): Ref;
axiom (forall Heap: HeapType, Mask: MaskType, this: Ref ::
  { state(Heap, Mask), foo'(Heap, this) }
  state(Heap, Mask) ==> foo'(Heap, this) == foo#frame(EmptyFrame, this)
);
...
procedure foo#definedness(this: Ref) returns (Result: Ref)
  modifies Heap, Mask;
{
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: c := (b ? null : foo(this)) -- internal-bug-6.vpr@8.5--8.41
    
    // -- Check definedness of (b ? null : foo(this))
      if (b_24) {
      } else {
        if (*) {
          // Stop execution
