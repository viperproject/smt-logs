  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: this.x := this.x + 1 -- internal-bug-2.vpr@12.3--12.23
    ...
    // -- Check definedness of this.x + 1
      assert {:msg "  Assignment might fail. There might be insufficient permission to access this.x (internal-bug-2.vpr@12.3--12.23) [147564]"}
        HasDirectPerm(Mask, this, x_36);
    assert {:msg "  Assignment might fail. There might be insufficient permission to access this.x (internal-bug-2.vpr@12.3--12.23) [147565]"}
  ...
  //   old((unfolding acc(inv(this), write) in this.x)) -- internal-bug-2.vpr@14.3--14.111
    ...
    // -- Check definedness of (unfolding acc(inv(this), write) in this.x) == old((unfolding acc(inv(this), write) in this.x))
      ...
      assume state(UnfoldingHeap, UnfoldingMask);
      assert {:msg "  Assert might fail. There might be insufficient permission to access this.x (internal-bug-2.vpr@14.11--14.110) [147567]"}
        HasDirectPerm(UnfoldingMask, this, x_36);
      
      // -- Free assumptions (exp module)
        Heap := Heap[null, inv#sm(this):=Heap[null, inv#sm(this)][this, x_36:=true]];
        assume state(Heap, Mask);
      ...
      assume state(UnfoldingHeap, UnfoldingMask);
      assume state(UnfoldingHeap, UnfoldingMask);
      assert {:msg "  Assert might fail. There might be insufficient permission to access this.x (internal-bug-2.vpr@14.11--14.110) [147569]"}
        HasDirectPerm(UnfoldingMask, this, x_36);
      
      // -- Free assumptions (exp module)
        Heap := Heap[null, inv#sm(this):=Heap[null, inv#sm(this)][this, x_36:=true]];
        assume state(Heap, Mask);
    assert {:msg "  Assert might fail. Assertion (unfolding acc(inv(this), write) in this.x) == old((unfolding acc(inv(this), write) in this.x)) might not hold. (internal-bug-2.vpr@14.11--14.110) [147570]"}
