...
// ==================================================
// Translation of method koko
// ==================================================

procedure koko(this: Ref) returns ()
  modifies Heap, Mask;
{
  var perm: Perm;
  ...
  // -- Translating statement: this.x := this.x + 1 -- internal-bug-2.vpr@12.3--12.23
    ...
    // -- Check definedness of this.x + 1
      assert {:msg "  Assignment might fail. There might be insufficient permission to access this.x (internal-bug-2.vpr@12.3--12.23) [93096]"}
        HasDirectPerm(Mask, this, x_42);
    assert {:msg "  Assignment might fail. There might be insufficient permission to access this.x (internal-bug-2.vpr@12.3--12.23) [93097]"}
  ...
  //   old((unfolding acc(inv(this), write) in this.x)) -- internal-bug-2.vpr@14.3--14.111
    ...
    // -- Check definedness of (unfolding acc(inv(this), write) in this.x) == old((unfolding acc(inv(this), write) in this.x))
      ...
      assume UnfoldingHeap[null, inv(this)] == FrameFragment(UnfoldingHeap[this, x_42]);
      ExhaleWellDef1Heap := UnfoldingHeap;
      ExhaleWellDef1Mask := UnfoldingMask;
      perm := FullPerm;
      if (perm != NoPerm) {
        assert {:msg "  Assert might fail. There might be insufficient permission to access inv(this) (internal-bug-2.vpr@14.11--14.110) [93098]"}
      ...
      assume state(UnfoldingHeap, UnfoldingMask);
      assert {:msg "  Assert might fail. There might be insufficient permission to access this.x (internal-bug-2.vpr@14.11--14.110) [93099]"}
        HasDirectPerm(UnfoldingMask, this, x_42);
      
      // -- Free assumptions (exp module)
        Heap := Heap[null, inv#sm(this):=Heap[null, inv#sm(this)][this, x_42:=true]];
        assume state(Heap, Mask);
      ...
      perm := FullPerm;
      if (perm != NoPerm) {
        assert {:msg "  Assert might fail. There might be insufficient permission to access inv(this) (internal-bug-2.vpr@14.11--14.110) [93100]"}
          perm <= UnfoldingMask[null, inv(this)];
      }
      UnfoldingMask := UnfoldingMask[null, inv(this):=UnfoldingMask[null, inv(this)] - perm];
      perm := FullPerm;
      ...
      assume state(UnfoldingHeap, UnfoldingMask);
      assume state(UnfoldingHeap, UnfoldingMask);
      assert {:msg "  Assert might fail. There might be insufficient permission to access this.x (internal-bug-2.vpr@14.11--14.110) [93101]"}
        HasDirectPerm(UnfoldingMask, this, x_42);
      
      // -- Free assumptions (exp module)
        Heap := Heap[null, inv#sm(this):=Heap[null, inv#sm(this)][this, x_42:=true]];
        assume state(Heap, Mask);
    assert {:msg "  Assert might fail. Assertion (unfolding acc(inv(this), write) in this.x) == old((unfolding acc(inv(this), write) in this.x)) might not hold. (internal-bug-2.vpr@14.11--14.110) [93102]"}
