...
function  get(Heap: HeapType, this: Ref, index: int): int;
function  get'(Heap: HeapType, this: Ref, index: int): int;
axiom (forall Heap: HeapType, this: Ref, index: int ::
  { get(Heap, this, index) }
  get(Heap, this, index) == get'(Heap, this, index) && dummyFunction(get#triggerStateless(this, index))
);
axiom (forall Heap: HeapType, this: Ref, index: int ::
  { get'(Heap, this, index) }
  dummyFunction(get#triggerStateless(this, index))
);

// Definitional axiom
axiom (forall Heap: HeapType, Mask: MaskType, this: Ref, index: int ::
  { state(Heap, Mask), get(Heap, this, index) } { state(Heap, Mask), get#triggerStateless(this, index), valid#trigger(Heap, valid(this)), valid#trigger(Heap, valid(this)) }
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> get(Heap, this, index) == 0
);

// Framing axioms
function  get#frame(frame: FrameType, this: Ref, index: int): int;
axiom (forall Heap: HeapType, Mask: MaskType, this: Ref, index: int ::
  { state(Heap, Mask), get'(Heap, this, index) }
  state(Heap, Mask) ==> get'(Heap, this, index) == get#frame(Heap[null, valid(this)], this, index)
);

// Trigger function (controlling recursive postconditions)
function  get#trigger(frame: FrameType, this: Ref, index: int): bool;

// State-independent trigger function
function  get#triggerStateless(this: Ref, index: int): int;

// Check contract well-formedness and postcondition
procedure get#definedness(this: Ref, index: int) returns (Result: int)
  modifies Heap, Mask;
{
  var perm: Perm;
  ...
  // -- Inhaling precondition (with checking)
    ...
    // -- Check definedness of (unfolding acc(valid(this), write) in true)
      UnfoldingHeap := Heap;
      UnfoldingMask := Mask;
      assume valid#trigger(UnfoldingHeap, valid(this));
      assume UnfoldingHeap[null, valid(this)] == CombineFrames(FrameFragment(UnfoldingHeap[this, elem_1]), CombineFrames(FrameFragment(UnfoldingHeap[this, next]), FrameFragment((if UnfoldingHeap[this, next] != null then UnfoldingHeap[null, valid(UnfoldingHeap[this, next])] else EmptyFrame))));
      ExhaleWellDef0Mask := UnfoldingMask;
      ExhaleWellDef0Heap := UnfoldingHeap;
      perm := FullPerm;
      assert {:msg "  Contract might not be well-formed. There might be insufficient permission to access valid(this) (internal-bug-1.vpr@9.15--9.56) [143259]"}
        NoPerm < perm ==> NoPerm < UnfoldingMask[null, valid(this)];
      perm := FullPerm;
      assume this != null;
      UnfoldingMask := UnfoldingMask[this, elem_1:=UnfoldingMask[this, elem_1] + perm];
      ...
      // -- Free assumptions (exp module)
        Heap := Heap[null, valid#sm(this):=Heap[null, valid#sm(this)][this, elem_1:=true]];
  ...
  modifies Heap, Mask;
{
  ...
  // -- Check definedness of predicate body of valid
    ...
    // -- Check definedness of this.next != null
      assert {:msg "  Predicate might not be well-formed. There might be insufficient permission to access this.next (internal-bug-1.vpr@6.3--6.142) [143260]"}
        HasDirectPerm(Mask, this, next);
    if (Heap[this, next] != null) {
      
      // -- Check definedness of acc(valid(this.next), write)
        assert {:msg "  Predicate might not be well-formed. There might be insufficient permission to access this.next (internal-bug-1.vpr@6.3--6.142) [143261]"}
          HasDirectPerm(Mask, this, next);
      perm := FullPerm;
