...
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  { state(Heap, Mask), fun01(Heap, x) } { state(Heap, Mask), fun01#triggerStateless(x), P#trigger(Heap, P(x)) }
  state(Heap, Mask) && AssumeFunctionsAbove < 1 ==> fun01(Heap, x) == Heap[x, g]
);
...
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  ...
  state(Heap, Mask) ==> fun01'(Heap, x) == fun01#frame(Heap[null, P(x)], x)
);

// Postcondition axioms
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  { state(Heap, Mask), fun01'(Heap, x) }
  state(Heap, Mask) && (AssumeFunctionsAbove < 1 || fun01#trigger(Heap[null, P(x)], x)) ==> fun01'(Heap, x) > 0
);
...
// Uninterpreted function definitions
function  fun02(Heap: HeapType, x: Ref): int;
function  fun02'(Heap: HeapType, x: Ref): int;
axiom (forall Heap: HeapType, x: Ref ::
  { fun02(Heap, x) }
...
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  ...
  state(Heap, Mask) && AssumeFunctionsAbove < 0 ==> fun02(Heap, x) == Heap[x, g] + Heap[x, g]
);

// Framing axioms
function  fun02#frame(frame: FrameType, x: Ref): int;
axiom (forall Heap: HeapType, Mask: MaskType, x: Ref ::
  { state(Heap, Mask), fun02'(Heap, x) }
  state(Heap, Mask) ==> fun02'(Heap, x) == fun02#frame(FrameFragment(Heap[x, g]), x)
);
...
// State-independent trigger function
function  fun02#triggerStateless(x: Ref): int;

// Check contract well-formedness and postcondition
procedure fun02#definedness(x: Ref) returns (Result: int)
  modifies Heap, Mask;
{
  var perm: Perm;
  var y_1: int;
