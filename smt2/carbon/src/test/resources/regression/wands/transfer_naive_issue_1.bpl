  ...
  modifies Heap, Mask;
{
  ...
  // } -- transfer_naive_issue_1.vpr@8.1--8.54
    havoc Ops_1Heap;
    Ops_1Mask := ZeroMask;
    ...
    // -- Transfer of acc(x.f.f, write)
      ...
      // -- Creating state which is the sum of the two previously built up states
        ...
        b_4 := b_4 && state(Result_1Heap, Result_1Mask);
      b_1_1 := b_1_1 && b_4;
    Mask := Mask[null, wand(x, FullPerm, x, FullPerm, x, FullPerm, x, FullPerm):=Mask[null, wand(x, FullPerm, x, FullPerm, x, FullPerm, x, FullPerm)] + FullPerm];
    assume state(Heap, Mask);
    assume state(Heap, Mask);
    assume state(Heap, Mask);
}