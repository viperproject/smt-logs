...
// 
// Date:         2025-01-07 13:52:28
// Tool:         carbon 1.0
// Arguments: :  --disableCaching --boogieExe /home/runner/.dotnet/tools/boogie --timeout 10 --print /home/runner/work/smt-logs/smt-logs/carbon/../smt2/carbon/src/test/resources/regression/wands/wand_shapes_1.bpl --boogieOpt /proverLog:/home/runner/work/smt-logs/smt-logs/carbon/../smt2/carbon/src/test/resources/regression/wands/wand_shapes_1-@PROC@.smt2 --ignoreFile dummy-file-to-prevent-cli-parser-from-complaining-about-missing-file-name.silver
// Dependencies:
  ...
  modifies Heap, Mask;
{
      ...
      if (b_1_1) {
        if (b_1_1) {
          ...
          // -- Check definedness of acc(p.f.g, write)
            assert {:msg "  Packaging wand might fail. There might be insufficient permission to access p.f (wand_shapes_1.vpr@10.3--10.42) [598]"}
  ...
  // -- Translating statement: exhale acc(p.f, write) -- wand_shapes_1.vpr@11.3--11.18
    ...
    perm := FullPerm;
    if (perm != NoPerm) {
      assert {:msg "  Exhale might fail. There might be insufficient permission to access p.f (wand_shapes_1.vpr@11.10--11.18) [601]"}
        perm <= Mask[p_1, f_7];
    }
    Mask := Mask[p_1, f_7:=Mask[p_1, f_7] - perm];
    // Finish exhale
  ...
  // -- Translating statement: exhale acc(r.f, write) && acc(r.f.g, write) --* true -- wand_shapes_1.vpr@13.3--13.41
    ...
    // -- Check definedness of acc(r.f, write) && acc(r.f.g, write) --* true
      if (*) {
        ...
        // -- Check definedness of acc(r.f.g, write)
          assert {:msg "  Exhale might fail. There might be insufficient permission to access r.f (wand_shapes_1.vpr@13.11--13.40) [602]"}
    ...
    // permLe
    assert {:msg "  Exhale might fail. Magic wand instance not found. (wand_shapes_1.vpr@13.11--13.40) [603]"}
  ...
  // -- Translating statement: assert false -- wand_shapes_1.vpr@16.3--16.15
    ExhaleWellDef0Mask := Mask;
    ExhaleWellDef0Heap := Heap;
    assert {:msg "  Assert might fail. Assertion false might not hold. (wand_shapes_1.vpr@16.10--16.15) [604]"}
      false;
    assume state(Heap, Mask);
}
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: exhale y == 2 && x == 2 --* true -- wand_shapes_1.vpr@25.3--25.34
    ...
    // permLe
    assert {:msg "  Exhale might fail. Magic wand instance not found. (wand_shapes_1.vpr@25.10--25.34) [606]"}
  ...
  // -- Translating statement: assert false -- wand_shapes_1.vpr@28.3--28.15
    ExhaleWellDef0Mask := Mask;
    ExhaleWellDef0Heap := Heap;
    assert {:msg "  Assert might fail. Assertion false might not hold. (wand_shapes_1.vpr@28.10--28.15) [607]"}
      false;
    assume state(Heap, Mask);
}
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: exhale y == 2 && x == 2 --* true -- wand_shapes_1.vpr@39.3--39.34
    ...
    // permLe
    assert {:msg "  Exhale might fail. Magic wand instance not found. (wand_shapes_1.vpr@39.10--39.34) [609]"}
      FullPerm <= Mask[null, wand_1(y == 2 && x == 2, true)];
    Mask := Mask[null, wand_1(y == 2 && x == 2, true):=Mask[null, wand_1(y == 2 && x == 2, true)] - FullPerm];
    // Finish exhale
    havoc ExhaleHeap;
  ...
  modifies Heap, Mask;
{
  ...
  // -- Translating statement: exhale acc(q.f, write) && acc(p.f, write) --* true -- wand_shapes_1.vpr@46.3--46.37
    ...
    // permLe
    assert {:msg "  Exhale might fail. Magic wand instance not found. (wand_shapes_1.vpr@46.10--46.37) [611]"}
